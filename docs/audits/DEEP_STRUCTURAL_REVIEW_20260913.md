...
| ٣ | **الأمان بنيويّاً صحيح، وتوزيعُه غيرُ موحَّد**: RS256 إلزاميّ في الإنتاج، لكنّ التحقّق من JWT مكتوبٌ في ١٦ ملفّاً | `services/auth/main.py:67,159-162` · فكُّ JWT في **١٦** ملفّاً خارج `shared/security/` (بأسماء مستعارة) بينما ١٢ خدمة تستورد `shared.security` — كاشفٌ AST في `test_jwt_decode_outside_shared_security_ratchet.py` |
...
- أرقامُ الواجهة من `grep` لا من رسمِ استدعاءات — **والرقمُ الأوّل (٦٢) كان خطأً** صُحِّح إلى ٨ مواضع/٣ ملفّات (§٥)؛ يبقى هنا تاريخاً للخطأ لا قياساً.
