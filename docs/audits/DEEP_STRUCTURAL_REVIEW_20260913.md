...
| ٦ | **الواجهة تحمل نفسَ مرض المنصّة**: ثلاثة ملفّات بأكثر من ٣٬٠٠٠ سطر و٦٢ موضع `fetch` تتجاوز طبقة الـAPI | `services/api.ts` 3,782 · `hooks/useApi.ts` 3,373 · `sections/MapHub.tsx` 3,234 · `AddFieldWithMap.tsx` 1,810 |
...
- لكنّ **`jwt.decode(` يظهر في ١٥ ملفّاً خارجه** (`actuator_runtime` · `guardrails-engine/main` · `local-ai-rag/main` · `mcp_servers` ×٢ · `odoo-bridge/main` · `supervisor-agent/main` · `tts-service/main` · `vegetation_runtime` · `video-processor/main` · وثلاثةٌ في المنصّة). بعضُها يستورد `shared.security` **ويفكّ التوكن بنفسه أيضاً** — ازدواجٌ لا غياب. كلُّ موضعٍ منها سطحٌ يمكن أن ينحرف عن سياسة المفاتيح المركزيّة (`jwt_key_validation`) بلا أن يحمرّ شيء.
...
- **بلا فحص صحّة (٨):** `minio-init` · `migrate` · `qdrant-seed` · `zlmediakit-config` (مهامُّ مرّةٍ واحدة، مقبول) — و**`sahool-qdrant`** · `model-lifecycle-adapter` · `irrigation-reservation-lifecycle-worker` · `reservation-dispatch-relay-worker` (خدماتٌ دائمة، غيرُ مقبول: `depends_on: condition: service_healthy` لا يستطيع انتظارها).
...
- **العطل البنيويّ:** `services/api.ts` **3,782** سطراً بـ١٤٧ تصديراً · `hooks/useApi.ts` **3,373** بـ٢٠٦ خطّافات · `MapHub.tsx` **3,234** · `AddFieldWithMap.tsx` 1,810 · `SettingsPage.tsx` 1,044. و**٦٢ ملفّاً يستدعي `fetch(` مباشرةً** خارج `services/api.ts` — أي أنّ طبقةَ الـAPI الموحّدة **ليست حدّاً**، وأيُّ تغييرٍ في ترويسة مصادقةٍ أو مسارٍ (كما في #993) يحتاج مسحاً لا تعديلاً في مكانٍ واحد. `MAPHUB-WEBGL-VISUAL-DEBT-01` مسجَّلةٌ ومحروسة.
...
| **P1** | خطّةُ استخراج من `sahool-platform` — تبدأ بـ`routers/fields.py` و`weather.py` وحارسِ حجمٍ للراوترات | ٦٤٪ تركّز · انحدارٌ انتقل حيث لا قياس | قرارُ الحدّ (سطور/ملفّ) · خطّة كخطّة raster |
| **P1** | توحيدُ فكّ JWT على `shared/security` وإزالةُ الـ١٥ موضعاً | كلُّ موضعٍ سطحُ انحرافٍ عن سياسة المفاتيح | حارسٌ ساكن «لا `jwt.decode` خارج shared» + رقعة لكلّ خدمة |
| **P1** | اسمُ GUC واحد (`app.current_tenant`) وترحيلٌ للـ٦٩+٦ سياسةً الأخرى | نفسُ القاعدة، منطقان للعزل | ترحيلٌ + خفضُ الأساس المجمَّد |
| **P1** | توحيدُ إصدارات FastAPI/Pydantic/httpx وإدخالُ الـ١٧ ملفّاً في `pip-audit` | التشعّبُ مقيسُ الكلفة (٣٢ حالة حمراء بيئيّاً) | قرارُ الإصدار الهدف · ترقياتٌ مفحوصة |
...
| **P2** | حدودُ ذاكرة على `raster-tiler-service` وفحصُ صحّة على `qdrant` والعاملَين الدائمَين | `depends_on` لا يستطيع انتظارها | تعديلُ compose |
| **P2** | طبقةُ API في الواجهة حدّاً: حارسُ «لا `fetch(` خارج `services/api.ts`» + تقسيمُ الملفّات الثلاثة | ٦٢ موضعاً تتجاوز الطبقة | قرارُ الحدّ + تقسيمٌ تدريجيّ |
