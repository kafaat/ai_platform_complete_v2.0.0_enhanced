...
> **تحديث تصحيحي:** هذه الوثيقة قياس تاريخي؛ صُححت الاستنتاجات غير الدقيقة بعد مراجعة المصدر. [المراجعة المكملة](DEEP_REVIEW_SUPPLEMENT_20260913.md) تعرض 14 تصحيحاً و16 نتيجة إضافية، و[سجل التصحيحات](PATCH_CLOSURE_20260913.md) يحدد ما نُفذ وما بقي بلا دليل حي.
...
| ٤ | **ثلاثة أسماء GUC للمستأجر تحتاج توحيداً مدروساً** | `app.current_tenant` و`app.tenant_id` و`app.current_tenant_id`؛ أمّا `app.current_role` و`app.current_user_id` فللدور والمستخدم، وليسا اسمين آخرين للمستأجر |
| ٥ | **الحوكمة أكبر من المنتج**: ٥٨ workflow · ٧٨ تعريف وظيفة؛ لا تعمل كلها بالضرورة على كل PR · ٢٧٢ حارساً يحجب — **٤٩ فقط مُثبَتة بالتكذيب** | `GUARD_CATALOGUE.md` (مولَّد): «٢٢٣ حارساً يحجب الدمج ولم يُثبَت قطّ أنّه يفشل حين يوجد العطل» |
...
| ٨ | **النشرُ قانونيٌّ واحد ومحاطٌ بأربعة عشر بديلاً**: `docker-compose.v9.yml` (٦٨ خدمة) وحدَه محروسٌ من الانحراف | ١٥ ملفّ compose · `env_compose_drift_guard` يقرأ v9 فقط · `unified`/`light` بأسماء خدمات **غير متوافقة** (`auth-service` مقابل `sahool-auth`) · Helm: ٦ workloads؛ راجع التصحيح C09 |
...
| ١٠ | **الأثر الفيزيائي يحتاج دليلاً مستقلاً**: مصادقة NATS موجودة في Compose؛ أوامر الجهاز الفعلية تسلك MQTT | مشكلة ربط توقيع الأمر بالجهاز والحمولة وإعادة التشغيل في SUP-08، محجوبة بـGATE-01؛ القياسات الزراعية وشهادة الإنتاج تبقيان غير مثبتتين |
...
- **صفر ملفات اختبار داخل الخدمة في الجرد المحلي** لا يساوي غياب الاختبارات المركزية؛ توجد تغطية لهذه الخدمات تحت `tests_v9` و`tests/`. راجع C05 وحدود كل مجموعة في المراجعة المكملة.
- **خمسة مرشحين في رسم تبعيات محدود** لا يثبتون غياب التشغيل: توجد إحالات إعداد أو HTTP أو تشغيل لا يلتقطها رسم الاستيراد. راجع C06؛ لا حذف تلقائي للخدمات بناء على هذا التصنيف.
...
- **الأطوار shared/phase9..12 لها مستوردون إنتاجيون في API**، إلى جانب اختبارات وحداتها؛ وصفها بأنها بلا مسار تشغيل كان استنتاجاً غير صحيح من جرد محدود. هذا الربط في المصدر لا يثبت استخدامها في نشر حي.
- ملفّاتُ اختبار (`shared/test_*.py`) على جذر `shared/` بجوار الشيفرة لا في `tests/`.

### ٢.٥ الشيفرة الميّتة والتكرار (مولَّد، ساكن)
629 مرشحاً و60 مجموعة تكرار في القياس التاريخي؛ النتيجة ترشيح ساكن. معالجات `irrigation.real` مسجلة عبر decorators، ووسيط التربة مسجل في التطبيق؛ لا يجوز حذفها بوصفها غير موصولة. `_apply_tenant_guc` يحتاج فحصاً مستقلاً ولا يثبت أن الوسيط المسجل ميت. راجع C02.
...
- **ملكيّةُ الكتابة:** `OWNERSHIP-CONTRACT-DECLARED-BUT-NEVER-MEASURED-01` (open) — ٧٣ موضع كتابة في الأساس المجمد تحتاج حكماً لكل مالك وفق عقد الملكية؛ ليست كلها حكماً مثبتاً بكتابة غير مشروعة موزّعةً على عشر خدمات؛ الحارسُ `db_writer_ownership_guard` يجمّد الأساس ولا يُقلّصه.
...
### ٤.٣ Kubernetes/Helm
القياس الصحيح قبل الإصلاح: ٦ workloads و٨ قوالب Helm. أصلحت الشريحة المكملة خمسة منافذ، وأسماء متغيرات المصادقة، والتخزين الدائم وملفات الكتابة المؤقتة. اجتاز القالب lint/render محلياً؛ لم يُنشر على Kubernetes ولم تُثبت جاهزية جميع خدمات Compose من خلاله.

### ٤.٤ المراقبة
قبل الإصلاح كانت إعدادات OTel موجودة دون خدمة collector في Compose v9، وكانت أهداف exporters بلا خدمات مقابلة. أضافت الشريحة المكملة الحاويات والربط والتتبع للمنصة والإشعارات؛ صحة المسار الحي واللوحات تبقى بحاجة إلى قياس.
...
- **Mobile (Flutter):** 48 ملفّاً في `lib/` · **12 ملف اختبار** · Dart ≥3.6 · CI يشغّل `flutter analyze` + test ✓. رفيعٌ مقارنةً بالواجهة، والفجوةُ `C4/M1` في تقرير الإنتاج (WebSocket · push · timeline) ما تزال «تتطلّب بيئة Flutter». راجع SUP-14 وسجل إغلاق التصحيحات لحالة المشاريع الأصلية وقفل الحزم.

---

## ٦ · الحوكمة وCI — الحجم مقابل الإثبات

- **الجرد المحلي وجد 58 workflow و78 تعريف وظيفة؛ لا يعني ذلك تشغيلها كلها على كل PR.** عقد الحالات المطلوبة يحوي 15 سياقاً محلياً. فروع الأحداث والفلاتر والـRuleset الحي تحدد ما يعمل وما يحجب فعلياً؛ لم يُقَس الـRuleset الحي في هذه المراجعة (C14).
...
- **تصحيح:** مصادقة NATS موجودة وممررة في Compose. اسم الفجوة التاريخي لا يثبت غيابها من المصدر الحالي. MQTT هو مسار أوامر الجهاز؛ ربط توقيع الأمر والسياق وذاكرة replay ومصادقة الإيصال هي SUP-08، وتحتاج شريحة مصرحاً بها ودليلاً على الجهاز.
...
|---|---|---|---|
| **P0** | ربط أمر MQTT والإيصال بالجهاز والمستأجر والحمولة ومنع إعادة التنفيذ (SUP-08) | حد فيزيائي محكوم | patch مراجعة منفصل، تفويض GATE-01 مطابق، ثم اختبارات جهاز وناقل حية |
