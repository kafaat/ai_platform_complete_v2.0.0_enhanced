...
- activation-safety-contract: 16 match(es)
  - `services/agriai-engine/main.py` ← `AGRIAI_PRODUCTION_MODE`
  - `services/agriai-engine/wofost_adapter.py` ← `AGRIAI_PRODUCTION_MODE`
  - `services/agriai-engine/wofost_adapter.py` ← `SIM_PCSE_ENABLED`
  - `services/agriai-engine/simulation_capability.py` ← `SIM_PCSE_ENABLED`
  - `services/agriai-engine/simulation_capability.py` ← `uncalibrated_pending_golden`
...
- job-contract: 2 match(es)
  - `services/gis-workflow-service/tests/test_run_bundle.py` ← `run_bundle`
  - `.github/workflows/gis-workflow-service-gates.yml` ← `gis-workflow-service`

### `remote-sensing-workspace-bff` — pass
classification: `ui-bff`
- ui-and-gateway-consumer: 6 match(es)
  - `frontend/src/services/api/remoteSensingWorkspace.ts` ← `getRemoteSensingWorkspaceOverview`
  - `frontend/src/services/api/remoteSensingWorkspace.ts` ← `/api/remote-sensing-workspace/`
  - `frontend/src/sections/FieldWorkspaceImageryPanel.tsx` ← `getRemoteSensingWorkspaceOverview`
  - `frontend/nginx.conf` ← `/api/remote-sensing-workspace/`
  - `nginx/nginx.v9.conf` ← `/api/remote-sensing-workspace/`
