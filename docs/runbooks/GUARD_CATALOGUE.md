...
- استجابةُ خطأٍ من GitHub كائنٌ (`{"message": "Not Found"}`) لا مصفوفة؛ قبولُها يجعل التكرار يمرّ على **مفاتيحها** نصوصاً فتُقرأ «لا قواعد» بدل «لم يُقرأ». استجابةٌ لا تُفهَم فشلٌ لا قبول — يُسقِط `test_a_non_object_response_is_rejected`
