...
- إجماليّ الطفرات المُسجَّلة: **416**
- وطفراتٌ **سلوكيّة** تُزرَع في منطق الإنتاج نفسه: **349** على 124 مصدراً
...
### `brain_annotated_supersession_guard.py`

**يفرض:** مفرداتُ وسم السجلّ إعلانٌ — هذا الحارسُ يجعلها **مقيسة**.

**يحجب في:** `no-report-only-change.yml` → `no-report-only-change`

**الاختبار الشاهد:** `tests_v9/test_brain_annotated_supersession_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- نزعُ البند ② كلِّه ⇒ يعود المسارُ الذي فتح الفجوة أخضرَ: تُنسَخ فجوةٌ `open`، تُوسَم النسخةُ `fixed` بـ`current` والأصلُ `historical`، فتختفي من كلّ عدٍّ يُقرأ بلا حارسٍ يحمرّ — يُسقِط `test_an_annotated_closure_without_a_cited_witness_is_reported`
- إسقاطُ شرطِ الاتّجاه ⇒ يُطالَب مدخلٌ `current` حالتُه `open` بشاهدِ إغلاق، فتصير إعادةُ فتحِ فجوةٍ وُسِمت مرّةً **مستحيلة**. هذا صنفُ «حارسٍ لا يقبل حالةً صادقة» المُغلَق في #1026 — لا إيجابيٌّ كاذب ولا سلبيّ، بل استحالة — يُسقِط `test_a_reopening_is_never_asked_for_a_closure_witness`
- نزعُ فحص الوجود ⇒ اسمٌ ما زال مذكوراً في workflow لحارسٍ حُذِف ملفُّه يُقبَل ثمناً لإغلاق. وهذه الطفرةُ **نجت أوّلَ قياس**: ذراعُ الاختبارات كانت مُغطّاةً صدفةً عبر `except OSError`، وذراعُ الحرّاس غيرَ مُغطّاةٍ بتاتاً — فأُضيف الشاهدُ ولم تُستبدَل الطفرةُ بألطفَ منها — يُسقِط `test_a_wired_guard_that_is_gone_from_the_tree_is_not_a_witness`
- قبولُ ملفٍّ باسمٍ صحيحٍ بلا دالّةِ اختبار ⇒ ثمنُ الإغلاق يصير `touch`، أي صفراً بخطوةٍ إضافيّة — يُسقِط `test_an_empty_test_file_is_not_a_witness`
- نزعُ البند ① ⇒ معرِّفٌ واحدٌ بحالتين بلا وسمِ تاريخ يمرّ. و`gap-registry-report` تحمل `continue-on-error` فلا تحجب، أي أنّ نزعَه يترك التناقضَ بلا حاجبٍ واحد في الشجرة — يُسقِط `test_an_unannotated_contradiction_is_reported`
- إعادةُ تبعيّةٍ لا تُثبِّتها وظيفةُ `no-report-only-change` — حرّاسُ الدماغ الخمسةُ فيها بايثون قياسيّةٌ صرفة. هذا هو العطلُ الذي وقع فعلاً على أوّل تشغيلٍ لهذا الحارس في CI (`ModuleNotFoundError: No module named 'yaml'` عبر `guard_catalogue`)، بينما مرّ `preflight --fast` أخضرَ لأنّ `PyYAML` مُثبَّتةٌ محلّيّاً: أخضرُ قِيس في كونٍ غيرِ المنشور — يُسقِط `test_the_guard_runs_where_its_job_runs_without_any_dependency`
...
- إسقاطُ الإلزام ⇒ يعود الحارس إلى فحص HEAD وحده بلا `--base` ويطبع ok — GUARD-RUN-WITHOUT-THE-ARGUMENTS-CI-PASSES-01 نفسها: خضرةٌ عن سؤالٍ لم يُطرَح. الاختبارُ يستدعي السكربت بلا وسائط عبر subprocess ويطلب رمز 2 وذكرَ الراية، فلا skipif ولا حاجة إلى تاريخ. — يُسقِط `test_omitting_base_is_a_usage_error_not_a_green_measurement`
...
- إسقاطُ قيد النطاق ⇒ يصير استثناءُ إزالة الصفوف المكرّرة إعفاءً ملفّيّاً عاماً، فتُمحى بايتاتٌ من `log`/`hot` أو من سجلٍّ شبيه بلا أن يحجب الحارس — يُسقِط `test_lossless_row_deduplication_does_not_exempt_other_files`
- تركُ المعرّف يحكم وحده بلا بايتات الصفّ كاملةً ⇒ صفّان بالهوية نفسها لكن بدليلٍ أو حالةٍ أو مسافاتٍ مختلفة يُقرأان «نسخةً زائدة»، فيُفقد أحد النصّين المتعارضين صامتاً — يُسقِط `test_lossless_row_deduplication_rejects_information_loss`
- إسقاطُ شرط البادئة المحفوظة ⇒ إعادةُ ترتيب الصفوف أو ملءُ الحذف بحشوٍ جديد تُقرأ «ضمّاً lossless»، مع أنّ النصّ المحفوظ لم يبقَ بادئةً كما كان — يُسقِط `test_lossless_row_deduplication_rejects_information_loss`
- قراءةُ أمثلة الكتل المسوّرة كسجلّ حيّ ⇒ صفوفُ الشرح تموّل التقلّص، فيُحذَف مثالٌ موثّق ويُقال إنّه صفٌّ زائد — يُسقِط `test_lossless_row_deduplication_preserves_fenced_examples`
- إسقاطُ حدّ السطر ⇒ لاحقةٌ جديدة تعيد كتابة السطر الأخير غير المنتهي بدل أن تُلحَق بعده، فيُمحى نثرٌ قائم ويُسمّى ذلك إضافة — يُسقِط `test_lossless_row_deduplication_rejects_information_loss`
- تركُ الابن يحمل التكرار بعد «الإصلاح» ⇒ الحارس يقبل تقلّصاً لم يحقق التفرّد العالميّ أصلاً، فيبقى الصفّ المكرّر أو يعود في اللاحقة — يُسقِط `test_lossless_row_deduplication_rejects_information_loss`
- إبطالُ مسار القبول الضيّق نفسه ⇒ الشاهدُ الذي يجعل الحارسين يحكمان المرشّحَ نفسه يحمَرّ، ويعود الإصلاح lossless إلى `JOURNAL_SHRANK` رغم أنه أزال النسخ المتطابقة فقط — يُسقِط `test_lossless_row_deduplication_satisfies_both_real_guards`
- إسكاتُ اشتقاق موضع الانخفاض ⇒ تعود الرسالةُ الثابتة وحدها، فيُقال لقارئٍ انخفاضُه في التاريخ إنّ علاجَه إعادةُ البناء من كلا الوالدَين — علاجٌ لا يعمل على حالته، وهو ما وقع في #1036 — يُسقِط `test_a_historical_dip_says_that_a_later_commit_cannot_repair_it`
- طباعةُ علاج التاريخ دائماً ⇒ يُطالَب دمجٌ يُكتب الآن بنقض تاريخٍ لا داعي له. رسالةٌ صحيحةٌ في غير موضعها تُدرِّب قارئَها على تجاوزها — يُسقِط `test_a_dip_at_the_head_keeps_the_tip_remedy_only`
...
- تحويلُ الصنف الخامس من **تحقّقٍ في دفتره** إلى استثناءٍ يجعل `E-2026-09-21-99` الملفَّق يمرّ — وهو بعينه ما يرفضه هذا الحارسُ في صنفَي التفويض وحاجبِ الاعتماد: الاستشارةُ وحدَها تُستثنى اضطراراً لأنّ مصدرها خارج الشجرة، وما كان ملفّاً هنا يُقاس وجودُه. — يُسقِط `test_a_fabricated_evidence_record_id_is_still_rejected`
- عناوينُ الدفتر تحمل وصفاً بعد المعرّف (`### E-… — <وصف>`)، فمطابقةُ السطر كاملاً تُعيد مجموعةً فارغةً ويُبلَّغ **كلُّ** معرّفٍ صادقٍ «غيرَ مسجَّل» — صفرٌ كاذبٌ يقلب الحارسَ ضدّ العمل الصحيح. وقع فعلاً أثناء كتابة هذا الصنف وكشفه تشغيلُ الحارس على التزام الشريحة. — يُسقِط `test_the_record_ids_are_read_from_headings_that_carry_a_title`
...
- تحييد الفحص العالميّ للصفوف ⇒ يعود صنف BRAIN-DUP-ROW-ESCAPES-THE-ADJACENCY-NET-01 (صفّ GATE-READS المكرّر غير المتلاصق) يمرّ صامتاً — يُسقِط `test_a_non_adjacent_row_duplicate_is_caught_globally`
- إسقاط النقطة لا يدمج الهويّات بل يُعمي الشبكة عن الصفوف المنقوطة كلّها — مرساة \s*\| تُفشِل المطابقة عند النقطة (مقيس بالزرع، لا مفترَض) — يُسقِط `test_a_dotted_row_id_duplicate_is_still_visible_to_the_global_net`
...
- نزعُ استثناء موضع الادّعاء القانونيّ ⇒ عنوانٌ أو سطرُ `- **الحالة:**` يحمل الرمزَ بين ` ` يُقرأ اقتباساً فيمرّ إغلاقٌ دماغيٌّ صرف من أخطر موضع — سلبيّةٌ كاذبةٌ يصنعها إصلاحُ إيجابيّةٍ كاذبة — يُسقِط `test_a_claim_in_its_canonical_position_is_read_whatever_its_typography`
- توسيعُ المُرساة إلى الخطّ العريض ⇒ `**البوّابة CLOSED عالميّاً**` يُقرأ اقتباساً. والقياسُ يقول إنّ الادّعاءَ يقع في المتن فعلاً (`**SAM2** closed` في strategy.md)، فهذا التوسيعُ يُطلِق ادّعاءاتٍ حقيقيّة — يُسقِط `test_bare_prose_is_still_read_because_a_claim_does_land_there`
- نمطٌ جشِعٌ يبتلع ما بين اقتباسَين ⇒ رمزٌ عارٍ بينهما يُفرَّغ معهما. ثقبٌ صامت: السطرُ يبدو مقتبَساً وهو ليس كذلك — يُسقِط `test_a_bare_token_between_two_quotations_is_still_a_claim`
...
- **ما تُثبِته هذه الطفرة تغيّر مع تعطيل البند (أمرُ المالك 2026-09-19)، فيُقال صراحةً:** كانت تُثبِت أنّ البند **يحجب**، وصارت تُثبِت أنّه **ما يزال يُقاس ويُعلَن**. الحجبُ لم يعد مُثبَتاً لأنّه لم يعد قائماً. وإسقاطُ القياس أسوأُ من إسقاط الحجب: تعطيلٌ مُعلَنٌ يُقرأ في السجلّ ويُراجَع، وتعطيلٌ صامتٌ يجعل المستودع يظنّ الشرطَ مفروضاً وهو ليس كذلك — فيصير `approved_by: owner` حقلاً نصّيّاً لا يعلم أحدٌ أنّه بلا منشأ. — يُسقِط `test_touching_the_authorization_path_reports_but_no_longer_blocks`
...
- الغياب يعني أنّ الشرط **لم يُرَ**، وقراءتُه «مُفعَّل» هي بعينها «نتيجةٌ عن سؤالٍ لم يُطرَح» — وهو الصنف الذي وُجِد هذا الملفّ كلّه ليطارده. والتعطيلُ لم يمسّ هذا: تحت الطفرة يختفي السطرُ الإرشاديُّ نفسُه، فيُقرأ الشرطُ نافذاً وهو غائب — عطلٌ مستقلٌّ عن قرار الحجب. — يُسقِط `test_a_missing_code_owner_key_is_still_not_read_as_enabled`
- إسقاط اتّجاه «سياقٌ في العقد وليس مفروضاً» ⇒ بوّابةٌ تسقط من الـRuleset تصير إرشاديّةً صامتة بلا أن يحمرّ شيء — وهو العطل الأصليّ الذي فُتِح العقد لأجله — يُسقِط `test_a_context_missing_from_enforcement_is_a_silent_advisory_gate`
- إسقاط الاتّجاه الآخر ⇒ اسمٌ مفروضٌ لا تُبلِّغه وظيفة يُعلِّق كلّ PR إلى الأبد، أو وظيفةٌ حاجبة تبقى غير مُسجَّلة — يُسقِط `test_an_enforced_context_absent_from_the_contract_is_a_failure`
- غيابُ قاعدة required_status_checks يُقرأ مساواةً فارغة صحيحة ⇒ «لا فحص يحجب» يمرّ أخضر: أخطر صور الخضرة الكاذبة — يُسقِط `test_no_required_status_checks_rule_means_nothing_blocks`
- نزعُ البند الثالث يُعيد `dismiss_stale_reviews_on_push` إلى ما كان: **مُفعَّلٌ بلا تكذيب**، فحمايةٌ قائمةٌ تُقرأ عقداً وهي إعدادٌ يزول بنقرة. والأثرُ يمسّ صدقَ الاعتماد نفسِه — يعتمد المراجعُ بايتاتٍ ثمّ يُدفَع غيرُها فيبقى الاعتمادُ سارياً على شيفرةٍ لم يرَها أحد، ولا تُغني `required_status_checks` لأنّها تشهد للآلة لا لأنّ إنساناً قرأ — يُسقِط `test_a_stale_approval_that_survives_a_new_head_is_a_failure`
- نزعُ الرجوع الصامت يُسقِط الحالةَ إلى فرع «غير مُفعَّل» فيطبع `dismiss_stale_reviews_on_push = []` وكأنّ القيمة رُصِدت وهي لم تُقرأ أصلاً — ملاحظةٌ ثانيةٌ مُضلِّلة عن عطلٍ واحد تدفع المُشخِّص إلى مطاردة إعدادٍ سليم. والحكمُ لا يتغيّر، فالتكذيبُ بالمخرَج لا برمز الخروج — يُسقِط `test_an_unreadable_parameters_block_is_diagnosed_once_not_twice`
...
**يفرض:** حارس CI: كلُّ مسارٍ يطلبه عميلُ خدمةٍ في المنصّة **مُعلَنٌ** في الخدمة التي يناديها.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_cross_service_path_contract_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- قلبُ الطرح إلى تقاطعٍ يجعل الحارسَ يُبلِّغ عمّا **يعمل** ويصمت عمّا ينكسر. وهو أخبثُ من تعطيله: يطبع قائمةً طويلةً تُقرَأ عملاً، والمسارُ المنحرفُ وحدَه هو الغائبُ منها. — يُسقِط `test_no_requested_path_is_absent_from_the_target_service`
- القراءةُ الصفريّة هي **كيف يمرّ هذا الصنفُ نفسُه**: عميلٌ بشكل نداءٍ لا يعرفه المستخرِج يُقرأ منه صفرُ مسار، فيمرّ أخضرَ عن سؤالٍ لم يطرحه (مقيس: `soil_hydraulic_client` أعطى صفراً بأوّل قارئ). وهذه الطفرةُ **نجت أوّلَ مرّة** لأنّ شاهدَها كان تأكيدَ غيابٍ على شجرةٍ سليمة — والقائمةُ تبقى فارغةً إن عُطِّل الكشفُ رأساً. فأُخرِج التصنيفُ نقيّاً وصار له شاهدٌ موجب. — يُسقِط `test_a_client_that_reads_zero_is_actually_reported_blind`
- `irrigation_activation_gate` يحمل مسارَه في ثابتِ وحدةٍ ويبنيه داخل دالّة، فلا يُرى عند موضع النداء. إسقاطُ هذا الشكل يُعيد عقداً حقيقيّاً إلى خارج النطاق بلا أن يُعلَن. — يُسقِط `test_both_call_shapes_and_the_module_constant_are_read`
- هذه هي الطفرةُ الوحيدةُ التي **وقعت فعلاً**: `endswith` أفلت `raster_get_json_sync` فضاع مسارانِ حقيقيّان، ولم يُكشَف إلّا بمقابلة قارئَين. إعادتُها تُعيد الثقبَ بعينه. — يُسقِط `test_the_structural_reader_sees_everything_a_textual_one_sees`
- صفرُ عقدٍ مكتشَفٍ أخطرُ من عقدٍ منحرف: الحارسُ يعمل ويُنهي بنجاحٍ وهو لا يفحص شيئاً — تأمينٌ على لا شيء، وهو الصنفُ الذي سُجِّل في `MUT-REGISTRY-STRANDED-SPECS-01`. — يُسقِط `test_the_guard_sees_exactly_the_modules_that_name_an_internal_service`

### `db_writer_ownership_guard.py`

**يفرض:** `OWNERSHIP-CONTRACT-DECLARED-BUT-NEVER-MEASURED-01`: عقدٌ يُعلَن ولا يُقاس.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_db_writer_ownership_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- حارسٌ يأذن للجميع يُبلِغ صفراً على شجرةٍ تحمل 75 مخالفة — ويُقرَأ ضماناً ويُسجَّل تغطية. وهو بعينه الصنفُ الذي جعل M-03 يبيت حتّى كشفه فحصٌ يدويّ. — يُسقِط `test_a_write_the_contract_does_not_permit_is_caught`
- إدانةُ خمسةِ جسورٍ موثَّقة (status: interim-bridge) إنذارٌ كاذب — ويُدرَّب الناسُ على تجاهل الحارس، فيموت وهو أخضر بينما الانحرافُ الحقيقيّ يمرّ معه. — يُسقِط `test_a_documented_interim_bridge_is_not_a_violation`
...
### `gap_heading_state_guard.py`

**يفرض:** راتشِتُ العناوين اليتيمة — `GAP-HEADING-WITHOUT-A-STATE-RECORD-IS-INVISIBLE-01`.

**يحجب في:** `no-report-only-change.yml` → `no-report-only-change`

**الاختبار الشاهد:** `tests_v9/test_gap_heading_state_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- راتشِتٌ لا يحجب يُعيد العطلَ كما وقع: عنوانٌ جديد يحمل حالتَه في العنوان يختفي من كلّ عدٍّ يُقرأ ووظيفةُ القياس خضراء — وهو بعينه ما كشفه فارقُ مصنوعَتَي #1033 و#1035. — يُسقِط `test_growth_beyond_the_baseline_blocks`
- الخطأُ في الجهة الأخرى: أساسٌ لم يتغيّر يُقرأ نموّاً فيُحمِرّ كلَّ PR لا تمسّ السجلّ أصلاً — وحمايةٌ تُوقِف العملَ كلَّه تُطفَأ، فتُنتِج حمايةً صفراً. — يُسقِط `test_equality_is_not_growth`
- رسالةٌ تقول «ارتفع العدد» بلا الصيغة المقبولة تُرسِل قارئَها ليخمّن — والعطلُ نفسُه نشأ من أنّ الصيغة المرفوضة تبدو صحيحةً للقارئ البشريّ. فالعلاجُ في الرسالة جزءٌ من الحارس لا زينة. — يُسقِط `test_the_message_names_the_remedy_and_the_rejected_spelling`
- أساسٌ مفقودٌ يُقرأ «لا حدّ» يُفرِّغ الراتشِت بلا أن يُحمِّره — «لم يُقرأ» تصير «لا شيء»، وهو الصنفُ الذي يطارده هذا المستودع في كلّ حارس. — يُسقِط `test_a_missing_baseline_fails_closed`
- فصلُ اكتشاف المستوى غير القانونيّ عن أسباب الحجب ⇒ يُقاس ويُطبَع ولا يحجب. وهذا بعينه ما وقع على `a0bba343`: «Gap registry measurement» تقريرٌ لا يحجب، فمرّت `V25-AI-RUNTIME-LOCAL-ACCEPTANCE-01` غائبةً عن كلّ عدّ والقياسُ أخضر. — يُسقِط `test_the_level_finding_is_blocking_not_advisory`

### `gap_registry_claim_guard.py`
...
- إسقاطُ المقارنة نفسِها ⇒ يعود العطلُ كما وقع على `98a61c5f`: السجلُّ يقول 272/51 والمقيسُ 273/52، ووظيفةُ القياس خضراء لأنّها تقيس ولا تقارن. — يُسقِط `test_a_present_tense_claim_that_disagrees_blocks`
- حجبُ الادّعاءات التاريخيّة أيضاً ⇒ يُحمِّر الحارسُ على سردٍ سليم (اثنا عشر ادّعاءً صحيحاً عن أشجارٍ ماضية)، وحمايةٌ تُوقِف العملَ تُطفَأ فتصير حمايةً صفراً. — يُسقِط `test_a_claim_that_names_its_tree_is_not_blocked`
...
- إغفالُ الحقل يصير باباً صامتاً: تفويضٌ بلا أجلٍ يمرّ، فيعود `GATE01-AUTHORIZATION-NEVER-EXPIRES-01` من حيث أُغلِق — وحقلٌ إعلانيٌّ لا يفرضه شيء هو بعينه الفرقُ بين `expired_at` المفروض و`# Expiry:` الخامل. — يُسقِط `test_an_issued_authorization_without_an_expiry_is_rejected`
- أجلٌ لا يُقارَن هو أجلٌ غيرُ موجود. والطفرةُ مستقلّةٌ عن سابقتها: تلك تحرس **وجودَ** الحقل وهذه تحرس **إعمالَه** — وحقلٌ حاضرٌ لا يُقرأ يمرّ على الأولى ويسقط على الثانية. — يُسقِط `test_an_expired_issued_authorization_is_rejected`
- الخطأُ في الجهة الأخرى: إذنٌ مُنِح «حتّى الثلاثين» يسقط صباحَ الثلاثين فيخسر صاحبُه يوماً مُنِحه. وتضييقٌ يبدو أشدَّ حرصاً هو عطلٌ مثل تساهُله. — يُسقِط `test_an_authorization_expiring_today_is_still_valid`
- فرضُ الأجل بأثرٍ رجعيّ يُحمِّر المستودعَ على خمسةِ تفويضاتٍ استُهلِكت صحيحةً قبل وجود الحقل — وحارسٌ يُدين ماضياً امتثل لعقدِ زمانه يُدرَّب قارئُه على تعطيله. — يُسقِط `test_expiry_is_not_enforced_on_a_settled_authorization`
- بلا التحذير يصير أوّلُ خبرٍ عن الأجل هو الحجبَ نفسه — وهي الإصابةُ المنشورة بحرفها: ستّةُ إعفاءاتٍ انتهت دفعةً واحدةً فحجبت البناء بلا إنذار. **ومرساها مُوسَّعةٌ عمداً:** الصيغةُ المختصرة صارت تتكرّر بعد إضافة `alias_deadline_warnings`، فسقط الزرعُ على تصادمِ نصوصٍ لا على عطل — والمراسي نصّيّةٌ بالتصميم. — يُسقِط `test_a_near_expiry_warns_before_it_blocks`
- حذفُ الحقل يصير طريقَ الخلاص من الأجل، فيعود الإقرارُ إعفاءً دائماً — وهو ما يصف الإقرارُ نفسَه بأنّه ليسه («تسجيلُ حالةٍ تنتظر حكماً، لا إعفاء»). — يُسقِط `test_an_acknowledgement_without_a_deadline_is_rejected`
- سكوتٌ لا ينقضي هو السكوتُ الأصليُّ بعينه. والمقيس أنّ النظيرَ الحيَّ مُسّ في ٤ من ٤ من الشرائح المُحكَّمة — فالثقبُ يُستعمَل في كلّ مرّة، وأجلٌ لا يُقارَن لا يسدّه. — يُسقِط `test_a_lapsed_acknowledgement_blocks_again`
- حجبٌ داخل المدّة يجعل تسجيلَ الحالة مستحيلاً: لا صيغةَ للإقرار تمرّ، فيُطفَأ الحقلُ أو يُطفَأ الحارس. صنفُ «حارسٍ لا يقبل حالةً صادقة» المُغلَق في #1026. — يُسقِط `test_a_live_acknowledgement_still_silences_the_report`
- شحنُ «يحذّر قبل أن يحجب» للتفويضات وتركُ الإقرار يحجب فجأةً تناقضٌ في الملفّ نفسِه — والقاعدةُ واحدة: أوّلُ خبرٍ عن الأجل يجب ألّا يكون الحجب. — يُسقِط `test_the_alias_deadline_warns_before_it_blocks`
...
- تحييد المسار السريع ⇒ كلّ زرعة تعود تشغّل الملفّ كاملاً — عودة صنف MUT-SWEEP-RUNS-THE-WHOLE-FILE-PER-PLANT-01 صامتةً (انحدار أداء لا صحّة، يُمسَك بعدّ الاستدعاءات) — يُسقِط `test_a_narrow_kill_skips_the_full_file_run`
- إسقاط التراجع إلى الملفّ الكامل ⇒ مرورُ المتوقَّع وحده يُختَم unexpected_green/survived من مشهدٍ ناقص، وتصنيفا wrong_test/التشخيص يفقدان مصدرهما — يُسقِط `test_a_passing_narrow_run_falls_back_to_the_full_file`
- نزعُ كشفِ الزيادة غير المُقرَّة ⇒ تجميدُ سطح الحجب يصير اسماً بلا قياس: ينمو السطحُ صامتاً وتبقى الوظيفةُ خضراء، وهو صنفُ «الحارس الذي لا يرى العطل أصلاً» — يُسقِط `test_an_undeclared_addition_is_reported`
- نزعُ شرطِ «الاسمُ مسجَّلٌ لهذا الحارس» يُعيد `mutation` حقلاً نثريّاً: يُستوفى بالكتابة لا بالتسجيل، فيمرّ إقرارٌ يسمّي تكذيباً لا وجودَ له. وهو صنفُ «بوّابةٌ تُغلَق بالنثر» — قِيس على أوّل إقرارٍ كُتِب في هذه الشجرة، إذ حمل اسمَ الاختبار داخل جملةٍ وكان الحقلُ سيقبل «طفرةٌ ما» بالقدر نفسِه — يُسقِط `test_a_mutation_naming_an_unregistered_test_is_reported`
- `expect` اسمُ الحالة و`test` ملفُّ الجناح؛ وقراءةُ الثاني مكانَ الأوّل تجعل الفحصَ يُبلِّغ عن طفرةٍ **مسجَّلةٍ** بأنّها غيرُ مسجَّلة — إنذارٌ كاذبٌ يدفع كاتبَ الإقرار إلى تعطيل الفحص لا إلى تسجيل طفرة. وقعت فعلاً عند كتابة هذه الشريحة وكشفها تشغيلٌ لا قراءة — يُسقِط `test_the_registry_reader_reads_the_expected_test_not_the_suite_file`
- نزعُ الاتّجاه الرابع يُعيد **الصمت المقيس**: نُزِع استدعاءُ حارسٍ من `ci.yml` على `a3124ccf` فهبط السطحُ ٣٠١ ⇒ ٣٠٠، ولم يشتكِ إلّا فحصُ انحراف الكتالوج — وعلاجُه المنصوصُ عليه إعادةُ التوليد، فإذا فُعِل قال الاثنان أوكيه. أي أنّ العلاجَ الذي يأمر به النظامُ هو ما يمحو الدليل، ويبقى ملفُّ الحارس يبدو حمايةً ولا يُشغّله شيء — يُسقِط `test_a_blocker_that_vanished_is_reported`
- إقرارُ تقاعدٍ لحاجبٍ يعمل بابُ إسكاتٍ دائم: يُعلَن التقاعدُ اليومَ ويُعاد الاستدعاءُ غداً فلا يُبلَّغ عن نزعِه ثانيةً — أي أنّ سطراً واحداً في السجلّ يشتري صمتاً دائماً عن ثلاثيّة — يُسقِط `test_a_retirement_declared_for_a_blocker_still_running_is_reported`
- نزعُ فحص اليُتم يُعيد `retired` حقلاً يقبل أيّ مفتاح، فتتراكم فيه أسماءٌ لا تخصّ ثلاثيّةً كانت في الأساس قطّ — سجلٌّ يبدو نظيفاً وهو يحمل ما لا وجودَ له، وهو الصنفُ نفسُه المُغلَق للإقرارات في الاتّجاه الثالث. أصابت مراجعةٌ آليّة على #983 — يُسقِط `test_a_retirement_for_a_triple_never_in_the_baseline_is_reported`

### `json_duplicate_key_guard.py`

**يفرض:** مفتاحٌ مكرَّر في وثيقة JSON يُسقِط ما قبله **صامتاً**.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_json_duplicate_key_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- نزعُ تسجيلِ ما رآه المحلّل مكرَّراً يُعيد الحارسَ إلى عمى `json.load` الافتراضيّ بعينه — يقرأ الوثيقة، ويطوي التكرار، ويُعلِن نجاحاً. وهو العطلُ الأصليّ لا محاكاتُه. — يُسقِط `test_a_duplicate_inside_behavioural_is_detected`
- `mutations` مصفوفةٌ من كائنات، فتكرارٌ داخل عنصرٍ منها لا يُرى إن لم يعبر المشيُ القوائم. وحارسٌ يفحص الكائناتِ وحدها يمرّ على نصفِ الوثيقة وهو يظنّ أنّه مسحها. — يُسقِط `test_a_duplicate_nested_inside_a_list_is_detected`
- جردٌ فارغ (نمطٌ لا يطابق، جذرٌ خطأ) يصير نجاحاً — الحارسُ يُخضِر لأنّه لم ينظر لا لأنّه لم يجد. وهو صنف «أخضرُ عن سؤالٍ لم يُطرَح». — يُسقِط `test_scanning_nothing_is_a_failure_not_a_pass`
- وثيقةٌ تالفة تُقرَأ سليمةً: التكرارُ فيها غيرُ مقيسٍ أصلاً، فابتلاعُها يقول «لا تكرار» عن ملفٍّ لم يُفحَص. — يُسقِط `test_invalid_json_fails_closed_with_a_named_reason`
- ملفٌّ في الجرد ولا يُقرَأ يُطرَح صامتاً، فينكمش المسحُ بلا أن يُعلَن — والخضرةُ تصير عن جردٍ أصغر ممّا يقوله اسمُها. — يُسقِط `test_a_tracked_file_missing_from_disk_fails_closed`
- تعذُّرُ الجرد يُقرَأ «لا تكرار» — أسوأُ صور الفشل المفتوح: الحارسُ يُعلِن سلامةَ ما لم يستطع النظر إليه. — يُسقِط `test_an_unusable_inventory_source_fails_closed`
...
**يفرض:** حارس CI: يمنع إيداعَ نسخةٍ احتياطيّةٍ يدويّةٍ في الشجرة المتعقَّبة.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_no_manual_backup_files_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- `main.before_p2` هو الملفُّ الذي وقع مرّتين في خدمتين. حارسٌ يفقد هذا البديل يبقى أخضرَ على الحادثة التي وُجِد لأجلها حرفيّاً — وهو الشكلُ الأدقّ لِـ«حارسٍ لا يفعل شيئاً». — يُسقِط `test_a_manual_backup_name_is_rejected`
- `.bak`/`.orig`/`.rej` هي مخرجاتُ أدوات الدمج والنسخ نفسِها — أكثرُ الأشكال وروداً. إسقاطُها يُبقي الحارسَ يحرس صنفاً واحداً من ثلاثة ويُعلِن تغطيةَ الثلاثة. — يُسقِط `test_a_manual_backup_name_is_rejected`
- فخُّ رمز الخروج: الحارسُ يطبع الشكوى كاملةً ثمّ يُنهي بصفر، فتقرأ CI نجاحاً ويقرأ الإنسانُ فشلاً. لا شيءَ يُحجَب، ولا شيءَ يبدو مكسوراً. — يُسقِط `test_the_guard_exits_nonzero_when_it_finds_one`
- مسحٌ يُعيد قائمةً فارغةً دائماً يمرّ أخضرَ على كلّ شجرة. وهي الخاصّيّةُ التي يُحقّقها حارسٌ محذوف — فوجودُه في CI يصير تأميناً على لا شيء. — يُسقِط `test_the_guard_exits_nonzero_when_it_finds_one`
- `main.py~` مخرَجُ محرّراتٍ شائعة (vim/emacs/gedit) ويُلتزَم سهواً لأنّه غيرُ مرئيٍّ في `ls`. إسقاطُ هذا البديل يترك أكثرَ الأشكال إفلاتاً من العين البشريّة بلا كاشف. — يُسقِط `test_a_manual_backup_name_is_rejected`
...
- نزع استثناء الوثيقة + مصنوعات إعادة التوليد يعيد العطل المقيس على #1012 حرفيّاً: شريحةٌ وثائقيّة تُطيع إعادةَ التوليد الإلزاميّة تُحجَب «report-only» إلى الأبد — والرأسُ يَعِد بعكسه — يُسقِط `test_a_plain_doc_with_regenerated_artifacts_is_docs_only`
- توسيعُ الاستثناء من «كلّ التقارير مصنوعاتُ توليد» إلى «توجد وثيقة» يجعل أيَّ Markdown مفتاحاً يفتح تقريراً بخطّ اليد أو سجلَّ الاعتماد — وهو بعينه ما يحرسه الحارس — يُسقِط `test_a_hand_edited_report_beside_a_doc_stays_blocked`
- إسقاط config/ من التصنيف يعيد العطل المقيس على #1011 حرفيّاً: شيفرةُ تشغيلٍ حقيقيّة + مصنوعاتُ إعادة التوليد الإلزاميّة تُحجَب «report-only» — الـPR تمرّ ما دامت لم تُصلِح حزمتَها وتُحجَب لحظةَ إصلاحها — يُسقِط `test_runtime_config_code_with_regenerated_artifacts_is_substantive`
- إعادةُ ملفٍّ يحمل حقلاً مُحكَّماً بخطّ اليد (`proven_live` يحمله المولِّد ولا يشتقّه) إلى مصنوعات إعادة التوليد تفتح ادّعاءَ إثباتٍ حيٍّ خلف وثيقة — مقيسٌ على مراجعة Copilot ٣ في #1012 — يُسقِط `test_a_baseline_carrying_a_hand_adjudicated_field_is_not_a_regeneration_artifact`
- ردُّ إعفاء الدماغ إلى بادئةٍ خام يُمرِّر شيفرةً تحته (`sahool-brain/extra.py`) مع تقريرٍ مولَّد بوصفها «وثائقيّة» — وعقدُ الدماغ Markdown — يُسقِط `test_a_non_markdown_file_under_the_brain_does_not_launder_artifacts`
- نزعُ ذراع «لا شيء سوى مصنوعات» يعيد العطل المقيس في مراجعة Copilot ٤ على #1012: `release/FILE_CHECKSUMS.sha256` و`SBOM_MINIMAL.json` لا يطابقان أسماءَ التقارير ولا لاحقاتِها فيمرّان منفردَين، والعقدُ المكتوب يَعِد بحجبهما — يُسقِط `test_a_lone_generated_artifact_is_blocked_even_without_a_report_name`
- نزعُ الذراع الثالثة يعيد العطل المقيس في مراجعة Copilot ٥ على #1012: ملفٌّ بخطّ اليد خارج `docs/` بلا تلميحٍ في اسمه (`RELEASE_NOTES_20260626.md`) يحمل مصنوعاً معه ويعبر، لأنّ `artifacts_only` يشترط أن يكون كلُّ مسارٍ مصنوعاً — يُسقِط `test_an_artifact_riding_with_an_unapproved_path_is_blocked`

### `ownership_extraction_alignment_guard.py`

**يفرض:** `A-RATCHET-CLEARED-BY-DELETING-WHAT-IT-MEASURES-01` — علاجٌ يُنزِل العدّاد ويهدم الهدف.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_ownership_extraction_alignment_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- الحارسُ يصير أخضرَ على كلّ عقد، فيعود البابُ الذي وُجِد لإغلاقه مفتوحاً — ويُقرأ وجودُه تغطيةً لا يملكها. — يُسقِط `test_an_extra_declared_writer_on_a_live_target_is_refused`
- حصرُ النطاق بالأهداف الحيّة هو ما يُبقي بابَ التقاعد المشروع مفتوحاً. بإسقاطه يصير العقدُ مجمَّداً ولا يمكن تصحيحُه ولو تقاعد الهدفُ فعلاً — «بوّابةٌ لا تُغلَق بعملٍ صحيح»، وهي الصنفُ الذي يُطفَأ فتصير الحمايةُ صفراً. — يُسقِط `test_retiring_the_target_reopens_the_correction`
- `mirror` جسرٌ انتقاليٌّ يحترمه `db_writer_ownership_guard`. وحارسان يختلفان في الاستثناء نفسِه يُنتِجان حكمين عن سؤالٍ واحد، فيُنذِر أحدُهما كذباً ويُدرَّب قارئُه على تجاهله. — يُسقِط `test_the_documented_interim_bridge_stays_allowed`
- خريطةٌ بلا أهدافٍ تجعل النطاقَ خالياً فيمرّ الحارسُ على كلّ شيء — خُضرةٌ عن سؤالٍ لم يُطرَح، وهي أخطرُ من غياب الحارس. — يُسقِط `test_an_empty_extraction_map_fails_closed_not_open`
...
**يفرض:** `TENANT-GUC-NAME-DIVERGES-ACROSS-POLICY-FAMILIES-01` — اسمُ مُتغيّر المستأجِر واحدٌ أو معدود.

**يحجب في:** `ci.yml` → `structural-lint`

**الاختبار الشاهد:** `tests_v9/test_tenant_guc_name_convergence_guard.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- تعميةُ كاشف الاسم الجديد تُعيد الصنفَ بحرفه: عائلةُ سياساتٍ رابعة تدخل الشجرة باسمٍ لا يضبطه أحد، فتُعيد جداولُها صفرَ صفوفٍ في كلّ مسارٍ حيّ — والبوّابةُ خضراء. — يُسقِط `test_a_fourth_name_is_actually_reported`
- اسمٌ تقرؤه سياسةٌ ولا يضبطه مسارٌ يعني عزلاً يفشل مغلقاً **دائماً**: الجدولُ لا يُقرَأ منه شيءٌ أبداً. وتعميةُ هذا الكاشف تُبقي الحالةَ غيرَ مرئيّة، وهي أخبثُ من انحرافٍ مُبلَّغ لأنّ ناتجَها إجابةٌ ناقصةٌ تُقرَأ كاملة. — يُسقِط `test_a_name_no_path_sets_is_actually_reported`
- انكسارُ المُستخرِج (تبدّلُ صياغةِ `current_setting` أو نقلُ الهجرات) يجعل النطاق صفراً، فيمرّ الحارسُ أخضرَ عن سؤالٍ لم يطرحه — وهو `GUARDS-WITHOUT-A-PLANTED-DEFECT-01` بعينه: «لم يجد» تُقرَأ «لا يوجد». — يُسقِط `test_a_zero_discovery_run_fails_instead_of_passing_green`
...
- نزعُ حجب بيات الإسناد ⇒ يبقى provenance قديم إلى الأبد بحجّة أنّ النتيجة نفسها. وإعادةُ الاشتقاق لا تسدّ هذا: هي تُجيب عن **النتيجة** لا عن **الأساس** الذي أنتجها. — يُسقِط `test_a_changed_basis_with_an_identical_result_is_provenance_only`
- غيابُ البصمة يُقرأ تطابقاً ⇒ مصنوعةٌ سابقة للعقد تُصادِق نفسها. وهو صنفُ «يفشل مفتوحاً» بعينه: غيابُ الدليل ليس دليلاً. — يُسقِط `test_a_missing_digest_is_an_explicit_migration_state_not_implicit_freshness`
- نزعُ مكوّن الخوارزميّة ⇒ يعود الأساس بصمةَ مدخلاتٍ وحدها، فيعمى عن تغيُّر منطق المولّد بمدخلاتٍ ثابتة — وهو أخطر الانحرافين لأنّه يبدّل معنى الرقم بلا أثرٍ في مدخلاته. — يُسقِط `test_the_basis_binds_all_three_components`
- عدُّ المولّد مُدخَلاً لنفسه ⇒ يُبصَم مرّتين بقاعدتَي حسّاسيّة مختلفتين (بايتات · AST)، فيُبطِل تطبيعَ التوثيق: تعديلُ تعليقٍ فيه يصير انحرافَ مدخلات، وهو الـchurn الذي وُجِد العقد ليمنعه. — يُسقِط `test_the_generator_is_not_counted_as_its_own_input`
- عودةُ `measured_on` سلطةَ طزاجةٍ من الباب الخلفيّ — لا في حساب الأساس بل **فرعاً في مسار القرار**. وثباتُ الأساس وحده لا يمسك هذا: الختمُ يبقى خارج الحساب ويدخل الحكم. مقيسٌ أثرُه: كلّ squash وكلّ التزامٍ توثيقيّ يصير عندئذٍ generated churn. — يُسقِط `test_a_measured_on_only_change_leaves_the_check_verdict_identical`
...
## استدعاءات غير مانعة بحسب continue-on-error (1)

يشمل التصريح على الخطوة أو الوظيفة؛ تبقى هذه الاستدعاءات ظاهرة ولا تُعدّ حجبًا.

| الأداة | workflow | الوظيفة |
|---|---|---|
| `gap_registry_measure.py` | `capability-governance.yml` | `gap-registry-report` |
