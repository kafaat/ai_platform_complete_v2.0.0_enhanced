...
- حرّاس تحجب في CI: **266**
- منها **مُثبَتة بالتكذيب** (لها مواصفة طفرة نُفِّذت): **47**
- إجماليّ الطفرات المُسجَّلة: **340**
- وطفراتٌ **سلوكيّة** تُزرَع في منطق الإنتاج نفسه: **180** على 60 مصدراً
...
**يحجب في:** `ci.yml` → `lint` · `ci.yml` → `mutation-sweep` · `ci.yml` → `unit-tests`
...
### `path3_image_evidence_guard.py`

**يفرض:** `IMAGE-MANIFEST-WITHOUT-ITS-EVIDENCE-READS-AS-CLEAN-01` — البصماتُ الأربعُ تُفرَض.

**يحجب في:** `path3-runtime-verification.yml` → `verify-images`

**الاختبار الشاهد:** `tests_v9/test_image_supply_chain_contract.py`

**ما يمسكه** — كلّ بند مُثبَت بزرع العطل وتشغيله:

- **طولٌ ليس شكلاً.** أربعةٌ وستّون محرفاً أيّاً كانت تمرّ، فيقبل الحارسُ بصمةً مُختلَقة ويُعلِن البيانَ سليماً. وهذا وقع حرفيّاً في مسار الترقية قبل إصلاحه: `test "${#TARGET_SHA}" = 40` كان يقبل أربعين محرفاً غيرَ ستّ عشريّة — **فحصٌ يبدو تحقّقاً ولا يقيّد المجموعة أسوأُ من غيابه، لأنّه يُطمئن القارئ فيكفّ عن النظر**. — يُسقِط `test_the_evidence_guard_is_falsifiable_without_a_workflow`
- تخطّي الصفِّ بلا كتلةِ أدلّةٍ يجعل «لا مخالف» يعني «لم يُنظَر»: بيانٌ خالٍ من الأدلّة كلّيّاً يمرّ أخضرَ، وهو **أسوأُ من التجاوز لأنّ التجاوزَ يُرى**. — يُسقِط `test_the_evidence_guard_is_falsifiable_without_a_workflow`
