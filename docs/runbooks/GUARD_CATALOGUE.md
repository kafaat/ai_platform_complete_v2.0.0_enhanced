...
- حرّاس تحجب في CI: **225**
- منها **مُثبَتة بالتكذيب** (لها مواصفة طفرة نُفِّذت): **15**
- إجماليّ الطفرات المُسجَّلة: **87**
...
- مقارنةٌ بالصدق بدل المنطقيّ: `"false"` نصّاً **صادقةٌ** في بايثون، فتُقرأ قفلاً مُفعَّلاً وهو مُطفَأ. طفرةٌ مستقلّة عن سابقتها: تلك تحرس **الحكم** وهذه تحرس **نوع القيمة** — وحقلٌ يتغيّر تمثيله في API يُصيب الثانية وحدها — يُسقِط `test_a_non_boolean_enabled_is_rejected`
- قراءةُ المفتاح الغائب قبولاً ⇒ «لم يُقرأ» تصير «مُفعَّل». وهو صنف «نتيجةٌ عن سؤالٍ لم يُطرَح» الذي عولج ستّ مرّات في الشريحة التي أنشأت هذه الفجوة أصلاً — يُسقِط `test_a_missing_key_is_not_read_as_enabled`
- ملفٌّ غائب يُقرأ «مضبوطاً» ⇒ الحارس أخضرُ **بالضبط حين لا يُقاس شيء**. والحالة مرجَّحة لا نادرة: `GITHUB_TOKEN` الافتراضيّ لا يقرأ `branches/*/protection` فسوءُ الإعداد يُنتِج غياباً لا رفضاً. **ولم تكن أوّل صياغةٍ للطفرة تزرع شيئاً** — تبديلُ صنف الاستثناء يلتقطه `except OSError` التالي فيبقى الفشل مغلقاً؛ صُحِّحت لتزرع العطل فعلاً — يُسقِط `test_an_unreadable_protection_file_fails_closed`
- استجابةُ خطأٍ من GitHub قد تكون قائمة أو نصّاً؛ قبولُها كائناً يُنتِج `AttributeError` غامضاً أو — أسوأ — قراءةً جزئيّة. استجابةٌ لا تُفهَم فشلٌ لا قبول — يُسقِط `test_a_non_object_response_is_rejected`
