...
- نزعُ إدانة خصائص الدور ⇒ كلّ ادّعاء عزلٍ تحته يمرّ مجّاناً، والأدلّة تبقى خضراء وهي لا تقيس عزلاً — يُسقِط `test_every_forbidden_role_attribute_is_rejected_on_its_own`
...
- إسقاط `rolcreaterole` من العقد ⇒ أخطر مسار تصعيد يصير غير مقروء أصلاً: دورٌ يحملها يمنح نفسه عضويّة أدوارٍ أخرى فيبلغ مستأجِرين آخرين **بلا** superuser وبلا BYPASSRLS — أي أنّ أقصر طريق كان الوحيد الذي لا يراه — يُسقِط `test_the_forbidden_set_covers_the_four_escalation_paths`
- نزعُ كتابة المصنوعة ⇒ الأدلّة تعيش في سجلّ الرَّنَر وحده فتفنى معه، فيصير «قِيس على هذا الالتزام» ادّعاءً غير قابل للمراجعة — يُسقِط `test_the_attestation_is_written_even_when_the_verdict_is_failure`
...
- إعادة الصيغة التي أفلتت `ADD COLUMN IF NOT EXISTS`: يُلتقَط `IF` بوصفه اسم العمود فيمرّ الاسم المحظور. والنمط يظهر ٢١ مرّة في هجرات هذه الخدمة — فأرجح طريقٍ إلى العطل كانت الوحيدة التي لا يراها الحارس — يُسقِط `test_add_column_if_not_exists_does_not_slip_through`
