...
# ٨) المتّجه الذي يخفيه Linux: ترميز لغة الآلة (§٣.٩)
...
### ٣.٤ حلّ التعارض له **صنفان**، وخلطهما يفسد الشجرة
...
### ٣.٥ `Capability-Impact` يُحسَب **بعد** الدفع
...
### ٣.٦ `brain_commit_claim_guard`: ذكر معرّف فجوة = ادّعاء وجودها
...
### ٣.٧ `brain_state_transition_guard`: مفردات الإغلاق تحتاج شيفرة خارج الدماغ
...
### ٣.٨ `no_report_only_change_guard`: تغيير تقاريريّ صرف يُحجَب
...
### ٣.٩ ترميز لغة الآلة — العطل الذي يخفيه Linux
...
### ٣.١٠ اختبار بلا علامة اختبارٌ ميت
...
### ٣.١٢ ميزانية المسارات وموضعها القانونيّ
...
### ٣.١٣ `%G?` لا يقيس وجود التوقيع — يقيس قدرتك على التحقّق منه
