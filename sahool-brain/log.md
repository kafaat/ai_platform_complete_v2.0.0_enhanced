...
## 2026-07-28 — ARCH-TESTS-UNLISTED-IN-CI-01: من 37/54 إلى 52/55 موصولاً

- **العلّة:** `pytest.ini` يحصر `testpaths` في `tests_v9`، فكلّ ما تحت `tests/` يُشغَّل بقائمة مسارات **يدويّة** في الـworkflows. والقائمة اليدويّة تبيت بصمت: ملفّ جديد يقع خارجها ولا شيء يقول ذلك.
- **الفرز قبل الإدراج:** شغّلتُ الـ١٧ فوجدتُ **١٤ تمرّ نظيفة** و**٣ تفشل**. الإدراج الأعمى كان سيُحمِّر CI، والإعفاء الأعمى كان سيُخفي أربعة عشر اختباراً صالحاً. الأوّل أدرجتُه (٩٤ اختباراً تمرّ في بيئة مطابقة لوظيفة CI)، والثاني أعفيتُه بسببه وشرط إغلاقه.
- **الثلاثة المُعفاة ليست صنفاً واحداً:** اثنان يفشلان **لأنّهما صادقان** — الأثر المُلتزَم يدّعي جاهزيّة لا تسندها الشجرة (`PATH3-READINESS-CLAIM-UNBACKED-01`، ينتظر قرار المالك)؛ والثالث يؤكّد على **بيئة المُشغِّل** فيفشل على أيّ آلة غير المولِّدة.
- **الحارس أمسكني قبل أوّل تشغيل:** أدرجتُ اسم اختباره في الـworkflow **قبل أن أكتبه**، فالتقطه كشف «الأسماء البائتة». اسم بائت في workflow يمرّ أخضر بلا تنفيذ شيء — وهو نفس عائلة العلّة التي يعالجها.
- **وأوصلتُه بنفسه** (`test_this_guard_is_itself_wired`): مكنسة #691 واختبارها وقعا خارج CI بعد بنائهما. حارس اكتمالٍ خارج الاكتمال لا يحرس شيئاً، وهذا الفخّ وقع فيه غيري مرّتين اليوم.
- **وصار ادّعاء `hot.md:1` صادقاً:** كان يقول إنّ ثلاثيّة عدّ المسارات «يفرضها» اختبار خارج CI؛ صحّحتُ النصّ أمس إلى «يفحصه يدويّاً»، واليوم أُدرِج الاختبار فعلاً — فأعدتُ «يفرضه» لأنّها صارت وصفاً لا ادّعاءً.
- **خطأ تشغيليّ متكرّر:** استرجعتُ حقن اختبار التكذيب بـ`git checkout --` على ملفّ **غير متتبَّع**، فلم يفعل شيئاً وبقي المدخل المحقون. أزلتُه برمجيّاً بتأكيد صريح. هذه ثاني مرّة اليوم يخذلني فيها `git checkout` كأداة تراجع — أوّلها محا تعديلاً غير مُلتزَم، وثانيها لم يمحُ ما أردت.
- **دليل رابع سُجِّل ولم يُصلَح:** `path3-runtime-verification.yml` و`runtime-verification-promotion.yml` (`workflow_dispatch:` فقط) يُنشئ لهما GitHub تشغيلات `push` تفشل عند الإقلاع على **كلّ فرع بما فيه main** — ١١٣ تشغيلة على الأوّل — **ولا تظهر في فحوص أيّ PR**. خارج نطاق هذا الحارس عمداً (يفحص اختبارات لا workflows)، ومُسجَّل كي لا يبقى ملاحظةً في محادثة.

## 2026-07-28 — UNIT-TEST-DORMANCY-01: إيقاظ ٤٧٩ اختبار وحدة + نقض قرار #590
- **الهدف:** إغلاق `UNIT-TEST-DORMANCY-01` بعدما رفع `29809dc` (إغلاق `APP-ROUTES-EMPTY-01`) حاجبَها الوحيد المتبقّي.
- **لماذا كان النقض مشروعاً لا انقلاباً:** قرار #590 اشترط أنّ «إضافة fastapi تُوقظ خامدةً **تفشل**». تحقّقتُ من الشرط بدل افتراض بقائه: ١١ من الإخفاقات كان سببها ترقية fastapi غير المثبَّتة (0.140.13) لا الإيقاظ نفسه، والباقي `scipy` و`pillow` — وscipy مُعلَنة أصلاً في `api/requirements.txt:20`. القرار كان صحيحاً يوم اتُّخِذ؛ سقط شرطه لا منطقه. سُجِّل النقض صراحةً في `decisions/ledger.md` مع إحالة متبادلة على مدخل #590 (`⟲ نُقِض`)، لا محواً له.
- **القياس التفاضليّ (بيئة نظيفة من `requirements-test.txt` وحدها، نفس أمر CI):**
  ...
  الفارق ٤٧٩ — مطابق للتقدير المسجَّل ولقياس المالك رقماً برقم. (كلّ ملفّ خامد كان يُحتسَب **تخطّياً واحداً** لا عدد اختباراته؛ هذا يفسّر لماذا لا يساوي فرق التخطّيات فرق النجاحات.)
- **لزوم كلّ تبعيّة مُثبَت بالإسقاط:** أسقطتُ `pillow` وحدها وأعدتُ التشغيل ⇒ فشل واحد بعينه (`test_segmentation_frontend_contract_20260702::test_segment_request_accepts_frontend_field_names`). وبلا `scipy` يتوقّف الجمع كلّه عند `api/trial_engine.py:28`. لم أُضِف تبعيّة «احتياطاً» — الاحتياط في ملفّ تبعيّات ادّعاء بلا دليل.
- **التثبيت على إصدار الإنتاج لا على نطاق مفتوح:** `fastapi==0.136.3` · `scipy==1.13.1` · `pillow>=10.0.0`. ولنفس السبب أُزيل `fastapi` غير المثبَّت من `raster-validated-product.yml:22` — كان يُرضيه الحلّ اليوم صدفةً، وهذا بالضبط شكل الفخّ الذي فجّر `APP-ROUTES-EMPTY-01`.
- **فحص الثغرات قبل الإضافة (قاعدة CLAUDE.md):** `pip-audit` على الملفّ قبل التعديل وبعده ⇒ **النتيجة نفسها حرفيّاً** (`ecdsa 0.19.2 / PYSEC-2026-1325`، قائم سلفاً عبر `python-jose[cryptography]`، بلا إصدار مُصلِح). الإضافات الثلاث لم تُدخِل استشارة واحدة. وتحقّقتُ من `pip_audit_resolution_guard`: الثلاث تطابق تثبيت الأسطول (٢٨ ملفّاً على `0.136.3`) ⇒ لا `ResolutionImpossible` جديد.
- **`requirements-field-forms.txt` حُذِف:** وجوده كان لعزل fastapi عن البيئة المشتركة؛ زال السبب فزال الملفّ. الخطوة باقية لأنّ ثلاثة من ملفّاتها الأربعة **بلا `pytestmark = unit`** ⇒ يستبعدها `-m unit` (40 نجحت مُجمَّعةً — شُغِّلت معاً عمداً، قاعدة «ناجح منفصلاً ≠ ناجح مُجمَّعاً» من #590 نفسه).
- **التغطية — ما لا يقوله الرقم:** 44.55٪ ⇒ 46٪، لكنّ المقام تضاعف ثلاثاً (20655 ⇒ 60870 عبارة) لأنّ وحدات fastapi صارت تُستورَد فتُحصى. `cdse_client.py` حتّى **نزلت** 66٪⇒56٪ للسبب نفسه — لا انحدار اختبار، تغيّر مقام. المكسب في الأصفار: `auth/main.py` 3٪⇒50٪ · `auth/routers/*` 0٪⇒14–62٪ · `field_state_gateway.py` 29٪⇒100٪ · `field_state_projection.py` 44٪⇒85٪ · `supervisor-agent/main.py` 0٪⇒37٪. الأرضيّة 40⇒42 بقاعدة `floor(المقيس)-~4` المُعلَنة في `docs/testing/coverage_ratchet.md` — لا اجتهاداً في اللحظة.
- **حدّ الصدق المكتوب:** انتهى سُبات **الوحدة** وحده. لم يُغلَق `AUTH-E2E-UNDER-RESTRICTED-ROLE` ولا `MCP-PREAUTH-STATUS-01` (تكامل، لا يمسّهما هذا)، ولا `APP-ROUTES-INTROSPECTION-COUPLING-01` — التثبيت رفع الحاجب ولم يفكّ الاقتران بـ`app.routes`.
- **فرع سابق لنفس الشريحة كان قائماً على الريموت — فحصتُه قبل أن أدفع فوقه:** `origin/claude/wake-dormant-unit-tests` (٣ التزامات على أساس `4eded7a`، أي قبل دمج #682–#686). رفض الدفع لم يكن عطباً شبكيّاً فلم أُعِد المحاولة عمياء؛ قارنتُ الفرعين. عمله مُتجاوَز في معظمه (حاجبه أُغلِق على main بطريقة أخرى)، لكنّه حمل **قطعة تنقصني**: `tests_v9/test_unit_environment_completeness.py` — حارس ضدّ عودة الخمود الصامت. نقلتُها بدل إعادة اختراعها، وأثبتُّها بالتكذيب (إسقاط `pillow` يُفشِلها). ونقلتُ وسمَي `xfail` لاختباري التكامل المستيقظَين بعد **تصحيح معرّف ميت** فيهما: أحدهما كان يستشهد بـ`CI-RLS-SUPERUSER-ROLE-01` وهو المعرّف المكرّر الذي حُذِف في #685 ⇒ صار `AUTH-E2E-UNDER-RESTRICTED-ROLE`. **الفرع القديم لم أدفع فوقه ولم أحذفه** — قرار المالك.
- **الأثر الذي كاد يفوتني:** وظيفة *Integration Tests* تُثبّت `requirements-test.txt` نفسه (`ci.yml:611`)، فإيقاظ الوحدة يُوقظ التكامل أيضاً. اكتشفتُه من فرع الوكيل السابق لا من تحليلي — ولولاه لدفعتُ شريحةً تُحمِّر وظيفةً لم أفحصها. لا أستطيع تشغيل `-m integration` محلّيّاً (تحتاج Postgres+PostGIS+Redis)، فالحكم النهائيّ لـCI: أدفع وأقرأ النتيجة بدل أن أفترضها.
- **الحارس أسقط التزامي — وكان محقّاً بقاعدته، مخطئاً بمقصده:** `brain_commit_claim_guard` طالب بتسجيل `PYSEC-2026-1325` كفجوة. الشكل مُطابِق (ثلاثة مقاطع كبيرة بشرطات)، والصنف مختلف: معرّف استشارة تُصدره جهة خارجيّة. لو تُرِك، لدفع الحارسُ كلَّ رسالة إلى **كتمان** رقم الاستشارة — عكس غرضه بالضبط. وُسِّع الاستثناء صنفيّاً (CVE/GHSA/PYSEC/OSV). **والأهمّ:** نسختي الأولى كانت `startswith("CVE-")` وكانت ستبتلع معرّف فجوة اسمه `CVE-LIKE-BUT-NOT` — **التقطه اختبار التكذيب الذي كتبتُه للاتّجاه الذي لم أتوقّع فشله**، قبل الدفع. هذا عكس عطبَي هذه الجلسة (`registry_ids()` على نصف الملفّ، واختبار يؤكّد توقّع مؤلّفه): الفارق الوحيد أنّي هذه المرّة فحصتُ الاتّجاه المضادّ أوّلاً.
- **اكتشاف جانبيّ سُجِّل لا أُجِّل (`ARCH-TESTS-UNLISTED-IN-CI-01`):** أثناء تشغيل `tests/architecture/` فشل `test_compose_runtime_target_resolver::test_generated_artifacts_match`. تحقّقتُ قبل أن أنسبه إلى تغييري: `git stash` ثمّ تشغيل على شجرة `origin/main` النظيفة ⇒ **يفشل هناك أيضاً**. البحث كشف الآليّة: `pytest.ini` يحصر `testpaths` في `tests_v9`، وكلّ ما تحت `tests/` يُشغَّل بقائمة مسارات صريحة — ١٧ من ٥٣ ملفّاً في `tests/architecture/` خارجها، بينها حُرّاس بنيتُها في هذه الجلسة. وأخطر ما فيه أنّ `hot.md:1` كان يقول إنّ ثلاثيّة عدّ المسارات «يفرضها» اختبار **لا يعمل في CI** — صُحِّح النصّ إلى «يفحصه يدويّاً». لم أُدرِج الـ١٧ في هذه الشريحة: أربعة تفشل (واحد انحراف أثر حقيقيّ، اثنان يؤكّدان على بيئة التوليد نفسها فيفشلان على أيّ آلة أخرى) ⇒ الإدراج الأعمى يُحمِّر CI. سُجِّلت كفجوة بعلاجها المقترح بدل تركها في نصّ جلسة.
- **بوّابة أثر القدرات حجبت — وسببها أنّ الشريحة نمت بعد فتح الـPR:** أعلنتُ `INT-004, IRR-003, IRR-009` عند الفتح، ثمّ أضفتُ استنقاذ القرارات وإصلاح الحارس ووسم `soil-service`، فصار الأثر يمسّ تسع قدرات. البوّابة تقرأ **جسم الـPR** وقت التشغيل (`capability-governance.yml:87` عبر `github.event.pull_request.body`) لا رسالة الالتزام — وتعديل الجسم وحده **لا يُعيد تشغيل CI**، فيلزم تعديل الجسم ثمّ دفع التزام. **القاعدة:** إعلان الأثر ليس شيئاً يُكتب مرّة عند الفتح؛ يُراجَع مع كلّ التزام يُضاف، وإلّا صار الإعلان بائتاً بصمت. تحقّقتُ محلّيّاً بالاتّجاهين قبل الدفع: الثلاثة ⇒ BLOCK، والتسعة ⇒ PASS بلا `unaffected_declared`.
- **الحارس التقط عطباً في استخراجه هو — والأخطر أنّه كان يعمل في اتّجاهين متعاكسين:** رسالة الالتزام تذكر `لـAUTH-E2E-UNDER-RESTRICTED-ROLE`. الحرف العربيّ حرف كلمة، فلا `\b` قبل `AUTH`، فبدأ التطابق بعد أوّل شرطة: طالبني بتسجيل `E2E-UNDER-RESTRICTED-ROLE` (وهميّ) **وفاته** `AUTH-E2E-UNDER-RESTRICTED-ROLE` (الحقيقيّ). أي أنّ الحارس كان يُفوّت كلّ معرّف مذكور بالصيغة العربيّة الطبيعيّة — وهي الغالبة هنا؛ فما بدا إزعاجاً كان ثغرة في التغطية. الحلّ حدّ صريح `(?<![-A-Za-z0-9_])` يقبل العربيّة ويرفض الشرطة، على **الحارسَين** لأنّ النمط مكرّر فيهما. لم أُصلح الحالة بتعديل نصّ رسالتي.
- **وأخطأتُ في التوقّع مرّتين متتاليتين داخل اختبار التكذيب نفسه:** أكّدتُ أنّ `XAPP-ROUTES-EMPTY-01` ثمّ `APP-ROUTES-EMPTY-01X` يجب أن تُرفَض، وسقط التأكيدان واحداً تلو الآخر. كلاهما رمز صالح الشكل؛ الضمانة المقصودة **قراءة كاملة لا رفض**. صحّحتُ التوقّع لا النمط — ولو صحّحتُ النمط لأرضي اختباري لأضعفتُ الحارس لإرضاء خطئي، وهو عين ما تحرسه هذه المنظومة.
- **CI ردّ أحمر، والفشلان مختلفان جوهريّاً:** (١) **تكامل** — اختبار ثالث لم أُعلّمه (`test_services_functional`) سقط بنفس عيب `AUTH-E2E-UNDER-RESTRICTED-ROLE` لكن في `soil-service` لا `auth`. هذا هو بالضبط ما قلتُ إنّ CI سيقوله ولا أستطيع قوله محلّيّاً؛ وسمَاي السابقان عملا (`2 xfailed`) والمجموع كان **فشلاً واحداً**. (٢) **`capability-registry`** — عيبي أنا: `capability_mapping_engine` ليس في سلسلة إعادة التوليد المُوثَّقة، وأيّ ملفّ **جديد** يُدرِجه في `unmapped_artifacts` فتسقط البوّابة.
- **لم أطارد المولِّد الساقط:** كنستُ **الثمانية والثلاثين مولِّداً** كلّها بوضع `--check` — نفس درس «الكنس غطّى ٤٦ من ٢١١ استدعاءً». المسح أعطى أربعة انحرافات، وكان الفرز بينها هو العمل الحقيقيّ: ثلاثة **قائمة على main سلفاً** وتُثبَّت على بيئة التوليد (إصدار بايثون، نصّ خطأ Docker) ⇒ إعادة توليدها تبصم بيئتي على المستودع، فتُركت لـ`ARCH-TESTS-UNLISTED-IN-CI-01`.
- **خطأ قياس صحّحتُه فوراً:** فحصتُ انحراف main بـ`git stash` — و`stash` لا يُلغي الالتزامات، فكنتُ أقيس فرعي وأسمّيه main. النتيجة الأولى قالت إنّ الأربعة قائمة سلفاً (أي لا ذنب لي)، وهي **مريحة وخاطئة**. أعدتُ القياس على worktree منفصلة على `origin/main` فانفصل الأربعة إلى واحد لي وثلاثة قائمة. الراحة في نتيجة قياس سببٌ لإعادة فحص القياس لا لقبوله.
- **جرد الفروع بطلب المالك («احذف ما لا عمل فريد فيه») — لم أحذف شيئاً، والسبب ليس تردّداً:** `git push --delete` يُجاب بـHTTP 403، ولا أداة حذف فرع في GitHub MCP المتاح. لكنّ القياس نفسه أنتج ما هو أهمّ من النظافة. جرّبتُ ثلاثة اختبارات وأسقطتُ اثنين بصراحة: الأسلاف يُثبت فرعين فقط (المستودع يدمج بالضغط)، و`merge-tree` يتعارض على ٤١٩ فرعاً وكلّ التعارض في الآثار المولَّدة. الثالث — مقارنة محتوى المصدر مع التحفّظ عند أيّ اختلاف — أثبت **٤ فروع** خالية فقط، واستثنيتُ `develop` رغم خلوّه لأنّه فرع تكامل قائم لا شريحة.
- **ما غيّر جواب المهمّة:** `claude/brain-session-close` كان يحمل **٨ أسطر في `decisions/ledger.md` غائبة عن main**، منها **قراران كاملان** — ربط النواة الرابعة بأمر المالك **بعد اعتراضي المُسجَّل** (#682)، وتقسيم شريحة الإيقاظ بدل وسم ١١ اختباراً بـ`xfail` (#681⇒#683). لو حذفتُ الفرع «لأنّ شيفرته مدموجة» لمحوتُ قرار مالك بسببه. استُنقِذا **حرفيّاً** لا مُعاد صياغتهما. **القاعدة المستخلَصة:** الدمج بالضغط ينقل ما في الـPR لا ما كُتب على الفرع؛ فلا يُحذَف فرع قبل مقارنة سطريّة لملفّات الدماغ عليه.
- **ضريبة التسجيل:** حذف ملفّ متتبَّع + تعديل `ci.yml`/`requirements-test.txt` ⇒ أُعيد توليد السلسلة كاملةً بالترتيب (impact-index → execution-dependency → static-governance → route-binding → `build_release_bundle` **أخيراً**)، لا تحرير بصمة يدويّاً.
...
1. **إصلاح زرّ «3 أشهر» في MapHub** (PR #629، `9e0967d`): كان يجلب 24 شهراً — الجذر أنّ التواريخ المُعالَجة غير محدودة بـ`months` في `available-dates`؛ أُضيف قطع `months` لكلّ التواريخ عند `include_provider` (`services/raster-service/routers/fields.py`).

2. **المزامنة التلقائيّة 6h→24h مثبَّتة على وقت الالتقاط السابق** (PR #629): `scan_all(lookback_days=10, min_hours_since_last_capture=24.0)` بحارس تخطٍّ لكلّ حقل + عدّاد `skipped` + `_parse_capture_time` (`imagery_automation.py`)؛ و`scan_new_imagery` 21600→86400 (`scheduler.py`).

3. **قفل نسخة-واحدة للمُجدوِل** (PR #630، `5cd063a`): `cluster_singleton` عبر `pg_try_advisory_lock(hashtext($1))` يلفّ 4 مهامّ دوريّة — يُغلق فجوة «المُجدوِل بلا قفل موزَّع».

4. **بطاقة الأصناف (VarietyCatalogPage)** (PR #632): أوّل مستهلك UI للكتالوج المحكوم `GET /api/v1/varieties/food-grains[/{id}]` — مرشّح محصول مشتقّ + موسّع نَسَب + وسم `reference_only_not_operational` + عرض `quality_issues`. **ترقية إعفاء→core:** نقطتا varieties من `endpoint_ui_coverage_waivers.json` (52→50) إلى `core_endpoints` (463→465) لأنّ الواجهة أعطتهما دليلاً حقيقيّاً (وإلّا فشل `test_no_waiver_has_real_ui_evidence`)؛ pin الكتالوج 52→50.

5. **إغلاق ست فجوات كود** (PR #632، `71ddbc8`):
   - **B1** `vegetation-analysis/all_fields`: حلقة تسلسليّة (N×15s) → `asyncio.gather` بتزامن محدود `Semaphore(8)` + `zip(strict=True)` — نفس النتائج، زمن الجدار = أبطأ سلسلة.
   - **B2** `routers/fields.py` تتبّعيّة: غلّة الحصاد = **أحدث** نتيجة مُسجَّلة (`ORDER BY outcome_recorded_at DESC LIMIT 1`) لا `MAX` (كان يتحيّز صعوديّاً مع حصاد جزئيّ/إعادة تسجيل).
   - **B3** `outcome_reconcile.py`: فكّ العدّ المزدوج في `by_kind`/`success_rate` بمفتاح `decision_id` (أولويّة outcome_record عبر setdefault؛ صفوف بلا مفتاح تُعدّ منفردةً) + اختبار يقفل عقد «لا عدّ مزدوج».
   - **F1** دبابيس استكشاف MapHub **دائمة على الخادم** (نمط SatellitePage): `useScoutingPins`+`useCreateScoutingPin` بإدراج تفاؤليّ وتراجُع عند الفشل؛ «مسح» يزيل غير المحفوظ فقط. حارس ساكن جديد `MapHubScoutingPinsPersistence.static.test.ts`.
   - **F2** تعليقات scouting بائدة صُحِّحت في `MapHub.tsx`+`HubMap.tsx`.
   - **F3** حذف استيرادَي `lazy()` ميّتَين في `App.tsx`.

6. **PR #628 (Dependabot) فُصِل:** postcss يبقى، react-router v7 **مُؤجَّل** (ignore major) — دَيْن هجرة كاسر عبر 21 ملفّاً.

**درس مثبَّت (ضريبة التسجيل):** **أيّ** تعديل مصدر متتبَّع (Python أو واجهة) يغيّر LOC/أرقام أسطر ⇒ يجب إعادة توليد الجرد (`generate_service_inventory.py --write-registry`) **وحزمة الإصدار** (`build_release_bundle.py`) معاً — تعثّرتُ بفحص `drift`+`Repository Structural Lint` بعد تعديلات B1/B2/B3 (LOC انزاح: decision +11، vegetation +8، platform +1) وأُصلح بـ`000e809`. تعديل `main.py` وحده يضيف artifact ثالثاً: `platform_main_subinventory_guard.py --write`.

**Ratchet:** 68 فحصاً success/skipped على `000e809` + `mergeable_state:clean` ⇒ دُمج squash `10a8fae`.
...
- **A7 — طبقة الحدود الإداريّة المرجعيّة (v200، أوّل جدول shared-reference معلَن):** الدراسة المقارنة رصدت A7 غائباً (محافظات/مديريّات نصّ حرّ لا هندسة). **القرار الحاكم (المالك):** `admin_boundaries` **مرجع مشترك** (طبقة خريطة عامّة لكلّ المستأجرين، لا بيانات مستأجِر) — أوّل جدول public-read معلَن. سابقتان داخليّتان خفّضتا كلفة القرار من «استثناء» إلى «ثاني تطبيق»: `weather_automation_locations` (جدول عالميّ موثّق، test_rls_fk_isolated_tables) وST_AsSVG حيّ في gis-workflow (قارئ A6). **التصميم (٥ إجابات + شرط):** بلا `tenant_id` (لا RLS-مستأجِر — لا معنى)؛ **قراءة-عامّة/كتابة-محمِّل:** sahool_app **SELECT فقط** (REVOKE INSERT/UPDATE/DELETE في bootstrap+apply — طبقة مرجعيّة تتغيّر بلا provenance = انجراف صامت، فلسفة append-only للأدلّة)؛ **provenance إلزاميّ** (`admin_boundaries_source`: source/dataset_version/license_title/license_url/url/retrieved_at + feature_count + invalid_geometry_fixed) — لا حدّ بلا مصدر؛ **سلامة الهندسة كسمة:** trigger BEFORE INSERT/UPDATE يرفض `NOT ST_IsValid` (**برهان سلبيّ**)؛ **المالك = gis-workflow-service** (كاتب/محمِّل، readers=["*"] — ملكيّة الكتابة لا حكر القراءة). **المُحمِّل** `services/gis-workflow-service/load_admin_boundaries.py` (أداة مشغِّل: يقرأ HDX/OCHA GeoJSON admin1+admin2 → ST_IsValid/ST_MakeValid + عدّ المُصلَّح → provenance لكلّ مستوى + ON CONFLICT idempotent؛ يلتقط الرخصة/الإصدار/وقت الجلب كما لحظتها). **قناة واحدة: كلّ البيانات بالمُحمِّل أو لا (لا seed في git — يجمّد لقطة تنفصل عن المصدر).** admin1+admin2 معاً (٢٢ محافظة + ٣٣٣ مديريّة، تافه لـPostGIS). provenance بديل بيئيّ: البنية الآن، بيانات HDX يحمّلها المشغّل (شبكة محجوبة + حجم). **برهان حيّ (PG16+PostGIS):** v200 يُطبَّق نظيفاً · trigger يحجب هندسة bowtie غير صالحة · هندسة صالحة تُدرَج. **حارس** `tests_v9/test_v200_admin_boundaries_static.py` (9: shared-reference بلا tenant_id · شكل الجدول+FK · trigger برهان سلبيّ · SELECT-only في المُشغّلَين · ملكيّة gis-workflow قراءة-عامّة · المُحمِّل يتحقّق الهندسة+الرخصة · لا كتابة تطبيقيّة · تزامن MANIFEST/runner). **بوّابات:** ruff · `-m unit` 3267 · production_validation_gate · مسح استباقيّ كامل · bundle 4656. **ملاحظة RLS:** حارس التغطية يفحص جداول tenant_id فقط ⇒ shared-reference (بلا tenant_id) يمرّ طبيعيّاً (لا يحتاج INTENTIONAL_GLOBAL — تلك لجداول tenant_id العالميّة). **التالي: A6** (خريطة SVG للطباعة، تستهلك A7 + نقطة قراءة الحدود).
- **A6 — خريطة الطباعة المتجهة (يفكّ حظر A7؛ يُغلق الحلقة التي بدأت بسؤال «ما فائدة Map2SVG؟»):** الاكتشاف التأسيسيّ: `bulletin_figure.py` كان يحمل self-check `admin_geometry_present` **يتخطّى دائماً** («لا حدود ⇒ ليست خريطة») — إعلان صادق. A6 لا يضيف ميزة بل **يفكّ حظراً صادقاً**: الشرط نفسه يبقى، والتخطّي الصادق يبقى عند الغياب (fail-closed لا يُزال بل يُروى). **الانقلاب ذو الاتجاهين (المالك: الأخفى والأهمّ):** حدود حاضرة ⇒ `passed=True` · غائبة ⇒ **يبقى None** (لا `False` أبداً — الغياب حالة degraded لا فشل، لا خريطة مزيّفة تولد). **المُجمِّع** `services/gis-workflow-service/svg_print_map.py` (وحدة صرفة): SVG بطبقات **مستقلّة** `<g id="admin/field/indicator">` (شرط Map2SVG — يفتحها مصمّم في Illustrator ويعدّل طبقة دون طلب) + **تذييل إسناد ثنائيّ اللغة مشتقّ من سجلّ مرجعيّة A7** (source/version/license_title/retrieved_at) لا نصّ ثابت — تبدّل الرخصة عند إعادة تحميل A7 ⇒ تبدّل التذييل تلقائيّاً (إسناد قانونيّ ذاتيّ، CC-BY تُلزِمه). **الشرط الرابع (المانع الأحدث والأخفى — كتم الخصوصيّة على المتجه):** الشكل التصنيفيّ يحجب المكتوم نصّاً، لكن choropleth قد يُسرِّبه **باللون**؛ فوحدة `suppressed_for_privacy` تُرسَم هندستها **بلا قيمة بيانات** — نمط «محجوب» (hatch) + `class="suppressed-no-data"` متميّز عن أيّ فئة NDVI — يمتدّ `privacy_floor` من النصّ إلى المتجه. **القراءة** `services/raster-service/routers/fields.py::GET /gis/admin-boundaries` (توكن خدمة، bbox مُطهَّر: أرقام + سقف مساحة 200°² — حارس ضدّ bbox وحشيّ يجرّ الطبقة) + `gis_boundaries_read.py` (ST_AsGeoJSON+ST_AsSVG، ST_Intersects على GIST). **مشروعيّة القراءة موثَّقة:** raster يقرأ admin_boundaries كـ**استهلاك shared-reference معلَن** (readers=["*"]، A7) لا تسلّل p4 — مسجَّل صراحةً في db_ownership.yml + gis_boundaries_read. **الحُرّاس الأربعة** `tests_v9/test_a6_print_map.py` (5): انقلاب الاتجاهين (حاضرة⇒True·غائبة⇒None لا False) · بيانات حدود حقيقيّة + طبقات مستقلّة · تذييل من المرجعيّة يتبدّل · **كتم الخصوصيّة على المتجه (برهان سلبيّ: مسار المكتومة بلا لون البيانات)** · مطهِّر bbox. + تسجيل `/gis/admin-boundaries` في SERVICE_ONLY (حارس تغطية auth راستر). صفر انحدار (bulletin 36 أخضر، التوقيع متوافق للخلف). **بوّابات:** ruff · `-m unit` 3272 · مسح استباقيّ كامل + ui-contract 32/32 · bundle 4659. **الحلقة اكتملت:** طبقة حدود موثَّقة المصدر (A7) + خرائط طباعة متجهة بإسناد قانونيّ ذاتيّ + كتم خصوصيّة على المتجه (A6). **بند الدراسة المقارنة A6/A7 (P2) مُغلَق.** المتبقّي من الخطة: SIM-GOLDEN-01 محجوب بصدق (بيانات حقول حقيقيّة).
- **السجلّ التشغيليّ #3 — حارس انجراف env↔compose (صنف عضّ مرتين، فأُغلِق):** httpx غير المعلَن + JWT_SECRET المقروء بلا تزويد (اكتُشِفا بالصدفة) = «كود يقرأ env وcompose لا يزوّدها». `scripts/ci/env_compose_drift_guard.py` (نطاق ذكيّ: المقروء **بلا افتراضيّ** فقط — getenv/environ[]/environ.get بلا default؛ ذوات الافتراضيّات خارج النطاق) يطابق ضدّ compose **و**.env.example (الموثَّق-لا-في-compose = «محقون» لا «مفقود»). **التطوّر تقرير→حارس (شرط المالك ④):** بُدِئ تقريراً (32 مفقود جُرِدت)، ثمّ **راجعنا كلّ عنصر** — بما فيها 5 مشتبَهات فُحِصت سطراً-سطراً (soil_render→None شفّاف صادق + capability honest؛ weather redis/rate-limit ⇒ الافتراضيّ 'memory' يبقى فعّالاً لا يُعطَّل، status مكشوف؛ WORKFLOW_WORKER_ID⇒uuid فريد آمن؛ ACTUATOR_IDEMPOTENCY_MODE⇒'local' الافتراضيّ الآمن؛ **SAHOOL_ALLOW_RLS_BYPASS_ROLE⇒enforcement_active fail-closed: غيابه=لا تجاوز أبداً+رفض الإقلاع على دور يتجاوز RLS**) — **كلّها «تنحدر بوضوح» أو «افتراضيّ آمن»، صفر كسر صامت، صفر إصلاح لازم**. صُنِّفت الـ32 في قائمة استثناء مُراجَعة كمراجعة كود (`env_compose_drift_allowlist.json`، كلّ مدخل {category A/B/C/D, why مع file:line}: A رايات · B أسرار غيابها=الوضع الآمن ولا في compose · C عناوين بنية/مُنتقيات بيئة · D ضبط أداء بافتراضيّ آمن). قُلِب حازماً: `--check` يُحمِّر (exit 1) على أيّ مفقود غير مُصنَّف؛ مركّب في ci.yml (env-compose-drift-guard بعد compose-env-contract-gate). حارس `tests_v9/test_env_compose_drift_guard.py` (6: النطاق الذكيّ ببرهان سلبيّ · env.example=محقون · كلّ مدخل مُبرَّر · الشجرة نظيفة · **برهان سلبيّ: متغيّر جديد غير مُصنَّف يقع في «مفقود»**). **درس:** الحارس ذو الأسنان يحتاج برهاناً سلبيّاً (متغيّر مُصطنَع يُحمِّره) وإلّا هو زخرفة.
- **درس (تكرار) — تعديل ci.yml يكسر بوّابة مانيفست الإصدار:** بوّابة `Release manifest checksum gate (mirror of main-only production gate)` (`validate_release_package.py --root .`) تُحمِّر على أيّ checksum mismatch لملفّ مشمول بالمانيفست — و`.github/workflows/ci.yml` **مشمول**. أضفت خطوة env-compose-drift-guard (#3) فتغيّر checksum الـci.yml ⇒ فشلت البوّابة على الفرع (6917316). **الإصلاح:** `python3 scripts/release/build_release_bundle.py --root .` ثمّ validate (4665 checksum أخضر) = db99c27. **التعميم:** المسح الاستباقيّ قبل الدفع يجب أن يشمل build_release_bundle+validate **كلّما لُمِس أيّ ملفّ مشمول بالمانيفست — ci.yml خصوصاً**، لا فقط عند إضافة خدمة. (الحارس نفسه مرّ أخضر في تلك الدورة.)
- **بند (ج) مُقفَل — قائمة التشغيل الرئيسيّة لبنود ① (الكود جاهز، ينقص تنفيذ حيّ):** `docs/runbooks/OPERATOR_DISPATCH_BACKLOG_LIVE_EXECUTION.md` — فهرس مرتَّب بالخطر التصاعديّ (①-1 دفن الفروع صفر خطر ← ①-2 عامل الإبطال منخفض عكوس ← ①-3 SoR-promotion عالٍ لا يُعكَس ← ①-4 تحقّقات+DEM)، كلّ بند بأربعة حقول (شرط مسبق · أمر · برهان نجاح · تراجع) ويربط الرنبوكات الجاهزة لا يكرّرها. **قرارات المالك:** (١) ①-2 قبل ①-3 بحجّتين — إحماء يد المشغِّل على البيئة + فصل أنماط الفشل (عامل الإبطال = اختبار دخان مقنَّع كعمل منتج)؛ (٢) **مرحلة صفر إلزاميّة في ①-3: بروفة staging خضراء على نفس SHA** — تُعاد إن تغيّر أيّ migration/سكربت (درس «الشهادة تُربَط بالـSHA»). **اكتشاف مُصحِّح:** عامل الإبطال موصول أصلاً في compose (docker-compose.v9.yml:1038) فالبند تحقّق نشريّ لا بناء. **الخطر الترتيبيّ الوحيد:** SoR — schema→backfill+verify(0 quarantine)→⛔حاجز→قلب الملكيّة؛ backfill قبل القلب لا العكس. مسار المواصفة: spec→review→adopt (المسودّة رُوجِعت في المسوّدات قبل الأرشفة). **الباقي بعد (ج):** لا شيء code-actionable — فقط ما ينتظر البيئة (①) والموسم/البيانات (②) والمحفّزات (③).
- **بند (ج) — تنقيحان أخيران بعد مراجعة المالك (قائمة التشغيل مُغلَقة نهائيّاً):** (١) ①-1 أُضيف **تحقّق ما قبل الحذف خاصّ بـ`claude/code-review-34hO3`**: `git log --oneline main..origin/<b>` — فارغ/مدموج ⇒ حذف آمن؛ أيّ commit غير محصود ⇒ توقّف+cherry-pick ثمّ احذف (الفرع كان +22 commit وحصاده تقرير لا برهان). (٢) ①-3/ج أُضيف **توضيح أنّ قلب ملكيّة `db_ownership.yml` تغيير مستودع (PR+CI+regen) لا خطوة خادميّة** — الحُرّاس الساكنة تقرأه والانجراف يُحمِّر البوّابات. المانيفست أُعيد بناؤه (تطبيق درس ci.yml↔المانيفست). **بهذا يُقفَل السجلّ التشغيليّ كلّه: لا بند معلّق على قرار.**
- **SEASON-RECORD-01 (v201) — سجلّ الموسم المُدار، مفتاح فكّ حجب SIM-GOLDEN (بُنِي بعد spec→review×3):** خمسة جداول (season_records/crop/events/harvest/cost_items) بـtenant_id + **RLS+FORCE+سياسة عزل** على الخمسة، يملكها **scout-ingest-service** (Q1: كاتب واحد، لا خدمة جديدة، درس #180) — نفس بوّابة الثقة untrusted→accepted و**append-only (لا DELETE أبداً؛ REVOKE DELETE في المُشغّلَين، يبقى INSERT/UPDATE للقبول)**. المبدأ: لا بيانات مُخترَعة (NULL صريح، low_confidence لا تخمين). الهندسة ملك سجلّ الحقول (field_id FK لا polygon، Q3/قرار③). تعدّد عملات آمن (ISO 4217 CHECK، YER افتراضيّة، لا تجميع بلا سعر صريح — Q4). **calibration_eligible = VIEW مُشتقّ لا عمود مُولَّد** (انحراف مبرَّر: GENERATED لا يعبر ثلاثة جداول) بشروط المالك الثلاثة: **① security_invoker=true** (وإلّا يتخطّى RLS ويسرّب عبر المستأجرين — فخّ v198) · **② لا عمود مطابق** (مصدر حقيقة واحد) · **③ توثيق الانحراف في الرأس**. **برهان حيّ كامل على PG16:** POSITIVE (موسم A→eligible=true) + NEG (المستأجِر B يرى 0 في الجدول والـVIEW ⇒ security_invoker · 5 قيود مرفوضة · DELETE permission denied · الـVIEW non-editable). حارس `test_v201_season_records_static.py` (12، الشروط الثلاثة + append-only ثنائيّ-المُشغّل). حارسا تزامن المُشغّلَين وتغطية RLS خضراوان. pytest -m unit=3290. **درس:** الـVIEW متعدّد-الجداول يحتاج security_invoker صراحةً (نفس فخّ DEFINER/RLS الذي حُلّ في v198) — لا يُفترَض. **الفجوة المسجَّلة:** SIM-PATHS-DUAL (أيّ محرّك محاكاة يُعايَر ضدّ season_harvest.yield_kg_ha — يُحسم عند SIM-GOLDEN-01).
- **درس حاسم — البرهان الحيّ بـstub لا يطابق الواقع = عيب مُقنَّع (v201 أوقع CI رغم «برهان حيّ» محلّيّ):** بنيتُ برهان v201 على stub `fields(id uuid PK)`، لكنّ `fields` الحقيقيّ `field_id VARCHAR(50) PK` (v9_foundation.sql:61). فالـFK `REFERENCES fields(id)` مرّ محليّاً (stub) وفشل في CI (`ERROR: column "id" ... does not exist`). **الإصلاح:** `field_id TEXT NOT NULL` بلا FK صارم — نمط الشقيق v199 (اقتران فضفاض عبر-خدمات؛ fields ملك المنصّة). **التعميم:** البرهان الحيّ لتبعيّة مشتركة يجب أن يُطبَّق على **السلسلة الحقيقيّة** (v1..v201) أو stub **مطابق للمخطّط الفعليّ** (نفس اسم/نوع PK) — لا stub مريح. وظيفة التكامل في CI (تطبّق السلسلة كاملة) هي الحكَم النهائيّ.
- **SEASON-RECORD-01 v201 — إغلاق نطاق UPDATE (سؤال المالك ①: هل yield يُعدَّل بعد القبول؟):** الجواب **رفض بـtrigger** لا سماح — UPDATE على مدخلات `calibration_eligible` بعد `trust_status='accepted'` يُسمّم سلسلة المعايرة (الرقم يتغيّر والأهليّة تبقى true). أُضيف: `season_child_immutable_after_accept` (يجمّد season_harvest+season_crop عند قبول الأب) + `season_records_acceptance_immutable` (trust_status لا يرتدّ عن accepted + audit accepted_by/at مُجمَّد). التصحيح = إصدار موسم مُبطِل. **برهان حيّ PG16:** yield قابل للتعديل قبل القبول → بعد القبول UPDATE على yield/sowing_precision/trust_status/accepted_by **كلّها مرفوضة**، وnotes (لا مدخل معايرة) يبقى قابلاً للتعديل. حارس ساكن مُحدَّث (13 اختباراً).
- **WATER-SALT-02 — محوّل AquaCrop للمسار الملحيّ (Maas-Hoffman الداخليّ، §5 معتمدة من المالك):** `services/agriai-engine/aquacrop_adapter.py` محوّل صرف يعكس wofost_adapter + `salt_profile` (ec_e/ks_salt/leaching). **Maas-Hoffman ثابت** (Ks FAO-56 Eq.81 + غسيل Eq.82)، بلا حزمة aquacrop/numba (مؤجَّلة §5-2). **قاعدة توجيه بعتبة ECe صريحة** (§5-1): `ec_e>=2.0` ⇒ المحرّك الملحيّ وإلّا PCSE — مصدر حقيقة واحد. راية `AQUACROP_ENABLED` default-off؛ إنتاج بلا الحزمة = فشل مُغلَق مُصنَّف؛ provenance دائماً `aquacrop_uncalibrated` (§5-3، المعايرة حكر SIM-GOLDEN). عقد `AQUACROP_SALT_CAPABILITY` + حارس (7 ببراهين: عقد سلبيّ · توجيه · رتابة ECe↑⇒غلّة≤ · إنتاج fail-closed). **انحراف صدق عن §Q2:** المواصفة طلبت نقل «لا نقل ملح زمنيّ» من limits لcovers، لكنّ Maas-Hoffman **ثابت لا ديناميكيّ** فأُبقي في limits (لا يُدّعى نقل زمنيّ قبل حزمة aquacrop) — نفس انضباط security_invoker/field_id. حُسِمت حالة الملوحة في SIM-PATHS-DUAL.
- **SEASON-RECORD-ENTRY-01 — واجهة إدخال سجلّ الموسم (شريحتان 2a+2b، بُنِيت بعد spec→review، معتمدة بأربعة تعديلات + إضافة ملزمة):** الجسر الذي يحوّل أساس v201/v202 إلى **أداة ترقيم**. **الشريحة 2a (نواة أمان نقيّة):** تصديق الحافّة HMAC (`compute/verify_edge_attestation` في trusted_tenant، ±120ث ضدّ إعادة اللعب، `SEASON_EDGE_HMAC_KEY` فئة B) + `require_attested_reviewer` (gateway_deps: 401 unattested/stale · 403 non-reviewer) + سلامة المرفق (`shared/security/season_logbook`: magic-bytes JPEG/PNG/PDF لا الامتداد، حجم بعد الاستلام 10MiB، مفتاح خادميّ `season-logbooks/<tenant>/<season_id>/<sha>.<ext>` يضمّ season_id، فحص ملكيّة المفتاح، presign TTL≤300). **الشريحة 2b (النقاط الستّ + المخزن):** `season_api.py` على scout-ingest (POST مسودّة idempotent على draft_key/v202 · PATCH يرفض accepted بـ409 من النقطة · POST logbook خدميّ يتدفّق لملفّ مؤقّت لا الذاكرة · GET presigned · POST accept مُصدَّق+دور+برهان-وجود-كائن HEAD · GET list) — **صفر مسار منصّة** (route-residual أخضر). **مخزن عامّ جديد `shared/storage/blob_store`** (bytes-in/key-out، يعيد استخدام S3_* المشترك بلا متغيّر جديد، scheme صريح من S3_USE_SSL لا http صامت، بوّابة تدهور S3_ALLOW_FILE_FALLBACK، fail-closed بلا مفاتيح، سقف TTL داخليّ، object_exists يرفض file:// في الإنتاج) — **الحدّ:** blob_store = التخزين العامّ الوحيد؛ object_store = تخصّص raster/COG. **منح v202:** sahool_ingest SELECT/INSERT/UPDATE على الجداول الخمسة (لا DELETE) في المُشغّلَين (كان بلا وصول). **10+ براهين حيّة PG16 (المدخل الحقيقيّ):** 401 بلا توكن/تصديق/توقيع-باطل · 403 بلا دور · 415 magic كاذبة · 413 >10MB · 404 B-يطلب-presign-موسم-A (لا تسريب وجود) · 422 logbook_missing لمرجع ميّت · 409 PATCH-بعد-accept عبر مسار القبول الحقيقيّ · عزل RLS عابر المستأجرين · idempotency · no-DELETE. حارس ساكن AST (9) + `test_season_live.py` (integration، CI_INTEGRATION_REQUIRED يقلب skip⇒fail — درس fields.id). pytest -m unit=3321.
- **درس شامل (رابع من العائلة نفسها) — «يعمل محليًا» لأنّ المحليّ أكرم من CI؛ السكربت قبل الدفع يحاكي CI لا البيئة المحلّيّة:** الشريحة 2 أوقعت أربع وميضات من أمّ واحدة: (1) مانيفست الإصدار · (2) lint+format · (3) requirements-completeness (pydantic غير مُعلَن صراحةً) · (4) **مسار CI**: `test_season_api_static` استورد season_api (⇒fastapi) في طبقة `-m unit` **بلا fastapi**، و`sys.path.insert(دليل الخدمة)` جعل `shared.storage` هشّاً (المرشّح ①: استيراد من داخل دليل الخدمة ⇒ `No module named 'shared'`). **الإصلاح:** حارس AST ساكن يفحص النصّ بلا استيراد (نمط الحُرّاس الساكنين القائم) — **برهان أمين: حجب fastapi فعليّاً ثمّ pytest ⇒ 9 passed** (لا «يعمل عندي»). **إضافةً — ضريبة التسجيل (درس #180 مُوسَّع):** إضافة نقاط/تبعيّات لخدمة تُشعِل ~6 حُرّاس مولَّدات (service/route/health/dep-conflict inventory + dep-bundle) + عقد compose-env (.env.example) + عقد service-feature-UI — تُعاد كلّها **في التزام الشريحة نفسه** لا كمتابعات. **القاعدة المُثبَّتة:** قبل الدفع: `ruff check --fix + ruff format` (ملفّات ملموسة) → regen كلّ مولَّد مسّه التغيير → pip-audit عند تبعيّة → أيّ حارس جديد يُختبَر بـ`cd` لدليل الخدمة (محاكاة عدم-وجود-جذر-CI) → ثمّ push.
- **SEASON-RECORD-ENTRY-01 الشريحة 3 (جزئيّة، مُقفَلة عند حدّ أخضر) — 3a + 3b-core:** **3a (تقييد الوجهة، تعديل المالك ①):** تصديق الحافّة HMAC صار يغطّي `(user_id, roles, method, path, body_sha256, timestamp)` لا الهويّة فقط — تصديقٌ صُكّ لمسار بريء لا يُعاد لعبه على `.../accept`. الخدمة تحسب method/path/body من طلبها المُستلَم لا من ترويسة. براهين وحدة (cross-path/method/body-tamper ⇒ 401 · وجهة ناقصة fail-closed) + برهان حيّ PG16 (تصديق مسار PATCH ⇒ 401 على accept). `6158688`+`81a9fc4` (الثاني: مزامنة جرد بسبب إزاحة أرقام سطور accept — درس: أيّ إضافة أسطر لملفّ فيه مسارات مسجَّلة تُحرّك أرقام السطور ⇒ انجراف جرد؛ regen في التزام الشريحة). **3b-core (نموذج المُراجِع، قرار المالك):** auth دور مفرد (RBAC) فلا مطالبة roles قابلة للمنح بعد؛ `season_reviewer_roles_for` يشتقّ سلطة `season-reviewer` لـ**{owner, expert} فقط** — القبول فعل زراعيّ لا تشغيليّ، **admin مستثنى عمداً** (الاستثناء مُوثَّق بجانب `SEASON_REVIEWER_SOURCE_ROLES` — هو القرار الذي يسأل عنه قارئ مستقبليّ). برهان لكلّ دور. `19c4e63`. **قرار الإقفال:** توقّف عند الحدّ الأخضر — بوّابة auth+nginx (أخطر شيفرة السلسلة: تجريد ترويسات + حقن توكن + سلسلة ثقة ثلاثية) لا تُبنى في ذيل جلسة طويلة (نفس أمّ دروس الجلسة: «قطعة صغيرة على عجل»). **الاستئناف:** 3b-infra (نقطة `/auth/edge-sign` + nginx: تجريد X-Canonical-*/X-Edge-* + مسار قانونيّ داخليّ + auth_request + حقن توكن الخدمة؛ البرهان السلبيّ الثالث «X-Canonical مزوّرة من عميل لا تصل auth» يتطلّب nginx حيّاً لا وحدة) ثمّ 3c (الواجهة). فجوة مسجّلة: SEASON-REVIEWER-GRANT-MODEL (المحفّز: أوّل مُراجِع ليس مالكًا ولا خبيرًا — مُدقّق تصدير خارجيّ).
- **SEASON-RECORD-ENTRY-01 الشريحة 3 مُكتمِلة (3a + 3b-core + 3b-infra + 3c) — الجسر صار أداةً حيّة:** **3b-infra (البوّابة، أخطر شيفرة السلسلة):** نقطة `services/auth/routers/season_edge_sign.py` (`GET /auth/edge-sign`، طلب فرعيّ لـnginx: يتحقّق JWT ⇒ يشتقّ سلطة {owner,expert} ⇒ يوقّع HMAC مقيَّد الوجهة على method/path القانونيّة التي **تُعلنها** nginx + هاش الجسم الفارغ؛ fail-closed: وجهة ناقصة 401 · مفتاح غير مُهيّأ 503). nginx (`nginx.v9.conf`): `map $request_uri $season_canonical_path` **مصدر قانونيّ واحد** يُغذّي X-Canonical-Path لـauth **و** هدف proxy_pass معاً (شرط المالك ② — لا انحراف) · upstream `scout_ingest_backend` · `/_auth_edge_sign` داخليّ يكتب X-Canonical-* بنفسه ويُفرِغ ترويسات العميل قبل التوقيع (شرط ③ — المزوَّرة لا تصل auth؛ البرهان الحيّ مؤجَّل لـstaging، فجوة SEASON-EDGE-LIVE-PROOF + مهمّة #225) · مسار القبول (أضيق، قبل العامّ) `auth_request /_auth_edge_sign` + `auth_request_set` يرفع التصديق + حقن توكن الخدمة + لا تسريب القانونيّة upstream (شرط ①). `SEASON_EDGE_HMAC_KEY` يبقى فئة B (لا في compose؛ في allowlist). **ضريبة التسجيل — درس جديد:** المسار الجديد أوقع CI مرّتين: أوّلاً 6 مولَّدات inventory (d9f5aa4)، ثمّ **`route_mount_contract_guard` (المولّد السابع، فاتني)** أوقع «Repository Structural Lint» و«Runtime Real Smoke» (047486b). **القاعدة المُوسَّعة:** قائمة regen قبل الدفع ليست ستّة بل **سبعة** (+ route_mount_inventory). حارسان ساكنان (`test_season_gateway_nginx_static.py` 6 · `test_season_edge_sign_endpoint_static.py` 4). **الطرف الأخضر:** `047486b` (Unit Tests + Runtime Real Smoke + Repository Structural Lint كلّها ✅).
  **3c (الواجهة، الأخيرة):** `frontend/src/sections/SeasonRecordEntryPage.tsx` تدفّق متدرّج (رسم حقل عبر `AddFieldWithMap` → مسودّة+محصول → رفع دفتر → مراجعة+تسليم للقبول) يعيد استخدام `StepShell`؛ عميل `services/api/season.ts` (النقاط الستّ عبر kongApi على `/api/v1/seasons` النسبيّ، nginx يوجّه؛ رفع الدفتر بايتات خام لا multipart كي يعمل كشف magic-bytes) · استمرار `lib/seasonDraftStorage.ts` بـdraft_key عبر الجلسات (نفس مفتاح تكرار الخلفيّة) · توجيه App.tsx+routes.ts (قسم العمليّات)+permissions.ts (owner/manager/agronomist؛ worker/viewer محجوبان) · حارس ساكن `SeasonRecordEntry.static.test.ts` (10). **حدّ نطاق صادق (لا نصف حلّ):** جداول events/harvest/cost_items موجودة (v201) لكن **بلا نقاط API** (2b بنت الستّ فقط) — فبناء نماذج تُرسِل للعدم ممنوع؛ صُرِّح في خطوة المراجعة كـ«شريحة لاحقة» + فجوة SEASON-ENTRY-EVENTS-UI. تحقّق: typecheck + vitest كامل (187 ملفّ/1275) + service-feature-ui 32/32. الطرف: `9f92c7d`.
  **الرَّنبوك القابل للنسخ (طلب المالك):** `sahool-brain/runbooks/season-record-entry.md` — تفعيل الميزة + خريطة الثقة + البراهين الحيّة المؤجَّلة (curl جاهز) + استكشاف الأعطال.
...
## 2026-07-20 (تكملة) — التوفيق: دمج فرع البناء في main (اتحاد 7 hardening + \gexec) + 3 إصلاحات CI
- **الاكتشاف الحاسم:** فرع البناء `claude/code-review-34hO3` كان **أحمر على CI** منذ 12:53 رغم خضرته محليّاً — تحقّقتُ من CI الفعليّ (Actions API) قبل الدمج فوجدتُ وظيفتَي *Unit Tests* و*Integration Tests* حمراوين. **لم أدمج شفرة حمراء في main.**
- **العطبان (من إصلاح \gexec نفسه):** (١) `test_v200`/`test_v201` حارسان ساكنان يطابقان `REVOKE … ON <table> FROM` حرفيّاً؛ `\gexec` يكتب `ON TABLE public.<t>` (وv201 على قائمة VALUES مُقنَّعة بـ%I) ⇒ لا تطابق. وُسِّعا للتسامح مع `[TABLE] [public.]`/الكتلة المُولَّدة (يفرضان النيّة، والقابليّة للتنفيذ يبرهنها الاختبار الحيّ). (٢) `test_role_bootstrap_executable` قرأ `DATABASE_URL` (رابط socket `?host=/tmp/postgres` لا يعمل في وظيفة Integration) ⇒ فُضِّل `TEST_DATABASE_ADMIN_URL` ثمّ `TEST_DATABASE_URL`، و`_split_conn` يعزل `?query` قبل rpartition. **إصلاحان دُفِعا للفرع (a59b998, fa6a128) → CI أخضر تماماً.**
- **الدمج (b01c75b، `--no-ff`، أبوان fbaccd2+fa6a128):** تعارضات migrate ⇐ `\gexec` · inventory/release ⇐ أُعيد توليدها من شجرة الدمج (4700 checksum). **عطب ثالث طفا عند الدمج:** `season_gateway_live_gate.py` كان يُضمّن `CERT_NONE` (الفرع افتقر `tls_policy`) ⇒ وصلته بـ`shared.security.tls_policy` المركزيّة (يُرضي `test_tls_policy`).
...
## 2026-07-21 — axios 1.17→1.18 (أمنيّ): #586 مرفوض (لغم رجعيّ) → #589 نظيف مدموج
- **الطلب:** "اخضر ادمج" على PR #586 (Dependabot: axios bump). لكنّه لم يكن جاهزاً.
- **اكتشاف اللغم:** #586 قاعدتُه الفرع البائد المدموج `claude/code-review-34hO3` (لا main). عند إعادة استهدافه إلى main تبيّن أنّ رأسه يحمل **نسخة أقدم** من ملفّات جسر ERP (`health.py`/`sync.py`/`v9_odoo_bridge.sql`) — دمجه كان **سيدهس عمل #587**. أُثبِت ببصمات فعليّة (main `3a96d624`/`8ce8c9be` مقابل رأس #586 `3d9675c4`/`86b3daa8`). لم يُدمَج. **درس: تحقّق من قاعدة PR + فرقه الفعليّ قبل الثقة بـ«أخضر».**
- **الحلّ النظيف (#589):** أعدتُ تطبيق ترقية axios **معزولةً** على فرع من main (ملفّان فقط: package.json + package-lock.json). CI الأوّل أحمر: 3 بوّابات checksum (`Lint & Format`/`release-package`/`pytest-contracts`) لأنّ ملفّي frontend مُتتبَّعان في `FILE_CHECKSUMS.sha256` (ضريبة التسجيل). أعدتُ توليد الحزمة (4716 بصمة) → **الـ58 فحص أخضر على `76999c4`**.
- **الدمج:** squash → main `e3ff38f`. أُغلِق #586 كمُستبدَل. القيمة: تصلّب رؤوس إعادة التوجيه عبر الأصول + رفض روابط مشوَّهة (axios #10892/#11000).
- **SHA:** axios `76999c4` (رأس أخضر) · دمج main `e3ff38f`.

## 2026-07-21 — PR #590 (field-forms API integration tests P1): إخضار معماريّ → دُمِج
- **الغاية:** اختبارات field-forms API حيّة عبر TestClient.
- **العطب المعماريّ:** إضافة fastapi إلى `requirements-test.txt` المشترك أيقظت ~15 اختباراً خامداً (`importorskip("fastapi")` مُعلَّمة `unit`) تبني `api.main` كاملاً ⇒ فشل في البيئة الدنيا (مسارات فارغة/PIL/superuser-RLS). main أخضر لأنّها كانت تُتخطّى (لا fastapi).
- **الحلّ (الخيار أ المُنقّح — المالك + وكيله):** (١) `requirements-test.txt` أُعيد مطابقاً لmain ⇒ الخامدة تنام. (٢) `requirements-field-forms.txt` (fastapi فقط) + **خطوة CI معزولة** في وظيفة Unit Tests بعد بوّابة التغطية تشغّل ٣ ملفّات صراحةً بالمسار (36 اختباراً) — تغطية حقيقيّة بلا أثر جانبيّ. رقعة `ci.yml` دفعتُها أنا (توكن المالك بلا صلاحيّة workflows).
- **عطب خفيّ أصلحتُه (لم يكن في الرزمة):** `test_field_forms_sync_binding_p02` كان يفشل عند تشغيل الثلاثة **معاً** (لا منفردة): `test_field_forms_api_integration` يضبط `FIELD_FORMS_SYNC_HMAC_KEY="it-secret"` أوّلاً، فـ`setdefault` لا يتجاوزه، لكنّ التوكن كان يُصدَر بسرّ مُثبَّت مخالف ⇒ فشل معتمِد على الترتيب. جعلتُ الإصدار يقرأ نفس مفتاح البيئة ⇒ مستقلّ عن الترتيب (36/36 معاً). درس: «41/41 محليّاً» كان تشغيلاً منفصلاً؛ التشغيل المُجمَّع كشف العطب.
- **البصمات:** 4718. الـ58 فحص أخضر فعليّاً على `8f5634e` (الخطوة المعزولة: 36 passed مؤكَّد من السجلّ).
- **SHA:** رأس أخضر `8f5634e` · دمج main `35a4ae0`.

## 2026-07-21 — PR #591 (ADR-0035 relocation): نقل حكم PHYSICS-AI-CALIBRATION-01 لموقعه الاتّفاقيّ → دُمِج
- **التغيير:** نقل الحكم المصادَق من `docs/architecture/ADR_PHYSICS_AI_CALIBRATION_01.md` (أُرشف #588) إلى `docs/adr/ADR-0035-physics-ai-calibration.md` (الترقيم المتسلسل بعد ADR-0034) + فهرسته في `docs/adr/README.md` + حذف القديم. وثيقة صرفة.
- **الفرع كان متأخّرًا** (قاعدته `14da286` قبل #590) و**بصماته لم تُحدَّث** — دمجتُ main الحاليّ فيه + أعدتُ توليد الحزمة (نقل: -architecture +adr، صافي 0 → 4718).
...
## 2026-07-22 — PR #593 (field-forms Slice 2: mobile UI + BFF proxy + guards): تشخيص جذرَين → دُمِج
- **المصدر:** الفرع الأساسيّ غير المدموج `claude/field-forms-01-slice2` (شقيق #592 الذي كان compose-only). صولح مع main المدموج أوّلاً (دُمِج `caface2` + استُعيد `docker-compose.v9.yml` مطابقًا بايت-ببايت) ⇒ الفرق صار **الميزة فقط** (23 ملفًّا: 17 Flutter + service_proxy.py BFF + حارسان + بصمات).
- **CI كشف أربع بوّابات حمراء على الرأس القديم `fe4636b` — جذران متمايزان:**
  1. **drift + Lint & Format + Repository Structural Lint:** مسار BFF الجديد في `service_proxy.py` أُضيف بلا إعادة توليد جرد الخدمات + الحزمة، + ملفّ اختبار غير مُنسَّق. أُصلِح في `edbeff8` (جرد: 32 خدمة/1082 مسار · ruff-format · bundle 4737).
  2. **Platform Unit Tests — ليست drift إطلاقًا (السجلّ الفعليّ حسم):** فشلان بالضبط في حارس P0 `test_p0_platform_route_ownership_guard`: (أ) `test_every_platform_route_has_explicit_target_owner` — مسار `proxy_field_forms` غير مُسجَّل في `platform_extraction_map.json`؛ (ب) `test_platform_route_budget_does_not_grow` — عدّاد مسارات المنصّة نما 614→615. أُصلِح في `c4e0f68`: سُجِّل المسار `route_key = api/routers/service_proxy.py::GET,POST /api/field-forms/{path:path}::proxy_field_forms` بـ`target_owner: scout-ingest-service` (نمط `proxy_soil→soil-service`) + رُفِع `baseline_route_count` 614→615 + أُعيد بناء الحزمة. اختبار p2_6 غير متأثّر (يعدّ ديكوريتر الطريقة الواحدة فقط، لا `api_route`).
- **درس:** «Platform Unit Tests أحمر» ليس بالضرورة drift — اسحب سجلّ الوظيفة الفعليّ قبل الحدس. الحدس الأوّليّ كان drift؛ السجلّ كشف حارسَي ملكيّة مسار مختلفَين تمامًا. أيّ مسار BFF جديد على المنصّة يستلزم قيدًا في خريطة الاستخراج + رفع الميزانيّة بوعي.
...
## 2026-07-22 — PR #596 (72h infra-hardening pack): تصليب RLS+odoo+redis → دُمِج + #594 أُغلِق
- **المصدر:** فرع `claude/infra-hardening-72h` (الوكيل المحلّي بدأه). صُولح بالكامل: rebase على main (`d7de40e`) + تصليب compose + إصلاح v206.
- **v206_rls_final_hardening.sql** (migration جديدة، آخر مدخل، خطوة runner 211): fail-closed للكتابة بلا `app.current_tenant` + تأكيد catalog (ENABLE+FORCE+WITH CHECK). **علّتان أُصلِحتا بالتتابع (كلاهما «الحيّ يفحص التنفيذ»):**
  1. الحلقة طابقت سياسات `tenant_isolation` على جداول **بلا عمود tenant_id** (عزل أبويّ: `field_lifecycle_transitions ← field_lifecycle.tenant_id` عبر `lifecycle_id`) فأعادت كتابة WITH CHECK إلى `(tenant_id = …)` ⇒ `column "tenant_id" does not exist`. الحلّ: `IF NOT FOUND THEN CONTINUE` بعد بحث العمود (متّسق مع تأكيد catalog). **مُتحقَّق حيًّا على PG16** (تخطّي صحيح + تشديد صحيح + 0 أخطاء).
  2. **عقد أمنيّ قائم انكسر:** `test_no_context_write_allowed` (على main) يؤكّد أنّ الكتابة بلا سياق مسموحة على جداول tenant، وv206 يزيل هذا المهرب عمدًا. **الحسم (قرار المالك، Option A):** أُعيدت تسمية الاختبار إلى `test_no_context_write_blocked_fail_closed` يؤكّد الرفض — متّسق مع عقد fail-closed القائم (fii v192 + external_submissions v197 يفرضان الرفض أصلًا؛ الهجرات=sahool_user superuser والوظائف=sahool_jobs BYPASSRLS تتجاوز RLS، فلا كاتب مقيَّد شرعيّ يحتاج المهرب). المبرّر: الثغرة نفسها (دور runtime نسيَ set_config يكتب لمستأجر آخر).
- **odoo دور مقيَّد:** `USER: ${ODOO_DB_ROLE:-odoo_app}` + `depends_on: sahool-migrate` — `odoo_app` يُنشئه `apply_in_compose.sh` (LOGIN NOBYPASSRLS CREATEDB + REVOKE CONNECT من PUBLIC). `.env.example` يُعلن المتغيّرين.
- **redis-state أمنيّ:** #595 (المدموج) يضع **auth + guardrails** على `sahool-redis-state` (تصحيح: نصّ #595 سمّاهما «platform+edge» خطأً؛ الفعليّان auth+guardrails — وهما هدف الحارس بالضبط). فالاتّحاد كان مُرضيًا سلفًا بعد الـrebase.
- **تصحيح MANIFEST كارثيّ سابق (الوكيل المحلّي):** commit `850d2f8` استبدل MANIFEST بقائمة مُختلقة (205 مدخلًا، 197 بلا ملفّ)؛ استُعيد الحقيقيّ من main + v206 (`f241f04`/`c2edc61`).
- **الدرس المتكرّر:** «Lint & Format» تشغّل أيضًا بوّابة checksum (`validate_release_package`) — فأيّ تعديل ملفّ مُتتبَّع بلا إعادة توليد الحزمة يُسقِط ٣ بوّابات (Lint&Format + release-package + pytest-contracts) لا واحدة.
...
## 2026-07-23 — IRR-F01 دورة حياة الحجز + field-forms Slice 3 عميل Flutter #602 — مدموج (`main` = `7606901`)
- **السياق:** المالك رفع حزمة تكامل كاملة (لقطة، أساس `3f2a010`، عنوان `IRR_F01_field_forms_integrated`) وطلب «المراجعة وتنفيذ ما هو صحيح». راجعتُ كلّ ملفّ متغيّر مقابل الأساس (لا ثقة بالتقرير)، ونفّذتُ الصحيح فقط، مع استبعاد الضجيج (`.pytest_cache`) وإعادة توليد الجرد/الحزمة بنفسي.
- **الدلتا المُطبَّقة:**
  - **IRR-F01 (خلفيّة):** (١) خيط التتبّع correlation/causation يُنقَل داخل **payload** (لا عقد emit_event الثابت ذي الأعمدة الثمانية)؛ حمولات الإرسال + تعويض الفشل تحملهما، والـOutboxWorker يقرأهما من `row["payload"]`. (٢) تعبئة أدلّة تقييم السعة: `capability_digest` يُحفَظ، `maximum_safe_flow_m3h`/`remaining_allocatable_flow_m3h` من `derated_capacity_m3h` الموثوق. (٣) عامل جديد `api/irrigation_reservation_lifecycle_worker.py` (Pattern A: `JOBS_DATABASE_URL` عابر المستأجرين · `set_config('app.current_tenant',...)` لكلّ مستأجر · `expire_due` المحكوم v205 · `FOR UPDATE SKIP LOCKED`) كخدمة `sahool-irrigation-reservation-lifecycle-worker` opt-in تحت بروفايل `irrigation-runtime` (default-off، مُقسّاة: read_only/cap_drop ALL/no-new-privileges) + اختباراته. (٤) `mark_dispatch_failed` يكتسب `correlation_id` اختياريًّا مع fallback على `TypeError` للتوقيع القديم.
  - **Flutter Slice 3 (عميل — ملفّات copy-as-is التي كانت محجوبة):** `device_id_service.dart` (secure storage، `Random.secure()`، محفوظ عبر logout — `clearAuth` يحذف مفاتيح الجلسة الأربعة فقط لا `deleteAll`)، `field_forms_coordinator.dart` (Dio مشترك عبر `ApiService.dio`، download + تصريف FIFO، موصول عند بدء المصادقة/التخلّص/فتح الحقل)، `X-Device-Id` على الإرسال، + اختبارات Dart + حارس ربط بايثونيّ `tests_v9/test_field_forms_mobile_runtime_wiring.py` (unit).
  - **`.github/dependabot.yml`** لتحديثات github-actions أسبوعيّة.
- **تصحيحاتي (الصحيح لا النسخ الأعمى):**
  - **إرجاع تعديل `to_emit_args()`** الذي أضاف مفتاحَي correlation/causation: لا مستهلك يقرأهما (كلا المستدعيَين يقرآن بالاسم للأعمدة الثمانية) وكسرا الحارس `test_to_emit_args_matches_emit_event_contract`. أمسكته بوّابة `-m unit` الكاملة لا تشغيل الحزمة «المركّز».
  - إعلان `IRRIGATION_RESERVATION_LIFECYCLE_POLL_SECONDS` في `.env.example` (وإلّا `compose_env_contract_gate` يحجب — نفس فخّ #601).
- **قيادة CI للأخضر (3 رؤوس، كلّ حمراء ثغرة تسجيل):**
  1. `416b9c6` ⇒ 5 حمراء.
  2. `f3e7669`: (أ) **فكّ تجاهل** `field_forms_coordinator.dart` — قاعدة `.gitignore: data/` ابتلعت ملفّ lib شرعيًّا؛ الحارس المحلّيّ نجح (الملفّ على القرص) لكنّ CI (نسخة نظيفة) لم يجده ⇒ كسر Flutter + الحارس البايثونيّ ⇒ `git add -f`. (ب) رفع ميزانيّة وحدات المنصّة 652→653 + `modules[]` + note (حارس `test_p0_platform_module_growth_guard`). (ج) تسجيل العامل في قائمة `JOBS_DATABASE_URL` في **نسختين متوازيتين**: السكربت `scripts/security/rls_runtime_gate.py` واختباره `tests/security/test_phase12_final_production_gates.py`. (د) إعادة توليد `REPORT_INDEX.md`.
  3. `a8a3d5a`: خطأ Flutter الوحيد `missing_required_argument` — الدلتا جعلت `deviceId` إلزاميًّا على `FieldFormsApi.submit/download` لكن **مستدعٍ قائم خارج الدلتا** (`field_form_screen.dart::FieldFormCubit.sync`) لم يُحدَّث ⇒ جلب `DeviceIdService.instance.deviceId()` وتمريره (تحقّقت grep أنّ كلّ مستدعٍ يمرّره الآن).
- **الدروس:** (أ) `.gitignore` النمطيّ (`data/`) قد يبتلع ملفّ مصدر شرعيًّا ⇒ حارس محلّيّ يقرأ القرص يخدع؛ CI هو الحكم ⇒ `git add -f`. (ب) وحدة منصّة جديدة = عدّاد الميزانيّة + `modules[]` + قائمة `JOBS_DATABASE_URL` بنسختيها. (ج) تغيير معامل API إلى إلزاميّ ⇒ حدّث **كلّ** المستدعين بمن فيهم من خارج الدلتا؛ «Flutter Analyze & Test» في CI هو الشاهد الحقيقيّ. (د) تشغيل الحزمة «المركّز» يفوّت حُرّاس البوّابة الكاملة.
...
## 2026-07-23 — HISTORICAL-SEASON-BRIDGE-01 (v207) #605 دُمِج + تحقّق مراجعة مركز القرار (`main` = `bbfbf95`)
- **السياق:** المالك رفع حزمة `historical_season_bridge_v2` (أساس `9a218c7`) وطلب «اخضر». الأساس على main = `ccfa03b` (بعد #604 whole-tree ruff).
- **الدلتا المطبَّقة (الصحيح فقط، بإعادة توليد inventory/registry/bundle لا الوثوق بنسخ الأرشيف):**
  - `migrations/v207_historical_season_simulation_bridge.sql` (جديد): `season_record_links` (immutable، trigger يفرض `season_records.trust_status='accepted'`+تطابق tenant/field) + `season_simulation_runs` (append-only). **يُعلن RLS ذاتيّاً** ENABLE+FORCE+`tenant_isolation` عبر `public.sahool_effective_tenant_id()`. **مُدرَج قبل v206** في `MANIFEST.txt`+`run_migrations.sql` (v207 خطوة 212، v206 آخِراً 213) كي يبقى v206 آخِر ملفّ ويُعيد تغطية catalog RLS للجديد. `validate_migrations` (v206 آخِراً) + runner-sync 102 يمرّان.
  - `core/historical_season_context.py` (جديد) + تعديلات `season_simulation`/`season_models`/`agronomic_replay`(+router)/`seasons` router/`decision_service_client`/`frontend/src/services/api.ts` + 3 اختبارات.
  - `.env.example: HISTORICAL_SEASON_DECISION_CONTEXT_ENABLED=false` (default-off؛ مرآة decision-service مغلقة حتى برهان SoR حيّ).
- **تصحيحي المحلّيّ الوحيد (أحمر ruff/inspector):** حارس `tools/sahool_inspector.py::check_rls_coverage` false-positive على جدولَي v207 — استدلاله بحث عن السلسلة الحرفيّة `current_setting` بينما v207 (كـv206 نفسه) يستعمل المساعد القانونيّ `sahool_effective_tenant_id()` (v122). وُسِّع `has_cs` ليقبل المساعد أيضاً — **غير مُضعِف** (كلاهما مسنِد tenant صحيح؛ الحكم الرسميّ يبقى Integration Tests). أمسكه `test_no_false_positives_vs_green_ci_guards` محليّاً.
- **تصحيح تسجيل CI (رأس `243ec93`→`428e508`):** 3 حُرّاس *Platform Unit Tests* الحمراء تعيش في `tests/test_p0_*` **لا** في `pytest -m unit` المحلّيّ (الدرس المتكرّر — «الأخضر يقيس ما تختبره»): (أ) `db_ownership.yml` ينقصه جدولا v207 ⇒ أُضيفا كـ`sahool-platform`؛ (ب) ميزانيّة الوحدات 653→654؛ (ج) `core/historical_season_context.py` غير مُبرَّر ⇒ `modules[]`+`historical_season_context_note`. + إعادة توليد `release/`.
- **التحقّق:** محليّاً 4 حُرّاس المنصّة خضراء · `-m unit` 3400 · migration validate/runner-sync 102 · اختبارات الجسر 9 · ruff whole-tree نظيف · bundle 4800. CI: **كلّ 65 فحص success/skipped على `428e508`** (production-validation-gate · Repo Structural Lint · Platform Unit Tests · Structure Inspector · drift/contract/no-report-only-change · Security Scan) ⇒ دمج merge-commit `bbfbf95`.
- **معلّق بصدق:** تطبيق PG16 + برهان RLS بجلستَين على جدولَي v207 · مرآة decision-service SoR default-off حتى الشهادة. لا SoR موازٍ للموسم/المحاكاة.
- **تحقّق مستقلّ من مراجعة توصيل المحرّكات↔مركز القرار عند `9a218c7` (طلبها المالك، REQUEST CHANGES):** 4 وكلاء قراءة-كود عدائيّين مستقلّين، **الادّعاءات 11 كلّها CONFIRMED** بأدلّة `file:line`. جوهر: مرشّح القرار مبنيّ على مدخلات العميل (ET0/NDVI/تربة/طيف)، GDD وحده مجلوب خادميّاً (`routers/crop_twin.py:136`)؛ `_compose_state` لا يمرّر `weather_state` ⇒ heat/frost ميّتان + `crop_water=unavailable` (أقوى من «degraded»)؛ لا SoilProfileSnapshot/CanonicalWaterState/طيف موثَّق؛ سلطات قرار متوازية (`/decision`+`/profit-aware` تكتبان `decision_record` المنصّة authoritative، `run_field_intelligence.executable` يُفتح بـguardrails وحده دون سجلّ موافقة)؛ المركز default-off كـSoR (submit fail-closed 502). **أخطر نقطة:** البوّابة fail-closed سليمة لكنّ المسارات المتوازية تلتفّ حولها. حكمي: **أؤيّد REQUEST CHANGES — حجب الاعتماد الإنتاجيّ مبرَّر.** اقترحتُ فجوة `DECISION-CENTER-UNIFY-01` (المسار 3/4/5 قابل للتنفيذ الآن بلا migration) — بانتظار موافقة المالك.
- **SHA:** رؤوس `243ec93`→`428e508` (أخضر) · دمج `bbfbf95`.

## 2026-07-23 — تسوية تصنيفات السجل (بعد #616) + تجميد العمل الكوديّ العامّ
- جرد عدائيّ للبنود المؤجَّلة (وكيل + تحقّق يدويّ) ⇒ **صفر قابل للإغلاق من الكود وحده** دون خرق «لا نصف حلّ»/الصدق.
- بأمر المالك: صحّحت التصنيفات في `gaps/registry.md`: `SHARED-PACKAGE-NAME-COLLISION` OPEN→**ACCEPTED_RISK/WONTFIX** (المناعة القائمة كافية) · `DECISION-CENTER-UNIFY-01` المتبقّي وُسِم صراحةً **BLOCKED-DESIGN+RUNTIME** (عمل معماريّ غير منفَّذ، لا PG16 وحده) · قسم جديد `#CODE-CLOSABLE-DEFERRED-SWEEP` يحمل تصنيف المُحجِّبات الدقيق + تصنيف جرد المستهلكين.
...
## 2026-07-23 — consumer-routes (بإصلاحات) + الكتالوج الموحّد U0-U2 (فرع b734e81)
- حزمة consumer-routes: دُمجت بعد GO-WITH-FIXES (أصفار ملفَّقة HIGH + انتحال tenant بوّابة dev MED + اختباران خاملان MED + regex LOW) + v212 بدل تحوير v211.
- الكتالوج الموحّد: U0 مثبَّت (32/982/50/14-مقيس) · U1 (erp-bridge قانونيّ؛ 182 تعارضاً زائفاً ذاب؛ كُشف جدول IF الزائف) · U2 (مُصرِّف حتميّ + 8 مخرجات + بوّابة 9 اختبارات + خطوة CI).
- الحُرّاس كلّها نظيفة، السويت 3428، الحزمة 4832. U3–U9 شرائح قادمة.

## 2026-07-23 — الكتالوج الموحّد U3+U4: تكامل حزمة U3_U4_hardened المرفوعة (فرع claude/code-review-34hO3)
- تبنّي مُكيَّف داخل مُصرِّفنا (لا استبدال): بوّابة عقود UI مُقوّاة fail-closed (جذور+كلّ الأنماط، file:line، استبعاد .generated.، فرز حتميّ) + عقود v2 بـdispositions (consumed 29 / intentional-unconsumed agriai-engine بمُحفِّز SIM-GOLDEN-01 / standalone-job qdrant-seed+gis-workflow-service) ⇒ status.wired مدفوع بالأدلّة (القديم بقي runtime.compose_reachable).
- U4: قرارات التكرارات 14/14 في overrides (مفردات ثابتة + سبب + expires_on للمؤقّت + facade≠owner من أعضاء المجموعة؛ STAC→raster-service، plan→agriai-engine) + حَوكمة الإعفاءات الـ50 (مالك مُشتقّ من @services/<svc>/ أو السياسة + انتهاء + تتبّع).
- إصلاح مصدر: إزالة مدخل IF الزائف من db_ownership.yml ⇒ صفر تعارضات ملكيّة (أيّ جديد يُفشِل U3). تسجيل ملفّات الكتالوج في قائمة حزمة الإصدار.
...
- **الدمج:** PR #627 مدموج تحت Ratchet (64 فحصاً success/skipped على head `2246844`، mergeable clean) — main `1ce31df`.
  ضريبة الحوكمة الكاملة: تصنيف UI + waiver-ين (backlog-ui، priority=low، expiry 2026-10-31) · خريطة الاستخراج
  (route_count 627→629، new_max 623→625) · pins الكتالوج (982→984، waivers 50→52) · ميزانيّة المنصّة 623→625.
- **متابعة (مؤجَّلة، waiver ينتهي 2026-10-31):** واجهة React لبطاقة الأصناف تستهلك النقطتَين.

## 2026-07-25 — إصلاحات مسار الصور الثلاثة (PR #629 مدموج، main `9e0967d`)
- **1) الشريط الزمنيّ محكوم بالفترة (`services/raster-service/routers/fields.py:1417`):** أزرار MapHub
  (3/6/12/24 شهر) تستدعي `available-dates?include_provider=true&months=N`، لكن النقطة كانت تقصّ **تواريخ
  المزوّد فقط** بـ`months`؛ التواريخ **المعالَجة** (طبقات الذاكرة + `list_available_asset_dates`) تُعاد بلا قصّ
  ⇒ backfill سابق لـ24 شهراً يُبقي الشريط 24 شهراً مهما اختير 3/6/12. الإصلاح: عتبة `now − months×31` على
  **كامل** التواريخ حين `include_provider` فقط؛ منتقي المشهد (بلا provider) يبقى بلا قصّ (كلّ مشهد جاهز قابل
  للاختيار). حارس ساكن في `test_available_dates_provider_datetime.py` (3 اختبارات).
- **2) المزامنة التلقائيّة كلّ 24 ساعة من وقت التقاط الصورة السابقة:** `scheduler.py:175` كادينس
  `scan_new_imagery` 6→24 ساعة (`21600`→`86400`)؛ `imagery_automation.scan_all` حارس per-field — لا يُعاد
  فحص حقل إلّا بعد مرور 24 ساعة على `last_image_date`؛ حقل بلا وقت التقاط يُفحَص دائماً؛ فشل التحليل fail-open
  (يُفحَص لا يُتخطّى صامتاً)؛ عدّاد `skipped` + `_parse_capture_time` (ISO8601 + تاريخ خام، tz-aware). الأرشيف
  التاريخيّ يبقى عند الطلب. 3 اختبارات وحدة.
- **3) تسجيل دمج كتالوج الأصناف #627** في `hot.md`/`log.md` (بروتوكول الصيانة).
- **قيادة الأخضر (درس مُعاد تأكيده):** أوّل دفعة كسرت `Repository Structural Lint` (جرد drift) ثمّ
  `release-package`/`Lint & Format`/`pytest-contracts` (كلّها جذر واحد: `checksum mismatch: SERVICE_REGISTRY.md`).
  السبب: عدّلتُ الجرد لكنّ حزمة الإصدار تأخّرت التزاماً. **القاعدة: أيّ تعديل مصدر ⇒ إعادة توليد الجرد
  والحزمة معاً في نفس الدفعة، لا مُقسَّمَين.** الدمج: كلّ الـ67 فحصاً success/skipped على head `2fb724c`.
...
## 2026-07-26 — الطبقة الختاميّة PATH-3: سياسة التصديق الموقَّع (attestation policy) (`d52a6b6`, #656)
- **المصدر:** مرفق المالك `sahool_main_12c10fa_path3_attestation_policy.zip` — آخر شريحة PATH-3، superset لـ`path3_signed_attestation`. أدوات evidence-only فوق الـworkflow اليدويّ (`workflow_dispatch`، self-hosted). لا تلمس كود تشغيل ولا تدّعي verified/certified.
- **`path3_run_attestation.py`:** إنشاء/تحقّق تصديقات HMAC-SHA256 تربط plan_sha256 + بصمتَي compose (config/images) + tested_sha + environment_id + مجموعة الأدلّة المختارة. مفتاح التوقيع من env فقط (`PATH3_ATTESTATION_KEY`, ≥32 محرفاً) fail-closed — مفتاح ناقص/قصير مرفوض، وأيّ عبث بحقل موقَّع يُبطِل التوقيع.
- **`path3_attestation_policy.py`:** سياسة ترقية بيئة — نضارة (`--max-age-seconds`) + قائمة إبطال (`attestation_revoked`) + تطابق source→target (`environment_promotion_mismatch`). `promotion_record` لا يُصدِّق إنتاجاً أبداً (`promotion_scope=runtime_evidence_only`, `production_certified=false`).
...
## 2026-07-26 — دمج نظام Capability Parity & Investment (evidence-only) — parity_investment bundle
- **المصدر:** مرفق المالك `sahool_main_d52a6b6_parity_investment.zip` (لقطة كاملة؛ الدلتا = نظام قدرات تحليليّ جديد). أُعيد توليد كلّ المصنوعات على main الحاليّ (75b3ad5) لا اللقطة.
- **4 محرّكات (`scripts/ci/`):** `capability_registry_v1` (سجلّ قدرات قانونيّ من `docs/capability-registry/` YAML: 81 قدرة/10 نطاقات) · `capability_mapping_engine` (خريطة أدلّة مستودع محافظة، 77/81 مربوطة، «لا يرفع النضج/التشغيل/الإنتاج») · `capability_evidence_maturity_engine` (مصفوفة أدلّة + نضج مُقيَّم fail-closed) · `capability_parity_investment_engine` (تكافؤ/استثمار مقابل benchmark المنافسين + خريطة حرارة نطاقات؛ 12/81 مُقيَّم، 69 غير مُقيَّم).
- **مدخلات يدويّة:** `docs/capability-registry/` (بطاقات YAML لكلّ نطاق + schema + benchmark CSVs + README/model). المولَّدات تُعاد لا تُنسَخ.
- **الوصل:** 4 فحوص `--check` + 4 اختبارات إضافيّة في `capability-governance.yml` (بعد القدرات القائمة، قبل static closure).
- **إصلاحات مطلوبة للـCI:** (1) ruff (0.15.8): المحرّكات بأسلوب مضغوط (semicolons/multi-import) رست 145 خطأ lint + كامل التنسيق — نُظّفت بـ`ruff check --fix --unsafe-fixes` + `ruff format`. (2) `capability_mapping_engine` غير حتميّ محلّيّاً: `SKIP_PARTS` أغفل `.pytest_cache` (يحوي `README.md` مسحوباً) — أضفتُ `.pytest_cache`/`.mypy_cache`/`.ruff_cache` (لا تُلتزَم ⇒ مخرَج الشجرة النظيفة بلا تغيّر؛ يُحصّن التشغيل بعد pytest). (3) عدم-تطابق نقطة ثابتة: المحرّك يمسح مخرجاته ⇒ يُعاد توليد السلسلة مرّتين للتقارب.
- **صدق:** صفر `runtime_verified/production_certified=true` في المخرَجات؛ التقرير يعلن «Unassessed remain unassessed · repository maturity is not competitor evidence · no runtime or production claim». determinism 3.11==3.12 byte-identical. البوّابات: 24 فحص حوكمة، 18 اختبار قدرات، `pytest -m unit` 3494، bundle 5072 checksums.

## 2026-07-26 — تصحيح Capability Parity & Investment (المرفق المُصحَّح) — يحلّ محلّ #658 الأوّل
- **المصدر:** `sahool_main_d52a6b6_parity_investment_corrected_20260726.zip` — مراجعة جوهريّة للمحرّكات الأربعة + نموذج بيانات benchmark مُقنَّن (rubric + `canonicalization_map_v3_to_registry_v1.csv` + `sahool_capability_assessments.csv` + `legacy/*_precanonical.csv`)؛ بطاقات النطاقات YAML بلا تغيير.
- **تحسينات المالك تحلّ مشاكلي:** المحرّك المُصحَّح يستثني `generated`/`release`/`.generated.` صراحةً (لا حلقة ذاتيّة-المرجع ⇒ لا حاجة نقطة ثابتة مزدوجة) + `.pytest_cache` في SKIP_PARTS. لذا mapping أصبح نقطة ثابتة بمرور واحد، وغير مقترن بإعادة بناء الـbundle.
- **ما بقي عليّ:** ruff (0.15.8، 116 خطأ ⇒ نُظّف) + إعادة توليد على main الحاليّ. **أبقيتُ workflow الخاصّ بي** (لا المُصحَّح المبنيّ على d52a6b6) كي لا أفقد إضافات path3 (#656).
- **النتيجة:** parity 15/81 مُقيَّم (كان 12)، 66 غير مُقيَّم. 23 اختبار قدرات (كان 18). determinism 3.11==3.12. 24 فحص حوكمة، `pytest -m unit` 3494، bundle 5077. صدق: «No runtime or production claim is produced by this engine».
- **الدمج:** دُمِج #658 تحت Ratchet (SHA `c1c5422`، squash). جذر الانجراف في CI: مسح `rglob` كان يلتقط ملفّات محلّيّة غير مُتعقَّبة (`.claude/settings.local.json` + `frontend/test-results/.last-run.json`) غائبة على checkout الـCI ⇒ `files_scanned` 4767 محليّاً مقابل 4765 على CI. الإصلاح (`50eac02`): `capability_mapping_engine.iter_files()` يمسح `git ls-files` فقط (مُتعقَّب) لا نظام الملفّات ⇒ حتميّة عبر الآلات/CI. 62 فحص نظيف (Security Scan منهما ⇒ success).

## 2026-07-26 — تكامل roadmap→capability linker + PR capability-impact gate (المرفق المُصحَّح)
- **المصدر:** `sahool_main_d52a6b6_roadmap_pr_impact_corrected_20260726.zip` (لقطة كاملة؛ استخرجتُ الجديد فقط).
- **الملفّات الجديدة (5):** `scripts/ci/capability_roadmap_linker.py` (يربط `roadmap_items.yaml` بالسجلّ؛ يتطلّب source_anchor يحوي معرّف البند + rationale؛ fail-closed، لا ادّعاء تشغيل) · `scripts/ci/pr_capability_impact_gate.py` (يحسب أثر PR من current+merge-base snapshots؛ الحوكمة-النواة [workflow/gate/linker/legacy/roadmap_items] = governance_wide ⇒ كلّ القدرات؛ يفرض سطر `Capability-Impact:` في وصف الـPR) · `docs/capability-registry/roadmap/roadmap_items.yaml` (7 بنود WX-10/11 + CI-7..11) · اختبارا `tests/architecture/test_capability_roadmap_linker.py`+`test_pr_capability_impact_gate.py`.
- **أبقيتُ محرّكاتي** (mapping بمسح git-tracked، registry/maturity/parity، `capability_impact.py`) — لم أنسخ نسخ المرفق (d52a6b6، بلا إصلاح المسح المُتعقَّب). ruff-cleanتُ الملفّين الجديدين (UP035 collections.abc + UP022 capture_output) على 0.15.8.
- **الوصل:** أضفتُ 3 خطوات (`roadmap_linker --check` · `pr_capability_impact_gate --check-index` · خطوة الفرض على pull_request) + اختبارَين إلى `capability-governance.yml` **الخاصّ بي** (لا اعتماد كامل لـworkflow المرفق) + `fetch-depth: 0`.
- **إعادة التوليد على شجرتي:** mapping (files=4770، +5 للملفّات الجديدة المُتعقَّبة) ⇒ maturity ⇒ parity ⇒ roadmap (7 بنود، 34 قدرة مرتبطة، 48 علاقة) ⇒ impact index (2043 مسار، 81 قدرة). حتميّة 3.11==3.12. bundle 5087 checksums.
- **قيد الوصل الحرِج:** لأنّ PR يعدّل workflow/gate/linker ⇒ الأثر = كلّ القدرات ⇒ **وصف الـPR يجب أن يحمل `Capability-Impact: ALL`** وإلّا BLOCK (تأكّدتُ سلوكيّاً: بلا تصريح ⇒ BLOCK/81 مباشر؛ ALL ⇒ PASS). honesty نظيف.
- **إصلاح انجراف CI (#659، الجولة 2):** `capability-registry` سقط على `capability mapping drift` رغم تطابق `files_scanned`. الجذر: محرّك الخريطة كان يمسح `sahool-brain/hot.md` ويلتقط الرمز الزائف `IF` كدليل قاعدة بيانات لـIRR-003؛ تعديلي على `hot.md` أزاح سطر `IF` (63↔65)، وكسر التعادل بين مطابقتَين متساويتَي النقاط (26) يُحسَم مختلفاً بين Python 3.11 (محلّيّ) و3.12 (CI). أضفتُ `sahool-brain` إلى `SKIP_PARTS` — العقل ذاكرة سرديّة لا دليل تنفيذ، ليس في الحزمة، ويُعاد كتابته كلّ جلسة ⇒ مسحه يضخّ مطابقات زائفة + انجرافاً دائماً. النتيجة: mapping حتميّ 3.11==3.12 (تجزئة متطابقة)، files=4743 (كان 4770)، mapped=76/81 (بلا تغيير — أُزيلت 3 مدخلات زائفة فقط). الآن تعديلات العقل لا تُحرّك الخريطة إطلاقاً.

## 2026-07-26 — دمج #659 + إصلاح PHOTOMETRIC لـtruecolor COG
- **#659 دُمِج** تحت Ratchet (SHA `be85939`، squash): roadmap linker + PR capability-impact gate + استثناء `sahool-brain` من مسح الخريطة (حلّ انجراف CI الحتميّ عبر الإصدارات).
- **إصلاح COG (تقرير المالك):** `RGBA_COG_PROFILE` كان بلا `photometric` ⇒ GDAL يكتب MINISBLACK ⇒ عند القراءة «Sum of Photometric … ExtraSamples doesn't match SamplesPerPixel» ويعيد تعريف RGB كـExtraSamples. أضفتُ `photometric="RGB"` (+`alpha="YES"` للنطاق الرابع) وقت الإنشاء في `write_rgba_cog` (`services/raster-service/cog_writer.py`). **ملاحظة إصدارات:** GDAL المحلّيّ (rasterio 1.4.4) ينشر RGB من colorinterp فيخفي العيب؛ الإنتاج لا ⇒ التصريح وقت الإنشاء هو الإصلاح الحتميّ.
- **الاختبار السلوكيّ** `services/raster-service/test_rgba_cog_photometric.py`: يثبت على COG ناتج فعليّاً — count=4، colorinterp RGBA، tag262=RGB(2)، لا تحذير GDAL عبر مُسجّل `rasterio._env`، شفّاف خارج AOI + RGB مقروء داخلها؛ + حالة 3-نطاقات + حارس ساكن للمصدر. raster quality gate: 277 نجح.
- **صدق:** الإصلاح للملفّات الجديدة فقط؛ القديمة تحتاج إعادة معالجة انتقائيّة idempotent (رَنبوك `sahool-brain/runbooks/truecolor-cog-photometric-reprocessing.md` — عند الطلب/الحقول النشطة، لا bulk). لم أوسّع النطاق للصورة المصغّرة الفارغة (بند مستقل).

## 2026-07-26 — دمج #660 (truecolor COG PHOTOMETRIC) + سلسلة إصلاحات حتميّة الحوكمة
- **#660 دُمِج** تحت Ratchet (SHA `9d08568`، 68 فحص أخضر). المضمون: `write_rgba_cog` يصرّح `photometric=RGB`+`alpha=YES` وقت الإنشاء (يحلّ تحذير GDAL «Sum of Photometric … ExtraSamples doesn't match SamplesPerPixel») + اختبار سلوكيّ على COG ناتج + رَنبوك إعادة معالجة انتقائيّة.
- **درس مهمّ — سلسلة انجراف CI الحتميّة:** إضافة ملفّ اختبار واحد تحت `services/` فجّرت انجرافات متتابعة لأنّ 4 مولّدات حوكمة تمسح `.py`/الملفّات عبر `rglob` وتُضمّن **عدّاً خاماً** (`python_files_parsed`) أو تُجزّئ المجموعة، و`--check` يُعيد الكتابة/التجزئة. أيّ ملفّ `.py` عابر/غير مُتعقَّب على عامل CI (غائب على checkout نظيف) يجعل المخرَج غير قابل لإعادة الإنتاج. الحلّ الموحّد: مسح `git ls-files` المُتعقَّب فقط. أُصلِحت الأربعة: `capability_mapping_engine` (#659)، ثمّ `static_governance_closure` (بصمة artifacts) + `duplicate_definition_guard` + `execution_dependency_audit` (#660). أضفتُ تشخيصاً لبوّابة الإغلاق يسمّي الأداة المنجرفة بدقّة (كان مفتاح تتبّع كلّ حلقة). المولّدات الأخرى ذات rglob (architecture_graph، route/router guards) تُفهرِس بأنماط وحدات-خدمة لا عدّاً خاماً ⇒ غير هشّة (أخضر عبر كلّ PR سابق).

## 2026-07-26 — تكامل PA-003 (استيعاب خرائط الغلّة) على main الطازج
- **المصدر:** `sahool_main_d52a6b6_pa003_yield_map_ingestion` — دلتا مركّزة (بقيّة ميزة الغلّة مدموجة سابقاً). الجديد فعليّاً: router `yield_map_ingestion` + خدمة API + `migrations/v215_yield_map_ingestion.sql` + اختبارَان + أساس registry.
- **v215:** جدولا `yield_map_ingestions` (منشأ idempotent) + `yield_map_records` (نقاط PostGIS 4326، append-only)، FORCE-RLS + tenant_isolation، مُشغّلات نطاق (الموسم/ST_Covers داخل هندسة الحقل) + منع تحوير. أُدرِج في المُشغّلَين (run_migrations خطوة 220، MANIFEST قبل v206) + حارس التزامن أخضر.
- **دلتا غير مُسمّاة yield التقطتها بالبحث:** `main.py` CRITICAL_EVENT_TYPES + `event_bus.py` (YIELD_MAP_INGESTED="yield_map.ingested") + `test_critical_event_fail_closed` — أُضيف الحدث الحرِج (استمرار بلا حدث outbox = التزام جزئيّ مرفوض).
- **حوكمة:** router يُكتشَف تلقائيّاً (`router_registry` pkgutil). أعدتُ توليد السلسلة كاملة (inventory 1101 مسار، catalog 989 method/path). مسارا الاستيعاب/السجلّات farmer بلا شاشة ⇒ إعفاءان operational صريحان في `endpoint_ui_coverage_waivers.json` (لا اختلاق دليل واجهة). حدّثتُ تثبيتات U0 (989) وU4 (51). 21 بوّابة انجراف نظيفة، production_validation_gate أخضر، bundle 5094. صدق: أساس registry يُظهر production/runtime=false.

## 2026-07-26 — دُمِج #661: PA-003 + طبقة إدارة القدرات + إصلاح mapper صادق (`f15a4ee`)
- **#661 دُمِج** تحت Ratchet (squash `f15a4ee`، كلّ الفحوص success/skipped على الرأس). ثلاث شرائح متكاملة على فرع واحد.
- **PA-003:** كما سُجِّل أعلاه (v215 + router + خدمة + YIELD_MAP_INGESTED). أُضيفت رفعات أساس تفكيك P0 موثّقة: db_ownership +2 جدول، module baseline 663→665، ميزانيّة المسارات 630→633 (p2_6 626→629)، سقف p1 LOC 2550→2553.
- **بوّابة التتبّعيّة — إغلاق الثغرة لا التقليم:** المُدقِّق ادّعى أنّ FM-003/OPS-004 تُحسَبان على ٥ ملفّات Flutter «غير موجودة». التحقّق: على شجرتنا **كلّ** المؤشّرات موجودة ومُتعقَّبة (81/81 تمرّ) — الادّعاء يخصّ حالة المرفق القديمة (`d52a6b6`) حيث الملفّات مُقلَّمة. الحكم الصادق (اختار المالك «Close the gate blind spot»): **لا تُحذَف روابط صادقة**؛ أُضيف `test_capability_dart_imports_resolve` يحلّ استيرادات Dart الأوّليّة (relative + `package:sahool_app/`) ويفشل عند ملفّ مستورَد مفقود — أُثبِت أنّه يكشف الشكل المعيب على لقطة المرفق.
- **طبقة إدارة القدرات (من مرفق `capability_management_layer_implemented`):** أُضيفت `capability_management_engine`/`capability_release_history`/`capability_regression_gate` + سياسات lifecycle/maturity/parity/investment + أسس إصدار 001–003 + ٣ اختبارات معماريّة، مربوطة في `capability-governance.yml`. اختبار الاكتمال كشف فجوة تغطية حقيقيّة (76/81) في الماسح الحدسيّ.
- **إصلاح mapper صادق (fail-closed):** `capability_management_engine.generate_payload` صار يحتسب `mapped` كاتحاد الماسح ∪ أدلّة السجلّ المُعلَنة **الموجودة على القرص**؛ `validate()` يفشل fail-closed إن لم يُحلّ أيّ مؤشّر دليل (services/apis/tests/ui/mobile) — تُحقِّق من الوجود لا من مجرّد ذكر في YAML. الأبعاد المحدّدة فقط تقرّر mapped (استبعاد `other_evidence`/`governance` catch-all). النتيجة: IRR-010/OPS-001/OPS-006/OPS-008 تُحتسَب من أدلّتها المُعلَنة، PA-003 متعدّد الأبعاد.
- **حكم INT-004 بالمحتوى (لا الأسماء):** ملفّاتها المرشّحة موجودة لكنّها مسلسلات تصدير ISOXML/VRT ساكنة (docstrings: «TaskData skeleton … can be transformed by an equipment-specific adapter»؛ «serialize already-approved zones»)، الاختبار سلوك ساكن، **صفر router/API/consumer حيّ**. ⇒ groundwork لا integration lifecycle ⇒ **INT-004 تبقى الفجوة الوحيدة (80/81)، لا تُرفَع**. accounted_for=80، adjudicated=0.
...
- **وفاء للإنتاج + idempotency (تأكيد):** المخطّط المُتحقَّق مطابق للإنتاج لا stubs مريحة — `fields(tenant_id,field_id)` مفتاح مركّب (v114) · `fields.geom geometry(Geometry,4326)` (v104/v43؛ أُعيد برهان ST_Covers على النوع الدقيق: داخل مقبول/خارج مرفوض متطابق) · `seasons.season_id` PK (v32). `scripts/ci/rls_policy_guard.py --check` أخضر مع v215. وv215 **idempotent:** إعادة تطبيقها على نفس القاعدة exit=0 بلا خطأ، الأدلّة append-only سليمة (1 استيعاب/2 سجلّ)، 4 مُشغّلات + سياستان + FORCE=all(true) — بفضل `IF NOT EXISTS`/`DROP POLICY IF EXISTS`/`CREATE OR REPLACE`.
...
## 2026-07-26 — فحص وظيفيّ: سلسلة الهجرات الكاملة على PG16 — **CHAIN_PG_PROVEN**
- **المُشغّل:** `scripts_v9/run_migrations.sql` (المُولَّد من `migrations/MANIFEST.txt`، ٢٢١ خطوة) على PG16.13+PostGIS3.4.2، `ON_ERROR_STOP=1`.
- **البذرة الوحيدة:** ٣ أدوار GRANT (`sahool_app`/`sahool_ingest`/`sahool_jobs`) + امتدادات مُثبَّتة على النظام (postgis/pgcrypto/uuid-ossp/btree_gist؛ **timescaledb مُعلَّق** في `init_v8.sql:20` ⇒ لا اعتماد فعليّ). الامتدادات تُنشئها الهجرات ذاتيّاً (`CREATE EXTENSION IF NOT EXISTS`).
- **النتيجة:** **runner exit=0، صفر ERROR/FATAL عبر الـ221 خطوة**، ووصلت إلى الخطوة 221 (`v206_rls_final_hardening.sql` — يجب-أن-يبقى-آخر). v215 طُبِّقت في ترتيبها الحقيقيّ (خطوة 220) ضدّ مخطّط `fields`/`seasons` **الحقيقيّ** — FKs حُلَّت (`yield_map_ingestions_tenant_id_field_id_fkey` + `..._season_id_fkey`)، الجدولان FORCE-RLS.
...
## 2026-07-26 — إغلاق فجوة INT-004 بتنفيذ حقيقيّ ⇒ 81/81 mapped صادقاً
- **الفجوة:** INT-004 «تكاملات آلات خارجيّة» كانت القدرة الوحيدة غير المربوطة (80/81) — تصدير ISOXML/VRT ساكن (`core/isoxml_vrt.py`) بلا مستهلِك حيّ.
- **الإغلاق (تنفيذ، لا توثيق):** `api/machinery_export.py::build_prescription_isoxml` — مستهلِك نقيّ يحوّل وصفة VRT محفوظة (`routers/prescriptions.py`) + ملفّ قدرات مُتحكِّم مُمرَّراً ⇒ **ISO11783 TaskData حقيقيّ** عبر `core/isoxml_vrt.export_taskdata_xml`. مربوط في نقطة التصدير `GET .../prescriptions/{id}/export?format=isoxml` (كانت TODO موثَّق «ISOXML مؤجَّل» ⇒ 422؛ الآن تُنتِج TaskData). **fail-closed:** مُتحكِّم ناقص/غير متوافق، وحدات مختلطة/غير مدعومة، نوع منتج غير معروف، صفر مناطق ⇒ 422 بلا ملفّ جزئيّ. 8 اختبارات (تصدير seed/fertility + 6 مسارات رفض).
- **صدق الترقية:** حدّثتُ `precision.yaml` INT-004 (services/apis/tests حقيقيّة، status behavioral_test_linked_runtime_unverified، maturity 3، evidence 3، owner sahool-platform). محرّك الإدارة صار يحتسبها mapped بأدلّة **تنفيذيّة حقيقيّة** (backend+routes+tests) ⇒ dashboard **81/81 mapped، unmapped=[]** — 81/81 مشروع الآن لأنّ تنفيذاً حقيقيّاً ظهر (كما اشترط المالك)، لا رقم مُضخَّم. `runtime_verified`/`production_certified`=false في كلّ مكان.
- **حارس الثبات (مُعاد توجيهه):** `test_scaffold_with_no_specific_evidence_is_not_promoted` صار **تركيبيّاً** (cap تركيبيّ ZZ-999 بلا أدلّة محدّدة + other_evidence فقط ⇒ يبقى unmapped) — يحفظ الثبات دون الاعتماد على INT-004 الذي صار مُنفَّذاً.
- **بوّابات:** رفع أساس الوحدات 665→666 (machinery_export.py، موثّق). أُعيد توليد السلسلة (service inventory/exec-audit/route/event/contract/mapping/management/impact-index/PATH-2) + static-closure LAST + bundle 5120. كلّ البوّابات خضراء؛ ruff نظيف؛ 8+16 اختباراً أخضر.
- **حدّ الصدق:** يُنتِج الملفّ القابل للرفع عند حافّة المنصّة؛ لا يقود جهازاً فعليّاً — يبقى برهان رفع/استهلاك end-to-end على مُتحكِّم حيّ كبند تشغيليّ لاحق.

## 2026-07-26 — إعادة تحجيم INT-004 إلى شريحة المحوِّل الصحيحة (INT-004A) + براهين PG16 حيّة
- **تصحيح المالك (حاسم):** أوّل تنفيذ لـINT-004 استخدم ملفّ تحكّم من جسم/استعلام الطلب بلا إدامة — المالك رفضه كمسار إنتاج. النطاق الصحيح: **مصدر سجلّ ملفّات تحكّم مُدام** + **لقطة غير قابلة للتغيير** + **حزمة قابلة للرفع مُدامة** + checksum + سجلّ تنزيل + تدقيق؛ الملفّ المضمّن مسار تطويريّ/مُميَّز فقط.
- **الهجرة `migrations/v216_machinery_export.sql`:** جدولان معزولان بالمستأجِر + FORCE-RLS: (١) `machine_control_profiles` — SoR لقدرات المُتحكّم (vendor/model/firmware/units/ISOXML)، قابل للتعديل (مُشغّل updated_at)، FK مركّب آمن للمستأجِر إلى `equipment(tenant_id, equipment_id)` (أُضيف `equipment_tenant_equipment_uk` لأنّ PK equipment على العمود وحده)؛ (٢) `machinery_export_artifacts` — سجلّ **append-only غير قابل للتحوير** (مُشغّل BEFORE UPDATE OR DELETE) لحزمة ISOXML: `profile_snapshot` jsonb (لقطة مجمّدة) + `package_bytes` bytea + `package_sha256` + قيود CHECK (checksum hex، zone_count>0). مُدرَجة في **كلا** المُشغّلين قبل v206 (يبقى v206 آخِر مدخل؛ حارس التزامن أخضر، 222 خطوة).
- **الخدمة `api/machinery_export.py`:** `resolve_persisted_profile` (fail-closed: مفقود⇒«not found or not authorized»، غير نشط، لا يدعم ISOXML) + `build_profile_snapshot` (لقطة حتميّة، وحدات مرتّبة) + `package_taskdata` (**ZIP حتميّ** — اسم/ختم زمنيّ ثابت ⇒ بايتات وchecksum قابلة للتكرار) + `generate_export_package` (نواة نقيّة: حلّ⇒تحقّق⇒توليد⇒تغليف⇒checksum⇒لقطة). المسار المضمّن يقبل الآن شكلَي `controller`/`controller_model`.
- **النقاط `routers/prescriptions.py`:** المسار القانونيّ `POST .../{id}/machinery-export` {machine_profile_id} (EQUIPMENT_MANAGE) ⇒ حلّ+تخويل⇒توليد⇒تغليف⇒**إدامة أثر** + مرجع تنزيل + منشأ (created_by/at). `GET .../machinery-export/{artifact_id}/download` (FIELD_VIEW) يعيد ZIP + ترويسة `X-Package-SHA256`. المسار المضمّن `export?format=isoxml` صار محجوباً خلف **PLATFORM_MANAGE** (تطويريّ/مُميَّز، لا يُدام). ردّ المسار القانونيّ يعلن صراحةً `delivered_to_device=false`, `machine_consumed=false`.
- **براهين PG16 حيّة (تحت `SET ROLE sahool_app` NOBYPASSRLS):** السلسلة الكاملة (222 خطوة) تُطبَّق exit=0، v216 بجدولَيه ENABLE+FORCE-RLS + قيد equipment الفريد + مُشغّلان. **١١ برهان جدول:** عزل A/B، WITH CHECK يمنع وسم مستأجِر آخر، GUC غير مضبوط fail-closed، append-only (UPDATE+DELETE محجوبان)، CHECK (checksum/zone_count)، FK لملفّ غير موجود يفشل، updated_at للملفّ القابل للتعديل. **برهان E2E حيّ:** إدراج أثر من `generate_export_package` الحقيقيّ (525 بايت، sha 32e7f4…) ثمّ SELECT التنزيل ⇒ بايتات/checksum/طول مطابقة تماماً؛ مستأجِر B يرى 0 آثار (عزل).
- **الصدق (دلالة الإغلاق):** هذا يُغلق فقط **INT-004A** (محوِّل الأثر ISOXML): `adapter_implemented`/`artifact_generation_verified`/`machine_upload_package_verified`=true؛ **مفتوح:** INT-004B (نقل CAN/ISOBUS إلى مُتحكّم) + INT-004C (تأكيد استهلاك/تنفيذ الآلة) — `device_delivery`/`machine_consumption`/`physical_execution`/`runtime`/`production`=false. maturity/evidence تبقى 3. `precision.yaml` يحمل كتلة `scope` صريحة بهذه الأعلام + INT-004B/C كبنود مفتوحة.
- **بوّابات:** أُعيد توليد السلسلة كاملة (graphs/exec-audit/mapping/management/impact-index/PATH-2) + static-closure PATH-1 20/20 LAST + bundle 5120. `capability_management_ok total=81 mapped=81` (INT-004 مربوطة بأدلّة حقيقيّة). 22 اختبار قدرات معماريّ + 15 اختبار machinery (8 قديمة + 7 جديدة: تغليف حتميّ/checksum/لقطة/inactive/no-ISOXML/no-row/units) + حارس التزامن + router-decomposition — أخضر. ruff نظيف؛ bandit HIGH نظيف. DB مؤقّتة `int004_test` أُسقِطت.

## 2026-07-26 — تصليب حوكمة: مصالحة الماسح الخام مع المصفوفة المرجعيّة (مراجعة 5aeefe6)
- **المصدر:** مراجعة جنائيّة لأرشيف `5aeefe6` (حالة ما قبل شريحة INT-004A). النتيجة الوحيدة القابلة للتنفيذ = **P1 وضوح حوكمة**: أداة `capability_mapping_engine` الخام كانت ترقّي `mapped=True` بقاعدة أوسع من محرّك الإدارة المرجعيّ ⇒ تناقض دلاليّ بين قِطعتَين مولّدتَين.
- **التحقّق على الشجرة الحاليّة (لا الأرشيف):** العيب بنيويّ وما يزال قائماً بقدرات مختلفة — `capability_mapping_engine.py:426` كان يجمع **كلّ** الدلاء بما فيها `governance`+`other_evidence` ⇒ **INT-001 وWX-001** كانتا mapped بصفر بُعد تنفيذيّ محدّد (ذكر ID عابر). بالمقابل الماسح **يُنقِص** IRR-007/IRR-010/OPS-001/OPS-006/OPS-008 (يربطها محرّك الإدارة عبر أدلّة السجلّ المُعلَنة). المصفوفة المرجعيّة صحيحة (81/81)؛ الخام وحده كان يخالفها في الاتجاهين.
- **الإصلاح:** `mapped = coverage_dimensions > 0` (الأبعاد التنفيذيّة المحدّدة فقط: backend/routes/database/events/web/mobile/tests). `governance`/`other_evidence` تُذكَر ولا ترقّي أبداً — مطابق لقاعدة محرّك الإدارة. الخام الآن **حدّ أدنى صادق** (⊆ المرجعيّ). أُضيف للقطعة الخام أعلام صريحة: `authoritative=false` + `authoritative_mapped_state_source` (لوحة الإدارة) + `mapped_semantics`؛ وعنوان التقرير صار «Raw Scanner Candidates (NOT authoritative)». (محرّك الإدارة لا يقرأ `mapped` الخام أصلاً — يقرأ الدلاء — فبقيَ 81/81 دون تغيير.)
- **الثابت العابر للقطع (اختبار):** `raw_scanner_mapped ⊆ management_mapped` (كلّ ما يربطه الماسح يربطه المرجعيّ؛ صفر مخالفة). **المساواة مرفوضة عمداً** (المرجعيّ يربط 7 أكثر عبر أدلّة السجلّ التي لا يراها الماسح) — المساواة تتطلّب تكرار منطق المصالحة في الماسح ⇒ مصدر ثانٍ للحقيقة + انجراف. الاشتمال (subset) هو الثابت الصادق. + اختبار «لا ترقية عبر governance/other_evidence» + اختبار علم `authoritative=false`.
- **P2 قابليّة التدقيق دون git (ZIP):** الماسح يستدعي `git ls-files`؛ أُضيف **احتياطيّ fail-closed**: عند غياب `.git` يُقرأ **البيان المُوقَّع** `release/FILE_CHECKSUMS.sha256` (قائمة سماح موقَّعة) ويُمسح ما فيه فقط — **لا** مسح للملفّات غير المتعقّبة أبداً؛ وغياب الاثنين ⇒ رفع خطأ (لا مسح خام). في CI (git دائماً) يُستخدم مسار git فيبقى الإخراج قابلاً للتكرار.
- **اكتشاف جانبيّ (مُصحَّح):** التزام سابق (9f8044f) ولّد الحوكمة و`v216` **غير متعقَّب** بعد (الماسح/الحزمة يعتمدان git ls-files) ⇒ فاتت أدلّة v216 من الحزمة (5120). بعد التتبّع أُعيد توليد السلسلة كاملة ⇒ v216 صار database-evidence لـINT-004 + الحزمة **5121**.
- **بوّابات:** أُعيد توليد السلسلة (mapping 74/81 خام، management **81/81** ثابت، exec-audit، evidence/maturity، impact-index، PATH-2، PATH-1 20/20 LAST) + bundle **5121**. 9 اختبارات mapping (5 قديمة + 3 ثوابت + 1 احتياطيّ) + management/registry/closure أخضر؛ ruff نظيف. لم يُعَد فتح #661 (بند تصليب مستقلّ على `claude/code-review-34hO3`).

## 2026-07-26 — إصلاح بوّابات CI لشريحة INT-004A: طيّ المسارات (Ratchet) + تسجيلات الحوكمة
- **المصدر:** CI على الفرع كشف 6 فئات حوكمة لم تُحدَّث مع المسارَين الجديدين (لقطات المستخدم).
- **القرار الحاسم (ميزانية المسارات):** الحاجب الحقيقيّ = `test_platform_route_budget_does_not_grow` (AST 635>633) + `test_p2_6_platform_route_budget_reduction` (regex 631>629). **سقف p2_6 مثبّت بـ`assert budget <= 629` وسياسته «No route growth»** ⇒ لا يمكن رفعه ولا رفع baseline (تأكيد المراجع). النتيجة: **صافي مسارات جديدة = صفر إلزاميّ.** لا يوجد مسار تنزيل عامّ لإعادة استخدامه. ⇒ **طُوِيَت الشريحة كاملةً في نقطة `export` القائمة** (لا POST جديد ولا download جديد):
  - `GET .../export?format=isoxml&machine_profile_id=…` (EQUIPMENT_MANAGE) = المسار القانونيّ: حلّ ملفّ مُدام ⇒ توليد ⇒ تغليف حتميّ ⇒ **إدامة أثر مُتماثِلة** (dedup على sha256 الحتميّ ⇒ استدعاء متكرّر يعيد الأثر نفسه، فيبقى GET آمناً) ⇒ ZIP + ترويستَي X-Artifact-Id/X-Package-SHA256.
  - `GET .../export?format=isoxml&artifact_id=…` = تنزيل أثر مُدام سابق.
  - المسار المضمّن (بلا profile/artifact) يبقى تطويريّاً/مُميَّزاً (PLATFORM_MANAGE، لا يُدام).
  - AST=633، regex=629 (ضمن الميزانية). حُذِف نموذج `MachineryExportRequest` و`_download_ref`.
- **مبرّر الانحراف عن spec الـPOST:** المالك الأصليّ طلب POST؛ لكنّ Ratchet المنصّة (سقف p2_6) يمنع أيّ مسار جديد. حُفِظت **كلّ** المتطلّبات الجوهريّة (ملفّ مُدام SoR + لقطة غير قابلة للتغيير + أثر مُدام + checksum + مرجع تنزيل + fail-closed + RLS) عبر النقطة القائمة — والمراجع نفسه أوصى صراحةً بعدم رفع baseline وإعادة استخدام مسار قائم.
- **تسجيلات الحوكمة:** (أ) `db_ownership.yml`: أُضيف `machine_control_profiles` + `machinery_export_artifacts` (owner sahool-platform، source v216). (ب) `test_simple_farm_book.py`: تحديث fixture ترتيب الهجرات إلى `entries[-7:]` بإدراج v216 قبل v206 (v206 يبقى آخِراً). (ج) `precision.yaml` INT-004 apis/rationale: تشير الآن لنقطة `export` المطويّة. (د) endpoint-ui-coverage-reverse-gate: نجح تلقائيّاً (المساران المنفصلان زالا).
- **إعادة توليد:** platform catalog (fingerprint حتميّ fd139c0b1ab1، بُنيَ مرّتين متطابقتين) + السلسلة كاملة (graphs/route/service inventories/exec-audit/mapping/management **81/81 ثابت**/impact-index/PATH-2) + static-closure PATH-1 20/20 LAST + bundle **5121**. صفر drift على كلّ البوّابات (`--check`). 42 اختباراً معماريّ/منصّة أخضر (شمل الأربعة الحاجبة سابقاً + mapping/management)؛ ruff نظيف.
- **ملاحظة بيئة:** `tests_v9` (endpoint_ui_coverage/platform_catalog) تعذّر تشغيلها محلّيّاً (panic في cryptography/pyo3 داخل conftest — عطل بيئة لا اختبار)؛ تُحقّق منطقها مباشرةً: reverse-gate=PASS، catalog حتميّ بلا drift بعد الالتزام.

## 2026-07-26 — إغلاق تشغيل CI (ruff format) + توضيح ميزانيّة المسارات (629 مقابل 633)
- **سياق:** سجلّات CI التي عُرضت كانت من `f14e646` (قبل الطيّ `7b405aa`) — تُظهِر `create_machinery_export`/`download_machinery_export` اللذين أُزيلا. تحقّقتُ أنّ HEAD=`7b405aa`=origin والمسارات محذوفة (grep=0)، وأنّ كلّ الحواجز (route budget/ownership، db ownership، migration fixture، endpoint-UI-reverse، platform catalog) تمرّ الآن على HEAD.
- **خطأ حقيقيّ استمرّ (أُصلِح):** `ruff format --check .` كان أحمر على ملفّين — `tests/architecture/test_capability_mapping_engine.py` (من f14e646) و`api/routers/prescriptions.py` (من الطيّ 7b405aa). السبب: شغّلتُ `ruff check` (lint) لا `ruff format --check` في الالتزامين. طُبِّق `ruff format` على كليهما ⇒ 2805 ملفّ نظيف. **درس:** أُشغّل `ruff format --check` دائماً قبل الالتزام، لا `ruff check` وحده.
- **إعادة توليد:** إعادة تنسيق prescriptions.py أزاحت أرقام الأسطر في مؤشّرات أدلّة الماسح (543→547 …) ⇒ أُعيد توليد السلسلة كاملة (mapping/management **81/81 ثابت**/exec-audit/graphs/inventories/catalog حتميّ/PATH-2/PATH-1 20/20) + bundle **5121**. صفر drift على كلّ البوّابات.
- **توضيح ميزانيّة المسارات (رد على تشخيص المراجع):** لا تناقض بين 629 و633 — هما **عدّادان مختلفان**: `baseline_route_count=633` يقترن بعدّاد AST (test_p0، `api/` فقط + `api_route`)، و`p2_6.new_max_platform_routes=629` يقترن بعدّاد regex (كامل الشجرة عدا tests). `assert budget <= 629` يقرأ `new_max` (629) لا `baseline` (633) ⇒ 629<=629 يمرّ (ليس 633<=629). بعد الطيّ: AST=633≤633 وregex=629≤629 — كلاهما عند سقفه ومُرضٍ. لم يغيّر فرعي `baseline_route_count` (ثابت على main منذ #661/`2246844`). ⇒ لا حذف 6 مسارات (سيُزيل نقاطاً شرعيّة قائمة).
- **دحض «إسقاط v211»:** تشخيص المراجع أنّ v211 أُسقِطت خطأ — v211 موجودة في كلا المُشغّلين؛ آخِر 7 مدخلات MANIFEST = v211..v216 ثمّ v206 آخِراً. فرق pytest كان انزلاق نافذة `[-6:]` عند إدراج v216 (أصلحته إلى `[-7:]`)، لا حذفاً فعليّاً.

## 2026-07-27 — إغلاق P2 (قابليّة التدقيق دون git) لـstatic_governance_closure
- **المصدر:** bundle مُدرَج (`sahool_main_37c3b56_static_closure_zip_fallback_implemented`) بعد دمج #662.
- **التغيير:** تعميم احتياطيّ fail-closed (نظير ما طُبِّق على `capability_mapping_engine`) على `scripts/ci/static_governance_closure.py`: `_tracked_files()` يحاول `git ls-files`؛ عند غياب `.git` (أرشيف ZIP مستخرَج) يعود إلى **البيان المُوقَّع** `release/FILE_CHECKSUMS.sha256` كقائمة سماح، وإلّا يرفع خطأ (لا مسح للملفّات غير المتعقّبة). + دالّة `_manifest_files()` + اختباران (`test_tracked_files_uses_signed_manifest_when_git_is_unavailable`، `test_tracked_files_fails_closed_without_git_or_signed_manifest`).
- **الصدق/الحتميّة:** في CI/التطوير (git متاح) يعيد نفس مجموعة الملفّات تماماً ⇒ **صفر تغيير في مخرَج الإغلاق** (STATIC_GOVERNANCE_ARTIFACTS بلا انجراف؛ `--check` أخضر). الاحتياطيّ يعمل فقط دون git. أُعيد بناء الحزمة (5121) لأنّ ملفَّي المصدر تغيّرا.
- **بوّابات:** static-closure/mapping/management/impact-index `--check` أخضر؛ 5 اختبارات إغلاق (3 قائمة + 2 جديدة)؛ ruff check+format نظيف؛ المفتّش exit 0. الفرع أُعيد تأسيسه من main (37c3b56) بعد دمج #662 (بروتوكول الـPR المدموج).

## 2026-07-27 — تصليب احتياطيّ static_closure دون git: نزاهة البيان + حدّ git (على #663)
- **المصدر:** bundelان مُدرَجان (`..._manifest_integrity_hardened`، `..._git_boundary_hardened`) فوق أساس احتياطيّ #663. **git_boundary مجموعة عليا** تشمل manifest_integrity كاملاً + تصليبَين إضافيَّين ⇒ اعتُمِد وحده (والآخر مُلغى).
- **التصليب:** (١) `_manifest_entries()` تحليل صارم للبيان: صيغة قانونيّة `<sha256>␠␠<path>`، رفض digest مشوَّه/مسار غير آمن (مطلق/`..`/غير مُطبَّع)/تكرار متعارض. (٢) `_verify_manifest_file()` يتحقّق من checksum كلّ ملفّ مُدرَج قبل استخدامه دليلاً (لا مجرّد وجود المسار). (٣) `_git_tracked_files()` يشترط `git rev-parse --show-toplevel == ROOT` (أرشيف متداخل داخل checkout أب لا يرث حدوده). (٤) `_tracked_inventory()` يحمل **مصدر الثقة** صراحةً (git محقّق ⇒ None؛ احتياطيّ ⇒ إدخالات البيان) فلا يعطّل علامةُ `.git` مزوّرة تحقّقَ الـchecksum بعد فشل git — العيب الذي أصلحه git_boundary في نسخة manifest_integrity (التي اعتمدت وجود `.git`).
- **الحتميّة:** في CI (git، toplevel==ROOT) يعيد نفس المجموعة، source=None ⇒ لا تحقّق per-file ⇒ **صفر انجراف** في الإغلاق (`--check` أخضر). 9 اختبارات (5 قائمة + 4: unsafe-paths/checksum-verified/stale-git-marker/parent-worktree). bundle 5121؛ mapping/management/impact-index/static `--check` أخضر؛ ruff نظيف.

## 2026-07-27 — تصليب symlink/confinement لـstatic_closure (على #663، فوق git_boundary)
- **المصدر:** bundle `..._git_symlink_hardened` — **مجموعة عليا** فوق git_boundary. اعتُمِد.
- **التصليب:** (١) `_verify_regular_confined_file()` يُطبَّق على كلّ مُدخَل إغلاق (git و offline معاً): resolve صارم، تأكيد بقاء المسار داخل ROOT (لا هروب عبر symlink)، رفض أيّ مكوّن symlink، وأنّه ملفّ عاديّ — يمنع استبدال ملفّ متعقَّب بـsymlink في worktree معدّل. (٢) `_verify_required_evidence()` يُستدعى في مطلع `evaluate()`: كلّ ملفّ REQUIRED يجب أن يكون ضمن مخزون الثقة + مُتحقَّقاً (checksum offline / regular-confined في git) قبل قراءته — يسدّ ثغرة قراءة `evaluate` المباشرة لملفّات REQUIRED قبل فحص العضويّة/السلامة.
- **الحتميّة:** التحقّق تحقّقٌ فقط (يرفع عند symlink/هروب)؛ قائمة الملفّات لا تتغيّر ⇒ **صفر انجراف** (`--check` أخضر). 13 اختباراً (9 + 4: required-in-inventory، manifest-symlink-escape، git-tracked-symlink-reject، git-required-regular-file). bundle 5121؛ mapping/management/impact-index أخضر؛ ruff نظيف.

## 2026-07-27 — تصليب manifest_anchor لـstatic_closure (بعد دمج #663)
- **المصدر:** bundle `..._manifest_anchor_hardened` فوق main المدموج. أُدمِج ملفّان فقط (static_closure + اختباره)؛ **تُجوهِل** فرق `P3_4_PLATFORM_WEATHER_FACADE_REPORT.md` (نسخة فارغة في الأرشيف = أثر لقطة، لا تغيير مقصود — حذفه كان سيُتلف تقريراً حقيقيّاً).
- **التصليب:** (١) **تثبيت مرساة البيان:** قبل تحليل `FILE_CHECKSUMS.sha256` يُمرَّر عبر `_verify_regular_confined_file` — فالمرساة نفسها لا يمكن إعادة توجيهها عبر symlink خارج الجذر. (٢) **رفض كلّ مسار مكرّر** (لا المتعارض فقط): مسار مذكور مرّتين — حتّى بنفس الـdigest — يُرفَض (مؤهِّل "conflicting" فقط عند اختلاف الـdigest). اختباران جديدان: `test_release_manifest_itself_rejects_symlink_escape`، `test_manifest_parser_rejects_duplicate_path_even_with_same_digest`.
- **الحتميّة:** مسار git يبقى مطابقاً ⇒ صفر انجراف (`--check` أخضر). 15 اختباراً (13 + 2). bundle 5121؛ mapping/management/impact-index أخضر؛ ruff نظيف. فرع مُعاد تأسيسه من main (2ce8be7).

## 2026-07-27 — طبقة functional probes مستقلّة (weather-service فقط) — الخطوة 2 من التحقّق التشغيليّ
- **السياق:** بعد إثبات أنّ منصّة التحقّق التشغيليّ تعمل حيّاً (liveness)، والقرار (المالك، الخيار A) بعدم قلب `runtime_verified` بأدلّة liveness/بيئة مؤقّتة، والانتقال للخطوة 2: **probes وظيفيّة لا صحّيّة فقط**.
- **الاكتشاف الحاكم (مُوثَّق للخطوة اللاحقة):** سجلّ الأدلّة يفهرس الخدمات بالاسم (`weather-service`) بينما سجلّ القدرات يفهرسها بالمسار (`services/weather-service/main.py`) ⇒ بوّابة الشهادة لا تُمرِّر خدمةً محقّقة إلى قدراتها. جسر الهويّة شرط لأيّ قلب `runtime_verified` لاحق.
- **التسليم (طبقة مستقلّة، لا تمسّ خطّ health/readiness القائم ولا حرّاس الحالة-صفر):** (١) خطّة تعريفيّة `runtime-verification/functional_probes/weather-service.json` — probes على نقاط الحوسبة النقيّة (`agro/et0` FAO-56 · `agro/gdd`) بجسم طلب + status + مؤكِّدات استجابة (schema/type/min/max/equals+tolerance) + حتميّة. (٢) `scripts/ci/functional_probe_runner.py`: `--check` تحقّق ساكن (schema + وجود المسار في entrypoint، بلا شبكة، آمن لـCI) · `--run` تنفيذ حيّ اختياريّ ينتج **أدلّة وظيفيّة** (schema+قيمة+حتميّة). (٣) اختبار ساكن `tests/architecture/test_functional_probe_plan.py` (5). **الأدلّة الوظيفيّة gitignored — تُنتَج حيّاً لا تُودَع** (يبقى المستودع عند الحالة-صفر الصادقة).
- **برهان حيّ (ديمو، غير مُودَع):** ET0=4.113mm (fao56_penman_monteith، ضمن نطاق FAO-56 ~3.9) · GDD=26.0 (مطابق للحساب اليدويّ 7.5+10+8.5)؛ كلاهما schema-conformant + حتميّ. الـrunner --run: ALL PASS، exit 0.
- **حدّ الصدق:** الخطّة تعريف لا دليل — لا تحمل `runtime_verified`/`production_certified`، والـprobes compute-only فقط (لا DB/Redis/provider). قلب `runtime_verified` يبقى خطوةً لاحقةً بأدلّة وظيفيّة من بيئة مُعتمَدة + جسر الهويّة. bundle/mapping/management/impact-index/static أخضر؛ ruff نظيف.

## 2026-07-27 — جسر الهويّة التشغيليّ (identity bridge) — جاهز وخامل، بلا أيّ flip
- **الطلب (المالك):** بناء جسر الهويّة الآن ثمّ التوقّف قبل أيّ flip. نطاق ضيّق حوكميّ: mapping صريح من اسم الخدمة في السجلّ إلى مسار/مالك الخدمة في سجلّ القدرات؛ منع المطابقة التقريبيّة/substring؛ فرض one-to-one أو one-to-many معلن مع رفض الغموض/التكرار المتعارض؛ ألّا يغيّر وجود الجسر runtime_verified؛ اشتراط دليل Step 3 صالح (SHA+بيئة+زمن+نوع probe)؛ تحديث القدرة فقط عند تغطية الأدلّة لمتطلّباتها لا لمجرّد نجاح الخدمة؛ production_certified=false؛ اختبارات لكلّ الحالات؛ dry-run؛ fail-closed.
- **التسليم (حوكميّ فقط، لا تغيير على runtime_verified):** (١) خريطة صريحة `runtime-verification/service_identity_map.json`: `service_identity` (weather-service → services/weather-service/main.py، one-to-one، owner) + `capability_functional_coverage` (WX-004→[agro-et0-fao56]، WX-006→[agro-gdd-accumulation]) + `evidence_policy` (functional فقط، رفض liveness، مطابقة SHA، بيئة مطلوبة، max_age 30 يوماً). (٢) أداة `scripts/ci/runtime_identity_bridge.py` **للقراءة فقط**: `--check` تحقّق fail-closed للخريطة؛ `--dry-run` يُظهر ما سيتغيّر ولماذا وما يبقى صفراً — لا يكتب runtime_verified/production_certified أبداً. (٣) 11 اختباراً: خدمة مجهولة، mapping غامض، تكرار متعارض، بلا دليل، liveness-only، عدم تطابق SHA، دليل قديم، تغطية جزئيّة، وتغطية كاملة **مع تأكيد أنّ السجلّ لا يُمَسّ**. (٤) ربط الدليل بـSHA: أضيف `tested_sha` لمخرَج `functional_probe_runner`.
- **برهان السلسلة (ديمو، أدلّة gitignored غير مُودَعة):** بأدلّة وظيفيّة حيّة (tested_sha=HEAD، env، functional، كلا الـprobe مرّا) ⇒ dry-run يُظهر WX-004/WX-006 **WOULD SET** لكنّه **يكتب 0** (read-only)؛ بعد حذف الأدلّة gitignored ⇒ dry-run يعود إلى **0** would-flip، 2 stays_zero.
- **الحالة النهائيّة (كما طُلِب):** identity bridge = ready · runtime_verified = 0 · production_certified = 0. Step 3 (تشغيل حيّ في بيئة معتمدة) وStep 4 (flip على القدرات المثبتة) يبقيان خطوتين مستقلّتين لاحقتين. management 81/81، runtime/production 0؛ كلّ البوّابات خضراء.

## 2026-07-27 — دمج طبقة functional probes (#665 `7f8f3dd`) + جسر الهويّة (#666 `479c09d`)
- المدخلان أعلاه (طبقة الـprobes + الجسر) كُتِبا قبل الدمج؛ يثبّت هذا السطر SHA الدمج: **#665 `7f8f3dd`** · **#666 `479c09d`**. الحالة بعد الدمج بلا تغيير: bridge ready · runtime_verified 0 · production 0 · management 81/81 · bundle 5127.

## 2026-07-27 — تعميم جسر الهويّة لخدمة ثانية (soil-service) — #667 `4ac10bf`، خامل
- **الطلب (المالك، الخيار الثاني):** توسيع الجسر لخدمة ثانية مع بقاء `runtime_verified=0` — قابل للإنجاز في التطوير (probe نقيّ) دون بيئة معتمدة، بدل flip مستحيل صدقاً في sandbox.
- **التسليم:** `runtime-verification/service_identity_map.json` صار متعدّد الخدمات (`soil-service → services/soil-service/main.py`, one-to-one) + تغطية **SOIL-001** (قوام USDA) تتطلّب probe‑ين. خطّة `runtime-verification/functional_probes/soil-service.json` على `POST /soil/suitability` (حسابيّ حتميّ نقيّ؛ تحقّقت المخرجات مقابل `services/soil-service/soil_science.py::usda_texture_class`: {20,40,40}→loam، {5,90,5}→sand). `scripts/ci/functional_probe_runner.py`: الماسح صار يطابق `@router.<method>` (soil على APIRouter بلا prefix) + دعم `headers` بتوسيع `${ENV}` لبوّابة التوكن (لا سرّ مُودَع؛ متغيّر غير مضبوط→فارغ→فشل الـprobe لا تمرير كاذب).
- **الاختبارات:** +12 (`tests/architecture/test_runtime_identity_bridge.py` + `test_functional_probe_plan.py`): تحقّق متعدّد الخدمات، عزل الأدلّة لكلّ خدمة، ومصفوفة fail-closed للخدمة الثانية (خدمة مجهولة، تعارض مسار، دليل قديم/SHA غير مطابق/liveness فقط، تغطية جزئيّة، تغطية كاملة would-flip دون كتابة). bundle 5128؛ management 81/81 runtime/production 0؛ ruff نظيف.

## 2026-07-27 — تعميم جسر الهويّة لخدمة ثالثة (sahool-platform) — #668 `b8ddc5f`، خامل
- **الطلب (المالك):** «استمر» — خدمة ثالثة بنفس النمط الصادق.
- **اكتشاف حاكم (صدق):** المرشّحون الأنقى (`agriai-engine /replay/verify` · `guardrails /validate`) **غير مسجَّلين في `services[]` لأيّ قدرة** ⇒ ربطهم بالجسر اختلاق يرفضه `validate_identity_map`. من الخدمات العشر المسجَّلة، `services/sahool-platform/api/main.py` هي الوحيدة بـendpoint نقيّ حتميّ.
- **التسليم:** `sahool-platform → services/sahool-platform/api/main.py` (one-to-one) + **IRR-009** «ملاءمة مياه متعدّدة المخاطر» + **IRR-010** «الغسيل والصرف»، كلٌّ بـprobe مستقلّ على `POST /api/v1/salinity/assess` (`runtime-verification/functional_probes/sahool-platform.json`، entrypoint `routers/salinity.py` على APIRouter بلا prefix). القيم مُتحقَّقة مقابل `api.salinity_management`+`core.engines.fao56`: `soil_salinity.class=moderately_saline` · `water_salinity.risk=severe` · `sodium_hazard.class=medium` · `leaching.leaching_fraction=0.111` (11.1٪). endpoint محميّ بـJWT ⇒ header `Authorization: Bearer ${SAHOOL_PLATFORM_PROBE_JWT}` (اسم فقط).
- **عقبتا CI حُلّتا بصدق:** (١) `no-report-only-change` حجب لأنّ `SUBSTANTIVE_PREFIXES` تجهل `tests/` و`runtime-verification/` — أُضيفتا (جوهريّتان: اختبار = فئة «test» التي يدعوها الحارس؛ خطّة probe/خريطة هويّة = مواصفة سلوكيّة) مع إبقاء حجب التقرير-فقط البحت + اختبارَي انحدار في `tests_v9/test_no_report_only_change_guard.py`. (٢) `capability-registry` سباق توقيت: البوّابة قرأت نصّ الـPR قبل تحديثه؛ التصريح الصحيح `Capability-Impact: IRR-009, SAT-007, SOIL-001, WX-004, WX-006` (السجلّ يربط `tests_v9/test_no_report_only_change_guard.py` بـSAT-007 «كشف التغيير»)، أُعيد التشغيل برأس جديد `71011b1` فمرّ. bundle 5129؛ **3 خدمات/5 قدرات**؛ runtime/production 0؛ ruff نظيف.

## 2026-07-27 — صيانة الدماغ لقوس التحقّق التشغيليّ + إعفاء توثيق الدماغ — #669 `a00891c`
- **السياق:** بروتوكول نهاية الجلسة في `CLAUDE.md` كان متأخّراً ثلاث دمجات (الدماغ متوقّف عند 2026-07-20). سُجِّل قوس #665→#668 كاملاً (لقطة hot + مداخل log بـSHA + قرارا ledger + فجوة `RUNTIME-FUNCTIONAL-LIVE-PROOF`).
- **عقبة حوكميّة حقيقيّة:** `no-report-only-change` حجب تحديث الدماغ **المستقلّ** لأنّ `sahool-brain/gaps/registry.md` يطابق تلميح «REGISTRY» فيُصنَّف «تقرير». الدماغ **توثيق مُلزَم** بتسويغ صارم لكلّ حقيقة، لا تقرير شهادة ⇒ أُعفي `sahool-brain/` من تصنيف report-like في `scripts/ci/no_report_only_change_guard.py`، مع **إبقاء الحجب** على سجلّ الشهادة `capabilities/` والتقارير المُولَّدة، واختبارَي انحدار يثبتان الأمرين.
- **الدرس:** حين يمنع حارسٌ صيانةً مُلزَمة، الإصلاح الصادق توسيع الحارس بمبرّر مكتوب — لا افتعال تغيير كوديّ لا لزوم له لتمرير الحزمة.

## 2026-07-27 — طبقة ثقة الأدلّة التشغيليّة (runtime_trust_v2) — #670 `0dad1a1`، للقراءة فقط وخاملة
- **المصدر:** حزمة `sahool_main_a00891c_runtime_evidence_full_provenance_closed.zip` (مرآة مستودع كاملة فوق `a00891c`) + تقريرها `RUNTIME_EVIDENCE_PROVENANCE_CLOSURE_REPORT_20260727.md`.
- **تحقّق الصدق قبل الدمج (أربع فحوص، كلّها مرَّت):** (١) لا دليل وظيفيّ/تشغيليّ مُودَع؛ (٢) `capabilities/registry/capabilities.json` **مطابق بايتيّاً** لـmain ⇒ لا قدرة قُلِبت؛ (٣) لا سكربت يكتب `runtime_verified`/`production_certified` (تحقّق grep + تصريح التقرير «No Step-4 writer exists»)؛ (٤) `evidence_policy` صار **أصرم فقط** (+`require_attestation` +`require_external_provenance` +`require_trusted_environment` +`require_runtime_identity` +`require_artifact_digests` +`max_future_clock_skew_seconds`) وإدخالات الهويّة/التغطية لم تتغيّر.
- **التسليم:** ورشة بناء صور موثَّقة (`runtime-image-provenance.yml`) · `prepare_attested_runtime_images.py` · `provenance_receipt.py` · `runtime_deployment_manifest.py` (يفصل digest التوزيع عن digest تهيئة Docker ويشترط مطابقة RepoDigest للصورة العاملة) · `runtime_replay_guard.py` (رفض digest مستهلَك + علامة استهلاك append-only) · `trusted_environments.json` (بيئة واحدة مؤهّلة `staging-pg16`؛ `local`/`sandbox` **صراحةً** `eligible_for_runtime_verified: false`) · `shared/runtime_identity.py` + نقطة `GET /runtime-identity` (weather/soil فقط) · `functional_probe_runner` أُعيد كتابته لينتج أدلّة موقَّعة مربوطة بالصورة (schema 2.0) ويشترط بيان نشر · CI صار يشغّل `--check` للـprobes والجسر و**يؤكّد** بقاء dry-run على `would_set_runtime_verified: 0` / `production_certified: 0` · حارس `brain_state_transition_guard`.
- **قرار المالك (حاسم):** المنصّة **لا** تعرض `/runtime-identity` — أُسقِط المسار حفاظاً على سقف المسارات (633/629، نفس الراتشِت الذي حماه المراجع في INT-004)؛ هويّة صورة المنصّة من **بيان النشر المرجعيّ** (`runtime_deployment_manifest`: «image identity may not come from runtime env»)، والنقطة كانت تدقيقاً ثانويّاً فقط. weather/soil تحتفظان بنقطتيهما.
- **ضريبة التكامل (درس تشغيليّ مهمّ):** حزمة بهذا الحجم تُبطِل **عشرات** الملفّات المولَّدة عبر ~28 بوّابة `--check`، وكلّ وظيفة CI تتوقّف عند **أوّل** انجراف ⇒ تظهر واحدة تلو الأخرى عبر جولات متتالية. الأنجع: عند أوّل انجراف، أعِد توليد **كامل** سلسلة الوظيفة (mapping · impact-index · architecture/event/database/decision-lineage graphs · execution-dependency-audit · gateway/router reachability · route-conflict · integration-runtime-closure · platform catalog · main-subinventory · route-residual · report-index · production-evidence-pack) ثمّ **بيان static-closure** ثمّ **حزمة الإصدار أخيراً**. تحذير: الماسحات البطيئة (`architecture_graph`/`capability_mapping`) تتجاوز 30 ثانية — `timeout` قصير يعطي «انجرافاً» كاذباً.
- **إصلاحات تكامل محفوظة السلوك:** `ruff format` لنمط الحزمة المضغوط (467 خطأ → نظيف) · دبابيس كتالوج المنصّة رُفِعت بوعي (`PINNED_UNIQUE_METHOD_PATH` 989→990 و`duplicate_groups` 14→15 لمجموعة `/runtime-identity` في weather+soil، مُتحقَّقة مقابل الكتالوج المُعاد توليده) · متغيّرات هويّة البناء الأربعة في `.env.example` · إزالة `sahool-platform` من قائمة اختبار source-grep للنقطة المُسقَطة. **الحالة النهائيّة:** 70/70 فحص أخضر · management 81/81 · runtime 0 · production 0 · bundle 5144.

## 2026-07-27 — صيانة الدماغ لـ#669/#670 + إسناد تنفيذيّ للادّعاءات — #671 `f396221`
- **السياق:** الدماغ كان متوقّفاً عند #668؛ سُجِّل الدمجان الناقصان (لقطة hot لـ`runtime_trust_v2` + مدخلا log + ثلاثة قرارات ledger + إعادة كتابة فجوة `RUNTIME-FUNCTIONAL-LIVE-PROOF` بوصفة Step-3 التشغيليّة الدقيقة).
- **عقبة حقيقيّة (وقرار صدق):** `brain_state_transition_guard` — عمره يوم واحد — حجب تعديلاً **توثيقيّاً بحتاً** لأنّ أسطري تقول `runtime_verified=0`؛ الحارس يطابق **الرمز** لا معناه، فيحجب النفي كما يحجب الادّعاء. رفضتُ إضعافه ورفضتُ تجريد الدماغ من الحقل الذي يقيس صدقه؛ المخرج الثالث: `tests/architecture/test_brain_state_consistency.py` يقرأ الادّعاءات **الرقميّة فقط** (`_NUMERIC_CLAIM`) ويطابقها مع السجلّ المُولَّد — ضيّق عمداً كي يبقى النثر النافي شغّالاً، ومُثبَت أنّه قابل للتكذيب.
- **الدرس (مكرَّر ومكلِف):** بوّابة `capability-registry` تقرأ **وصف الـPR وقت تشغيل الوظيفة**؛ تحرير الوصف وحده **لا** يعيد تشغيل CI ⇒ يلزم إعادة إطلاق (`git commit --amend --no-edit --reset-author` + `--force-with-lease`). سقطتُ فيها أكثر من مرّة هذه الجلسة.

## 2026-07-27 — ميزانية المسارات ثلاث طبقات + تثبيت الموضع بعقد — #672 `cae51f7`
- **المصدر:** خمس حزم متتالية من المالك (`…step4_route_budget_ownership_reconciled` → `…step4_route_governance_attested` → `…step4_route_release_bound` → `…runtime_identity_route_placement_contract_closed` + sidecar `.routegovernance.json`).
- **قرار المالك:** **«Exclude infra from budget»** — ناقضاً قرار #670 (إسقاط المسار). العدّ صار ثلاث طبقات معلَنة: **خام 630 · بنية 4 · نطاق 626 ≤ 629**؛ السقف لم يُرفَع والجرد الخام بقي ظاهراً. المطابقة **دقيقة** على (method، مسار مُطبَّع سلاشيّاً) من `INFRASTRUCTURE_ROUTES` المركزيّة، واستخراج AST حرفيّ-فقط يفشل مُغلَقاً ⇒ `/fields/runtime-identity` و`/runtime-identity/export` تبقيان محسوبتين (اختباران).
- **تثبيت الموضع (بأمر المالك «ثبّت موضع الراوتر في مصدر الحزم»):** **أربع** حزم متتالية أعلنت `/runtime-identity` في `api/main.py` فأسقطت `p1_main_decomposition_guard` على أشجارها — العُرف في `CLAUDE.md` وحده لم يمنع التكرار. اعتُمِد عقد الحزمة `docs/architecture/platform_route_placement_contract.json` ووُسِّع من نقطة إلى أربع، ويفرضه `platform_route_placement_guard.py` **بلا pytest** (وظيفة `platform-route-budget` عديمة التبعيّات عمداً). أُسقطت نسختي المكرَّرة (`CANONICAL_DECLARATION_SITES`) لصالح العقد الواحد.
- **تصحيح واقعيّ لمراجعة المالك:** حكم REQUEST CHANGES ادّعى أنّ الحزمة تفتقر القائمة المركزيّة والمطابقة الدقيقة؛ شغّلتُ اختبارات الحزمة **في شجرتها هي** فمرّت (p2_6 عشرة اختبارات) وطبع حارسها 630/4/626 — و`assert 630 <= 629` جاء من p2_6 **ما قبل الحزمة**. أبلغتُ ذلك صراحةً، وأشرتُ إلى العيب الحقيقيّ الذي فاتها: حارس p1.
- **أخطائي في هذه الجولة (للسجلّ):** (أ) تعليق أضفتُه إلى `main.py` خرق سقف الأسطر (2559 > 2553) وأسقط اختباري أنا ⇒ رُجِّع، والحُرّاس كانت على حقّ. (ب) أقحمتُ خطوة pytest في `platform-route-budget.yml` فأخرجت `No module named pytest` ⇒ نُقِل الاختبار إلى `capability-governance.yml`.
- **ترتيب المولَّدات (تثبيت الدرس):** `execution_dependency_audit` و`static_governance_closure` يبصمان مجموعة الملفّات المتعقّبة كاملة ⇒ يُعادان **أخيراً**؛ مجموعة حوكمة المسارات تُبصَم بعد كلّ كاتب آخر؛ حزمة الإصدار **آخر ما يُبنى**؛ ثمّ التكرار حتى **نقطة ثابتة**.
- **الحالة النهائيّة:** الراتشِت 66/66 (نجاح أو skip) · `runtime_verified=0` · `production_certified=0` · management 81/81 · خام 630/بنية 4/نطاق 626/ملكيّة 634 · bundle 5176 · `platform route release binding: PASS`.

## 2026-07-27 — تدقيق المالك على حزمة `cae51f7`: إغلاق فجوة ربط عقد الموضع
- **المصدر:** `sahool_main_cae51f7_route_governance_placement_audited_fixed.zip` (sha256 `e800e75f…`، 14,073,778 بايت — طابقت ادّعاء المالك بالضبط).
- **التحقّق قبل الدمج:** سجلّ القدرات **مطابق بايتيّاً** (`79ceee97…`) ⇒ صفر قلب؛ 81 قدرة 0/0؛ `runtime_verification_apply.py` موجود سلفاً على `cae51f7` و**غير معدَّل** (ليس كاتب Step-4 جديداً). الفرق مقابل `cae51f7` **أربعة ملفّات فقط** (حارس + اختباره + checksums + manifest) — أنظف حزمة حتى الآن، بلا حمولة جانبيّة.
- **الفجوة صحيحة ومؤكَّدة تجريبيّاً** (لم أكتفِ بقراءة الوصف): على `cae51f7` تحريران متناسقان (`INFRASTRUCTURE_ROUTES` + allowlist خريطة الاستخراج) يُعيدان تصنيف `/api/agent/health` كبنية ⇒ **النطاق 626⇒625 والهامش 3⇒4**، وكلّ الحُرّاس تمرّ. الإصلاح يوقفها.
- **ملاحظة تنسيق متكرّرة:** ملفّ الاختبار في الحزمة لم يكن نظيف `ruff format` (نفس نمط الحزم السابقة) ⇒ نُسِّق قبل الالتزام.
- **الحالة:** 8 اختبارات موضع + 254 اختبار معماريّ ناجح (كانت 250 قبل إضافة الأربعة). الأربعة الفاشلة بيئيّة **مُثبَتة مسبقة الوجود** على شجرة `origin/main` نظيفة (python 3.11 مقابل 3.12 المسجَّل + رسالة docker socket مختلفة). سطح المسارات وبيان الحوكمة بلا تغيير؛ bundle 5176.

## 2026-07-28 — بدء WX-10 (CanonicalWeatherState): الإنكرمنت 10.4 — خانة `current`
- **لماذا الطقس أوّلاً:** الترتيب المُعتمَد في `sahool-brain/decisions/architecture_roadmap_wx10_ci11.md` يضع WX-10 قبل CI-7/CI-8، وCI-7 ينصّ: «Crop لا يعرف المنتجات الجزئيّة إطلاقاً». الواقع كان مخالفاً: `core/crop_intelligence/models.py:10-11` يحمل `gdd_cumulative`/`gdd_to_maturity` كـscalars، و`CanonicalWeatherState` بلا مستهلك إنتاجيّ **خارج** weather-service.
- **تصحيح لتقديري السابق:** قلتُ «صفر مستهلك إنتاجيّ» — الأدقّ: صفر **خارج** weather-service. داخلها، `weather_runtime` يفوّض للـViews سلفاً وWS-C.2 يقفلها. الفجوة الحقيقيّة كانت الخانات الأساسيّة لا الاشتقاقات.
- **المُنجَز (WX-10.4):** `current` من deferred إلى composed · `_current_product` نقيّ fail-closed · `current_view` بعقد النَّسَب الكامل · `current_weather` يفوّض بدل تمرير حمولة المزوّد · WS-C.3 في `consumer_contract_gate`.
- **خطأ رصدتُه في نفسي قبل الدفع:** النسخة الأولى من `_current_product` بَنَت المخرَج من قائمة حقول معروفة ⇒ كانت **ستُسقِط** كلّ حقل خارجها (`wind_speed_10m_kmh`, `soil_temperature_*`, `soil_moisture_*`, `timestamp`) من ردّ نقطة عامّة. اختبار الـruntime في CI يؤكّد حقلين فقط (`source`, `temperature_c`) فكان سيمرّ **أخضر وهو يفقد بيانات**. أُعيدت الكتابة: نسخ المشاهدة كاملة ثمّ تركيب الغلاف، + اختبار `test_current_slot_preserves_every_normalizer_field_not_in_the_quality_list` يحرس ذلك، + اختبار تعارض مفاتيح الغلاف.
- **قيد بيئيّ:** `fastapi` غير مثبَّت في هذه الحاوية ⇒ 10 وحدات اختبار weather لا تُجمَّع محلّيّاً. عوّضتُ بمحاكاة fixture الـruntime الحقيقيّ (`_sample`) وحمولة `normalize_current` الحقيقيّة عبر مسار الكود الجديد: التأكيدان يمرّان وصفر حقل مفقود. CI هو الفصل.
- **الحالة:** 37 اختبار حالة كنسيّة أخضر · `consumer_contract_gate` أخضر وقابل للتكذيب · `weather_engine_formula_guard` أخضر (Zero-Legacy LOCKED) · ruff نظيف.

## 2026-07-28 — WX-10.5: خانتا `forecast` و`historical`
- **المُنجَز:** منتَج نقيّ **واحد مشترك** للسلسلتين (`_daily_series_product` بمعامل `slot`) لأنّ المُطبِّع واحد — لا ازدواج منطق ولا عقدان يتباعدان. `forecast_view`/`historical_view` فوق مساعد نَسَب واحد `_slot_view` (يُفحَص عقد النَّسَب فيه مرّة واحدة في الحارس بدل تكراره). المُعالِجان يفوّضان، وWS-C.4 يقفلهما.
- **قرارات جودة صادقة:** يوم واحد ناقص يُنزِل الجودة ويُسمّي الحقل (لا يبتلعه متوسّط) · حقول التوقّع الحصريّة غائبة في الأرشيف ⇒ `optional_missing_fields` بلا إنزال جودة (وإلّا صار كلّ ردّ أرشيفيّ «degraded» كذباً) · `days: []` ⇒ `insufficient` لا «سليمة فارغة» · خانتان مستقلّتان (تمرير إحداهما لا يُتيح الأخرى).
- **`_DEFERRED_SLOTS`: 7 ⇒ 4.** لم يبقَ خارج الحالة إلّا مشتقّات (`heat_load`/`chill_hours`/`frost_risk`/`operation_windows`)، لا مشاهدات أساسيّة.
- **توسيع الفجوة:** `normalize_daily` يحمل عيب الصفر نفسه (`_at(..., idx, 0)`) ⇒ أُعيدت تسمية الفجوة إلى `WEATHER-NORMALIZER-ZERO-COERCION` وشملت المسارات الثلاثة. `temp_max_c` مفقودة تصير 0°م فتنهار GDD/ET0 المشتقّة — أخطر من الحالة الآنيّة.
- **التحقّق:** 48 اختبار كنسيّ · الحارس أخضر وقابل للتكذيب على `forecast_weather` · محاكاة fixtures الـruntime الحقيقيّة للتوقّع والأرشيف: التأكيدان (`len(days)==2`, `model=='ERA5'`) يمرّان و**صفر حقل مفقود** · ruff نظيف.

## 2026-07-28 — دمج حزمة «القدرات العشر القابلة للإغلاق» (فرع مستقلّ عن WX-10)
- **المصدر:** `sahool_main_01168b0_ten_capability_closable_execution.zip` (sha256 `9f1f319c…`). الفرق مقابل `01168b0`: **14 مدخلاً**، بلا حمولة جانبيّة. السجلّ مطابق بايتيّاً (`79ceee97…`) 81 قدرة **0/0**.
- **الفجوة التي بلّغتُ عنها أُغلِقت فعلاً:** `ROUTE-ARCHIVE-BINDING-CLI-TAUTOLOGY` — وضع الفحص صار فرعاً `elif` مستقلّاً **قبل** الكتابة. اختبرتُه **بالوصفة الموثَّقة نفسها** التي كانت تمرّ كذباً: العبث بـ`domain_budget_routes` يُسقِطه الآن، والـsidecar المعبوث **لم يُعَد كتابته**. 7/7 اختبارات ربط.
- **CI-7 موصول حقيقيّ:** `resolve_phenology_inputs` في `engine.py` — الـGDD من CanonicalWeatherState، والـscalars القديمة جسر توافق **موسوم**، وحالة مشوّهة **تفشل مُغلَقة بلا ارتداد صامت**. 42 اختبار crop intelligence أخضر (لا انحدار من تعديل المحرّك).
- **ما سجّلتُه بصدق:** أربع وحدات (`canonical_field_state`/`yield_intelligence`/`equipment_intelligence`/`economic_scenarios`) **بلا أيّ مستهلك غير اختباريّ** ⇒ groundwork لا دورة تكامل، بسابقة INT-004. الوثيقة صادقة ولا تدّعي أكثر، لكنّ التسجيل يمنع قراءة «CORE CLOSED» كأنّها إنتاج. الفجوة: `CAPABILITY-CORES-NOT-WIRED`.
- **تصحيح ذاتيّ:** ظننتُ أنّ اختبارات main تستورد وحدة مفقودة — خطأ منّي؛ الاستيراد لـ`canonical_field_state_lock` الموجودة، وmain سليم (8/8).
- **نظافة:** الحزمة حملت `__pycache__` (استُبعِد)، و24 خطأ ruff بقيت بعد الإصلاح التلقائيّ ⇒ نُسِّقت 8 ملفّات قبل الالتزام.
- **قرار نطاق:** فرع/PR مستقلّ حفاظاً على نقاء #674 (WX-10) — كلّ PR قابل للمراجعة والارتداد وحده.

## 2026-07-28 — CI-9 محرّك السياسات (حزمة `…ci9_policy_engine_closed`) على فرع القدرات نفسه
- **المصدر:** sha256 `e8f0631f…`. **مجموعة فائقة نظيفة** من حزمة «القدرات العشر»: الفرق بينهما **6 بنود فقط** (وثيقة + `policy_engine.py` + اختباره + تعديل `__init__`/`engine`/`recommendation_context`) ⇒ ضُمَّت إلى فرع #675 بدل فتح PR ثالث (CI-9 يبني على CI-7 مباشرة).
- **ما يفعله فعلاً:** يُخرِج سلسلة `if` المبعثرة من `engine.py` إلى حِزَم سياسات مُصدَّرة — الأنواع الستّة كما في خارطة الطريق (crop/water/weather/regional/cultivar/business)، بـ`policy_id@version` في `evidence_ids` و`policy_digest` حتميّ، وقائمة triggers مغلقة تفشل على غير المعروف، وتحقّق من النوع المفقود/المكرَّر والقاعدة بلا أثر.
- **حدّ صدق مضمَّن في المخرَج:** `decision_boundary: {is_decision: false, consumer: "decision-service"}` — المحرّك **لا يقرّر**، يُنتِج تقييماً؛ حدّ الحوكمة محفوظ.
- **تغيير سلوكيّ رصدتُه وتحقّقت من مأمونيّته:** مصدر علم الإجهاد تغيّر من `water_state`/`weather_state` إلى `policy:sahool.water.baseline` وأمثالها. فحصتُ المستهلكين: `decision_explain` يقرأ `code`/`label_ar` فقط، و`crop_decision_bridge` يمرّر القائمة كما هي، و`crop_twin.py` يبني أعلامه بمسار مستقلّ ⇒ **لا مستهلك يقرأ `source`**. والتغيير تحسين نَسَب (المصدر صار يسمّي السياسة التي أطلقت العلم).
- **التحقّق:** 47 اختبار crop intelligence أخضر (شمل الاختبارات القائمة قبل CI-9 ⇒ لا انحدار) · `crop_intelligence_boundary_gate` أخضر · 4 أخطاء ruff أُصلِحت و3 ملفّات نُسِّقت.

## 2026-07-28 — ترتيب المولّدات الطرفيّ: اكتمل بمولِّدَين كانا ناقصَين
- **العطلان في CI (لا في المنطق):** `generate_service_inventory` ثمّ `runtime_contract_generator`. كلاهما **يبصم مجموعة الملفّات لا العقود** — فأيّ إضافة كود تُسقِط بوّابته حتى لو لم يتغيّر أيّ مسار أو عقد. الأوّل: عدّادات `python_files`/`python_loc`/`tests`. الثاني: قائمة ملفّات الخدمة في `runtime-contracts/generated/runtime_contracts.json`.
- **الحلّ المُلزَم:** إعادة توليد، **لا تحرير يدويّ لأيّ بصمة** — تحرير checksum يدويّاً يُنتِج شجرة لا توجد، وهو «الأخضر الكاذب» بعينه.
- **القائمة الكاملة الآن (ثمانية، تُعاد حتى نقطة ثابتة):** `runtime_contract_generator` · `generate_service_inventory` · `capability_mapping_engine` · `capability_management_engine` · `pr_capability_impact_gate --generate-index` · `report_index_guard` · `execution_dependency_audit` · `static_governance_closure` — ثمّ `platform_route_release_binding --write-source` وأخيراً `build_release_bundle`.
- **درس:** «المولّدات التي تبصم مجموعة الملفّات» فئة قائمة بذاتها؛ لا تكتشفها بقراءة الـdiff لأنّ تغييرك قد لا يمسّ ملفّاتها إطلاقاً.
- **تصحيح منهج (بعد ثلاث جولات CI متتالية على العيب نفسه):** كنت أُصلح مولِّداً واحداً في كلّ جولة فيكشف CI التالي مولِّداً آخر — هدرُ وقت وسلوك تفاعليّ. **الطريقة الصحيحة:** استخراج **كلّ** خطوات `--check` من `.github/workflows/*.yml` (٤٥ خطوة) وتشغيلها محلّيّاً دفعة واحدة. كشفت **ستّة** منجرفة فوراً (`capability_evidence_maturity` · `decision_lineage_graph` · `event_contract_graph` · `execution_dependency_audit` · `runtime_verification_harness` · `static_governance_closure`)، ثمّ **اثنان إضافيّان** ظهرا بعد إعادة التوليد لأنّهما يقرآن مخرجاتها (`integration_runtime_governance_closure` · `runtime_evidence_ingestion`). الأمر الواحد:
  `grep -rhoE "python scripts/(ci|release)/[a-z0-9_]+\.py --check[a-z-]*" .github/workflows/*.yml | sed 's/^python //' | sort -u`
- **حدّ صدق عند لمس أدلّة التشغيل:** `runtime_evidence_ingestion` يحسب `runtime_verified` **لكلّ خدمة في سجلّ الأدلّة** (لا في سجلّ القدرات)، ونصّه صريح: «Static repository evidence cannot set runtime_verified». تحقّقتُ قبل/بعد التوليد: **سجلّ الأدلّة 0 خدمة موثَّقة · سجلّ القدرات 0/0 · `capabilities/` بلا أيّ تعديل**.

## 2026-07-28 — استخراج من `claude/verify-all-generated`: الزاوية لا الشيفرة
- **الطلب:** «استخرج ما يضيفه الفرع الآخر أوّلاً». الفرع بنى نسخة bash موازية لمكنسة المصنوعات تحت معرّف آخر (`GENERATED-CHAIN-UNDOCUMENTED-01`). **الشيفرة مكرّرة، والزاوية ليست:** المكنسة المدموجة تكتشف خطواتها من الـworkflows، فمولّد **لا يذكره أيّ workflow** خارج مداها بحدّ تعريفها.
- **أُعيد القياس لا نُقِل:** الفرع الأصل قاس ثلاثة منحرفة. القياس على `780d0544`: **٦١ سكربتاً** يُعلن `--check`، **٦** خارج كلّ workflow، **٥** منها منحرفة. الفارق ليس اجتهاداً: `capability_linker` غاب عن قياسه، و`platform_route_release_binding` (في `scripts/release`) لم يُمسح أصلاً.
- **خطأ قياس داخل الجلسة، صُحِّح قبل الالتزام:** عددتُ `api_versioning_policy_guard` سليماً في أوّل جولة. السبب أداتيّ لا حُكميّ: `python … | tail -1` ثمّ قراءة `$?` تُعيد رمز `tail` (صفر دائماً). أُعيد القياس بلا أنبوب ⇒ خمسة منحرفة لا أربعة. **الدرس:** رمز الخروج خلف أنبوب ليس رمز الخروج.
- **السبب ثلاثيّ لا واحد** — وهذا أهمّ ما أضافه التحقّق: ثلاثة حرّاس في `tests/architecture/` خارج `testpaths` · واحد في `tests_v9` **بلا علامة** فتستبعده كلّ وظائف CI · وواحد بلا حارس. الأساس يسجّل **سبب صمت الحارس** لكلّ مدخل، لا وجوده فقط.
- **فجوة جديدة `TESTS-UNMARKED-DESELECTED-01` (P1):** **٦٩ من ٥٩٢** ملفّاً في `tests_v9/` بلا علامة، ولا وسم تلقائيّ في `conftest.py`. الملفّ داخل `testpaths` فيبدو حيّاً ويُجمَع محلّيّاً، بينما `-m unit`/`-m integration`/`-m security` تستبعده كلّه. الدليل الملموس: `test_api_versioning_policy_guard` يفشل عند تشغيله (الجرد منحرف على main) ولا يعمل في أيّ وظيفة.
- **الحارس الجديد كاد يولد خاملاً:** `tests/architecture/test_verify_all_generated.py` المدموج في #691 لم يكن على قائمة أيّ workflow. أُدرِج في `capability-governance.yml` — لولا ذلك لكان الحارس مثالاً حيّاً على الفجوة التي بُني ليحرسها.
- **التكذيب (أربعة اتّجاهات):** مولّد جديد يدعم `--check` بلا تصنيف ⇒ فشل · مدخل بائت في الأساس ⇒ فشل · «الأساس يقول سليم وقد انحرف» ⇒ فشل · «الأساس يقول منحرف وقد سلُم» ⇒ فشل يطالب بحذف المدخل (فالقائمة تتقلّص ولا تُهادَن).
- **قرار تصميم:** التصنيف **لا ينفّذ** افتراضيّاً. خمسة من الستّة تكتب ملفّات متعقَّبة أثناء `--check` (`CHECK-STEPS-MUTATE-THE-TREE-01`)؛ تشغيلها ضمن المكنسة يُوسّخ شجرة من يُشغّلها بلا طلبه. التنفيذ خلف `--uncovered` صراحةً.
- **التحقّق:** `pytest -m unit` ⇒ **3482 نجح · 26 تخطٍّ · 0 فشل** · قائمة `capability-governance` كاملة (34 ملفّاً) ⇒ **207 نجح** · ruff نظيف بعد التنسيق · `verify_all_generated --fix` ⇒ اتّساق كامل (٤٧ خطوة).
- **ملاحظة بيئة:** `cryptography` في `/usr/lib/python3/dist-packages` كان ينهار بـ`pyo3_runtime.PanicException` فيمنع جمع `tests_v9` كلّه؛ حُلَّ بتثبيت نسخة في `/usr/local` تسبقه في `sys.path` + `tests_v9/requirements-test.txt`. بيئة الحاوية فقط — لا أثر في الشجرة.

## 2026-07-28 — مراجعة لقطة `imagery_empty_raster_cache_guard_delta`: تنفيذان يتّفقان، وتغطيتان لا
- **ما هي:** لقطة على أساس `121ab09` تحمل تنفيذاً مستقلّاً لنفس إصلاح `IMAGERY-BLANK-THUMBNAIL-01` المدموج في #688 — `raster_has_observable_content` بتوقيع `(raster_path, index)` بدل `(cog_path)`.
- **لم أقارن نصّاً، شغّلتُ الاثنين:** سبع حالات بُنيت كملفّات راستر حقيقيّة (RGB صالح · RGB بقناع صفر · مؤشّر محدود خلف قناع باطل · RGBA بألفا صفر · RGBA ببكسل معتم · كلّه NaN · nodata كامل). **٧/٧ متطابقة** ⇒ لا سلوك يُستورَد.
- **ورفضتُ توقيعها بسبب:** وسيط `index` يُكرّر `cdse_client.is_truecolor` بقائمة مكتوبة تختلف عنها في الاتّجاهين (تقبل `rgb`/`rgba`، وترفض `true-color` لأنّها لا تطبّع الشرطات). التنفيذ عندنا يقرّر بالبنية فلا يحتاج السلطة المُكرَّرة.
- **المُستخرَج فعلاً — أربع حالات كانت بلا حارس:** ثلاث على القناع الداخليّ (`write_mask`) وعلى RGB بثلاثة نطاقات، وواحدة على المسار الموصول ببايتات تالفة. مجموعنا صار ١٨ بعد ١٤.
- **لمَ تهمّ (وهذا جوهر الفائدة):** تنفيذنا صحيح في الثلاث **بالتبعيّة** — من دلالة `read(1, masked=True)` — لا بفرع مكتوب. **مُكذَّب:** استبدالها بـ`src.read(1)` يُسقط الجديدتين ويُبقي الإحدى عشرة القديمة خضراء. أي أنّ «تبسيطاً» معقولاً كان سيمرّ.
- **حدّ سجّلتُه بدل ادّعائه:** اختبار البايتات التالفة لا يحرس مظروف `try/except` الداخليّ — إزالته تُبقيه أخضر لأنّ `except` الخارجيّ يبتلع الاستثناء (مُقاس). كُتب الحدّ في docstring الاختبار.
- **بقيّة اللقطة:** آثار حوكمة مُعاد توليدها على أساس أقدم (‏`service_inventory` · `execution-audit` · `governance sha256`) — لا قيمة فيها لـmain الحاليّ. ولها دلالة واحدة: **نفس ضريبة التسجيل** ظهرت عندي الآن — ملفّا اختبار ⇒ ١١ أثراً مُعاد توليده.
- **التحقّق:** ١٨ اختبار راستر أخضر · مجموعة `services/raster-service` كاملة كما يشغّلها `raster_quality_gate.sh` ⇒ **298 نجح · 2 تخطٍّ · 3 فشل بيئيّ سابق لتغييري** (‏`shapely` و`python-multipart` غير مثبّتتين محليّاً؛ CI يثبّتهما من `requirements.txt` — مُثبَت بـ`git stash`) · ruff نظيف.

## 2026-07-28 — إيقاظ ٢٤١ اختباراً كانت مُستبعَدة بالعلامة، وفجوة ظهرت في الطريق
- **الآليّة:** `-m` ينتقي، وما لا علامة له يُستبعَد من **كلّ** وظيفة. ٦٩ ملفّاً بلا علامة، ٤ منها تُشغَّل بمسار صريح ⇒ **٦٥ لا تعمل في أيّ مكان**. `conftest.py` لا يَسِم تلقائيّاً (لا `pytest_collection_modifyitems`) — الغياب غياب فعليّ.
- **قِستُ في venv نظيفة لا في بيئتي:** بيئتي مُلوَّثة (ثبّتُّ rasterio/cryptography/shapely للتحقّقات السابقة)، فبنيتُ venv بـ`requirements-test.txt` وحدها — نفس ما تثبّته وظيفة *Unit Tests*. النتيجة **مطابقة** للبيئة الملوّثة (‏٤١٥/١٣)، لكنّ المطابقة نتيجة لا افتراض.
- **«صفر تخطٍّ» هي مبرّر الوسم:** لو كان بعضها يتخطّى لكان `unit` كذباً على ما يحتاج خدمات. صفر تخطٍّ ⇒ كلّ ناجح نُفِّذ فعلاً بلا خدمة ⇒ الوسم صادق بتعريف `pytest.ini`.
- **شغّلتُ المجموعة كاملةً قبل الالتزام** لا الملفّات وحدها: الدرس مكتوب في `ci.yml` نفسه («ناجح منفصلاً ≠ ناجح مُجمَّعاً»، #590 — وبسببه تُشغَّل أربعة ملفّات معاً عمداً). لم يظهر تفاعل ترتيب: **3722 نجح · صفر فشل**.
- **عطل أداتيّ صغير أثناء الوسم، وقيمته في التشخيص:** الإدراج الآليّ بعد أوّل كتلة استيراد أشعل `E402` في خمسة ملفّات. السبب ليس الإدراج بل أنّ تلك الملفّات تُدخِل `sys.path` **بين** استيراداتها؛ ruff يسمح بذلك، ولا يسمح بإسناد قبل استيراد. عولجت بوضع العلامة بعد آخر استيراد يسبق أوّل تعريف. ruff نظيف على `tests_v9/` كلّها.
- **النمط في الثمانية الفاشلة (وهو الاكتشاف الحقيقيّ):** سبعة تؤكّد **نصّاً حرفيّاً في مصدر** انتقل بتفكيك لاحق. `test_p3_legacy_routes` مثلاً يؤكّد وجود `@app.get(...)` داخل `api/main.py` — وهو ملفّ خالٍ من المسارات **بالعقد**، يفرضه `p1_main_decomposition_guard`. أي أنّ الاختبار يصف بنيةً نُقِضت عمداً. **الخلاصة:** الاختبار الخامد يتعفّن إلى إنذار كاذب، فيبدو إيقاظه لاحقاً كسراً — وهكذا يُبرِّر الخمولُ استمرارَه. لم أُصلح السبعة: كلّ واحد يحتاج حكماً عن **أين** يجب أن يشير التأكيد الآن.
- **فجوة ظهرت لا بُحِثت — `API-VERSIONING-GUARD-IS-A-MIRROR-01`:** إغلاق `test_api_versioning_policy_guard` استلزم قراءة حارسه. «قائمة السماح» تُعاد كتابتها من الشجرة في كلّ تشغيل ⇒ الفحص «هل يطابق المُلتزَم ما أُعيد حسابه» لا «هل هذا المسار مسموح»: مسار عمل جديد غير مُصدَّر **لا يمكن** أن يُسقِط الحارس. وأشدّ: `_classify` يعرف `/v1/` ولا يعرف `/api/v1/` — **٥١٧ من ٧٤٩** في قائمة «غير المُصدَّر» مُصدَّرة فعلاً، فالدَّين المُعلَن مُضخَّم ثلاثة أضعاف. سُجِّلت ولم تُصلَح: تصحيح المُصنِّف يُعيد تصنيف ٥١٧ مساراً.
- **حدّ صدق سجّلتُه:** إعادة توليد الجرد (شرط إيقاظ الاختبار) أدخلت **٢٢** مساراً إلى الملفّ المُشتَقّ. لا إذن يُمنَح — الملفّ لا يُنفَّذ به شيء — لكنّه مُسجَّل كي لا يُقرأ لاحقاً كقبول صامت.
- **التغطية:** 46٪ ⇒ **47٪** بنفس أمر CI ⇒ الأرضيّة 42→43 بقاعدة «~٤ نقاط دون المقيس» المكتوبة في `docs/testing/coverage_ratchet.md`، لا باجتهاد.

## 2026-07-28 — شجرة `tests/` دخلت البوّابة كاملةً: إلغاء القائمة اليدويّة لا إطالتها
- **خالفتُ اقتراحي المُسجَّل عن قصد.** كنتُ كتبتُ في السجلّ: «حارس يقارن `tests/architecture/test_*.py` بقائمة الـworkflows ويفشل على أيّ غير مُدرَج، ثمّ إدراج الـ١٣». تنفيذه كان سيُبقي **العلّة نفسها** (قائمة سماح يدويّة) ويجعل الحارس صيانةً دائمة لها. المعتمَد: تشغيل الشجرة كاملةً ناقص أساس مُبرَّر.
- **قياسي الأوّل كان ناقصاً بخطأ نمط بحث:** `tests/**/test_*.py` **لا يطابق** ملفّات جذر `tests/`. المسح الصحيح: ١١٢ ملفّاً، **٦٦ غير مذكور في أيّ workflow**، بينها الخمسة عشر في الجذر كلّها. الدرس: نمط الجلوب حكمٌ ضمنيّ على النطاق — يُتحقَّق منه لا يُفترض.
- **الشكل المعتمَد ولماذا هو أقوى:** الاستثناءات تُشتقّ من `tests_tree_baseline.json` عبر `--pytest-ignores`، والحارس **يفشل** إن ظهر `--ignore=tests/` مكتوباً في أيّ workflow. أي أنّ الالتفاف على الأساس ليس ممنوعاً بالعُرف بل بالبوّابة.
- **التكذيب الحاسم (وهو الوحيد الذي يُثبت زوال العلّة):** أنشأتُ ملفّ اختبار فاشلاً في **دليل جديد** (`tests/probe_zz/`) لا يسمّيه أيّ workflow ⇒ **نُفِّذ وأسقط الوظيفة**. تحت الشكل القديم كان سيمرّ الأمر أخضر ولا شيء يُنفَّذ. تكذيبان آخران: `--ignore` يدويّ في YAML ⇒ فشل · مدخل أساس لملفّ غير موجود ⇒ فشل.
- **برهان ذاتيّ:** `tests/architecture/test_tests_tree_coverage_guard.py` لا يذكره أيّ workflow، ويحمل تأكيداً يقول ذلك عن نفسه. لو عاد الشكل إلى قائمة مكتوبة لسقط التأكيد.
- **النمط تكرّر بعيّنة ثانية مستقلّة:** خمسة من العشرة المستثناة تؤكّد **نصّاً حرفيّاً** في مصدر تغيّرت صياغته **بينما المقصد قائم**: `test_riv_p0_consumer_truth_guard` يطلب `LEGACY_DIRECT_SENTINEL_ENABLED = False` والمصدر يُعلن `"direct_provider_fetch": False` و`runtime_role: compatibility-only`. الحارس يفشل والحقيقة التي يحرسها صادقة. هذا تأكيد مستقلّ لِما رُصِد أمس في `tests_v9`: **التأكيد على النصّ يبيت، والتأكيد على السلوك لا يبيت** — ولذلك كُتب `to_close` لكلّ واحد بصيغة «حوّله إلى تأكيد سلوكيّ» لا «احذفه».
- **ما لم أفعله وسجّلتُه:** القائمة الصريحة في `capability-governance.yml` صارت زائدة (الوظيفة الجديدة تشغّلها كلّها). حذف ٣٥ سطراً من بوّابة حوكمة قرار مراجعة مستقلّ لا أثر جانبيّ لشريحة.
- **التحقّق:** أمر الوظيفة نفسه ⇒ **٥٢٤ نجح · صفر فشل · ٨ تخطٍّ** بعد إعادة توليد الحزمة · بوّابة الوحدة 3723 · ruff نظيف · `--check` أخضر على الحارسَين.

## 2026-07-29 — أوّل جولة CI على #693: عطلان، كلاهما لي، وكلاهما مُفيد
- **`Repository Tests` سقطت — والوظيفة تعمل كما صُمِّمت:** ثلاثة إخفاقات في `tests/architecture/test_brain_commit_claim_guard.py` بـ`git log … exit 128`. السبب: checkout ضحل (بلا `fetch-depth`)، والاختبار يشغّل `git log origin/main..HEAD` وعلى مدايات تاريخيّة مسمّاة (`4eded7a..121ab09` · `37c3b56~1..37c3b56`). محلّيّاً يمرّ لأنّ نسختي كاملة التاريخ.
  - **لماذا لم يظهر قبل اليوم:** الملفّ **لم يكن يعمل في أيّ مكان** — أحد الستّة والستّين. أي أنّ الوظيفة الجديدة كشفت في أوّل تشغيل لها متطلَّباً لم يكن أحد ملزَماً باستيفائه. هذا بالضبط ما كانت `ARCH-TESTS-UNLISTED-IN-CI-01` تُخفيه.
  - **العلاج:** `fetch-depth: 0` على checkout الوظيفة (كما تفعل `capability-governance`)، لا استثناء الاختبار — الاختبار صادق والبيئة كانت ناقصة.
- **`Security Scan` سقطت بـB613 — وتعلّمتُ القاعدة الدقيقة أخيراً:** bandit يحجب على **RLM (‏U+200F)** ولا يحجب على **LRM (‏U+200E)**. المسح الكامل: ١١ ملفّاً مُتعقَّباً يحمل علامات ثنائيّة الاتّجاه، **تسعة منها LRM فقط وقائمة على main وتمرّ**، واثنان RLM وكلاهما لي.
  - **تصحيح على أوّل ردّة فعل لي:** جرّدتُ الأحد عشر كلّها، وكان توسّعاً بلا سبب — ضجيج في diff على ملفّات لا علاقة لها بالـPR. أُعيدت التسعة كما كانت، وبقي التجريد على الاثنين اللذين يحجبهما الحارس فعلاً.
  - **ثُبِّت محلّيّاً:** `bandit -r services/ bots/ agents/ --severity-level high` ⇒ **High: 0**، exit=0. (‏bandit لم يكن مثبَّتاً في بيئتي — ولذلك لم يُمسَك قبل الدفع؛ صار مثبَّتاً الآن.)

## 2026-07-29 — تعارض دمج مع #692/#690: شريحتان عالجتا الفجوة نفسها بشكلين متضادّين
- **ما حدث:** بينما كان #693 في CI، دُمِج في main **#692** (يُغلق `ARCH-TESTS-UNLISTED-IN-CI-01` **بإطالة القائمة اليدويّة** + حارس يقارنها بالمجلّد) و**#690** (يفرض `CHECK-STEPS-MUTATE-THE-TREE-01` ويُبلِّغ عن المولّدات العمياء). أي أنّ الشريحتين عالجتا زاويتين كنتُ أعالجهما، بشكل مختلف.
- **التعارض الحقيقيّ لم يكن نصّيّاً بل ثابتَين متناقضَين:** حارسهم يُلزِم **كلّ** ملفّ في `tests/architecture/` بأن يُذكَر في workflow؛ واختباري `test_tests_tree_coverage_guard` يؤكّد عن نفسه أنّه **غير مذكور** (برهانه الذاتيّ). وضعه هناك يعني إسقاط أحد الثابتين.
  - **الحلّ بلا مهادنة أيّهما:** نُقِل إلى `tests/ci/` — خارج نطاق حارسهم، وداخل وظيفة الشجرة الكاملة. الحدّان صحيحان في نطاقيهما، والفصل يُبقيهما معاً. الأربعة عشر ملفّاً التي أدرجوها + ملفّي بقيت كلّها.
- **`verify_all_generated`: اتّحاد لا اختيار.** أضافوا `unreferenced_generators()` يُبلِّغ عن المجموعة، وأضفتُ `classify_uncovered()` يفرض أن يحمل كلّ عضو مدخلاً مُبرَّراً. الإبلاغ يُسمّي الحدّ في المخرَج والفرض يمنع اتّساعه — أُبقي الاثنان، وعُدِّلت رسالتهم لتبقى صادقة بعد أن صار الفحص يقع فعلاً.
- **`PATH3-READINESS-CLAIM-UNBACKED-01` (P0 عندهم) هو نفسه مدخلان في أساسي:** ربطتُ `compose_runtime_target_resolver` و`path3_runtime_readiness_closure` بمعرّفهم في `to_close` بدل إبقاء ادّعاءين مستقلّين لأثر واحد. القياسان تطابقا: نفس بصمتَي الخطّة (`f82e9be9…` مقابل `07820538…`).
- **آثار الحوكمة المولَّدة:** لم تُحَلّ يدويّاً — أُخِذت نسخة main ثمّ أُعيد التوليد. حلّ تعارض في ملفّ مبصوم بالتحرير يُنتِج شجرةً لا توجد.
- **التحقّق بعد الدمج:** الحرّاس الثلاثة خضراء (`test_marker_coverage_guard` · `tests_tree_coverage_guard` · `arch_test_ci_coverage_guard`) · وظيفة الشجرة **٥٤٠ نجح · ٨ تخطٍّ · صفر فشل** (كانت ٥٣٢ — الزيادة من اختباراتهم) · بوّابة الوحدة 3723 · التغطية 47.39٪ · bandit High: 0 · الحزمة 5226.

## 2026-07-29 — خمسة اختبارات بائتة أُعيد توجيهها بدل حذفها: الأساس ١٤ ⇒ ٩
- **الفرضيّة المُسجَّلة أمس صمدت أمام المعالجة:** الخمسة كلّها **قواعد قائمة انتقل موضعها** بتفكيك P1/P2 — لا انحدار واحد بينها. كلّ سطر مُؤكَّد موجود حرفيّاً، في ملفّ آخر. لو حُذفت الاختبارات لضاعت خمس قواعد حقيقيّة.
- **المرساة تُختار بحسب ما يُحرَس لا بقاعدة واحدة** — وهذا أهمّ ما تعلّمتُه في هذه الشريحة:
  - مسارات احتياطيّة ⇒ **كائن الراوتر** (تُستورَد وحدات `api/routers/` ويُقرأ `router.routes`). ينجو من النقلة التالية، وبلا `app.routes`.
  - حذف مقيَّد ببادئة ⇒ **سلوك**: تُقاد `restore_drill` بعميل مسجِّل ويُقاس ما حُذف فعلاً.
  - قاعدة داخل دالّة ⇒ **دالّة بالاسم عبر AST**، مع تتبّع التفويض (الغلاف يستدعي المُنفِّذ، والقاعدة تُقاس عند المُنفِّذ).
  - رموز/أعلام في خدمة ⇒ **وحدات الخدمة مضمومة**. حدّ مُعلَن: يبقى نصّيّاً، تحرّر من الموضع لا من النصّ.
- **`test_qdrant_snapshot_manager` مثال معاكس مفيد:** الاختبار سقط لأنّ المصدر تغيّر من `if target.startswith(...)` إلى `if not target.startswith(...)` — أي أنّ **الحماية صارت أقوى** (حارس fail-closed) والاختبار قرأ ذلك انحداراً. الإنذار الكاذب لا يأتي من ضعف الكود بل من تشبّث التأكيد بصياغته.
- **تكذيب فاشل، سجّلتُه لأنّه غيّر ما أدّعيه:** أوّل محاولة لتكذيب اختبار الراستر استبدلت `raise` واحداً بـ`pass` — وبقي أخضر. السبب: العبارة ترد ثلاث مرّات في الوحدة وواحدة منها داخل الدالّة المقيسة. التكذيب الصحيح كان إسقاط الفرع كاملاً. **الحدّ المعلَن الآن:** التأكيد النصّيّ يمسك **حذف** القاعدة، ولا يمسك إعادة صياغة تُبقي الكلمات.
- **ما لم أفعله:** الثلاثة التي تحتاج حكماً (`test_tool_contracts` · `test_sahool_brain_forensic` · `test_roadmap_phase23`) بقيت في الأساس بأسبابها. إعادة توجيهها بلا حكم تُنتِج اختباراً يمرّ ولا يحرس شيئاً — وهو ما رُفض في `ledger.md` أصلاً.
- **التحقّق:** بوّابة الوحدة **3750 نجح · ٢٦ تخطٍّ · صفر فشل** (من 3723) · التغطية 47.39٪ فوق أرضيّة 43 · أربعة تكذيبات ناجحة · ruff نظيف · bandit High=0 بعد تجريد RLM من ملفّين (نفس درس أمس، مُطبَّق قبل الدفع هذه المرّة).
- **تصحيح على إعلان الأثر في #701 (بعد حجب `capability-registry`):** أعلنتُ `Capability-Impact: ALL` فرُفِض بـ«ALL is allowed only for governance-wide changes». الرفض صحيح، وسببي في الخطأ **قابليّة النقل بلا إعادة قياس**: الشريحة السابقة (#693) كانت تمسّ نواة الحوكمة (`scripts/ci/` وworkflows) فمرّ فيها `ALL`، فنقلتُ الإعلان معي إلى شريحة تمسّ خمسة ملفّات اختبار وأساساً والدماغ. أُعيد القياس على هذا الفرق: البوّابة تسمّي خمس قدرات مباشرة (`IRR-003` · `IRR-009` · `OPS-004` · `SEC-004` · `WX-002`) ⇒ أُعلِنت فمرّت. **الدرس هو الدرس نفسه المتكرّر في هذه الجلسة:** نتيجة مقيسة على مُدخَل آخر ليست معرفةً بل تاريخاً — وقد وقعتُ فيها هنا على إعلانٍ لا على كود.

## 2026-07-30 — ثلاث حزم خارجيّة مُدقَّقة (#702/#703) + إغلاق `APP-ROUTES-INTROSPECTION-COUPLING-01`
- **حزمتان مُدمَجتان بعد تدقيق مستقلّ ضدّ مصدر:** جلستان أخريان (بيئة بلا `git`، ملفّات مضغوطة فقط) أنتجتا `tests_tree_baseline` صفر و`brain_deferral_baseline` ١٤ (#702) و`test_roadmap_phase23.py` (#703). لم يُنقَل أيّ ادّعاء بلا تحقّق: **بُغِتَ عطبان حقيقيّان في تلك الحزم نفسها** — `test_phase17_runtime_bootstrap_doctor.py` كان «١٢/١٢ نجح» فيهم مقيساً ببيئة تسرّبت أسرارها من الصدفة (لم يضبطوا سوى ٢ من ~٢٥ متغيّراً يطلبهما `docker-compose.v9.yml`)، و`env_doctor.py`'s `merged_env()` لم يكن يمرّر `S3_ACCESS_KEY` فيقرأ افتراضي `.env.example` دوماً — كلاهما أُصلِح قبل الدفع لا بعده.
- **درسان دُفِع ثمنهما مرّتين في PR #703 نفسه:** نسيتُ إعادة توليد سلسلة `capability_mapping_engine`/`capability_evidence_maturity_engine`/`pr_capability_impact_gate --generate-index` بعد تعديل نصّ اختبار غيّر ما يلتقطه الفحص القائم على المحتوى (كلمات MFA/TOTP انتقلت بين الأسطر) — CI أمسكها بدقّة (`checksum mismatch`)، أُصلِحت بدفعة تالية. **لا تراجع عن الملفّات الثمانية إلى نسخة z3** كما اقترح ردّ خارجيّ لاحق — الاتّجاه كان صحيحاً من البداية (من z3 إلى المشروع، إذ z3 يحمل الإصلاحات والمشروع كان يحمل النسخ البائتة)؛ التحقّق بمنطق حالات `diff -q` (rc=0/1/غيره) أثبت ذلك.
- **فرعان `workflow_dispatch` يفشلان عند البدء على كلّ فرع بما فيه `main` (صفر jobs) — غير مرئيَين في أيّ PR:** `path3-runtime-verification.yml` (`trusted-signer` يُعلن `permissions.attestations` مرّتين: `read` ثمّ `write`) و`runtime-verification-promotion.yml` (خطوة واحدة تُعلن `env:` مرّتين — `GH_TOKEN` قبل `run:` و`GH_TOKEN`+`TARGET_SHA` بعده، فيضيع `TARGET_SHA` الذي يقرؤه السكربت المُضمَّن). PyYAML لا يرفض المفاتيح المكرَّرة (يأخذ الأخير بصمت) فمرّ `yaml.safe_load` بلا خطأ؛ حارس مفاتيح مكرَّرة مكتوب خصّيصاً (يبني القاموس يدويّاً ويُبلِّغ عند التصادم) هو ما كشفهما، ومُسِح به كلّ ملفّ workflow — لا ثالث. **لم يُتحقَّق منهما بتشغيل حقيقيّ** (self-hosted/بيئة محميّة، لا داعن Docker في هذه الحاوية) — التحقّق بالبناء فقط (PR #704).
- **`MCP-PREAUTH-STATUS-01` — لم يتكرّر:** أعدتُ بناء بيئة الدمج التي يعمل عليها الحاوي (`shared/` الجذر + `services/mcp_servers/shared/` منسوخين في مجلّد واحد، كما يفعل `services/mcp_servers/Dockerfile:26-27` فعليّاً) على نسخة fastapi المثبَّتة نفسها (0.136.3) لكلٍّ من `weather_server` و`sentinel_hub_server`، وطلب غير مُصادَق على `/mcp/v1/tools/call` أعاد **401 «Missing token»** في الحالتين — لا ٤٠٠ كما يصف السجلّ. لم أُغلق الفجوة (قياس واحد لا يكفي لإثبات أنّ سبباً سابقاً زال)، لكن لم أُصلِح شيئاً لا أستطيع إثبات وجوده. مسجَّل للمالك: يحتاج إعادة قياس في البيئة الأصليّة التي وُلِّد منها الادّعاء قبل أيّ إجراء آخر.
- **`AUTH-E2E-UNDER-RESTRICTED-ROLE` — التصميم معروف، التنفيذ يحتاج Postgres حيّاً:** النمط قائم فعلاً في `ci.yml` لخطوتَي IRR-F01/H5.1 (دور `sahool_app_test` مقيَّد `NOSUPERUSER NOBYPASSRLS` يُنشَأ بعد الترحيلات، بمنح صريح). الوظيفة المشتركة (`integration-tests`) تتّصل عمداً بـ`sahool_test` superuser — موثَّق بأنّ اختبارات أخرى **تعمل شرعاً** بصلاحيّات superuser، فتحويل الاتّصال المشترك بالكامل يكسرها. الإصلاح الصحيح مطابقةُ النمط: سحب الاختبارين الفاشلين (`test_mfa_hardening_integration_v29_5::test_mfa_end_to_end_via_app` و`test_services_functional::test_services_functional`) إلى خطوة مخصَّصة بدور `sahool_app_test`، وإسقاط `xfail`. لم أنفّذه — لا داعن Docker هنا لتشغيل Postgres+PostGIS والتحقّق من RLS حيّاً، وتغيير حارس أمنيّ بلا تحقّق حيّ هو بالضبط ما تحظره القاعدة الدائمة.
- **`APP-ROUTES-INTROSPECTION-COUPLING-01` — أُغلقت:** مساعِدان في `tests_v9/conftest.py` (`registered_paths`/`registered_methods` يقرآن `app.openapi()["paths"]`) حلّا محلّ `app.routes` في **١٢** ملفّاً (لا ١١ — العدّ الأصليّ تقريبيّ). ٨٤ اختباراً عبر الملفّات الأربعة عشر المتأثّرة تمرّ، بوّابة الوحدة 3750 بلا انحدار.

## 2026-07-30 — CHECK-STEPS-MUTATE-THE-TREE-01 أُغلقت (تدقيق حزمة خارجيّة على أساس مطابق تماماً)
- **أوّل مرّة يصل فيها تدقيق حزمة خارجيّة بلا ضجيج نسخة:** الحزمة بُنيت فوق نفس `main` (`dfc0be4e`) الذي أرسلته أنا (`git archive`)، فقارنتُ الملفّات مباشرة بلا حاجة لإعادة بناء مرجعيّة. الستّة حرّاس (`generate_service_inventory` · `execution_dependency_audit` · `capability_runtime_evidence` · `service_dependency_conflict_guard` · `route_mount_contract_guard` · `raw_data_processing_contract_guard`) كذّبتُ كلّاً منها فرديّاً (حقن انحراف حقيقيّ في ملفّه المملوك ⇒ `rc=1` باسمه، إصلاح ⇒ `rc=0`) — صمدت الستّة، وصفر منها كتب أثناء `--check`.
- **الحارس أمسك مؤلّفه سابعاً — على اختبار وُلِد لإغلاق فجوة أخرى:** `tests/architecture/test_check_steps_are_read_only.py` الجديد (البرهان بالتكذيب على الشريحة نفسها) لم يكن مُدرَجاً في أيّ workflow؛ `arch_test_ci_coverage_guard` أعلن `57/58` قبل الإصلاح. نفس نمط `ARCH-TESTS-UNLISTED-IN-CI-01` — اختبار جديد يُنسى تسجيله حتى في الشريحة التي تحرس شيئاً مشابهاً تماماً. أُضيف إلى `capability-governance.yml` ⇒ `58/58`.
- **نقطة ضعف حقيقية وجدتُها بالتكذيب، لكنّها موروثة لا مُدخَلة:** `execution_dependency_audit --check` يقارن بصمة SHA-256 لمخرَجه المُعاد توليده بقيمة مخزَّنة (`.audit.sha256`)، لا محتوى الملفّ الملتزَم بايتاً ببايت كالخمسة الأخرى. تحرير `route_handlers.csv` الملتزَم يدويّاً (بلا تغيير مصدر) لا يُكتشَف، لأنّ `generate()` يُعيد كتابته من الصفر داخل المجلّد المؤقّت قبل حساب البصمة — فمحتواه الملتزَم المُتلاعَب به لا يُقارَن أبداً. تحقّقتُ من النسخة الأصليّة (قبل هذه الشريحة): نفس منطق «قارن بصمة بالبصمة» كان قائماً هناك أيضاً؛ الشريحة نقلت مكان الكتابة (إلى مجلّد مؤقّت) ولم تُغيّر قوّة الاكتشاف. مُسجَّل لا مُصلَح — ليس تراجعاً عن هذه الشريحة.
- **التحقّق:** ٢٣ اختباراً معماريّاً جديداً/معدَّلاً يمرّون · حرّاس الدماغ الثلاثة يمرّون · `arch_test_ci_coverage_guard` `58/58` · السلسلة المولَّدة أُعيد توليدها بالترتيب الصحيح وسويبة `verify_all_generated.py` نظيفة.

## 2026-07-30 — FIELD-STATE-PRODUCERS-MISSING-01 (نصف التربة) مُغلَق (#708) + درس git-tracked-scan
- **الوصل:** `api/canonical_soil_state.py` جديد — عميل HTTP بنمط `soil_hydraulic_client.py` (`X-Agent-Token`/`X-Tenant-Id`، `SOIL_SERVICE_URL`، لا اختلاق عند أيّ فشل) يقرأ `GET /v1/fields/{field_id}/soil/profile` من `soil-service` ويُطبِّع `contract_version` إلى `schema_version` بلا حذف الأصل. `_compose_canonical` في `internal_service.py` يستدعيه بدل `soil=None` الثابتة. الطقس يبقى `None` عمداً — `weather-service/canonical_weather_state.py` كامل وموصول عبر `POST /v1/weather/agro/canonical-state`، لكنّ نقطتَي `GET current`/`forecast` الأبسط تُعيدان views جزئيّة بلا غلاف `schema_version`؛ كشف عقد HTTP جديد قرار معماريّ خارج هذه الشريحة.
- **البرهان بالتكذيب:** `test_consumer_sources_soil_from_a_resolver_call_not_a_literal` — حقن `soil={"schema_version":..., "fake": True}` مكان استدعاء `resolve_canonical_soil_state` ⇒ فشل فوريّ يسمّي المخالفة؛ استعادة الاستدعاء الحقيقيّ ⇒ نجاح. + ٩ اختبارات وحدة على العميل نفسه عبر `respx` بلا شبكة.
- **درس مُكلِف — التوليد قبل `git add` يُنتج مصنوعاً بائتاً وقت CI:** `duplicate_definition_guard.py` و`execution_dependency_audit.py` يستخدمان `git ls-files` عمداً (لا `rglob`) ليبقى العدّ قابلاً للتكرار بين جهاز المطوّر وCI نظيفاً — لكن هذا يعني أنّ تجديد مصنوعاتهما **قبل** `git add` على ملفّ جديد يُنتج محتوًى صحيحاً في تلك اللحظة (الملفّ غير مُتتبَّع فيُستبعَد) لكن **بائتاً** عند CI (الملفّ مُتتبَّع من أوّل نسخة). سقطت `capability-registry` و`execution_dependency_audit` على هذا بالضبط بعد الدفعة الأولى؛ أُصلِح بإعادة التوليد الكاملة بعد `git add -A`. **الترتيب الصحيح للمستقبل: `git add` أوّلاً، ثمّ التوليد، ثمّ التحقّق.**
- **درس ثانٍ من نفس الدورة:** حدّث جسم PR (Capability-Impact الصحيح) بعد دفع الالتزام مباشرة — لكنّ وظيفة `capability-registry` كانت قد بدأت بالفعل قارئةً `github.event.pull_request.body` من لحظة التشغيل، فحملت الجسم القديم وحُجِبت زوراً. إعادة تشغيلها لا تُعيد جلب الجسم الحيّ (تُعيد استخدام حمولة الحدث الأصليّة) — الإصلاح دفعة فارغة (`git commit --allow-empty`) لإطلاق حدث جديد يقرأ الجسم الحاليّ.
- **`platform_extraction_map.json` مصدر يدويّ لا مولَّد:** تعديل docstring غيّر رقم سطر `internal_ai_advice_event` بمقدار ٢؛ `platform_route_ownership_guard.py` قارنه ضدّ AST حيّ لا ضدّ مصنوع مولَّد، فلا سكربت `--generate` يُصلحه — تحديث يدويّ لرقم السطر هو العلاج الصحيح هنا تحديداً (خلافاً لقاعدة «أعِد التوليد» المعتادة).
- **راتشِت جديد وُلِد ولوحظ فوراً:** `docs/architecture/platform_python_module_baseline.json` (عدد وحدات `sahool-platform` shrink-only) رفض `api/canonical_soil_state.py` الجديد حتى أضفتُ ملاحظة تبرير بنفس أسلوب `int004_machinery_export_note` ورفعتُ ٦٧٣ ⇒ ٦٧٤ صراحةً — لا حارس صامت.

## 2026-07-30 — تصحيح مصدر جرد API-VERSIONING-GUARD-IS-A-MIRROR-01 (false positive وحيد)
- **كيف ظهر:** تدقيق مستقلّ لتقرير خارجيّ ادّعى أساساً «مصحَّحاً» (٢٥٠/٢٣٠/٥٥) — وُجد صحيحاً في إجماليّاته لكن جدول الخدمات فيه تبادل موضعاً واحداً بين `sahool-platform`/`soil-service`. عند إعادة اشتقاق الرقمين مباشرة من `api_versioning_inventory.generated.json` ظهر أنّ ٢٥٠ يفترض استبعاد `GET /probe` (داخل `services/sahool-platform/tests/test_correlation_middleware.py:18`) من الناتج — لكنّ لا سكربت في الشجرة يفعل ذلك؛ الاستبعاد كان افتراضاً في التقرير الخارجيّ لا واقعاً في الأداة.
- **الفحص الشامل:** بحث عبر التصنيفات الأربعة كلّها (لا `legacy_unversioned_business` وحدها) أثبت أنّ `/probe` **هو الحالة الوحيدة** في الجرد كلّه التي يقع ملفّها تحت `tests/` أو اسمه يبدأ بـ`test_` — لا false positive آخر مختبئ.
- **الإصلاح البنيويّ لا اليدويّ:** `collect()` في `scripts/ci/api_versioning_policy_guard.py` كانت تمسح `services/**/*.py` بلا استبعاد. أُضيف `_is_test_file()` (نفس معيار `duplicate_definition_guard.py`/حارس نموّ الوحدات: `/tests/` في المسار، أو البادئة `tests/`، أو اسم الملفّ يبدأ بـ`test_`) — استبعاد وقت الجمع، لا حذف سطر يدويّاً من الناتج المولَّد بعد التوليد.
- **الأثر المقيس:** `legacy_unversioned_business` ٢٥١ ⇒ **٢٥٠** موضعاً، ٢٣١ ⇒ **٢٣٠** عقداً فريداً، ٥٦ ⇒ **٥٥** ملفاً — مطابق تماماً لِما ادّعاه التقرير الخارجيّ **بعد إصلاح جدول خدماته**. `ceiling` في `api_versioning_legacy_baseline.json`: ٢٣١ ⇒ ٢٣٠.
- **البرهان بالتكذيب:** `test_collect_excludes_test_file_routes_structurally` — إزالة استدعاء `_is_test_file` من `collect()` تُسقِط الاختبار فوراً باسم `/probe` صراحةً في رسالة الفشل؛ استعادته يُعيده أخضر. اختبار ثانٍ (`test_is_test_file_excludes_by_directory_and_filename`) يثبّت الدالّة المساعدة على خمس حالات (دليل/بادئة/اسم ملفّ، إيجاباً وسلباً).
- **حدّ صدق:** إصلاح مصدر بيانات ساكن فقط — لا يمسّ الـ٢٣٠ مساراً غير المُصدَّر حقيقةً المتبقّية؛ تلك تبقى شرائح منتَج مستقلّة (انظر التصنيف بالأولويّة P0=16/45 · P1=31/178 · P2=8/27 المُتصالِح تماماً مع ٥٥/٢٥٠ بلا باقٍ).

## 2026-07-30 — Option B: عمى `APIRouter(prefix=...)` في `generate_service_inventory.py` أُصلِح (فجوة كشف رابعة من عائلة API-VERSIONING-GUARD-IS-A-MIRROR-01)
- **الأمر الصريح من المستخدم:** بعد اكتمال شرائح الوكيل D الخمس (السقف ٢٢٩⇒٨٩)، طلب المستخدم إصلاح هذا العطل المُكتشَف (وغير المُصلَح) في شريحة الوكيل D الخامسة **قبل** أيّ هجرة راستر-سيرفس، بحجّة أنّ الهجرة على جرد بائت قد تُنتج تكرارات وهميّة. نطاق صارم: مُصنِّف فقط، لا هجرة مسارات في نفس الالتزام.
- **العطل ومصدره:** `routes_for_file()` في `scripts/ci/generate_service_inventory.py` تقرأ نصّ المُزخرِف الحرفيّ فقط دون تركيبه مع `APIRouter(prefix=...)` المُعلَن في نفس الملفّ — نفس عطل `api_versioning_policy_guard.py` (PR #717) في مولِّد شقيق مختلف (يُستخدَم لكشف التكرار النصّيّ عبر الخدمات في `build_platform_catalog.py`، لا لراتشِت الإصدار). تحقّق مزدوج (بحث حرفيّ + مسح AST) أكّد صفر استخدام لـ`include_router(prefix=...)`/مراجع عبر-ملفّات في المستودع كلّه، فاقتصر الإصلاح على تركيب نفس-الملفّ فقط، مطابقاً نطاق PR #717 حرفاً بحرف.
- **الإصلاح:** `router_prefixes(tree)` + `_decorator_object_name(dec)` + `_compose(prefixes, object_name, path)` جديدة في `generate_service_inventory.py`، مربوطة بحلقتَي المُزخرِف والتسجيل الصريح في `routes_for_file()`. اختبار جديد `tests/architecture/test_generate_service_inventory.py` (خمسة اختبارات، منها حارس نطاق يُسقِط الافتراض فوراً لو ظهر `include_router(prefix=...)` مستقبلاً).
- **الأثر المقيس:** ٩٩ صفّاً في ستّة ملفّات سحول-بلاتفورم (مطابقة تماماً لِـPR #717) صارت تُقرَأ بمسارها المركَّب. فرادة Method/Path ٩٩٢⇒**٩٩٨** (+٦: ٢ من انفصال stac عن raster-service الحقيقيّة، ٤ من نصوص كانت مشترَكة *داخل* الملفّات الستّة نفسها — `POST /cycle` أربعة صفوف انهارت لنصّ واحد قبل الإصلاح فصارت أربعة، و`POST /models/register` صفّان انهارا لنصّ واحد فصارا اثنين).
- **أثر جانبيّ حوكميّ ثانٍ من نفس العطل، غير متوقَّع عند البدء:** `build_platform_catalog.py --generate` فشل بـ`GET /stac`/`GET /stac/collections: stale decision` بعد الإصلاح — نفس نمط `/plan` الوهميّ (شريحة الوكيل D الخامسة): سحول-بلاتفورم كانت تُقرَأ زوراً بنصّ خام يطابق نصّ raster-service الحقيقيّ، فصنعت تكراراً وهميّاً معه. بعد الإصلاح، raster-service الوحيد يبقى — حُذِف القراران من `config/platform_catalog_overrides.yml` نهائيّاً (لا إعادة ربط، خلافاً لِـ`/products`). مجموعات التكرار ١٤⇒**١٢**.
- **الثمانية المسارات المتبقّية في سحول-بلاتفورم `legacy_unversioned_business` (مطلوبة صراحةً بالتوثيق):** `POST /api/chat` (`chat_proxy_reference.py:110`) + سبعة في `routers/compat_gateway.py` (`readyz`×3، `agent/health`، `vegetation/v1/all_fields`، `vegetation/v1/analyze`، `raster/{path:path}`) — ديون توافقيّة مؤجَّلة عمداً (بوّابة توافق قديمة صريحة)، خارج مدى `APIRouter(prefix=...)` (مسارات `@app.` مباشرة). لم يتغيّر شيء منها بهذا الإصلاح.
- **ثابتان مُثبَّتان (pinned) في `tests_v9/test_platform_catalog_gate.py` احتاجا تحديثاً واعياً:** `PINNED_UNIQUE_METHOD_PATH` (٩٩٢⇒٩٩٨) و`duplicate_groups_classified` (١٤⇒١٢) + تعليق `facades[("GET","/stac")]` الذي بات KeyError (لم يعد `/stac` مجموعة تكرار إطلاقاً) — استُبدل بتأكيد سلبيّ صريح.
- **معزول تماماً عن راتشِت الإصدار:** `git status --short` على الأربعة ملفّات `api_versioning_*`/`docs/architecture/api_versioning_legacy_baseline.json` عاد فارغاً — السقف يبقى ٨٩ بلا أثر.
- **التحقّق:** `verify_all_generated.py` ⇒ rc=0 (٥٦ خطوة) بعد ثلاث دورات تصحيح متتالية (`capability_linker.py --apply` تغيّر أوّلاً — ملفّ الاختبار الجديد أُضيف تلقائيّاً كدليل FM-007 — ثمّ `static_governance_closure.py --generate` تبع) · `pytest -m unit`: **3761 نجح**، 26 تخطّي، صفر فشل · `ruff check .`/`ruff format --check .` نظيفان.
- **القرار التالي (مُقرَّر مسبقاً):** raster-service على هذا الأساس المُعاد قياسه — لا افتراض بقاء عدده ٣٠. يُقسَّم خمس PRs بالملكيّة لا الحجم؛ بعده soil-service ⇐ mcp_servers ⇐ erp-bridge (odoo-bridge اسم مستعار تاريخيّ لـerp-bridge). Auth يبقى مؤجَّلاً حتى إثبات مستهلكين حيّين.
- **PR #722** (فرع `fix-route-inventory-classifier-prefix`، اندمج `aeb9b1a7`): التزام أوّل `e87bba05` (الإصلاح + التوثيق)، التزام تابع `a42dc1d7` أصلح فشل CI (اختبار جديد غير مُسجَّل في `capability-governance.yml` — نفس نمط `ARCH-TESTS-UNLISTED-IN-CI-01` المتكرّر)، والتزام ثالث `e94d538a` (فارغ) أطلق حدثاً جديداً بعد تحديث جسم الـPR إلى `Capability-Impact: ALL` — لأنّ تعديل `capability-governance.yml` نفسه مسار حوكميّ شامل (`WORKFLOW_PATH` في `pr_capability_impact_gate.py`) يرفض إعلاناً جزئيّاً؛ إعادة تشغيل الوظيفة الفاشلة وحدها لا تُعيد قراءة جسم الـPR (نفس درس #708 المُسجَّل أعلاه — يلزم حدث جديد).
- **PR #723** (فرع `brain-docs-pr722-followup`، اندمج `59f8721b`): توثيق فقط — ملء رقم PR #722 وSHA الدمج في `gaps/registry.md`/`log.md` (كانا يُشيران للأمام قبل فتح الـPR) + قيد `decisions/ledger.md` لقرار «Option B قبل raster-service». **فشل CI أوّل مُصحَّح:** `Capability-Impact: NONE` كان خطأً — `gaps/registry.md`/`log.md` مُطابَقان كأدلّة لِـIRR-003/IRR-009 (إشارات هندسة ريّ في مواضع أخرى من الملفّين)، فأعلن الحارس `direct impact` رغم أنّ التغيير توثيقيّ بحت غير متعلّق بالريّ إطلاقاً — صُحِّح إلى `Capability-Impact: IRR-003, IRR-009` + التزام فارغ لإطلاق حدث جديد (نفس درس #722 أعلاه).

## 2026-07-30 — raster-service PR-R1: جرد/تصنيف الملكيّة على أساس مُعاد قياسه
- **الأمر:** «ابدأ بـ raster-service» — التنفيذ المباشر للقرار المُسجَّل مسبقاً (Option B أوّلاً، ثمّ raster-service على أساس مُعاد قياسه، بتقسيم خمس PRs بالملكيّة لا الحجم).
- **إعادة القياس أوّلاً، لا الافتراض:** `api_versioning_inventory.generated.json` بعد Option B: raster-service ٤٨ `versioned` + ٣ `infra` + **٣٠ `legacy_unversioned_business`** (٨١ إجمالاً) — الرقم نفسه المُقدَّر قبل الإصلاح، لأنّ عطل Option B كان محصوراً بستّة ملفّات سحول-بلاتفورم ولم يمسّ raster-service إطلاقاً. قياس مستقلّ حقيقيّ من الجرد الحيّ، لا نقل رقم سابق.
- **التصنيف بالملكيّة (لا الحجم فقط)، بقراءة الكود الفعليّ لا التخمين من الأسماء:**
  - PR-R2 داخليّة/تشغيليّة (٨): `jobs.py` + `storage.py` — كلّها `require_service_token`.
  - PR-R3 صور/كتالوج/معالجة (٢٠): `analysis.py`(٨) + `processing.py`(٣) + `stac.py`(٣) + `observability.py`(٢) + `timeseries_routes.py`(٣) + `fields.py`(١).
  - PR-R4 بلاط/عرض (٢): `tiles.py` فقط.
  - المجموع ٨+٢٠+٢=٣٠، مُثبَّت آليّاً بأربعة اختبارات في `tests/architecture/test_raster_service_route_migration_plan.py` (مُثبَتة بالتكذيب: حذف `/gis/admin-boundaries` أسقط اختبارين باسم المسار حرفيّاً، الاستعادة أعادتهما أخضر).
- **بحث مستدعين حقيقيّين عبر المستودع كلّه، لا الاكتفاء بقراءة راستر-سيرفس نفسها — كشف ثلاثة مستهلكين حقيقيّين لم يكونا واضحين من الأسماء:** `services/sahool-platform/api/raster_service_client.py` يستدعي مباشرةً `GET /jobs/{job_id}/result` (`:466`)، `GET /indices` (`:182`)، `POST /process/batch` (`:448`، يقود `imagery_automation.py`). هذه تختلف جوهريّاً عن حالة Option B (سحول-بلاتفورم بلا مستدعٍ خارجيّ حقيقيّ إطلاقاً) — نقل أيٍّ منها يتطلّب تحديث `raster_service_client.py` في **نفس** PR-R2/R3، لا تأجيلاً لـR5.
- **تحقّق حذر مسارات البلاط (المطلوب صراحةً) — أُجري لا افتُرِض:** nginx (`nginx.v9.conf:252` وغيره) بروكسي شفّاف يُجرِّد `/api/raster/` بلا مسارات مُشفَّرة، فلا يحتاج تعديلاً. صفر مرجع حرفيّ في `frontend/src/`/`mobile/sahool_app/lib/` لمساري `tiles.py` — الواجهة تستدعي عائلة `/v1/fields/{field_id}/tiles/...` المنفصلة (`routers/fields.py`) فعليّاً. **اكتشاف غير متوقَّع:** `raster_job_orchestration.py:230` يُضمِّن `tile_url_template: /tiles/{layer_id}/...` في نتيجة `/process` — تُتُبِّع كلّ قارئ لـ`get_job_result()` (`imagery_automation.py`'s `_fetch_index_mean`) فتبيّن أنّه يقرأ `stats.mean` فقط؛ **لا قارئ لحقل `tile_url_template` في كامل الشجرة** — بيانات ميّتة. الخلاصة: R4 يهاجر مباشرة بلا اسم مستعار توافقيّ رغم الحذر العامّ المُبرَّر لمسارات البلاط.
- **الوثيقة:** `docs/architecture/raster_service_route_migration_plan.md` — تفصيل كامل بمصادر path:line لكلّ الثلاثين مساراً، مرجع تصالحيّ لتصنيف `tests_v9/test_raster_endpoint_auth_coverage.py` الأمنيّ الموجود مسبقاً (`service_only`/`layer_scoped`/`public_catalog`، متعامد مع تقسيم PR).
- **معزول تماماً عن Option B:** لا لمس لأيّ ملفّ سحول-بلاتفورم أو مصنوع مولَّد؛ إضافة وثيقة تخطيط + اختبار جديد فقط.
- **التالي:** PR-R2 (الأبسط، بلا مستهلك واجهة/موبايل حيّ رغم وجود مستهلك خدمة-لخدمة واحد يحتاج تحديثاً متزامناً).

## 2026-07-31 — raster-service PR-R2: ثمانية مسارات داخليّة/تشغيليّة هاجرت فعليّاً
- **الأمر:** «ابدأ بـ PR-R2» — تنفيذ الشريحة الثانية المخطَّطة في PR-R1 (jobs.py + storage.py، ثمانية مسارات `service_only`).
- **الهجرة:** تعديل حرفيّ للديكوريتر (نفس نمط كلّ الشرائح السابقة — `register_routers()` يضمّ راوترات raster-service **بلا** prefix، فنصّ الديكوريتر هو المسار الفعليّ حرفيّاً): `GET /jobs/{job_id}`⇐`/v1/jobs/{job_id}` وGET `/jobs/{job_id}/result`⇐`/v1/jobs/{job_id}/result` (`routers/jobs.py:18,36`)، وستّة مسارات `storage.py` (`upload/raster`·`upload/drone`·`storage/stats`·`storage/cleanup`·`offline/packs`·`offline/packs/{pack_name}`) ⇐ `/v1/*`.
- **المستهلك الحقيقيّ الوحيد حُدِّث في نفس الالتزام، لا تأجيلاً:** `raster_service_client.py:466`'s `get_job_result` — `f"/jobs/{job_id}/result"` ⇐ `f"/v1/jobs/{job_id}/result"`. اختبار الواجهة `services/sahool-platform/tests/test_p2_1_imagery_automation_raster_facade_guard.py`'s `test_raster_facade_exposes_automation_primitives` (يفرض حرفيّاً وجود مسار العميل داخل الملفّ) حُدِّث معه.
- **بحث شامل عبر المستودع كلّه للمراجع الحرفيّة الثمانية، لا الاكتفاء بالملفّين المصدَرين — كشف مواقع لم تكن واضحة من البحث الأوّليّ:**
  - `tests_v9/test_raster_endpoint_auth_coverage.py`'s `SERVICE_ONLY` — الحارس يطابق نصّاً حرفيّاً، فتحديث ثمانية إدخالات إلزاميّ (لا بادئة عامّة تُغني عنه).
  - `tests_v9/test_fields_put_and_mfa_api_contract_20260626.py` — أربعة تأكيدات substring حرفيّة على قسم SERVICE_ONLY أعلاه.
  - `services/raster-service/test_stac_vrt.py:68` — النداء الوظيفيّ الحقيقيّ الوحيد داخل الخدمة نفسها (`TestClient.get(f"/jobs/{job}")`) — سكربت مستقلّ (`if __name__ == "__main__"`) لا يُكتشَف بـpytest تلقائيّاً، حُدِّث احتياطاً.
  - **درسٌ مُكرَّر من بحث PR-R1 (`tile_url_template`) — حقلان ديناميكيّان أُخريان مُضمَّنان في استجابات حيّة:** `download_url` في استجابة `GET /offline/packs` (`storage.py:104`، يُبنى per-file عبر f-string) و`note` استشاريّ في استجابتَي `POST /process/batch` (`processing.py:162`) و`process-cdse` (`routers/fields.py:839`) — الثلاثة تُضمِّن المسار الخام حرفيّاً في الحمولة المُعادة للعميل. حُدِّثت كلّها وإن لم يُثبَت مستهلك حيّ يقرأها اليوم — انضباط «لا نصّ شبح» ذاته من الجلسة كلّها، لا مجرّد وسواس.
  - `docs/openapi/API_MAP.md` (ثمانية أسطر، «مُولَّد آليّاً» بلا مولِّد حيّ — يُصحَّح يدويّاً كما في شرائح سابقة) و`docs/api/BACKEND_FRONTEND_COVERAGE.md` (أُعيد توليده فعليّاً بمولِّده الحقيقيّ `endpoint_ui_coverage_gate.py --report`، ٨ أسطر تغيّرت بالضبط).
- **تحقّق مباشر بدل افتراض قبل ترك ملفّين بلا لمس:** `tests_v9/test_mobile_backend_contract.py` فُحِص فوُجِد **غير مُؤكِّد أصلاً** — `test_every_mobile_call_has_backend` تُعيد قائمة نتائج بـ`return` لا `assert`، فلا تفشل مهما كان المحتوى؛ ويمسح `sahool-raster-service/app/api/endpoints.py` — مسار **غير موجود** في هذا المستودع (`ls` أكّد الغياب) — بنية مشروع سابقة، بائت كليّاً سابق الوجود. `tests_v9/test_roadmap_phase23.py`'s فحصان ناعمان يمسحان `services/raster-service/main.py` بحثاً عن المسارات — لكن `grep` مباشر أكّد أنّها لم تكن هناك أصلاً حتى قبل هذه الشريحة (انتقلت إلى `routers/storage.py` منذ تفكيك Phase 13 أقدم) — بائتان سابقا الوجود، لا انحداراً من هذه الشريحة.
- **اختباران جديدان، كلاهما مُثبَت بالتكذيب (كسر فعليّ ثمّ استعادة):** `tests_v9/test_api_versioning_policy_guard.py::test_raster_service_pr_r2_routes_are_versioned` (يفرض غياب الخام + وجود الجديد + تحديث العميل معاً) و`tests/architecture/test_raster_service_route_migration_plan.py::test_pr_r2_routes_stay_versioned_not_legacy` (حارس انحدار مستقلّ يُسمِّي أيّ مسار يعود خاماً). ملفّ الخطّة نفسه أُعيد تنظيمه: PR-R2 وُسِمت "MIGRATED"، `ALL_BUCKETED` صار ٢٢ (PR-R3+PR-R4 فقط) بدل ٣٠.
- **راتشِت الإصدار:** `docs/architecture/api_versioning_legacy_baseline.json`'s `ceiling` ٨٩ ⇒ **٨١** يدويّاً (نفس نمط كلّ شريحة سابقة — الحارس كاشف انحراف لا بوّابة سياسة، العلاج المُوثَّق «حدّث الأساس»).
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف (rc=0، ٥٦ خطوة) · `pytest -m unit`: **3762 نجح** (+١) · `ruff check .`/`ruff format --check .` نظيفان.
- **التالي:** PR-R3 (٢٠ مساراً — صور/كتالوج/معالجة؛ مستهلكان حقيقيّان: `GET /indices`، `POST /process/batch`، كلاهما في `raster_service_client.py` أيضاً).

## 2026-07-31 — raster-service PR-R3: صور/كتالوج/معالجة (20 مساراً)
- **الأمر:** «ابدأ بـ PR-R3» — التنفيذ المباشر للقرار المُسجَّل مسبقاً (PR-R1 تصنيف ⇐ PR-R2 داخليّة/تشغيليّة ⇐ **PR-R3 صور/كتالوج/معالجة** ⇐ PR-R4 بلاط).
- **إعادة القياس أوّلاً:** بعد PR-R2، raster-service ٧٦ `versioned` + ٣ `infra` + ٢٢ `legacy_unversioned_business` — مطابق تماماً لخطّة PR-R1 (٢٠ PR-R3 + ٢ PR-R4، بلا انحراف).
- **العشرون مساراً هاجرت ⇐ `/v1/*`:** تحاليل (`analysis.py`: zones/classify·change/detect·fvc/compute·sar/rvi·terrain/slope·cog/validate·salinity/classify·salinity/calibrate) + معالجة غير متزامنة (`processing.py`: process·raw/process·process/batch) + واجهة STAC داخليّة (`stac.py`: stac·stac/collections·stac/mosaicjson) + مراقبة (`observability.py`: info/{layer_id}·indices) + سلسلة زمنيّة (`timeseries_routes.py`: الثلاثة) + حدود GIS (`fields.py`: gis/admin-boundaries). السقف ٨١ ⇒ **٦١**.
- **الفارق الجوهريّ عن PR-R2 — مستهلكان حقيقيّان لا صفر:** `raster_service_client.py:191`'s `get_indices_sync` و`:448`'s `process_indicator_batch` (يقود تجميع مؤشّرات `imagery_automation.py` — NDVI/NDRE/NDSI دفعةً لصورة جديدة). حُدِّثا في نفس الالتزام إلى `/v1/indices`/`/v1/process/batch`، ومعهما `test_p2_1_imagery_automation_raster_facade_guard.py` ودocstrings `imagery_automation.py`.
- **بحث حرفيّ شامل لكلّ من الـ٢٠ مساراً عبر المستودع كلّه (لا الاكتفاء بالراوترات نفسها) كشف مواقع تحديث لم تكن واضحة سلفاً:**
  - `tests_v9/test_raster_endpoint_auth_coverage.py`'s SERVICE_ONLY/PUBLIC_CATALOG (١٧ إدخالاً) — الحارس يصنِّف بمطابقة نصّيّة حرفيّة، فتحديثه إلزاميّ.
  - ثلاثة اختبارات وظيفيّة حقيقيّة (لا صوريّة) تؤكّد وجود الديكوريتر الفعليّ عبر `raster_route_source` (main.py+routers/ مجتمعَين): `test_change_detection.py`، `test_fvc.py`، `test_sar_rvi.py`، وكذلك `test_field_intelligence_cloud.py`'s `test_indices_endpoint_wired_and_guarded`.
  - **حارس CI حقيقيّ اكتُشف أثناء البحث، لا اختبار:** `scripts/ci/raw_data_processing_contract_guard.py` يفحص وجود `@router.post("/raw/process")` حرفيّاً + غياب المسار من `main.py` — لولا هذا البحث الشامل لبقي يفحص المسار الخام البائت بصمت (اختبار محليّ أكّد `raw_data_processing_contract_ok` بعد التحديث).
  - **درس `tile_url_template` من بحث PR-R1 تكرّر هنا مرّتين:** روابط STAC ذاتيّة الإشارة (self/search/data) في `cloud_native_catalog.py:37-45` تُضمَّن في استجابة `/v1/stac` الحيّة — حُدِّثت رغم عدم استهلاكها خارجيّاً اليوم؛ وحقلا `note` استشاريّان في `timeseries_routes.py` يُرشِدان العميل لمسارَي `/process`/`/imagery/timeseries/analyze` القديمَين.
  - وصف نصّيّ في مهارة LLM (`services/supervisor-agent/skills/remote_sensing_skill.py` ونسختها المكرَّرة — تكرار ملفّ سابق الوجود غير مرتبط بهذه الشريحة) يصف `/change/detect`/`/fvc/compute` بلا نداء HTTP فعليّ — حُدِّث للدقّة رغم عدم كونه مستهلكاً حيّاً.
- **تحقّق حوكميّ إضافيّ لم يكن مؤكَّداً سلفاً — نفس فحص `/stac` الوهميّ من Option B مُعاد تطبيقه استباقيّاً:** بعد الهجرة، `build_platform_catalog.py --generate` نظيف بلا `stale decision` — لا تكرار نصّيّ وهميّ جديد نتج عن نقل `/stac`/`/stac/collections`/`/stac/mosaicjson` إلى `/v1/`، إذ raster-service كانت أصلاً المالك الحقيقيّ الوحيد المتبقّي لهذه النصوص بعد إغلاق شبح سحول-بلاتفورم في Option B (لا عضو آخر يتشارك النصّ الخام الآن).
- **تُركت عمداً بعد تحقّق مباشر لا افتراض:** فحوص `test_roadmap_phase23.py`'s الناعمة (تمسح `main.py` الذي لم يحمل هذه المسارات قطّ بعد تفكيك الراوترات — `grep` أكّد الغياب المسبق) وفحصا مطابقة `"/process" in url` على عنوان طلب مُموَّه في اختبارَي mock (يبقيان صحيحَين حتماً كسلسلة جزئيّة، بصرف النظر عن بادئة `/v1/`). `docs/openapi/API_MAP.md` حُدِّث للأحد عشر سطراً الموجودة فعلاً منه فقط — تسعة مسارات كانت غائبة عن الوثيقة أصلاً قبل هذه الشريحة، نقص سابق الوجود لم يُملأ.
- **اختباران جديدان، كلاهما مُثبَتان بالتكذيب (كسر فعليّ ثمّ استعادة):** `tests_v9/test_api_versioning_policy_guard.py::test_raster_service_pr_r3_routes_are_versioned` و`tests/architecture/test_raster_service_route_migration_plan.py::test_pr_r2_and_pr_r3_routes_stay_versioned_not_legacy`. ملفّ الخطّة أُعيد تنظيمه: PR-R3 وُسِمت "MIGRATED"، `ALL_BUCKETED` صار ٢ فقط (PR-R4).
- **راتشِت الإصدار:** `ceiling` ٨١ ⇒ **٦١** يدويّاً (نفس نمط كلّ شريحة سابقة).
- **دورتا تصحيح إضافيّتان في `verify_all_generated.py` قبل الاستقرار:** `capability_evidence_maturity_engine.py --check` اكتشف انحرافاً (`capability_evidence_matrix.json`/`evidence_manifest.json`) لم تلتقطه الدورة الأولى من سلسلة التوليد المعتادة — أُصلِح بـ`--generate` ثمّ إعادة بناء حزمة الإصدار مرّتين متتاليتين حتى استقرّت بصمات checksum.
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف (rc=0، ٥٦ خطوة) · `pytest -m unit`: **3763 نجح** (+١) · `ruff check .`/`ruff format --check .` نظيفان.
- **التالي:** PR-R4 (٢ مساراً فقط — `tiles.py`، بلاط/عرض؛ صفر مستهلك خارجيّ حيّ مؤكَّد بالبحث الشامل في بحث PR-R1 — nginx شفّاف، صفر مرجع واجهة/موبايل، `tile_url_template` بيانات ميّتة — يهاجر مباشرة بلا اسم مستعار توافقيّ). بعده: soil-service ⇐ mcp_servers ⇐ erp-bridge.

## 2026-07-31 — raster-service PR-R4: بلاط/عرض (2 مساراً) — إغلاق raster-service بالكامل
- **الأمر:** «استكمل» — التنفيذ المباشر للقرار المُسجَّل مسبقاً (PR-R1 تصنيف ⇐ PR-R2 داخليّة/تشغيليّة ⇐ PR-R3 صور/كتالوج/معالجة ⇐ **PR-R4 بلاط/عرض**، الشريحة الخامسة والأخيرة).
- **إعادة القياس أوّلاً:** بعد PR-R3، raster-service ٩٦ `versioned` + ٣ `infra` + ٢ `legacy_unversioned_business` — مطابق تماماً لخطّة PR-R1 (المتبقّي الوحيد: `tiles.py`).
- **المساران هاجرا ⇐ `/v1/*`:** `GET /tiles/{layer_id}/{z}/{x}/{y}.png`⇐`/v1/tiles/{layer_id}/{z}/{x}/{y}.png` وGET `/layers/{layer_id}/tilejson`⇐`/v1/layers/{layer_id}/tilejson` (`services/raster-service/routers/tiles.py:22,36`). السقف ٦١ ⇒ **٥٩**.
- **الفارق عن PR-R2/PR-R3 — صفر مستهلك خارجيّ حيّ، مؤكَّد لا مفترَض:** بحث شامل عبر `frontend/src/`، `mobile/sahool_app/lib/`، كلّ `nginx/*.conf`، كلّ `docker-compose*.yml`، وكلّ خدمة أخرى — نفس نتيجة بحث PR-R1 بالضبط، مُعاد تأكيدها لا نقلاً من الذاكرة. الواجهة تستدعي عائلة `/v1/fields/{field_id}/tiles/...` المنفصلة معماريّاً (`routers/fields.py`، مُصدَّرة فعلاً منذ زمن) فلا صلة لها بمساري `tiles.py`.
- **رابط ذاتيّ-الإشارة حُدِّث رغم غياب المستهلك — نفس درس `tile_url_template`/STAC من PR-R1/PR-R3، لا وسواس:** `tiles.py:63`'s حقل `tiles` في استجابة `"static-pregenerated"` (fallback عند غياب TiTiler) و`raster_job_orchestration.py:230`'s `tile_url_template` (بيانات ميّتة مؤكَّدة في بحث PR-R1، حُدِّثت للاتّساق الداخليّ فقط لا لمستهلك حقيقيّ).
- **فجوة تصنيف مصاحبة أُصلِحت استباقيّاً قبل تشغيل السويت، لا بعد فشل CI:** `tests_v9/test_raster_endpoint_auth_coverage.py`'s `_is_layer_scoped(path)` كانت تطابق البادئة الخام `/tiles/{layer_id}`/`/layers/{layer_id}/` — لو تُركت كما هي، كان المساران سيصيران فور الهجرة «غير مُصنَّفين» (لا حارس ملكيّة/خدمة، وليسا في `PUBLIC_CATALOG`/`SERVICE_ONLY`). حُدِّثت **قبل** تشغيل الاختبارات، وبُرهِنت بالتكذيب: عكس ديكوريتر واحد مؤقّتاً عبر `sed` أنتج رسالة الفشل المتوقَّعة حرفيّاً، ثمّ استُعيد الإصلاح وتأكَّد الأخضر.
- **مسح شامل لهذه الشريحة كشف بقايا تراكميّة من PR-R2/PR-R3 لم تُصلَح وقتها — تنظيف لا خارج نطاق:**
  - `docs/openapi/API_MAP.md`/`ROUTE_INVENTORY.json` — إدخالا `tiles.py` الأخيران.
  - `skills/sahool-gis/RASTER_LAYER.md` (`/info/{layer_id}` أيضاً، فاتت PR-R3) و`skills/sahool-gis/TERRAIN_DEM.md` (`/terrain/slope`، فاتت PR-R3).
  - `docs/architecture/GIS_CLOUD_NATIVE_BEST_PRACTICES_PHASE4.md` (ثلاثة مسارات STAC، فاتت PR-R3) و`docs/architecture/db_ownership.yml` (تعليق `/gis/admin-boundaries`، فاتت PR-R3 — تعليق فقط، لا يمسّ تحليل YAML).
  - أربعة ملفّات `docs/capability-registry/domains/*.yaml` (gis.yaml ٤ أسطر، satellite.yaml ١٨ سطراً، irrigation.yaml ٣ أسطر، operations.yaml سطران — `/offline/packs`+`/offline/packs/{pack_name}`، فاتا PR-R2). سكربت مسح نهائيّ أكّد صفر مرجع raster-service خام متبقٍّ في أيّ ملفّ `capability-registry/domains/*.yaml`.
- **تُركت عمداً بعد فحص مباشر لا افتراض:** `docs/specs/A7_admin_boundaries_spec.md` (رأسه يُعلِن صراحةً «⏳ مسودّة للمراجعة» وملكيّة TBD — منفصل عن التنفيذ الحقيقيّ، تعديله يُزيّف مسودّة لم تُعتمَد)، `docs/history/RASTER_SERVICE_REVIEW.md` و`docs/audits/RIV_DURABLE_PRODUCT_IDENTITY_AND_LEASES_REPORT_20260712.md` (سجلّ تاريخيّ مؤرَّخ، تعديله يُزيّف ما وُصِف وقتها)، و`shared/enterprise_gis/phase8_global_scale.py:124` (قالب اختبار حمل عامّ يذكر `/stac` بلا صلة بـraster-service — false positive نصّيّ).
- **اختباران جديدان، كلاهما مُثبَتان بالتكذيب (كسر فعليّ ثمّ استعادة):** `tests_v9/test_api_versioning_policy_guard.py::test_raster_service_pr_r4_routes_are_versioned` و`tests/architecture/test_raster_service_route_migration_plan.py::test_pr_r2_pr_r3_pr_r4_routes_stay_versioned_not_legacy`. ملفّ الخطّة أُعيد هيكلته بالكامل: `ALL_BUCKETED` صار مجموعة فارغة، واختبار جديد يفرض صراحةً أنّ `legacy_unversioned_business` لِـraster-service صفر — لا مجرّد وسم "MIGRATED" نصّيّاً.
- **راتشِت الإصدار:** `ceiling` ٦١ ⇒ **٥٩** يدويّاً في `api_versioning_legacy_baseline.json` (نفس نمط كلّ شريحة سابقة — الحارس كاشف انحراف لا بوّابة سياسة).
- **التحقّق — أسرع دورة تحقّق في سلسلة raster-service كلّها:** `verify_all_generated.py` ⇒ صفر انحراف **من أوّل تشغيل** (بلا دورات تصحيح إضافيّة، خلافاً لـPR-R2/PR-R3 اللتين احتاجتا `capability_evidence_maturity_engine.py --generate` وإعادة بناء حزمة الإصدار). `guard.collect()` الحيّ يُظهِر raster-service الآن `Counter({'versioned': 78, 'infra': 3})` — صفر `legacy_unversioned_business`. `pytest -m unit`: **3764 نجح** (+١)، 26 تخطّي، صفر انحدار · `ruff check .`/`ruff format --check .` نظيفان (بعد `ruff format` على ملفّي الاختبار الجديدين).
- **بهذه الشريحة يُغلَق raster-service بالكامل:** ٣٠ مساراً عبر أربع شرائح (PR-R1 تصنيف بلا هجرة، PR-R2 ٨ مسارات، PR-R3 ٢٠ مساراً، PR-R4 ٢ مساراً)، خمس PRs بالملكيّة لا الحجم كما قرَّر المستخدم صراحةً قبل البدء.
- **التالي وفق الترتيب المُعلَن مسبقاً من المستخدم:** soil-service ⇐ mcp_servers ⇐ erp-bridge (وodoo-bridge اسم مستعار تاريخيّ لـerp-bridge لا خدمة مستقلّة). Auth (٢٤ مساراً) يبقى مؤجَّلاً حتى إثبات مستهلكين حيّين — لم يُبدَأ في هذه الجلسة.

## 2026-07-31 — soil-service: خامس خدمة بعد اكتمال raster-service — 5 مسارات مُهاجَرة
- **الأمر:** «استكمل» — التنفيذ المباشر للترتيب المُقرَّر مسبقاً (raster-service اكتمل بأربع شرائح ⇐ **soil-service** ⇐ mcp_servers ⇐ erp-bridge).
- **إعادة القياس أوّلاً:** `guard.collect()` الحيّ لخدمة soil-service: ٥٠ `versioned` + ٥ `infra` + **٥ `legacy_unversioned_business`** — أصغر بكثير من raster-service (٣٠)، فلا حاجة لتقسيمها بالملكيّة كما قرَّر المستخدم صراحةً لـraster-service. قراءة الكود أظهرت الخمسة كلّها `_require_service_token` بلا تمايز — هجرة واحدة، PR واحد، مطابقة لنمط الخدمات الصغيرة السابقة (tts-service، edge-inference، knowledge-graph، rag-retrieval).
- **الخمسة مسارات هاجرت ⇐ `/v1/soil/*`:** `POST /soil/decode/modbus` (`services/soil-service/routers/modbus.py:35`) · `GET /soil/readings/{field_id}`، `POST /soil/ingest` (`routers/readings.py:22,37`) · `POST /soil/suitability`، `GET /soil/soilgrids` (`routers/soil_profile.py:46,57`).
- **بحث مستدعين حقيقيّين عبر المستودع كلّه — كشف مستهلكاً واحداً حقيقيّاً وأربعة بلا مستهلك خارجيّ:** `services/sahool-platform/core/field_intelligence_adapters.py:105`'s `fetch_soil_baseline()` يستدعي `/soil/soilgrids` مباشرةً — حُدِّث في نفس الالتزام إلى `/v1/soil/soilgrids`، ومعه docstring الدالّة وثلاثة تعليقات وصفيّة في ملفّات أخرى (`field_intelligence_card.py`، `soil_climate_sources.py`، `api/routers/field_intelligence.py`). الأربعة الباقية (modbus/readings/ingest/suitability) بلا مستدعٍ خارجيّ حقيقيّ — `frontend/src/services/api.ts`'s `soilApi` يستدعي مسارات مختلفة تماماً (`/soil/wofost_params/{id}`، `/soil/nitrogen/recommendation`، `/soil/{fieldId}`، إلخ — راوترات أخرى غير هذه الخمسة في نفس الخدمة). بوّابة nginx's `/api/soil/` (`nginx.v9.conf:302`) تمرّ عبر `service_proxy.py`'s `proxy_soil` — بروكسي شفّاف بلا rewrite (نمط field-segmentation/edge-inference)، فلا حاجة لتعديل nginx نفسه.
- **اكتشاف غير متوقَّع أثناء البحث — مرجع حيّ غير-اختباريّ لم يكن ليظهر ببحث نصّيّ سطحيّ:** `runtime-verification/functional_probes/soil-service.json` — خطّة probe فعليّة تُشغَّل حيّاً عبر `scripts/ci/functional_probe_runner.py --run` (لا اختبار pytest، جزء من سلسلة إثبات `runtime_verified`) تحمل حقلَي `path: "/soil/suitability"` مرّتين (لعيّنتَي texture مختلفتين) + جملة توضيحيّة في `note` — حُدِّثا كلاهما إلى `/v1/soil/suitability`. `functional_probe_runner.py --check` أُعيد تشغيله للتأكّد من صحّة البنية (`functional_probe_runner_ok plans=3 probes=6`).
- **كلّ مرجع حرفيّ آخر عبر المستودع حُدِّث:**
  - أربعة ملفّات اختبار وظيفيّة حقيقيّة تستدعي المسار الفعليّ عبر `TestClient`: `tests_v9/test_modbus_decoder_20260703.py` (docstring + ثلاثة نداءات)، `tests_v9/test_soil_science_20260703.py` (docstring + أربعة نداءات)، `tests_v9/test_services_functional.py` (معلَّم `pytest.mark.integration` لا يعمل في `-m unit` لكن حُدِّث لنفس انضباط عدم ترك مرجع بائت — ثلاثة نداءات).
  - `services/soil-service/test_soil_router_decomposition_guard.py`'s `_CRITICAL_ROUTES` (readings/ingest — أرضيّة الانحدار الحرجة).
  - `tests_v9/test_soil_field_tenant_authz.py`'s docstring افتتاحيّ (سرد ثغرة IDOR سابقة مُصلَحة مسبقاً — يسمّي المسار الحيّ فحُدِّث للدقّة، لا لتزييف سرد الثغرة نفسها).
  - `docs/openapi/API_MAP.md`+`ROUTE_INVENTORY.json` (سطرا readings/ingest فقط — الثلاثة الأخرى [modbus/suitability/soilgrids] كانت غائبة أصلاً عن هاتين الوثيقتين قبل هذه الشريحة، نقص سابق الوجود خارج النطاق، لم يُملأ).
  - `docs/capability-registry/domains/soil.yaml`'s SOIL-001 apis (suitability/soilgrids).
  - `docs/NGINX_ROUTING.md`'s صفّ `/api/soil/*` اليدويّ — وصف upstream `soil_backend` فيه بائت أصلاً قبل هذه الشريحة (الطبولوجيا الفعليّة تمرّ عبر `service_proxy` لا upstream مباشر، `nginx.v9.conf:90` يوثّق ذلك صراحةً في تعليق)، لكن نصّ المسار المذكور حُدِّث للدقّة رغم أنّ وصف الآليّة حوله بقي بائتاً (خارج نطاق هذه الشريحة إصلاحه).
- **تُركت عمداً بعد تحقّق مباشر:** `sahool-brain/hot.md`/`log.md`'s سرد تاريخيّ لبناء هذه المسارات أصلاً (وصف جلسة ماضية 2026-07-03، تعديله يُزيّف ما وُصِف وقتها)، و`docs/api/BACKEND_FRONTEND_COVERAGE.md` (مُعلَن «مُولَّد آليّاً» بمولِّد حيّ فعليّ — أُعيد توليده عبر `endpoint_ui_coverage_gate.py --report`، لا تحريراً يدويّاً). `docs/capability-registry/domains/gis.yaml`'s مطابقة نصّيّة جزئيّة على `soil/soilgrids-spatial` (مسار مختلف تماماً بالفعل، `p1_products.py`، مُصدَّر مسبقاً) — فُحِصت وتأكَّد أنّها إيجابيّة كاذبة من `grep` الجزئيّ لا مرجعاً حقيقيّاً.
- **عثرة اختبار غير مرتبطة اكتُشفت أثناء الفحص، مُوثَّقة لا مُصلَحة (خارج نطاق هذه الشريحة):** `test_soil_router_decomposition_guard.py::test_no_app_route_decorators_in_main` يفشل على `main` الحاليّ (قبل أيّ لمس لهذه الشريحة) — `services/soil-service/main.py:197`'s `@app.get("/runtime-identity", include_in_schema=True)` مُعلَن مباشرةً بلا راوتر، بخلاف افتراض الحارس أنّ تفكيك الراوترات مكتمل بالكامل بلا استثناء. تحقّق مباشر عبر `git stash` على الشجرة النظيفة (بلا تعديلات soil-service) أكّد أنّ الفشل قائم سلفاً، لا ناتجاً عن هذه الهجرة — لم يُصلَح، مُسجَّل هنا للتوثيق فقط (نفس معاملة raster-service's `test_mobile_backend_contract.py`/`test_roadmap_phase23.py` البائتين).
- **اختبار تكذيب جديد، مُثبَت بكسرين منفصلين ثمّ استعادتهما كلاهما:** `tests_v9/test_api_versioning_policy_guard.py::test_soil_service_routes_are_versioned` — كسر أوّل (إعادة `@router.get("/soil/soilgrids")` الخام في `soil_profile.py`) أسقط الاختبار فوراً باسم المسار المتسرِّب حرفيّاً؛ استُعيد الإصلاح، ثمّ كسر ثانٍ منفصل (إعادة `f"{SOIL_URL}/soil/soilgrids"` الخام في `field_intelligence_adapters.py`) أسقطه مرّة أخرى باسم السطر المتسرِّب حرفيّاً؛ استعادة كليهما أعادته أخضر — برهان مزدوج يغطّي كلّاً من مصدر الحارس ومستهلكه.
- **راتشِت الإصدار:** `ceiling` ٥٩ ⇒ **٥٤** يدويّاً في `api_versioning_legacy_baseline.json` (نفس نمط كلّ شريحة سابقة).
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف من أوّل تشغيل (بلا دورات تصحيح، كما في raster-service PR-R4) · `pytest -m unit`: **3765 نجح** (+١)، 26 تخطّي، صفر فشل جديد (فشل `test_no_app_route_decorators_in_main` قائم سلفاً كما وُثِّق أعلاه) · `ruff check .`/`ruff format --check .` نظيفان.
- **التالي وفق الترتيب المُعلَن مسبقاً:** mcp_servers ⇐ erp-bridge (وodoo-bridge اسمه التاريخيّ لا خدمة مستقلّة). Auth (٢٤ مساراً) يبقى مؤجَّلاً حتى إثبات مستهلكين حيّين.

## 2026-07-31 — mcp_servers: سادس خدمة بعد soil-service — 7 مسارات REST سوق مُهاجَرة
- **الأمر:** «ابدأ بـ mcp_servers» — التنفيذ المباشر للترتيب المُقرَّر مسبقاً (raster-service ⇐ soil-service ⇐ **mcp_servers** ⇐ erp-bridge).
- **إعادة القياس أوّلاً:** `guard.collect()` الحيّ لخدمة mcp_servers: ١١ `versioned` + ١٤ `infra` + **٧ `legacy_unversioned_business`** — كلّها في `services/mcp_servers/market_server.py`، عقد REST سوق منفصل تماماً عن عقد بروتوكول MCP (`/v1/mcp/tools*`) المُهاجَر سابقاً في شريحة الوكيل A الأولى من نفس الملفّ.
- **الفارق الجوهريّ عن soil-service/raster-service — نمط مصادقة مختلف تماماً:** الخمسة/الثلاثون مساراً المُهاجَرة سابقاً في هذه الجلسة كانت جميعاً `require_service_token` (توكن خدمة-لخدمة). السبعة هنا `Depends(_get_current_user)` — Bearer/JWT، مصادقة مستخدم متصفّح فعليّة. الملفّ نفسه يستعمل `@app.<method>` مباشرةً بلا `APIRouter`/`routers/` تفكيك (بخلاف soil-service/raster-service المُفكَّكين) — لكن هذا يطابق تماماً نمط تسجيل `GET /v1/products` المُصدَّر مسبقاً في نفس الملفّ (شريحة الوكيل A الأولى)، فالهجرة تعديل حرفيّ مباشر للديكوريتر بلا اعتبارات تركيب راوتر جديدة.
- **السبعة مسارات هاجرت ⇐ `/v1/*`:** `GET /suppliers/{supplier_id}` (`market_server.py:643`) · `POST /procurement` (`:648`) · `GET /procurement/{order_id}` (`:654`) · `POST /sales` (`:661`) · `GET /sales` (`:666`) · `GET /price-history/{category}` (`:683`) · `GET /analytics/{tenant_id}` (`:690`).
- **بحث مستدعين حقيقيّين عبر المستودع كلّه — صفر نتائج للسبعة كلّها، خلافاً لكلّ شريحة سابقة (raster-service/soil-service كانت تحمل مستهلكاً حقيقيّاً واحداً على الأقلّ):**
  - frontend/mobile: صفر مرجع حرفيّ لأيٍّ من المسارات السبعة.
  - `nginx/nginx.v9.conf` (البوّابة الأساسيّة الحيّة): **لا بوّابة market فيه إطلاقاً** — تعليق صريح في الملفّ يوثّق أنّ edge/soil يمرّان عبر service_proxy، ولا ذكر لـmarket بتاتاً.
  - `nginx/nginx.fixed.conf` و`nginx/nginx.unified.conf` (طبولوجيّتان بديلتان): `location /api/market/` موجودة، بروكسي شفّاف بلا `proxy_pass` URI (`http://market_backend/;`) — العميل يحدّد المسار النهائيّ، فلا حاجة لتعديل نغينكس نفسه (نمط field-segmentation/edge-inference/soil-service).
  - `services/supervisor-agent/main.py`: يستدعي `MCP_MARKET_URL` لكن فقط لعقد بروتوكول MCP (`/v1/mcp/tools*`) المُهاجَر سابقاً — صفر نداء لأيٍّ من نقاط REST السبعة.
  - `services/odoo-bridge/`: صفر نداء HTTP فعليّ لهذه المسارات — مجرّد ذكر عابر في تعليق واحد (`main.py:146`، سياق توثيقيّ لا نداء).
- **مرجع توضيحيّ اكتُشف ولم يُخلَط بمستهلك حقيقيّ — درس تحقّق مباشر:** `docs/MARKET_SYSTEM.md`'s قسم «تكامل Flutter» يعرض مثال Dart كامل (`lib/services/market_service.dart`، صنف `MarketService` باستدعاء `Uri.parse("$baseUrl/procurement")`) — تحقّق مباشر بـ`find`/`ls` أكّد أنّ هذا الملفّ **غير موجود إطلاقاً** في `mobile/sahool_app/lib/services/` الفعليّة — مثال توضيحيّ في الوثيقة فقط (كيف *يمكن* لعميل Flutter التكامل)، لا كود مُشغَّل. حُدِّث للدقّة (`$baseUrl/v1/procurement`) رغم كونه غير وظيفيّ، إذ ترك مثال بائت في وثيقة معلنة "مُنفَّذ بالكامل" يُضلِّل قارئاً مستقبليّاً يبني عليه فعليّاً.
- **اكتشاف جانبيّ في `docs/capability-registry/domains/farm_management.yaml` — درس عن حدود ربط `capability_linker.py`:** قدرتان تحملان أدلّة `apis` مربوطة تحفّظيّاً بـ`GET /analytics/{tenant_id}` وبـ`POST`/`GET /procurement*` على التوالي؛ الثانية تحمل أيضاً `mobile_consumers: [mobile/sahool_app/lib/screens/inventory_screen.dart]`. تحقّق مباشر (`grep procurement\|market` على الملفّ) أكّد أنّ `inventory_screen.dart` **لا يستدعي فعليّاً** أيّاً من مساري `/procurement`/`/procurement/{order_id}` — الربط heuristic تحفّظيّ (نفس الملفّ/الدليل، لا دليل نداء HTTP حقيقيّ)، موثَّق صراحةً في السكربت نفسه («generated conservatively»). حُدِّثت الأسطر الثلاثة (نصّ المسار فقط) دون تعديل `mobile_consumers`/`ui_consumers` (المتروكة كما كانت — لا دليل جديد يستوجب تغييرها، ولا سبب لحذفها إذ الربط التحفّظيّ ذاته لم يتغيّر منطقه).
- **مرجعان حيّان غير-كوديَّين حُدِّثا بالكامل:** `docs/MARKET_SYSTEM.md` (رسم ASCII للبنية سطرا ٢٨-٣٣ + خمسة أمثلة `curl` أسطر ١١٦/١١٩/١٣١/١٤١ + مثال Dart سطر ٢٠٣) و`docs/UNIFIED_SETUP.md`'s مثال `curl` حيّ ضمن سير عمل تشغيل سريع موثَّق (`https://localhost/api/market/procurement` ⇐ `https://localhost/api/market/v1/procurement` — البروكسي الشفّاف يعني أنّ العميل هو من يحدّد المقطع بعد `/api/market/`، فتغيّر النصّ المُوثَّق لا كود الخادم).
- **اختبار تكذيب جديد، مُثبَت بكسره فعليّاً ثمّ استعادته:** `tests_v9/test_api_versioning_policy_guard.py::test_mcp_servers_market_rest_routes_are_versioned` — كسر مقصود (إعادة `@app.post("/sales")` الخام) أسقط الاختبار فوراً باسم المسار المتسرِّب حرفيّاً؛ استعادة الإصلاح أعادته أخضر.
- **راتشِت الإصدار:** `ceiling` ٥٤ ⇒ **٤٧** يدويّاً في `api_versioning_legacy_baseline.json` (نفس نمط كلّ شريحة سابقة).
- **دورة تصحيح واحدة في `verify_all_generated.py` — جديدة لم تظهر في الشرائح السابقة:** `capability_management_engine.py --check` اكتشف انحرافاً في `capability_management_matrix.json` لم تلتقطه الدورة الأولى من سلسلة التوليد المعتادة — أُصلِح بـ`--generate` صريح ثمّ إعادة بناء حزمة الإصدار مرّة واحدة إضافيّة.
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف بعد دورة تصحيح واحدة · `pytest -m unit`: **3766 نجح** (+١)، 26 تخطّي، صفر فشل جديد · `ruff check .`/`ruff format --check .` نظيفان.
- **التالي وفق الترتيب المُعلَن مسبقاً:** erp-bridge (وodoo-bridge اسمه التاريخيّ لا خدمة مستقلّة). Auth (٢٤ مساراً) يبقى مؤجَّلاً حتى إثبات مستهلكين حيّين.

## 2026-07-31 — odoo-bridge/erp-bridge: سابعة وآخر شريحة هجرة نشطة مُعلَنة — 7 مسارات
- **الأمر:** «ابدأ بـ erp-bridge» — التنفيذ المباشر للترتيب الأخير المُقرَّر مسبقاً (raster-service ⇐ soil-service ⇐ mcp_servers ⇐ **erp-bridge**، ثمّ Auth مؤجَّل).
- **إعادة القياس أوّلاً:** `guard.collect()` الحيّ لخدمة odoo-bridge: ١ `versioned` + ٣ `infra` + **٧ `legacy_unversioned_business`** — كلّها عبر أربعة ملفّات راوتر (`catalog.py` أربعة، `health.py` واحد، `sync.py` واحد، `webhooks.py` واحد).
- **السبعة مسارات هاجرت ⇐ `/v1/*`:** `GET /erp/provider` (`catalog.py:17`) · `GET /config` (`:33`) · `GET /logs` (`:50`) · `GET /suppliers` (`:78`) — أربعتها `main.require_auth` (Bearer/JWT). `GET /readyz/capabilities` (`health.py:76`) — بلا مصادقة، بيانات تشغيليّة HTTP 200 دائماً. `POST /sync` (`sync.py:78`) — `require_auth`. `POST /webhook/odoo` (`webhooks.py:21`) — سرّ `X-Webhook-Secret` مشترك، مُصادَقة مختلفة تماماً عن البقيّة (لا JWT).
- **قرار تصنيف مدروس — `/readyz/capabilities` رغم تصميمه كمستوى ثالث مستقلّ بجوار `/healthz`/`/readyz`:** docstring الملفّ نفسه يوثّق «المستويات الثلاثة المستقلّة» صراحةً بوصفها معماريّة واحدة متكاملة، ومع ذلك الحارس صنَّف الثالث فقط `legacy_unversioned_business` (الاثنان الأوّلان في allowlist البنية الصريح، الثالث ليس فيه). فحصتُ المحتوى الفعليّ قبل قبول التصنيف أو الطعن فيه: `readyz_capabilities()` يُعيد اسم مزوّد ERP وحالة التهيئة كـ**بيانات قدرة تشغيليّة** لا حكماً على صحّة الحاوية (بخلاف `/readyz`'s فحص DB). التصنيف التجاريّ محقّ دلاليّاً لا تقنيّاً فقط. **لم أطلب استثناء تصنيفيّ جديد على غرار `/runtime-identity`** — ذاك مدعوم بعقد مُجمَّد صريح مذكور في CLAUDE.md نفسه (`platform_route_placement_contract.json`)، وهذا بلا عقد مماثل؛ طلب استثناء بلا أساس مُوثَّق كان سيكون اجتهاداً غير مُبرَّر.
- **بحث مستدعين حقيقيّين عبر المستودع كلّه — صفر نتائج للستّة الأولى (خلافاً لكلّ شريحة raster-service/soil-service السابقة، لكن مطابق لنمط mcp_servers السابقة مباشرة):**
  - frontend/mobile: صفر مرجع حرفيّ.
  - `nginx/nginx.v9.conf` (البوّابة الأساسيّة الحيّة): **لا بوّابة odoo/erp فيه إطلاقاً** — نفس نمط mcp_servers السابق.
  - `nginx/nginx.unified.conf`: `location /api/odoo/` و`location /api/erp/` كلاهما بروكسي شفّاف بلا rewrite (`proxy_pass http://erp_bridge_backend/;`) — تعليق صريح في الملفّ يوثّق أنّ `/api/odoo/` اسم مستعار توافقيّ قديم، `/api/erp/` هو العامّ الجديد. لا حاجة لتعديل نغينكس.
  - `services/mcp_servers/market_server.py`: يُعرِّف `ERP_BRIDGE_URL`/`ODOO_BRIDGE_URL` لكن **لا يستدعيهما أبداً** — بيانات ميّتة (مطابقة تماماً لدرس `tile_url_template`)، ولا حاجة لتحديث حتى لو كانا مُستهلَكين إذ القيمة قاعدة URL بلا مقطع مسار مُلحَق.
- **`POST /webhook/odoo` حالة خاصّة تستحقّ حدّ صدق صريح:** هذا المسار **يُستدعى من نظام Odoo خارجيّ**، لا من كود داخل هذا المستودع — الإعداد يتمّ يدويّاً عبر Odoo's Automation Rule UI (مُوثَّق في `docs/ODOO_INTEGRATION.md`'s قسم «إعداد Webhook»، كود Python يُلصَق داخل Odoo نفسه). لا سبيل تقنيّاً للتحقّق من أنّ نظام Odoo حيّ فعليّ (لو وُجِد) مُهيَّأ بالمسار القديم أو الجديد — هذا خارج حدود المستودع تماماً. عاملتُ هذا كنظير المستهلكين الخارجيّين غير المؤكَّدين في شرائح سابقة: حدَّثتُ المثال التوثيقيّ لضمان صحّة أيّ إعداد **مستقبليّ**، مع تسجيل صريح أنّني لا أستطيع إثبات ولا نفي وجود إعداد حيّ قائم فعلاً يُشير للمسار القديم.
- **مرجع مُضمَّن حيّ في استجابات API فعليّة — نفس درس `tile_url_template` بشكل أوضح هذه المرّة:** `routers/sync.py`'s ثلاث رسائل خطأ HTTPException (424 مزوّد غير مهيّأ، 503×2 مزوّد غير مستجيب/timeout) تُضمِّن نصّاً استشاريّاً حرفيّاً "Check /readyz/capabilities for ..." **يصل فعليّاً لجسم استجابة أيّ عميل يستدعي `/v1/sync` ويصطدم بهذه البوّابات** — هذا أوضح من حالة raster's `tile_url_template` (بيانات ميّتة لا يقرؤها أحد) لأنّ رسائل الخطأ هذه *تُقرأ* فعليّاً من قِبَل أيّ مستدعٍ حقيقيّ لـ`/sync`. حُدِّثت الثلاثة فوراً دون تردّد.
- **بحث توثيقيّ وُسِّع إلى `docs/audits/` — اكتشاف مهمّ عن طبيعة هذا المجلّد:** المجلّد ليس متجانساً؛ يحوي تقارير تدقيق مؤرَّخة حقيقيّة (مثل `RIV_DURABLE_PRODUCT_IDENTITY_AND_LEASES_REPORT_20260712.md` المتروك في شرائح سابقة) **وأيضاً** أدلّة إعداد/بناء ميزات حاليّة بلا طابع تاريخيّ في محتواها (`ERPNEXT_SETUP_GUIDE.md`، `ERP_PROVIDER_SWITCH.md`) رغم وجودهما في نفس المجلّد. فحصتُ الترويسة/المحتوى لكلّ ملفّ بدل الحكم على أساس المسار وحده — كلاهما بلا تاريخ صريح، بصيغة أمر/سرد بناء حاضر لا تقرير تدقيق زمنيّ، فحُدِّثا. **الدرس:** موقع الملفّ (`docs/audits/`) ليس دليلاً كافياً على أنّه سجلّ تاريخيّ يُترَك بلا لمس — المحتوى هو الحكم.
- **`docs/ODOO_INTEGRATION.md` (مؤرَّخ 2026-05-19، نفس معاملة `MARKET_SYSTEM.md` من شريحة mcp_servers):** ثمانية أمثلة `curl` (فحص اتصال، أربعة أمثلة مزامنة يدويّة، سجلّات، منتجات، تكاليف) + مثال Python كامل لإعداد Odoo Automation Rule — حُدِّثت جميعها.
- **`docs/UNIFIED_SETUP.md`** مثال `curl` حيّ واحد ضمن سير عمل تشغيل سريع (`/api/odoo/sync` ⇐ `/api/odoo/v1/sync`).
- **تصحيح ترافقيّ اكتُشف بالمصادفة أثناء تحرير `docs/openapi/API_MAP.md`'s قسم odoo-bridge:** `GET /products` (بلا `/v1/`) كان لا يزال مُدرَجاً — بائت سابق الوجود من شريحة الوكيل A الأولى الأصليّة (المسار الحقيقيّ `/v1/products` مُصدَّر منذ ذلك الحين، لكن الوثيقة لم تُصحَّح يومها). صُحِّح ضمن نفس التعديل بدل تركه متناقضاً بجوار الأسطر المُهاجَرة حديثاً — نفس درس raster PR-R4's اكتشافات الترافقيّة لبقايا سابقة.
- **اختبار تكذيب جديد، مُثبَت بكسره فعليّاً ثمّ استعادته:** `tests_v9/test_api_versioning_policy_guard.py::test_odoo_bridge_erp_routes_are_versioned` — كسر مقصود (إعادة `@router.get("/erp/provider")` الخام) أسقط الاختبار فوراً باسم المسار المتسرِّب حرفيّاً؛ استعادة الإصلاح أعادته أخضر.
- **راتشِت الإصدار:** `ceiling` ٤٧ ⇒ **٤٠** يدويّاً في `api_versioning_legacy_baseline.json`.
- **خمس دورات تصحيح متتالية في `verify_all_generated.py` — أطول سلسلة في هذه الجلسة، أطول حتى من actuator-service شريحة الوكيل A الثالثة (خمس دورات أيضاً لكن بسبب مختلف):** الدورة الأولى اكتشفت `runtime_contract_generator.py` (لم يظهر في أيّ شريحة سابقة من raster/soil/mcp_servers)، الثانية `runtime_verification_harness.py`، الثالثة `runtime_evidence_ingestion.py`، الرابعة كشفت ثلاثة معاً (`compose_runtime_target_resolver.py`+`integration_runtime_governance_closure.py`+`path3_runtime_readiness_closure.py`) — كلّ دورة تلتها إعادة بناء حزمة الإصدار فوراً قبل الدورة التالية. **الفرضيّة الأرجح:** ملفّ `main.py`/`routers/webhooks.py`'s تحديث المُستهلَك من `runtime_contract_generator.py` (يفحص عقود service runtime) أوّل مرّة يُلمَس فيها odoo-bridge منذ إدخال هذه السلسلة من المولّدات في جلسات سابقة — لم يظهر هذا التسلسل الطويل في raster/soil/mcp_servers لأنّها لم تُحرِّك عقود runtime لتلك الخدمات بنفس الطريقة.
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف بعد خمس دورات تصحيح · `pytest -m unit`: **3767 نجح** (+١)، 26 تخطّي، صفر فشل جديد · `ruff check .`/`ruff format --check .` نظيفان.
- **بهذه الشريحة تُغلَق كلّ سلاسل الهجرة المُعلَنة صراحةً من المستخدم في هذه الجلسة (raster-service أربع شرائح، soil-service، mcp_servers، erp-bridge).** إعادة قياس نهائيّة للجرد الخام بعد الهجرة كشفت توزيعاً مفيداً للمتبقّي ٤٠ مساراً `legacy_unversioned_business` عبر ثلاث خدمات فقط لا أكثر:
  - **auth (٢٤):** مؤجَّل صراحةً بقرار مُسجَّل مسبقاً في هذه الجلسة (ينتظر إثبات مستهلك حيّ).
  - **sahool-platform (٨):** `POST /api/chat` + سبعة في `routers/compat_gateway.py` — ديون توافقيّة بوّابة قديمة صريحة، مُوثَّقة ومؤجَّلة عمداً منذ شريحة Option B (2026-07-30)، لا فجوة اكتشاف جديدة.
  - **video-processor (٨، `routers/streams.py`) — اكتشاف جانبيّ جديد لم يظهر في أيّ نقاش أو تخطيط سابق لهذه الجلسة، وليس جزءاً من التسلسل المُعلَن صراحةً (raster⇐soil⇐mcp_servers⇐erp-bridge):** `POST /streams`، `DELETE /streams/{stream_id}`، `GET /streams/{stream_id}`، `GET /streams`، `POST /streams/{stream_id}/snapshot`، `GET /streams/{stream_id}/snapshot`، `POST /streams/{stream_id}/record/start`، `POST /streams/{stream_id}/record/stop`. **لم يُبدَأ أيّ بحث استهلاك ولا أيّ تصنيف تفصيليّ لهذه الشريحة** — مجرّد قياس عدديّ عابر أثناء التحقّق النهائيّ بعد erp-bridge. بنفس الانضباط المُطبَّق طوال هذه الجلسة (كلّ شريحة جديدة تبدأ فقط بعد أمر مستخدم صريح «ابدأ بـ...»)، هذا الاكتشاف **يُسجَّل ولا يُنفَّذ** — يحتاج قرار نطاق مستخدم صريح قبل أيّ لمس فعليّ.

## 2026-07-31 — video-processor: اكتشاف جديد نُفِّذ بأمر صريح — 8 مسارات streams
- **الأمر:** «ابدأ بـ video-processor» — أمر مستخدم صريح بعد أن كشف قياس ما بعد erp-bridge أنّ هذه الخدمة تحمل ٨ مسارات `legacy_unversioned_business` لم تكن جزءاً من أيّ خطّة أو تسلسل مُعلَن مسبقاً في هذه الجلسة.
- **إعادة القياس أوّلاً:** `guard.collect()` الحيّ لخدمة video-processor: ٣ `infra` + **٨ `legacy_unversioned_business`** — كلّها في ملفّ واحد `services/video-processor/routers/streams.py`، لا تفكيك متعدّد الملفّات كما في soil-service/odoo-bridge.
- **الثمانية مسارات هاجرت ⇐ `/v1/streams*`:** `POST /streams` (`:46`) · `DELETE /streams/{stream_id}` (`:91`) · `GET /streams/{stream_id}` (`:112`) · `GET /streams` (`:135`) · `POST /streams/{stream_id}/snapshot` (`:161`) · `GET /streams/{stream_id}/snapshot` (`:179`) · `POST /streams/{stream_id}/record/start` (`:201`) · `POST /streams/{stream_id}/record/stop` (`:221`) — كلّها `main._get_current_user` (Bearer/JWT، عزل مستأجِر صارم يُشتقّ من الرمز لا من الجسم/المسار).
- **بحث مستدعين حقيقيّين عبر المستودع كلّه — صفر نتائج لكلّ الثمانية:**
  - frontend/mobile: صفر مرجع حرفيّ لأيّ من المسارات الثمانية، وصفر ثابت `VIDEO_PROCESSOR_URL`/`videoApi` في أيّ كود عميل.
  - لا خدمة أخرى في المستودع تُعرِّف ثابت URL يُشير لـvideo-processor إطلاقاً (بحث `VIDEO_PROCESSOR_URL`/`video-processor:8000`/`sahool-video-processor` عبر `services/*.py` — صفر نتائج) — فارق عن mcp_servers's `ERP_BRIDGE_URL` (كان مُعرَّفاً لكن ميّتاً)؛ هنا الثابت نفسه غائب.
- **فارق مهمّ اكتُشف واستحقّ تحقّقاً إضافيّاً — video-processor له بوّابة نغينكس فعليّة في الملفّ الأساسيّ الحيّ، بخلاف mcp_servers/erp-bridge:** `nginx/nginx.v9.conf:480` يحمل `location /api/video/` فعليّاً (لم يكن موجوداً لـmarket/odoo في نفس الملفّ). فحصتُ محتواه كاملاً قبل الحكم بعدم الحاجة لتعديل: مُقيَّد صراحةً بنطاقات شبكة داخليّة فقط (`allow 127.0.0.1; allow 10.0.0.0/8; allow 172.16.0.0/12; allow 192.168.0.0/16; deny all;`) مع تعليق مصدريّ صريح يصف video-processor وagriai كـ«أسطح داخليّة/خدمة-لخدمة» محميّة بـJWT/توكن خدمة، لا واجهة متصفّح مباشرة. البروكسي نفسه شفّاف بلا `proxy_pass` URI (`http://video_backend/;`) فلا حاجة لتعديله رغم وجوده الفعليّ — نفس خلاصة الشرائح السابقة لكن بمسار تحقّق مختلف (تأكيد القيد الشبكيّ بدل تأكيد الغياب الكامل).
- **اكتشاف جانبيّ أثناء البحث عن capability-registry — فجوة تغطية قدرات سابقة الوجود:** `capabilities/registry/capabilities.json` وكلّ ملفّات `docs/capability-registry/domains/*.yaml` **لا تذكر video-processor إطلاقاً** — صفر ربط قدرة من أيّ نوع (لا حتى ربطاً تحفّظيّاً heuristic كما رأينا مع mcp_servers/erp-bridge). هذه فجوة تغطية قائمة سلفاً، خارج نطاق هذه الهجرة تحديداً (لا مسار مُدرَج يحتاج تحديث نصّي، لا شيء لأصلحه هنا) — سُجِّلت للتوثيق لا للإصلاح.
- **مطابقات نصّيّة كاذبة فُحِصت بعناية وتُركت — درس عن الاعتماد على بحث نصّيّ سطحيّ وحده:** بحث `grep` عن كلمة "video"/"stream" عبر `docs/` أعاد ثلاثة ملفّات (`docs/audits/PONYTAIL_MCP_STREAMING_KG_EXECUTION_20260625.md`، `docs/audits/MCP_STREAMING_KG_COMPLETION_20260625.md`، `docs/architecture/agentic_rag_mcp_streaming_plan.md`) — تحقّق مباشر بفتح كلّ ملفّ والبحث فيه تحديداً عن `video-processor`/`/streams` أكّد أنّها تخصّ مفهوماً مختلفاً تماماً (بثّ استجابات نموذج/MCP تدريجيّاً — «streaming responses» — لا بثّ فيديو كاميرا)؛ صفر مرجع حقيقيّ. لم تُلمَس.
...
- **راتشِت الإصدار:** `ceiling` ٤٠ ⇒ **٣٢** يدويّاً في `api_versioning_legacy_baseline.json`.
- **دورة تصحيح واحدة فقط في `verify_all_generated.py` — أسرع من erp-bridge بكثير، درس تشغيليّ مطبَّق:** بعد أن اكتشفت شريحة erp-bridge خمس دورات متتالية (`runtime_contract_generator.py`، `runtime_verification_harness.py`، `runtime_evidence_ingestion.py`، `compose_runtime_target_resolver.py`، `path3_runtime_readiness_closure.py`)، شغَّلتُ هذه الخمسة استباقيّاً في هذه الشريحة قبل أوّل فحص `--check` بدل انتظار اكتشافها تباعاً — النتيجة: صفر انحراف من أوّل تشغيل لسلسلة `verify_all_generated.py` الكاملة، توفير خمس دورات تصحيح كانت متوقَّعة.
- **التحقّق:** `verify_all_generated.py` ⇒ صفر انحراف من أوّل تشغيل · `pytest -m unit`: **3768 نجح** (+١)، 26 تخطّي، صفر فشل جديد · `ruff check .`/`ruff format --check .` نظيفان.
- **بهذه الشريحة يُغلَق كلّ ما هو `legacy_unversioned_business` قابل للهجرة الفعليّة في هذه الجلسة.** الجرد الخام المتبقّي ٣٢ مساراً مُوزَّعة على خدمتين فقط، كلتاهما مؤجَّلتان بقرار صريح سابق لا فجوة اكتشاف جديدة:
  - **auth (٢٤):** مؤجَّل حتى إثبات مستهلك حيّ.
  - **sahool-platform (٨):** ديون توافقيّة `compat_gateway.py`/`chat_proxy_reference.py` مؤجَّلة عمداً منذ شريحة Option B.
  - **لا شريحة هجرة نشطة متبقّية إلّا بأمر مستخدم صريح جديد** — إن أراد المستخدم متابعة auth أو sahool-platform، يحتاج ذلك قراراً صريحاً منفصلاً (auth تحديداً مشروط بإثبات مستهلك حيّ لم يُقدَّم بعد).

## 2026-07-31 — auth: الشريحة الأخطر والأخيرة المُعلَنة صراحةً — 24 مساراً
- **الأمر:** «ابدأ بـ auth» — التنفيذ المباشر للترتيب الأخير المُقرَّر مسبقاً (raster-service ⇐ soil-service ⇐ mcp_servers ⇐ erp-bridge ⇐ video-processor، ثمّ **auth**). أعلى مخاطرة في هذه الجلسة كاملة: ٢٤ مساراً تحرس تسجيل الدخول/الجلسة/التفويض على مستوى المنصّة كلّها، بخلاف كلّ شريحة سابقة (خدمات منتَج معزولة نسبيّاً).
- **بحث ذاتيّ استباقيّ + وكيل بحث خلفيّ مُستقلّ قبل أيّ لمس كود:** نظراً لارتفاع المخاطرة، أُجري بحث شخصيّ عميق في بوّابتَي nginx الداخليّتين (`_auth_verify`/`_auth_edge_sign`) موازياً لتشغيل وكيل استكشاف خلفيّ لمسح شامل لكلّ مرجع حرفيّ متبقٍّ عبر المستودع — لم يُلمَس أيّ كود حتى اكتمل كلا المسارين، مطابقةً لحدود المخاطرة/القابليّة للعكس في تعليمات النظام.
- **إعادة القياس أوّلاً:** `guard.collect()` الحيّ لخدمة auth: ٤ `infra` + **٢٤ `legacy_unversioned_business`** عبر تسعة ملفّات راوتر.
- **الأربعة وعشرون مساراً هاجرت ⇐ `/v1/auth/*`:** `email_verify.py` (٤) · `invitations.py` (٤) · `mfa.py` (٣) · `password_reset.py` (٢) · `registration.py` (٢) · `season_edge_sign.py` (١) · `session.py` (٤) · `tenants.py` (١) · `users.py` (٣). نفس نمط التسجيل المسطّح (`register_routers(app)` بلا prefix، نصّ الديكوريتر هو المسار الفعليّ حرفيّاً) المُتّبَع في كلّ شرائح هذه الجلسة.
- **رسائل استشاريّة عميلة داخل استجابات API — قرار مبدئيّ مختلف عن erp-bridge:** `session.py`/`mfa.py` تُضمِّن نصوصاً مثل "أكمِل الإعداد عبر /auth/mfa/setup" في `HTTPException.detail` تصل فعليّاً للعميل. خلافاً لدرس erp-bridge (`sync.py`'s رسائل حُدِّثت للمسار الجديد لأنّه لا طبقة ترجمة بينها وبين العميل)، هذه الرسائل **تُركت `/auth/*` كما هي عمداً** — العميل الحقيقيّ (متصفّح/تطبيق جوّال) يستدعي دائماً الاصطلاح الحرفيّ `/auth/*` عبر nginx الذي يترجمه إلى `/v1/auth/*` خادميّاً؛ تحديث الرسالة إلى `/v1/auth/mfa/setup` كان سيُعطي العميل مساراً لا يعرفه (nginx لا يترجم مسارات داخل نصّ JSON).
- **طبقة أولى لم تظهر بأيّ شريحة سابقة — بوّابتا nginx داخليّتان حرجتان يستهدفهما `auth_request` من مواقع متعدّدة:** `location = /_auth_verify` (٧ مواقع `auth_request` في `nginx.v9.conf` وحده: vegetation، raster، remote-sensing-workspace ×2، seasons ×2 — زائداً موقع مستقلّ ثانٍ بهدف مختلف `sahool-auth:8000` (لا `auth_backend` عبر upstream) في `frontend/nginx.conf`) و`location = /_auth_edge_sign` (يستهدفها `auth_request` من مسار قبول الموسم الحسّاس `POST /api/v1/seasons/{id}/accept`، `SEASON-RECORD-ENTRY-01` شريحة 3b). كانت `proxy_pass http://auth_backend/auth/verify;`/`.../auth/edge-sign;` و`http://sahool-auth:8000/auth/verify;` حرفيّاً. عدم تحديثها كان سيُسقِط JWT verification صامتاً (فشل مغلق 401 على كلّ الطلبات المحكومة — لا اختراق أمنيّ، لكن انقطاع منصّة كامل) خلف **كلّ** سطح محكوم بـ`auth_request` في ثلاثة ملفّات nginx. حُدِّثت الثلاث المواضع (اثنان في `nginx.v9.conf`، واحد في `frontend/nginx.conf`) إلى `/v1/auth/verify`/`/v1/auth/edge-sign`.
- **طبقة ثانية — موقع `/auth/` العميل المُواجِه في خمسة ملفّات nginx، اصطلاحان متضاربان قبل التعديل:** الواجهة (`authApi` قاعدتها `/auth`، تستدعي `.post('/auth/login')` ⇒ طلب مزدوج `/auth/auth/login` على السلك) وعميل مباشر (`/auth/login` مفرد، مستخدَم فعليّاً في تطبيق الجوّال Flutter عبر `dio` بقاعدة API الكاملة). قبل التعديل: `nginx.v9.conf`/`nginx.fixed.conf`/`frontend/nginx.conf` استخدمت `rewrite ^/auth/auth/(.*)$ /auth/$1 break;` (يُطبِّع المزدوج فقط، يُمرِّر الـURI كما هو لكلا الشكلين — كانت تعمل صحيحاً لكلا الاصطلاحين على المسار القديم غير المُرقَّى)؛ `nginx.unified.conf`/`nginx.light.conf` استخدمتا تقنيّة مختلفة تماماً `proxy_pass http://auth_backend/;` (تجريد شرطة لاحقة). حُقِّق بجدول حقيقة يدويّ (لا مُصدِّق nginx محليّاً في هذا الصندوق): هذه التقنيّة الثانية تدعم الاصطلاح المزدوج بالمصادفة فقط (تجريد طبقة `/auth/` واحدة من `/auth/auth/login` يُنتِج `/auth/login` بالضبط) لكنّها **لم تدعم قطّ** الاصطلاح المفرد/المباشر (`/auth/login` ⇐ تجريد ⇐ `/login`، ٤٠٤) — ثغرة سلوك **سابقة الوجود** في `unified`/`light`، لم تُدخِلها هذه الشريحة، ولا يمكن تمديدها ببساطة لإضافة `/v1/` (إلحاق `/v1/auth/` بهدف `proxy_pass` كان سينتج مساراً مضاعَفاً `/v1/auth/auth/login` للاصطلاح المزدوج). الإصلاح المُوحَّد في الخمسة كلّها — استبدال بقاعدتَي `rewrite`:
  ```
  location /auth/ {
  ...
  ```
  مُتحقَّق بجدول حقيقة يدويّ لكلا المدخلين (`/auth/auth/login` ⇐ القاعدة الأولى تطابق وتُنهي بـ`break` ⇐ `/v1/auth/login`؛ `/auth/login` ⇐ الأولى لا تطابق (تتطلّب حرفيّاً `auth/auth/`) ⇐ الثانية تطابق ⇐ `/v1/auth/login`) ومُتحقَّق أنّه يُبقي `tests_v9/test_auth_rls_and_routing.py::test_nginx_auth_routing_preserves_prefix` أخضر بلا تعديل جوهر التأكيدات (تبحث عن `rewrite\s+\^/auth/auth/` حرفيّاً — لا تزال موجودة كالقاعدة الأولى؛ وعن غياب شكل شرطة لاحقة `proxy_pass\s+http://auth_backend/\s*;` — لا يزال غائباً). لـ`unified`/`light` هذا **تحسين سلوك حقيقيّ**، لا مجرّد إعادة تسمية — يضيف دعماً للاصطلاح المفرد لم يكن موجوداً قطّ.
- **فجوة CI ثالثة اكتُشفت بالقراءة الذاتيّة لكود الحارس، لا بالافتراض:** `scripts/ci/endpoint_ui_coverage_gate.py`'s `has_frontend_evidence()` طبَّعت تاريخيّاً `/api/v1/auth/*` فقط (اصطلاح sahool-platform المنفصل الخاصّ به، مسار مختلف كليّاً عن auth) ⇄ `/auth/*` — بُنيت لاصطلاح لم تستخدمه خدمة auth قطّ. `config/endpoint_ui_coverage.json`'s قائمة `classifications` تحمل قاعدة صريحة `{"prefix": "/auth/", "audience": "farmer"}` تُصنِّف كلّ مسار auth مواجِهاً للمستخدم فعليّاً (وليس `internal` كما افتُرِض ابتدائيّاً قبل قراءة الكود مباشرة) — و`core_endpoints` يحمل ٢٠ إدخالاً صريحاً لهذه المسارات (٢٤ ناقص اثنين مُعفيَين `/auth/edge-sign`/`/auth/me` في `endpoint_ui_coverage_waivers.json`، ناقص تداخل GET/POST على نفس مسار `/invitations`). بعد الترقية، وإعادة تسمية القاعدة `/auth/` ⇒ `/v1/auth/` (لازمة كي تبقى الـ٢٢ مساراً غير المُعفاة مُصنَّفة مواجِهاً بدل الانحدار الصامت إلى `internal` المُعفى تماماً من البوّابة العكسيّة — فقدان تتبّع صامت لا فشل CI، لكن ما زال قراراً سيّئاً)، كانت القاعدة القديمة في `has_frontend_evidence()` ستُنتِج ٢١ «مساراً هارباً» مُزيَّفاً لأنّها لا تعرف بادئة `/v1/auth`. الإصلاح: `_AUTH_BACKEND_PREFIX_RE = re.compile(r'^/(?:api/v1|v1)/auth')` تطبّع كلا الاصطلاحين (منصّة + auth) إلى `/auth` بدل نمط `.replace()` الحرفيّ القديم. **مُكذَّب فعليّاً بمحاكاة المنطق القديم مؤقّتاً:** استبدال طريقة التطبيع الجديدة بالقديمة حرفيّاً (بلا لمس بقيّة الملفّ) أنتج فشلاً حرفيّاً بالضبط بـ٢١ مساراً مُسمّاة صراحةً (`endpoint-ui-coverage-reverse-gate: FAIL`)؛ استعادة الإصلاح أعادت `PASS — 425 core + 52 إعفاء`. حُدِّثت أيضاً ٢٠ إدخال `core_endpoints` (حقل `endpoint` فقط؛ حقل `evidence` بقي كما هو تماماً — نصّ استدعاء العميل الحرفيّ `/auth/x` لم يتغيّر، nginx يترجمه لا الفحص) وإدخالا إعفاء إلى `/v1/auth/*`.
- **اكتشاف رابع بالقراءة الذاتيّة لكود frontend، لم يُذكَر في تقرير وكيل البحث الخلفيّ:** `frontend/src/config/endpoints.ts`'s `VITE_API_MODE=dev` (منافذ localhost مباشرة صراحةً «بلا بوّابة» بحسب تعليق الملفّ نفسه) كان افتراض قاعدة auth `http://localhost:8120` بلا `/v1` — استدعاء `authApi.post('/auth/login')` غير المُعدَّل (وهو الصحيح تركه بلا تعديل، انظر أعلاه) كان سيصل حرفيّاً `http://localhost:8120/auth/login` في هذا الوضع، مساراً لم يعد قائماً على الخدمة بعد الترقية إلى `/v1/auth/login`، خلافاً لوضع الإنتاج حيث nginx يترجم. أُصلِح بتضمين `/v1` في `devDefault` نفسه (`` `${localHttp(8120)}/v1` ``) — القاعدة فقط، لا نصّ استدعاء العميل — فتبقى `authApi.post('/auth/login')` بلا تغيير وتُنتِج المسار الصحيح `http://localhost:8120/v1/auth/login`. لا اختبار frontend يُثبِّت هذه القيمة حرفيّاً (لا Vitest لهذا الملفّ)، فالتحقّق قراءة يدويّة لسلسلة الاستدعاء الكاملة (`ENDPOINTS.auth` ⇐ `resolveHttpBase` ⇐ `authApi = makeClient(AUTH_URL)` ⇐ `.post('/auth/login')`).
- **مستهلكون حقيقيّون فُحِصوا بالكامل — بحث شامل لا افتراض، مطابق لانضباط كلّ شريحة سابقة:**
  - `frontend/src/services/api/auth.ts` (٢٣ استدعاءً مُؤكَّداً) و`mobile/sahool_app/lib/services/api_service.dart` (٨ استدعاءات) — **لم يُعدَّلا عمداً**: كلاهما يستدعي `/auth/*` الحرفيّ عبر nginx الذي يترجمه الآن؛ تعديلهما كان سيُغيّر عقداً عميلاً بلا حاجة فعليّة.
  - `bots/telegram/main.py:350` استدعاء مباشر لـ`AUTH_SERVICE_URL` (افتراضه `http://sahool-auth:8000` — **بلا** nginx بينهما، خلافاً لكلّ ما سبق) — **عُدِّل** حرفيّاً `/auth/login` ⇐ `/v1/auth/login`.
  - `services/mcp_servers/market_server.py`'s و`services/odoo-bridge/erp_runtime.py`'s `SAHOOL_API_URL` (كلاهما يشير افتراضيّاً لمنفذ auth) — متغيّران مُصرَّحان بلا أيّ مسار `/auth/*` مُلحَق بهما في أيّ مكان بالمستودع، بيانات ميّتة غير مُستهلَكة فعليّاً (نفس درس `ERP_BRIDGE_URL`/`tile_url_template`)، لا حاجة لتعديل.
  - سكربتات E2E (`scripts/smoke_e2e.py`، `scripts/e2e/live_full_e2e.py`، `scripts/e2e/spatial_flows.py`، `tests_v9/smoke_frontend_activation.sh`، `tests_v9/test_smoke_e2e.py`) تعمل عبر البوّابة فعبرت بلا تعديل — قاعدتها الافتراضيّة `http://localhost[:8080]` تمرّ عبر nginx الذي يترجم؛ مُتحقَّق فرداً فرداً من مصدر كلّ متغيّر قاعدة قبل الاستنتاج، لا تعميم.
...
- **ملفّات اختبار مُحدَّثة** (استدعاءات `TestClient`/HTTP مباشرة بلا nginx بينها، فتتطلّب المسار الجديد حرفيّاً وإلّا فشلت 404): `test_auth_e2e.py`، `test_auth_service.py`، `test_security.py`، `test_mfa_hardening_integration_v29_5.py` (مُعلَّم `integration` — يعمل بغياب DB حيّ في CI، لكن يجب أن يبقى صحيحاً حين يعمل). زائداً تحديثات نصّيّة/توثيقيّة بحتة (لا تأكيدات تنفيذيّة تغيّرت — الاصطلاح الحرفيّ الجديد يبقى صحيحاً كسلسلة substring في بعضها) في `test_auth_rls_and_routing.py`، `test_dev_gateway_raster_tile_auth_guard.py`، `test_season_gateway_nginx_static.py`، `test_season_edge_sign_endpoint_static.py`، `test_nginx_tenant_injection_guard.py`.
- **وثائق مُحدَّثة:** `docs/openapi/API_MAP.md`/`ROUTE_INVENTORY.json` (١٢ من ٢٤ المُدرَجة سلفاً فقط — نقص جزئيّ سابق الوجود، مطابق لنمط الشرائح السابقة، لم يُملأ)، `docs/capability-registry/domains/farm_management.yaml` (٢٥ سطراً بما فيها تكرار `edge-sign` في قدرة الموسم المنفصلة)، `satellite.yaml` (٢)، `security.yaml` (٣)، `docs/security/SEC-3.1_approvals_user_role.md`، `docs/runbooks/REAL_ENV_VERIFICATION_RUNBOOK.md`، `docs/api/UI_DEBT_MAP.md` (٦ إشارات في جدول دَين واجهة تاريخيّ لكنّه حيّ يُحرَّر يدويّاً — لا مولِّد). `docs/NGINX_ROUTING.md` أعيدت كتابة قسم `/auth/*` بالكامل (الجدول + الملاحظة ١) — كانت تصف تقنيّة القطع بشرطة لاحقة (`proxy_pass http://auth_backend/;`) وهي أصلاً **لا تطابق** ما في `nginx.v9.conf` الفعليّ (كان يستخدم `rewrite` سلفاً قبل هذه الشريحة بالكامل) — تصحيح استباقيّ لتوثيق كان زائفاً من قبل هذه الهجرة، لا فقط تحديث مسار.
- **تُركت عمداً بعد فحص مباشر (سجلّ تاريخيّ، لا يُزيَّف بأثر رجعيّ):** `docs/audits/CODE_REVIEW_REPORT.md`، `CRITICAL_REVIEW_RESPONSE.md`، `NEW_TESTS_AND_FIXES.md` (روايات جولات مراجعة ماضية توثّق ما كان صحيحاً وقتها)، `docs/history/*`، و`docs/architecture/brain_deferral_baseline.json` (لقطة مجمَّدة لحارس انحراف مؤجَّل — تعديلها يُزيّف الأساس المرجعيّ لا يُصحِّحه).
- **راتشِت الإصدار:** `ceiling` ٣٢ ⇒ **٨** يدويّاً في `api_versioning_legacy_baseline.json`، مُتحقَّق بـ`api_versioning_policy_guard.py --check` بعد التحديث.
- **التحقّق:** `guard.collect()` الحيّ يُظهِر صفر `legacy_unversioned_business` لـauth (٢٤/٢٤ `versioned`) · `endpoint-ui-coverage-reverse-gate` ⇒ PASS (425 core + 52 إعفاء صريح، لا مسار فالت) · `verify_all_generated.py` ⇒ صفر انحراف من أوّل تمريرة (نفس تحسين video-processor: تشغيل كلّ المولِّدات المكتشَفة سابقاً استباقيّاً بدل انتظار اكتشافها تباعاً) · `pytest -m unit`: **3771 نجح** (+٣)، 26 تخطّي، صفر فشل.
- **بهذه الشريحة يُغلَق `legacy_unversioned_business` عند ٨ مساراً فقط — كلّها sahool-platform (ديون توافقيّة `compat_gateway.py`/`chat_proxy_reference.py` موثَّقة صراحةً منذ شريحة Option B، مؤجَّلة عمداً لا فجوة اكتشاف).** **auth كانت آخر شريحة هجرة نشطة مُعلَنة صراحةً في هذه الجلسة — لا شريحة متبقّية إلّا بأمر مستخدم صريح جديد.**

## 2026-07-31 — إحكام حوكميّ لحارس API-VERSIONING-GUARD-IS-A-MIRROR-01 (لا هجرة مسار جديدة)
- **الأمر:** المستخدم دقّق نتائج شريحة auth (PR #732، مدموجة) وسأل عن ضمانة الحارس الفعليّة، فطرح سؤال حوكميّ دقيق: هل `--check` يفرض `generated == recomputed` و`recomputed ≤ frozen_ceiling` فقط (عدّ)، أم يفرض أيضاً أنّ المجموعة الحاليّة **مجموعة جزئيّة** من المجموعة المُجمَّدة (`current_set ⊆ frozen_baseline_set`) لا مجرّد أنّ حجمها لا يتجاوز السقف؟ قرأتُ `scripts/ci/api_versioning_policy_guard.py:182-224` مباشرةً للإجابة بدقّة بدل الافتراض: أكّدت أنّ الشرط الوحيد المفروض فعليّاً هو `len(current) <= ceiling` — عدّ صرف، لا مطابقة هويّة. هذا يعني أنّ استبدال دَين مُقفَل بدَين جديد مختلف كلّياً (إغلاق مسارَين ثمّ اكتشاف/فتح مسارَين آخرَين) كان سيمرّ صامتاً طالما العدد الكلّي لا يتجاوز السقف — فجوة حوكميّة حقيقيّة، مؤكَّدة بقراءة الكود لا افتراضاً. أكّدتُ أيضاً بشكل منفصل أنّ ادّعاء المستخدم عن كون الأساس (`api_versioning_legacy_baseline.json`) لا يزال يسبق هجرة auth كان **غير صحيح**: الأساس كان مُحدَّثاً فعلاً (`ceiling: 8`) في نفس PR الذي دمجناه للتوّ — تحقّقتُ من `git show HEAD:...` على فرع auth-v1-migration المدموج لإثبات ذلك بدلاً من الافتراض.
- **الإصلاح:** أُضيف حقل `routes` (مصفوفة) إلى `docs/architecture/api_versioning_legacy_baseline.json` — المجموعة المُجمَّدة الصريحة للثمانية مسارات `legacy_unversioned_business` المتبقّية حاليّاً (سبعة من `services/sahool-platform/api/routers/compat_gateway.py` + واحد من `chat_proxy_reference.py`، مُتحقَّق بقراءة `guard.collect()` الحيّ مباشرةً قبل الكتابة، لا نسخاً من ملفّ آخر). في `main()`'s فرع `--check`، أُضيف شرط ثانٍ مستقلّ بعد الشرط القديم (العدّ): `current_set = set(current_routes)`، `frozen_set = set(baseline_data.get("routes"))`، فإن `current_set - frozen_set` غير فارغة ⇒ `SystemExit` يسمّي المسار/المسارات الهاربة صراحةً، بصرف النظر عن كون العدد الكلّي تحت السقف أم لا. الحقل `ceiling` (عدد صرف) أُبقِي كما هو للتوافق الرجعيّ وسرعة العرض، لكنّ الإنفاذ الحقيقيّ الآن مزدوج: عدّ + هويّة.
- **حادثة تشغيليّة عرضيّة أثناء التنفيذ — خطأ نصف آليّ صحّحته فوراً:** عند بدء هذه الشريحة نفَّذتُ `git checkout main -- .` (لمزامنة working tree مع الفرع الرئيسيّ بعد دمج PR auth) بينما `local main` كان لا يزال يشير إلى commit **قبل** دمج auth (لم أكن قد حدّثت الفرع المحلّي `main` طوال الجلسة، عملتُ دائماً على فروع feature منفصلة). النتيجة: الأمر أعاد كتابة working tree إلى المحتوى **القديم** (بما فيه `/auth/login` الخام قبل الترقية!) رغم أنّ `HEAD` (الفرع الفعليّ `auth-v1-migration`) كان لا يزال يشير للـcommit المدموج الصحيح — تعارض بين index/working tree وHEAD. اكتُشف فوراً بفحص `git status --short` (كلّها `M `، أي مُهيَّأة staged) ثمّ `grep` مباشر على `session.py` كشف عودة النصّ القديم. أُصلِح بـ`git reset --hard HEAD` (استعادة فوريّة، صفر خسارة — لم يُلتزَم أو يُدفَع شيء)، ثمّ `git branch -f main origin/main` لتحديث الفرع المحلّي الراكد فعليّاً بدل تركه مصدر خطأ متكرّر. **الدرس المُسجَّل:** `git checkout <ref> -- .` بمرجع محلّيّ راكد (لم يُحدَّث بـ`pull`/`fetch` منذ إنشائه) يُعيد كتابة working tree صامتاً بمحتوى بائت حتى لو كان HEAD الفعليّ محدَّثاً — تحقّق دائماً من حداثة أيّ مرجع محلّيّ (`git log --oneline <ref> -3` مقابل `origin/<ref>`) قبل استخدامه كمصدر checkout، لا الاكتفاء بالاسم.
- **اختبار تكذيب جديد، مُثبَت بطريقتين منفصلتين:**
  1. محاكاة يدويّة أوّليّة (قبل كتابة اختبار pytest): تعديل مباشر لِـ`api_versioning_legacy_allowlist.generated.json` (إزالة `POST /api/chat`، إضافة `GET /api/fake-new-legacy-route` وهميّ، العدد الإجماليّ يبقى ٨) — اصطدمت بحارس الانحراف **الموجود مسبقاً** (`api versioning inventory drift`) قبل الوصول للمنطق الجديد إطلاقاً، لأنّ `write(rows)` يعيد كتابة الملفّ من الحساب الحيّ الحقيقيّ فيكتشف تلاعبي كفارق نصّيّ خام. **درس منهجيّ:** الطريقة الصحيحة لاختبار منطق `--check` الجديد فعليّاً هي تعديل `routes` في **الأساس المُجمَّد** (`api_versioning_legacy_baseline.json`) لا الملفّ **المولَّد** (`allowlist.generated.json`) — الأخير يُعاد حسابه وكتابته دائماً في بداية كلّ تشغيل، فأيّ تلاعب فيه يُبتلَع فوراً بحارس الانحراف الأقدم.
  2. بعد تصحيح المنهجيّة: إزالة `POST /api/chat` من `routes` في الأساس المُجمَّد (فقط، لا الملفّ المولَّد) ⇒ `--check` فشل حرفيّاً باسم `POST /api/chat` صراحةً ("قائمة السماح تحمل مساراً/مسارات جديدة ليست في المجموعة المُجمَّدة... POST /api/chat")؛ استعادة الأصل ⇒ `api_versioning_policy_check_ok`. هذه هي الطريقة المُثبَّتة في `tests_v9/test_api_versioning_policy_guard.py::test_baseline_check_rejects_legacy_route_swap_not_just_count` — تعدّل `routes` مؤقّتاً داخل `try/finally` (يضمن الاستعادة حتى عند فشل التوكيد نفسه)، تُشغِّل `--check` عبر subprocess حقيقيّ (لا استدعاء دالّة مباشر — يثبت السلوك من سطر الأوامر كما يراه CI فعليّاً)، تؤكِّد `returncode != 0` واحتواء اسم المسار المُسقَط في المخرجات، ثمّ تستعيد وتؤكِّد `returncode == 0`.
- **دورتا تصحيح في `verify_all_generated.py`:** تعديل `api_versioning_legacy_baseline.json` (إضافة حقل `routes`) أحدث انحرافاً تسلسليّاً — أوّلاً `capability_mapping_engine.py --check` (أُصلِح بـ`--generate`، استغرق أكثر من ١٢٠ ثانية فانتقل للخلفيّة)، ثمّ `pr_capability_impact_gate.py --check-index` (أُصلِح بـ`--generate-index`)، ثمّ فحص checksum لحزمة الإصدار (أُصلِح بإعادة `build_release_bundle.py`+`validate_release_package.py`) — نفس نمط "أيّ تعديل على `api_versioning_legacy_baseline.json` يستوجب إعادة بناء حزمة الإصدار كاملةً" المُلاحَظ في شريحة auth أيضاً.
- **التحقّق:** `ruff check`/`ruff format --check` نظيفان · `pytest -m unit`: **3772 نجح** (+١ عن auth)، صفر فشل، 26 تخطّي · `verify_all_generated.py` ⇒ صفر انحراف بعد الدورتين.
- **لا هجرة مسار في هذه الشريحة.** سقف `legacy_unversioned_business` يبقى ٨ (كلّها sahool-platform). هذه شريحة تحصين حوكميّ للحارس المشترك الذي اعتمدت عليه كلّ شرائح هذه الجلسة، لا تغييراً في أيّ خدمة منتَج.

## 2026-07-31 — إغلاق موضوعيّ للحملة: إيجابيّة كاذبة صُحِّحت + السبعة الباقية أُعيد توصيفها عقوداً دائمة (8 ⇒ 7)
- **الأمر:** «ابدأ» — تنفيذ الشريحة التي عرضتُها بعد دمج #733: تطبيق البند (١) وحده (استبعاد الإيجابيّة الكاذبة) مع توثيق الباقي، وإبقاء البند (٢) (إعادة تصنيف الأربعة الصحّيّة) معلّقاً على قرار صريح.
- **البحث قبل اللمس (لا افتراض):** قرأتُ `compat_gateway.py` و`chat_proxy_reference.py` و`router_registry.py` كاملةً قبل أيّ تعديل. النتيجة قلبت التوصيف: **لا واحد من الثمانية قابل للهجرة**، وواحد منها ليس مساراً مخدوماً أصلاً.
- **الإيجابيّة الكاذبة — `POST /api/chat`:** أربعة أقفال بنيويّة مستقلّة تُثبِت عدم التركيب: (١) الملفّ في `api/` لا `api/routers/`؛ (٢) `register_routers()` يُسجّل تلقائيّاً وحدات تلك الحزمة وحدها (`pkgutil.iter_modules(_routers_pkg.__path__)`، `router_registry.py:41`)؛ (٣) صفر استيراد إنتاجيّ في المستودع — بحث شامل أعاد مطابقتين فقط، كلتاهما **نصّ** لا كود (تعليق عربيّ في `ai_provider_config.py:4` يصف تاريخ الملفّ، وسطر docstring داخل الملفّ نفسه يعرض أمر `uvicorn` المثال)؛ (٤) الملفّ **لا يُصدِّر `router` إطلاقاً** بل `app = FastAPI(title="SAHOOL Chat Proxy")` داخل `try/except ImportError` — فحتى لو مسحته آليّة التسجيل لما وجدت ما تضمّه (`getattr(mod, "router", None)`).
- **سابقة مستقلّة قوّت الحكم ولم أعتمد عليها وحدها:** `tests_v9/test_endpoint_auth_coverage.py:60` يستثني هذا الملفّ **بالاسم** (`EXCLUDED_FILES`) منذ قبل هذه الشريحة، بتعليق يصفه «نمط مرجعيّ لـproxy آمن لـClaude». حارسان مستقلّان وصلا للحكم نفسه بأدلّة منفصلة.
- **الاستبعاد مسنود ببرهان لا بادّعاء — قرار تصميميّ مقصود:** كان أسهل أن أستثني بالاسم وأكتفي (كما تفعل السابقة). بدل ذلك أضفتُ `_UNMOUNTED_REFERENCE_FILES` **مع** اختبار `test_chat_proxy_reference_is_structurally_unmounted` يُثبِت الحقائق الأربع من الشجرة الحيّة (موضع الملفّ · نصّ آليّة التسجيل · مسح استيرادات إنتاجيّة · تحليل AST لغياب `router`) — فالاستثناء صار **ثابتاً مُتحقَّقاً** لا سطراً في قائمة. لو رُكِّب الملفّ يوماً أو نُقِل إلى `api/routers/` يسقط CI ويُجبِر إعادة التقييم بدل بقاء الاستثناء صامتاً يخفي مساراً صار حقيقيّاً.
- **تكذيب رباعيّ فعليّ (لا وصفيّ):** كسرتُ الأقفال الأربعة واحداً واحداً وأثبتُّ سقوط الاختبار برسالة مُسمّاة في كلٍّ منها، ثمّ استعادة خضراء: (أ) نسخة داخل `api/routers/` ⇒ `assert not (...).exists()`؛ (ب) تغيير `_routers_pkg.__path__` في آليّة التسجيل ⇒ «router_registry no longer auto-mounts by scanning api/routers/»؛ (ج) إضافة `from api import chat_proxy_reference` إلى `ai_provider_config.py` ⇒ فشل يسمّي الملفّ المستورِد حرفيّاً؛ (د) إضافة `router = None` ⇒ «module now exports `router` — it may be auto-mounted».
- **إعادة توصيف السبعة الباقية — الاستنتاج الأهمّ في هذه الشريحة:** كلّها `compat_gateway.py`، وكلّها **أسماء بديلة لمسارات قديمة بالتصميم**: أربعة تُعيد حمولة صحّيّة خالصة `{"status": "ready", "service": …, "mode": "alias"}` بصفر بيانات عمل (`:26-49`)؛ وثلاثة تمرير ضيّق موثَّق بالمصدر نفسه بأنّه «fallback ضيق وآمن… حين تُمرّر بيئات nginx/compose **قديمة** مساراتها إلى المنصّة» (`:105-109`). **نقلها إلى `/v1/` يُلغي الغرض الذي وُجِدت له**: العميل القديم الذي يستدعي `/api/indicators/readyz` سيتلقّى 404 — وهو عين الانقطاع الذي أُنشِئ الاسم البديل لمنعه. فهي ليست ديناً مؤجَّلاً بل **عقد دائم**، والتوصيف السابق («ديون توافقيّة مؤجَّلة عمداً») كان يوحي بعمل متبقٍّ لا وجود له. حُدِّث `measurement` في الأساس ليقولها صراحةً.
- **حدّ صدق: ما لم أفعله ولماذا.** الأربعة الصحّيّة تفوتها بوّابة `infra` لسبب تقنيّ بحت — `_classify()` يطابق `/health` بادئةً أو عضويّة **مضبوطة** في `{"/readyz", "/metrics", …}`، و`/api/…/readyz` **لاحقة لا بادئة**. إعادة تصنيفها كانت ستخفض ٧ ⇒ ٣ بسطر واحد، ودفاعها الدلاليّ قويّ (حمولتها صحّيّة خالصة) ولها نظير سابق (`/runtime-identity` أُعيد تصنيفه 2026-07-30). **لم أفعلها من طرفي:** نظير `/runtime-identity` كان مسنوداً بعقد مُجمَّد صريح مذكور في CLAUDE.md (`platform_route_placement_contract.json`) وهذه بلا نظير؛ وخفض عدّاد الدَّين بإعادة تصنيف بلا عقد يُقرأ **تحسينَ رقم لا تصحيحاً**، وهو بالضبط الصنف الذي بُنيت هذه المنظومة لكشفه. عُرِضت على المالك بحجّتها كاملةً من الطرفين وتُرِكت لقراره.
- **التحقّق:** `--check` أخضر على السقف ٧ والمجموعة المُجمَّدة السبعة (شرط المجموعة الذي أُضيف في #733 يعمل على القيمة الجديدة) · `verify_all_generated.py` ⇒ صفر انحراف بعد سلسلة التصحيح المعتادة لأيّ تعديل على الأساس (`capability_mapping_engine` ⇒ `pr_capability_impact_gate --generate-index` ⇒ إعادة بناء الحزمة) · `pytest -m unit`: **3773 نجح** (+١)، صفر فشل · `ruff` نظيف.
- **حالة الفجوة:** `API-VERSIONING-GUARD-IS-A-MIRROR-01` مكتملة موضوعيّاً — ٧٤٩ ⇒ ٧ عبر الجلسة، والسبعة الباقية عقود دائمة موثَّقة لا ديون. لا شريحة هجرة متبقّية.

## 2026-07-31 — عقد التوافق الدائم: legacy_unversioned_business ⇐ 0 (تصحيح توصيف لا هجرة)
- **المُحفِّز:** مراجعة المالك على #734 أقرّت النطاق والحكم (false positive مُزال · ٧ عقود مقصودة · migratable debt = ٠)، ورصدت بدقّة أنّ **شرطاً واحداً من ثمانية غير مستوفٍ**: `legacy migration debt = 0` صحيح تحليليّاً لكنّ الآلة ما زالت تُسمّي السبعة `legacy_unversioned_business`، فالتقارير تعرض «سبعة ديون» بينما الحقيقة «صفر ديون + سبعة عقود». وحدّدت المراجعة الشكل المطلوب: عقد مركزيّ صريح بـ**method + normalized path**، وفئتان مُسمّاتان، ومساواة مجموعات لا سقف عدديّ.
- **تصحيح لازم سجّلتُه أوّلاً:** المراجعة كُتبت بصيغة «بعد اكتمال CI… عند إحضار الحالة النهائيّة أتحقّق»، بينما **#734 كان مدموجاً بالفعل** عند وصولها — دُمِج بعد تحقّقي المباشر من ٦٤ فحصاً terminal (٦٣ success + skipped واحد بالتصميم). أبلغتُ ذلك صراحةً مع الـSHA النهائيّ (`025c726c`) بدل ترك انطباع بأنّه ما زال معلَّقاً.
- **تدقيق الشروط الثمانية على الشجرة المدموجة — تنفيذ حيّ لا ادّعاء:** سبعة مستوفاة، وواحد (`debt = 0`) غير مستوفٍ كما رصدت المراجعة. وأثناء التدقيق **كذّبتُ شرط الاستبدال أمام المالك**: حذف عقد وإدخال آخر عند عدد ثابت ٧ ⇒ رُفِض بالاسم — أي أنّ ملاحظة «`count <= 7` وحده يسمح بالاستبدال» صحيحة نظريّاً لكنّها كانت **مُعالَجة سلفاً** في #733 بشرط المجموعة المستقلّ.
- **ما نُفِّذ:** `docs/architecture/permanent_compatibility_contract.json` مصدراً قانونيّاً وحيداً. `_classify()` صار `(path, method=None)` — التوقيع متوافق رجعيّاً (الاستدعاءات القائمة بمسار وحده في الاختبارات تبقى صالحة)، والمطابقة تتطلّب `method` **بالتصميم لا بالسهو** لأنّ مفتاح العقد مركّب؛ وثّقتُ ذلك في docstring وأكّدتُه باختبار (`_classify("/api/agent/health")` ⇒ legacy، وبـ`"GET"` ⇒ الفئة الدائمة).
- **التطبيع اختيار مقصود:** كلّ معامل ⇐ `{}` فيُعرَّف شكل المسار لا تسمية معاملاته. البديل (مطابقة حرفيّة) أشدّ لكنّه يُنتِج إنذاراً كاذباً عند إعادة تسمية معامل تجميليّة؛ والتطبيع لا يفتح ثغرة لأنّ الشكل نفسه يبقى مقيَّداً بالـmethod والمسار الثابت.
- **الجرد الخام يبقى ظاهراً (مبدأ CLAUDE.md):** أُضيف حقل `permanent_compatibility_routes` إلى قائمة السماح المولَّدة، فيقرأ المراجع الفئتين ومحتواهما بلا فتح ملفّ العقد. العقود ليست ديناً لكنّها **ليست مخفيّة**.
- **الإنفاذ الثلاثيّ ومسار تكذيب كلٍّ منه:** (١) السقف ٠ — والاختبار يتجاوز الرفض السطحيّ: بوّابة الانحراف تسبق السقف، وعلاجها المُوثَّق «أعِد التوليد»، فالاختبار **يطبّق العلاج ثمّ يؤكّد الفشل المستمرّ على `0 ⇒ 1`**، وهو البرهان الحقيقيّ أنّ إعادة التوليد لا تُبيّض مساراً جديداً. (٢) adjudication إلزاميّ — مِسبار على شكل alias صحّيّ رُفِض. (٣) إنفاذ عكسيّ لعقد بائت — إدخال شبح رُفِض بالاسم.
- **فشل حقيقيّ كشفه التشغيل وأُصلِح بإعادة بناء لا بتخفيف:** اختبار المجموعة من #733 سقط بعد أن صارت مجموعة legacy فارغة — الشرط لم يعد قابل الوصول عبر بيانات حقيقيّة. **لم أحذفه ولم أُرخِّه**؛ أعدتُ بناءه ليصنع الحالة صناعيّاً (سقف يقبل العدد ١≤١ + مجموعة لا تحوي المسار الحيّ) فيبقى الثابت محروساً ليوم يعود فيه دَين، مع تأكيد صريح أنّ الرفض جاء من المجموعة لا العدد (`assert "نمت" not in output`).
- **الحدّ المعلَّق كما هو:** إعادة تصنيف الأربعة الصحّيّة إلى `infra` **لم تُنفَّذ** — موثَّقة في حقل `not_infra` داخل العقد نفسه بوصفها قراراً معماريّاً مستقلّاً. الفئتان الجديدتان لا تُعفيان المسار من الظهور، فالفرق عن إعادة التصنيف المرفوضة جوهريّ لا لفظيّ.
- **التحقّق:** `pytest -m unit`: **3776 نجح** (+٣)، صفر فشل · `verify_all_generated.py` ⇒ صفر انحراف · `ruff` نظيف · العدّاد الحيّ بلا `legacy_unversioned_business` إطلاقاً.

## 2026-07-31 — P0-1: حارس منع اكتمال القالب (إغلاق المسار الوحيد للنجاح غير الحقيقيّ)
- **المُحفِّز:** مراجعة خارجيّة قِيست على الشجرة رتّبت الأولويّات، ورجّحت هذه الشريحة على «PR جرد فقط» بحجّة أنّ الجرد يؤجّل الإغلاق بينما الحارس يشتريه فوراً بكلفة أصغر. صادقتُ على الترتيب بعد قياسي الخاصّ، وأضفتُ سبباً لم تذكره: الحارس **قابل للتكذيب مباشرةً** بينما الجرد يوثّق ولا يمنع.
- **البرهان قبل اللمس:** مسحتُ `core/workflow_engine.py` كلّه — **صفر** `result.get(...)`/`result[...]`. فالمحرّك لا يملك ما يميّز به القالب من الحقيقيّ، والاكتمال دالّة في «هل رمى المعالِج؟» وحدها. ومسحتُ قراءات `_template` عبر المستودع: خمس، كلّها في الاختبارات، **صفر** في الإنتاج. فالوسم ميت لا ضعيف — وهذا يرفع الفجوة من «إشارة غير كافية» إلى «لا إشارة».
- **قياس نطاق الأثر قبل تغيير سلوك محرّك إنتاجيّ:** بحث شامل عن مستهلك حيّ لسير الريّ خارج ملفّ التعريف ⇒ **صفر**. وفحص الاختبارات ⇒ لا اختبار يؤكّد اكتمال القالب عبر المحرّك (الاختبار الوحيد المعنيّ يؤكّد أنّ **الدالّة تُعيد** القالب، لا أنّه يكتمل). فالمخاطرة محصورة.
- **التصميم — أرخص إصلاح صحيح:** `_reject_template_result()` داخل `try` في كلا المسارين (713 متزامن، 844 async). وضعه **داخل** `try` مقصود: يرث `FAILED` + تعويض Saga + قابليّة الاستئناف من المسار القائم بلا مضاعفة منطق ولا حالة جديدة. الفحص عامّ (أيّ معالِج يَسِم `_template=True`) لا خاصّ بالريّ.
- **التكذيب:** إزالة الحارس ⇒ **٤ من ٥** تسقط، والخامس يبقى أخضر عمداً (`test_real_result_still_completes`) فيُثبِت التمييز لا الإفشال الشامل. ومخرَج الفشل يعرض العلّة حرفيّاً: `status=COMPLETED` لسير عمل قالبيّ بالكامل.
- **الاكتشاف الأهمّ أثناء التنفيذ — اختبار يحرس الكذبة:** تشغيل مجموعة المنصّة بعد الحارس أسقط اختباراً واحداً: `test_end_to_end_built_steps_run_to_completion`، وهو يؤكّد أنّ دورة ريّ من **أربعة قوالب لا تفعل شيئاً** تبلغ `COMPLETED` بخطواتها الأربع. أي أنّ الكذبة لم تكن مُهمَلة بل **محروسة**: أيّ إصلاح كان سيُكسِر «اختباراً ناجحاً» فيبدو الإصلاح هو العطل. أعدتُ بناءه ليؤكّد الحقيقة (عدم الاكتمال + تسمية السبب) مع إحالة إلى الاختبار الذي يغطّي المسار الحقيقيّ — ولم أحذفه ولم أُرخِّ الحارس.
- **حدّ صدق صريح:** توثيق العلم يَعِد «صفر كسر على السلوك القائم» عند الإطفاء. هذا الحارس **ينقض ذلك عمداً**، لأنّ السلوك القائم كان نجاحاً زائفاً وضمانه ضمانٌ للكذبة. سجّلتُ هذا في الفجوة صراحةً بدل تمريره بصمت.
- **التحقّق:** `pytest -m unit`: 3776 (ملفّي خارج `testpaths=tests_v9` بالتصميم) · مجموعة المنصّة عبر أمر وظيفة `Platform Unit Tests` حرفيّاً: **4014 نجحت** صفر فشل · `ruff` نظيف. (`tests/test_geo_import.py` يفشل جمعه محليّاً لنقص `defusedxml` — مُتحقَّق أنّه سابق لتغييري بـ`git stash`، وCI يثبّت التبعيّة.)

## 2026-07-31 — إغلاق دَينَين مُسجَّلَين: الخضرة الزائفة + قرار infra
- **الأمر:** «قم بإغلاقهما» — الدَّينان اللذان سجّلتُهما في جرد المتبقّي ولم يُغلَقا.
- **الدَّين (١) الخضرة الزائفة — TESTS-PASS-WITHOUT-ASSERTING-01:** نشأ من ملاحظة مراجعة خارجيّة، وتحقّقتُ منه بالقياس: `test_roadmap_phase23.py` يجمع ١٤٣ اختباراً و**واحداً** فقط يحوي `assert`. ثمّ ذهبتُ أبعد من عدّ الدوالّ: شغّلتُ الدوالّ وقرأتُ حمولاتها (٦٨٣ علامة) فكشفتُ **✗ حقيقيّتين تحت خضرة تامّة** — `فجوات تبعيّات: ['decision-service:redis']` و`11 silent متبقٍّ` بمصادر `path:line`. أي أنّ العلّة ليست نظريّة: نتائج حقيقيّة كانت مخفيّة خلف «نجاح».
- **اختيار قاعدة الكشف — قياس قبل ترميز:** قِستُ نمطين. **٢٠٧** دالّة بلا `assert`/`raises` **وتُرجِع قيمة** (قاطع — pytest نفسه يحذّر منه)، و**٧٤** بلا تأكيد ولا إرجاع (قد تكون دخاناً مشروعاً «لا ينهار»). اخترتُ الأوّل وحده: **صفر إيجابيّة كاذبة أهمّ من شمول أوسع** في حارس سيحجب الدمج. و`pytest.raises` يُعَدّ تأكيداً صحيحاً وإن لم يكن `assert` نصّاً.
- **الراتشِت على المجموعة لا العدد** (تطبيق مباشر لدرس #733): إدخال جديد يُسقِط CI **بالاسم** حتى لو انخفض العدد الكلّي — فلا يُستبدَل دَين بدَين. مُكذَّب: إسقاط إدخال من الأساس يُسقِط الفحص مسمّياً الدالّة، والاستعادة تُعيده أخضر.
- **عثرة تصميم أصلحتُها جذريّاً لا سطحيّاً:** سمّيتُ السكربت أوّلاً `scripts/ci/test_assertion_presence_guard.py`، فاصطدم اسمه باسم ملفّ الاختبار في `tests_v9/` — فاستورد الاختبارُ **نفسَه** بدل السكربت وسقط بـ`AttributeError`. الحلّ السطحيّ كان alias عبر importlib؛ الصحيح إزالة السبب: بادئة `test_` في `scripts/` مُضلِّلة أصلاً (توحي بأنّه اختبار وقد يجمعه pytest). أُعيدت التسمية إلى `assertion_presence_guard.py` والاختبار إلى `test_fake_green_test_guard.py`.
- **حدّ صدق صريح:** الـ٢٠٧ **لم تُصلَح**. إصلاحها تحويل كلّ دالّة إلى `assert` حقيقيّ — عمل منفصل كبير. الحارس **يمنع النموّ** ويُبقي الدَّين مرئيّاً مُعدّاً، ولا يدّعي إغلاق العلّة. والنتيجتان الحقيقيّتان المكشوفتان تستحقّان معالجة مستقلّة (سُجِّلتا في الفجوة).
- **الدَّين (٢) قرار `infra` — أُغلِق بـNO مُسجَّل لا بتنفيذ:** طلب المستخدم إغلاقهما، والإغلاق الصادق هنا **قرار موثَّق** لا تنفيذ آليّ. السؤال («أنُعيد تصنيف الأربعة الصحّيّة إلى infra؟ ٧ ⇒ ٣») طُرِح حين كانت الأربعة مُصنَّفة `legacy_unversioned_business` — فبدا النقل تصحيحاً يخفض عدّاد دَين. لكنّ عقد التوافق الدائم (#735) **ألغى المقدّمة**: صارت في فئة مُسمّاة صريحة مسنودة بعقد بمفتاح method+مسار مُطبَّع، ومحروسة بإنفاذ عكسيّ، ولا تُضاف إليها واحدة بلا adjudication. فالنقل الآن **يخسر معلومة ولا يكسب دقّة**: يذوّبها في دلو `infra` العامّ (١٠١ مساراً) فتفقد اسمها وعقدها وحارسها، مقابل صفر مكسب — العدّاد الذي كان يُقلِق صار صفراً أصلاً. الدقّة الدلاليّة تحقّقت **بالتسمية لا بالإخفاء**. سُجِّل القرار في `infra_reclassification_decision` مع **شرط نقض صريح** (إن ظهر مستهلك يحتاجها infra فعليّاً — كاستثنائها من ميزانية مسارات النطاق — يُعاد الفتح بعقد موضع صريح لا بتعديل `_classify` مباشرةً).
- **التحقّق:** `assertion_presence_guard --check` أخضر (207/207) · `api_versioning_policy_guard --check` أخضر (العقد ما زال متّسقاً بعد إضافة حقل القرار) · اختبارات الحارس الأربعة تمرّ.

## 2026-07-31 (تكملة) — تنقيح #737 بمراجعة المالك: فصل التصنيف + فجوتان مستقلّتان + شرط نقض أدقّ
- **(١) فصل التصنيف — تصحيح لغويّ له أثر حقيقيّ:** كنتُ قد سجّلتُ فجوة واحدة (`TESTS-PASS-WITHOUT-ASSERTING-01`) بحالة «مرصودة ومُجمَّدة»، وهي صياغة تُخفي أنّ شيئين مختلفين حدثا: **النموّ أُغلِق** فعلاً (لا دالّة جديدة تمرّ)، بينما **الدَّين القائم مفتوح** (٢٠٧ لم يُصلَح منها شيء). الحالة الواحدة كانت ستُقرأ لاحقاً «مُعالَجة». فُصِلت إلى `PYTEST-NONASSERTING-GROWTH-01` (CLOSED) و`PYTEST-NONASSERTING-EXISTING-DEBT-01` (OPEN, baseline 207) بشرط إغلاق صريح للثانية (الأساس ⇐ صفر).
- **(٢) الإخفاقان صارا فجوتين مستقلّتين لا دليلاً داخل الحارس:** كانا مذكورَين كـ«دليل على أنّ العلّة حقيقيّة»، وهذا يدفنهما — من يقرأ فجوة الاختبارات لن يبحث فيها عن عيب في raster أو decision-service. سُجِّلا الآن مستقلّين بشرط إغلاق لكلّ منهما.
- **استخراج المصادر كاملةً قبل التسجيل:** رسالة `test_exception_hygiene` تسمّي **٣ من ١١** فقط (`prod_silent[:3]`) — فتسجيل الفجوة بها كان سيُنتِج فجوة بمصادر ناقصة. أعدتُ تشغيل منطق الكشف نفسه واستخرجتُ الأحد عشر بمساراتها الكاملة (agriai/raster×5/sahool-platform×3/soil-service). الفجوة بلا مصدر كامل تُخالف قاعدة CLAUDE.md صراحةً.
- **(٣) شرط النقض أُعيدت صياغته بصيغة المالك — وهي أدقّ من صيغتي:** كتبتُ أوّلاً شرطاً واحداً فضفاضاً («إن ظهر مستهلك يحتاجها infra»). صيغة المالك تربط النقض بـ**انهيار الأساس الذي بُني عليه الرفض** لا بظهور رغبة: (أ) لم تعد محكومة بعقد مستقلّ · (ب) لم تعد الفئة shrink/frozen · (ج) دخل مسار خامس بلا adjudication. أي: الرفض صحيح **ما دامت التسمية محكومة**؛ فإن سقط الحُكم سقط مبرّر الرفض. اعتُمِدت حرفيّاً.
- **التحقّق:** `api_versioning_policy_guard --check` أخضر بعد تعديل العقد · `brain_deferral_registry_guard` أخضر · JSON صالح.

## 2026-07-31 — P0-5: حذف نسخ مهارات supervisor-agent الجذريّة الميتة (584 سطراً)
- **الأمر:** «ابدأ» بعد ترتيب معتمد من المالك، بشرطه الصريح: **إثبات reachability كامل عبر الشجرة كلّها** لا الاعتماد على `main.py` وحده، ثمّ حذف مباشر بلا wrappers إن بقيت النتيجة صفراً.
- **الشرط كان محقّاً — والاعتماد على `main.py` وحده كان سيُسقِط CI:** البحث الشامل أظهر صفر مستهلك **تنفيذيّ** (صفر استيراد جذريّ/ديناميكيّ، صفر `__main__`، entrypoint `uvicorn main:app` يمرّ عبر `skills/`)، **لكنّ ثلاثة مستهلكين يقرؤون الملفّ الجذريّ نصّاً** عبر `(ROOT / rel).read_text()`: حارس CI (`intelligence_governance_gate.py`) واختباران. حذف الملفّ قبل تقليم قوائمهم كان يعني `FileNotFoundError` في CI. قُلِّمت الثلاث **قبل** الحذف لا بعده.
- **اكتشاف رابع:** `legacy_quarantine_allowlist.json` كان يحجر نسخة الجذر من `advisory_skill` — دليل مستقلّ أنّها معروفة كـlegacy، وسطر وجب حذفه مع الملفّ (وإلّا صار الحجر يشير لملفّ غير موجود).
- **تصحيح لكلامي السابق أُبلِغ به المالك:** كنتُ قد صادقتُ على بند المراجعة «لا حارس CI يمنع تجاوز الوكيل إلى actuator» بعد بحث **بالاسم** في `scripts/ci/`. البحث بالمحتوى أثناء هذه الشريحة كشف أنّ الحارس **موجود جزئيّاً** داخل `intelligence_governance_gate.py:23-31` — يمسح `ai_agronomist`/`agriai-engine`/`supervisor-agent` بحثاً عن `actuator_service_url`/`sahool-actuator`/`mqtt.publish(`. فـP0-7 يصير **توسيع حارس قائم** لا بناءً جديداً. الدرس: البحث بالاسم ليس بحثاً.
- **العلاج كما اشترط المالك:** حذف مباشر للأربعة (٥٨٤ سطراً)، **بلا wrappers وبلا نقل الفروق الـ١٢٣ سطراً** من `advisory_skill`. الكود الميت لا يصير قدرة مطلوبة لأنّه أطول؛ ولو كان في الفرق قيمة لاحتاج إثبات مستهلك أو عقد أو اختبار — ولا شيء منها قائم.
- **حارس منع العودة + تكذيب ثلاثيّ:** ملفّ يعود للجذر ⇒ فشل مُسمّى · استيراد جذريّ عارٍ ⇒ فشل يسمّي الملفّ والسطر · حذف مهارة حيّة ⇒ فشل مُسمّى (الحارس يحمي الحيّ لا الميت فقط). الاستعادة خضراء في الثلاثة.
- **سبع دورات توليد — أطول سلسلة في الجلسة:** الحذف حرّك جرد الخدمات ورسم النَّسَب والرسم المعماريّ وتدقيق الاعتماديّات وعقود التشغيل وحزمة الإصدار والإغلاقين الساكن والتكامليّ. وأثناءها ظهرت رسالة مفيدة من المكنسة: «حارس كتب أثناء الفحص — الشجرة تغيّرت بين بدايته ونهايته» أي انحراف أُصلِح صامتاً وضاع دليله؛ عولج بإعادة التوليد بالترتيب حتى استقرّ.
- **التحقّق:** `tests/` **611 نجحت صفر فشل** (كانت ٧ إخفاقات قبل إعادة التوليد — كلّها مصنوعات) · `pytest -m unit` **3780** · `intelligence_governance_gate` ⇒ `ok` · `verify_all_generated` ⇒ صفر انحراف.

## 2026-07-31 — P0-7: حدّ الأثر الفيزيائيّ (BRAIN-ACTUATOR-BYPASS-UNGUARDED-01)
- **الأمر:** «تحقّق وعمل اللازم» — بعد دمج #737 و#738، والتالي في الترتيب المعتمَد هو P0-7.
- **قِستُ قبل أن أبني (درس #738 مطبَّقاً):** بحثتُ بالمحتوى لا بالاسم، فوجدتُ `decision_candidate_boundary_gate.py` قائماً — لكنّه محصور في **ملفّين مُسمّيين** (`crop_decision_bridge.py` · `crop_twin.py`) ولا يمسّ مناطق الدماغ. فالفجوة حقيقيّة، والحارس الجزئيّ في `intelligence_governance_gate.py:24-32` يغطّي المسار الأصرح وحده.
- **الجرد الحيّ قبل التصميم:** ٢٣ ملفّاً يحمل رموز الأثر، أغلبها **حرّاس آخرون** يذكرون الرمز كي يفحصوه — فاستثناء `scripts/` قرارٌ مقصود: حارس يُسقِط حارساً إيجابيّة كاذبة بالبناء.
- **ثلاث حالات مشروعة كانت ستسقط بقاعدة ساذجة:** docstring في `ai_evidence_runtime.py:136` · قراءة تدقيقيّة `GET /v1/agent/actuator-audit` · تصنيف `SideEffectClass.ACTUATOR` في `tool_contracts.py`. تبنّيتُ تجريد docstrings بالـAST من `decision_candidate_boundary_gate.py` — نمط قائم في المستودع لا اختراع.
- **قاعدتان لا واحدة:** الإطلاق مقيَّد عالميّاً بقائمة سماح مُعلَّلة؛ البلوغ بالاستيراد مقيَّد داخل مناطق الدماغ وحدها. توحيدهما كان سيُغرِق المنصّة بضجيج (استيراد وحدات التوزيع داخلها سلوك طبيعيّ).
- **قائمة السماح بالقياس لا بالتوقّع:** توقّعتُ أربع فئات فأعطى القياس **إدخالين**. منفذ طلبات التنفيذ في `decision-service` **للقراءة فقط** (تغذية اكتشاف)، و`dispatch_executor` يُدرِج ولا يُطلِق، و`actuator_command` نقيّ. سجّلتُ الثلاثة في `not_in_allowlist_by_measurement` كي لا يُقرأ غيابها سهواً.
- **التكذيب كشف ثغرة في الحارس نفسه:** بنيتُ المسارات الأربعة **كملفّات حقيقيّة في مناطق حقيقيّة**، فمرّ الرابع: `from api import phase_runtime_workers` يضع الاسم بعد `import` بينما تعبيري النمطيّ يفحص ما بعد `from`. أصلحتُه **جذريّاً** بتحليل الاستيرادات بالـAST (يلتقط `import a.b`, `from a import b`, والأسماء المستعارة) لا بترقيع النمط. لولا التكذيب بملفّ حقيقيّ لمرّت الثغرة — المقتطف الصناعيّ وحده لم يكن ليكشفها.
- **مصدر حقيقة واحد:** حُذِفت القاعدة الجزئيّة من `intelligence_governance_gate.py` بعد التغطية الكاملة؛ نسختان لقاعدة واحدة مصدرا انحراف صامت.
- **حدّ صدق:** صفر خرق اليوم. الحارس **يمنع الانحدار ولا يُصلِح عطلاً**، والتمييز مكتوب في `state_at_adjudication` داخل العقد وفي سجلّ الفجوات — كي لا يُقرأ الإغلاق إصلاحاً.

## 2026-07-31 — الاستيراد المؤجَّل غير المُعلَن + إغلاق فجوة redis
- **الأمر:** «استكمل المهام بشكل متتابع… وإذا هناك مهام منفصلة يمكنك عملها بعدد من الوكلاء بشكل متوازي».
- **التوازي كان للاستقصاء لا للتحرير:** ثلاثة وكلاء **للقراءة فقط** على مسارات مستقلّة. وكلاء يحرّرون شجرة عمل واحدة كانوا سيتصادمون تماماً كما تصادمت مجموعتا pytest عندي قبل ساعة — فالتنفيذ بقي متتابعاً بيدي.
- **تحقّقتُ من ادّعاءات الوكلاء قبل البناء** (لا أخذها كما هي): قرأتُ `requirements.txt` و`main.py:225` و`__init__.py` بنفسي. الثلاثة صحّت.
- **الاكتشاف أكبر من الفجوة المُسجَّلة:** الفجوة كانت «decision-service:redis». إصلاح **جذر** الحارس (AST بدل تعبير مرتكز على العمود صفر + إضافة redis للخريطة) أظهر **ثمانية** مواضع في **خمس** خدمات. الفجوة المُسجَّلة كانت عيّنة من صنف، لا حالة مفردة.
- **صنّفتُ بقراءة كلّ موضع لا بالنمط:** `field_boundary_backends` يسقط لمستطيل نقيّ · `bivariate_raster` يرمي `RuntimeError("Pillow مطلوب — pip install pillow")` صريحاً · `gis_geometry_guard` يعيد المُدخَل مُطبَّعاً — ثلاثتها اختياريّة صادقة. بينما `zones_kmeans:88` **استيراد عارٍ بلا معالِج إطلاقاً**، و`terrain_source_registry:63` معالِجه لا يلتقط ImportError، و`agent_stores:138` يرفع RuntimeError إنتاجاً. الفرق فرق سلوك لا فرق أسلوب.
- **رفضتُ الإعلان الجَماعيّ:** `CLAUDE.md` يوجب pip-audit قبل أيّ إضافة تبعيّة، وهو غير متاح هنا؛ وscikit-learn قرار وزن صورة. فجُمِّدت الثلاث بشرط إغلاق مكتوب لكلٍّ، بدل إضافة ثلاث حزم ثقيلة بلا الفحص الذي يفرضه المستودع.
- **الإصلاح الوحيد المُنفَّذ شقّان لا شقّ:** إعلان redis **وفصل** `ModuleNotFoundError` عن تعذّر الاتّصال — لأنّ الشقّ الثاني هو الضرر الحقيقيّ (مشغّل يُرسَل إلى Redis سليم). وأثبتُّه بالكاشف نفسه: ٨ ⇒ ٧.
- **درس تغطية:** `test_worker_identity_binding.py:185-195` كان يمرّ سواء أُثبِّتت redis أم لا — كلا العطلين يعطي 503 نفسه. اختبار يمرّ في الحالتين لا يغطّي أيّاً منهما.
- **مُكذَّب ثلاثاً** بإسقاط إدخال · إدخال بائت · استيراد مؤجَّل جديد حقيقيّ في `services/auth` — كلٌّ فشل بالاسم والموضع، والاستعادة أعادت الأخضر.

## 2026-07-31 — تشخيص المكنسة: تصحيح ذاتيّ قبل البناء
- **الأمر:** «استكمل» ضمن ترتيب معتمد يضع إصلاح المكنسة **قبل** دورة المعالجات الصامتة، لأنّه يخفض كلفة كلّ شريحة لاحقة.
- **قرأتُ المصدر قبل أن أُصلِح ما وصفتُه — فتبيّن أنّ وصفي خاطئ.** كنتُ قلتُ للمالك إنّ المكنسة «تفترض `--generate`». الحقيقة أنّ `_GENERATE_FLAG` خريطة صريحة تحمل `--apply` و`--write-registry`، و`regenerate()` تضع غير المُدرَج في `manual` بلا تخمين. أي أنّ الخيار الذي اقترحه المالك ثانياً (metadata صريحة) **هو التصميم القائم**، والخيار الأوّل (توحيد الأعلام) كان سيغيّر ثلاث واجهات عموميّة بلا داعٍ. أبلغتُه بالتصحيح قبل كتابة سطر.
- **العطل الثاني ألطف وأخطر:** طباعة «فُحِصت ولا تُولَّد آليّاً» موجودة ومقصودة لهذه الحالة بالضبط — لكنّها بعد الحلقة، ومسار الفشل يعود `return 1` قبلها. المعلومة محسوبة ثمّ مرميّة. عيب **ترتيب** لا عيب منطق.
- **قِستُ قبل أن أعمّم:** ٣٨ سكربتاً في قائمة اليدويّ، **٢٣ تُعلن علم كتابة**. فالثلاثة التي اصطدمتُ بها لم تكن استثناءً بل عيّنة — الخريطة تغطّي أقلّ من نصف ما تستطيع.
- **ورفضتُ إضافة العشرين الباقية:** استدعاء علم كتابة آليّاً يفترض أنّه إعادة توليد نقيّة، و`--apply` في `capability_linker` يطبّق مرشّحات فوق عتبة. الإضافة الجماعيّة كانت ستشغّل عشرين كاتباً بلا مراجعة دلاليّة — سجّلتُها قراراً مستقلّاً بدل تنفيذها صامتاً.
- **التكذيب الثاني هو الأهمّ:** ربط `capability_linker` بـ`--generate` (وهو لا يُعلنه) يُسقِط CI باسم عدم المطابقة — أي أنّ الحارس يمنع تحديداً الحلّ الخاطئ الذي كنتُ سأقترحه لو لم أقرأ المصدر.
- **درس تشغيليّ مكلّف تكرّر أربع مرّات اليوم:** عمليّتان تلمسان شجرة العمل معاً. آخرها `--fix` في الخلفيّة يكتب بينما فحص للقراءة يعمل، فقرأتُ سبعة ملفّات «منحرفة» وهي نتاج عمليّتي أنا. أوقفتُ الاثنتين وأعدتُ التشغيل واحدةً. القاعدة الآن: **عمليّة واحدة تلمس الشجرة، ويُقرأ رمز خروجها لا مخرَجها الجزئيّ.**

## 2026-08-01 — تسجيل نتائج مقيسة قبل ضياعها
- **الأمر:** «استمر» بعد تأجيل Node 20؛ والتسجيل واجب بعقد `CLAUDE.md` نفسه («لا فجوة بلا مصدر + حالة»).
- **لماذا التسجيل شريحة مستقلّة:** النتائج **قياس** لا تغيير كود. دمجها في شريحة CDSE كان سيخلط «ما قِسناه» بـ«ما غيّرناه»، ويجعل التراجع عن أحدهما تراجعاً عن الآخر.
- **الخطر الحقيقيّ:** الفجوات الخمس كانت موجودة **في المحادثة وحدها**. الحاوية زائلة، والجلسة تنتهي — فكلّ ما لم يُكتَب يضيع كأنّه لم يُقَس. وهذا بالضبط صنف العطل الذي أغلقته الجلسة مراراً (ادّعاء بلا مصدر، فحص يمرّ بلا تأكيد، توثيق بائت).
- **فصّلتُ خمساً لا واحدة** لأنّ إصلاح أيّها لا يُغني عن الآخر: السباق غير المتزامن · العلم المُطفأ · غياب MSI عن القوائم · الادّعاءات البائتة · والحكم الجامع. دمجها كان سيُنتج «فجوة طيف» تُغلَق بإصلاح ثُلثها.
- **تحقّقتُ من ادّعاءات الوكيل بنفسي قبل التسجيل** — قرأتُ `processing.py:154-162` و`imagery_automation.py:610-625` و`grep` للعلم عبر ملفّات النشر. الأربعة صحّت.
- **ولم أُصلح السباق:** تغيير توقيت كتابة صفوف سلوكيّ، ويستحقّ شريحة بقرار المالك لا دسّاً في تنظيف — القاعدة نفسها التي فصلت PR-A عن PR-B.
- **Node 20 سُجِّل مؤجَّلاً بمُحفِّز إعادة فتح مكتوب**، لا محذوفاً: التأجيل قرار يُراجَع، والحذف نسيان.

## 2026-08-01 — إغلاق `SILENT-EXCEPTION-HANDLERS-11-01` بقياس على `main` النهائيّ
- **الأمر:** «تحقّق» ثمّ الدمج؛ والشرط المُعلَن من المالك: **«إعادة القياس على `main` النهائيّ بعد دمج الفرعين، لا جمع الأرقام حسابيّاً فقط.»**
- **ما فعلتُه:** دمجتُ #746 بعد التأكّد من أنّ **٧٠ فحصاً** كلّها منتهية وخضراء (تخطّيان بالتصميم: `runtime-stack-e2e-chaos` و`docker-build`)، ثمّ حدّثتُ `main` محلّيّاً إلى `a545d6e3` وأعدتُ تشغيل **قاعدة الكاشف نفسها** (`tests_v9/test_roadmap_phase23.py:4707`) على الشجرة ⇒ **صفر**.
- **لماذا القياس لا الجمع:** ١١ ⇒ ٥ ⇒ ٣ ⇒ ١ ⇒ ٠ أرقامٌ صحيحة كلٌّ في لحظته، لكنّ جمعها يفترض أنّ الشجرة لم تكتسب موضعاً جديداً بين الشرائح — وهو افتراض لا يُقاس إلّا بإعادة التشغيل على الشجرة النهائيّة. الفرق بين «أثبتُّ» و«حسبتُ».
- **بانر تعارض بائت:** لافتة التعارض التي وصلتني لـ#746 سبقت إعادة الأساس التي دفعتُها؛ الحالة الفعليّة وقت الدمج كانت `clean`. اللافتة المنسوخة ليست قياساً — الاستعلام هو.
- **ما بقي مفتوحاً بعد الإغلاق ولا يُخفيه:** سياسة `missing` في CDSE مؤقّتة بالتصميم («اقبل، قِس، ثمّ شدّد») وعدّاداتها الثلاثة مُعلَنة على `/metrics` كي لا تصير المؤقّتة دائمةً بالصمت.

## 2026-08-01 — جرد «ما المتبقّي» كشف فجوتين غير مسجَّلتين
- **الأمر:** «ماهو المتبقّي» ثمّ «ادمج ثمّ سجّل الفجوتين».
- **كيف ظهرتا:** لم أُجب من ذاكرة المحادثة. شغّلتُ `grep -rn "LOCAL-RASTER-UNKNOWN-WORKTREE|CANONICAL-SNAPSHOT-ELIGIBILITY" sahool-brain/ docs/` ⇒ **لا نتيجة**، بينما `git branch -a --list '*wip*'` يُظهر `wip/raster-quarantine-20260801` موجوداً **ومدفوعاً**. أي أنّ الأثر الفيزيائيّ قائم والوصف غائب.
- **المفارقة التي تستحقّ التسجيل:** هذا هو **نفس** صنف العطل الذي وُجِدت #747 لإغلاقه — نتيجة مقيسة تعيش في محادثة وحدها. وقعتُ فيه وأنا أُغلِقه، ولم يكشفه إلّا سؤال المالك عن المتبقّي.
- **لماذا لم أضِفهما إلى #747:** كانت فحوصها تعمل (٦٤ فحصاً). أيّ دفعة جديدة تُبطِل التشغيل كاملاً وتُعيده من الصفر — فذهبتا إلى شريحة تالية بعد الدمج.
- **`LOCAL-RASTER-UNKNOWN-WORKTREE`:** سُجِّلت بأساسها الحقيقيّ (`b3af02a7` = دمج #724) لا بـ`main`، لأنّ إعادة الأساس كانت ستوجب **اختراع محتوى داخل عمل مجهول المؤلّف**. وسُجِّل قرار المالك حرفيّاً: الاحمرار جزء من الوصف الصادق، ولا استثناء لـ`wip/**` من CI. وشرط الإغلاق قرار مالك لا عمل هندسيّ — **وحذف الفرع قبل البتّ فقدانٌ لا إغلاق**.
- **`CANONICAL-SNAPSHOT-ELIGIBILITY-POLICY-01`:** أسندتُ كلّ ادّعاء إلى موضعه قبل الكتابة — الاشتقاق في `generate_indicator_artifacts.py:58-59`، والاستهلاك في `indicator_registry.py:69,77`، وكون اللقطة معنونة بمحتواها في `decision-service/main.py:332` وإلغاء التكرار على `(tenant_id, snapshot_hash)` في `persistence.py:4473`. القيد الأخير هو ما يجعل «أضِف حقلاً للقطة» حلّاً خاطئاً: يُغيّر كلّ هاش قائم ويكسر إعادة التشغيل.
- **بقاؤها P1 مُبرَّر لا افتراضيّ:** البيان يسافر إلى `decision-service` كـ`feature_manifest_id/hash` فقط، وإلى `agriai-engine` الذي يفحص حضور `manifest_id/version` لا الأهليّة ⇒ لا مستهلك تنفيذيّ خطر. ومُحفِّز الترفيع إلى P0 مكتوب: إثبات أنّ `decision_eligible` يسمح اليوم بتنفيذ غير آمن.

## 2026-08-01 — إصلاح السباق: انتظِر الدفعة قبل قراءتها
- **الأمر:** «اخضر ادمج ثمّ ابدأ بإصلاح السباق» — الإذن الصريح المطلوب، لأنّ الإصلاح **سلوكيّ** يمسّ توقيت كتابة صفوف.
- **قرأتُ المسار كاملاً قبل أن أكتب سطراً:** `routers/processing.py:154` (`add_task` ثمّ `pending`) · `raster_job_orchestration.py:442-451` (المهامّ الفرعيّة تُنشَأ **داخل** العمل الخلفيّ) · `routers/jobs.py:41-44` (404/409) · `imagery_automation.py:610-625` (قراءة فوريّة). القراءة هي التي حسمت **أين** يُصلَح: خطأ التوقيت في المستهلك لا في المنتِج، فلا داعي لتغيير عقد raster-service.
- **`/result` لا يصلح للانتظار** — يردّ 409 حتّى الاكتمال فلا يميّز «لم تنتهِ» من «انتهت فارغة». فأُضيف `get_job_status` على `/v1/jobs/{id}`. اختيار نقطة الاستطلاع الخاطئة كان سيُنتِج انتظاراً يبدو صحيحاً ويقرأ إشارةً مبهمة.
- **ثلاث نهايات لا نهايتان:** أهمّها فصل **المجهولة (404)** عن **المهلة**. الأولى عطل نشر (حالة مهامّ بالذاكرة موزَّعة على نسختين) والزمن لا يُصلحها؛ خلطها بالمهلة كان سيجعل مشغّلاً يرفع المهلة إلى الأبد بحثاً عن حلّ لا وجود له. والمجهولة تتوقّف **فوراً** بلا استهلاك ميزانيّة.
- **`processed_unpublished` كادت تُنسى:** لولا قراءة `raster_persistence_policy.terminal_status` لَما دخلت مجموعة النهائيّات، ولكان الإصلاح يُنتِج مهلةً في **كلّ** دورة تعمل بوضع الإدامة أفضل-جهد — أي يعمل في الاختبار ويفشل في التشغيل.
- **عيب ثانٍ اكتُشِف أثناء التنفيذ لا في الاستقصاء:** `_fetch_index_mean` كان يترك 409 ينتشر، و`_collect_spectral_values` يقرأ NDMI ثمّ MSI **بالتتابع** — فمؤشّر فاشل واحد يُضيع الناجح بعده. صار 409 ⇒ `None`.
- **الرؤية جزء من الإصلاح:** ثلاثة عدّادات في `status()` المنشور عبر `routers/automation.py:152`. انتظار ينتهي بلا نتيجة **بصمت** كان سيُعيد إنتاج العطل نفسه بشكل آخر — وهو الدرس المتكرّر لهذه الجلسة كلّها.
- **التكذيب أوّلاً لا الادّعاء:** حذف نداء الانتظار ⇒ **٨ من ٩** تسقط، والناجية الوحيدة هي اختبار ميزانيّة الصفر (وصف العطل). وتعطيل فرع 409 ⇒ يسقط اختبار واحد بعينه. القفلان مُثبَتان كلٌّ على حدة.
- **التحقّق:** `pytest -m unit` **3874 نجحت / صفر فشل** · اختبارات المنصّة **4014** (استُثني `test_geo_import.py`: `defusedxml` غير مثبَّت في بيئتي محليّاً — نقص بيئة سابق لا علاقة له بالتغيير، وCI يثبّته) · `ruff` نظيف.
- **درس المكنسة تكرّر:** `--fix` وحدها لم تكفِ — أربعة مولّدات في دلو «اليدويّ» (`compose_runtime_target_resolver` · `path3_runtime_readiness_closure` · `runtime_evidence_ingestion` · `integration_runtime_governance_closure`) احتاجت `--generate` صريحاً **بترتيب**، ثمّ `--fix` مرّةً أخرى لإعادة بناء `FILE_CHECKSUMS`. الحكم بالتشغيل للقراءة فقط: `EXIT=0` و«5246 checksums verified».
- **ما لم يُدَّع:** لا برهان تشغيليّ حيّ بأنّ الأعمدة الثلاثة تُكتَب في بيئة حقيقيّة ⇒ سُجِّلت «مُصلَحة في الكود» لا «مؤكَّدة».

## 2026-08-01 — الأساس يتقلّص بالبرهان، والعائق الحقيقيّ حارس أعمى
- **الأمر:** «استكمل» أثناء انتظار فحوص #750. فنُفِّذ شرط الإغلاق الذي كتبتُه بنفسي في `generated_sweep_unmapped_generators.json` بدل تركه نيّة مكتوبة.
- **الملكيّة قيست ولم تُخمَّن:** لكلّ مولّد، لقطة أزمنة تعديل كلّ الملفّات المتعقَّبة قبل تشغيل علم الكتابة وبعده. لم يكن في المستودع ما يسجّل أيّ مصنوعة يكتبها أيّ مولّد؛ صار مسجَّلاً بالقياس.
- **عشرة أُغلِقت بالشرط نفسه:** أفسِد مصنوعة يملكها ⇒ `--check` بغير صفر ⇒ شغّل العلم ⇒ `--check` بصفر **والملفّ مُستعاد بايتاً بايت**. وكلّها خاملة على شجرة نظيفة، فوصلها لا يُوسّخ شجرة أحد. و`--fix` ما زال يتقارب في **دورة واحدة** بعد وصل عشرة كتّاب — وهو الاختبار الذي يثبت غياب دورة ترتيب.
- **ثلاثة تكتب في كلّ تشغيل حتّى من نظيف** (`capability_management_engine` · `capability_registry_guard` · `runtime_environment_preflight`) ⇒ وصلها يجعل كلّ مكنسة تُنتج فرقاً لا يفسّره تغيير مصدر.
- **والستّة الباقية عائقها أخطر من الإغلاق:** `--check` عندها **لا يرى إفساد ما تكتبه**. ١٢ مصنوعة من ١٦ تُفسَد وCI أخضر؛ ثلاثة منها عمياء تماماً. النمط في الجزئيّة: تُقارَن `*.generated.json` ويُترك `.csv` و`.md` بلا حراسة. سُجِّلت `GENERATED-CHECK-IGNORES-ITS-OWN-COMPANION-ARTIFACTS-01`.
- **وصل مولّد حارسه أعمى إصلاحٌ بلا عطل مرئيّ يجيب عنه** — ولذلك بقيت التسعة، لا لأنّ القياس نقص.
- **اختبار من اختباراتي سقط وسقوطه صحيح:** اختبار نزاهة الأساس كان يُثبِّت `report_index_guard.py` بالاسم؛ ولمّا أُغلِق ذلك المدخل وخرج من الأساس صار المسبار يُفسد مدخلاً لا وجود له فلا يُرصَد شيء. **أمسك بيات نفسه.** صار يختار ضحيّته من الأساس الحيّ.
- **وفشل CI أمسك ادّعاءً لي:** `brain_commit_claim_guard` أسقط `09e34e1d` لأنّه يذكر معرّف فجوة غير مُسجَّل. الحارس محقّ — ذكر المعرّف ادّعاء وجوده. سُجِّلت الفجوتان بمصدرهما وحالتهما وشرط إغلاقهما، ومنهما واحدة (`GENERATED-WRITE-ONLY-GENERATORS-UNCLASSIFIED-01`) لم يمسكها أيّ حارس لأنّها في نصّ كود لا في رسالة التزام — سُجِّلت طوعاً لا اضطراراً.
- **التحقّق:** `ruff` فحصاً وتنسيقاً · حرّاس العلامات وشجرة الاختبارات ووصل الاختبارات المعماريّة وحضور التأكيدات · المكنسة `EXIT=0` في دورة واحدة بلا لمس الشجرة · ٢٦ اختباراً خضراء · حرّاس الدماغ الثلاثة خضراء.
- **وإعلان الأثر بايت بين الدفعتين:** كُتِب `Capability-Impact: OPS-004, WX-002` محسوباً بالبوّابة على فرق الـPR **يوم فتحه**. ثمّ نما الفرق مرّتين (جولة الإغلاق، ثمّ تسجيل الفجوتين)، فحجبت البوّابة بـ`missing_direct: [IRR-003, IRR-009]` — والسببان `sahool-brain/gaps/registry.md` و`log.md` نفسه. **الحساب لم يكن خاطئاً؛ كان عن فرقٍ آخر.** الدرس المتكرّر بصياغة ثالثة: قياس صحيح على أساس متحرّك يصير ادّعاءً بائتاً ما لم يُعَد عند كلّ دفعة، تماماً كختم `measured_on` في أساس غير المُوصَّلين. وتعديل متن الـPR وحده لا يُعيد تشغيل البوّابة: `capability-governance.yml:4` يستمع لأنواع `pull_request` الافتراضيّة (بلا `edited`)، وإعادة تشغيل الوظيفة تُعيد تشغيل **الحمولة الأصليّة** بمتنها القديم — فيلزم دفعٌ جديد ليُقرأ المتن المُصحَّح.

## 2026-08-01 — P0-2 (نصف الطيف): وصلٌ ناقص لا بيئة ناقصة
- **الأمر:** «P0 قوم بتنفيذ» ⇒ سألتُ عن المقصود بدل التخمين (قراءتان تقودان إلى عملين مختلفين)، فحُدِّد P0-2.
- **الاكتشاف الأهمّ:** الغياب كان **مُصطنَعاً**. `spectral=None` حرفيّة، والمُنتِج والمُحلِّل قائمان — الثاني حبيس راوتر واحد. لو شُغِّل اختبار حيّ على هذه الحالة لأثبت الغياب وسُمّي «يحتاج stack»، وهو تشخيص خاطئ تماماً: الناقص **تنفيذ** لا بيئة.
- **لم أُنشئ نسخة ثانية:** المُحلِّل **نُقِل** إلى `api/canonical_spectral_state.py` واستورده `crop_twin` بالاسم، فبقيت اختباراته العاملة بالترقيع (`monkeypatch.setattr(ct, "_resolve_server_spectral", …)`) خضراء بلا تليين.
- **قفل صدق أضفتُه قبل أن يُطلَب:** الطيف ليس من `required`، فوصله لا يرفع `operational_eligible`. بلا هذا القفل يُقرأ الوصل تقدّماً نحو أهليّة لم تتغيّر.
- **كدتُ أشحن ادّعاءً كاذباً:** كتبتُ أنّ «كلّ مسار في weather-service يُرجِع مشاهدة»، ثمّ أشار سجلّ الفجوات نفسه إلى `POST /v1/weather/agro/canonical-state` — ففحصتُه ووجدته **يُرجِع الغلاف**. السبب الحقيقيّ أدقّ: النقطة **حاسبة على مدخلات المُستدعي**. صُحِّح النصّ قبل الالتزام. الدرس: السجلّ ليس زينة — قراءته منعت شحن كذبة.
- **ولم أسكّ معرّفاً جديداً:** كتبتُ `CANONICAL-WEATHER-ENVELOPE-NOT-EXPOSED-01` ثمّ وجدتُ الحالة مُسجَّلة تحت `FIELD-STATE-PRODUCERS-MISSING-01`، فحذفتُ معرّفي. الخطأ نفسه مُسجَّل سابقاً في `registry.md:428-430`.
- **التكذيب:** إعادة `spectral=None` ⇒ حارس AST يسقط؛ إرجاع منتَج فارغ بدل `None` ⇒ ٣ تسقط. استُعيد الأصل ⇒ ١٨ نجحت.
- **القياس:** المنصّة **4023** (من مجلّدها كما في CI) · `-m unit` **3864** بعد إعادة التوليد · راتشِت الوحدات 674 ⇒ 675 بمذكّرة مُعلَّلة.
- **درس المكنسة، الرابعة اليوم:** المولّدات اليدويّة يجب أن تُشغَّل **بعد** آخر `--fix` لا قبله — العكس يُعيد إسقاطها. التسلسل الناجح: `--fix` ⇒ الخمسة يدويّاً ⇒ `static_governance_closure` ⇒ `build_release_bundle` ⇒ تحقّق للقراءة فقط ⇒ `EXIT=0`.

## 2026-08-01 — حارسٌ يُعلن نظافةً وهو أعمى: عائلة أعلام ومتابعة سطر
- **الأمر:** «قم بما تراه صحيحاً وكاملاً». اخترتُ البند الوحيد المتبقّي الذي **يُخفي أعطالاً مستقبليّة بصمت**، لا الأكبر ولا الأسهل.
- **بدأتُ بالتسجيل لأنّ الاكتشاف كان غير مُسجَّل:** `grep` عن `write-generated` في `sahool-brain/` و`docs/architecture/*.json` ⇒ **صفر**. كان يعيش في متن #751 وحده — نفس العطل الذي أغلقته #747 و#748، وثالث مرّة أقع فيه.
- **عيبان لا واحد، والثاني أعمق:** (١) `_WRITE_FLAGS` قائمة مغلقة بتطابق تامّ ⇒ عائلة `--write-*` غائبة. (٢) `_STEP` مقصور على سطر واحد بلا طيّ متابعات ⇒ ثلاثة استدعاءات **غير مُكتشَفة أصلاً**. الثاني أعمق لأنّ **ما لا يُكتشَف لا يُصنَّف**، فلا ينقذه أيّ حارس مهما تحسّن.
- **التناقض كان داخل الملفّ الواحد:** `_WRITE_FLAG_DECL` يطابق `--write-*` بالبادئة و`_WRITE_FLAGS` لا. معياران متعارضان لسؤال واحد في ملفّ واحد — وهذا ما جعل الخطأ غير مرئيّ للمراجعة.
- **البادئة لا التعداد، بحجّة عدم التماثل:** خطأ البادئة «سمِّه في الأساس» كلفته سطر مُعلَّل؛ خطأ التعداد «لا يُعاد توليده أبداً» كلفته انحراف صامت. فاخترتُ الاتّجاه الذي يُخطئ بالضجيج لا بالصمت.
- **لم أصل شيئاً بالثقة:** طبّقتُ الشرط المُعلَن في الأساس على الأربعة — إفساد مصنوعة ⇒ `--check` يخرج `1` ⇒ تشغيل العلم ⇒ `--check` بصفر **والملفّ مُستعاد بايتاً بايت**. أربعة من أربعة. ولو سقط أحدها لَذهب إلى الأساس لا إلى الخريطة.
- **وتحقّقتُ من `--write-source` قبل وصله:** الاسم يوحي بكتابة كود مصدريّ، والقياس أظهر أنّه يكتب `release/PLATFORM_ROUTE_GOVERNANCE_BINDING.json` — مصنوعة مولَّدة. الوصل على الاسم كان سيُدخِل كاتب كود إلى `--fix`.
- **عيب ثالث كشفه الوصل نفسه:** بعد الوصل قِستُ الترتيب فوجدتُه `budget → attestation → ownership` — التصديق **قبل** أحد مصدرَيه. لم يكن ليظهر لولا الوصل، ولم يكن ليُنتِج خطأً خاصّاً به: يبقى بائتاً بصمت وتُصلحه دورة `--fix` ثانية **مصادفةً**. صار صريحاً في `_ORDER_TIER`.
- **تصحيح ذاتيّ أثناء العمل:** كتبتُ في التعليق أنّ الترتيب الأبجديّ يُنتِج التسلسل الصحيح بلا تدخّل، ثمّ قِسته فكان خاطئاً — وصحّحتُ التعليق قبل الالتزام. التعليق الذي لم يُقَس ادّعاءٌ لا توثيق.
- **ثلاثة أقفال مُكذَّبة كلٌّ على حدة** (لا دفعةً واحدة): التعداد المغلق ⇒ يسقط اختبار العائلة · إلغاء الطيّ ⇒ يسقط اثنان · حذف طبقة الترتيب ⇒ يسقط اختبار الترتيب. استُعيد الأصل ⇒ ٢٤ نجحت.
## 2026-08-01 — مراجعة الفرع الرئيسيّ: بوّابة اكتمال لا تقيس، وانتظارٌ صيّر الكنسة طابوراً
- **الأمر:** «قوم بمراجعة الفرع الرئيسي» ⇒ مراجعة `main` @ `fde6f955`، ثمّ «قوم بتنفيذ الجميع» ⇒ تنفيذ ما وجدته.
- **المراجعة وجدت `main` سليماً، والأحمر الوحيد بنية تحتيّة لا كوداً:** `Flutter Analyze & Test` خرج بالرمز 35 (TLS في curl) بعد **٦٧ مللي ثانية** عند تنزيل SDK — `pub get`/`analyze`/`test` لم تُشغَّل، و#749 لم يلمس `mobile/`، و١٩ تشغيلاً سابقاً أخضر بنفس التثبيت. لم أُصلح شيئاً في الشجرة لأنّ الشجرة لم تكن مكسورة.
- **ولا أبلغتُ `ecdsa` فجوةً:** ظهرت في `pip-audit` المحلّيّ لأنّي أغفلتُ `--ignore-vuln` — والاستثناء موثَّق في `ci.yml:1112-1121` بتعليل سليم. نتيجة حمراء كاذبة من أمري لا من الشجرة.
- **أهمّ اكتشاف — الصمت كان يُقرأ «لا شيء هنا»:** `ENV_CALL_RE` يلتقط الحرفيّ فقط، فـ`os.getenv(_CLOUD_POLICY_ENV, …)` غير مرئيّ و`--check` **يمرّ**. وإصلاح النمط أظهر **١٣ متغيّراً عبر ٥ خدمات** منهم **مفتاحا تعمية MFA**. لم تكن فجوة متغيّر؛ كانت فجوة بوّابة تُبلِغ باكتمال لا تملكه.
- **أوّل تكذيب لي فشل، وهذا أهمّ من نجاحه:** فصلتُ الحلّ عن مسار المسح فبقيت الاختبارات خضراء — اختبار الأثر يقرأ ملفّاً مُولَّداً سلفاً، واختبار الدالّة يستدعيها مباشرة، فلا أحد يمرّ بالمسح. أي أنّ اختباري كان سيسمح بعودة العطل **بشكله الأصليّ بالضبط**: قدرة موجودة لا تجري. الدرس: التكذيب الذي لا يمرّ بمسار الإنتاج ليس تكذيباً.
- **وجلسة أخرى سبقتني إلى العطل نفسه وأحسنت:** #750 أصلح خريطة كتّاب المكنسة بعشرة مقابل خمستي (وخمستي **مجموعة جزئيّة** منها)، وأغلق كلّاً ببرهان إفساد-واستعادة بايتاً بايت لا بالثقة بوجود العلم. أخذتُ نسخته حرفيّاً وأسقطتُ نسختي، وتركتُ `e35ed137` قائماً **فارغ الأثر** بدل إعادة تشكيله ليبدو ضروريّاً. تلاقي جلستين على عطل واحد ليس تكراراً يُخفى.
- **خطأ ارتكبتُه وصحّحه CI:** ألغيتُ توليداً جارياً ثمّ رأيتُ أربعة ملفّات في `capability-registry/generated/mapping/` مُعدَّلة، فحكمتُ أنّها «بقايا توليد مُجهَض» وأرجعتُها. لم تكن بقايا — كانت خرجاً شرعيّاً لفرعي، وسقطت `test_mapping_has_no_drift` على `unmapped_artifacts` ناقصاً ملفّ اختباري الجديد. الدرس: «مُعدَّل بعد إلغاء» ليس مرادفاً لـ«غير مقصود»؛ القياس (`--generate` ثمّ الفرق) كان يحسم في ثانية ما حسمتُه بالحدس خطأً.
- **وحارس ادّعاء الدماغ أمسكني كما أمسك غيري:** سككتُ `RUNTIME-CONTRACT-INDIRECT-ENV-01` و`IMAGERY-SCAN-SERIAL-WAIT-01` في رسالتَي التزام بلا تسجيلهما. الحارس محقّ — ذكر المعرّف ادّعاء وجوده. سُجِّلا بمصدرهما وحالتهما وحدّ صدقهما.
- **حجم الادّعاء ضُبِط عمداً في الشريحة الثانية:** الجدولة لا تضع مهلة، فأثر التسلسل **انزياح دورة** لا تعليق، و١٢٠ث سقفٌ عند المهلة لا كلفة لكلّ حقل. النموّ الخطّيّ حقيقيّ والمنصّة المتوقّفة ليست كذلك — وكتبتُ ذلك في الالتزام والسجلّ معاً كي لا يُقرأ الإصلاح أكبر ممّا هو.
- **التحقّق:** `-m unit` **3892** ناجحاً · صفر فشل · تغطية **47.73%** · المكنسة `EXIT=0` · ruff نظيف · واختبارا `main` الأحدث (`test_canonical_field_state_wiring` + `test_canonical_spectral_state_resolver`، ١٨) خضراوان على الشجرة المدموجة — الدمج النظيف لا يُثبت اتّفاق تغييرَين على الموضوع نفسه.
- **وإعلان الأثر بات على أساس متحرّك — الدرس نفسه بصياغة رابعة:** حسبتُ `Capability-Impact: SAT-001, SEC-004` بالبوّابة على `origin/main` عند `d4549ef6`. ثمّ دُمِج #752 فصارت القاعدة `bbfe121f`، ونما الفرق بملفّات الدماغ (`gaps/registry.md` · `log.md`)، فحجبت البوّابة بـ`missing_direct: [IRR-003, IRR-009]`. **الحساب لم يكن خاطئاً؛ كان عن فرقٍ آخر.** والسجلّ يحمل الدرس نفسه من جلسة سابقة (`09e34e1d`) بحرفه — قرأتُه ووقعتُ فيه، وهو ما يجعله سطراً في بروتوكول لا ملاحظة: **يُعاد حساب الإعلان عند كلّ دفع تتغيّر فيه القاعدة، لا مرّةً عند فتح الـPR.** وتعديل المتن وحده لا يكفي: `capability-governance.yml` يستمع لأنواع `pull_request` الافتراضيّة بلا `edited`، وإعادة تشغيل الوظيفة تُعيد الحمولة الأصليّة بمتنها القديم ⇒ يلزم دفعٌ جديد ليُقرأ المتن المُصحَّح.
- **~~مُتروك للمالك~~ ⇒ نُفِّذ بأمره في الشريحة التالية (#757):** كُتِب هنا «٢٤ متغيّراً بلاحقة `_KEY` مُصنَّفة `configuration`، يلزمها قرار مالك». **والرقم كان خاطئاً**: ٢٤ هي أزواج (خدمة × متغيّر)، والصحيح **١٣ متغيّراً متمايزاً** أُعيد تصنيف **١٠** منها. أمر المالك جاء صريحاً فنُفِّذ: قاعدة اللاحقة `_KEY`/`_KEYS` ⇒ سرّ **مُغلَقة عند الفشل**، والإعفاء إعلانٌ يحمل سطر مصدره. **وحدسه عن `JWT_PUBLIC_KEY` عمّ أبعد ممّا قاله**: قراءة المستهلكين أظهرت `MFA_ALLOW_DERIVED_KEY` **راية منطقيّة لا مفتاحاً** و`STAGING_PROBE_IDEMPOTENCY_KEY` معرّفَ إلغاء تكرار بقيمة صريحة — وكلاهما كان سيُصنَّف سرّاً خطأً بقاعدة عمياء. **تصحيح سطر بائت في السجلّ نفسه جزء من البروتوكول:** بندٌ يقول «مفتوح» بعد إغلاقه يجعل السجلّ يكذب بصمت.

## 2026-08-01 — قاعدة مكتوبة في الأداة ولا تفرضها الأداة
- **المُدخَل:** رابط تشغيل CI فاشل على فرع آخر (`30711928974`)، و«أكمل بما تراه صحيحاً».
- **قرأتُ السجلّ بدل تخمين السبب:** `only-fresh=['tests_v9/test_generated_artifact_contract.py']` — مصنوعة `capability_mapping` انحرفت بملفّ اختبار **جديد**. ورسالة الالتزام تقول إنّ المكنسة تقاربت عند `EXIT=0`. **الجملتان صحيحتان معاً**، وهذا هو التشخيص: المكنسة رأت شجرةً بلا ذلك الملفّ لأنّه لم يكن في الفهرس.
- **ولم أُصلح فرع غيري:** الأصحّ أن يُغلَق الصنف لا الحالة. الفخّ **موثَّق في docstring المكنسة نفسها** («`git add` قبل التشغيل ضرورة لا عادة») ولا تفرضه — فأيّ شخص أو وكيل سيقع فيه ثانيةً.
- **قِستُ قبل أن أكتب:** `tree_state` يستعمل `--untracked-files=no`، ولا دالّة في الوحدة تحمل كلمة `untracked` غيرها. أي أنّ الأداة تفحص حالة git **ولا تسأل عن اكتمال المُدخَل**. الخلط بين السؤالين هو ما أبقى الثغرة مفتوحة رغم قرب الأداة منها.
- **موضع البوّابة مقصود:** أوّل ما يُنفَّذ بعد الاكتشاف. كلّ قياس بعدها مبنيّ على افتراض اكتمال المُدخَل، فتأخيرها كان سيُنتج تقريراً مفصَّلاً عن مُدخَل ناقص — أسوأ من الفشل المبكر لأنّه يبدو معرفةً.
- **والمُتجاهَل مستثنى بحجّة لا بتساهل:** المولّد لا يراه ولا يُفترَض أن يراه، فإدراجه يُنتج ضجيجاً يُعلّم الناس تجاهل البوّابة — وهو ما يُبطِلها فعليّاً.
- **تكذيبان، والثاني أخطأتُ فيه أوّل مرّة:** أعدتُ `--untracked-files=no` ⇒ سقط اثنان. ثمّ أضفتُ `--ignored` ظنّاً أنّه يُدخِل المُتجاهَل — ولم يسقط اختبار الاستثناء، لأنّ المرشِّح يقرأ `?? ` وسطور المُتجاهَل تبدأ بـ`!! `. أي أنّ **تكذيبي لم يكذّب شيئاً**. أعدتُه صحيحاً (توسيع المرشِّح) فسقط الاختبار كما يجب. تكذيب لا يكسر شيئاً ليس دليل صلابة بل دليل أنّ التجربة أُسيء تصميمها.

## 2026-08-01 — إعادة قياس ادّعاء `decision_linked`: من «كاذب» إلى «غير مُبرهَن حيّاً»
- **الأمر:** «استكمل». اخترتُ الفجوة التي **يمكن أن يكون حكمها هو نفسه قد بات** — لا الأكبر.
- **الفرضيّة التي فحصتُها:** الحكم «كاذب تشغيليّاً» كان مبنيّاً على أنّ الكاتب لا يعمل. و#749 أصلح الكاتب. فهل بقي الحكم صحيحاً؟ **لا.** إبقاؤه كان سيُنتج انحرافاً معاكساً: سجلّ يقول «كاذب» عن ادّعاء لم يعد كذلك — وهو نفس صنف العطل في الاتّجاه المضادّ.
- **قِستُ السلسلة كاملةً لا طرفاً منها:** الكاتب ⇒ `field_state_projection:145` ⇒ `canonical_water_stress:142` ⇒ مخرَج الحالة. وتحقّقتُ من نقطة كانت ستُفسِد الحكم: علم التصعيد يحكم **التصعيد** لا **التأكيد**، فالمؤشّران يُغيّران المخرَج حتّى والعلم مُطفَأ.
- **الحالة الجديدة ليست «مُغلَق»:** الوصل قائم ومحروس، والملء الحيّ غير مُبرهَن. ثلاث حالات لا اثنتان، ولم يُرفَع `runtime_verified`.
- **العيب في الحارس القائم:** اختبار الجسر يؤكّد وجود مفتاحين **في قاموس تكتبه الوحدة نفسها**. حلقة مغلقة: لو انقطعت السلسلة كلّها لبقي أخضر. أضفتُ حارساً يربط الوسم بالمسار بدل أن يفحص إعلاناً بإعلانه.
- **وخطأ في حارسي أنا كشفه التكذيب:** أوّل صيغة كانت نصّيّة (`"last_ndmi_mean" in source`)، فكسرتُ جملة `SELECT` **ومرّ الاختبار** — لأنّ الاسم يظهر في سطر آخر. لولا التكذيب لشحنتُ حارساً لا يحرس. استُبدِل بفحص سلوكيّ عبر conn وهميّ يُجيب ذلك الاستعلام وحده. **الدرس نفسه للمرّة الرابعة هذه الجلسة: الحارس المصدريّ يقيّد الإملاء لا العقد.**
- **وقوّيتُ حارس الكاتب للسبب نفسه قبل أن يُكذَّب:** كان `"_await_batch_terminal" in automation` — يمرّ لو بقي التعريف وحُذِف الاستدعاء. صار فحصاً على AST يشترط الاستدعاء **داخل** `_trigger_indicators` وأن تُحكَم به القراءة بـ`return`.
- **أربعة أقفال مُكذَّبة كلٌّ على حدة**، ثمّ استُعيد الأصل ⇒ ١١ نجحت.

## 2026-08-01 — حللتُ تعارضاً وشحنتُ فيه علامة تعارض
- **السياق:** إعادة تأسيس `bind-spectral-decision-link-claim` على `main` بعد #754 — سبع مصنوعات مولَّدة و`log.md`.
- **القاعدة طُبِّقت صحيحةً:** المصنوعات أُخِذت من `main` **لتُعاد من المصدر** لا لتُدمَج يدويّاً (والدليل أنّ إعادة التوليد بعدها غيّرت السبعة نفسها). و`log.md` دُمِج **زمنيّاً** لا باختيار طرف.
- **والعطل:** سكربت الدمج أبلغ `merged log conflicts: 1 | markers left: 7`. تعبيري النمطيّ غير الجَشِع ابتلع بنيةً غير متوقّعة فترك `<<<<<<< HEAD` يتيمة عند السطر 4641 بلا `=======` ولا `>>>>>>>` — **والتزمتُها فعلاً**.
- **المفارقة:** `log.md:3404` يحمل الدرس مكتوباً («فرع مدموج بعلامات تعارض + CI أخضر — المراجعة البشريّة لم تلتقطه»). وقعتُ في العطل الذي يسجّله السطر الذي كنتُ أدمجه.
- **لماذا نجحت المراجعة هذه المرّة:** لأنّي **عددتُ ثمّ قرأتُ**. الرقم «7» وحده إنذار كاذب — اثنان منه ذكرٌ نصّيّ للكلمة في سطرَي 152 و3404. والفحص الصحيح `^<<<<<<<` (بداية سطر) لا `<<<<<<<` (أيّ موضع). **الفرق بين قياس وضجيج هو الالتصاق ببداية السطر.**
- **بعد الإصلاح:** صفر علامات على أوّل السطر · صفر ملفّات في كامل المستودع · القسمان (‏`main` وقسمي) موجودان كلاهما.
- **تصحيح لادّعاء كتبتُه هنا قبل أن أقيس:** كتبتُ «لم أُضِف حارس CI لعلامات التعارض… يستحقّ شريحته»، وهذا **خطأ**. الحارس قائم منذ 2026-07-13: `scripts/ci/no_merge_conflict_markers_guard.py` موصول في `.github/workflows/ci.yml:159` (و`conflict_marker_guard.sh` عند `:457`)، وله اختباره. واختبرتُه الآن بحقن علامة في `sahool-brain/log.md` فرصدها بالسطر والملفّ وخرج بـ`1`. **فلا فجوة هنا.**
- **ولماذا لم يلتقط علامتي إذن:** لأنّه لم يُشغَّل عليها. لا يوجد أيّ تشغيل CI على `38ff2550` — عدّلتُ الالتزام (`--amend`) ودفعتُه خلال دقيقتين، فلم يبدأ التشغيل أصلاً. أي أنّ مراجعتي سبقت CI لا أنّ CI أخفق. **والفرق جوهريّ:** «الحارس أعمى» يستدعي بناءً جديداً، و«الحارس لم يُسأل» لا يستدعي شيئاً — وبناء حارس ثانٍ على تشخيص خاطئ كان سيُضيف سطحاً بلا سبب.

## 2026-08-01 — جردٌ إلزاميّ قلب الفجوة ثلاث مرّات
- **الأمر:** المسار (3) بتوجيه المالك: صحّح السجلّ، سجّل الجديدة، **ولا تُدخِل `require_scope` قبل إثبات المستهلكين**.
- **الفجوة المطلوب إغلاقها لم تكن قائمة:** `MCP-PREAUTH-STATUS-01` تقول «400 بدل 401». شغّلتُ الحارس الحقيقيّ فأعطى **401** في الحالات الثلاث، والمسار المذكور `/mcp/v1/tools/call` **غير موجود** في المستودع كلّه. أغلقتُها `STALE/MISDIAGNOSED` بالقياس لا بالرأي.
- **والتزمتُ بتحفّظ المالك على الـ422:** لم أُصنّفه عيب pre-auth. FastAPI يفشل في التحليل قبل الـdependency، و«auth قبل تحليل الجسم» سياسةٌ إن اتُّخِذت صارت فجوة ترتيب middleware **مستقلّة** — لا تُدمَج في هذه لتضخيمها.
- **الجرد كشف ما لم أبحث عنه:** `generic_context_server` بلا أيّ مصادقة، و**ستّ** خدمات منشورة تشتقّ منه لا أربع (تصحيح لرقمي الأوّل الذي قلتُه قبل القياس).
- **ثمّ كشف الجرد أصل الخطأ في الفجوة الأولى:** عميل المشرف يبني `base_url` بلاحقة `/mcp/v1` ثمّ يطلب `/v1/mcp/tools/call`، وhttpx **يُضيف لا يستبدل** ⇒ `/mcp/v1/v1/mcp/tools/call`. قِستُه بـ`build_request` لا بذاكرتي عن httpx. من كتب الفجوة قرأ الإعداد واستنتج المسار — وهذا يفسّر الخطأ بدل أن يكتفي بوصفه.
- **الجرد هو ما أباح التغيير السلوكيّ:** صفر مستهلك مُهيّأ (لا اسم خدمة، ولا متغيّر بيئة، والعميل الوحيد يستهدف الأربعة المحروسة ويرسل توكناً دائماً). فلا فترة سماح ولا bypass — والافتراضيّ آمن. **وحدّه مكتوب:** «لا مستهلك في المستودع» ≠ «لا مستهلك في نشر المُشغّل».
- **النطاق موحَّد بحجّة:** `mcp:context:read`. نطاق مجاليّ واحد يكذب على خمسة مجالات، وستّة نطاقات تجعل الحارس يعتمد على `MCP_SERVICE` — **مُدخَل بيئة لا هويّة**، أي حارس يُضبَط من خارجه.
- **اختبار الوراثة كما طلب المالك:** «المشتقّ» ليس صنفاً وارثاً بل نفس الوحدة بقيمة بيئة مختلفة، فبُنيت الستّة بإعادة تحميل. والقائمة **تُشتقّ من compose**: خدمة سابعة تُسقِط الفحص بدل أن تمرّ.
- **خطآن في اختباري أنا كشفهما التشغيل:** قرأتُ `dep.call` بدل `dep.dependency`، واستعملتُ اسم أداة وهميّاً فأعطى 404 **بعد** الحارس — أي أنّ «مرور النطاق الصحيح» لم يكن مُختبَراً أصلاً وكان سيبدو ناجحاً لو كتبتُ `!= 401`. أصلحتُهما بقراءة `TOOLSETS` الحقيقيّة.
- **والتكذيب الحاسم هو الثاني:** استبدال الحارس بـdependency لا-عمل ⇒ **١٣ تسقط**. لولاه لكان فحصي «هل توجد أيّ dependency؟» — وهو يمرّ على حارس زائف.
- **واكتشاف رابع لم يكن في الخطّة:** اختباراتي سقطت مُجمَّعةً ونجحت منفردة. لم أعدّل العتبة ولم أُعِد الترتيب — قِستُ: الـdependency المرتبطة بالمسار كانت `<lambda>` من `test_mcp_weather_et0_engine_delegation.py`، الذي يحقن كعب `shared.oauth_middleware` في `sys.modules` **وقت الجمع**. فأيّ خادم MCP يُستورَد بعده يُبنى بلا حراسة. وتشغيل ذلك الملفّ قبل `test_mcp_functional` يُسقِط **ثلاثة** تأكيدات أمن قائمة — أي أنّ خضرتها اليوم **مصادفة ترتيب أبجديّ**. سُجِّلت `MCP-TEST-STUB-NEUTERS-AUTH-01`، واحتويتُها في شريحتي بتحميل الوحدة الحقيقيّة بمسارها، ولم أُصلح ملفّ الكعب لأنّ تصميمه يخصّ غرضه.
- **الدرس المتكرّر بصيغة جديدة:** اختبار الأمن يجب أن يؤكّد **سلوكاً** ووحدةَ الحارس، لا «هل توجد dependency؟». الصيغة الأخيرة كانت ستمرّ على حارس مُبطَل — وهي التي كتبتُها أوّلاً.

## 2026-08-01 — P0-3: «موجود» و«صالح» ليسا سؤالاً واحداً (#752 `bbfe121f`)
- **الأمر:** «قوم بتنفيذ P0-3 (operational_eligible)».
- **العلّة مُثبَتة بالتشغيل لا بالقراءة:** منتَج حاضر بمخطّط صحيح يُعلن مالكه أنّه `degraded` كان يُبقي `operational_eligible=True` — فتبدو الحالة صالحة لتوصية تنفيذيّة بينما مالك البيانات نفسه يقول إنّها ليست كذلك.
- **ما لم يُغيَّر عمداً:** الحقل القديم باقٍ بمعناه حرفيّاً. له مستهلكون قائمون (`core/field_digital_twin.py:71`)، وتشديد معناه تحتهم صامتاً أخطر من نقصه. أُضيف بجانبه `eligibility`: سُلَّم رتيب `discover` · `diagnose` · `propose` · `execute`، كلّ مستوى بـ`allowed` وأسبابه.
- **الحدّ الحاكم:** المُجمِّع يحكم على **الإعلانات** لا على الوقائع. حساب عتبة حداثة هنا كان سيفرّع منطق المالك إلى نسخة ثانية تتباعد صامتاً — وهو ما يُحرّمه عقد الوحدة المكتوب أصلاً.
- **مفردات الجودة غير موحَّدة، مقيس:** الطقس يُخرِج validated/degraded/insufficient/invalid والماء يُخرِج verified/degraded. فالمجهول يُعامَل **غير مُثبَت لا سليماً** — قائمة سوداء كانت ستُمرّر أيّ مصطلح جديد من مالك جديد بلا قراءة.
- **`execute` ممنوع دائماً من هذه البنية:** لا إذن ولا توقيع ولا هويّة مُوافِق فيها؛ فالصادق أن تُعلن أنّها لا تملك الجواب بدل أن تُجيب `true` من مُدخَلات لا تخصّ الإذن.
...
- **مُكذَّب أربعاً:** قائمة سوداء ⇒ يسقط اختبار المفردة المجهولة · الصمت سلامةً ⇒ يسقط اختبار الجودة غير المُعلَنة · `execute` يتبع `propose` ⇒ يسقط اختبار الإذن · إسقاط الصحّة من `propose` ⇒ تسقط خمسة.
- **حدّ صدق:** يُغلق **مستوى الإعلانات** وحده. لا يُغلق ربط المستأجر/الحقل ولا نسخة الهندسة ولا المواءمة الزمنيّة ولا الطابع الزمنيّ المستقبليّ — تبقى مفتوحة بأسمائها، ولا يُدَّعى أنّ P0-3 أُغلِقت كاملةً.
- **خطأ إجرائيّ منّي، مُصحَّح هنا:** هذه الشريحة سُجِّلت في `gaps/registry.md` ولم تصل `log.md` ولا `hot.md` ولا سجلّ القرارات — أي أنّ بروتوكول نهاية الجلسة في `CLAUDE.md` لم يُستوفَ لها. لا حارس يمسك هذا اليوم، فسُجِّل طوعاً.
- **وعيب صادف أثناء التنفيذ:** استعملتُ سبع علامات اتّجاه U+200F في توثيق عربيّ فحجب `bandit` بـB613. أُزيلت ولم تُستبدَل بـU+200E (يُعلن الاتّجاه المعاكس). ثمّ رفض `brain_commit_claim_guard` رسالتَي التزام: الأولى تذكر عبارة كبيرة موصولة، والثانية تذكر **نصّ التعبير النمطيّ** الذي كتبتُه أشرح به الرفض الأوّل. الحارس محقّ في الحالتين؛ إيجابيّته الكاذبة ثمن دقّته، ولم أُليّنه.

## 2026-08-01 — الحارس الذي لا يحرس ما يكتب: ١٣ من ١٦ مصنوعة
- **الأمر:** «استكمل» — إتمام الفجوة التي عرقلت الجولة السابقة بدل تركها مسجَّلة.
- **القياس على `bbfe121f`:** ستّة حرّاس تُشغَّل بـ`--check` في workflows، و**١٣ من ١٦ مصنوعة يملكونها تُفسَد والفحص أخضر**. ثلاثة منهم لا يحرسون شيئاً ممّا يكتبون البتّة.
- **تصحيح رقم سجّلتُه بنفسي:** كتبتُ «١٢ من ١٦». إعادة الجمع على نفس الأرقام لكلّ حارس (٢/٢ · ١/١ · ٢/٢ · ٤/٥ · ٢/٣ · ٢/٣) تُعطي **١٣**. جمعٌ خاطئ لا قياسٌ مختلف.
- **وتصحيح سبب سجّلتُه بنفسي:** كتبتُ أنّ الثلاثة العمياء تماماً تُعيد التوليد أثناء الفحص فتمحو الإفساد. قياس أزمنة التعديل كذّب ذلك: **واحد فقط** يكتب أثناء `--check`. الآخران لا يكتبان و**لا يقارنان الملفّ إطلاقاً** — يعيدان بناء الجرد في الذاكرة ويؤكّدان قواعدهما الدلاليّة عليه. الأثر واحد والسبب مختلف، والعلاج يختلف باختلافه.
- **وحدة واحدة لا ستّة إصلاحات:** `scripts/ci/generated_artifact_contract.py`. الكتابة والفحص يمرّان بدالّة الرسم **نفسها**، فلا يتباعد ما يُكتَب عمّا يُقارَن — وهو الفخّ الذي أوقع `capability_linker` سابقاً. والمقارنة بالبايت: `read_text` يُوحّد نهايات الأسطر فيُخفي إفساداً حقيقيّاً في CSV تنتهي أسطره بـCRLF.
- **مُكذَّب بإعادة المصادر الستّة إلى `bbfe121f`:** عاد العمى **١٣ من ١٦** بالضبط، وبعد الوصل **صفر من ١٦**.
- **وتكذيب فاشل أضاف اختباراً:** جعلتُ `enforce` تكتب بلا شرط (فيصير الفحص إصلاحاً) وبقيت الملفّة خضراء — الخاصّيّة كانت مُدّعاة في التوثيق وغير محروسة. أُضيف `test_checking_never_repairs_what_it_is_checking`.
- **العائق أُزيل فانفتح ما خلفه:** الستّة كان **شرط إغلاقها نفسه غير قابل للتحقّق** في أساس غير المُوصَّلين — الشرط يبدأ بـ«أفسِد ⇒ يجب أن يُرصَد». بعد الإصلاح مرّ الشرط كاملاً على كلٍّ منها فوُصِلت بـ`_GENERATE_FLAG` (٩ ⇒ ٣). والثلاثة الباقية سببها مختلف تماماً: **غير خاملة** — تكتب في كلّ تشغيل حتّى من شجرة نظيفة.
- **درس أداة، وهو أثمن ما في الجولة:** مِسبَري كان يستعيد الشجرة بـ`git checkout -- .` بعد كلّ حارس، فمحا **تعديلات المصدر الستّة نفسها** التي يقيسها وأنتج «١١ من ١٦ ما زالت عمياء» — رقم كاذب عن شجرة عادت إلى `bbfe121f` بلا أن أدري. النمط الذي كشفه: أوّل حارس يرصد ثمّ كلّ ما بعده أعمى. الاستعادة الصحيحة من نسخة **في الذاكرة** للملفّ الواحد المُفسَد وحده.
- **قفل الانحدار لا يُفسِد ملفّات مُلتزَمة:** `tests_v9/test_generated_artifact_contract.py` (‏`unit`، ١٦ فحصاً). إفساد ملفّات في كلّ تشغيل وحدات يترك الشجرة موسَّخة إن قُتِل التشغيل قبل الاستعادة (`finally` لا يُنقِذ من SIGKILL). بدلاً منه ثلاثة شروط قراءة صرفة مجموعها يُساوي ما قاسه الإفساد — ومنها أنّ كلّ حارس يُمرّر إلى `drift`/`enforce` نتيجة `artifacts(...)` الخاصّة به، مُتحقَّقاً **بشجرة AST** لا بـgrep: فحص نصّيّ يقبل حارساً يستورد العقد ثمّ يقارن غيره.
## 2026-08-01 — تقرير خارجيّ قال «بيئة Windows»، والقياس قال «الكود يعتمد ترميز الآلة»
- **الأمر:** تقرير فحص لخمس عشرة فجوة، أبلغ عن **١٢ فشلاً** ونسبها كلّها إلى بيئة Windows: «لا فشل منطقيّ في الكود».
- **لم أُصدّقه ولم أرفضه — أعدتُ إنتاجه:** فرضتُ لغة C على Linux (`LC_ALL=C PYTHONUTF8=0` ⇒ الافتراضيّ ANSI_X3.4-1968) فظهرت **١٢ بالضبط**، كلّها Unicode. الرقم نفسه على نظام آخر ليس مصادفة: العطل ليس خاصّيّة Windows بل **اعتماد الشيفرة على ترميز الآلة**، وWindows يكشفه فقط لأنّ افتراضيّه ليس UTF-8.
- **ولماذا التصنيف مهمّ:** «عطل بيئة» يُغلَق بتغيير الجهاز والكود كما هو؛ و«اعتماد على ترميز الآلة» يُغلَق بإصلاح الكود. القبول بالأوّل كان سيجعل CI الأخضر شهادةً على أنّ Linux افتراضيّه UTF-8 لا على أنّ الحارس يقرأ ما يدّعي.
- **العلاج الذي وصفه التقرير غير كافٍ — مقيس لا مُقدَّر:** «أضِف `encoding='utf-8'` إلى القراءات» أغلق **٨ من ١٢**. بقيت أربعة لأنّ المتّجهات ثلاثة: قراءة مباشرة · `subprocess(..., text=True)` حيث **الأب** يفكّ بترميز اللغة · و**مخرَج الابن نفسه** الذي لا يبلغه `encoding` الأب إطلاقاً — فرسالة تحوي عربيّة أو «—» تُسقِط الابن بـ`UnicodeEncodeError`. الثالث لا يظهر لأيّ فحص يبحث عن `open(` وحده.
- **القياس النهائيّ:** ١٢ ⇒ **٠** تحت نفس اللغة، `EXIT=0`، 3895 نجحت.
- **الإنفاذ لا الإصلاح وحده:** أساس ١٨٨ ملفّاً يتقلّص ولا ينمو، بإنفاذ عكسيّ (مدخل بائت يُسقِط الاختبار). ولم أُنشئ سكربت حارس جديداً عمداً: سكربت يُعلن `--check` ولا يذكره workflow يُسقِط `classify_uncovered` — فالمنطق في ملفّ اختبار تلتقطه وظيفة الوحدات بلا ضريبة تسجيل.
- **تكذيب مُدمَج ضدّ نفسي:** أضفتُ فحصاً يُثبِت أنّ الماسح يرى متّجه العمليّات، لأنّ ماسحاً يبحث عن `open(` وحده كان سيمرّ خضراء ويترك ثلث الفشل غير مرئيّ — وهو بالضبط الخطأ الذي وقع فيه التقرير.
- **رقم تحقّقتُ منه بدل تمريره:** بقي حجم الأساس ١٨٨ بعد إصلاح ملفّين، فبدا متناقضاً؛ الفرق المباشر بين المجموعتين أظهر تطابقاً تامّاً — الملفّان كانا نظيفين وقت القياس الأوّل. لا تناقض، لكنّي لم أعرف ذلك إلّا بعد القياس.
- **حدّ صدق:** هذا يُغلق **بنداً واحداً** من أربعة عشر في التقرير — الوحيد الذي لا يحتاج stack حيّاً. الثلاثة عشر الباقية تحتاج خدمات حيّة، ولا يرفع هذا الإصلاح `runtime_verified` ولا `production_certified` على أيّ منها.

## 2026-08-01 — حزمة إصلاح خارجيّة: تحقّقٌ بالتشغيل لا بقراءة التقرير
- **الأمر:** «تحقق ما تم اصلاحه من هذا» + حزمة delta تُعلن أنّ فشل UTF-8 على Windows «FIXED IN CODE».
- **لم أقرأ التقرير وأحكم — طبّقتُ الحزمة:** شجرة عمل على `e73944e7`، نُسِخت ٨١ ملفّاً من الدلتا، ثمّ شُغِّلت **نفس الاثني عشر** تحت نفس اللغة. النتيجة **١١ ما زالت تفشل**. الحزمة تُصلح واحداً.
- **السبب بنيويّ لا تنفيذيّ:** نطاقها `scripts/ci` و`scripts/release` فقط، و**سبعة** من الملفّات الثمانية المُثبَت انهيارها في `tests_v9/`. لا يوجد فيها ملفّ اختبار واحد من السبعة — مُتحقَّق بالبحث عن كلٍّ منها بالاسم.
- **ومتّجهان لا يراهما حارسها:** يفحص `read_text`/`write_text`/`open` وحدها ⇒ ٢٨ موضع `subprocess(..., text=True)` داخل نطاقه هو بلا ترميز، ومتّجه مخرَج الابن (`PYTHONIOENCODING`) غير مذكور أصلاً — وهو سبب أربعة من الاثني عشر.
- **وبوّابة تسقط:** `tests/architecture/test_text_encoding_guard.py` غير مذكور في أيّ workflow ⇒ `arch_test_ci_coverage_guard --check` يخرج بـ**1**. عقد المستودع «لا إعفاء: كلّ اختبار معماريّ يُشغّله workflow». وهذه بالضبط الضريبة التي تجنّبتُها بوضع منطقي في `tests_v9/`.
- **وأمسكت الحزمة عيباً فيّ، وهو أهمّ ما في الجولة:** ماسحي أدرج أربعة ملفّات دَيناً وهي نظيفة، لأنّه يقرأ وسيط الوضع من الموضع الثاني دائماً — وموضعه في `path.open("rb")` **الأوّل**. فحُسِب كلّ فتح ثنائيّ على كائن مسار عيباً. رأيتُه فقط لأنّ حارسهم قال PASS حيث قال ماسحي «ستّة متبقّية»، فقِستُ الخلاف بدل أن أفترض صواب نفسي. ١٨٨ ⇒ ١٨٤ ملفّاً، ٤٦٠ ⇒ ٤٥٤ موضعاً.
- **الدرس:** أساسٌ يُبالغ في الدَّين ليس أأمن من أساس يُقصّر فيه — كلاهما يُدرَّب قارئه على تجاهل الحارس. والخلاف بين حارسَين ليس إزعاجاً بل قياسٌ مجّانيّ.
- **ما لا يُنكَر على الحزمة:** حدّها الصادق مكتوب («لم تكتمل المكنسة الشاملة — مهلة») بدل ادّعاء اكتمالها، و١٨٤ موضعاً في `scripts/` أُصلِحت فعلاً ولها قيمة وإن لم تُغلِق الفشل المقيس.

## 2026-08-01 — علامة تعارض عبرت حارسَين، وأمسكتها مقارنة لا حارس
- **الحادثة:** دفعة متزامنة على فرعي نفسه (`e20a0ccd`) حملت `=======` عارياً في `log.md`. رفض `--force-with-lease` دفعي، فقارنتُ نسختي بنسختها بدل أن أفرض — والمقارنة هي ما كشف العلامة.
- **الحارسان مرّا خضراوين:** الأوّل نمطه يطابق `=======$` لكنّ pathspec عنده بلا `*.md`؛ والثاني يتجاهل `=======` وحدها **عمداً** (مسطرة setext شرعيّة). عيبان مستقلّان يلتقيان على ثغرة واحدة — وهي أخطر أنواع الثغرات لأنّ إصلاح أحدهما لا يكشفها.
- **والصنف المكشوف هو الأكثر تعارضاً:** `sahool-brain/*.md` ملحقة، فتتعارض في كلّ إعادة تأسيس — أربع مرّات في هذه الجلسة. أي أنّ أكثر الملفّات تعرّضاً للتعارض كان أقلّها حراسةً.
- **الضرر صامت:** `=======` يُحوّل السطر فوقه إلى عنوان H1 في Markdown، فبند قائمة يصير عنواناً بلا أن يبدو الملفّ مكسوراً.
- **القياس قبل التوسيع:** صفر ملفّ `.md`/`.txt`/`.toml`/`.cfg`/`.ini` يحوي النمط اليوم ⇒ التوسيع بلا إنذار كاذب.
- **التكذيب بالحادثة عينها:** إعادة إدراج السطر ⇒ خروج `1` مع تسمية `log.md:4721`؛ والـpathspec القديم يقول «لا شيء». فالإصلاح pathspec لا نمط.
- **ولم أفرض على عمل غيري بلا سبب:** نسختها = نسختي + علامة التعارض + سطر فارغ + طابع زمنيّ أحدث. لا محتوى يضيع بأخذ نسختي، وفيها عيب لا يمرّ. لو كانت تحمل مدخلاً حقيقيّاً لَدُمِج لا لَأُهمِل.
- **حدّ صدق:** يمنع الالتزام بالعلامة ولا يمنع نشوءها. الجذر أنّ «أبقِ الجانبين» صحيح للمداخل المختلفة وخاطئ حين يكون أحد الجانبين نسخةً أقدم من المدخل نفسه — وهو ما كرّر إفساد `hot.md` قبله.

## 2026-08-01 — البروتوكول كان صحيحاً وترتيبه خاطئاً (#760)

- **الحادثة:** كتبتُ في `415fa520` أنّ إعلان الأثر يُحسَب **عند الدفع**، وأنّ تعديل المتن وحده لا يُعيد تشغيل البوّابة لأنّ `capability-governance.yml` يستمع لأنواع `pull_request` الافتراضيّة بلا `edited`. ثمّ نفّذتُ: حسبتُ ⇒ **دفعتُ** ⇒ حدّثتُ المتن. فقرأت البوّابة المتن **كما كان لحظة الدفع** وحجبت بـ`missing_direct: [IRR-009]`.
- **العلّة ليست في القاعدة بل في ترتيبها:** «احسب عند الدفع» جملة ناقصة. الصحيح **ثلاث خطوات مرتّبة**: (١) احسب على `origin/main` الحاليّة ⇒ (٢) **حدّث متن الـPR أوّلاً** ⇒ (٣) ثمّ ادفع. الدفع يلتقط المتن لحظته، فأيّ تصحيح بعده يصل متأخّراً دورةً كاملة.
- **لماذا يستحقّ التسجيل:** لأنّي **كنتُ أعرف الآليّة وكتبتُها**، وأخطأتُ التسلسل. قاعدة صحيحة بترتيب خاطئ تفشل تماماً كقاعدة غائبة — والفرق أنّها تُعطي إحساساً زائفاً بأنّ الدرس مُستوعَب. الدرس المُستخلَص: **البروتوكول الذي لا يُحدِّد ترتيبه ليس بروتوكولاً بل نيّة.**
- **وتصحيح ثانٍ في الاتّجاه نفسه:** الإعلان الزائد يحجب كالناقص (`unaffected_declared`) — فـ`IRR-009` سقط من المتأثّرين بعد دمجٍ ثمّ عاد بعد آخر. أي أنّ إعادة الحساب ليست مرّةً واحدة عند أوّل دفع بل **عند كلّ دفع تتحرّك فيه القاعدة**، صعوداً ونزولاً.

## 2026-08-01 — مادّة المفاتيح سرٌّ بقاعدة، والإعفاء إعلانٌ بدليله (#757، `6182d9d8`)
- **الأمر:** «قوم بتنفيذ» لبندٍ كنتُ قد امتنعتُ عنه وسمّيتُ سبب امتناعي: تصنيف مفاتيح `_KEY`. والمالك ردّ بصياغة الحدّ نفسه — `JWT_PUBLIC_KEY` يصحّ ألّا يكون سرّاً، فالأمر قاعدة تفصل مفاتيح التوقيع عن العامّة لا تعديل قائمة.
- **العطل:** `SECRET_MARKERS` تُفحَص **كسلاسل جزئيّة**، فيلزم الاسمَ أن يحوي `PRIVATE_KEY`/`API_KEY`/`ACCESS_KEY`. واللاحقة المجرّدة لا تطابق شيئاً ⇒ **عشرة مفاتيح توقيع وHMAC** نُشِرت تهيئةً عاديّة. عقدٌ يذكر مفتاح توقيع بجوار مستوى السجلّ **يُبلِّغ سطح الأسرار أصغر ممّا هو** — نفس شكل `RUNTIME-CONTRACT-INDIRECT-ENV-01`: بوّابة خضراء عن سؤال لم تطرحه.
- **حدس المالك عمّ أبعد ممّا قاله:** قراءة المستهلكين أظهرت إعفاءَين لم يكونا في الحسبان — `MFA_ALLOW_DERIVED_KEY` **راية منطقيّة** (`mfa_crypto.py:119` يقرأها نصّ صدق) و`STAGING_PROBE_IDEMPOTENCY_KEY` معرّف إلغاء تكرار **بقيمة افتراضيّة صريحة في المصدر** (`staging_probe.py:238`) وهو ما لا يفعله سرّ. قاعدة عمياء «اللاحقة ⇒ سرّ» كانت ستُخطئ فيهما معاً — وهذا وحده يبرّر الامتناع الأوّل.
- **ولماذا إعلان لا نمط أذكى:** `MFA_ALLOW_DERIVED_KEY` و`MFA_AUDIT_HASH_KEY` يفترقان بكلمة واحدة في الخدمة نفسها. نمطٌ يفصلهما يكون مُفصَّلاً على ثلاثة عشر اسماً معروفاً — **قائمةً في ثوب نمط** تنهار عند الرابع عشر. فالدليل قراءة المستهلك لا قراءة الاسم.
- **بوّابة الوحدات أمسكت اختباري أنا:** `test_runtime_contract_indirect_env` كان يُثبّت `DECISION_WORKER_ASSERTION_KEY` تهيئةً. أي أنّي حين استعدتُ المتغيّر من العمى في الشريحة السابقة، **سجّلتُ تصنيفه الخاطئ وأقفلتُه باختبار يحرسه**. سقوطه صحيح. **الدرس:** اختبارٌ يلتقط الحالة الراهنة يُثبّت خطأها معها ما لم يُسأل عمّا **يجب** أن تكون.
- **وعمى التكذيب وقع مرّتين في جلسة واحدة:** أوّل صياغة لاختبار الإعفاءات قرأت الأثر المُولَّد، فتعطيل الإعفاءات كلّيّاً **بقي أخضر** — نفس العمى الذي أصلحتُه صباحاً في `RUNTIME-CONTRACT-INDIRECT-ENV-01`، والثانية **وأنا أعرف الأولى**. القاعدة المستخلَصة: تكذيبٌ لا يمرّ بمسار الإنتاج ليس تكذيباً.
- **وإشعارا «اكتمال» لمكنستَين لم تجريا:** `b4gm3shhm` بقي في حلقة انتظار خلف PIDs بائتة و`r1.log` لم يُنشَأ قطّ. لو قرأتُهما نتيجةً لالتزمتُ على مكنسة لم تُنفَّذ. **صار الفحص: هل يكتب السجلّ فعلاً؟ لا هل وصل الإشعار؟**
- **والقاعدة تحرّكت ثلاث مرّات أثناء الشريحة** (#754 ← #756 ← #755)، وفي كلّ مرّة كشف حسابُ الإعلان **عند الدفع** التحرّكَ قبل أن تحجب البوّابة — البروتوكول المكتوب في `415fa520` مُطبَّقاً على نفسه لا مقتبَساً.
- **التحقّق:** ٣٩٢٢ ناجحاً · صفر فشل · تغطية 47.72% · مكنسة `FIX=0 VERIFY=0` · **٦٤ من ٦٤ فحصاً خضراء** (قُرِئت على أربع صفحات؛ أوّل قراءتين غطّتا ٤٩ فقط، وإعلان «الكلّ أخضر» عندها كان سيكون ادّعاءً عن ٧٦٪ من الدليل).
- **حدّ صدق:** يُغيّر ما **يُبلِّغه العقد** لا مكان تخزين القيم؛ لا نظام أسرار أُعيد ضبطه ولا برهان تشغيليّ حيّ.

## 2026-08-02 — ثلاث آليّات متتابعة لعطلٍ واحد متكرّر (فرع `claude/main-branch-review-uf4pdk`)
- **الأمر:** «ابداء بتنفيذ الكل بشكل متتابع» — تنفيذ الآليّات الثلاث التي اقتُرِحت جواباً على «كيف يمكن حل هذه المشاكل المكررة جذريا».
- **النمط الجامع المقيس، لا المُستشعَر:** ستّ حوادث في هذه الجلسة (ثلاث لي، وثلاث لجلسات أخرى في #750/#753/#758) شكلها واحد — **كاشفٌ يُعرَّف بنمطٍ لا تُقاس تغطيته هو نفسه**، فيُبلِّغ نتيجةً عن سؤال لم يطرحه. والخضرة في هذه الحالة **ليست خطأً في الجواب بل غياباً للسؤال**.
- **الشريحة ① `resolve_merge_conflicts.py` (`00f1bf1a`):** القواعد الثلاث كانت معروفة ومكتوبة ومُطبَّقة يدويّاً **خمس مرّات في الجلسة نفسها**، ثمّ نُقِضت في السادسة: أربعة ملفّات إلحاقيّة حُلَّت صحيحاً، ثمّ مرّت حلقة `checkout --theirs` على «المتبقّي» — والمحلولة لم تكن مُفهرَسة فبقيت في `--diff-filter=U` فدهستها. **خمس قطع عمل ضاعت وgit أبلغ دمجاً نظيفاً**، ولم يمسكها إلّا `brain_commit_claim_guard` مصادفةً. فالعطل في **ترتيب العمليّات** لا في المعرفة، والفهرسة صارت داخل تكرار الحلّ.
- **وعيبٌ ثانٍ أُمسِك أثناء البناء:** أيّ الجانبين هو `main` **ليس ثابتاً** — `--theirs` في الدمج و`--ours` في إعادة التأسيس (HEAD أثناءها هو المُنبَع). سكربت يُثبّت جانباً يكون صحيحاً في عمليّة وخاطئاً في الأخرى **بلا إبلاغ**، وهو صنف العطل نفسه الذي وُجِد السكربت ليمنعه.
- **الشريحة ② `claim_base_guard` (`d194f805`):** القياس يَبيت بحركة الشجرة تحته، والقرار لا يَبيت بها؛ وخلطُهما هو كيف يصير رقمٌ مقيس عقداً لا يُعاد قياسه. **الدليل حيّ:** `platform_extraction_map.json` يقول `baseline_route_count = 633` وقائمة `routes` فيه تعدّ **635** — انحرف عن القائمة التي يعدّها، **ولا حارس ولا اختبار ولا workflow يقرأ ذلك الحقل** (مقيس بالبحث، صفر نتيجة). لم يُصحَّح بتحرير الرقم: تصحيح قياس بلا إعادة اشتقاق يُنتِج رقماً آخر لم يحسبه أحد.
- **والمفردتان لم تُخترَعا:** `measured_on` و`adjudicated_on` كانتا في ٨ من ٢٢ مصنوعة، اختارها كاتبوها كلٌّ على حدة ولم تُسمَّ قاعدةً. التغطية توقّفت عند أقلّ من نصف كلّ صنف لأنّ ما لا يُسمّى لا يُقاس.
- **وقرارٌ مقصود في ②:** **البيات يُبلَّغ ولا يُحجَب عليه.** الحجب على تقادم رقم يُحوّل كلّ PR إلى حملة إعادة قياس ويُدرّب قارئه على تجاهل الحارس — أشيع أعطال الحرّاس في هذا المستودع. وأثمر التقرير فوراً: **ثلاثة من الأختام الخمسة القائمة تُسمّي شجرةً غير قابلة للحلّ** (رموز فروع ابتلعها الدمج squash) — كُتِبت صادقةً وصارت غير قابلة للتحقّق.
- **الشريحة ③ `guard_mutation_guard` (`76fb4f40`):** الحارس الأخضر يقول شيئاً واحداً بيقين — **لم يُطلِق**، و«لم يجد عطلاً» و«لا يرى العطل أصلاً» لا يفرّق بينهما شيء. **و«له اختبار» ليس دليلاً:** أكثر من ثلثي حرّاس المستودع لهم ملفّ اختبار وكان أخضر طوال المدّة التي كان فيها ثلاثة منهم عُمياً، لأنّ اختبار الحارس المعتاد يقيس أنّه **يمرّ على شجرة سليمة** — خاصّيّة يُحقّقها حارسٌ لا يفعل شيئاً على الإطلاق.
- **وشرطان لا واحد في ③:** «أخضر تحت عطل مزروع» إخفاق، و«أحمر بغير الاختبار المُسمّى» إخفاق أيضاً — لأنّ «سقط شيء ما» يمرّ على طفرةٍ كسرت الاستيراد لا القاعدة.
- **وسقطتُ في العطل نفسه داخل الآليّة المصنوعة لمنعه:** أوّل مواصفتين كتبتهما حملتا `expect: "test_"` — بادئة يُحقّقها أيّ انهيار — **ومرّتا خضراوين**. أي أنّ آليّة إثبات التغطية أثبتتها بشرطٍ لا يقيس شيئاً. صار `expect` يُطابَق بوجود `def <name>(` في ملفّ الاختبار، وصار ذلك الفحص نفسه تحت طفرة.
- **وعمى التكذيب للمرّة الثالثة في هذه الجلسة، وهي الأهمّ لأنّي كنتُ أعرف عنه:** أوّل اختبار للخاصّيّة المحوريّة في ① — «الفهرسة في نفس التكرار» — **مرّ أخضر تحت طفرته**. كان يُشغّل الكنسة المُتلِفة **بعد** عودة الدالّة، وعندها يكون كلّ شيء مُفهرَساً في الحالتين؛ فهو يقيس «هل انتهت الدالّة بفهرسة» لا «هل يبقى ملفّ محلول عارياً لحظةً». والخطر يقع **بين** الحلّ والفهرسة، فصار الاختبار يقطع الدفعة في منتصفها ويسأل عمّا حلّ بما سبق. **معرفةُ العمى لا تحصّن منه؛ الذي يحصّن هو زرع العطل.**
- **وحارسٌ من شريحة سابقة أمسك ملفّاتي أنا:** `test_text_encoding_locale` رفض ثلاثة من ملفّاتي الجديدة لأنّها تفكّ مخرَج `subprocess` بترميز الآلة. أُصلِحت قبل الالتزام ولم تُضَف إلى أساسه.
- **التحقّق:** ٤٠٦٠ ناجحاً · صفر فشل · `ruff check/format` نظيفان · مكنسة `FIX=0 VERIFY=0` · `brain_state_transition_guard` و`conflict_marker_guard` و`claim_base_guard` و`guard_mutation_guard --run` (**١٨ طفرة مزروعة ومُشغَّلة**) كلّها خضراء محلّيّاً.
- **حدّ صدق لكلّ شريحة:** ① **يُتاح ولا يُفرَض** — لا حارس يمنع حلّاً يدويّاً ولا شيء يقيس أنّ سجلّاً إلحاقيّاً لم ينقص عبر دمج · ② يحرس مصنوعات `docs/architecture/` المُعلَنة وحدها، وادّعاءات نثر الدماغ أكثر منها بفارق كبير وخارج مداه · ③ خضرته تعني «**ما قِيس** يُطلِق» لا «كلّ حارس يُطلِق»: ١١٧ من ١٢٠ لم تُقَس تغطيتهم، والآليّة تمنع نموّ العدد ولا تُقلّصه من تلقاء نفسها.

## 2026-08-01 — دليل تشغيل حيّ للوكيل المحلّيّ، وثلاثة أشياء صحّحها القياس أثناء كتابته
- **الأمر:** دليل Markdown بأوامر حيّة قابلة للنسخ واللصق، يُنفّذها الوكيل المحلّيّ على الخادم لمعالجة ما تبقّى من فجوات.
- **هذه الشريحة توثيق لا برهان.** لا تُغيّر حالة أيّ فجوة، ولا تُنتج دليلاً تشغيليّاً — تصف كيف يُنتَج. حارس انتقال الحالة رفض صيغتي الأولى بحقّ لأنّها استعملت مفردات الإقرار في ملفّ دماغ بلا شيفرة تنفيذيّة معها؛ صياغةُ الادّعاء صارت تصف ما هي عليه.
- **لم أكرّر القائم:** `REAL_ENV_VERIFICATION_RUNBOOK.md` يصف الرفع والبيئة؛ الناقص كان إجراءً لكلّ فجوة على حدة مع حزمة أدلّة. فالدليل الجديد يبني عليه ولا يستنسخه.
- **حزمة الأدلّة سبعة عناصر** (SHA · image digests · حالة الهجرات · الأمر حرفيّاً · stdout+stderr · طوابع زمنيّة · حكم صريح)، وأيّ عنصر ناقص يُبقي الفجوة على حالها. النجاح على الشاشة ليس برهاناً؛ البرهان أن يُعيد غيرُك إنتاجه.
- **تصحيح ادّعاء في تقرير خارجيّ:** قال إنّ موضع مصادقة MCP يُعيد 400 بدل 401؛ الشيفرة تُظهر `HTTPBearer(auto_error=False)` فالغياب يصل الدالّة وترفع **401**. كُتِب في الدليل «قِس ولا تفترض».
- **وأربعة أسماء جداول كتبتُها كانت مُختلَقة** — لا وجود لـ`execution_requests` ولا `actuator_receipts` ولا `decision_reservation_dispatch_inbox` ولا `evidence_snapshots`. استُبدِلت بالمقيسة من `migrations/`: `execution_ledger` · `as_applied_irrigation_receipts` · `actuator_command_outbox` · `field_evidence_snapshots`. وأُضيفت خطوة اكتشاف إلزاميّة من `information_schema` قبل أيّ استعلام، لأنّ استعلاماً على جدول غير موجود يُنتِج خطأً يُقرأ «لا بيانات».
- **وأثمن ما كشفه القياس:** `correlation_id` غائب عن `decision_record` و`execution_ledger` والإيصالات و`approvals` — موجود في جداول الحجز و`workflow_state` فقط. أي أنّ الجسر مفقود **في المخطّط نفسه**، وهو تأكيد للفجوة لا نفي لها. فبُدِّل سؤال الكتلة من «هل تمرّ السلسلة؟» إلى «إلى أين يصل المعرّف وأين ينقطع؟»، وكُتِب صراحةً أنّ الوصول اليوم جزئيّ وأنّ معالجته تحتاج هجرة تُضيف العمود.
- **وكذلك `snapshot_digest`:** مفهوم في `services/decision-service/main.py:332` لا عمود في المخطّط، فصار إثباته من **الخدمة** (طلبان بنسختَي سياسة، والبصمة تبقى واحدة) لا من SQL.
- **الفرع أُعيد تأسيسه من `main` لا فوق تاريخ مدموج:** الـPR السابق دُمِج بالضغط، وقياس الفرق أظهر أنّ البقاء على التاريخ القديم كان **سيتراجع** عن عمل #760 (حارس ووحدة اختبار). إعادة التأسيس ليست نظافة هنا بل منع تراجع.
- **حدّ صدق مكتوب في الدليل نفسه:** لا يعالج الفجوتين المعالَجتين ساكنتين (التشغيل الحيّ لهما تأكيديّ)، ولا بقيّة طبقات الأهليّة (تحتاج حقولاً لا تحملها البنية)، ولا يقلب مصدر الحقيقة للقرارات (قرار مالك بستّ بوّابات)، ولا يرفع أعلام الإقرار التشغيليّ تلقائيّاً.
## 2026-08-02 — «قابل للنسخ» ليست صفة تحريريّة بل خاصّيّة تُختبَر
- **الأمر:** اجعل دليل التشغيل الحيّ قابلاً للنسخ من شاشات المحادثة.
- **لم أُعامله طلباً تجميليّاً:** الصفة تعني أنّ الكتلة **تعمل كما هي عند لصقها**. ففحصتُ ما يمنع ذلك فعلاً فوجدتُ ثلاثة أعطال حقيقيّة، لا واحداً.
- **① كتلة شيفرة داخل اقتباس:** كتلة الاكتشاف كانت مكتوبة بـ`> ` في كلّ سطر، فنسخها ينقل علامات الاقتباس ولا يعمل الأمر. أُخرِجت من الاقتباس.
- **② heredoc مُقتبَس يُبطِل التوسيع:** `python - <<'EOF'` مع `$EV` في متن بايثون — والاقتباس يمنع توسيعه، فيفتح بايثون مساراً حرفيّاً اسمه `$EV/...` **وتفشل الكتلة بصمت**. العلاج تمرير المسار وسيطاً (`python - "$EV"` + `sys.argv[1]`) لا داخل النصّ.
- **③ وأخطرها: كتل بلا حرّاس.** ٦٦ استعمالاً لـ`EV` وخمسة لـ`FIELD_ID` بلا أيّ شرط. كتلةٌ تُلصَق في صدفة جديدة كانت ستكتب الأدلّة في جذر النظام، وأخرى ستطلب `/fields//canonical-state` فتعود بردٍّ **يُقرأ نتيجةً وهو خطأ مسار**. أُضيف `: "${VAR:?...}"` إلى ٣٢ كتلة: الفشل الصاخب أأمن من النجاح في المكان الخطأ.
- **وأُضيفت كتلة إعداد واحدة (0.0)** تُملأ مرّة وتُعاد عند كلّ صدفة جديدة، فلا يبقى المتغيّر مسألة ذاكرة.
- **التحقّق بالتشغيل لا بالقراءة:** `bash -n` على **٣٤ كتلة ⇒ صفر خطأ نحويّ** · وتكذيب مباشر: تشغيل كتلة بلا `EV` يخرج بـ**1** برسالة تسمّي المتغيّر · وتشغيل كتلة الـheredoc فعليّاً بمُدخَلات اصطناعيّة ⇒ خرجت بصفر وطبعت الحكم وكتبت ملفّ الدليل.
- **وعطل أدخلتُه أنا في هذه الشريحة نفسها، ثمّ أمسكتُه:** تمريرة الحرّاس وضعت `: "${EV:?…}"` على كتلة **0.1** — وهي التي تُنشئ مجلّد الأدلّة وتُصدِّر `EV`. فصارت نقطة الدخول مستحيلة التشغيل: تُلصَق في صدفة نظيفة فتخرج بـ1 مطالِبةً بمتغيّر كانت على وشك تعريفه. السبب أنّ سكربتي يتخطّى الكتلة إن كان الإسناد في بداية سطر، و0.1 تُسنِده **وسط** أمر مركّب. وتركت التمريرة حارساً مكرّراً في كتلة الاكتشاف. الحارس الذي يمنع الخطوة التي ينتمي إليها يُدرِّب المُشغّل على حذف الحرّاس — فرُفِع عن bootstrap وحده وبقي على **٣١ كتلة مستهلِكة**، لا حُذِف.
- **مُثبَت بالتشغيل:** كتلة 0.1 في صدفة نظيفة داخل مستودع git فارغ ⇒ خروج **0** وطباعة مجلّد الأدلّة · صفر حارس مكرّر · `bash -n` على ٣٤ كتلة ⇒ صفر خطأ.
- **وإنذار كاذب من ماسحي أنا:** فحصي عن heredocs تُبطِل التوسيع أبلغ عن موضعين، وكلاهما من **تعليقي الشارح** الذي يذكر `<<'EOF'` نصّاً. قرأتُ الموضع قبل أن أُصلح شيئاً — الماسح الذي يُنذر على شرحه لنفسه يُدرَّب قارئه على تجاهله.

## 2026-08-02 — حزمة خارجيّة أغلقت ثغرة مصادقة، وصحّحت استنتاجاً لي
- **الأمر:** «أدخلها في شريحة مستقلّة على نفس الفرع» — حزمة رقعة خارجيّة لثلاث فجوات جذريّة.
- **لم أُدخِلها على الثقة:** طبّقتُ **اختبار العقد وحده** على شجرة `main` غير المُرقَّعة فسقط: `assert 422 == 401`. مُستدعٍ **بلا مصادقة** يُرسل JSON مُشوَّهاً فيحصل على 422 **مع تفصيل خطأ التحليل** (`loc` · `msg` · `ctx`) — أي أنّ تحليل الجسم يسبق المصادقة، وتُسرَّب بنية الطلب لمن لم يُصادَق. العطل حقيقيّ ومقيس.
- **وهذا يُصحّح استنتاجاً لي:** كتبتُ سابقاً أنّ ادّعاء تقرير خارجيّ («يُعيد 400 بدل 401») غير مسنود، لأنّ `HTTPBearer(auto_error=False)` يجعل الموضع يرفع 401. كنتُ محقّاً في **ذلك السطر** ومخطئاً في الاستنتاج: السطر **لا يُنفَّذ أصلاً** حين يكون الجسم مُشوَّهاً. العَرَض كان حقيقيّاً عبر مسار آخر، وكتلة MCP في دليلي كانت تفترض أنّ المصادقة تجري أوّلاً فلم تختبره.
- **شيئان أصلحتُهما لم تدّعِهما الحزمة، وصدقُها في ذلك محسوب لها:** ① خطأ ترتيب استيراد يُسقِط `ruff` وكان سيحجب وظيفة Lint (قالت صراحةً «ruff غير متاح ولم أدّعِ تشغيله»). ② جرد المسارات صار بائتاً لأنّ الرقعة غيّرت مسارات `generic_context_server` — قِستُ الفرق على الشجرتين (يمرّ على `main`، يسقط بعد الرقعة) بدل التخمين، ثمّ أُعيد التوليد.
- **التكذيب على فرعي:** إعادة `oauth_middleware.py` وحده إلى نسخة `main` ⇒ يسقط اختبارا العقد كلاهما؛ والاستعادة تُعيدهما خضراء. فالإصلاح هو الوسيط لا شيء آخر في الرقعة.
- **القياس:** `pytest -m unit` **4072** · حرّاس العلامات وشجرة الاختبارات وخريطة القدرات ووصل الاختبارات المعماريّة وحضور التأكيدات كلّها صفر · `bandit` عالي صفر · `ruff` نظيف.
- **حدّ صدق:** هذا يُغلق مسار «المصادقة قبل تحليل الجسم» لنقطة السياق العامّة، ولا يُغلق مصفوفة حالات MCP كاملةً (403 لصلاحيّة ناقصة · 2xx لمسموح) — تلك تبقى في كتلة الدليل وتحتاج بوّابة حيّة.

## 2026-08-02 — الحزمة هبطت على main، ووُثِّقت آليّة CI لمنع تكرار الإخفاقات
- **الأمر:** «قوم بالاشتراك الآليّ النشط مع توثيق آليّة CI والفحوصات لتجنّب أيّ فشل مستقبليّ».
- **الاشتراك أُغلق بنتيجة لا بانتظار:** #769 اكتملت فحوصها الـ**٦٥** بلا إخفاق (آخرها *Unit Tests*)، ودُمِجت squash إلى `54a91d8e`. الوظيفتان اللتان أسقطتا المحاولة السابقة (*Lint & Format* و`guard`) خضراوان.
- **التوقيع — وتصويب على قراءتي:** قرأتُ `%G?` أوّلاً على أنّه مقياس وجود توقيع، فقلتُ إنّ `E` توقيع حاضر و`N` غيابه. الفحص المباشر يكذّب ذلك: التزامي المحلّيّ يطبع `N` **وترويسته `gpgsig -----BEGIN SSH SIGNATURE-----` حاضرة**. الحرفان معاً يعنيان «تعذّر التحقّق» لا «بلا توقيع» — `gpg.ssh.allowedSignersFile` غير مضبوط والمفتاح العموميّ صفر بايت. المميّز الصحيح `git cat-file commit HEAD | grep '^gpgsig'`، والتحقّق موضعه GitHub. لا عيب جديد يُسجَّل، والوثيقة صُحّحت (§٣.١٣) لئلّا تُعلّم فحصاً خاطئاً.
- **الوثيقة:** `docs/runbooks/CI_GATES_AND_PRE_PUSH_PROTOCOL.md` — خريطة الوظائف الحاجبة بملفّاتها وأسطرها · كتلة واحدة قابلة للّصق تُعيد إنتاجها محلّيّاً · ثلاثة عشر صنف فشل، كلٌّ منها أسقط بناءً حقيقيّاً في هذا المستودع. ومدخلها من `CLAUDE.md` فتُقرأ في بداية كلّ جلسة لا عند تذكّرها.
- **ثلاثة أخطاء صدفة موثّقة في ثماني أسطر الدالّة المساعدة وحدها**، وكلّها وقعت فعلاً هنا: `[ $rc -ne 0 ] && …` كآخر تعليمة تُعيد ١ عند النجاح · `"$@"; rc=$?` بلا `||` يقتل السكربت تحت `set -e` عند أوّل فشل · `if "$@"; then …; fi; rc=$?` يقرأ حالة جملة `if` لا الأمر فيطبع `0` لفحص فاشل. الثالث **مقيس**: `bash -c 'exit 4'` يُبلَّغ `fail (0)` بالصيغة الخاطئة و`fail (4)` بالصحيحة.
- **ادّعاء الوثيقة عن `no_report_only_change_guard` مُكذَّب لا مفترَض:** نفس المحتوى باسم يحمل `CHECKLIST` خارج `docs/runbooks/` ⇒ يُحجَب (rc=1)؛ وفي موضعه ⇒ يمرّ (rc=0).
- **وثغرة كشفها التحضير نفسه:** `brain_state_transition_guard` مفرداته إنجليزيّة وحدها، والسجلّ يكتب حالاته بالعربيّة. المقيس: ١٠٠ عنوان فجوة، **١٦** بحالة إغلاق عربيّة، يلتقطها الحارس **٠**. سُجِّلت `BRAIN-TRANSITION-GUARD-BLIND-TO-ARABIC-STATUS-01` مفتوحةً، **ولم يُوسَّع النمط**: توسيعه بالكلمة المجرّدة يُعيد إنتاج الإيجابيّ الكاذب المُصلَح سابقاً (٤٠ سطراً تصف «يفشل مُغلَقاً» تصميماً لا حالة). والاتّجاه الصحيح ربط المطابقة ببنية العنوان لا بالكلمة.
- **وإقرار مقصود:** تحديث حالتي الفجوتين في هذه الشريحة يمرّ من ذلك الحارس **بسبب تلك الثغرة**. ما يجعله صادقاً هو `54a91d8e` المدموج وفحوصه، لا خُضرة الحارس — ومُدوَّن في السجلّ حتّى لا يُقرأ المرور شهادةً.
- **تصحيح بيات:** عنوان `TEXT-DECODED-WITH-THE-MACHINES-LOCALE-01` كان يقول `baseline 188` بعد أن صار **184**؛ التصحيح كان في متن المدخل لا في العنوان، والعنوان هو ما يُقرأ.

## 2026-08-02 — الجذر لا العَرَض: حتميّة المصنوعات وأمان دمج الملفّات الإلحاقيّة
- **الأمر:** «كيف يُحلّ هذا جذريّاً وضماناً لعدم التكرار — ابحث في الويب عن الحلّ الكامل وفق أفضل الممارسات»، ثمّ مراجعة المالك التي اعتمدت L0+L2+L3+L4 وأجّلت L1.
- **البحث أعطى نتيجة حاسمة تُبطِل الوصفة الشائعة:** كلّ المصادر توصي بـ`merge=union`، و**GitHub يتجاهل `.gitattributes` المُعرَّف من المستخدم** (ردّ دعم صريح، المطلب مفتوح منذ 2021). فأيّ إجابة تقف عند union تُصلح سطر الأوامر ولا تُزيل لافتة التعارض. سُجِّل هذا حدّاً في `.gitattributes` نفسه لا في وثيقة بعيدة.
- **القياس فصل جذرين:** لاحتميّة المصنوعات (البيان يكتب ساعة الحائط بدقّة الميكروثانية ⇒ تعارض مضمون بين فرعين متوازيين) وتعدّد الكتّاب على أربعة ملفّات دماغ (٢٧٣ من ٢٨١ لمسة).
- **تصحيح رقم لي:** «٧٤ من ١٢٨ ختماً فقط» كان تقريباً خشناً. الأدقّ: **٣** غيّرت `generated_at` وحده، ٧١ غيّرته مع حمولة حقيقيّة، ٥٢ محتوى آخر. المكسب الصحيح: إعادة التوليد بلا تغيّر حمولة تُنتج **صفر فرق**.
- **مُثبَت:** ثلاث توليدات تحت منطقتين زمنيّتين ولغتين ومصدرَي زمن ⇒ نفس SHA-256.
- **وحقل يسرّب الآلة حُذِف:** `source_root` كان يطبع اسم مجلّد السحب (قِيس: كتب `wt768` داخل شجرة عمل) وله صفر قارئ.
- **الحارس صُحِّح تصميمه بالقياس قبل أن يُكتَب:** «معرّف مكرّر ⇒ فشل» كان سيرفع ١١ إصابة، **عشرٌ منها سلاسل تاريخيّة مقصودة**. القاعدة الصحيحة **التلاصق** (بصمة ضمّ union لنسختَي سطر واحد). وأوّل إصابة حقيقيّة كانت **قائمة في الشجرة قبل وصل الحارس** (`BRANCH-GRAVEYARD-POLICY`).
- **وتكذيب فاشل، وقعتُ فيه داخل الحارس المصنوع لمنع صنفه:** أربع طفرات على المرساة تركت ١٦ اختباراً خضراء، لأنّ اختبار «الذكر النثريّ» استعمل سطراً بلا `##` أصلاً — يوثّق النيّة ولا يقيسه. أُعيدت صياغته على الحالتين الحسّاستين (عنوان مُقتبَس، وعنوان يسبقه نصّ) فصارت الطفرة المزدوجة تُسقِط اختباراً. هذا ثاني وقوع في نفس الصنف خلال جلسة واحدة (الأوّل: اختبار `--global` عاقب التعليق الشارح).
- **مؤجَّل بقرار المالك:** `BRAIN-FRAGMENTED-SOURCE-OF-TRUTH-01` — الاتّجاه معتمَد والتصميم شريحة مستقلّة، لأنّه يُغيّر مصدر الحقيقة وبروتوكول الكتابة لا مجرّد آليّة دمج.

## 2026-08-02 — حزمة خارجيّة أغلقت لا-تناظر بين بانيَي رابط، ونفّذتُ تكذيبها الذي عجزت عنه
- **الادّعاء:** `fieldCdseThumbnailUrl` بلا `source=persisted` ⇒ مسار حيّ ⇒ `TRANSPARENT_PNG` بحالة **200** ⇒ `<img>` يُطلِق `onLoad` ⇒ بطاقة `ready` وهي بكسل شفّاف. **مُتحقَّق منه في الشجرة**، لا مقروءاً من التقرير.
- **الفجوة عدم تناظر لا عيب عقد:** الخادم يبني الرابط بـ`&source=persisted` منذ `IMAGERY-BLANK-THUMBNAIL-01` (`field_workspace_imagery.py:132`)، والعميل لم يُحدَّث.
- **التكذيب الذي لم تستطعه الحزمة:** بيئتها بلا `node_modules` وفشل `npm ci` على مرآة داخليّة. هنا `vitest` متاح: **اختباراتها وحدها** على المصدر غير المُرقَّع ⇒ **٣ تسقط** بالرسالة الصحيحة · بعد الإصلاح ⇒ **٢٦ تمرّ** · الجناح الكامل **١٣٠٠/١٩٤ ملفّاً** أخضر.
- **مسحتُ المستهلكين قبل التعميم** بدل قبول «كلّهم أسطح تاريخيّة»: اثنان إنتاجيّان، وكلاهما يُمرّر تاريخاً صريحاً؛ MapHub لا يبني الرابط إلّا حين `has_cog`، وSatellitePage احتياطيّ لرابط خادم يحمل `persisted` أصلاً — فالإصلاح يجعله متناظراً لا مُنحدِراً.
- **وشكٌّ فحصتُه فسقط:** ظننتُ `services/raster-service/test_persisted_thumbnail_path.py` غير موصول بـCI لأنّ `testpaths = tests_v9`. القياس نفاه: `raster_quality_gate.sh` يُشغّل `pytest` من داخل مجلّد الخدمة فيجمع **٣٠٣** اختباراً منها هذا. فحصتُ بدل أن أُسجّل فجوة وهميّة.
- **وتلوّث منّي، مُسجَّل:** عدّلتُ الشجرة (نسخ ملفّات الحزمة) **أثناء** تشغيل `verify_all_generated`، فبلّغت «حارس كتب أثناء الفحص» عن تغييري أنا لا عن حارس. أُعيدت على شجرة مستقرّة. نفس درس الشجرة الملوَّثة المسجَّل سابقاً، وقعتُ فيه ثانيةً.
- **وانحراف كشفته CI على #770 لا أنا:** `integration_runtime_governance_closure` — بيان جزئات يُجزّئ `runtime_environment_preflight.json` الذي غيّرتُ ختمه في L0. أثر صحيح لسلسلة صحيحة؛ فاتني لأنّي **أوقفتُ المكنسة الكاملة** لبطئها. الدرس: المكنسة تُشغَّل كاملةً أو لا يُدّعى أنّها شُغِّلت.

## 2026-08-02 — «تحقّق» كشف أنّ ادّعائي في نصّ مدموج كان أوسع من المقيس
- **الأمر:** «تحقّق» بعد دمج #770. لم يكن استعراضاً: أعدتُ توليد بيان الإصدار على `main` النظيف فانحرف.
- **ما تحقّق:** الملفّات السبعة موجودة · شجرة فرعي مقابل `main` **فرق صفر** · ٤٠ اختباراً جديداً أخضر · `5281 checksum` · كلّ البوّابات 0.
- **وما لم يتحقّق — وهو ادّعائي:** «إعادة التوليد بلا تغيّر حمولة ⇒ صفر فرق». ربطتُ الختم بـ`HEAD`، و`HEAD` يختلف بين فرعين، فبقي الفرعان يكتبان قيمتين مختلفتين في نفس السطر.
- **ولماذا أفلت:** اختبار القبول يُثبّت `SOURCE_DATE_EPOCH` صراحةً فلا يعبر حدّ الالتزام أبداً. **رابع** وقوع في صنف «اختبار يقيس ما في ذهن كاتبه» في جلسة واحدة.
- **الإصلاح:** نطاق **الحمولة** بدل `HEAD`، مُشتقّاً من `EXCLUDE_DIRS` و`SELF_GENERATED` نفسيهما لا من قائمة ثانية تنحرف. مُثبَت على مستودع بفرعين: `HEAD` ⇒ قيمتان مختلفتان · الحمولة ⇒ قيمة واحدة. والتكذيب: `scope = []` يُسقِط الاختبار المُسمّى وحده.
- **وحدٌّ مقابل:** اختبار يُثبِت أنّ تغيّر الحمولة **يُحرّك** الختم — وإلّا مرّ إصلاح يُعيد ثابتاً دائماً.
- **وتصنيف صحّحتُه:** حاولتُ تسجيل الطفرة في `guard_mutation_registry.json` فرفضها الحارس: `deterministic_time.py` **مكتبة لا حارس**. الرفض صحيح؛ موضع التكذيب هو الاختبار المُسمّى وسلسلة التوثيق.
- **ومسبارٌ صمّمتُه خطأً مرّتين قبل أن يصحّ:** ① قِستُ «قبل/بعد التزام دماغ» وأنا قد صنعتُ للتوّ التزام حمولة، فخلطتُ الأثرين وقرأتُ فشلاً كاذباً. ② ثمّ شغّلتُ البناء بـ`SOURCE_DATE_EPOCH=` — سلسلة فارغة **مضبوطة** لا غائبة — فرفضها العقد وانهار البناء، وابتلعتُ رمز الخروج بـ`>/dev/null 2>&1`، فبدت الشجرة «مستقرّة» وهي لم تُبنَ. القاعدة التي وثّقتُها بنفسي عن رموز الخروج، خالفتُها.
- **وخاصّيّة بنيويّة كشفها المسبار الصحيح:** المصنوعة لا تُطابق إعادة توليدها على التزامها هي (تُولَّد قبله)، ومطاردة نقطة ثابتة بـ`--amend` لا تنتهي. غير ضارّ مقيساً — لا فاحص لهذا التطابق — لكنّه مكتوب الآن في العقد وفي الفجوة بدل أن يُكتشَف لاحقاً.

## 2026-08-02 — تقرير اعتماد خارجيّ شخّص مِسبار اختبارنا على أنّه عيب إنتاج
- **الادّعاء:** NO-GO بـ**١٩ إخفاقاً**، سببها الجذريّ «تغيير غير محكوم أضاف `/api/probe-newservice/readyz` بلا مصادقة».
- **القياس الأوّل نفاه:** المسار **غير موجود على `main`**، و`git log --all -S` يُعطي التزاماً واحداً هو التزام الاختبار الذي يُعرّفه. المِسبار من `tests_v9/test_api_versioning_policy_guard.py`، يُحقَن في مصدر متعقَّب ويُستعاد في `finally`.
- **وأعدتُ إنتاج الحادثة:** `timeout -s KILL 3 pytest <الاختبار>` ⇒ `M compat_gateway.py` + ثلاثة جرود منحرفة. الملفّات الثمانية التي وصفها التقرير «شجرة غير نظيفة» هي أثر جناحنا لا أثر مطوّر.
- **والخطر أكبر من الاتّساخ:** التقرير أوصى بإضافة المسار إلى `PUBLIC_ALLOWLIST` أو تصنيفه — أي **تحويل مِسبار اختبار إلى عقد دائم** في سطح API العامّ. تشخيصٌ واثق وخاطئ أسوأ من فشل صاخب.
- **العلاج:** المِسبار إلى ملفّ غير متعقَّب (الحارس يقرأ بـglob وAST فلا يفرّق)، + `probe_leak_guard` يُسمّي ما يبقى ويمنع العلاج الخاطئ في رسالته. مقيس: نفس المقاطعة ⇒ صفر مصدر متعقَّب متأثّر.
- **وما تحقّق من التقرير فسُجِّل فجوات:** `raster-service` بلا `SAHOOL_GIT_SHA` (ثلاث خدمات فقط تحقنه) · `/readyz` و`/runtime-identity` بلا `location` في nginx فيسقطان إلى SPA بـ200 وHTML.
- **وما لم يكن عيباً:** `production_certified=0/81` **ثابت صدق يفرضه CI** (`grep -F "production_certified: 0"` في `capability-governance.yml`) — رفعُه بلا دليل يُفشِل البناء، فليس فجوة تُغلَق بإعادة توليد. و«لا جدول migrations» — `migrations/MANIFEST.txt` هو المصدر القانونيّ كما ذكر التقرير نفسه. و`/healthz` **معرَّف** في `frontend/nginx.conf:23`.
- **وذيل للحادثة، وقعتُ فيه أنا:** اختبار الحارس الذي كتبتُه لمنع تسريب المِسبار **سرّب هو نفسه** — كتب في `api_versioning_inventory.csv` المتعقَّب واستعاده بـ`write_text`، و`read_text` يُترجم `\r\n` إلى `\n` فالاستعادة مُفقِدة. النتيجة: ١٠٧١ سطراً «تغيّرت» بصفر تغيّر محتوى، وجزئة الملفّ تفشل. **أمسكته CI لا أنا.** العلاج: `leaks(root=...)` على مستودع مؤقّت — لا يُمَسّ متعقَّب أصلاً. والدرس مكرّر: «استعادة» بـنصّ ليست استعادة؛ البايتات وحدها تُستعاد بأمانة، أو لا تُمَسّ الملفّات أصلاً.
- **الهبوط والتأكيد المقيس (`b83c4ecf`، #772):** الفحوص **٦٤ من ٦٤ خضراء**. والمهمّ أنّ الفحصين اللذين أسقطتهما الجزئة البائتة — *Repository Tests (tests/)* و**خطوة `verify_all_generated` داخل `capability-registry`** — أخضران على الالتزام الهابط، وهو الشرط الوحيد الذي يُصدّق الإصلاح. **وسطح الفشل كان أوسع ممّا رأيتُه أوّلاً:** الخطوة الثانية تسقط **بعد** أن تُبلِغ الاثنتان والستّون خطوة `--check` كلّها `✓`، أي أنّ التحقّق النهائيّ من حزمة الإصدار هو مَن يمسك هذا الصنف لا أيّ حارس مفرد — سطرٌ واحد بائت في مصنوعة مولَّدة يمرّ من كلّ حارس محتوى ولا يمرّ من الجزئة.
- **وتصحيح لحكمي على السجلّات المُبلَّغة:** السجلّات التي وصلتني بختم 17:10–17:17 كانت من `c33ab9d4` المُتجاوَز، وجولة `50f422d0` بدأت 17:35. **ولم أُعامِلها مُتجاوَزة بالافتراض:** الفحصان أُعيد قياسهما على الالتزام الجديد قبل أيّ ادّعاء إصلاح — لأنّ «هذا سقط على SHA قديم» تخمينٌ ما لم يُقَس على الجديد، وهو بالضبط شكل الخطأ الذي أنتج تقرير v22.
- **وذيلٌ ثانٍ للجلسة، أطلقه حارسٌ عليّ فكشف عطلاً فيه:** التزام تصحيح الحالات البائتة رفضه `brain_state_transition_guard`، والسطر المُطابَق يقول حرفيّاً إنّ `production_certified=0/81` **ليس عيباً**. `CLOSED_RE` يطابق الرمز في أيّ موضع من سطر مُضاف، فذِكرُ اسم الحقل نفياً يُقرأ ادّعاءً. سُجِّلت `BRAIN-TRANSITION-GUARD-MATCHES-A-QUOTED-STATUS-TOKEN-01` **مفتوحةً** ولم يُعدَّل الحارس هنا — تغيير مرساة حارس حاجب يحتاج مواصفة طفرة وحزمة تكذيب خاصّة، وخلطه بتصحيح سجلّ يجعل الرجوع عن أحدهما رجوعاً عن الآخر. ومرّ الالتزام بحمل `tests_v9/` فيه، وهو ما يطلبه الحارس فعلاً.
- **واختبارٌ جديد وُلِد من الحادثة، وتكذيبه الأوّل فشل:** `tests_v9/test_gap_landing_claims_name_a_real_commit.py` يجعل «هبطت في `<sha>`» ادّعاءً قابلاً للتكذيب (التزام موجود + سلف لـHEAD). **أوّل صياغة تخطّت عند أيّ `cat-file` فاشل** بحجّة الاستنساخ الضحل — والطفرة المزروعة (`deadbeef`) أعطت **نفس الإشارة بالضبط** فتُخطّى بدل أن يُدان: أي أنّ الاختبار كان أخضر عن **الحالة الأساسيّة التي بُني لها**. العلاج: الضحالة تُقاس بـ`rev-parse --is-shallow-repository` لا تُستنتَج من فشل البحث. وقياسه احتاج نسخة غير ضحلة، فهذه النسخة ضحلة (173 التزاماً). التكذيبات الأربعة تُسقِط الآن: SHA مختلق · التزام غير سلف · حذف كلّ الادّعاءات (عمى الحارس) · عنوان يجمع الهبوط والترشيح.
- **ودرس أداة مكرّر بشكل جديد:** `ruff` من البيئة (0.16.0) أعطى **٢٦ ملفّاً «would be reformatted»** لا يمسّ أيٌّ منها تغييري — 0.16 تُنسّق كتل الكود داخل Markdown (3716 مفحوصاً مقابل 2915). بـ`ruff==0.15.8` المثبَّت في CI: **صفر**. الرنبوك يُثبّت الإصدار في كتلته، والخطأ أنّي شغّلتُ ما في مساري بدلها؛ أُضيف إلى §٣.٣ الاتّجاه المعاكس صراحةً — **أحمرُ محلّيّ لن يُنتِجه CI أبداً** يدعو إلى إصلاح ما لم يُطالَب به أو إلى الشكّ في خطّ أنابيب أخضر.
- **وإخفاق لم يتكرّر، مُسجَّل بلا تفسير:** `guard_mutation_guard --run` أخفق مرّة على `claim_base_guard.py[4]` («حمرّ بغير الاختبار المُتوقَّع»)، ثمّ مرّ تشغيلين تاليين على نفس الشجرة، وزرعُ الطفرة يدويّاً أحمرَّ الاختبار **الصحيح**، وعلى `origin/main` في شجرة عمل منفصلة rc=0. سُجِّل `GUARD-MUTATION-RUN-FLAKED-ONCE-UNREPRODUCED-01` مفتوحاً. **ولم أكتب سبباً لم أقِسه** — الفرضيّات متاحة وغير مقيسة. ويُسجَّل لأنّ `--run` بوّابة حاجبة (`ci.yml:548`)، وفحصٌ يخضرّ بإعادة التشغيل يُدرّب قارئه على إعادة التشغيل بدل القراءة، فيُطفَأ بلا تعديل سطر.

## 2026-08-02 — هويّة صورة raster موصولة كاملةً، ومنسّق جاهزيّة بخمسة إصلاحات مقيسة

- **المصدر:** حزمة خارجيّة أصلحت أربعة أسطح لهويّة raster، ولم تصل إلى المستودع (قِيس: `Dockerfile` بلا `ARG SAHOOL_GIT_SHA`، و`main.py` بلا النقطة، وملفّ الاختبار الجديد غير موجود). فما قِيس هو **العقود التي يصطدم بها التنفيذ**، ثمّ نُفِّذ هنا كاملاً.
- **ثلاثة مخاوف بنيويّة سقطت بالقياس، ولم تُكتَب مخاطرَ بلا سند:** عقد الموضع يمسح `services/sahool-platform` وحدها (`platform_route_placement_guard.py:49`) فلا تصادم مع `uniqueness: exactly_one` · وميزانية المسارات تعدّ مسارات المنصّة فقط فبقيت **627/629** بلا تغيير · و`_classify` يُصنّف **بالمسار لا باسم الدالّة** فصار `/runtime-identity` لراستر `infra` تلقائيّاً — صفٌّ واحد في الجرد.
- **والنمط ليس جديداً:** نفس `ARG`/`LABEL`/ملفّ الهويّة قائم حرفيّاً في weather وsoil وplatform. أي أنّ المطلوب تمديد عقد مُختبَر إلى خدمة رابعة لا اختراع عقد.
- **انحراف واحد مقصود ومُبرَّر عند موضعه:** كتلة الهويّة في `Dockerfile` راستر **متأخّرة** بخلاف الثلاثة، لأنّ فوقها طبقتَي GDAL والاعتماديّات — أثقل طبقتين في المستودع — وما يليها يتغيّر مع كلّ التزام أصلاً فالإبطال يقع هناك حتماً.
- **بوّابة تسجيليّة أطلقت ولم يُلتَفّ عليها:** المكنسة رفضت `production_readiness_orchestrator.py` لأنّه يذكر `--check` (يُمرّره إلى بوّابات أخرى؛ الكاشف يطابق السلسلة). والمخرج المطلوب كان تصنيفه في `generated_chain_known_drift.json` — **وأساسه صفريّ يتقلّص ولا ينمو**، فالإضافة كانت تُسمّيه مولّداً وهو ليس كذلك وتنقض ثابتاً. الحلّ نمط البيت لأدوات المشغّل: `production-readiness-report.yml` بـ`workflow_dispatch` **يستدعيه فعلاً** — كما `runtime-image-provenance` و`path3-runtime-verification`.
- **الأداة — خمسة عيوب مقيسة على الصياغة الواردة:** ① `${SAHOOL_AGENT_TOKEN}` كانت تُرسَل **حرفيّاً** (أُثبِت بالتقاط الترويسة ومتغيّر البيئة مضبوط) فمسبار «رمز صالح ⇒ 422» يحمل رمزاً غير صالح — والتوسيع الآن **يفشل عند الغياب** لأنّ `expandvars` يُبقي النصّ بصمت · ② `argparse` بـ`action="append"` وافتراضيّ غير `None` **يُوسّع ولا يستبدل** (`--required-rls-table audit` ⇒ أربعة جداول) · ③ `critical_failures: 0` بينما فحصان حَرِجان تُخُطِّيا ⇒ أُضيف `critical_skipped` وأسماؤه · ④ العناوين `127.0.0.1:8000/:8001` **لا وجود لها** — لا خدمة من الأربع الحاملة للهويّة تنشر منفذاً، والمنشور nginx وحده؛ صارت أسماء DNS داخل `sahool-internal` · ⑤ `/v1/mcp/tools/call` ليس مسار المنصّة بل خمسة خوادم MCP منفصلة.
- **وتشغيل الشطر الساكن من الأداة على الشجرة أعطى `verdict=release_candidate` و١٢/١٢** — أي أنّ البوّابات السبع المُسمّاة موجودة و`--root` مدعوم، وهو تصديق مستقلّ للفرع.
- **وثلاث إصابات صحيحة على تغييري أمسكها الجناح لا أنا:** تصنيف `/runtime-identity` في صلاحيّات راستر · تعليق يحوي `pip install` أطلق حارس المرآة · `subprocess(text=True)` بلا `encoding`. **الأولى صُنِّفت تصنيفاً مبدئيّاً لا تمريراً لتخضير** — نفس صنف `/healthz`+`/readyz`+`/metrics` المُدرَجة أصلاً، وحمولتها نَسَب عامّ بلا سرّ.
- **التحقّق:** `pytest -m unit` **4145** ناجحاً · نفسه تحت `LC_ALL=C PYTHONUTF8=0` **4145** · المكنسة نظيفة بعد إعادة التوليد بالترتيب (خريطة ⇐ فهرس أثر ⇐ حزمة أخيراً) · `guard_mutation --run` **٢١/٢١** · مسابير وظيفيّة `plans=4 probes=7` · ميزانية المسارات 627/629 · حزمة الإصدار **5289** جزئة · `ruff` 0/0 · الشجرة نظيفة بعد الجناح.
- **حدّ صدق:** كلّه ساكن. لم تُبنَ صورة ولم يُشغَّل مِسبار على مكدّس حيّ، ولا يُرفَع `runtime_verified` ولا `production_certified` على شيء من هذا.
- **حارسان حاجبان يُبلِّغان نتيجةً لا تكفي لتشخيصهما — عولِجا معاً:** ① `brain_state_transition_guard` كان يطابق رمز الحالة في **أيّ موضع** من سطر مُضاف، فاقتباسُ حقلٍ يُقرأ ادّعاءً له؛ المرساة الآن **دلاليّة**: اقتباسٌ يقول إنّ القيمة صفر لا يمكن أن يكون ادّعاء إغلاق. والقيد على **القيمة** لا على وجود `=` — وإلّا أُعفي إسنادُ **قيمة موجبة** لنفس الحقول، وهو شكل الادّعاء الحقيقيّ بعينه، فيصير إصلاح الإيجابيّة الكاذبة سلبيّةً كاذبة أخطر. ② `guard_mutation_guard` صار يطبع `الساقط فعلاً` بدل نفي المتوقَّع وحده.
- **و② أثمر في أوّل تشغيل، فأسقط وصف «إخفاق عابر»:** تكرّر الإخفاق على `claim_base_guard.py[4]` وسمّى الساقط — `test_a_measured_stamp_does_not_satisfy_a_decision`، أي **الاختبار المتوقَّع للطفرة المجاورة [3]** لا اختبارٌ عشوائيّ ولا انهيار استيراد. والتشغيل التالي مرّ. الفرضيّات ضاقت ولم تُحسَم، والسبب **لم يُكتَب** لأنّه لم يُقَس؛ الفجوة تبقى OPEN بخطوة تالية مُسمّاة.
- **ووقعتُ في «قدرة موجودة لا تجري» داخل اختبار التشخيص نفسه:** أوّل صياغة قاست `failing_tests` وحدها، والطفرة تنزع **النداء** من الفرع لا الدالّة — فبقي الاختبار أخضر تحت طفرته. أُعيدت صياغته ليمرّ بـ`run_mutations` على حارس صناعيّ. **والمخرَج الجديد نفسه هو ما سمّى لي الاختبار الساقط فانكشف العطل** — أي أنّ الأداة أمسكت عيب بانيها في أوّل استعمال.
- **مراجعة حزمة خارجيّة ودمجها — الجوهر سليم وخمسة عيوب كانت ستحجب CI:** الحزمة أغلقت `GATEWAY-READINESS-PATHS-FALL-TO-SPA-01` بقرار صحيح (مواضع `location =` قبل ارتداد SPA · بوّابة الإنتاج مقيَّدة بـloopback+RFC1918 مع `deny all` · `proxy_intercept_errors off`)، ومدّت هويّة البناء إلى **تسع خدمات** بأسطح متناظرة، وحصّنت حارس الطفرات بسجلّ استعادة و`SIGTERM/SIGINT/atexit` — وهذا الأخير يسدّ ثغرة لم أرها: تشغيلٌ يُقتَل كان يترك عطلاً مزروعاً في الشجرة، أي صنف `TEST-PROBE-LEAKS-INTO-THE-TREE-01` بعينه.
- **وحارس التناظر الذي كتبتُه أثبت نفسه:** بقي أخضر عبر التوسّع إلى تسع خدمات لأنّ المُوسِّع اضطُرّ إلى مزامنة المصفوفة و`expected={…}` وخطط المسابير معاً — ومنها المجموعة المثبَّتة نصّاً في `publish-manifest` التي كانت ستنهار وحدها.
- **والعيوب الخمسة المقيسة (كلّها حاجبة، ولم يذكرها ملخّص الحزمة):** ① نسخة ثانية من المنسّق في `scripts/ops/` بعيوبها الخمسة الأصليّة (بلا توسيع `${VAR}` · بلا عدّ متخطّى حَرِج · افتراضيّ `argparse` تراكميّ · عناوين `127.0.0.1` لا وجود لها) — و`scripts/ops/` **خارج `_SCRIPT_DIRS`** فلا تراها أيّ بوّابة: تهبط صامتة · ② أربع خطط مسابير من خمس بتأكيدات **فارغة** أو نقطة دخول خاطئة أو مسار مُورَّث غير مُعلَن ⇒ `functional_probe_runner --check` أحمر · ③ اختبار جديد **بلا `pytestmark`** فلا يعمل في أيّ وظيفة — «قدرة موجودة لا تجري» · ④ جرد الإصدارات بلا إعادة توليد ⇒ `--check` أحمر · ⑤ اسم بديل لبوّابة موصولة أصلاً، بثمانية أسطر وبلا مُستدعٍ واحد.
- **والعلاج لم يكن ترميم الأعراض:** النسخة المكرّرة حُذِفت والاختبار الذي كان **يُكرّسها** أُعيد توجيهه إلى الأداة القانونيّة، ويؤكّد الآن الإصلاحات الثلاثة نفسها (`expand_env` · `critical_skipped` · `default=None`) وغيابَ النسخة الثانية — فوجود الملفّ لا يكفي. وتأكيدات المسابير مقيسة من مصادرها (`health.py:23` · `main.py:516` · `ops.py:28`) لا مُخترَعة.
- **وافتراضٌ صامت وُثِّق بدل أن يُترَك:** قوائم `allow` تقرأ `$remote_addr`، فادّعاء الخصوصيّة صحيح ما دامت البوّابة تُنهي اتّصالات العملاء مباشرةً — وهي الطوبولوجيا الحاليّة. خلف موازِن حِمل ينقلب الادّعاء: `$remote_addr` يصير عنوان الموازِن وهو RFC1918 غالباً، فتُقبَل الدنيا عبر قائمةٍ ما زالت تُقرأ خاصّة. **والفشل صامت** لأنّ الاختبارات تفحص الملفّ لا الشبكة.

## 2026-08-03 — إعادة تشكيل #775 والحارس الذي كان ينقص

- **أربع بوّابات حمراء رُدّت إلى سبب واحد:** الموجة تُضيف ٤٠٨٧ سطراً بلا مستهلك واحد. قياسُ الاستيراد على كامل الشجرة أعطى **صفر مستوردٍ غير اختباريّ** لكلّ وحدة من الأربع عشرة، وجداول `v217`→`v231` لا يكتب فيها أيّ خدمة.
- **وكدتُ أُصلِح الرقم بمذكّرة:** كتبتُ رفع الأساس 675⇒689 مع مذكّرة تُعلن الانحراف صراحةً — ثمّ وجدتُ في سجلّ الفجوات خطأي المسجَّل بحرفه: «الراتشِت لا يقرأ المذكّرات، يقرأ الأرقام». أُلغِيت المذكّرة وأُسقِطت الموجة، كما فعلت سابقة #676.
- **والقياس أخرج عيباً حقيقيّاً لولا الوصل لبقي مستوراً:** `v219` يستشهد بعمود `id` غير موجود في `machinery_export_artifacts` (الحقيقيّ `artifact_id`) — ولم يظهر قطّ لأنّ الترحيلات كانت **مُعلَنة في البيان وغير موصولة بالمشغّل**، فلم تمسّ PostgreSQL في أيّ بوّابة رغم مرورها بكلّ الحرّاس الساكنة.
- **الحارس الناقص أُضيف وأُثبِت:** `tests/test_migration_manifest_execution_guard.py` — الاتّجاهان معاً (مُعلَن بلا وصل · موصول بلا إعلان) + التكرار + بقاء `v206` آخِراً. زرعان أسقطاه، والشجرة نظيفة بعد الاستعادة. المقيس: ٢٢٢=٢٢٢.
- **وفخّ بوّابةٍ لم أكن أعرفه انكشف بالفشل:** `capability-governance.yml` بلا `types` ⇒ الإقرار يُقرأ من حمولة الحدث. بعد إعادة التشكيل حجبت البوّابة على `unaffected_declared` (إقرارٌ قديم لمسارات لم يعُد الفرق يمسّها)، وتعديلُ النصّ لا يُعيد إطلاقها — الدفعة وحدها.
- **التحقّق بعد إعادة التشكيل:** `ruff` 0/0 · حارسا التعارض · `claim_base` · `guard_mutation` ساكناً و`--run` · `verify_all_generated` · حرّاس الدماغ الثلاثة · `arch_test_ci_coverage` · `marker_coverage` — خضراء. `pytest -m unit` **4177** ناجحاً (ونفسه تحت `LC_ALL=C PYTHONUTF8=0`) · `pytest tests/` **623** · Platform Unit Tests **4070** بتغطية **62.23%** · `pr_capability_impact_gate` ⇒ `PASS`.
- **حدّ صدق:** لم يُنزَل من الموجة شيء، ولم يُرفَع `runtime_verified` ولا `production_certified`. الموجة مؤجَّلة برأس مثبَّت (`98283bca`) لا مرفوضة.

## 2026-08-03 — إرساء كاشف الترميز أُنزِل شريحةً مستقلّة (بعد `d727573e`)

- **لماذا شريحة مستقلّة:** الإرساء المعتمَد بالخيار الثالث كان يسافر مع موجتَي القدرات المُسقَطتين من #775، فسقط معهما بلا ذنب. أُعيد إنزاله وحده على `main` المدموج.
- **الإرساء نفسه (كما اعتُمِد):** الكاشف يفصل `open` **بنوع المتلقّي** لا بالاسم — `_path_like_names` يُثبِت أنّ الاسم Path من ثلاثة مصادر (إسناد من `Path(...)` · تعبير مسار `base / "x"` · تعليق `: Path`) بمرّتين لالتقاط ما يُعرَّف لاحقاً، و`_receiver_is_path` يقبل `open` المُدمَج و`Path("x").open()` والقسمة، ويرفض ما عداه.
- **مُثبَت بالتكذيب في الاتّجاهين (شرط المالك للخيار الثالث):** ① نزع سطر الإرساء نفسه ⇒ **خمسة** اختبارات إيجابيّة كاذبة تسقط (`opener.open` · `build_opener().open` · `archive.open` · `client.open` · `rasterio.open`) — أي أنّ الإرساء حامل لا زينة · ② زرع `open(p).read()` حقيقيّ في ملفّ خارج الأساس ⇒ `test_no_new_file_decodes_text_with_the_machines_locale` يسقط — أي أنّ التضييق لم يُعمِ الكاشف. الشجرة نظيفة بعد استعادة الزرعين.
- **الأساس تقلّص 184⇒182** (`fetch_glo30_dem.py` · `scan_release_archive.py`): كلاهما كان `rasterio.open`/`archive.open` — نفس الإيجابيّة الكاذبة، لا تخفيفاً للعقد. الراتشِت يتقلّص ولا ينمو.
- **التحقّق:** `pytest -m unit` **4194** ناجحاً (ونفسه تحت `LC_ALL=C PYTHONUTF8=0` — والمتّجه المقصود بالذات) · `pytest tests/` **628** · `verify_all_generated` نظيف بعد إعادة التوليد بالترتيب · `ruff` 0/0 · الاثنان والعشرون اختباراً للكاشف خضراء.
- **وُجِد ولم يُنزَل — يُسجَّل صراحةً:** تصلّب `production_readiness_orchestrator.py` (~٥٦٠ سطراً، ومنها `_NoRedirect` الذي رفض المالك حذفه) و`production_readiness_db_policy.json` ما زالا على الفرع المحفوظ `98283bca` وحده. لم يُنزَلا هنا لأنّ الشريحة المطلوبة كانت الكاشف؛ التسجيل يمنع أن يُظنّ أنّهما هبطا.

## 2026-08-03 — سلسلة الحقيقة المُدامة: «مستهلك» ≠ «قابل للوصول»

- **أغلقتُ PRاً أخضر بقرار المالك لأنّ بديله أصدق:** #777 كان يصل الطور بـ`accumulated_gdd=None`؛ حزمة `persisted_sor_wiring_fixed` تقرأ الحالة المُدامة فعلاً وتصل ثلاث وحدات عبر المسار القائم نفسه. بلا مسار جديد في الحالتين.
- **الدرس الذي أخرجه القياس:** الحزمة الأولى أرادت ١٧ مساراً جديداً على ميزانية هامشها ٢ — فرُفِضت. الحزمة الثانية أصلحت ذلك، لكنّها استبدلت المشكلة بأخرى **أخفى**: أربع وحدات «مستهلِكة» لا يستوردها أحد، فصارت ثماني عشرة وحدة تبدو موصولة وهي غير قابلة للوصول. **الاستيراد ليس وصولاً**، وفحص «هل لها مستهلك؟» يمرّ عليها كلّها.
- **فحصتُه بمخطّط استيراد من الراوترات المُركَّبة لا بالانطباع:** ٥ قابلة للوصول من ٢٣. والخمس وحدها دخلت الأساس (675⇒680).
- **والحارس يزرع شكل الحزمة نفسه ليثبت نفسه:** وحدةٌ تستورد السلسلة ولا يستوردها أحد ⇒ الحارس أحمر. وأوّل صياغة للزرع **لم تُعِد إنتاج العيب** (أبقيتُ الراوتر مستورداً للوسيط فبقيت السلسلة قابلة للوصول عبره) — فأُعيد الزرع على شكله الحقيقيّ.
- **`orphan_module_guard` في الحزمة يقول `candidates=0` على شجرةٍ فيها ١٨ وحدة غير قابلة للوصول** — لأنّه يعدّ الذكر في YAML أو في اختبار دليلَ وصول. أداة جرد لا برهان استهلاك؛ رقمها يُقرأ «لا شيء غير موصول» وهو ليس ما قاسته.
- **وضريبة التسجيل غير مدفوعة كالعادة:** ٢٣ وحدة خارج الأساس · ١٨ خطأ `ruff` · مهيّئ `sys.path` يدويّ بدل المساعد القانونيّ. كلّها صُحِّحت هنا.

## 2026-08-03 — حارس الوصول بالتصنيفات: عيبان في حارسي أنا كشفهما التحقّق

- **المالك طلب أكثر ممّا كتبتُ:** لا يكفي «مستهلك غير اختباريّ»؛ الحارس يجب أن يُثبِت **جذراً تنفيذيّاً** ويصنّف خمس حالات. وحارس الحزمة (٥١ سطراً، جذر واحد مثبَّت نصّاً، بلا تصنيفات) لم يكن يفعل ذلك.
- **والدرس الأهمّ ليس في العيب بل في كيف ظهر:** الحارس أطلق على `core/crop_intelligence/canonical_inputs.py` — ووثائقي تقول إنّها **موصولة**. بدل قبول نتيجة الحارس، تتبّعتُ السلسلة يدويّاً فوجدتُ عيبي: `pkg/__init__.py` كان يُفهرَس `pkg.__init__` فقط، فكلّ استيراد لحزمة يضيع. ثمّ عيبٌ ثانٍ بالطريقة نفسها: الاستيراد النسبيّ لم يكن يُحَلّ. المقيس بعد الإصلاحين: 501⇒**522**.
- **لو وثقتُ بالحارس لكنتُ سجّلتُ عشر وحدات موصولة على أنّها ميتة** — وهذا أسوأ من عدم وجود حارس: رقمٌ يشهد زوراً في الاتّجاه المعاكس.
- **والدَّين الموروث جُمِّد لا حُذِف:** سبع وحدات كنسيّة كانت غير قابلة للوصول قبلي ⇒ مجموعة تتقلّص ولا تنمو، والحجب على الجديد وحده. و١٥٠ وحدة أساس أخرى تُبلَّغ ولا تحجب — لا أبني بوّابة تسقط كلّ تغيير على دَين لم يُحدِثه.
- **وحارسٌ موجود ولا يعمل — حاولتُ وصله فتراجعتُ بقياس:** `capability_registry_reconciliation_guard` غير موصول بأيّ workflow، فتباعد السجلّان بصمت (أوّل توفيق ٣٣٠ سطراً). لكنّ الوصل كشف **دورة** مع `capability_runtime_evidence` على الملفّ نفسه: تسع قدرات تتأرجح (`runtime` في ثمانٍ و`status` في `FM-003`)، ولا ترتيب يُنهيها. أخرجتُه من الشريحة: **لا أُنزِل تغييراً يجعل المكنسة غير قادرة على التقارب، ولا أُخمّن ملكيّة الحقول بين سلطتين**. الفجوة سُجِّلت بقياسها لقرار المالك.

## 2026-08-03 — الحزمة الرابعة: كُتّاب الإسقاط الكنسيّ، وعيبٌ لا يراه إلّا PostgreSQL حقيقيّ

- **الفحص الحاسم لم يكن قراءةَ كود بل تشغيلَ قاعدة:** الحزمة تُولّد `command_id` بـ`uuid5` وتصفه بأنّه «مفتاح ثابت يمنع نيّة outbox ثانية». تتبّعتُ الحقل إلى تعريفه فوجدتُ `UUID REFERENCES commands(command_id)` — ثمّ **أنشأتُ PostgreSQL 16 مؤقّتاً بمخطّط v11 وأدرجتُ الصفّ**: `violates foreign key constraint "events_command_id_fkey"`. لا تخمين، ولا «يبدو خطراً».
- **ولماذا مرّت الحزمة على نفسها خضراء:** ستّة اختبارات على `FakeConn`. اتّصالٌ مزيَّف يقبل أيّ وسيط، فهو يقيس شكل الاستدعاء لا قبول القاعدة. الاختباران اللذان أضفتُهما يقرآن SQL والوحدة المُحلَّلة بدل الاعتماد على الوسيط الممرَّر — و`ast` لا نصّ، لأنّ التعليق الذي يشرح العيب يذكر `uuid5` فيُطابِقه فحصُ النصّ ويعطي أخضر كاذباً (وقع فعلاً في أوّل صياغة).
- **الصنف الأهمّ الذي كشفته هذه الحزمة:** حارس الوصول الذي بنيتُه أمس يقيس **الملفّ**، والملفّ هنا قابل للوصول بحقّ. فستّة كُتّاب أُضيفوا داخله ورثوا قابليّةَ وصولٍ لم يكسبوها: صفر مستدعٍ إنتاجيّ، ولا أثر في أيّ جرد. «الوحدة حيّة» ليست «الدالّة تُستدعى» — والدرجة الأدنى أسهل في الإفلات لا أصعب.
- **ونقضٌ صامت لحكم مالك:** نُقِل `canonical_agronomic_context` خارج `input_snapshot` **وعُدِّل الاختبار ليطابق النقل**. اختبارٌ يُعاد كتابته ليوافق الكود لا يحرس شيئاً. أُعيد الموضع، ومعه مسارُ `CANONICAL_CONTEXT_UNAVAILABLE` الذي حذفته الحزمة — إسقاطُ المفتاح عند الفشل يجعل «لا بيانات» و«القراءة انكسرت» إشارةً واحدة لكلّ قارئ لاحق.
- **وما أُخِذ لأنّه صحيح:** توحيدُ المعاملة. قراءةُ الحالة المُدامة وكتابةُ التوصية وحدثُ الـoutbox صارت على اتّصال `tenant_connection` واحد — أي لقطة واحدة — فالسلالة المُسجَّلة هي الحالة التي قُرِئت فعلاً وقت القرار. `ruff` كان أحمر في الحزمة (I001) للمرّة الرابعة على التوالي.

## 2026-08-04 — الحزمة الخامسة: وصلُ الجذر أُخِذ، وبوّابةٌ حيّة لا تعمل على قاعدة حقيقيّة

- **الحزمة تبنّت ثلاثاً من أربع ملاحظات، وأهمّها أغلق فجوةً فتحتُها أنا:** العامل صار خدمةً في `docker-compose.v9.yml` ويستدعي `persist_*_projection` الثلاث على `agronomy.projection.requested` — فالجداول لم تعد «تُقرأ ولا تُكتَب». و`_projection_command_id` أُزيل ويُمرَّر `None` (مكافئ لـ`NULL::uuid`)، و`v227` أخذ لافتته الصحيحة.
- **والبوّابة «الحيّة» لا تعمل حيّاً — أثبتُّه بالتشغيل لا بالقراءة:** `command_event_causality_live_gate.sql` يمرّر `entity_id` بـ`::uuid`. على `v10+v11` (شرطها المُعلَن) تنجح؛ وعلى `v10+v11+v18` — وهو شكل كلّ قاعدة منشورة، إذ أعاد v18 نوع `entity_id` إلى TEXT و**أسقط** حِمل emit_event ذا الـUUID — تفشل بـ`function emit_event(… uuid …) does not exist`. ثلاثة تحويلات `::text` أصلحتها، ثمّ نجحت على الشكل الإنتاجيّ.
- **ودرسٌ يتكرّر بصيغة ثالثة:** اختبار عقد البوّابة يقرأ **نصّ** الملفّ (`"uuid_generate_v5_stub" not in text`). تأكيدٌ على شكل SQL ليس تنفيذاً له — تماماً كما أنّ `FakeConn` تقيس شكل النداء لا قبول القاعدة.
- **وحارساي أنا كانا يمرّان وهما يشهدان بالباطل:** حارس مواضع الاستدعاء يمسح `services/sahool-platform` وحدها، والعامل في `scripts/workers/` — فظلّ يعدّ الكُتّاب الستّة «بلا مستدعٍ» بعد أن صار لثلاثة منها مستدعٍ. وحارس الوصول يطابق `python -m api.X` فقط، والأمر في compose مسارُ سكربت — فعاملٌ مُسجَّل فعلاً لم يُحسَب جذراً. وُسِّع الاثنان، ثمّ **أطلق الراتشِت كما صُمِّم**: المجموعة المُثبَّتة تقلّصت ٦ ⇒ ٣، والثلاث الباقية سُجِّلت بوصفها «بلا مستدعٍ **خارج وحدتها**» لا «ميتة».
- **واختبارٌ ينكسر من التنسيق لا من السلوك:** `assert 'parser.add_argument("--preflight"' in text` سقط لحظة أن أعاد `ruff format` — وهو بوّابة حاجبة — لفّ النداء. أُعيدت صياغته على الشجرة المُحلَّلة. و`ruff` كان أحمر للمرّة الخامسة على التوالي (١٢ خطأ).

## 2026-08-04 — العامل الذي قبِلتُه لم يكن خدمةً أصلاً: كتلة YAML خاطئة، وحارسي طابق النصّ

- **قبلتُ أمس «عاملاً مُسجَّلاً» وأبلغتُ أنّه أغلق صنف الدوالّ الميتة — وكان تحت `networks:` لا `services:`.** الحزمة ألحقته بعد كتلة الشبكات، فصار مفتاحاً داخلها. Compose لا يشغّل شيئاً من هناك. البرهان بالتحليل لا بالنظر: `yaml.safe_load` يعطي `worker under services: False · worker under networks: True`.
- **ولم يكن ينبغي أن يمرّ من حارسي:** وسّعتُ `_WORKER_CMD` ليقرأ أوامر المسارات، لكنّي أبقيتُه يقرأ **نصّ** ملفّ compose. فالسلسلة `python /app/scripts/workers/…` موجودة — أينما كانت — فطابقت، وأبلغ الحارس جذراً غير موجود. أصلحتُ عمى بعمى آخر، في الدور نفسه الذي كنتُ أدقّق فيه هذا الصنف بعينه.
- **والمجاميع لم تكن لتكشفه أبداً:** الوحدات التي يستوردها العامل قابلة للوصول من مسار أصلاً، فترتيب الدلاء يصنّفها `MOUNTED_ROUTE`؛ الملخّص متطابق حرفيّاً في الحالتين. الكشف جاء من استجواب `registered_worker_roots` مباشرةً: تحت `networks:` ⇒ **NONE**، وتحت `services:` ⇒ الثلاث.
- **العلاج طبقتان:** الحارس صار يقرأ أوامر كتلة `services` المُحلَّلة (مع ارتداد إلى النصّ لو تعذّر التحليل — حارس بلا جذور يفشل بصوت عالٍ وهو المطلوب)، وحارسٌ جديد `tests_v9/test_compose_services_are_declared_as_services.py` يرفض أيّ تعريف يحمل `build`/`command`/`healthcheck` تحت `networks:`/`volumes:`. مُثبَت بالتكذيب على العيب الحقيقيّ نفسه: يفشل ويسمّي المفاتيح الخمسة.
- **وضريبة التسجيل للمرّة السادسة:** ملفّا اختبار جديدان بلا `pytestmark` — أي لا يعملان في أيّ وظيفة CI. وبقيّة ملفّات هذه الحزمة كانت **تراجعاً تنسيقيّاً** عن `ruff format` فحسب، فلم يُؤخَذ منها إلّا إصلاح compose.

## 2026-08-04 — الخطّ الزمنيّ للصور: تاريخٌ بلا شاهد، ولِنتٌ زخرفيّ، وحارسٌ يحمي عطله

- **راجعتُ ما طُلِب فوجدتُ العكس:** التاريخ المعروض **سليم** — `monthLabel` يقصّ نصّاً ولا يستدعي `new Date`، والفخّ الشائع (`new Date('2026-08-04')` يُظهر اليوم السابق في توقيت سالب) غير واقع. لكنّ `acquisition_datetime` يعبر ثلاث طبقات (raster-service ⇒ الواجهة ⇒ نوع TS بتعليق يقول إنّه «لعرض تاريخ الالتقاط») **ولا تقرؤه الشاشة**. فالبطاقة تدّعي تاريخ التقاط بلا شاهد عليه. وليس تجميلاً: مسار الطبقات المُدامة يأخذ التاريخ من عمود والطابع من عمود آخر — قابلان للتباعد — بينما مسار المزوّد يشتقّهما من الطابع نفسه.
- **والساعة تُعرَض موسومة UTC عمداً:** التاريخ فوقها مشتقّ خادميّاً بقصّ طابع UTC، فعرضها بتوقيت المتصفّح يضع سطرين متناقضين على البطاقة الواحدة. والقراءة من النصّ لا عبر `Date`، لأنّ `new Date('2026-08-04')` يُفسَّر UTC و`new Date('2026-08-04T10:37:00')` يُفسَّر محلّيّاً.
- **`npm run lint` لم يكن معطّلاً بل زخرفيّاً:** لا `eslint` في التبعيّات، ولا ملفّ تهيئة في الشجرة كلّها، ولا وظيفة تستدعيه، و`--ext` فيه محذوف أصلاً في ESLint 9. أربع طبقات ناقصة. **وأوّل تشغيل حقيقيّ له كشف انهياراً**: `useMemo` بعد خروج مبكر في `ds/table.tsx` ⇒ اختلاف عدد الـhooks ⇒ `Rendered more hooks than during the previous render` لحظة امتلاء جدول كان فارغاً، وهو المسار الاعتياديّ لأيّ جدول يُحمَّل لا تزامنيّاً.
- **وعيبٌ في كودي كشفه سؤالي عن حزمة، لا قبولي لها:** فرضت الحزمة `dir="ltr"` على الشريط. بدل أخذها أو رفضها سألتُ ما الذي تلتفّ حوله — فكان عيبي: الواجهة RTL، وفيها أصل التمرير عند اليمين و`scrollLeft` من سالب إلى صفر، و`Math.max(0, left)` يقصّ كلّ هدف سالب إلى صفر ⇒ لا يصل الشريط إلى تاريخ أقدم أبداً. لم آخذ `dir="ltr"` (يضع الأحدث يساراً عكس القراءة العربيّة) بل فرق المستطيلين + `scrollBy` — محايد الاتّجاه.
- **ونافذة الشريط كانت `months * 31` يوماً:** عند ٢٤ شهراً تُدخل **١٤ يوماً** من المشاهد خارج الفترة المطلوبة، و`date.today()` يقرأ توقيت الخادم بينما كلّ التواريخ مشتقّة بـUTC.
- **قِستُ حزمتَي ESLint المرفوعتين ولم آخذهما:** إحداهما بلا `eslint-plugin-react-hooks` إطلاقاً — أي أنّها ما كانت لتكتشف الانهيار أعلاه؛ ومُشغّلها يفرض `--max-warnings=0` بينما يُبقي `no-explicit-any` تحذيراً و٤٦ منها على الشجرة؛ ولِنتها غير موصولة بأيّ workflow. الأفضل تغليفاً ليس الأفضل حراسةً.

## 2026-08-04 — سؤال «هل في البيئة المحلّيّة؟» شُغِّل بدل أن يُقدَّر، فأسقط عاملاً لا يُقلِع

- **البيئة تكفي، والجواب مقيس لا مُقدَّر:** PostgreSQL 16 و`asyncpg` و`nats-py` حاضرة؛ `nats-server` كان غائباً فنُزِّل v2.10.22؛ Docker غائب فبُني مخطّط حيّ أدنى يدويّاً (v10 · v11 · نواة v18 · v181 · v227) على PG16 مؤقّت. و**PostGIS ليس مطلوباً أصلاً** — مطابقتي السابقة في v10 كانت على كلمة `POST_HARVEST` لا على PostGIS.
- **بوّابتان حيّتان خضراوان على قاعدة حقيقيّة:** `command_event_causality_live_gate.sql` مرّت. و`canonical_execution_learning_rls_live_gate.sql` فشلت أوّلاً بـ`permission denied for table decision_learning_runs` — وتتبّع المنح أظهر أنّ **٦١ من ٦٣ هجرة مُنشِئة للجداول لا تمنح شيئاً**، والمنح الوحيد في `migrations/bootstrap_postgres.sh:122-124`. بعد تطبيقه: PASS.
- **و`--preflight-json` مرّ — ثمّ مات العامل عند الإقلاع.** الفحص يسأل «هل يغطّي دفقٌ كلّ موضوع؟» فيجيب نعم؛ والتشغيل يسأل سؤالاً آخر. ثلاثة مواضيع باسم durable واحد، والمستهلك المُعمَّر يرتبط باشتراك واحد ⇒ `consumer is already bound to a subscription` في **الدورة الثانية** من الحلقة، قبل حلقة الخمول. أي أنّ العامل الذي صادقتُ عليه أمس بأنّه «مُسجَّل» **لم يعالج حدثاً واحداً قطّ**.
- **«مُسجَّل» ≠ «يعمل» للمرّة الثالثة في ثلاثة أيّام:** الهجرة المُعلَنة التي لا تُنفَّذ · الخدمة المكتوبة تحت `networks:` · والآن العامل الذي يُقلِع فيموت. الطبقات الساكنة الثلاث كلّها كانت خضراء في الحالات الثلاث. ما كشف الثالثة ليس حارساً جديداً بل `python worker.py` بلا رايات.
- **العلاج بخاصّيّة لا بشكل:** durable لكلّ موضوع مشتقّ من **الموضوع كاملاً**. لاحقةُ المقطع الأخير — وهي أوّل ما يخطر — تُعيد التصادم نفسه لأنّ `season.closed` و`irrigation.closed` يتشاركانه؛ فصار للاختبار تأكيدٌ يزرع هذا الاحتمال بعينه. طفرتان زُرِعتا فعلاً في الشجرة وشُغِّلتا: الأولى أسقطت اختبارين بالرسالة الحيّة نفسها، والثانية أسقطت اختبار التصادم.
- **والتحقّق أُغلِق على الحيّ لا على الوهميّ:** بعد الإصلاح شُغِّل `subscribe_subjects` الحقيقيّ على nats-server فعليّ ⇒ ثلاثة مستهلكين متمايزين، لكلٍّ `filter_subject` الصحيح. الوهميّ في الاختبار قاعدته **مقيسة** من هذا التشغيل لا مفترضة.
- **وحافّة ترتيب غير مُعلَنة:** الدفق `sahool` ينشئه `agents/notification/agent.py:449` — لا الناقل ولا الهجرات. أُعلِنت الحافّة في compose ولم تُنقَل الملكيّة: نقلها قرار معماريّ يمسّ كلّ مشترِك، وخارج نطاق شريحة إصلاح عامل.
- **حدّ صدق:** الرحلة الكاملة (نشر ⇒ استهلاك ⇒ إدامة ⇒ إعادة تشغيل بلا تكرار) **لم تُشغَّل بعد** — مخطّطي الأدنى ينقصه v224–v226 (`canonical_salinity_states`). فلا يتحرّك `runtime_verified` عن صفر بهذه الشريحة؛ ما تحرّك أنّ العائق الأوّل أمامه صار معروفاً ومُصلَحاً.
- **دُمِجت في `6b6ffe82` (PR #785):** ٣٣ تشغيل workflow على `f7ff4e02` كلّها `success`، و`mergeable_state: clean`، بلا تعليقات مراجعة. **ودرس أداة:** `get_status` أعاد `total_count: 0` لأنّه يقرأ *commit statuses* لا *check runs* — الاكتفاء به كان سيُبلِّغ «البوّابات لم تبدأ» وهي منتهية خضراء؛ الحالة الحقيقيّة من تعداد تشغيلات الـworkflows على الـSHA.
- **وتناقض مقيس لم يُلاحَق (خارج نطاق الشريحة):** `capability_impact.py` و`pr_capability_impact_gate.py` يحسبان `direct` مختلفة على نفس المسارات — الأوّل `IRR-003, IRR-009, SEC-003` والثاني يضيف `INT-002`. أُعلِن ما تطلبه البوّابة، والصنف (أداتان تُجيبان سؤالاً واحداً بجوابين) يستحقّ قياساً مستقلّاً.

## 2026-08-04 — الرحلة الحيّة اكتملت لأوّل مرّة، وكشفت أربعة عيوب متسلسلة يحجب كلٌّ منها التالي

- **٢٢٦ هجرة من `MANIFEST.txt` على PG16 نظيفة: صفر فشل، صفر مفقود، v206 آخِراً كما يقتضي العقد.** ٣٤١ جدولاً · `postgis, uuid-ossp, pgcrypto` · ودور `sahool_app` المقيَّد (`rolsuper=f`, `rolbypassrls=f`). PostGIS ثُبِّت من apt بعد `apt-get update` (أوّل محاولة فشلت على `libmysqlclient21` بائت 404 — عطل مرآة لا عطل تبعيّة). لا Docker، فبُنِيت نسخة من خطوات `bootstrap_postgres.sh` ٣–٥ على عنقود محلّيّ بنفس ترتيب MANIFEST ونفس نموذج الدورَين.
- **ثمّ فشلت الرحلة أربع مرّات، كلّ إصلاح يكشف التالي:** ① `dict()` على `jsonb` يعود نصّاً من asyncpg · ② `source` اسم خدمة في عمود enum · ③ تنظيف يحذف من جدولين append-only · ④ شاهد يختم SHA لشجرة متّسخة. **ولم يكن أيّ منها ليُرى قبل الآخر** — الترتيب وحده أخفى الثلاثة الباقية في كلّ مرّة.
- **والجامع بينها واحد: الاتّصال الوهميّ لا يفرض شيئاً.** لا `CHECK`، ولا `TRIGGER`، ولا أنواع asyncpg. فمسار إصدار الأحداث كلّه في التعلّم القانونيّ — الإسقاط وإغلاق الموسم وإغلاق حلقة الريّ — **لم يلمس قاعدة حقيقيّة قطّ**، وكان أخضر طوال ذلك. سُجِّل `FAKE-CONNECTION-ENFORCES-NOTHING-01`، ومداه في المستودع **غير ممسوح**.
- **وأخطر الأربعة ليس أكثرها إسقاطاً بل ④:** الأحمر يُوقِف، والأخضر المنسوب إلى الالتزام الخطأ يدخل الأرشيف بوصفه إثباتاً. الـSHA يُسمّي **شجرة** لا تشغيلاً.
- **وثالثها كشف عمى في حارسي أنا قبل أن يُغلَق:** أوّل صياغة لـ«لا تحذف من append-only» مسحت `CREATE TRIGGER` نصّاً فوجدت ٢٤ جدولاً **وفاتها `events`** — لأنّ `v9_append_only_enforcement.sql` يبنيها ديناميّاً من مصفوفة داخل `DO`. حارسٌ يقرأ شكلاً واحداً يُبلِّغ نظافةً وهو أعمى عن أهمّ حالة. أُصلِح ليقرأ الشكلين.
- **وتكذيبٌ كامل:** الأربعة أُعيدت إلى الشجرة وشُغِّلت ⇒ سقط **أربعة اختبارات، كلٌّ باختباره** (2 مرّا: لا إطلاق كاذب)، ثمّ استُعيدت ⇒ ١٠ خضراء.
- **النتيجة المقيسة بعد الإصلاحات:** الرحلة تُغلَق كاملةً — طلب إسقاط يُدام، حدث يُنشَر، العامل يستهلك، `canonical_salinity_states` صفّ واحد، `event_outbox` واحد، `command_id` = NULL (لا اختلاق)، وإعادة النشر **لا تُكرّر**. `PASS canonical_execution_learning_live_gate`.
- **حدّ صدق:** الشاهد الأوّل خُتِم على شجرة متّسخة فلا يُحتَجّ به؛ الشاهد المُعتَدّ به هو ما يُنتَج بعد الالتزام على شجرة نظيفة. و`production_certified` يبقى `false` بالبناء.

## 2026-08-04 — تكافؤ أدوات الأثر، وجرد الناقل قبل نقل ملكيّته

- **التناقض الذي سجّلتُه بوصفه «معرّفاً زائداً» كان طبقة كاملة عمياء.** على تجهيزة ثابتة من عشرة مسارات: `direct` الأداة **صفر** والبوّابة **خمسة**، و`affected` صفر مقابل **اثني عشر**. `capability_impact.py` كان يمشي على قوائم `capabilities/registry` المصونة يدويّاً وحدها، والبوّابة تقرأ معها `capability_mapping.json` المولَّد. فالفجوة **تتّسع باتّساع الفرق** — وهو عكس ما توقّعتُه حين قلتُ «الثاني يضيف INT-002».
- **ولماذا يهمّ عمليّاً:** الأداة هي ما يوصي `CAPABILITY_GOVERNANCE.md` بتشغيله قبل كتابة سطر `Capability-Impact:`، والبوّابة هي ما يحجب. جوابٌ أصغر من الحقيقة هو كيف يفلت تغيير من إعلان الأثر — وقد حُجِبتُ به أنا في #785.
- **العلاج محرّك واحد لا ثانٍ مُصحَّح:** الأداة صارت غلافاً على `pr_capability_impact_gate.impact`. ومصدر الحقيقة محرّك البوّابة لأنّه يقرأ خريطة مشتقّة من الشجرة لا قائمة تُصان بيد. ومع اختبار التكافؤ **اختبار عرضٍ للتجهيزة** يمنع الخضرة الفارغة: أداتان تتّفقان على «لا شيء» لا تُثبتان شيئاً.
- **وجرد الناقل أظهر أربعة بنود لم تكن معروفة**، وكلّها تسبق أيّ نقل ملكيّة: ① التهيئة **مفتوحة عند الفشل** — `add_stream` يبتلع كلّ استثناء عدا «already exists» ثمّ يُعلن الوكيل `✅ ready` بلا ناقل · ② الدفق **بلا سياسة احتفاظ** (`max_age=0`, `max_msgs=-1`, `max_bytes=-1`) والحدّ الوحيد `max_file_store: 2GB` على الخادم كلّه ⇒ نموّ حتّى الرفض، **والرفض صامت** · ③ `num_replicas=1` ⇒ فقد المجلّد يفقد مواضع كلّ المستهلكين · ④ **لا حدّ إعادة تسليم ولا DLQ** على أيّ من **الثلاثة عشر** مستهلكاً.
- **والملكيّة موزّعة على ثلاثة مستودعات فرعيّة بلا سجلّ جامع** — `agents/` و`services/weather-polygon-worker/` و`scripts/workers/`. الجرد هو أوّل سجلّ من نوعه، ولم أنقل ملكيّة: الحسم في الاحتفاظ وDLQ وملكيّة الأسماء الدائمة و`num_replicas` قراراتُ منتَج لا تنفيذ.

## 2026-08-04 — بوّابة محلّيّة تُفوَّض إليها السلطة، لا تُنسَخ منها

- **حزمتان مرفوعتان من وكيل آخر، والثانية أفضل من الأولى ومن بعض ما بنيتُه.** أخذتُ منها خمساً: رفض الأرشيف مُغلَقاً عند الفشل · تشغيل **بوّابة الحجب ذاتها** بـ`--pr-body-file` بدل طباعة اقتراح · `merge-base` بسلّم بدائل و`--no-fetch` · fail-closed على سكربت بوّابة مفقود · وحارس عقد على السكربت نفسه.
- **ونقد المالك انطبق عليّ حرفيّاً:** خطوة الفهرس عندي كانت تفحص **غير المتعقَّب وحده**. وتعديل غير مُدرَج على ملفّ متعقَّب يُخفي الانحراف بالقدر نفسه. صارت تُبلِّغ الحالتين.
- **والفارق المبدئيّ الذي ثبَّتّه:** نصيحة الدمج المتقلّبة (stacked · merge-async · merge queue) تخصّ **الدليل** الذي يحمل تاريخ تحقّق وعقد صيانة، لا **الأداة** التي لا شيء فيها يقول إنّ هذه المعلومة تفسد. اختبار العقد يمنع تسرّبها.
- **والأرشيف بلا `.git` أخطر ممّا بدا:** المولّدات تسقط على البيان الموقَّع فتُبلِغ أخضر عن سؤال لم تطرحه. حزمة الوكيل الآخر (٥٣٧٢ ملفّاً) بُنيت هكذا **وعلى خطّ أساس أقدم** — ٤٨٨٧ ملفّاً مبصوماً مقابل ٥٣٤٣ على `main` بعد دمج #781 — فإعادة توليدها بائتة قبل أن تُرفَع. وتحقّق المالك أثبت أنّ العمل لم يصل المستودع إطلاقاً (404 على الثلاثة).
- **ودرس عليّ من أوّل تشغيل كامل لأداتي:** احمرّت ببندين، وكلاهما من **تعديلي أنا للوثيقة أثناء تشغيلها** (`checksum mismatch: CLAUDE.md`). أي أنّ الأداة التقطت في ١٢ دقيقة ما كان سيكلّف جولة CI ~٢٠ دقيقة — وأنّني كرّرتُ خطأ القياس على شجرة متحرّكة الذي كتبتُ عنه مرّتين في اليوم نفسه.
- **وتصويبان قبلتُهما بالدليل:** زمن `Unit Tests` في CI **١٩:١٩–٢٠:١٠** على `c4938837` — فرقمي المحلّيّ (٥:٠٨) يقيس مقداراً آخر، ورفضي رقم «١٠–١٥» كان خلطاً بين سياقَي قياس. و`merge-async` لا يخالف تجربتي بل يكمّلها: المنفرد يمرّ (#785) والمكدَّس يُرفَض (#781 خلف #783) — **ورفضي كان تعميماً من عيّنة واحدة**، وهو الصنف الذي أُلاحقه في غيري.

## 2026-08-04 — §٢ صارت سكربتاً، وسبع حزم مرفوعة صُفِّيت بالقياس

- **`scripts/ci/preflight.sh`:** كتلة §٢ التي تُنسَخ باليد صارت أداة مُلتزَمة بثلاث طبقات (`--fast` ~٣٠ث · افتراضيّة ~٧د · `--full` ~١٢د). وأوّل تشغيل كامل لها **التقط انحرافاً حقيقيّاً** كنتُ سأدفعه (`checksum mismatch: CLAUDE.md` من تعديلي أنا أثناء القياس) — أي وفّرت جولة CI (~٢٠ دقيقة بقياس سجلّ التشغيل على `c4938837`).
- **وصفتها مُعلَنة في رأسها وفي اختبار عقدها: أداة تطوير محلّيّة، لا بوّابة CI ولا مصدر قرار.** ويتبعها قيد مفروض: **لا تقرأ حالة GitHub** — لا سرد PRs، ولا اخضرار، ولا ترتيب دمج. تلك أحكام متغيّرة موضعها الدليل حيث تحمل تاريخ تحقّق وعقد صيانة.
- **ومتطلّباتها بيانات لا سطور:** `docs/architecture/preflight_required.json` بـ`schema`+`version` منفصلَين. **وحارس قائم صادق عليه ولم أُصادِق أنا:** `claim_base_guard` رفضه حتّى صُنِّف `decided` بتاريخ حكم — وهو تمييز «اشتقّ إن أمكن، صرّح إن تعذّر» مفروضاً لا مُدّعى. ولذلك سُجِّل في الملفّ صراحةً لماذا **لا** تُصرَّح قائمة المولّدات: المكنسة تكتشفها من الـworkflows بقرار خلفه فشل مقيس.
- **`--fix` صار يتبعه `--check`:** رمز خروج الإصلاح يقول «انتهيتُ» لا «تقاربت»، و§٣.١٦ تقيس أنّ بعض خطوات `--check` تكتب أثناء الفحص. وأُضيفت `--check` صريحة إلى المكنسة نفسها (مرادف للافتراضيّ) كي لا يتّكل مُستدعٍ على افتراض.
- **سبع حزم مرفوعة، وما نجا منها بالقياس:** رفض الأرشيف · تشغيل البوّابة الحاجبة بـ`--pr-body-file` · `merge-base` بسلّم بدائل · fail-closed على مفقود · عقد على السكربت · فصل البيانات · إعادة الترتيب. **وما رُدَّ بالدليل:** جدول حساسيّة مُصلَّب (الأداة تشتقّه) · `CANDIDATES` بموطنَين لمصدر حقيقة واحد · قاعدة بادئة تُبطِل ما تفرضه الحوكمة · ومخطَّط JSON Schema **بلا مُتحقِّق**.
- **واكتشاف جانبيّ من فحص الأخير:** أربعة مخطَّطات JSON Schema قائمة في المستودع **ولا `jsonschema` مثبَّتة ولا مُعلَنة تبعيّةً ولا سطر يستوردها** ⇒ `JSON-SCHEMAS-WITH-NO-VALIDATOR-01`. ظهرت لأنّي سألتُ «هل سيُتحقَّق من الخامس؟» لا لأنّي فتّشت.
- **وتصويبان قبلتُهما بالدليل:** زمن `Unit Tests` في CI **١٩:١٩–٢٠:١٠** لا ٥ دقائق (المحلّيّ يقيس مقداراً آخر)، و`merge-async` مشروط بالـstack لا مطلق — **ورفضي الأوّل كان تعميماً من عيّنة واحدة**.
- **وخطأ إجرائيّ تكرّر منّي أربع مرّات:** عدّلتُ الشجرة أثناء قياسها. الأداة التقطت الأولى، وحارس الترميز التقط `text=True` في اختباري الجديد، والرابعة أمسكتُها قبل أن تُفسِد قياساً. الدرس مُثبَّت: **لا سلسلتَي تحقّق على شجرة واحدة، ولا تعديل أثناء قياس**.

## 2026-08-05 — رسالة فشل تُرشِد إلى ملفّ لا وجود له

- **مراجعة آليّة على #787 أصابت في أربعة مواضع، والعيب حقيقيّ:** المُتحقِّق الجديد واختباره والسياسة وسجلّ الفجوات كلّها تقول للقارئ «`jsonschema` مُعلَنة في `requirements-preflight.txt`» — وهو **ملفّ حزمةٍ مرفوعة لم أعتمدها**، لا وجود له في هذا المستودع قطّ (`git ls-files | grep -c preflight` = 0). الموضع الحقيقيّ `tests_v9/requirements-test.txt:58`.
- **والصنف هو صنف الجلسة نفسه معكوساً:** «مُسجَّل ≠ يعمل» مطبَّقاً على **الإرشاد** لا على البوّابة. من يصطدم بالفشل كان سيبحث عن ملفّ، لا يجده، فيستنتج أنّ الحارس معطوب — إرشادٌ يُضلِّل أسوأ من لا إرشاد.
- **الاسم تسرّب من حزمة مرفوعة إلى نصوصي:** أخذتُ **مبدأ** إعلان التبعيّة (وهو صحيح) وأخذتُ معه **اسم ملفّها** الذي لم أُنشئه. الدرس: عند تبنّي مبدأ من مصدر خارجيّ، تُشتقّ أسماء المسارات من الشجرة لا من المصدر.
- **مُثبَّت بالتكذيب لا بالتصريح:** `test_the_failure_message_names_a_requirements_file_that_exists_and_declares_the_library` يشتقّ كلّ اسم `requirements*.txt` من نصّ الحارس والسياسة بـregex، ثمّ يُلزِم وجود الملفّ **وإعلانه للمكتبة فعلاً**. أُعيد زرع الاسم القديم فسقط الاختبار بالرسالة المقصودة، ثمّ أُعيد.
- **ودُمِج #787 قبل وصول الإصلاح** (`094bc2e7`)، فالتصويب **تغيير لاحق مستقلّ** على فرع أُعيد تأسيسه من `main` المدموج — لا تعديل على PR مُغلَق. القاعدة المُطبَّقة: PR مدموج منتهٍ، والعمل التالي يبدأ من قاعدته الجديدة.

## 2026-08-05 — مُنتقي الاختبارات المتأثّرة: سقفٌ يُعلَن بدل توفيرٍ يُدَّعى

- **الشريحة الأولى من ترتيب المالك (`test_impact.sh` أوّلاً):** أداة تختار الاختبارات التي يمسّها التغيير، **بلا منطق أثر خاصّ بها** — كلّ قدرة من `pr_capability_impact_gate` نفسه الذي تُشغّله البوّابة الحاجبة، وكلّ سبب مطبوع هو حقل `sources` للمحرّك. محرّكان للسؤال الواحد قاسا هنا ٠ و١٢ على مُدخل واحد؛ ثالثٌ كان سيكون أسوأ.
- **والقياس قبل البناء غيّر الأداة مرّتين:** (أ) من ٤١٢٣ حالة وحدة، **١٩٨٢ (٤٨٪)** في ملفّات لا يبلغها أيّ مخروط — حرّاسٌ مُدخلُها الشجرة كلّها. فهذه ليست «غير مختارة» بل **أرضيّة** تعمل دائماً، والسقف تنصيفٌ لا إلغاء. (ب) **٩٥٦ من ٢٢٧٠** ملفّ مصدر غير اختباريّ قابلة للربط = **٤٢٪** فقط.
- **والقياس (ب) قلب قاعدة كنتُ كتبتُها خطأً:** بدأتُ بـ«المسار غير المربوط يُفصَح عنه ولا يُصعِّد» بحجّة أنّ التصعيد سيعمّ. لكنّ أوّل قياس واقعيّ أظهر أنّ **أكبر «توفير» جاء من أجهل الحالات**: `learning_feedback.py` أعطى صفر اختيار لأنّ المحرّك لا يعرفه أصلاً. توفيرٌ مصدره الجهل ليس توفيراً بل إسقاطاً صامتاً — فصار مصدرٌ غير مربوط **يُصعِّد**، بثمن مُعلَن: ~٥٨٪ من تغييرات المصدر تُصعَّد.
- **وأداتي التقطت عيباً في أداتي بالتكذيب:** النسخة الأولى قرأت الفرق المُلتزَم وحده، فحرّرتُ ملفّاً وشغّلتُها فقالت «٠ متغيّر · ٠ مختار» وخرجت بصفر. **أداة للمطوّر قبل الالتزام تعمى عمّا يحرّره الآن** — نفس صنف الجلسة بالضبط: تعمل، ولا تقيس شيئاً. أُصلحت بـ`git status --porcelain=v1 -z` مع معالجة حقل مصدر إعادة التسمية.
- **مُثبَّتة بأربع زراعات نُفِّذت فعلاً:** إسقاط الأرضيّة · العمى عن شجرة العمل · تفريغ محفّزات التصعيد · وصلُها إلى workflow. كلٌّ منها أسقط الاختبار المقصود وحده، ثمّ أُعيد.
- **وقيدٌ مفروض لا مُدَّعى:** `test_no_workflow_invokes_the_selector` — لأنّ CI التي تُشغّل جناحاً مُضيَّقاً تكفّ عن كونها المرجع. التوفير المقيس على تغييرات مفردة: **٢٤٪–٥٢٪** حين يُحسَم، **صفر** حين لا يُحسَم.

## 2026-08-05 — الجناح كان يعمل مرّتين: نصف المسار الحرج تكرار

- **الشريحة الثانية بدأت فرضيّةً وانتهت نقيضها.** الطلب كان ترشيح المسارات (٣٨ workflow بلا `paths:`). القياس نفاها من وجهين: **عشرة** من الـ٣٨ لا تُطلَق على `pull_request` أصلاً (`dispatch`/`schedule`) فالترشيح عليها بلا معنى — والعدد الحقيقيّ **٢٨**؛ و`Unit Tests` وحدها **٩٩٪ من المسار الحرج** (١٩:٠٩ من ١٩:١٨) بينما آخر workflow مفرد انتهى بعد **١:٠٦**. فترشيح الـ٢٨ يوفّر **`0:00`** من زمن الجدار مقابل ٢٨ سطح false-negative.
- **والعلّة الحقيقيّة ظهرت في القياس نفسه:** `ci.yml` كان يُشغّل **الجناح مرّتين كاملتين** في الوظيفة الواحدة — `pytest -v -m unit --cov=services --cov-report=…` ثمّ خطوة بوّابة التغطية `pytest -m unit --cov=services --cov-fail-under=43 -q`. الحساب يُطابق المقيس: ٦:٣٥ محلّيّاً بالتغطية × ~١.٤٥ × ٢ = **١٩:٠٦** مقابل **١٩:٠٩** في CI. فالفارق ٤.٧× ليس عتاداً ولا تجهيزاً بل تكراراً.
- **ولماذا لم يُرَ سنةً:** بوّابة التغطية (A1) أُضيفت **بجوار** خطوة قائمة لا **داخلها**. كلتاهما صحيحة منفردة، وبينهما إحدى عشرة سطر تعليق، ولا واحدة تقول «هذه الثانية». **الكلفة في الاجتماع لا في السطر** — وهو ما لا تراه مراجعة سطريّة، فثُبِّت كبيانات: `test_ci_suite_invocation_budget.py` يعدّ استدعاءات الجناح لكلّ وظيفة.
- **وغلافٌ يُشخّص خطأً أُسقِط معها:** `|| (echo "التغطية انهارت دون الأرضيّة 43%" && exit 1)`. رمز خروج pytest واحد (`1`) لفشل اختبار ولانهيار التغطية معاً. **مُثبَت محلّيّاً:** تغطية ١٠٠٪ بلغت الأرضيّة + اختبار فاشل ⇒ طُبِعت رسالة انهيار التغطية. ورسالة `pytest-cov` نفسها دقيقة — فالغلاف كان يستبدل تشخيصاً صحيحاً بآخر خاطئ، نفس صنف `requirements-preflight.txt` في #788.
- **وحارسي كشف عيبين في حارسي قبل أن يُثبَّت:** طابق `pip install … pytest-asyncio` لأنّه يحوي الكلمة، وقرأ `pytest -v \` (سطر متّصل) استدعاءً كاملاً للجناح. أُصلح بالتحليل — بداية الأمر لا وروده، ووصل الأسطر قبل القراءة.
- **ومُثبَّت بثلاث زراعات:** إعادة التشغيل الثاني · إعادة الغلاف المُضلِّل · إسقاط أرضيّة التغطية. كلٌّ أسقطت اختبارها وحده.
- **ومرجع معلّق قبضتُه في نصّي أنا:** أحلتُ إلى «§٣.٢٥» ولا وجود لها. أُصلح بوصف الصنف بدل رقم قسم — والدرس أنّ رقم القسم مرجعٌ يَبيت كما يَبيت اسم الملفّ.
- **يبقى غير مُثبَت:** `test_new_unversioned_route_is_rejected_without_adjudication` فشل مرّةً في جولة بدأت بعد قتلي جولةً في منتصفها، ونجح منفرداً وبملفّه وفي جولة التغطية. **لم أُثبِت السبب ولا أُغلقه بترجيح.**

## 2026-08-05 — الادّعاء قِيس، والاختباران سقطا لأنّهما افترضا بيئتي

- **٢.٣٣× مقيسة لا متوقَّعة:** أوّل جولة CI بعد دمج تشغيلَي الجناح (`03bd0fe3`، الوظيفة `92322852732`) طبعت **`494.01s (0:08:14)`** مقابل **١٩:٠٩** على #788. أفضل من التنصيف المتوقَّع (~٩:٣٥) لأنّ التجهيز الثابت — التثبيت والاستيراد والجمع — كان يُدفَع مرّتين أيضاً. الرقم استبدل التوقّع في §٣.٢٤ بـSHA ومعرّف الوظيفة.
- **وسقط اختباران — مِن كتابتي أنا:** `test_the_plan_reports_undecided_paths_for_a_real_change` و`test_selection_is_reproducible_from_git_alone` صلّبا `origin/main` أساساً. و`actions/checkout` بعمق ١ **لا يُنشئ هذا المرجع**، فمات `git merge-base` بـ`exit 128`.
- **والأداة كانت على حقّ والاختبار على خطأ:** أساسٌ لا يُحلّ لا يُنتج دلتا، فالرفض صحيح. لكنّ الاختبارين كانا **يؤكّدان طوبولوجيا git عند العدّاء** بدل سلوك المُنتقي. أُصلحا باشتقاق الأساس (`origin/main` ← `HEAD~1` ← `HEAD`) مع **سقوط صريح** إن لم يُحلّ شيء — لا تخطٍّ صامت.
- **وعيبٌ ثانٍ حقيقيّ كشفَته الجولة:** الأداة كانت تموت بـ`CalledProcessError` خاماً. الرفض صحيح، لكنّ **الأثر الراجع ليس تشخيصاً**. صارت تسمّي المرجع الذي تعذّر حلّه وتقول ما العمل — نفس المعيار الذي طُبِّق على رسالة المكتبة الغائبة في #788.
- **مُعاد إنتاجه محلّيّاً بالشرط نفسه:** `git update-ref -d refs/remotes/origin/main` ثمّ تشغيل الملفّ ⇒ ٢٧ ناجحاً، والأداة تُخرِج التشخيص وترجع `2` بلا أثر راجع.
- **والدرس:** اختبارٌ يمرّ محلّيّاً ويسقط في CI لأنّه يفترض مرجعاً محلّيّاً هو صنف «مُسجَّل ≠ يعمل» مقلوباً — البيئة التي كُتِب فيها ليست البيئة التي يعمل فيها. كلّ مرجع git في اختبار يُشتقّ أو يُثبَت وجوده.

## 2026-08-05 — سجلّ فُرِّغ خمس ساعات وكلّ البوّابات خضراء

- **`BRAIN-APPEND-ONLY-TRUNCATION-GUARD-01`:** `sahool-brain/log.md` ذهب من **١٬٣٨٣٬٣٦٨ بايت إلى صفر** في `cb6598fe` (#789) وبقي فارغاً على `main` نحو خمس ساعات. لم يُحجَب شيء — ولم يكن شيء معطوباً، **لم يكن للملفّ حارس**.
- **وثلاثة أسباب متراكبة، كلٌّ منها صحيح في نطاقه:** الحرّاس الثلاثة تفحص الادّعاءات والانتقالات لا الحجم · `resolve_merge_conflicts` يعرف التصنيف لكنّه لا يعمل إلّا عند تعارض مُبلَّغ **والدمج الصامت لا تعارض فيه** · و**بصمة ملفّ فارغ بصمة صحيحة**. لا عيب في أيّ منها؛ العيب في الفراغ بينها.
- **والاستعادة مُثبَتة لا مفترَضة:** دمج #791 أعاده إلى ١٬٣٩٢٬٥٤٥ بايت، ومحتوى `32efdc90` **بادئة بايتيّة تامّة** — لا بايت فُقِد بل زِيد ٩٬١٧٧.
- **وتقرير الشهادة الحيّة كان محقّاً في الواقعة وخاطئاً في العلاج:** أوصى باستعادة `log.md` من `afb1ea9f` (١٬٣٦٣٬١٤٤ بايت). المقيس أنّ هذا الحجم يخصّ `0df78a4e` — **سلف** لـ`main`، ومحتواه بادئة تامّة للحالي. فتنفيذ التوصية حرفيّاً كان **سيحذف ٢٩٬٤٠١ بايت**. الدرس: توصية استعادة تُقاس قبل تنفيذها، وإلّا صار العلاج هو العطل.
- **والخاصّيّة المفروضة مقيسة لا مفترَضة:** على ٢٠٢ زوج (التزام، والد) — «لا تقلّص» أطلق **مرّة واحدة** (الحادثة)، بينما «بادئة بايتيّة» كانت ستُطلِق **١٤١**، لأنّ `registry.md` يحمل تحديثات حالة يوجبها `CLAUDE.md` و`hot.md` لقطة تُعاد كتابتها. فحارسٌ يُطلِق على العمل الطبيعيّ يُدرّب قارئه على تجاوزه: **التقلّص يحجب وفقدُ البادئة يُبلَّغ**.
- **ووعي الدمج ليس زينة:** النجاة وقعت لأنّ والداً يحمل الملفّ والآخر لا. مُثبَت بدمج اصطناعيّ يأخذ الجانب الفارغ — الانتهاك مرئيّ مقابل والدٍ واحد من اثنين فقط.
- **وأربع زراعات نُفِّذت:** فحص والد واحد · جعل التقلّص إرشاديّاً · تصليب قائمة الملفّات بدل استيرادها من `resolve_merge_conflicts` · فصله عن الـworkflow. كلٌّ أسقطت اختبارها وحده.
- **وتصويب لقياسي:** التسريع صار **مدىً** (١١:٢٨ و١٢:٠٨ ⇒ ١.٥٨–١.٦٧×) لا قيمةً مفردة. قياسان يكشفان تشتّتاً ٤٠ ثانية؛ عيّنة واحدة لا تُنتِج ثابتاً — وهو الدرس نفسه الذي قبلتُه في `merge-async`.

## 2026-08-05 — الحرّاس أمسكتني ثلاثاً في شريحة واحدة، والرابعة أمسكتُها بنفسي

- **وسؤال المالك كشف فجوةً في التوثيق نفسه:** سُئلتُ هل وُثِّقت الأخطاء الثلاثة في الدروس المستفادة، فقِستُ بدل أن أُجيب — و**الدليل ينقصه أربعة من ستّة**. `STABLE_WRONG_TEST` كان **صفراً** في كلّ وثيقة (مطابقتي الأولى كانت فضفاضة على «مُتخطّى» فأعطتني ✓ كاذبة). الدروس كانت تعيش في **رسائل الالتزام والمحادثة** — وهما لا يُقرآن عند بدء مهمّة. أُضيفت §٣.٢٥ بأقسامها الخمسة.
- **والدرس عن التوثيق نفسه:** «سجّلتُه في رسالة الالتزام» ليس توثيقاً. الوثيقة التي تُقرأ عند بدء المهمّة هي المكان الوحيد الذي يمنع التكرار — وما عداه أرشيف.
- **`STABLE_WRONG_TEST`:** سجّلتُ أربع طفرات ونفّذتُها محلّيّاً فمرّت، ثمّ سقطت واحدة في CI: الاختبار المُسمّى مشروط بوجود SHAs الحادثة، و`actions/checkout` بعمق ١ لا يحملها ⇒ **يُتخطّى**. تكذيبٌ مُتخطّى صمتٌ لا دليل. والحارس **لم يقل «مرّ» ولم يقل «سقط شيء ما»** — قال «حمرّ بغير الاختبار المُتوقَّع» وسمّى الساقط فعلاً، فصار التشخيص دقيقة لا جولة.
- **fail-open داخل علاجي:** كلّ زوج يُتخطّى حين يغيب السجلّ عند الوالد، فشجرةٌ بلا سجلّات تُنتِج **صفر أزواج وتطبع `ok`** — صنف الفجوة نفسه داخل الحارس المبنيّ ضدّها. القاعدة المُستخلَصة: كلّ حارس يُسأل «ماذا يطبع حين لا يجد ما يقيسه؟».
- **وضريبة التسجيل وحارس الترميز:** رفض `guard_mutation_guard` الحارسَ بلا مواصفة («له اختبار ليس دليلاً»)، وأمسك حارس الترميز `write_text` بلا `encoding="utf-8"` وفكَّي ترميز خامَين. كلاهما قبل CI.
- **الحصيلة:** أربعة أصناف في شريحة واحدة، **ثلاثة أمسكتها حرّاس قائمة**. هذا ما تشتريه الحوكمة فعلاً — لا الخضرة، بل أنّ الخطأ يُمسَك في دقيقة بدل جولة، وبرسالة تسمّي ما سقط.

## 2026-08-05 — «هل وُثِّقت الأخطاء؟» — القياس قال لا، مرّتين

- **سُئلتُ مرّتين، وقِستُ مرّتين، وفي كلتيهما كان الجواب «لا».** المرّة الأولى كشفت أربعة دروس غائبة عن الدليل. والثانية — بجرد **٢٨ درساً** من الجلسة كلّها — كشفت **١١ غائباً**، تنقسم عائلتين.
- **العائلة (أ) عقود تشغيل:** عامل بـdurable مشترك · `jsonb` يعود `str` من `asyncpg` بينما المزيّفات تُرجِع `dict` · `events.source` قيمة مقيَّدة بـ`CHECK` لا اسم وحدة · teardown يحذف من append-only · مكتبة مثبَّتة حيث كُتِبت الخطوة لا حيث تعمل. أُضيفت §٣.٢٧.
- **والعائلة (ب) أثمن — أخطاء في طريقة التدقيق نفسها:** حارس يطابق نصّاً حيث يجب أن يُحلّل · ماسحٌ يعرف صيغةً واحدة للحقيقة (فوّت أربعة جداول من `ARRAY[...]`) · قياسٌ ضيّق يُجيب سؤالاً أوسع (٤ مقابل ١٥ مخطَّطاً) · `diff` مقابل الجار الخطأ · **تدقيقٌ بـregex يُعطي أخضر كاذباً** · قياسٌ على شجرة متحرّكة (ستّ مرّات) · حلقة انتظار تطابق سطر أمرها. أُضيفت §٣.٢٦.
- **والجامع بين العائلة (ب): كلّها تُنتِج صمتاً أو خضرة، لا خطأً.** ولذلك لا تُمسَك إلّا بسؤال مقصود: «لو كان العطل موجوداً، هل كان قياسي سيراه؟».
- **ولا كتالوج حرّاس كان موجوداً إطلاقاً.** §١ يسرد الوظائف و§٣ يسرد أصناف الفشل، ولا شيء يقول للمطوّر ما يفرضه حارسٌ بعينه وكيف يُرضيه. `scripts/ci/guard_catalogue.py` يولّده من ثلاثة مصادر قائمة: الـworkflows (أين يحجب) · سجلّ الطفرات (ما يمسكه بكلمات كاتبه) · وسطر التوثيق الأوّل في الحارس (ما يفرضه). **لا نسخ**، فلا يبيت.
- **وأوّل جرد أخرج رقماً لم يكن يعرفه أحد: ٢١٨ حارساً تحجب في CI، و٨ فقط مُثبَتة بالتكذيب.** أي أنّ **٢١٠** تحجب الدمج ولم يُقَس قطّ أنّها تفشل حين يوجد العطل. ليس اتّهاماً لها بل **قياس لِما نعرفه عنها** — والرقم ظهر لأنّ الكتالوج **يعدّ** لا يصف.
- **والمكنسة اكتشفته وحدها** (٦٤ ← ٦٥ خطوة) بلا تسجيل يدويّ، لأنّها تقرأ الـworkflows.
- **مُثبَت بأربع زراعات:** انحراف الكتالوج · استبدال تحليل YAML بمطابقة · إخفاء عدد غير المُثبَتة · جعل `--check` يُرجِع صفراً. **وزرعي الثالث فشل أوّلاً لأنّه لم يُعِد إنتاج العطل** (أخفيتُ الرقم من العنوان وبقي في الملخّص) — فأُعيد صوغه حتّى أسقط الاختبار. زرعٌ لا يُعيد إنتاج العطل يُثبّت وهماً لا خاصّيّة.
- **ودرس عن التوثيق نفسه:** «سجّلتُه في رسالة الالتزام» **أرشيف لا توثيق**. الوثيقة التي تُقرأ عند بدء المهمّة هي الوحيدة التي تمنع التكرار — ولذلك أُضيف §٠ «أين تبدأ عند فشل بوّابة» ورابط الكتالوج في `CLAUDE.md`.

## 2026-08-05 — حارسٌ ناقض تشخيصه، ومصدره كان يوثّق الحادثة سلفاً

- **`guard_mutation_guard` أسقط #795 بتصنيف `STABLE_WRONG_TEST` بينما إعاداته الثلاث قالت `expected_red`** — أي أنّ الطفرة تعمل. `stable` كانت تقيس اتّفاق الإعادات **مع بعضها** لا مع الملاحظة الأولى، فاتّفاقٌ على الصحيح صار «خطأً مستقرّاً».
- **والتعليق في مصدر الحارس كان يوثّق الحادثة نفسها**: «أخفق هذا الفرع مرّةً على `claim_base_guard.py[4]` ولم يتكرّر في ثلاثة تشغيلات» — فهي **الثانية** على الطفرة عينها. أي أنّ الكاتب رأى الشذوذ وسجّله، **ولم يُصحّح الاسم الذي يصفه**.
- **ولم أُصلحه بجعل البناء أخضر:** أُضيف `FLAKY_FIRST_OBSERVATION` ويبقى **حاجباً** — فحصٌ يخضرّ بإعادة التشغيل يُدرّب قارئه على إعادة التشغيل بدل القراءة. الاسم وحده تغيّر، والحادثة صارت تُقرأ من سجلّها.
- **وسبب الشذوذ نفسه غير مُثبَت** وسُجِّل كذلك. صحّحتُ **الوصف** لا الظاهرة، وقلتُ ذلك صراحةً في الفجوة.
- **والدرس:** رسالة الحارس **جزء من الحارس**. تصنيفٌ يقلب ما حدث يُرسِل القارئ إلى الملفّ الخطأ — نفس صنف `requirements-preflight.txt` و`|| echo "التغطية انهارت"`، الثالث في يوم واحد.
- **وطريقة الاكتشاف تستحقّ التسجيل:** لم أقرأ الكود أوّلاً بل **قارنتُ سطرَي السجلّ**: `expected_red ×3` هنا مقابل `wrong_test ×3` في #792. الفرق في المخرَج قاد إلى العطل في المنطق — وهو عكس ما يفعله المرء عادةً.

## 2026-08-05 — سبعة أعطال في نقطة واحدة، و«الماء الموفَّر» كان عُشر الطلب

- **البؤرة:** `/api/v1/simulate/what-if` — الشريحة التي طلبها المالك بعد دمج #795. سبعة أعطال من تقرير الشهادة الحيّة، كلّها **أُعيد قياسها محلّيّاً** بتشغيل المحرّك بطقس محقون (بلا شبكة) قبل أن يُكتب سطرُ إصلاح واحد.
- **`scenario` كان زخرفة مُثبَتة:** `reduce_irrigation` و`no_irrigation` ينتجان الأرقام **نفسها بالضبط**، لأنّ `irrigation` بوليان لا نسبة. صار `Literal` بثلاث قيم لكلٍّ نسبة تُمرَّر إلى المحرّك فعلاً ويُراكِم بها الماء **المُطبَّق** (`irrigation_applied_mm`).
- **وتشخيصي الأوّل لـ`water_saved_mm` كان ناقصاً فصحّحتُه بالقياس:** قلتُ «يخلط مقدارَين غير متجانسَين». القياس أدقّ: `engine.py:425` يحسب الطلب `max(0, etc − rain)` مضروباً في `1.1` حين `irrigation` صادقة، و`total_etc_mm` **متطابق** بين الفرعين (٧٣٩.٩). فالفرق هو **المعامل وحده**: ٨١٣.٨ − ٧٣٩.٩ = ٧٣.٩ = ٠.١ × الطلب. الرقم لم يكن مقلوباً — كان **معاملاً بلا دلالة**، يكبر كلّما جفّ الموسم ولا يعلم بالسيناريو.
- **ولم أُصلح المعامل:** تغييره يحرّك كلّ رقم طلبٍ في المنصّة، وذلك قرار مالك سُجِّل بمعرّفه (`WOFOST-IRRIGATION-EFFICIENCY-COEFFICIENT-01`). أُسقِط **الطرح المبنيّ عليه** لا المعامل.
- **الاحتياط الصامت صار مُفصَحاً لا ممنوعاً:** «محصول لا وجود له» كان يُعيد ٩.٧٨٨ ط/هـ — رقم القمح الصلب حرفيّاً. منعُه بـ`422` كان سيكسر نشراً شرعيّاً (ستّة محاصيل فقط في الجداول)، فبقي الاحتياط وظهر `parameter_resolution.degraded` مع اسم البديل.
- **مُثبَت بتسع زراعات نُفِّذت — والثانية مرّت أوّلاً:** توكيدي قارن «التقليل» بالطرف الأدنى وحده، فزرعةٌ تُعيد الملء الكامل عند أيّ نسبة موجبة كانت تمرّ. صُلِّب إلى الحصر بين الحدّين. **زرعةٌ لا تُنتِج العطل تُثبّت وهماً لا خاصّيّة** — الثانية في يومين.
- **وحدّ صدق سُجِّل ولم يُصلَح:** الغلّة لا تتحرّك بين نسبتَي ٠.٢٥ و١.٠ في هذا الطقس (٨.٨٧٩ في الأربع)، فـ`recommended_action_helps` تُخرِج `false` لـ«تقليل الريّ». مقيسٌ لا مُرجَّح، ولم يُغلَق.

## 2026-08-05 — سببُ استثناءٍ صحيحٌ عند كتابته، حاجزٌ دائم بعد أن بطل

- **رصدَته شريحة عمل عاديّة:** تغييري في محرّك WOFOST أكسب `IRR-004` بُعد `events`، فانحرفت مصنوعة `capability_management_engine` و`--fix` دارت **ثلاث دورات (~١٥د) بلا أن تستدعيه**. لم يكن مسحاً عن الحرّاس — كان طريقي إلى الدفع.
- **وكان استثناؤه مُسجَّلاً بسببٍ مقيس: «يُغيّر ملفّاً في كلّ تشغيل».** أعدتُ قياس الثلاثة المستثناة **فرداً فرداً** لا تعميماً من واحد: هذا صفر ملفّات، والآخران ملفّ لكلٍّ. فالسبب صحيح لاثنين وبائت لواحد.
- **ولم أصِله في الطبقة صفر:** يقرأ خمس مصنوعات والأبجديّة تضعه **قبل ثلاث منها** (`man` < `map`)، فكان «سيستقرّ» بمصادفة الدورة الثانية لا بترتيب صحيح — نفس فخّ `attestation` المُسجَّل في الملفّ ذاته. `_ORDER_TIER = 1`، والقياس بعده: استقرار في **دورة واحدة**.
- **وتراجعتُ عن تسجيله مرّةً ثمّ عدتُ إليه:** أوّل ما قرأتُ الاستثناء ظننتُه قراراً محكماً فألغيتُ تعديلي؛ ثمّ قِستُ الشرط المُعلَن في الوثيقة نفسها فوجدتُه مستوفىً بالكامل. **قراءة السبب ليست قياسه.**
- **الدرس:** سببٌ مُسجَّل لا يُعاد قياسه يصير حاجزاً دائماً أمام إغلاقٍ صار ممكناً. والتصحيح كُتِب بحقلٍ مسمّى في الوثيقة لا بحذف الجملة القديمة — فيبقى أثر أنّ القياس تغيّر، لا أنّ أحداً أخطأ.

## 2026-08-05 — عاملٌ «مُسجَّل» لم يكن ليُقلِع، وإصلاحٌ كان سيبدو ناجحاً

- **البند التالي في ترتيب المالك المُصحَّح (P0)، وبدأتُه بالقياس لا بالإصلاح:** العامل القانونيّ يُنفَّذ من `/app/scripts/workers/…` والـDockerfile ينسخ سطرَين فقط لا يضع أيّهما ذلك المسار. البرهان **شاملٌ على أوامر `COPY` الثلاثة**، لا عيّنة.
- **والاختبار القائم كان أخضر طوال الوقت:** `test_worker_is_registered_in_compose` يقرأ نصّ compose ويؤكّد أنّ الاسم والمسار مذكوران — وهما مذكوران. «مُسجَّل» ليس «يعمل».
- **وتحت العطل عطلٌ ثانٍ:** إضافة `COPY scripts/` كانت ستُنتِج `ModuleNotFoundError: No module named 'api'` لأنّ الصورة تنسخ **محتويات** جذر الخدمة إلى `/app`. إصلاحٌ يبدو ناجحاً ويظلّ مكسوراً — ولم أكتشفه بالقراءة بل **بتشغيل العامل في تخطيط صورةٍ محاكاة**.
- **فاخترتُ النقل لا النسخ:** تحت جذر الخدمة يصير `parents[1]` هو جذر الخدمة في المستودع وفي الصورة معاً؛ الشكلان يتطابقان فلا فرعَ لبيئة. ثلاث حالات مقيسة: غياب · سقوط استيراد · تجاوزٌ يسقط على الاتّصال (المتوقَّع بلا قاعدة).
- **وطفرتي الأولى مرّت، فأسقطني حارس الطفرات بحقّ:** غيّرتُ فرعاً لا يمسّ ما يؤكّده الاختبار المُسمّى. الثالثة في يومين — والفرق أنّ الأداة صارت تقولها بدل أن أكتشفها بالحظّ.
- **وعقدُ حارسي كشف فيه عيباً لم تكشفه قراءتي:** `def audit(root=ROOT)` يُجمّد الجذر وقت التعريف، فلا يُستجوَب فرع «صفر أزواج مفحوصة». حارسٌ لا يُمكن اختبار فشله المُغلَق ليس مُغلَقاً.
- **والحارس عامٌّ لا خاصّ:** كان سيمنع واقعة WOFOST (`cb6598fe`) أيضاً — تلك أُغلِقت بعقدٍ خاصٍّ بها، وهذا يُغلق الخاصّيّة التي كانت ستمنع الاثنتين.

## 2026-08-05 — البوّابة التي لم يقسها preflight كانت في المسار الذي عدّلتُه

- **CI أسقط `Platform Unit Tests`** على حارس نموّ وحدات المنصّة: نقلُ العامل **إلى** شجرة الخدمة رفع جردها ٦٨٠ ⇒ ٦٨١. شرعيّ تماماً، وأُغلِق بالمسار المُصرَّح في الحارس نفسه: تسجيل الوحدة + رفع العدّ + **تعليل مكتوب**.
- **والتعليل نقلٌ لا نموّ:** الملفّ موجود في المستودع منذ زمن، ويستورد **ستّ وحدات `api.*`** مملوكة للمنصّة، فموضعه القانونيّ هو الخدمة التي يقرأ داخلها. جرد وحدات المستودع لم يتغيّر. هذا ليس رفعاً للسقف لتخضرّ CI — وهو الفرق الذي يقيسه القيد.
- **لكنّ الخطأ الحقيقيّ خطأي:** `preflight.sh --full` يُعلن صراحةً أنّه يغطّي **٧٠ من ٢٠٩** بوّابة، ويطلب اشتقاق الباقي من المسارات المُعدَّلة. عدّلتُ `services/sahool-platform/**` ولم أُشغّل شجرة اختباراتها. الأداة قالت ما لم تقِسه، وأنا قرأتُ «إخفاقات ٠» على أنّه «CI ستخضرّ» — وهو بالضبط ما تحذّر منه §٣.١٨–§٣.٢١.
- **وحين شغّلتُها من الجذر سقط اختباران آخران** — ففتحتُ الأمر بدل أن أنسبه: كلاهما يفتح `core/crop_cards/wheat.yaml` **نسبةً إلى مجلّد العمل**، وCI يُشغّلها من داخل الخدمة. من موضعها الصحيح: **٤٠٩٢ من ٤٠٩٢ خضراء**. لا علاقة لهما بتغييري، وهشاشةٌ سابقة الوجود لا أُصلحها في هذه الشريحة.
- **الدرس:** «أخضر على ما قِيس» جملةٌ تطبعها الأداة، وقراءتُها بوصفها ضماناً هي العطل. البوّابة الغائبة كانت في المسار الذي عدّلتُه بالذات.

## 2026-08-05 — حارسٌ كان يُشفِّر الافتراضَ المكسور الذي جئتُ لأُصلحه

- **نقلُ العامل إلى شجرة الخدمة أسقط `platform_module_reachability_guard`:** الوحدة صارت داخل المنصّة بلا **جذر تنفيذيّ** مُعلَن، فصُنّفت `UNREACHABLE_TERMINAL_CHAIN` — «كودٌ ميت» بحكم الحارس، بينما compose يُشغّلها في كلّ إقلاع.
- **والسبب في الحارس نفسه:** تعبيره كان `python\s+(?:/app/)?(scripts/…)` — أي يقرأ `/app/scripts/x.py` بوصفه `scripts/x.py` في **جذر المستودع**. وهذا **هو** الافتراض الذي أنتج العطل الذي أُصلحه: `/app` هو حيث تُنسَخ محتويات **جذر الخدمة**، وجذرُ المستودع `scripts/` ليس في الصورة إطلاقاً. الحارس كان يحمل الخريطة الخاطئة ويمرّ.
- **وعيبٌ ثانٍ فيه:** كان يَعُدّ **واردات** المُشغِّل جذوراً ولا يَعُدّ المُشغِّل نفسه. فما إن صار العامل وحدةَ منصّة حتّى بقيت **نقطة الدخول** طرفيّة.
- **وأنا أدخلتُ التناقض بيدي:** إبدالي حدّث **تعليق** الحارس ليقول `/app/workers/…` ولم يمسّ التعبير النمطيّ. فصار المصدر يصف سلوكاً لا ينفّذه — نفس صنف «رسالةُ الحارس جزءٌ منه»، هذه المرّة في تعليقه.
- **مُثبَت بزرعتين نُفِّذتا:** جعل المُشغِّل قناةً لا جذراً · وقراءة `/app` جذرَ المستودع. والعدّ: العمّال المُسجَّلون ٧ ⇒ ٨، والطرفيّ ١٥١ ⇒ ١٥٠.

## 2026-08-05 — كرّرتُ الدرس ٢١ بنفسي: الأثر يُحسَب على الدلتا **النهائيّة**

- **`capability-registry` حجب #797 بـ`missing_direct: ["PA-003"]`.** حسبتُ `Capability-Impact` وكتبتُه في متن الـPR، ثمّ عدّلتُ الالتزام مرّتين (أساس وحدات المنصّة، ثمّ حارس الوصوليّة) ودفعتُ بلا إعادة حساب.
- **وهذا حرفيّاً الدرس الذي اقتبستُه موافِقاً في الجلسة نفسها** بعد أن أوقعني على #795: الترتيب هو **محتوى نهائيّ ⇒ أثر ⇒ متن ⇒ دفع**، ولا يجوز أن يسبق أيٌّ منها ما قبله.
- **ولا يكفي تصحيح المتن وحده:** `capability-governance.yml` يعمل على `pull_request` بلا `types`، فلا يستمع لـ`edited` وrerun يُعيد استخدام الحمولة القديمة. الطريق الوحيد لالتقاط متن جديد **دفعةٌ تُطلِق `synchronize`**.
- **الدرس عن الدرس:** معرفتي بالقاعدة لم تمنع تكرارها، لأنّ الفعل الذي يكسرها (`--amend` بعد الحساب) لا يُنبّه أحداً. القاعدة المكتوبة لا تحرس نفسها — يحرسها أن يكون **الحساب آخر خطوة قبل الدفع**، لا خطوةً في منتصف العمل.

## 2026-08-05 — عمودٌ يُقرأ ولا يُكتَب: «٣ فاشلة» بلا سبب واحد

- **`_process_scene_index` كان يُرجِع `bool`،** فثلاث حالات فشل متمايزة تنهار إلى `False`: استثناء · وظيفة لم تكتمل · **عولِجت ولم تُحفَظ**. والسبب يُسجَّل في السجلّ ثمّ يُرمى، بينما `backfill_run_items.error` عمودٌ قائم منذ `v144` يبقى `NULL`.
- **وما يجعله ادّعاءً كاذباً لا ديناً صامتاً:** `get_backfill_run_status` **يقرأ** `error` ويعرضه. الواجهة مبنيّة لعرض سببٍ لا يصل.
- **والكاتب عرف أهمّيّة الحقل:** مسار «المشهد غير موجود» يكتب سبباً صريحاً. المسار العامّ وحده لا يكتب — وهو الذي تمرّ منه كلّ الأعطال الحقيقيّة. **الاستثناء المكتوب دليلٌ على أنّ القاعدة كانت معروفة.**
- **وأخطر الحالات الثلاث `processed_not_persisted`** — يُحذّر منها تعليقٌ في المصدر نفسه: عملٌ تمّ وأثرٌ ضاع، ولا يُميَّز عن انقطاع شبكة.
- **زرعتان كشفتا عيبَين فيّ لا في الكود:** تجهيزتي كانت `except Exception: skip` مطلقاً فجعلت عطباً في النوع **تخطّياً** لا حُمرة؛ وكاشفُ الزرعات عندي قرأ `1 error` بوصفه «مرّ» لأنّه يبحث عن `failed` وحدها. **أداةُ قياس التكذيب تحتاج تكذيبها هي أيضاً.**

## 2026-08-05 — تكذيبُ حارسٍ قائم كشف أنّ إصلاحي **ناقص وكاسر**

- **حارس «الحقيقة التشغيليّة» سقط على تحسين:** كان يطابق النصّ `"api_models.JobStatus.completed and result.get"` — أي **تعبيراً بعينه** — بينما تعليقه يقول إنّ العقد هو الخاصّيّة «completed **و** persisted». وفصلي الشرط إلى فحصَين متمايزَين (كي يُميَّز «لم تكتمل» عن «عولِجت ولم تُحفَظ») يُقوّي الخاصّيّة ويكسر الإملاء. **مطابقةُ نصٍّ حيث تلزم بنية**، ثالث موضع في هذا المستودع.
- **ولمّا كذّبتُ الحارس المُعاد صياغته انكشف عيبٌ فيّ:** لِـ`_process_scene_index` **ثلاثة** مسارات معالجة (Landsat · CDSE · VRT) كلّها تنتهي بالتعبير نفسه مكرّراً حرفيّاً. حوّلتُ **واحداً** وتركتُ اثنين يُرجِعان `bool` — والمُستدعي يقرأ `outcome.ok` ⇒ `AttributeError` على المسار **الأشيع** (CDSE). كان سيكسر الإنتاج، ولم تكشفه القراءة ولا اللِّنت ولا الاختبارات الجديدة، بل **زرعةٌ في حارسٍ قديم**.
- **والتكرار هو ما جعل الإصلاح الجزئيّ ممكناً:** ثلاث نسخ من منطق واحد. فصار في `_outcome_from_job` — موضع واحد، ويفرض الحارس أنّ كلّ مسار عودة يمرّ به.
- **وأخطأتُ مرّتين في الحارس نفسه:** أوّلاً قسمتُ النصّ عند مسار النجاح وبحثتُ عن كلمة `persisted` فيما قبله — فطابقت **سلسلة السبب** `'processed_not_persisted'` وبقي الحارس أخضر وشرطُ الحفظ مُعطَّل. مطابقةُ نصٍّ داخل فحصٍ يدّعي أنّه بنيويّ: العيب نفسه، طبقةً أعمق. الآن تُستجوَب **شروط `if`** بأشجارها.
- **والدرس المركّب:** تكذيبُ حارسٍ **قائم** بعد تعديل الكود ليس طقساً — هو ما كشف العطل الحقيقيّ. وحارسٌ سقط بعد تحسين لا يُعدَّل ليطابق الإملاء الجديد، بل يُعاد إلى الخاصّيّة التي كُتِب لها.
