...
## 2026-06-28 (مصالحة) — استرجاع مدخلَي #483/#484 + توثيق دمج فروع التفكيك (#534→#539)

**رأس `main`:** `9567658`. **مصالحة دماغ:** مدخلا جلسة سابقة (#483/#484) سقطا من `log.md` أثناء دمج
الدماغ عبر جلسات متوازية (صفوف `registry` لهما باقية: SEC-INFOLEAK + imagery)؛ يُعاد توثيقهما هنا
بصدق، + توثيق جلسة دمج الفروع.

- **#483 (`09eeaeb`) — كنس تسريب المعلومات + عزل المستأجرين:** عبر ≈١٢ خدمة — لا نصّ استثناء في
  استجابات HTTP (`str(e)`⇐رموز عامّة)، health بلا broker URL/ERP url/UID، تحقّق tenant_id (regex).
  **إصلاحا IDOR:** `reports.py::operation_report_csv` يرفض tenant مختلف (مقارنة `str()` على الطرفين) ·
  `video-processor::stop_stream` كان `pop()` قبل فحص الملكيّة ⇒ حذف عابر، صار get→assert→pop + حجب
  اعتماد الكاميرا.
- **#484 (`b744f00`) — تصحيح تدفّق مؤشّر الصور:** هندسة محور جيوديسيّة WGS84 (تطابق frontend/backend) ·
  تحديث geometry يحدّث `area_ha`/`lat`/`lon` · سلسلة زمنيّة صارمة + `list_asset_dates` يرطّب تواريخ
  الـtimeline من `raster_assets` بعد إعادة التشغيل · واجهة خريطة المؤشّر.
- **جلسة دمج الفروع (#534→#539):** دُمجت ٥ فروع تفكيك (frontend SplitDrawMap + `fields.py`→
  soil-lab/timeline/agronomy/scouting) لجلسة `017Ee5` **تسلسليّاً** مع إعادة تأسيس على main بعد كلٍّ
  وإعادة توليد حزمة الإصدار + حارس التفكيك. **صدق التزامن:** الجلسة المالكة كانت تُعيد تأسيس فروعها
  (force-push) أثناء العمل ⇒ تبنّيتُ نسخها المُعاد تأسيسها بدل clobber؛ تعارض #539 الكبير (≈٢٥٠ سطراً)
  انتظرتُ rebase المالك له بدل حلّ يدويّ متهوّر. **#540** (مرآة Docker `mirror.gcr.io`) أُغلق **مكرّراً
  لـ#538** (نفس الإصلاح سبقه على main) — لا دمج صوريّ.

**ملاحظة:** `hot.md` لم يُمسّ (تركيز جلسة أخرى حيّ).

---

## 2026-06-28 — تراكب المؤشّر + تدقيق + تفكيك تدريجيّ + قفل تبعيّات (#506→#524)

**رأس `main` بعد الجلسة:** `305eeeb`. ثلاثة محاور: إصلاح عرض، تحقّق تدقيق، تفكيك ضخم + قفل تبعيّات.

- **إصلاح تراكب المؤشّر (#506 `4e87e0e`):** صورة قديمة لمشكلة عرض الصور الجوّيّة/الخريطة/المؤشّر داخل
  الحقول. تحقيق معمّق بـ3 وكلاء استكشاف متوازية ⇒ السبب الجذريّ: حاوية `sahool-platform` لا تحمل
  `SAHOOL_AGENT_TOKEN` فيفشل التمرير المُصادَق للراستر (تراكب فارغ صامت). الإصلاح:
  `SAHOOL_AGENT_TOKEN: ${SAHOOL_AGENT_TOKEN:?…}` في كتلة بيئة المنصّة (`docker-compose.v9.yml`) +
  راوتر تمرير `field_imagery_backfill` في `routers/fields.py` (يحقن `X-Agent-Token` + هندسة الحقل
  كـ`clip_polygon_geojson`). **تأكيد حيّ تحت `sahool_app` بيد Ops.**
- **تدقيق خارجيّ مُتحقَّق منه (#507 `7b17133`):** «تأكّد من الادعاءات» — فُحِص كلّ بند P0/P1 مقابل
  الكود نقديّاً. المؤكَّد فقط طُبِّق: محاذاة انجراف `MANIFEST`/`.env-test` + بداية أداة قفل التبعيّات
  (`scripts/deps/lock.sh` + README). غير المؤكَّد رُفِض بصدق (لا تطبيق أعمى لتقرير خارجيّ).
- **التفكيك التدريجيّ (سلوك محفوظ، #508→#524):** قرار المستخدم تفكيك الملفّات الضخمة على مراحل آمنة:
  - **raster-service:** #508 (`dd6b711`) نماذج/enums ⇒ `models.py` · #509 (`85d9143`) **شبكة أمان CI**
    (وظيفة `raster-tests` — كانت اختبارات الراستر بلا تشغيل في أيّ CI، فأُسِّست قبل أيّ تفكيك عميق) +
    إصلاح حارسين ساكنين متعفّنين · #510 (`cc7e02b`) مساعِدات صور نقيّة ⇒ `imagery_utils.py`.
  - **fields.py:** #511 (`911fdaf`) نماذج طلب ⇒ `field_request_models.py` · #515 (`28694f7`) منطق نقيّ
    (دمج/انقسام/تعارض) ⇒ `field_logic.py`.
  - **api.ts (3688→~3081):** #512 (`11b396f`) عملاء HTTP ⇒ `apiClients.ts` · #513 (`55ddf9c`) صور ⇒
    `imageryApi.ts` · #514 (`7e6fa25`) mocks ⇒ `apiMocks.ts` · #519 (`42d0170`) vegetation/weather/soil
    ⇒ `vegetationApi`/`weatherApi`/`soilApi` · #524 (`03fc9fd`) fields/account ⇒ `fieldsApi`/`accountApi`.
  - **useApi.ts (1793→1660):** #516 (`b250acb`) طقس/تربة/مفاتيح ⇒ `useWeatherApi`/`useSoilApi`/`useApiKeys`
    · #522 (`305eeeb`) معايرة ⇒ `useCalibrationApi.ts`.
  - **auth/main.py (1654→1519):** #517 (`4db3b9c`) نماذج ⇒ `models.py` · #523 (`0f84254`) بريد/SMS ⇒
    `mailer.py`.
  - **النمط:** إعادة تصدير (`export *`/barrel) فالعقد العامّ ثابت؛ الحُرّاس النصّيّة المقترنة تُحدَّث
    لتقرأ الموضع الجديد (api.ts + الوحدة) لا تُضعَّف؛ الاستيرادات غير المستعملة تُقلَّم (درس TS2459/TS7006).
- **قفل التبعيّات (#520 `61dba5c` · #521 `9f8a1e1`):** #520 أقفال `*.lock` مُلتزَمة للمسار الحرج (حلّ
  عابر كامل + تجزئات) + بوّابة CI «Dependency Lock Drift» + جعل `lock.sh` مستقلّاً عن بيئة المُولِّد
  (`--universal --python-version` + `--custom-compile-command` لإصلاح علّة الانجراف الزائف). #521 صور
  Docker (auth/platform/guardrails) تُثبّت من الأقفال بـ`--require-hashes` + محاذاة `PY_TARGET=3.11`
  (مطابقة لصور `python:3.11-slim`) + جعل auth/Dockerfile ينسخ `*.py` (لتضمين وحدات #523)؛ تُحقّق ببناء
  venv 3.11 فعليّ (تثبيت بالتجزئات نجح). **مؤجَّل بصدق:** بناء صور Docker كاملة + `pip-audit` على `*.lock`.
- **النمط التشغيليّ:** الدفعة الأخيرة (#521/#522/#523/#524) نُفِّذت بـ4 وكلاء متوازيين في عُزلة worktree
  (ملفّات منفصلة)؛ دمج متسلسل مرتَّب (#521 أوّلاً) مع إعادة توليف لكلّ PR لحلّ تعارض حزمة الإصدار
  المتقلّبة (generated_at/file_count/checksums).
...
(القرار→التنفيذ الفيزيائيّ) — لكن **قرار سلامة صريح من المستخدم: محاكاة-أوّلاً**. السياق: الحلقة مغلقة أصلاً
**عبر الإنسان** (`decision_dispatch` يُدرِج queued ثمّ يُخطِر إنساناً)؛ التنفيذ الفيزيائيّ (MQTT) محجوز في
`actuator-service` المُحصَّن الذي يُؤتمت من عتبات المستشعرات فقط ولا يقرأ قرارات المنصّة (grep=0 مؤكَّد). الجسر
**نظامان منفصلان عمداً** («البشر أوّلاً، المضخّات آخراً») ⇒ بناؤه تجاوز حدّ أمان ⇒ بُني محروساً مزدوجاً.
- **`actuator-service/main.py`** (الجسر داخل main.py لا وحدة منفصلة — `shared` بجذر الريبو، استيراد شقيق يكسر اختبار الـactuator): `dispatch_consumer_loop` (نظير `mqtt_sensor_listener`) يطالب `dispatch_decisions[queued]` ذرّيّاً (`UPDATE…WHERE exec_status='queued'…FOR UPDATE SKIP LOCKED` ⇒ لا إطلاق مزدوج) وينشر عبر `send_mqtt_command` القائم.
- **حراسة مزدوجة:** علم `FEATURE_DISPATCH_ACTUATOR` (default OFF ⇒ مسار الإنسان يبقى المستهلك) **+** `ACTUATOR_MODE=real` (وإلّا simulation ⇒ لا حركة فيزيائيّة). **قائمة مخاطر** `low,medium` افتراضيّاً — HIGH/CRITICAL وrisk=NULL **لا تُؤتمت** (تبقى للإنسان).
- **حارس عزل فيزيائيّ:** `_device_belongs_to_tenant` يتحقّق أنّ الجهاز يخصّ مستأجِر القرار قبل النشر (نظير `_authorize_device_control`) — **يمنع تحكّماً عابراً للمستأجرين** ويُفشِل بأمان تحت RLS صارم (fail-closed). (أُضيف من مراجعة عدائيّة ذاتيّة.)
- **صدق صريح:** `send_mqtt_command` ينشر بلا ack ⇒ `executed` = «نُشِر للوسيط» لا «الصمّام تحرّك» — مُعلَن في الكود وسجلّ `device_commands_log` (mode موسوم). fail-safe: أمر فاسد/استثناء ⇒ failed بلا كسر الحلقة.
- **حدود مُعلَنة (خارج النطاق):** تعافي صفّ عالق في `dispatched` (لا TTL) · تكامل `execution_ledger` الكامل · ack حقيقيّ من firmware.

**تحقّق:** ٨ اختبارات نقيّة جديدة (`test_dispatch_bridge.py`: علم OFF · HIGH مستبعَد · فكّ أمر فاسد→failed · حالة النتيجة) + ٨ انحدار dedup أخضر · ١٨ انحدار dispatch المنصّة (مسار الإنسان غير ممسوس) · المفتّش exit 0 · ruff كامل · **مراجعة عدائيّة: الادّعاءات الخمسة تصمد**.
