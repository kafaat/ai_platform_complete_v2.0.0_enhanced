...
- **مُتروك للمالك بصدق:** ٢٤ متغيّراً بلاحقة `_KEY` (منها `FCM_SERVER_KEY` و`SEASON_EDGE_HMAC_KEY`) تُصنَّف `configuration` لا `secrets`. توسيع `SECRET_MARKERS` يحتاج قاعدة تفصل مفاتيح التوقيع عن `JWT_PUBLIC_KEY` الذي **يصحّ** ألّا يكون سرّاً — قرار بأثر ١٣ خدمة، لا تعديل قائمة يُدسّ في شريحة غير ذات صلة.
