...
## 2026-07-28 — إغلاق CAPABILITY-CORES-NOT-WIRED (0/4 ⇒ 4/4) + هويّة بناء غير مزيّفة

- **الشرائح:** #677 `ac36c03` (equipment_intelligence.v2 fail-closed + الراتشِت نفسه) · #678 `fce91a9` (economic_scenario_comparison.v2) · #679 `aaefbe6` (مداخل بناء غير قابلة للتغيير) · #680 `4eded7a` (yield_intelligence) · #682 `247c69c` (canonical_field_state) · #683 `121ab09` (مُحمِّل الخدمات).
- **عيوب اختلاق أُصلحت قبل الربط لا بعده:** `equipment_intelligence` كان يُسقِط الغياب إلى 250 ساعة فاصلاً و0.0 خطّ أساس ⇒ `service_due=1` بلا قيد واحد، وجدول `equipment` لا يحمل العمودين أصلاً. و`yield_intelligence` كان سيُعطي متوسّطاً يبدو كإنتاجيّة الحقل من صفحة 1000 نقطة من حصاد 40,000، أو من موسمَين — الآن `not_evaluated` باسم السبب.
- **النواة الرابعة بأمر المالك بعد اعتراضي:** رُبطت بإعلان النواقص لا بتلفيقها؛ اختبار على AST يمنع تمرير حمولة بدل `None`، واختبار حامل يمنع رفع `operational_eligible` بمنتَجين من ثلاثة.
- **#681 أُغلق** بعد أن كذّب CI ادّعاءه المركزيّ («صفر إصلاح»): ١١ فشلاً في `-m unit` و٣ في التكامل. قُسّم إلى أربع، والأولى دُمجت (#683). التصحيح كُتب **فوق** النصّ والأصل حُفظ — حذف ادّعاء خاطئ أسوأ من تسجيل أنّه كان خاطئاً.
- **أخطائي المسجَّلة:** نقل «صفر إصلاح» بلا تقييد (القياس كان لـ`-m unit` وحده) · وصف MCP 400 بأنّه ثغرة تخويل قبل الفحص (التخويل موصول ويرفع 401؛ العيب في الترتيب) · كنس `--check` وحده (46 من 211) · إغفال `ruff format --check` كلّيّاً · تشغيل ruff 0.16.0 مقابل 0.15.8 المثبَّت رغم تعليق في `ci.yml:431-434` يحذّر من هذا بعينه.
- **الحالة النهائيّة:** الراتشِت 4/4 · خام 631 · بنية 4 · نطاق 627 ≤ 629 (الهامش 2) · `runtime_verified=0` · `production_certified=0` · management 81/81.
