...
## 2026-08-03 — أوّل شريحة وصل من الموجة المؤجَّلة: `canonical_phenology_state`

- **القاعدة طُبِّقت كما كُتِبت:** الوحدة دخلت الأساس (675⇒676) في نفس التغيير الذي أضاف مستهلكها وحارسه. لا «يهبط الآن ويُوصَل لاحقاً».
- **المستهلك حقيقيّ ومقيس:** النقطة القائمة كانت تشتقّ المرحلة من عمر التقويم وحده وتقدّمها بلا تمييز؛ فتعارضُ خبيرين كان يُعطي جواب اتّفاقهما نفسه. الآن `observed` آمرة و`blocked` تُعلَن عند التعارض. **بلا مسار جديد** (`?canonical=true` على نفس الـGET)، والردّ الافتراضيّ لم يُمَسّ.
- **رفضتُ مصدر GDD المتاح لأنّه كان سيغسل محاكاة:** `wofost_seasons.gdd_accumulated` موجود ويغري، لكنّه ناتج محاكاة. مُرِّر `None` والقيد مكتوب في docstring الراوتر وفي الحارس (`accumulated_gdd=None` مُلزَم نصّاً).
- **الحارس أُثبِت بأربعة زروع، كلٌّ أسقط اختباره المقصود وحده** — ومنها زرع «الاستيراد بلا نداء»، وهو الفخّ الذي وقعتُ فيه مرّتين سابقاً (ذكر الرمز ≠ استهلاكه).
- **واختبار قديم هشّ صُحّح بدل الالتفاف عليه:** `test_migration_is_append_only_rls_and_idempotent` كان يثبّت `entries[-7:]` حرفيّاً بينما تعليقه نفسه يقول إنّ الثابت هو «بقاء v206 آخِراً». أُعيدت صياغته على الثابت (v206 آخِر ومرّة واحدة · ترتيب سلسلة دفتر المزرعة نسبيّ وقبله)، وأُثبِت بزرعين: ترحيل بعد v206 ⇒ FAIL، وسلسلة مُعاد ترتيبها ⇒ FAIL.
- **وحارس #775 أثمر في أوّل استعمال:** `test_migration_manifest_execution_guard` أكّد وصل `v224` في البيان والمشغّل فور إدراجه — وهو بالضبط ما لم يكن موجوداً حين مرّت خمسة عشر ترحيلاً بلا تنفيذ.
- **مُستثنى بصدق:** سجلّ مصادر الحقيقة الزراعيّة (٢٧ مصدراً) — صفّه الطوريّ يُعلن كاتباً لا وجود له، فلم يُنزَل ولم يُخفَّف اختباره صامتاً؛ أُزيل من الشريحة بتعليق يسمّي السبب.
