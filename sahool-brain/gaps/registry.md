...
| CP997-08-EDGE-IDEMPOTENCY-KEY-WIDTH-API-128-VS-DB-32-01 | الـAPI يقبل مفتاحاً حتّى 128 حرفاً بينما العمود `VARCHAR(32)` — طلبٌ صالح بطول 33–128 يسقط عند حدّ القاعدة (500) لا عند التحقّق (422). | edge · db | [`services/sahool-platform/api/edge_models.py:21`](../../services/sahool-platform/api/edge_models.py) · [`migrations/v9_edge_idempotency.sql:7`](../../migrations/v9_edge_idempotency.sql) | **fixed** (#997، مراجعة Copilot) — `v231` يوسّع العمودَ إلى عرض العقد (128)؛ مفاتيحُ الجهاز تبقى 32. |
