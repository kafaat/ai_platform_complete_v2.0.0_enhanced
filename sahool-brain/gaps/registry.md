...
| PRODUCTION-CERTIFICATION-VERDICT-IS-FORGEABLE-AND-UNREACHABLE-01 | حكمُ الشهادة الإنتاجيّة يفشل **في اتّجاهين متعاكسين معاً**: المسارُ الشرعيُّ لا يستطيع النجاح (الأدلّةُ لا تنتقل بين وظائف الـworkflow)، والمسارُ المزيَّف ينجح (المُحكِّم يقرأ سلسلةَ الحالة ولا يقرأ ما تحتها). | governance · release | [`production_certification_blockers_status.py`](../../scripts/ci/production_certification_blockers_status.py) (`BLOCKERS:17-22` · شرطُ الحكم `:46-49`) · [`production_evidence_pack_guard.py`](../../scripts/ci/production_evidence_pack_guard.py) (`check_files:145-193`) · `.github/workflows/production-certification-blockers.yml` | **open** — **P0.** مُكذَّبٌ بالتنفيذ في رملٍ معزول، لا موصوفاً:** أربعةُ ملفّات JSON مكتوبةٍ باليد أنتجت `production_certified=true` وخروجاً `0` — وفيها بصمةٌ من **أربعين صفراً**، و`repository: attacker/x`، وقوائمُ فارغة، وقيمُ `null`، و**إعفاءٌ بلا حقل سبب أصلاً**. **والعلّةُ البنيويّة أنّ الحكمَ ليس حيث الصرامة:** `production_evidence_pack_guard` يفرض provenance (repository/workflow/run_id/commit) وشكلَ البصمة وطزاجةَ الختم — و`production_certification_blockers_status` **لا يستدعيه قطّ**، بل يقارن `row["status"] == "verified"` نصّاً. فهما حكمان لحقيقةٍ واحدة، **ولا يتّفقان حتّى**. **وأربعُ ثغراتٍ مقيسةٌ بالسطر:** (١) `BLOCKERS` أربعةٌ لا خمسة — **لا حاجبَ لـGUARDS** فلا يُقيَّم أصلاً. (٢) `waived_with_reason` يمرّ **بلا قراءة سبب** — والكلمةُ في اسم الحالة لا في البيانات، فالإعفاءُ اسمُه شرطُه. (٣) لا تحقّقَ من مطابقة `repository`/`workflow` للبيئة الفعليّة، ولا من tree SHA، ولا توقيعَ ولا attestation. (٤) الحقولُ الدنيا تُفحَص **وجوداً لا قيمة**، فقائمةٌ فارغة تُرضيها. **والاتّجاه الآخر:** لا `upload/download-artifact` بين الوظائف، ووظيفةُ الحكم تعمل على checkout جديد فترى placeholders المستودع — فالإغلاقُ الصادق **مستحيل** والإغلاقُ بالتزييف **متاح**. **نموذج التهديد مُعلَنٌ بلا تضخيم:** ليس استغلالاً بعيداً بلا صلاحيّة؛ يلزمه تعديلُ أدلّة المستودع أو مسار CI. لكنّه يُلغي الثقةَ ببوّابة الإصدار عند اختراق حساب مطوّر أو خطأ مراجعة. **والعلاجُ المُقترَح تعريفٌ واحد لا اثنان:** يُفوِّض المُحكِّمُ إلى الحارس الصارم ولا يحمل حكماً ثانياً · يُضاف الحاجبُ الخامس · تُقاس القيمُ لا المفاتيح · تُرفَض البصماتُ المُنحلّة وتُطابَق البيئةُ الفعليّة · ويحمل كلُّ إعفاءٍ سبباً ومالكاً ونطاقاً وانقضاءً وموافقاً مستقلّاً. **غيرُ مُصلَحٍ في هذه الشريحة ولا يُدَّعى** — سُجِّل بشاهده. |
| NATS-BROKER-HAS-NO-AUTHENTICATION-SO-ACTUATOR-COMMANDS-ARE-UNGUARDED-01 | وسيطُ NATS في مكدّس v9 بلا مصادقةٍ البتّة. **وقد سجّلتُ هذا الصفَّ أوّلاً بدعوى أثرٍ فيزيائيّ، والقياسُ نقضها** — فالموضوعُ المنشور بلا مستهلكٍ في المستودع، ووسيطُ MQTT الذي يبلغ الأجهزة فعلاً **مُصادَقٌ عليه**. | security · runtime · physical-effect | [`nats/nats.conf`](../../nats/nats.conf) · [`phase_runtime_workers.py:507`](../../services/sahool-platform/api/phase_runtime_workers.py) · [`actuator_runtime.py:584,1086`](../../services/actuator-service/actuator_runtime.py) · `mosquitto.conf` (`allow_anonymous false`) | **open** — **وأوّلُ ما فيه تصحيحُ خطأين ارتكبتُهما في نصّه الأوّل، وكلاهما من الصنف الذي يُغلقه هذا السجلّ:** (١) سمّيتُ الموضوعَ `actuator.command` — **أخذتُه من تقريرٍ منقولٍ ولم أقِسه**؛ والمقيسُ أنّ المنشور `sahool.actuator.dispatch.requested` (`phase_runtime_workers.py:507`)، بينما `actuator.command` شكلُ موضوعِ **MQTT** (`sahool/actuator/{device_id}/command`) — أي أنّي **خلطتُ ناقلين**. (٢) ادّعيتُ أنّ «حدَّ الأثر الفيزيائيّ يقوم على تقسيم الشبكة وحدَه» — **باطلٌ بالقياس**: `mosquitto.conf` يحمل `allow_anonymous false` و`password_file`، والاعتماداتُ تُزوَّد بـ`mosquitto_passwd` عند الإقلاع. **فالمسارُ الذي يبلغ الأجهزة مُصادَقٌ عليه، ولم يكن دفاعُه التقسيمَ قطّ.** **والمقيسُ الباقي، مُصفًّى ممّا لا يسنده دليل:** `nats/nats.conf` يحمل `http` و`jetstream` **فقط** — لا `authorization` ولا `accounts` ولا `users`؛ وتعليقُ رأسه «منفذ العميل 4222 افتراضيّ ولا يحتاج تصريحاً» صادقٌ عن **المنفذ** صامتٌ عن **المصادقة**، وغرضُه المُعلَن حدودُ التخزين فلم تدخلها قطّ. فالوسيطُ مفتوحٌ لكلّ من بلغ الشبكةَ الداخليّة. **واكتشافٌ ثانٍ أثقلُ من الأوّل، وارد من جلسةٍ شقيقةٍ وأُكِّد هنا:** `sahool.actuator.dispatch.requested` يظهر في **خمسة مواضع فقط** — تعليقٌ ونشرٌ واحد و**ثلاثةُ اختبارات** — و**صفرُ مشتركين**. و`actuator-service` يشترك عبر **MQTT** (`actuator_runtime.py:1086`) لا NATS. **فالنشرُ غيرُ المُصادَق لا يبلغ شيئاً اليوم** — ولا يُقرأ ذلك أماناً بل **مسارَ إرسالٍ غيرَ مكتمل**: إمّا أنّ المستهلكَ خارجيٌّ غيرُ موثَّق، وإمّا أنّ الحلقةَ مبتورة. **والصفُّ يبقى `open` بسببين لا بالسبب الأوّل:** وسيطٌ بلا مصادقةٍ يبقى عطلاً وإن لم يُستَغَلّ اليوم (فأيُّ مشتركٍ يُضاف غداً يرث الفتح)، ونشرٌ بلا مستهلكٍ عطلٌ قائمٌ بذاته يحتاج حسماً: يُوثَّق مالكُ المستهلك الخارجيّ أو تُغلَق الحلقة. **وحدُّ صدقٍ على مصدر هذا الصفّ نفسِه:** بُني أوّلاً على قياسٍ حيٍّ منقول (`nats_publish=succeeded_unauthenticated`) لم أُعِد التحقّقَ من **تسميته** — والدرسُ أنّ الشاهدَ الحيَّ يُصدَّق في **ما قاس** لا في **ما سمّى**. |
