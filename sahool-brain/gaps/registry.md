...
|---|---|---|---|---|
| GUARDRAILS-TOKEN-MESSAGE-NAMES-THE-WRONG-ROUTE-01 | تبعيّةُ توكن الخدمة تحرس `/v1/validate` و`/v1/evaluate` معاً، ورسالتا فشلها (503 و401) تُسمّيان `/v1/validate` حرفيّاً. فمن يصطدم بـ401 على التقييم يطارد مساراً لم يستدعه — ورسالةٌ خاطئة في نداء الطوارئ أسوأ من مقتضبة. | guardrails · observability | [`main.py`](../../services/guardrails-engine/main.py) (`_require_service_token`) · [`test_guardrails_evaluate_http_contract.py`](../../tests_v9/test_guardrails_evaluate_http_contract.py) | **fixed** — الرسالتان لا تُسمّيان مساراً بعينه، ويحرس ذلك تأكيدٌ في اختبار الرفض. مراجعةُ #1013. |
| GUARDRAILS-EVALUATE-HAS-NO-HTTP-CONTRACT-TEST-01 | `/v1/evaluate` كان مُختبَراً عبر مِرقاةٍ اصطناعيّة تُحاكي محرّك الطبقات فقط؛ لا اختبارَ يستدعي المسارَ على التطبيق الحقيقيّ. فانحدارُ توصيل — سقوطُ التسجيل، أو حذفُ التبعيّة، أو ارتدادٌ إلى الفرع الكاتب — يمرّ وكلُّ اختبارات B6 خضراء. | guardrails · ci | [`test_guardrails_evaluate_http_contract.py`](../../tests_v9/test_guardrails_evaluate_http_contract.py) · [`validation.py`](../../services/guardrails-engine/routers/validation.py) | **fixed** — ستُّ حالات على التطبيق الحقيقيّ: مصرَّحٌ وغيرُ مصرَّح، `evaluation_only=true` و`allowed=false` وبلا معرّف سير موافقة، والمسارُ القديم يبقى على سلوكه. **مُكذَّبةٌ بالتنفيذ:** أربعُ طفراتٍ مزروعة كلُّها مقتولة (ارتداد · حذفُ تبعيّة · إلغاءُ تسجيل · عودةُ الرسالة المُضلِّلة). |
| IMAGERY-AUTOMATION-TABLE-HAS-NO-LIVE-PG-WITNESS-01 | `imagery_automation_fields` — الجدولُ الذي وُلد منه العطلُ الصامت في #1009 — بلا شاهدٍ حيّ، بينما لأربعة جداول غيره شاهدُها. والشاهدُ العامّ يُعلن حدَّه: يُنشئ جدولَه وسياستَه بنفسه فيُثبِت أنّ **المحرّك يعزل** لا أنّ **سياسات الهجرات صحيحة**. | platform · rls · ci | [`test_imagery_automation_rls_live_pg.py`](../../tests_v9/test_imagery_automation_rls_live_pg.py) · [`ci.yml`](../../.github/workflows/ci.yml) (`Imagery automation RLS certification`) | **fixed** — شاهدٌ يقيس السياسةَ المُهاجَرة نفسَها بدورٍ مُقيَّد: الدورُ يُقاس أوّلاً، ثمّ موجبٌ وسالب، ثمّ **صفرٌ بلا سياق** (العطل مُثبَتاً لا موصوفاً)، وإبطالُ فراغٍ بصفوفٍ يراها المُدير. مُكذَّبٌ على PostgreSQL 16 محلّيّة: إلغاءُ FORCE · إلغاءُ RLS · منحُ BYPASSRLS ⇒ ثلاثُ حُمرٍ برسائلَ مسمّاة. |
| B6-STRUCTURED-PROPOSAL-CORE-LANDED-DISABLED-01 | نواةُ B6: عقدُ JSON محدود يختار فيه النموذجُ قيماً رقميّة مطابقةً لمنتجات المالك ومعرّفاتِ مقترحاتٍ قائمة؛ يُرفض النصُّ الحرّ والجرعاتُ المولَّدة والسياقُ الماليّ المُرسَل من النموذج وتغييرُ الهويّة والحقولُ الزائدة ومفاتيحُ JSON المكرّرة. ومسارُ تقييمٍ بلا كتابة `/v1/evaluate` يُرجِع دائماً `allowed=false` و`evaluation_only=true`. | ai · guardrails | [`structured_advisory.py`](../../shared/ai/structured_advisory.py) · [`validation.py`](../../services/guardrails-engine/routers/validation.py) · [`test_b6m4_structured_advisory.py`](../../tests_v9/test_b6m4_structured_advisory.py) | **open — نواةٌ مهبوطة غير مُفعَّلة.** لا تفكّ حجبَ النصّ الحرّ في الدردشة القائمة، ولم تُجرَّب على محرّك الحواجز المنشور ولا على قواعده الكيميائيّة والاقتصاديّة الحقيقيّة. ربطُ معاينة المالك بالدردشة عملٌ لاحق. |
| M4-BOOTSTRAP-JOURNAL-CORE-LANDED-DISABLED-01 | نواةُ M4: سجلُّ تهيئةٍ قابل للاستئناف بخمس مراحل (تربة · اشتراك طقس · تاريخ طقس · تاريخ صور · تحديث حالة)، نافذةٌ تقويميّة ٢٤ شهراً مثبَّتة من تاريخ الإنشاء الأصليّ لا تتحرّك بإعادة المحاولة، ومفتاحُ طلبٍ ثابت لكلّ مرحلة، ومخزنٌ على `processing_jobs` القائم بقفلٍ استشاريّ و`FOR UPDATE SKIP LOCKED` ورمزِ صلاحية. | platform · onboarding | [`field_bootstrap.py`](../../shared/field_bootstrap.py) · [`onboarding.py`](../../services/sahool-platform/api/onboarding.py) · [`test_b6m4_field_bootstrap.py`](../../tests_v9/test_b6m4_field_bootstrap.py) | **open — نواةٌ مهبوطة خلف راية مطفأة** (`FIELD_BOOTSTRAP_ENABLED` غير مضبوطة في compose ولا في `.env.example`). لا تُشغَّل مزوّداتُ التربة والطقس والصور تلقائيّاً، ولم يُختبَر المخزنُ على PostgreSQL حيّ ولا تحت RLS. `agronomy_ready` يأتي من إيصال المالك وحده. |
| FIELD-CREATE-INTENTS-MERGED-NOT-OVERWRITTEN-01 | حزمةُ B6/M4 كانت تُعيد كتابة كتلةِ نيّات الإنشاء في راوتر الحقول لتسجّل الصورَ ومهمّةَ التهيئة، وكانت `main` قد استبدلت الكتلةَ نفسَها لتسجّل الصورَ **والطقس** (#1009). أخذُ أيٍّ منهما حرفيّاً يُسقِط عملَ الآخر **صامتاً** — تعارضٌ دلاليّ لا نصّيّ، ولا يكشفه فحصُ بصمةٍ لأنّ البصمة تكشف الانحراف لا معناه. | platform · onboarding | [`onboarding.py`](../../services/sahool-platform/api/onboarding.py) (`register_field_creation_intents`) · [`fields.py`](../../services/sahool-platform/api/routers/fields.py) · [`test_b6m4_platform_store.py`](../../tests_v9/test_b6m4_platform_store.py) | **fixed** — مدخلٌ واحد يستدعي `register_field_tracking_intents` (صور + طقس) ثمّ مهمّةَ التهيئة، ويُرجِع الثلاثة صراحةً. وحالتان تحرسانه: بإحداثيّة مركز وبدونها. سقفُ الراوتر لم يتغيّر (4199). |
| PROMETHEUS-TARGETS-DECLARED-NEVER-IMPLEMENTED-01 | ثلاثةُ أهدافٍ مُعلَنة في `prometheus.yml` لخدماتٍ لا تُعرّف `/metrics` أصلاً (`indicators-service` · `weather-service` · `edge-inference`) ⇒ ٤٠٤ ⇒ `up=0` **أبداً**، وقاعدةُ `alerts.yml` على `up==0` تُسمّي الثلاثة بالاسم فتُطلِق دائماً حتّى تصير ضجيجاً يُتجاهَل. ومراقبةٌ تكذب دائماً أسوأ من غيابها. | observability · ci | [`prometheus.yml`](../../prometheus/prometheus.yml) · [`alerts.yml`](../../prometheus/alerts.yml) · [`test_prometheus_target_contract.py`](../../tests_v9/test_prometheus_target_contract.py) | **fixed** — أُضيفت النقاطُ الثلاث بنمط `generate_latest` القائم في سبع خدمات أخرى، وحُرِس الصنفُ باختبارٍ يشتقّ الهدفَ من الإعداد والمنفذَ من `EXPOSE` والنقطةَ من مصدر الخدمة. أُبلِغ عنها في جولةٍ حيّة 2026-09-16 وكانت تُقرأ عطلَ تشغيل؛ وهي تعارضٌ دائم بين الإعدادات والشيفرة. |
| PROMETHEUS-EDGE-SCRAPED-ON-THE-WRONG-PORT-01 | هدفُ `edge-inference` كان `sahool-edge:8000` والخدمةُ تُصرّح `EXPOSE 8100` وتُشغّل uvicorn عليه ⇒ **رفضُ اتّصال** لا ٤٠٤، وهو أصعبُ تشخيصاً لأنّه يبدو عطلَ شبكةٍ لا عطلَ إعداد. نفسُ صنف أسماء DNS الذي أُصلِح يدويّاً مرّتين ومكتوبٌ في رأس الملفّ. | observability · ci | [`prometheus.yml`](../../prometheus/prometheus.yml) · [`Dockerfile.arm64`](../../services/edge-inference/Dockerfile.arm64) (`EXPOSE 8100`) | **fixed** — صُحِّح المنفذ، ويحرسه `test_scrape_port_matches_the_port_the_image_actually_exposes`. |
| PROMETHEUS-ALERT-NAMES-A-JOB-NOBODY-SCRAPES-01 | قاعدةُ `SahoolRasterTileJSONUnavailable` تُسمّي وظيفتَي `raster-tiler` و`titiler` ولا هدفَ سحبٍ لأيٍّ منهما ⇒ التنبيهُ **لا يُطلِق أبداً**: صمتٌ يُقرَأ سلامةً، وهو عكسُ الصفَّين أعلاه تماماً. لم تره الجولةُ الحيّة لأنّ ما لا يُسحَب لا يظهر هدفاً ساقطاً. | observability · ci | [`alerts.yml`](../../prometheus/alerts.yml) (`:165`) · [`test_prometheus_target_contract.py`](../../tests_v9/test_prometheus_target_contract.py) | **fixed** — `raster-tiler` غلافُنا: أُضيفت له `/metrics` وهدفُ سحبٍ على 8088 فصارت القاعدة قابلةً للإطلاق. و`titiler` رُفِع اسمُه لأنّ صورتَه منبعيّة غير معدَّلة لا تكشف مقاييس، وكان ذكرُه يدّعي تغطيةً غير قائمة — الفجوةُ في الصفّ التالي لا مُسكَتة. |
| TITILER-UPSTREAM-IMAGE-HAS-NO-METRICS-01 | صورةُ `sahool-titiler` منبعيّة (`developmentseed/titiler`) بلا `/metrics`، فلا تغطية مقاييس عليها مباشرةً؛ الغطاءُ القائم هو غلافُنا `raster-tiler` وحده. | observability · third-party | [`docker-compose.v9.yml`](../../docker-compose.v9.yml) (`sahool-titiler`) | **open** — تغطيتُها تحتاج إمّا واجهةَ تصدير جانبيّة أو غلافاً، وكلاهما قرارُ نشرٍ لا إصلاحُ إعداد. مُسجَّلة كي لا يُظنّ الحذفُ من قاعدة التنبيه إغلاقاً. |
| EXTERNAL-RUNBOOK-TOOL-REJECTS-THE-LIVENESS-CONTRACT-01 | أداةُ تشخيصٍ خارجيّة (`v25_sahool-internal`) عدّت ٢٧ إخفاقاً كاذباً لأنّ `alive` ليست في قائمة حالاتها المقبولة، فبقي الأداءُ **بلا قياس** لا فاشلاً؛ ومِسبارُ RLS فيها استعمل `SET LOCAL app.current_role` وهو خطأ نحويّ لأنّ `current_role` كلمةٌ محجوزة، فلم يَقِس شيئاً. | tooling · external | مِسباراتُنا نفسُها تشترط `alive` صراحةً: [`functional_probes/`](../../runtime-verification/functional_probes/) · وشيفرتُنا تستعمل `set_config` حصراً: [`api/main.py:646`](../../services/sahool-platform/api/main.py) · [`auth/main.py:286`](../../services/auth/main.py) | **fixed (في الحزمة، خارج المستودع)** — سلّم المالكُ الحزمة فأُصلح العطلان فيها وأُعيد التحقّق **على PostgreSQL 16 حيّ** بدورٍ مُقيَّد (`NOSUPERUSER NOBYPASSRLS`) وجدولٍ تحت `FORCE RLS`: الصيغةُ القديمة تُعطي `ERROR: syntax error at or near "current_role"` ورمزَ خروج 3، والمُصلَحة تُعطي قياساً حقيقيّاً `{restricted:true, rls_active:true, own_visible:true, other_visible:false}` — أي إعادةُ إنتاجٍ لنتيجة المالك اليدويّة عبر الأداة نفسها. والعلاجُ **تسميةُ الـGUC باقتباس كلّ مقطع** لا `set_config`: الثانيةُ تُصدِر صفَّ نتيجةٍ يرفضه مُحلّلُ الحزمة سطراً-JSON فتُبدِل عطلاً بعطل، والاقتباسُ يُبقي الجملة صامتةً ويحفظ ثابتَ «SELECT/SET LOCAL فقط». و`alive` أُضيفت إلى الحالات المقبولة مع اختبارٍ يمنع تحوّل القائمة إلى متساهلة. الحزمة 1.1.1: سلامةٌ VERIFIED على 102 ملفّ و41 اختباراً ناجحاً. التغييرُ في الحزمة لا في هذا المستودع. |
| IMAGERY-SCHEDULER-READS-RLS-TABLE-WITHOUT-CONTEXT-01 | مجدوِلُ الصور يقرأ جدولَه بمسبح التطبيق بلا سياق مستأجِر، والجدولُ تحت ENABLE+FORCE RLS بسياسةِ قراءةٍ فاشلةٍ-مغلقة ⇒ **صفر صفوف بلا استثناء** تحت دور مُقيَّد، فيُطبَع عدّاً عاديّاً ويُرجِع المسحُ «لا حقول مُتابَعة» — عطلٌ صامت لا يُميَّز عن غياب الحقول، ويُبطِل ضمانَ الاستعادة الذي أُضيف M4 لأجله. | platform · rls · ops | [`main.py`](../../services/sahool-platform/api/main.py) (`imagery_automation.set_pool`) · [`v9_rls_tenant_isolation.sql`](../../migrations/v9_rls_tenant_isolation.sql) (`:46`) · [`imagery_automation.py`](../../services/sahool-platform/api/imagery_automation.py) (`load_from_db`) | **fixed** — نُقل إلى مسبح المهامّ (BYPASSRLS) كالطقس تماماً، وصُحِّح التعليلُ الذي بُني عليه الخطأ. التأكيدُ الحيّ يبقى مفتوحاً: لا شاهدَ live-PG على هذا الجدول بينما لأربعةٍ غيره شاهدُها. |
| READINESS-FLAG-PROMISES-24-MONTHS-MEASURES-ZERO-01 | رايةُ جاهزيّة الصور تُسمّى بأربعةٍ وعشرين شهراً وتُحسَب «عدد التواريخ = صفر» داخل النافذة المطلوبة وحدها، فحقلٌ بمشهد واحد يُبلِّغ «لا حاجة للتعبئة» ويُقرأ الاسمُ قياساً. | platform · ai · honesty | [`field_ai_context.py`](../../services/sahool-platform/api/routers/field_ai_context.py) (`readiness`) | **fixed** — صارت `imagery_history_absent` بجوار `imagery_observed_window_days`، وحُدِّث مستهلكوها الثلاثة فلا يبقى شرطٌ ميّت. |
| FIELD-CREATE-REGISTERS-IMAGERY-NOT-WEATHER-01 | إنشاءُ الحقل يُثبِّت نيّةَ متابعة الصور في معاملته ولا يُثبِّت نيّةَ الطقس؛ تسجيلُ الإحداثيّة كان في نقطة أتمتة يدويّة وحدها، فحقلٌ جديد لا يُسحَب طقسُه حتّى يمرّ عليه مشغّل. | platform · onboarding | [`fields.py`](../../services/sahool-platform/api/routers/fields.py) · [`weather_automation.py`](../../services/sahool-platform/api/weather_automation.py) (`register_location_persistent`) | **fixed** — `register_field_tracking_intents` يُثبِّت النيّتين بعقدٍ واحد يملك فيه المُستدعي المعاملة. والتربةُ وتعبئةُ التاريخ تبقيان خارجه بإعلان: الأولى مسارٌ مخبريّ بشريّ، والثانية نداءُ مزوّد لا يُنفَّذ داخل معاملة. |
| CONTAINER-GUARDS-SKIP-AI-SERVICES-01 | حارسا الحاويات يعملان بقوائم صريحة لا تذكر خدمات الذكاء المعتمدة على Ollama/Qdrant، فالفصلُ بين الحياة والجاهزيّة فيها قائمٌ في المصدر وبلا حارس: تحويلُ فحص الصحّة إلى `/readyz` يمرّ صامتاً ويجعل بطءَ تحميل النموذج إعادةَ تشغيلٍ دوريّة. | ci · containers | [`container_fleet_contract_guard.py`](../../scripts/ci/container_fleet_contract_guard.py) (`NO_READYZ_HEALTHCHECK`) | **fixed** — أُدرِجت Dockerfiles الثلاث. والحارسُ العميق يبقى على نطاقه المُعلَن «غير خدمات الذكاء»، ولخدمة المستشار لا فحصَ صحّة في compose أصلاً. |
| CDSE-EMPTY-SCENE-HAS-NO-ATTRIBUTABLE-CAUSE-01 | البايتاتُ الفارغة من CDSE تُحذَف عمداً ولا تُؤرشَف، والأثرُ الوحيد سطرُ سجلٍّ عابر بلا bbox ولا نافذة ولا عتبة سُحُب، فتعذّر الفصلُ بين «لا مشهد في النافذة» و«السُّحُب أقصتها» و«القناع أفرغها». | raster · diagnostics | [`raster_cdse_tile_runtime.py`](../../services/raster-service/raster_cdse_tile_runtime.py) | **open** — خُفِّف لا أُغلِق: صار السطرُ يحمل **طلبَه** كاملاً فالسببُ منسوبٌ للوقائع القادمة. أمّا الواقعةُ المفتوحة فتبقى بلا نسبة: مشهدُها لم يُؤرشَف ولا جدولَ يحفظ تشخيص المشاهد الفاشلة. |
...
| ONE-BAD-ROW-ABORTS-THE-WHOLE-SIGNAL-CYCLE-01 | `run_cycle` يمرّ على التراكبات بلا حراسةٍ لكلّ صفّ، فصفٌّ واحدٌ يرفع استثناءً يُسقِط **الدورة كلَّها** — وتبقى بقيّةُ الحقول بلا إشارات حتّى الدورة التالية، والسجلُّ لا يُسمّي الحقلَ الجاني. | weather · workers · resilience | [`weather-signal-engine/src/main.py`](../../services/weather-signal-engine/src/main.py) (`run_cycle` `:93-100` · الحلقةُ الخارجيّة `:112-119`) | **fixed** (#948 `7508702a`؛ صُوِّبت الحالة بالقياس 2026-09-13 — بقي الصفّ `open` بعد الإصلاح): `run_cycle` يحرس كلَّ صفٍّ بـ`try` ويُسمّي tenant/field ويعدّ الإخفاقات، والاختبار `tests_v9/test_signal_cycle_survives_one_bad_row.py`. الحكمُ الأصليّ: مقيسٌ بالبنية: `for ov in overlays: total += await process_overlay(conn, ov)` بلا `try` داخليّ. والحلقةُ الخارجيّة تلتقط (`except Exception` ⇒ تحذير + نوم `INTERVAL_SEC`)، فالمحرّكُ لا يموت — **لكنّ الدورة تسقط كاملةً**، والتحذيرُ يقول «تعذّرت دورة توليد الإشارات» بلا اسمِ الحقل. فالعطلُ صامتٌ للمُشغِّل ومُكلِفٌ للحقول السليمة في الدورة نفسها. **ونصفُ الدعوى الأصليّة سقط:** «سباقُ نسختَي engine» لا شاهدَ له — مُحرِّكٌ واحدٌ يكتب `weather_signals`، وثلاثةُ ملفّاتٍ تمسّ `field_weather_overlay` اثنان منها عاملُ مضلّعاتٍ وخطُّ أنابيب. سُجِّل النصفُ المقيس وحدَه. |
...
## SERVICE-LOADER-SKIPS-INTERNAL-IMPORT-FAILURE-01 — مُحمِّلُ الخدمة المشترك يحوّل انحدارَ التوصيل إلى تخطٍّ

- **الحالة:** fixed — مقيسٌ على #1014 (مراجعة Copilot المكبوتة على `16502f34`) وعُولج في شريحته المستقلّة.
- **المصدر:** [`tests_v9/service_module.py`](../../tests_v9/service_module.py) `load_service_main` — `except ImportError: pytest.skip(...)` كان يشمل كلَّ فشل استيراد. فانكسارُ `router_registry` أو `routers.validation` في خدمةٍ — وهو التوصيلُ الذي يقيسه عقدُ HTTP على الخدمة نفسها (`test_guardrails_evaluate_http_contract.py`) — كان يُتخطّى وCI أخضر. المُحمِّل تعتمد عليه اختباراتُ خدماتٍ عدّة (`test_tenant_provisioning` · `test_unit_environment_completeness` · عقدُ الحواجز).
- **الصنف:** «التخطّي الصامت يُقرَأ نجاحاً» في المساعد لا في اختبارٍ واحد — الصنفُ نفسُه الذي عُولج في شاهد RLS (#1014) لكن على نطاق مشترك.
- **العلاج:** `ModuleNotFoundError` يُصنَّف **بالمسار**: وحدةٌ لها ملفٌّ تحت جذر الخدمة أو جذر المستودع (ومنها `shared/`) داخليّة ⇒ فشلٌ صريح؛ حزمةٌ لا أثر لها في الشجرة خارجيّة ⇒ تخطٍّ مُعلَّل؛ الاسمُ المجهول و`main` نفسُها داخليّان (فشلٌ مغلق). و`ImportError` غيرُ `ModuleNotFoundError` (اسمٌ غائب من وحدةٍ موجودة، أو دورة) فشلٌ دائماً. دلالاتُ النجاح وهويّةُ الوحدة بالمسار لا تتغيّر.
- **الدليل:** `tests_v9/test_service_module_loader.py` (5 شواهد على خدمةٍ مؤقّتة؛ الشاهدان الداخليّان يحوّلان التخطّي إلى فشلٍ صريح كي لا يُقرأ «متخطّى» نجاحاً — وهو ما كشفته صياغتُهما الأولى بـ`pytest.raises` وحدها: تحت الطفرة سُجِّلا متخطّيَين). طفرتان مقتولتان: العودةُ إلى التخطّي الشامل (3 إخفاقات) · تصنيفُ كلّ شيءٍ خارجيّاً (إخفاق).
- **حدُّ صدق:** التصنيفُ بوجود ملفٍّ تحت الجذرين؛ حزمةٌ خارجيّة تحمل اسمَ مجلّدٍ في المستودع تُصنَّف داخليّة فتفشل بدل أن تُتخطّى — وهو الاتّجاه الآمن.
## AI-RUNTIME-WIRING-01 — خطُّ التوصية المحروس بلا مستهلكٍ إنتاجيّ، ولا يمكن أن يكون له واحدٌ في الصور المنشورة كما هي

- **الحالة:** open — P0 من ترتيب الشرائح المتّفق؛ التسجيلُ قبل التنفيذ (2026-09-17). الشريحةُ التالية تنقل وتوصل.
- **المصدر:** [`services/ai_agronomist/recommendation_runtime_pipeline.py:38`](../../services/ai_agronomist/recommendation_runtime_pipeline.py) — `RecommendationRuntimePipeline` مُعرَّفٌ ووثيقتُه تقول «a single safe callable that can be wired behind a feature flag»؛ المراجعُ غيرُ الاختباريّة **صفر** (`execution-audit/generated/dead_code_candidates.csv`: `RecommendationRuntimePipeline … 0 refs · review_before_delete`)؛ المستهلكُ الوحيد `tests/test_remaining_ai_runtime_phases.py` بمحرّكٍ وهميّ وسياقٍ مصنوع. وثيقةُ #1011 Finding 5 تقول حرفيّاً: «must not be described as the universal live recommendation path until a production call site and live certification prove it».
- **القياس الذي يُغيّر الخطّة (حدُّ الحاوية):** صورةُ المنصّة (`services/sahool-platform/Dockerfile:33-34`) تنسخ `shared/` و`services/sahool-platform/` فقط، وصورةُ ai_agronomist (`services/ai_agronomist/Dockerfile:11,16`) تنسخ `services/ai_agronomist` و`shared/` فقط. فالمسارُ الإنتاجيّ الوحيد الذي يُنتج توصيةً ويُثبِتها — `POST /api/v1/recommendations` → `orchestrate_recommendation` (`core/internal_orchestrator.py:160`، مدخلٌ مسجَّل في `execution_control_plane.py:362`) — يعيش في حاويةٍ **لا تستطيع استيراد** الخطّ الأنبوبيّ. ونقاطُ ai_agronomist (`/v1/recommend` وأخواتها) «evidence-only, never prescriptions» بعقدها (`main.py:394`)، و`decision_contracts.FORBIDDEN_DECISION_KEYS` يحظر مفتاح `recommendation` فيها. أي أنّ الخطّ ميّتٌ **بالبناء** لا بالإهمال: لا مكانَ له في أيٍّ من الحاويتين.
- **الخاصّيّة المنتهَكة:** «مسارٌ قانونيّ موثَّق لا يبلغه أيُّ طلب» — ومعه خطرُ عكسه: من يوصله بمستهلكٍ في المنصّة بالاستيراد المباشر يصنع شيفرةً تعمل في الاختبارات وتنكسر في الحاوية (الصنفُ نفسُه في `SERVICE-LOADER-SKIPS-INTERNAL-IMPORT-FAILURE-01`).
- **القرار المُسجَّل للشريحة التالية (بسبب):** (١) نقلُ عائلة الخطّ (الخطّ الأنبوبيّ · المُهيّئ المحروس · `decision_contracts` · الأحداث · المقاييس · مستودعُ المراجعة · مُحقِّق المستأجر — 512 سطراً) إلى `shared/ai/recommendation_runtime/` لأنّ `shared/` هي القطعةُ الوحيدة التي تنسخها الصورتان، مع إبقاء وحدات `services/ai_agronomist/*` قشوراً تُعيد التصدير كي لا يتغيّر مستهلكوها (`ai_evidence_runtime.py:29` · `agriai-engine/evidence_bundle.py` مرآة · الاختبارات الثلاثة)؛ (٢) موضعُ الاستدعاء الإنتاجيّ `orchestrate_recommendation` خلف رايةٍ جديدة مطفأة تُقرأ من البيئة (انظر `GUARDRAIL-FLAGS-FILE-NOT-IN-ANY-IMAGE-01`)، والمحرّكُ الممرَّر هو `generate_recommendation` V1 نفسُه، و`canonical_field_state` يُركَّب من `_compose_canonical` القائم لا يُختلَق؛ (٣) شاهدُ توصيلٍ بنمط `test_*_wiring.py` (يستورد، لا يصرف مساراً، لا يُعيد تنفيذ قواعد المراجعة)، وشهادةٌ حيّة قبل أيّ تفعيل. مرفوضٌ صراحةً: مسارٌ جديد في المنصّة (الميزانية 629)، أو استدعاءٌ غيرُ بالغٍ من مسار (`ADMISSION-GROUND-…-01`)، أو توصيلُ `preview_model_advisory` بوصفه هذا الإغلاق (عقدٌ آخر: B6).
- **الشاهد الحاليّ:** `tests_v9/test_recommendation_runtime_reachability.py` — يقيس صفرَ المستهلكين ويربطه بحالة هذا القسم (مستهلكٌ يظهر بلا قلبِ الحالة ⇒ أحمر)، ويمنع استيرادَ `ai_agronomist` من أيّ شيفرةٍ تدخل صورةَ المنصّة عبر حدّ الحاوية.
- **حدودُ قراءته، مُصلَحةٌ بمراجعة #1018 (قبل أن تُسقِط حارساً):** الصياغةُ الأولى قرأت أوّلَ مُعامِلٍ بعد `COPY` وحده فكانت `COPY --chown=… services/ai_agronomist /app/` و`COPY . /app` تمرّان؛ وأسقطت `from . import X` (وحدتُها `None` في الشجرة النحويّة) و`importlib.import_module("…")` الحرفيّ؛ وقصرت مسحَ الحدّ على شجرة المنصّة بينما تنسخ الصورةُ `shared/` أيضاً — وهو الموضعُ الذي تنتقل إليه العائلةُ في الخطوة ①، أي أعلى موضعٍ احتمالاً للانتهاك. الأربعةُ مُغطّاةٌ الآن بشواهد مباشرة في الملفّ نفسِه، وزرعُ `from . import ai_agronomist` في `shared/__init__.py` يُحمِّر الحدّ. يبقى خارج القراءة **بقصدٍ معلن**: وسيطُ استيرادٍ ديناميكيٍّ محسوب (لا يُقرأ بالتحليل الساكن)، ومصدرُ `COPY --from=<stage>` (مسارُ مرحلةِ بناءٍ لا شجرةُ المستودع — ومراحلُ البناء تُقرأ بأسطرها هي).
- **شرط الإغلاق:** طلبٌ حقيقيّ على مسارٍ مركَّب يبلغ `RecommendationRuntimePipeline.execute` ويُثبَت بشاهد توصيل، مع رايةٍ يمكن قلبُها في الحاوية، وشهادةٍ حيّة على القاعدة قبل التفعيل. لا يُغلَق بنقل الملفّات وحده.
- **الخطوة ① مُنجَزة (النقل، الفجوةُ تبقى open):** عائلةُ الخطّ السبعُ انتقلت إلى `shared/ai/recommendation_runtime/` بأسمائها نفسِها (فالاستيراداتُ النسبيّة داخلها لم تتغيّر)، و`services/ai_agronomist/*` قشورُ إعادة تصديرٍ بـ`__all__` صريح فلم يتغيّر مستهلكٌ ولا اختبار. لا مستهلكَ إنتاجيّ أُضيف — شاهدُ البلوغ ما يزال يقيس صفراً ويقيّده بهذه الحالة. الخطوةُ ② (التوصيل في `orchestrate_recommendation` خلف رايةٍ من البيئة) شريحةٌ تالية.
- **قياسُ موضع التوصيل قبل بناء الخطوة ②، وهو يُصحّح البند (٢) أعلاه:** البندُ كُتِب قبل قراءة المسار الإنتاجيّ قراءةً كاملة، وثلاثةُ قياساتٍ تجعله **غيرَ قابلٍ للتنفيذ حرفيّاً**. **(أ) تعارضُ تزامن:** `RecommendationRuntimePipeline.execute` معرَّفٌ `async` (`shared/ai/recommendation_runtime/recommendation_runtime_pipeline.py`)، بينما `orchestrate_recommendation` (`core/internal_orchestrator.py:162`) دالّةٌ **متزامنة** يبلغها المسارُ عبر `handle_recommendation_request` المتزامنة (`core/api_adapter.py:118`) من داخل حلقةِ أحداثٍ عاملة (`api/routers/recommendations.py:159` غيرُ متزامن). فتشغيلُ الخطّ من داخل المايسترو يحتاج إمّا تحويلَ السلسلة كلِّها إلى `async` (تمسّ `api_adapter` ومستهلكيه)، وإمّا `asyncio.run` داخل حلقةٍ قائمة — وذاك يرمي وقتَ التشغيل لا وقتَ الاختبار. **(ب) `canonical_field_state` غيرُ متاحٍ قبل القرار:** الخطُّ يشترطها **قبل** توليد التوصية (`runtime_guardrail_adapter.guarded_runtime_context` يرفع `MissingCanonicalFieldState`)، والمسارُ يقرأ السياقَ القانونيّ **بعد** عودة المحرّك (`api/routers/recommendations.py:262-292`: `load_agronomic_context` على اتّصالٍ محدودٍ بالمستأجر). و`_compose_canonical` المذكورة في البند (٢) تعيش في `api/routers/internal_service.py:228` وتطلب `conn`، ولا المايسترو ولا المحوّلُ يملك اتّصالاً. **(ج) الترتيبُ الحاليّ مقصودٌ وموثَّق، لا مصادفة:** التعليقُ في `recommendations.py:218-223` يقول إنّ القراءةَ تقع على **المعاملة نفسِها** التي تكتب صفَّ التوصية وحدثَ الصندوق — «لقطةٌ واحدة، فالنسبُ المسجَّلُ مع التوصية هو الحالةُ التي قُرِئت فعلاً حين اتُّخذت» — وأنّ العميل يرسل معرّفاتٍ فقط ولا يُقبَل منه كائنٌ قانونيّ ولا بصمة. فتقديمُ القراءة قبل المحرّك ليس نقلَ سطرٍ بل تغييرُ لحظةِ اللقطة بالنسبة إلى القرار.
- **ما يترتّب على القياس (قرارُ نطاقٍ لا تأجيل):** الخطوة ② ليست «توصيلاً خلف راية» صغيراً. الخياراتُ الثلاثة المقيسة: **(١)** تقديمُ قراءة السياق القانونيّ قبل المحرّك على الاتّصال المحدود بالمستأجر نفسِه ثمّ تمريرُه إلى الخطّ — يحفظ «اللقطة الواحدة» لكنّه يُعيد ترتيب معاملةٍ موثَّقةِ القصد، فيلزم قياسُ أثره على النسب قبل أيّ شيء؛ **(٢)** توصيلُ الخطّ في طبقة المسار غيرِ المتزامنة حيث يتوفّر `conn` والسياق، لا في المايسترو المتزامن — يحلّ (أ) و(ب) معاً؛ **(٣)** إبقاءُ الخطّ بلا توصيل وإعادةُ توصيف الفجوة. **تصحيحٌ لاستدلالٍ لي، بمراجعة #1020:** قلتُ أوّلاً إنّ (٢) «وحدَه يحلّ العائقَين ولا يمسّ ترتيبَ اللقطة» — وهذا **خطأ**. في المسار الحاليّ لا يُفتَح `conn` ولا تُقرأ الحالةُ إلّا **بعد** عودة `handle_recommendation_request`، أي بعد قرار المحرّك. فلكي يبلغ الخطُّ المحرّكَ قبلَه في الطبقة غيرِ المتزامنة، يجب تقديمُ فتحِ الاتّصال والقراءة إلى ما قبل ذلك الاستدعاء — وهو بعينه تغييرُ لحظةِ القراءة بالنسبة إلى القرار. فـ(١) و(٢) **ليسا بديلَين عن بعضهما في هذه النقطة**: كلاهما يقدّم القراءة، و(٢) يضيف إليها أنّه يحلّ عائقَ التزامن أيضاً. **وما يبقى من ميزته مقيسٌ لا مُدَّعًى:** خاصّيّةُ «لقطةٍ واحدة على معاملة الكتابة نفسِها» **تُحفَظ** إن فُتِحت المعاملةُ قبل المحرّك وبقيت مفتوحةً حتّى الكتابة — بل تصير الدعوى أصدق، إذ يستهلك القرارُ فعلاً ما قُرِئ. **وثمنُها معلَن:** معاملةُ مستأجرٍ مفتوحةٌ طوال حساب المحرّك، وذاك أثرٌ على القفل والاتّصالات يجب أن يُقاس قبل الالتزام به، لا أن يُفترَض. فالمرجَّحُ يبقى (٢) **بهذا القيد المكتوب**، ويُقاس ثمنُ المعاملة الممتدّة في شريحة التنفيذ. ولا يُشرَع في أيٍّ منها قبل تسجيل هذا القياس — وهذه هي الشريحة.
...
| EDGE-MODEL-DIGEST-CONTRACT-UNENFORCED-01 | **عقدُ بصمة نماذج الحافّة مُعلَنٌ في موضعَين ولا يربطهما حارس.** `edge_models.required.json` يُعلن لكلّ نموذج `env` و`default_path` و`sha256_env`، و`main.py:91` يُعلن الخريطةَ نفسَها ويصف نفسَه «المصدرُ الواحد للحكم» — فمصدران يدّعيان الوحدانيّة. والحارسُ الحاجب `edge_model_contract_guard.py:49-61` يقرأ `env` و`default_path` **فقط**: **(أ) `sha256_env` حقلٌ مُعلَنٌ بلا إنفاذ** — صفرُ مراجع في `scripts/` و`tests_v9/` (مقيس)، فإعادةُ تسميته أو خطأٌ إملائيّ فيه لا يراه CI، ويقرأ المُهيّئُ متغيّراً غيرَ موجود فتسقط القدرةُ مغلقةً **بصمت**؛ الإغلاقُ صواب، والصمتُ هو العطل. **(ب) لا فحصَ في الاتّجاه العكسيّ** — نموذجٌ يُضاف إلى `_MODEL_ENV` بلا صفٍّ في البيان يمرّ أخضر. **(ج) الفحصُ بالاحتواء النصّيّ** (`if env not in main_text`) فيُرضيه ذكرُ الاسم في تعليق — وهو صنفُ «قارئٌ أضيقُ من دعواه» الذي قِيس ثلاثَ مرّات في #1018 و#1019 و#1020. | edge-inference · supply-chain · CI | [`edge_models.required.json`](../../services/edge-inference/models_manifest/edge_models.required.json) · [`main.py:91`](../../services/edge-inference/main.py) · [`edge_model_contract_guard.py:49`](../../scripts/ci/edge_model_contract_guard.py) · [`model_artifact_gate.py:80`](../../services/edge-inference/model_artifact_gate.py) | **open** — **وحدُّها معلَنٌ كي لا تُقرأ أوسعَ ممّا هي:** البصمةُ نفسُها **مُنفَّذةٌ فعلاً** وقتَ التشغيل (`EDGE-MODEL-ARTIFACT-INTEGRITY-01` مُغلَقة: تحقّقٌ بالبايتات قبل كلّ استدلال بأسبابٍ مسمّاة). فهذه الفجوةُ **ليست** «أضِف بصمة» بل «اربط العقدَ المُعلَن حولها فلا ينحرف صامتاً». ولا تُعالِج ما أعلنته أختُها حدَّ صدقٍ لها: البصمةُ تُثبِت الهويّةَ لا الصلاحيّة، ولا وزنَ معتمدٌ في الشجرة. **وقياسٌ ثالثٌ أضافته مراجعةُ #1021 على سطحٍ مجاور، فأُعيد على هذا السطح:** ثلاثةُ ملفّات compose تُعرّف `edge-inference`، والبصمةُ تصل **واحداً** منها. و`docker-compose.light.yml:220` كان يشحن المسارَين ويُفعّل `AUTO_DOWNLOAD_MODELS` فوقهما، بينما `download_models.py:49` يرفض التنزيل بلا بصمةٍ صالحة — فلا يُنزَّل شيء، ويردّ كلُّ استدلالٍ 503 **دائماً**، ولا شيءَ في CI يقول ذلك. أُصلح السطح، والقاعدةُ المفروضة مبدئيّة: **من يشحن مسارَ نموذجٍ يشحن بصمتَه** — والمسارُ يقول أين والبصمةُ تقول هل يصلح. **واعترافٌ في محلّه:** أوّلُ صياغةٍ لشاهدي قرأت `docker-compose.v9.yml` وحدَه واسمُها يدّعي «تصل الحاوية» — رابعُ ظهورٍ لصنف «قارئٌ أضيقُ من دعواه» في يومٍ واحد، وأوّلُ مرّةٍ يُكتشَف في شيفرتي **قبل** شحنها، بفضل مراجعةِ طلبِ غيري. **و`sam2-inference`** سطحُ نماذجَ ثانٍ كان بلا أيّ تحقّقِ بصمة، **ويعالجه #1021** (`EDGE-MODEL-ARTIFACT-DIGEST-01`) — فلا يُوصَف هنا فجوةً مفتوحة ولا يُحرَس بشاهدٍ يصطدم به. **ومراجعةُ #1022 أضافت قياسَين، كلاهما من صنفٍ سبق تسميتُه:** (١) شاهدُ الأسطح كان يعدّدها بـ`ROOT.glob` بينما للمستودع **سطحٌ مشترك** مُعرَّف في [`compose_surface.py`](../../scripts/ci/compose_surface.py) يبنيه من `git ls-files` ويضمّ `frontend/` — فعدّادٌ خاصٌّ يرى غيرَ ما تراه البوّابات، وملفٌّ غيرُ متعقَّب يدخل القياس. (٢) شاهدُ استهلاك البصمة كان يقبل **أيَّ** `os.getenv` في المُهيّئ، فلو قرأ `expected_sha256` اسماً ثابتاً لمرّت السلسلةُ خضراء والعقدُ مقطوع — «قارئٌ أضيقُ من دعواه» للمرّة الخامسة، والآن تُقاس الحلقتان معاً: `model_capability` يمرّر `sha_env_name`، و`expected_sha256` يمرّر وسيطَه إلى `os.getenv`. ثلاثُ طفراتٍ أحمرت الشواهدَ الثلاثةَ المُشدَّدة. |
| SOIL-MOISTURE-UNIT-IDENTITY-01 | ثلاثةُ أسماءٍ لكمّيّةٍ واحدة بثلاث دلالات: `soil_telemetry.py` يقول «٪ من السعة المتاحة» · الكاتبُ القانونيّ يخزّن `"%"` عارية · `compute_rwc` يقرأ VWC · دفتر v98 وحدةٌ ثالثة بلا إعلان. وحقيقتا الرطوبة (الدلو والحسّاس) **لم تكونا تلتقيان**: توأمُ المياه لا يقرأ الحسّاس. حزمةٌ واردة حسبت `TAW×(1−pct/100)` على VWC (25٪ ⇒ 75 مم) ثمّ حجبت التوأمَ بـ409. | irrigation · water-twin · soil · alerts | [`soil_telemetry.py`](../../services/sahool-platform/api/soil_telemetry.py) · [`water_twin_seed.py`](../../services/sahool-platform/api/water_twin_seed.py) (`sensor_depletion_mm` · `join_sensor_with_ledger_seed`) · [`routers/water_twin.py`](../../services/sahool-platform/api/routers/water_twin.py) (`_join_sensor`) · [`device_registry.py`](../../services/sahool-platform/api/device_registry.py) · [`weather_advice.py`](../../services/sahool-platform/api/weather_advice.py) · [`alert_rules.py`](../../services/sahool-platform/api/alert_rules.py) · [`soil-service/evidence_adapters.py`](../../services/soil-service/evidence_adapters.py) · [`test_soil_moisture_unit_identity.py`](../../tests_v9/test_soil_moisture_unit_identity.py) | **fixed** — **الوحدةُ تُعلَن لا تُخمَّن:** القراءة تحمل `unit_kind` (`vwc_pct` · `available_pct` · `undeclared`) ممّا أعلنه المصدر؛ `%` العارية لا تُعلن شيئاً وتبقى القراءةُ مع هذه الحقيقة. **النضوبُ بالفيزياء:** VWC عبر `(θFC − θ)·Zr·1000`، لا نسبةً من TAW. **الوصلةُ غيرُ سلطويّة** (تصميم المالك): الدفترُ بذرةٌ إن وُجد · الحسّاسُ الطازج يهيّئ عند غيابه بقيد `seed_from_single_point_sensor` · الخلافُ > max(10, 0.15·TAW) قيدٌ بالقيمتين لا 409 · لا `assimilated`. **الطزاجةُ من إعلانٍ** في سجلّ الأجهزة (ساعة/٤ ساعات) لا رقمٍ في المستهلك. **المسارات الثلاثة تقرأ الوحدة:** التوأم يحوّل أو يُعلن؛ `irrigation_advice` لا يستعمل VWC ضدّ عتبات الماء المتاح ويكتب الافتراضَ الموروث في `rationale_ar`؛ `_low_moisture` يسقط إلى مسار الاحتياج مع VWC ويسمّي الافتراض مع المجهول. **جانبُ الكاتب:** `observations_from_properties(units=…)` و`EvidenceBatchIn.units` يحملان إعلانَ المصدر؛ الافتراضيُّ `%` يبقى غيرَ مُعلَن **ولا يُخمَّن**. **٢٨/٢٨ · ٧٣ + ٩ مجاورة · ٧/٧ طفرات.** **حدودُ صدق:** ما تُخرِجه الحسّاساتُ فعلاً **NOT_MEASURED** — كلُّ القراءات الحاليّة `undeclared` حتّى يقيس المالكُ جهازاً ويُعلن؛ الراوتر مُختبَرٌ بتركيبه لا بقاعدة. **وتصحيحٌ من مراجعة المالك المضادّة على `a7d64adf` (حالتان مُعاد إنتاجُهما):** (١) `float(True) == 1.0` كان يجعل قيمةً منطقيّة قراءةً ثمّ بذرةَ نضوبٍ (`true` تحت `m3/m3` ⇒ 100٪) — تُرفَض `bool` في القارئ **وعند الابتلاع** (`_require_soil_moisture_measurement` ⇒ 422)، و`0.0`/`1.0` العدديّتان تبقيان؛ (٢) الطزاجةُ كانت غيابَ نفيٍ (`not stale`) فغيابُ العمر/السقف وNaN وعمرٌ سالبٌ بيومٍ كلُّها «طازج» — صارت إثباتاً إيجابيّاً (`sensor_freshness` ⇒ `fresh`/`stale`/`unproven`/`future` بسياسة انحرافٍ مُعلَنة ٥ دقائق). **وهذا الصفُّ وحدَه حاكم:** صفٌّ ثانٍ بالمعرّف نفسه («fixed القارئ / open الكاتب») أُضيف في `e5b7f39e` وأُزيل هنا؛ تاريخُه في `log.md`. **حدودُ صدق:** وحدةُ الجهاز الفعليّ NOT_MEASURED فكلُّ القراءات القائمة `undeclared`؛ والراوتر مُختبَرٌ بتركيبه لا بقاعدة. **وتكميلٌ من المراجعة التكميليّة على `b9c5aceb` (حالتان مُعاد إنتاجُهما):** (٣) **تكافؤُ أبواب الابتلاع** — `/soil/evidence` كان يرفض `true` و`/v1/soil/observations` يقبله بـ201 ويُمرّره للحفظ، وبابٌ ثالث `/v1/soil/ingest` يُحوّل `true` إلى `1.0` في المخطّط؛ الحارسُ صار في **العقد** `SoilObservation` (`MEASUREMENT_ONLY_PROPERTIES`) فيبلغه كلُّ باب، و`SoilReading.moisture_pct` يرفض `bool` قبل التحويل — مقيسٌ بـHTTP على الأبواب الثلاثة (`test_soil_ingest_entrypoint_parity.py`). (٤) **فقدُ الجودة في الإسقاط** — `accepted`/`suspect`/`uncalibrated` كانت تتساوى بذرةً؛ القراءةُ تحمل الآن `observation_id`/`quality_status`/`calibration_id`/`confidence`/العمق، و`sensor_seed_eligibility` سياسةٌ صريحة: `accepted` وحدَها تهيّئ، وغيرُها شاهدٌ مرئيّ يُقارَن ولا يهيّئ، والغيابُ لا يؤهّل. **(٥) من المراجعة المستقلّة على `703607bf`:** افتراضُ العقد `quality_status=accepted` كان يجعل قراءةَ حسّاسٍ بلا إعلان جودة عبر `/v1/soil/observations` **مؤهّلةً** للتهيئة (42 مم) بينما البابُ المجمِّع يصنّف الحالةَ نفسَها `uncalibrated` — مُعاد إنتاجُه؛ حُسِم في العقد المشترك للحالة `sensor` + خاصّيّة قياس: الغيابُ ⇒ `uncalibrated`، والقبولُ الصريح كما هو، والمختبرُ والخصائصُ الأخرى لم تُمَسّ. مقيسٌ من الطلب حتّى الوصلة على البابين. |
| INGEST-ENTRYPOINT-PARITY | ثلاثةُ أبوابٍ تبني `SoilObservation` نفسَه وحارسُ «الرطوبة قياسٌ» كان عند بابٍ واحد: `/v1/fields/{id}/soil/evidence` يرفض `true` بـ422، و`/v1/soil/observations` يقبل القيمةَ نفسَها (و`"NaN"`/`"Infinity"`) بـ201 ويُمرّرها للحفظ، و`/v1/soil/ingest` يُحوّل `true` إلى `1.0` في مخطّط `SoilReading` قبل أن يبلغ العقد. | soil-service · contracts | [`observation.py`](../../shared/contracts/soil/observation.py) (`MEASUREMENT_ONLY_PROPERTIES` · `require_soil_moisture_measurement`) · [`routers/canonical.py`](../../services/soil-service/routers/canonical.py) · [`main.py`](../../services/soil-service/main.py) (`SoilReading._moisture_is_a_measurement`) · [`test_soil_ingest_entrypoint_parity.py`](../../services/soil-service/test_soil_ingest_entrypoint_parity.py) | **fixed** — مُعاد إنتاجُه في مراجعة المالك على `b9c5aceb` (بابان) وقِيس بابٌ ثالث لم تذكره. الحارسُ انتقل إلى **العقد** فيبلغه كلُّ بابٍ يبني الكائن؛ المحوِّلُ يستدعي الدالّةَ نفسَها لا نسخةً ثانية؛ والمخطّطُ الثالث يرفض `bool` قبل التحويل. `bool` يبقى مشروعاً لخصائصَ أخرى. HTTP حقيقيّ على الأبواب الثلاثة بمُثبِّتات (لا PostgreSQL): الخمسُ غيرُ القياسيّة ⇒ 422 بلا كتابة؛ الضوابطُ العدديّة تصل الحفظ. طفرةٌ على العقد مقتولة. **حدُّ صدق:** لا قاعدةَ حيّة؛ الصفوفُ التاريخيّة غيرُ معاد وسمها. |
| QUALITY-PROJECTION-LOSS | `_latest_soil_moisture` كان يُسقِط `observation_id` و`quality_status` و`calibration_id` و`confidence` والعمق، فتتساوى `accepted` و`suspect` و`uncalibrated` بذرةً للتوأم (42 مم في الثلاث بقيد `single point` وحدَه) — الوحدةُ الصحيحة والطابعُ الحديث كانا يُقرآن جودةً. | platform · water-twin · soil | [`field_context.py`](../../services/sahool-platform/api/field_context.py) · [`soil_telemetry.py`](../../services/sahool-platform/api/soil_telemetry.py) · [`water_twin_seed.py`](../../services/sahool-platform/api/water_twin_seed.py) (`sensor_seed_eligibility`) · [`test_soil_moisture_unit_identity.py`](../../tests_v9/test_soil_moisture_unit_identity.py) | **fixed** — القراءةُ تحمل الهويّةَ والجودةَ والمعايرةَ والثقةَ والعمق؛ أهليّةُ التهيئة من الحسّاس وحدَه سياسةٌ صريحة: `accepted` وحدَها تهيّئ، `suspect`/`uncalibrated` شاهدٌ مرئيّ يُقارَن ويُنشَر خلافُه ولا يهيّئ، والغيابُ لا يؤهّل. **وتتمّةٌ من مراجعة `703607bf`:** افتراضُ العقد `accepted` كان يُعيد الأهليّةَ من الباب المباشر — حُسِم في العقد (`sensor` + قياس بلا إعلان ⇒ `uncalibrated`). طفرتان مقتولتان. **حدُّ صدق:** لا 409؛ والدفترُ والتجاوزُ الصريح يستمرّان؛ لا قاعدةَ حيّة. |
| PREFLIGHT-BLIND-TO-PLATFORM-SERVICE-TESTS-01 | الطبقةُ الافتراضيّة في `preflight.sh` تُشغّل `pytest -m unit` (نطاقُه `tests_v9`) و`pytest tests/` فقط، بينما وظيفةُ *Platform Unit Tests* المطلوبة تُشغّل `services/sahool-platform/tests` — وفيها راتشِت ميزانية الوحدات (`test_p0_platform_module_growth_guard.py`). مقيس على `f95a78f5`: الطبقةُ الافتراضيّة 0/0 محلّيّاً، وCI أحمر لأنّ `api/request_dates.py` رفع عددَ وحدات المنصّة 680 ⇒ 681؛ وقبله عطلُ عزلٍ في `test_water_twin_sensor_join.py` لم يره لا preflight ولا CI (ترتيبٌ أبجديّ). | ci · preflight | [`preflight.sh`](../../scripts/ci/preflight.sh) · [`ci.yml`](../../.github/workflows/ci.yml) · [`test_p0_platform_module_growth_guard.py`](../../services/sahool-platform/tests/test_p0_platform_module_growth_guard.py) | **fixed** (2026-09-06 · فرع `claude/claude-md-docs-p6qqir` فوق `d95b8eaf`) — كان من صنف §٣.١٧: أخضرُ الطبقة يعني «ما قِيس مرّ». **العلاج على طبقتين:** (١) الراتشِت `test_p0_platform_module_growth_guard.py` خطوةً `٥ج` في الطبقة **السريعة** (يعدّ ملفّاتٍ بلا استيراد، ٠٫٦ ث، حتميٌّ في كلّ بيئة) ومُدرَجٌ في `preflight_required.json.required_tests`؛ (٢) الجناحُ كاملاً خطوةً `٨ج` في الطبقة الافتراضيّة **بشكل CI نفسه** (`cd services/sahool-platform && PYTHONPATH=. pytest tests`) **بشرطٍ مقيس:** يُشغَّل فقط إذا طابق `fastapi.__version__` المثبَّتَ في `api/requirements.txt`، وإلّا يُتخطّى بصوتٍ عالٍ ويُعَدّ «لم يُقَس» — لأنّ FastAPI 0.141 (هذه الحاوية) يُدرِج الراوترات كسولةً (`_IncludedRouter`) فيصير `app.routes` معتماً و٣٢ اختبارَ «مسارٌ مُسجَّل» تحمرّ لسببٍ بيئيّ بينما CI يثبّت 0.136.3 — حمرةٌ بيئيّة تُقرأ شيفريّةً أسوأ من تخطٍّ مُعلَن. ثلاثةُ اختبارات (موضعُ الراتشِت · شكلُ CI بعد الخروج المبكر · كتلةُ الشرط المشحونة على متطلّباتٍ مُختلَقة: تطابقٌ ⇒ 0 واختلافٌ ⇒ غيرُ صفر) وطفرتان مقتولتان. **حدُّ صدق:** في بيئةٍ بإصدارٍ مختلف يبقى الجناحُ «لم يُقَس» لا «مرّ». |
| APPEND-ONLY-GUARD-FORBIDS-ROW-DEDUPLICATION-01 | حارسان يتعارضان على الفعل نفسه: `brain_duplicate_gap_identity_guard.py` يطلب صفّاً حاكماً واحداً لكلّ هويّة في `gaps/registry.md`، و`brain_append_only_guard.py` يُحمِّر على **أيّ** نقصٍ في بايتات السجلّ بين التزامٍ ووالده. ضمُّ الصفّين المكرَّرين لـ`SOIL-MOISTURE-UNIT-IDENTITY-01` في `b9c5aceb` (بطلب المراجعة المضادّة) أنقص السجلَّ ١٬٠٩٠ بايتاً ⇒ `JOURNAL_SHRANK` على `a7d64adf → b9c5aceb` في وظيفة `no-report-only-change` على PR #985 (مقيس من سجلّ الوظيفة 101497219921؛ حارسُ التكرار في الوظيفة نفسها يمرّ). | brain · ci | [`brain_append_only_guard.py`](../../scripts/ci/brain_append_only_guard.py) · [`brain_duplicate_gap_identity_guard.py`](../../scripts/ci/brain_duplicate_gap_identity_guard.py) · [`no-report-only-change.yml`](../../.github/workflows/no-report-only-change.yml) | **implemented / pending final CI** — الحارسُ ما يزال يقيس **التاريخ** لا الرأس، ولا يُخفَّف ولا يُحشى السجلُّ ليعود إلى حجمه. **قرارٌ مُتَّخذ (2026-09-06، بتفويض المالك «لكل القرار»):** لا إعادةَ كتابةٍ لتاريخ فرعٍ تحت PR مفتوح راجعه المالك وCopilot بمراجع SHA وblob — التاريخُ هنا دليلُ مراجعة، والوظيفةُ غيرُ مطلوبة في الـRuleset. **التنفيذ على فرع PR #987 فوق `b54bf578`:** `brain_append_only_guard.py` صار يقبل استثناءً ضيّقاً قبل قاعدة الحجب نفسها، ويستورد من حارس التكرار **نفس** `GLOBAL_ROW_UNIQUENESS_TARGETS` ومحلّل `ROW_FULL_ID_RE` الكامل وقواعد `_strip_fenced_blocks` والتفرّد العالميّ؛ لا قائمةَ نطاق ثانية ولا محلّلَ هويةٍ موازياً. والصفّان المتعارضان (الهويّة نفسها/بايتات مختلفة) لا يُختار أحدُهما آليّاً: العلاجُ المقبول نموٌّ يُبقي الأصلين في سجلّ تدقيقٍ صريح، كما يشهد `test_lossless_row_deduplication_conflicts_require_preserved_audit_evidence`. **التكذيب المسجَّل الآن:** مدخلُ `brain_append_only_guard.py` في `guard_mutation_registry.json` اتّسع من ٤ إلى ١١ طفرةً تغطي النطاقَ وبايتاتِ الصفّ والبادئةَ والكتلَ المسوّرة وحدَّ السطر وتفرُّدَ الابن ومسارَ القبول، وقُتلت محليّاً كلُّها بالشواهد المُسمّاة (`11/11`)؛ وحارسُ التكرار بقي `3/3`. وما يزال الإغلاقُ **معلّقاً على CI النهائيّ** لأنّ الشجرةَ النهائية (التوليد · الحزمة · preflight · فحوص الرأس) لم تُقَس بعد على هذا الرأس. **ضمُّ صفَّين متعارضين (2026-09-07) — بالقاعدة التي هبطت للتوّ:** أنتج دمجُ `main` بعد #987 صفّاً ثانياً لهذه الهويّة من الفرع الموازي #988، والصفّان **مختلفان بايتاً** فلا يُحذف أحدُهما آليّاً (`is_lossless_row_deduplication` ⇒ False عن قصد). العلاجُ المطبَّق هنا هو الذي يشهد له `test_lossless_row_deduplication_conflicts_require_preserved_audit_evidence`: نموٌّ يُبقي الأصلين. **والقرار:** الحاكمُ ما هبط في #987 — قاعدةُ «الصفوف المتطابقة بايتاً فقط» **أضيقُ وأصحُّ** من قاعدة #988 («كلُّ سطرٍ اختفى صفٌّ بهويّةٍ مكرَّرة»)، لأنّ الثانية تُجيز إسقاطَ نصٍّ مختلف وهو الفقدُ الذي وُجِد الحارسُ لمنعه؛ فأُسقِط تنفيذُ #988 كلُّه لصالح المُنفَّذ. **والنصُّ الأصليّ للصفّ الموازي محفوظٌ كما كُتِب:** «APPEND-ONLY-GUARD-FORBIDS-ROW-DEDUPLICATION-01 \| حارسان يتعارضان على الفعل نفسه: `brain_duplicate_gap_identity_guard.py` يطلب صفّاً حاكماً واحداً لكلّ هويّة في `gaps/registry.md`، و`brain_append_only_guard.py` يُحمِّر على **أيّ** نقصٍ في بايتات السجلّ بين التزامٍ ووالده. ضمُّ الصفّين المكرَّرين لـ`SOIL-MOISTURE-UNIT-IDENTITY-01` في `b9c5aceb` (بطلب المراجعة المضادّة) أنقص السجلَّ ١٬٠٩٠ بايتاً ⇒ `JOURNAL_SHRANK` على `a7d64adf → b9c5aceb` في وظيفة `no-report-only-change` على PR #985 (مقيس من سجلّ الوظيفة 101497219921؛ حارسُ التكرار في الوظيفة نفسها يمرّ). \| brain · ci \| [`brain_append_only_guard.py`](../../scripts/ci/brain_append_only_guard.py) · [`brain_duplicate_gap_identity_guard.py`](../../scripts/ci/brain_duplicate_gap_identity_guard.py) · [`no-report-only-change.yml`](../../.github/workflows/no-report-only-change.yml) \| **fixed** — الحارسُ يقيس **التاريخ** لا الرأس، فلا يُرضيه إلّا تعديلُ التاريخ. **قرارٌ مُتَّخذ (2026-09-06، بتفويض المالك «لكل القرار»):** لا إعادةَ كتابةٍ لتاريخ فرعٍ تحت PR مفتوح راجعه المالك وCopilot بمراجع SHA وblob — التاريخُ هنا دليلُ مراجعة، والوظيفةُ غيرُ مطلوبة في الـRuleset. والحارسُ لا يُخفَّف ولا يُحشى السجلُّ ليعود إلى حجمه. **العلاجُ المقترح** (شريحةٌ مستقلّة على سطح الحجب): يُعلَّم الصفُّ المكرَّر منسوخاً بدل حذفه، أو يقبل حارسُ الإلحاق نقصاً يُثبَت أنّه إزالةُ صفٍّ بهويّةٍ مكرَّرة — كلاهما قرارُ سياسةٍ على حارسٍ حاجب. **تصحيحٌ (2026-09-06، بعد الدمج):** المالكُ اختار إصلاحَ التاريخ بنفسه قبل دمج #985 (`dd9a74af` + إعادةُ زرع الالتزامات؛ الشجرةُ النهائيّة مطابقةٌ بايتاً) فصارت الوظيفةُ خضراء على الرأس المدموج `d95b8eaf` — قرارُ المالك لا المؤلّف، والتعارضُ بين الحارسين **كسياسة** ما يزال قائماً: كلُّ ضمٍّ لصفٍّ مكرَّر في المستقبل سيُحمِّر الإلحاقَ ما لم يُتَّخذ أحدُ العلاجين. **العلاج (2026-09-06):** أُخِذ العلاجُ الثاني — لا تخفيفَ للقاعدة بل تصحيحُ صياغتها: كانت تُقاس بالبايتات وما تحمي هو **المحتوى**. صار `brain_append_only_guard` يقبل النقصَ على `gaps/registry.md` وحدَه، و**ببرهانٍ لكلّ زوج**: كلُّ سطرٍ اختفى يجب أن يكون صفّاً بهويّةٍ مُعلَنةٍ مرّتين فأكثر **عند الوالد** (فرقُ متعدّدات على أسطر الوالد نفسِه)، وإلّا حجَبَ كما كان. ومعرفةُ «ما هو الصفُّ المكرَّر» **مستورَدة** من الحارس الذي يفرضها (`duplicate_row_lines`) لا مُعادةَ الاشتقاق، وإلّا انحرفت القراءتان. النقصُ المقبول يُطبَع `DUPLICATE_ROW_RECONCILED` فلا يمرّ صامتاً. مُكذَّب بستّ حالاتٍ على مستودعاتٍ مؤقّتة (الضمُّ يمرّ · التفريغُ يحجب رغم وجود مكرَّر · حذفُ صفٍّ غير مكرَّر يحجب · `log.md` بالمحتوى نفسِه يحجب · سجلٌّ بلا مكرَّرٍ عند الوالد لا استثناءَ له · الاستيرادُ لا إعادةُ الاشتقاق) وطفرتين مُسجَّلتين تقتلان توسيعَ الاستثناء ورفعَ حصرِه بالملفّ. والحدُّ المُعلَن: الاستثناءُ يحدّ **أيَّ أسطرٍ تختفي**، لا ما يقوله الصفُّ الباقي — أمانةُ نصّ الضمّ مسألةُ مراجعةٍ لا قياس.» |
| DUPLICATE-GAP-ROW-GUARD-ABSENT-FROM-PREFLIGHT-01 | صفّان بالمعرّف `SOIL-MOISTURE-UNIT-IDENTITY-01` عاشا في السجلّ عبر التزامين (`e5b7f39e` ثمّ `a7d64adf`) ومرّ `preflight.sh --fast` أخضرَ عليهما — بينما `brain_duplicate_gap_identity_guard.py` **يراهما** (مقيس: `global_duplicate_row_identities` على سجلّ `a7d64adf` ⇒ `[5464, 5465]`). الحارسُ موصولٌ في `no-report-only-change.yml` وحدَه ولا يستدعيه `preflight.sh` ولا `ci.yml`، فيُكتشَف التكرارُ على PR لا محلّيّاً. | brain · ci · preflight | [`brain_duplicate_gap_identity_guard.py`](../../scripts/ci/brain_duplicate_gap_identity_guard.py) · [`no-report-only-change.yml`](../../.github/workflows/no-report-only-change.yml) · [`preflight.sh`](../../scripts/ci/preflight.sh) | **fixed** (2026-09-06 · فرع `claude/claude-md-docs-p6qqir` فوق `f9350d79`) — كان من صنف §٣.١٨–§٣.٢١ في الرَّنبوك: «أخضرُ preflight يعني ما قِيس مرّ»؛ مالكُ المراجعة التقط التكرارَ بالعين قبل أيّ بوّابة. **العلاج:** الحارسُ **نفسُه** خطوةً `٦د` في الطبقة السريعة عبر `require_file … && run …`، ومُدرَجٌ في `preflight_required.json.required_scripts`. **شروطُ الإغلاق الثلاثة التي طلبها المراجع تُقاس على الشيفرة المشحونة لا على نسخة:** `tests_v9/test_local_preflight_contract.py` يستخرج `require_file`/`run` وسطرَ الاستدعاء من `preflight.sh` ويُشغّلها في جذرٍ مؤقّت — صفٌّ مكرَّر غيرُ متلاصق ⇒ `failures=1` وسببُه في المخرج · حذفُ السكربت ⇒ `failures=1 · skipped=0` برسالة «سكربت بوّابة مفقود» لا «فشل (2)» · الحالةُ النظيفة ⇒ `0/0`. طفرتان في `preflight.sh` (إخراجُ الحارس · نزعُ `require_file`) مقتولتان. لا workflow موازٍ ولا كاشفَ ثانٍ. |
| MIGRATION-INDEX-NAMES-A-COLUMN-NO-TABLE-DEFINES-01 | `v9_new_tables.sql:165` يحرس فهرساً بوجود الجدول (`to_regclass('market_price_history')`) ثمّ يُنشئه على `(crop_type, recorded_at)` — وعمودان لا يُعرِّفهما **أيُّ** تعريفٍ للجدول في الشجرة (التعريفُ الوحيد `v229_market_mcp_schema.sql:35`: `category`/`recorded_date`، وفهرسُه الصحيح هناك :87). على قاعدةٍ جديدة يمرّ (الجدول غائب عند القراءة؛ v9 قبل v229 بسبعمئة سطر في MANIFEST) فلم تمسكه «18/18 على قاعدة جديدة»؛ على قاعدةٍ قائمة (`up --force-recreate` على volume طُبِّق عليه v229) الجدولُ موجود ⇒ `column "crop_type" does not exist` ⇒ `sahool-migrate` exit 3 والمكدّسُ لا يُقلِع. **مقيس على مكدّس المالك 2026-09-07** (`docker compose … --profile gpu up -d --force-recreate`). | migrations · bootstrap | [`v9_new_tables.sql:165`](../../migrations/v9_new_tables.sql) · [`v229_market_mcp_schema.sql:35`](../../migrations/v229_market_mcp_schema.sql) · [`apply_in_compose.sh`](../../migrations/apply_in_compose.sh) | **fixed** — حُذِفت الكتلةُ المستحيلة (لا عمودَ تُنشأ عليه في أيّ مخطّط، والفهرسُ الحقيقيّ يملكه v229). وحارسٌ ساكن جديد `tests/test_migration_index_columns_exist.py` يقرأ البيان بترتيبه ويجمع أعمدةَ كلّ جدول (CREATE TABLE · ADD COLUMN · RENAME) ويحكم على كلّ فهرسٍ بقائمة أعمدةٍ بسيطة **فقط إن عرف تعريفَ جدوله** (لا حكمَ بلا تعريف). مقيس على الشجرة: ٤٤٨ فهرساً بقائمة بسيطة من ٤٥١ · ٤٤٦ محكوم · جدولان غير مُعرَّفين في البيان (`workflow_instances`, `inventory_locations`) يُتخطّيان · **مخالفٌ واحد = الواقعة**، صفرُ إيجابيّات كاذبة. مُكذَّب بشاهدٍ موجب (أرضيّة ٣٠٠/٢٥٠ على الجرد) وبإعادة إنتاج الواقعة على بيانٍ مصطنع تتوقّع الحمرة. |
| MIGRATION-REAPPLY-IDEMPOTENCY-UNMEASURED-01 | `apply_in_compose.sh` يطبع «✓ طُبّقت N هجرة (idempotent — آمنة على التكرار)» **والتكرارُ لم يُقَس قطّ**: جناحُ التمهيد في CI (`Apply migrations to test DB`) يطبّق البيان على قاعدةٍ فارغة **مرّةً واحدة**، فكلُّ هجرةٍ تمرّ على الفراغ وتنكسر على القائم (حارسُ `to_regclass` الذي يحمي من «لم يُنشأ بعد» ويعمى عن «موجودٌ بشكلٍ آخر») غيرُ مرئيّة حتّى يُعيد المالكُ الإقلاعَ على volume قائم. الحالةُ الأولى المقيسة: `MIGRATION-INDEX-NAMES-A-COLUMN-NO-TABLE-DEFINES-01`. | migrations · ci | [`apply_in_compose.sh:53`](../../migrations/apply_in_compose.sh) · [`ci.yml` (Apply migrations to test DB)](../../.github/workflows/ci.yml) | **fixed** — **نُفِّذ بإذن المالك («نفّذ»، 2026-09-07):** خطوةٌ في وظيفة *Integration Tests* بعد التطبيق الأوّل وفحص «أُنشئ مخطّط» تُعيد تطبيقَ البيان **مرّةً ثانية على القاعدة نفسِها** بالمُشغِّل القانونيّ `apply_migration_manifest.sh` (لا حلقةٌ منسوخة؛ `ON_ERROR_STOP` على كلّ ملفّ + تحقّقُ عدّ المُطبَّق) قبل تشغيل اختبارات التكامل. المُشغِّل لا يحمل سجلَّ «مُطبَّق سابقاً» فالتشغيلُ الثاني تطبيقٌ كامل لا تخطٍّ صامت — وهذا ما يجعله قياساً. عقدُ العدّ في `tests_v9/test_apply_migration_manifest.py` صار ٣ استدعاءات بسببه، وفحصُ `done <` يبقى يمنع أيَّ مُعدِّدٍ منسوخ. **حدُّ صدق:** يُقاس التكرارُ على قاعدةٍ هاجرها البيانُ **الحاليّ** نفسُه؛ قاعدةٌ هاجرها بيانٌ أقدم ثمّ رُقّيت (شكلُ الترقية الحقيقيّ) لا يُقاس هنا، وبوّابةُ الترقية U1 تغطّي جزءاً منه. |
| CLAIM-LEASE-RACE-REPRODUCTION-IS-ITSELF-A-RACE-01 | اختبارُ البرهان الحيّ `test_the_old_pattern_really_does_double_claim_on_a_live_database` يشترط إعادةَ إنتاج العطل في الجولة نفسِها («وإلّا فالقياسُ غيرُ حاسم») — لكنّ إعادةَ الإنتاج كانت **موقوتة**: عاملان عبر `asyncio.gather` يرجوان أن يقع استعلامُ الثاني داخل نافذة `_HOLD_SECONDS=0.30` بين مطالبة الأوّل وتحديثاته. حمرّ في CI على `c84455d9` (`overlap=0 · assert 0 > 0`) بعد أن أضافت الشريحةُ تطبيقاً ثانياً للبيان قبل الاختبارات فأثقلت العدّاء. | testing · concurrency · ci | [`tests_v9/test_worker_claim_lease_live.py`](../../tests_v9/test_worker_claim_lease_live.py) · [job 101835345587](https://github.com/kafaat/ai_platform_complete_v2.0.0_enhanced/actions/runs/34151745834/job/101835345587) | **fixed** — **الآليّةُ مقيسةٌ لا مُخمَّنة** على PostgreSQL 16 محلّيّاً (عنقودٌ مؤقّت في الحاوية): النمطُ القديم يُعيد إنتاج العطل ١٠/١٠ على قاعدةٍ خاملة، و**تأخّرُ بدءِ العامل الثاني ٠٫٣٥ث وحدَه (> الحجز) يقلبه إلى `overlap=0` يقيناً** — لأنّ الأوّل يكون قد أنهى تحديثاته فيرى الثاني صفوفاً أخرى. وفرضيّةٌ أولى (تقاطعُ الاستعلامَين ⇒ `SKIP LOCKED` يفصلهما) **كُذِّبت بالقياس** فلم تُعتمَد. العلاجُ `_Rendezvous` **ثنائيُّ الاتّجاه**: التابعُ لا يُطالِب قبل أن يُنهي القائدُ مطالبتَه، **والقائدُ لا يُحدِّث قبل أن يُطالِب التابع** — والاتّجاهُ الثاني لازمٌ بالقياس (المواعدةُ الأحاديّة ما تزال تحمرّ عند تأخّرٍ ٠٫٦ث). النمطان يُواعِدان عند النقطة نفسِها فالمقارنةُ عادلة. مقيسٌ بعد الإصلاح: ١٢/١٢ خاملاً · ٤/٤ تحت حِمل `pgbench -c 8` · ومحصَّنٌ بالحقن عند تأخّرٍ ٠٫٦ث و٢٫٠ث (القديم `overlap=20`، الجديد `0`). **حدُّ صدق:** الحجزُ `_HOLD_SECONDS` بقي تمثيلاً للعمل الشبكيّ ولم يعد شرطاً للصحّة؛ والسقفُ `_RENDEZVOUS_TIMEOUT=20`ث يمنع تعليقَ الوظيفة إن انكسرت المواعدة. |
...
Post-merge follow-up (2026-09-09, PR #974): GATE01-ONE-SHOT-LIFECYCLE-INCOMPLETE-01 was detected again for GATE01-ADJ-2026-09-08-001 by CI job102488149206 after #990 merged. The detector worked as specified. The consumption record now names main merge `7407eae2ba006a4c4c07018988a0f680e176abea`; both authorized blobs match that merge, and all 50 existing Gate01 tests pass. This closes the stale record locally; automatic post-merge stamping and production certification are not claimed.
...
| UNRECORDED-IRRIGATION-READ-AS-ZERO-APPLIED-WATER-01 | **صفرُ صفوفٍ في `irrigation_runs` يُقرأ «لم يُروَ» لا «لا نعلم».** الميزانُ اليوميّ `raw_depletion = prev + etc − p_eff − irrigation_mm`، و`irrigation_mm` يُجمَع من `irrigation_runs` عند `status='completed'`. ومزارعُ حضرموت ومأرب والجوف وتهامة **يروي من بئرٍ طوال الموسم الشتويّ ولا يسجّل شيئاً** (إفادة المالك 2026-09-12) ⇒ صفرُ صفوف ⇒ `SUM=0` و`untracked=0`. **وعلَمُ `irrigation_volume_untracked` لا يُطلَق أصلاً**: شرطُه `COUNT(*) FILTER (WHERE volume_mm IS NULL)`، أي يلزمه صفٌّ موجودٌ بحجمٍ غائب — فلا يغطّي غيابَ الصفّ نفسِه. فالاستنزافُ يتراكم بـETc وحدَه، يُقصّ عند TAW (`depletion_capped_at_taw`) **ويثبت هناك** بـ`CONFIDENCE_AUTO` لا مُخفَّضة ولا bootstrap، فيُعلِن النظامُ إجهاداً مائيّاً أقصى **دائماً وبثقة** على حقلٍ يُروى فعلاً. والاتّجاه هو الأسوأ: إفراطٌ في التوصية بالريّ على خزّاناتٍ جوفيّةٍ متراجعة. ولا يُمسِكه حارسُ `inconsistent_depletion_exceeds_taw` في المُحلِّل القانونيّ لأنّ القصَّ يجعله **يساوي** TAW لا يتجاوزه. **وهذا صنفُ «الغيابُ يُقرأ قياساً» بعينه، في الموضع الأشدّ أثراً.** | irrigation · water-balance · GATE-01 | [`water_ledger_auto.py:74-78`](../../services/sahool-platform/api/water_ledger_auto.py) (`irrigation_volume_untracked` · `raw_depletion`) · [`phase_runtime_workers.py:583-591,631-632`](../../services/sahool-platform/api/phase_runtime_workers.py) (استعلامُ `irrigation_runs`) · [`v98_water_ledger.sql`](../../migrations/v98_water_ledger.sql) (`irrigation_mm`) · إفادةُ المالك عن الممارسة الحقليّة 2026-09-12 | **open** — **محجوبٌ بـGATE-01**: `phase_runtime_workers.py` في `frozen_paths`، والتفويضاتُ الأربعة كلُّها `CONSUMED` (مُتحقَّقٌ من حزمة `41b418a0` عند `1e0da4fb`) فلا إذنَ قابلاً لإعادة الاستخدام. **ويُضَمّ إلى تفويضِ عطل الرياح في الملفّ نفسِه** (`wind_2m_ms=day0.get("wind_max_ms")` عند السطر ٦٠٨) — إذنٌ واحدٌ يغطّي عطلَين في ملفٍّ مجمَّدٍ واحد بدل طلبَين. **وشرطُ الإغلاق المُصادَق عليه (المالك 2026-09-12): إعدادٌ ديناميكيٌّ لكلّ حقلٍ وموسم** يُعلِن مصادرَ الماء (بئر/سيول/أمطار/أخرى — متعدّدة) وأسلوبَ المتابعة (تلقائيّ/يدويّ مبسّط/تقديرات بلا سجل)، بثلاث خاصّيّاتٍ لازمة: **(أ)** صفرُ صفوفٍ على حقلٍ غيرِ مُجهَّزٍ = **مجهول** لا صفر ⇒ لا `CONFIDENCE_AUTO` ولا `depletion_mm` محسوبٌ قُدُماً، بل غيابٌ مُعلَن؛ **(ب)** اختيارُ «تلقائيّ» **ليس دليلاً على توفّر البيانات** حتّى ينجح الربطُ وتصل قراءة — فنوعُ التوصية يتبع البياناتَ المقيسة لا إجابةَ الاستبيان؛ **(ج)** منعُ احتسابِ الريّة مرّتين عند الجمع بين التلقائيّ وتصحيحِ المزارع، ونَسَبٌ ظاهرٌ للقراءة («من العدّاد» / «سجّله المزارع» / «تقدير المنصّة»). **ولم يُقترَح لهذا العطل شاهدٌ في حزمة البحث `757cbc86`** — خمسةَ عشرَ شاهداً مقترحاً ولا واحدَ يمسّ الريَّ غيرَ المسجَّل (مُقيسٌ بالبحث عن `untracked`/`unrecorded`/`يدوي`/`لا سجل` ⇒ صفر). |
| WORKERS-INHERIT-AN-HTTP-HEALTHCHECK-THEY-CANNOT-ANSWER-01 | **عاملا الريّ يُقرآن unhealthy دائماً بلا أن يفعلا شيئاً خطأ.** `sahool-irrigation-reservation-lifecycle-worker` و`sahool-reservation-dispatch-relay-worker` يُبنيان من صورة المنصّة التي تحمل `HEALTHCHECK curl :8000/healthz` في Dockerfile، ولا يخدم أيٌّ منهما HTTP، وcompose لا يُعرّف لهما `healthcheck:` فيرثان الفحصَ الموروث — وأيُّ `depends_on: service_healthy` عليهما يعلّق إلى الأبد. غيابُ الفحص كان سيكون أهونَ من وراثته. | platform · workers · compose | [`Dockerfile:44`](../../services/sahool-platform/Dockerfile) · [`docker-compose.v9.yml`](../../docker-compose.v9.yml) (الخدمتان) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٤.١ | **fixed** (2026-09-13) — نبضةُ `HeartbeatState` بنمط `phase_runtime_workers` + مسبارُ `worker_heartbeat check` في compose (`--max-age 120` · `300`)؛ المُطفَأ بالعلَم ينبض `idle` لا `running`. ٥ شواهد · ٤ طفرات مقتولة. **لم يُقَس حيّاً** (لا Docker). |
| JWT-DECODE-OUTSIDE-SHARED-SECURITY-01 | **فكُّ JWT مكتوبٌ ١٦ مرّة خارج `shared/security/`** بأسماء مستعارة (`_jwt` · `_jjwt` · `_v_jwt`) بينما ١٢ خدمة تستورد الوحدة المشتركة — ازدواجٌ لا غياب؛ كلُّ موضعٍ سطحُ انحرافٍ عن سياسة المفاتيح (الخوارزميّة · الجمهور · الإبطال) بلا أن يحمرّ شيء (#990 كان واحداً منها). | security · shared | [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٢.٤ · [`test_jwt_decode_outside_shared_security_ratchet.py`](../../tests/architecture/test_jwt_decode_outside_shared_security_ratchet.py) | **open — مجمَّدٌ بأعداده (راتشِت يتقلّص ولا ينمو)**؛ الإغلاقُ نقلُ كلّ موضعٍ إلى `shared.security` مع خفض الأساس. `services/auth` مُصدِرٌ فيبقى آخرَ ما يُنقَل. |
| ROUTER-SIZE-UNGUARDED-AFTER-MAIN-DECOMPOSITION-01 | **تفكيكُ `main.py` نجح والثقلُ انتقل حيث لا قياس:** `routers/` ١٦٨ ملفّاً، `fields.py` 4,241 سطراً · `weather.py` 3,022 · raster `routers/fields.py` 1,712؛ حارسُ `main.py` يحدّ الملفَّ المُسمّى له فقط. صنفُ `COVERAGE-MASKED-BY-A-NEIGHBOURING-GUARD-01`. | platform · architecture | [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٢.٢ · [`test_router_size_ratchet.py`](../../tests/architecture/test_router_size_ratchet.py) | **open — مجمَّدٌ (٦ ملفّات فوق ٨٠٠ بأعدادها)**؛ الإغلاقُ تقسيمٌ يُخفِّض الأساس، وخطّةُ استخراجٍ كخطّة raster قرارُ مالك. |
| FRONTEND-FETCH-OUTSIDE-API-LAYER-01 | **`fetch(` خارج `services/api.ts`: ٨ مواضع في ٣ ملفّات** (وحدة الطقس على الخريطة، كلُّها بـ`weatherFetchHeaders()`). **تصحيحُ قياس:** المراجعةُ قالت ٦٢ أوّلاً لأنّ `grep 'fetch\\('` التقط `refetch`/`prefetch`. | frontend | [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٥ · [`test_frontend_fetch_boundary_ratchet.py`](../../tests/architecture/test_frontend_fetch_boundary_ratchet.py) | **open — مجمَّد**؛ صغيرٌ لكنّ #993 أثبت أنّ كلَّ موضعٍ خارج الطبقة يتطلّب مسحاً عند تغيير ترويسة أو مسار. |
| DECISION-SERVICE-PINS-DIVERGED-WITH-VULNERABLE-STARLETTE-01 | الخدمةُ الوحيدةُ من ٢٨ على `fastapi==0.115.6`/`pydantic==2.10.4`؛ starlette 0.41.3 التابعةُ تحمل ٦ ثغرات معروفة (PYSEC-2026-248/249/1941/1942/2280/2281) غيرَ مرئيّة لأنّ الملفّ خارج `pip-audit` الحاجب. | decision-service · dependencies | [`requirements.txt`](../../services/decision-service/requirements.txt) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٧ | **fixed** (2026-09-13) — 0.136.3/2.13.4 (إصدارا المنصّة)؛ مقيس قبل/بعد 147 ناجحة · 134 متخطّاة سواءً؛ `pip-audit` نظيف. الحكمُ الأخير لوظيفة *Decision Service Tests* على PostgreSQL حيّ. |
| TEST-REQUIREMENTS-MISS-DEFUSEDXML-01 | `api/geo_import.py:27` يستورد `defusedxml` علويّاً فيسقط **جمعُ** جناح المنصّة كلُّه (4,285 حالة) في `preflight` الافتراضيّ على استنساخٍ بمتطلّبات الاختبار وحدها؛ CI لا تراه لأنّها تثبّت `api/requirements.txt`. ظهر في #993 ثمّ 2026-09-13. | testing · preflight | [`tests_v9/requirements-test.txt`](../../tests_v9/requirements-test.txt) | **fixed** (2026-09-13) — `defusedxml==0.7.1` (= `api/requirements.txt:5`). |
| PIP-AUDIT-GATES-19-OF-36-REQUIREMENTS-01 | **١٧ ملفّ متطلّبات خارج `pip-audit` الحاجب**، وتشغيلُه عليها يقيس ٤ حمراء: decision-service (أُصلِح) · `bots/telegram` (aiohttp 3.13.5 · ٢٨ ثغرة، انظر الصفّ التالي) · `requirements-dev.txt` (nltk 3.10.3 عبر `safety 3.8.1` — PYSEC-2026-3740 بلا إصدارٍ مُصلِح) · `requirements-irr-f01-test.txt` (pytest 8.4.2 — أُصلِح). المسحُ الإرشاديّ في CI يراها ويبتلعها بـ`\|\| true`. | dependencies · ci | [`ci.yml` (pip-audit gating)](../../.github/workflows/ci.yml) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٧ | **fixed (source/tests 2026-09-13)** — كل requirements تحت services/bots/agents أصبحت حاجبة بموافقة طلب إصلاح الكل؛ الصور تفحص مستقلة. كل الملفات الاثني عشر المتبقية اجتازت التدقيق محلياً؛ شاهدان للقبول والرفض في `test_runtime_image_audit_gate.py`. |
| TELEGRAM-BOT-AIOHTTP-VULNERABLE-UNDER-AIOGRAM-PIN-01 | `aiogram==3.28.2` يجرّ aiohttp 3.13.5 بـ٢٨ ثغرة (PYSEC-2026-237/2112/2113/3545/3546/3547…) و**يرفض** aiohttp ≥3.14.x (`pip` ResolutionImpossible). aiogram 3.31.0 يقبلها — لكنّ البوت بلا اختبارٍ يستورد aiogram (`tests_v9/test_telegram*.py` لا تلمسه) فترقيتُه بلا شاهد. | bots · dependencies | [`bots/telegram/requirements.txt`](../../bots/telegram/requirements.txt) | **fixed (source/tests 2026-09-13)** — aiogram 3.31.0، pip-audit نظيف، واستيراد وتنفيذ aiogram في `test_telegram_work_order_delivery.py`؛ دليل السلوك في `268f33c`. |
| UNWIRED-IRRIGATION-REAL-WORKFLOW-FUNCTIONS-01 | `workflow_definitions.py` تحمل **١٣ دالّةَ ريّ «حقيقيّ»** (`_irrigation_real_validate/schedule/execute/execute_compensate/verify/approval_gate/suspend_until_approved` …) لا يستدعيها أحد — سيرُ عمل الريّ الحقيقيّ مكتوبٌ **وغيرُ موصول**. | irrigation · platform | [`workflow_definitions.py:182-477`](../../services/sahool-platform/api/workflow_definitions.py) · `execution-audit/generated/EXECUTION_DEPENDENCY_AUDIT_REPORT.md` | **corrected finding (2026-09-13)** — معالجات `irrigation.real` مسجلة عبر decorators؛ الاستنتاج بأنها غير موصولة غير صحيح. راجع C02 في المراجعة المكملة؛ لا حذف ولا ادعاء تحقق فيزيائي. |
| TENANT-GUC-HELPER-NEVER-CALLED-01 | `_apply_tenant_guc` (`main.py:649`) تذكرها ثلاثةُ ملفّات في تعليقاتها بوصفها **ما يجب استدعاؤه** قبل الاستعلامات المُنطّقة بالمستأجر على اتّصالٍ خام (`internal_service.py:29` · `field_lifecycle.py:113` · `trueup.py:305`) ولا يستدعيها أحد. إمّا أنّ المسارات تضبط GUC بطريقٍ آخر (فالتعليقات كاذبة) أو أنّها لا تضبطه (فRLS معطَّل عليها). | tenancy · rls · platform | [`main.py:649`](../../services/sahool-platform/api/main.py) | **measured — لا اتّصالَ خامٍ موصولاً بمسار طلب** (2026-09-13): `FieldLifecycleEngine` لا يُنشَأ في `api/` إطلاقاً، و`TrueUpEngine` يُنشَأ في `main.py:1958` بلا pool (compute النقيّ وحدَه)، والتعليقاتُ الثلاثة صُحِّحت في #997؛ يحرس القياسَ `tests_v9/test_raw_pool_engines_are_not_wired.py` فلا يُوصَّل محرّكٌ خام بـpool إلّا مع ضبط GUC. الحكمُ الأصليّ: يلزم تدقيقُ كلّ اتّصالٍ خام في المسارات الثلاثة قبل الحكم. |
| ALEMBIC-MIGRATION-PATH-ORPHANED-01 | `alembic/` بنسختَين (`0001_baseline` · `0002_ai_recommendation_runtime`) لا يذكرهما `migrations/MANIFEST.txt` ولا `sahool-migrate` ولا أيُّ workflow؛ ثالثُ أنظمة الترحيل (مع `migrations/*.sql` وmigrations decision-service) وبلا مُشغِّلٍ مُعلَن. | migrations · governance | [`alembic/versions/`](../../alembic/versions/) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٣ | **documented legacy (2026-09-13)** — README يعلن أنه نموذج تاريخي خارج سلسلة الإنتاج ويمنع الاستدلال بإرشادات baseline القديمة. الهجرات القانونية باقية؛ تحويل مشغل الإنتاج إلى Alembic ليس جزءاً من الإصلاح. |
| COMPOSE-ALTERNATES-UNGUARDED-AND-NAME-INCOMPATIBLE-01 | ١٤ ملفّ compose غير v9؛ `env_compose_drift_guard` يقرأ v9 فقط؛ `unified`/`light` بأسماء خدمات لا تطابق v9 (`auth-service` مقابل `sahool-auth`) فأيُّ `depends_on`/upstream لأحدهما لا يعمل مع الآخر؛ ٤ منها لا يذكرها أيُّ workflow. | deploy · compose | [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٤.١ | **open** — تثبيتُ v9 وحدَه ونقلُ البدائل إلى `docs/history/` قرارُ مالك. |
| AGENT-MEMORY-PARALLEL-BRAIN-STALE-01 | `agent-memory/` (`FACTS.md` · `CORRECTIONS.md` · `JOURNAL.jsonl` ١٢٤ سطراً) ذاكرةٌ موازية لـ`sahool-brain/`، آخر تحقّقٍ مُعلَن 2026-06-18، بلا حارسٍ ولا رابطٍ من `index.md` — ذاكرتان تفترقان بصمت. | brain · governance | [`agent-memory/FACTS.md`](../../agent-memory/FACTS.md) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٨ | **fixed documentation (2026-09-13)** — جميع صفحات agent-memory موسومة تاريخية وتحيل إلى sahool-brain وCLAUDE؛ رابط الأرشيف في index، والتاريخ محفوظ. |
...
### 2026-09-13 — نتائج المراجعة المكملة وإصلاحاتها

المصدر: [سجل التنفيذ](../../docs/audits/PATCH_CLOSURE_20260913.md). الطلب الصريح
هو تنفيذ إصلاحات المراجعة تباعاً ثم PR؛ يشمل تحسينات المصدر والاختبار والنشر
القابلة للمراجعة، ولا ينشئ تفويض GATE-01 مستقلاً.

| المجموعة | الحالة | الدليل |
|---|---|---|
| SUP-01-NOTIFICATION-WS-AUTH-ALGORITHM-SPLIT-01 | auth يوقّع RS256 و`_validate_ws_token` يقبل HS256 فقط بـ`JWT_SECRET` ⇒ توكن RS256 صالح يُرفض وHS256 يُقبل في الإنتاج؛ compose لا يمرّر `JWT_PUBLIC_KEY`. | notification · auth · compose | [`agent.py:547`](../../agents/notification/agent.py) · [`auth/main.py:67`](../../services/auth/main.py) · شاهدا W01/W02 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — تحقّقٌ مشترك في `shared/security/access_tokens.py` ومفاتيح/سياسة البيئة في compose؛ لا شهادةَ WebSocket حيّة. |
| SUP-02-TENANTLESS-EVENT-BROADCAST-TO-ALL-TENANTS-01 | حدثُ نطاقٍ بلا `tenant_id`/`user_id` يُبثّ إلى كلّ المتّصلين من كلّ المستأجرين (شاهد: مستلمٌ في A وB). | notification · tenancy | [`agent.py:348`](../../agents/notification/agent.py) · W03 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — رفضُ الحدث بلا مستأجر وعزلُ التفضيلات وWebSocket ومعاملات القاعدة؛ RLS الحيّ يحتاج PostgreSQL. |
| SUP-03-NATS-AUTO-ACK-AFTER-FAILED-DELIVERY-01 | `subscribe` بلا `manual_ack` ⇒ إقرارٌ تلقائيّ بعد رجوع الـcallback؛ `handle_msg` يبتلع الاستثناء و`dispatch` يتجاهل فشل `send_push` ⇒ الحدث يُقرّ رغم فشل التسليم. | notification · nats | [`agent.py:399,428,458`](../../agents/notification/agent.py) · W04/W05 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — `manual_ack` وإقرارٌ بعد النجاح، `nak` بتأخير، DLQ، وسجلُّ تسليم لكلّ قناة؛ انهيارُ العملية بين قبول المزوّد وتثبيت الإيصال قد يكرّر التسليم (مُعلَن). |
| SUP-04-NOTIFICATION-READY-WITH-ZERO-SUBSCRIPTIONS-01 | أخطاءُ إنشاء الاشتراكات تُبتلع؛ `readyz` يفحص NATS و`SELECT 1` فقط ⇒ جاهزةٌ والاشتراكاتُ التسعة فاشلة. | notification · ops | [`agent.py:438,662`](../../agents/notification/agent.py) · W06 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — الجاهزيّة تفحص المستهلكين التسعة مع إعادة إنشاء المفقود؛ JetStream حيّ غير مُشغَّل. |
| SUP-05-ERP-SYNC-CURSOR-ADVANCES-ON-OUTAGE-01 | `_get_list` يعيد `[]` عند أيّ فشل، و`sync_products` يقرأه نجاحاً ويقدّم `last_sync` ⇒ تحديثاتُ فترة الانقطاع تُتخطّى عند الاستئناف. | odoo-bridge · erp | [`erp_provider.py:145`](../../services/odoo-bridge/erp_provider.py) · [`erp_runtime.py:266,344`](../../services/odoo-bridge/erp_runtime.py) · W10/W12 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — تمييزُ unavailable عن empty-success، والصفوفُ والمؤشّر في معاملةٍ واحدة؛ فرقُ ساعات المصدر يحتاج قياساً حيّاً. |
| SUP-06-TELEGRAM-ANNOUNCES-UNCREATED-TASK-01 | `cb_create_pest_task` يعدّل الرسالة ويجيب «تم إنشاء المهمة» بلا أيّ نداءٍ لخدمة المهام ولا معرِّف. | bots · work-orders | [`bots/telegram/main.py:804`](../../bots/telegram/main.py) · W07 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — إنشاءُ أمر عملٍ `planned` عبر المسار القائم بمفتاحٍ ثابت وعرضُ المعرِّف بعد الحفظ؛ لا تنفيذَ جهاز. |
| SUP-07-EDGE-SYNC-ENVELOPE-DROPS-FIELD-ID-AND-TIME-01 | `sync_result`/`queue_result` يضعان `field_id` داخل `data` بينما `EdgeSyncRequest` يقرأه علويّاً ⇒ `None`؛ مسارُ الفشل لا يحفظ `occurred_at`؛ الواجهة تعلن `synced` من `OFFLINE_MODE` لا من نتيجة الإرسال. | edge-inference · platform | [`sync_service.py:69`](../../services/edge-inference/sync_service.py) · [`edge_models.py:17`](../../services/sahool-platform/api/edge_models.py) · W13/W14 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — مظروفٌ واحد يُحفظ قبل الإرسال بهويّة/حقل/جهاز/توقيت ويشترط إيصالاً مطابقاً؛ لا جهازَ حيّاً ولا INSERT على PostgreSQL. |
| SUP-08-DEVICE-COMMAND-SIGNATURE-UNBOUND-TO-DEVICE-01 | `HMAC(cmd|ts)` بسرٍّ واحد لا يشمل tenant/device/payload/idempotency_key ⇒ التوقيعُ نفسُه لجهازٍ وحمولةٍ مختلفَين؛ firmware يحفظ ١٦ طابعاً في RAM بلا انتهاء. | actuator · firmware · GATE-01 | [`actuator_runtime.py:570`](../../services/actuator-service/actuator_runtime.py) · [`esp32_mesh_gateway.ino:122,151`](../../firmware/esp32_mesh_gateway/esp32_mesh_gateway.ino) · W15 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **open — محجوبٌ بـGATE-01**: المصدرُ مجمَّد؛ patch مراجعةٍ ببصماته في `docs/audits/patches/` لم يُطبَّق ويحتاج تفويضاً مطابقاً واختباراتِ جهاز. |
| SUP-09-ACTIVATION-ENV-NOT-PASSED-TO-CONSUMERS-01 | v9 لا يمرّر `CMD_HMAC_SECRET` للمشغّل، ولا `PEST/YIELD_MODEL_SHA256` للحافّة، ولا `FEATURE_MOBILE_PUSH`/`FCM_*`/`JWT_PUBLIC_KEY` للإشعارات؛ `SAHOOL_CLOUD_URL` ثابتٌ. | compose · config | [`docker-compose.v9.yml`](../../docker-compose.v9.yml) · [`model_artifact_gate.py:34`](../../services/edge-inference/model_artifact_gate.py) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — التمريرُ في compose بقيمٍ فارغة افتراضاً؛ القيمُ السرّيّة والنماذج مسؤوليّة التزويد. |
| SUP-10-BACKUP-CLEANUP-XARGS-CALLS-SHELL-FUNCTION-01 | `cleanup_old` يمرّر `log` (دالّة shell) إلى `xargs` ⇒ `xargs: log: No such file` ورمز 127 بعد نجاح النسخة. | ops · backup | [`scripts/backup_postgres.sh:178,228`](../../scripts/backup_postgres.sh) · W08 في [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed** (#997، 2026-09-13) — العدّ ثمّ `log` مباشرةً، وعدمُ تسجيل النجاح قبل الاكتمال؛ ٦ اختبارات shell. |
| SUP-11-PITR-RUNBOOK-NOT-EXECUTABLE-AS-WRITTEN-01 | إرشاداتُ PITR تبدأ من `pg_dump` منطقيّ، وتسمّي `recovery.conf` على PostgreSQL 16، و`archive_command` يستدعي أمراً لا يوجد. | ops · recovery | [`scripts/backup_postgres.sh:128,193`](../../scripts/backup_postgres.sh) · [`scripts/restore_postgres.sh`](../../scripts/restore_postgres.sh) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **source repaired** (#997) — نسخةٌ فيزيائيّة + manifest + `recovery.signal`؛ **RPO/RTO واستعادةٌ حيّة `NOT_MEASURED`**. |
| SUP-12-HELM-PORTS-DIVERGE-FROM-IMAGES-01 | خمسةُ منافذ في `values.yaml` لا تطابق Dockerfiles (raster 8000≠8001 · agronomist 8093≠8000 · rag 8130 · KG 8140 · guardrails 8097) و`X_AGENT_TOKEN` بدل `SAHOOL_AGENT_TOKEN`؛ الحارس لا يطابق المنافذ. | helm · deploy | [`helm/sahool/values.yaml:46`](../../helm/sahool/values.yaml) · [`validate_helm_readiness.py:118`](../../scripts/deploy/validate_helm_readiness.py) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **source repaired** (#997) — المنافذ والمتغيّر وPVCs؛ `helm lint/render` واختبارُ رفض المنفذ الخاطئ؛ **لا نشرَ Kubernetes**. |
| SUP-13-MONITORING-TARGETS-WITHOUT-EXPORTERS-01 | Prometheus يطلب `/metrics` من NATS 8222 بلا exporter و`sahool-nginx:9113` بلا listener؛ collector غير معرَّف في v9؛ postgres-exporter معطَّل وقواعدُ إنذارٍ تعتمد عليه. | observability | [`prometheus/prometheus.yml:97,102`](../../prometheus/prometheus.yml) · [`alerts.yml:32`](../../prometheus/alerts.yml) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **source repaired** (#997) — exporters (NATS · nginx · postgres) وcollector في v9 بـdigests، وOTLP من المنصّة والإشعارات؛ **scrape حيّ غير مقيس**. |
| SUP-14-MOBILE-PRODUCT-PATH-INCOMPLETE-01 | لا `android/`/`ios/` ولا `pubspec.lock` متتبَّعة؛ CI يحلّل ويختبر ولا يبني؛ الخادم على FCM legacy. | mobile · notification | [`mobile/sahool_app/pubspec.yaml`](../../mobile/sahool_app/pubspec.yaml) · [`agent.py:203`](../../agents/notification/agent.py) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **source repaired** (#997) — مشاريعُ Android/iOS وقفلُ الحزم وبناءُ Android debug في CI وFCM HTTP v1؛ **جهازٌ حقيقيّ وتوقيعٌ وAPNs غير مقيسة**؛ `C4/M1` و`CONN-07` تبقيان مفتوحتين. |
| SUP-15-IMAGE-DEPENDENCIES-OUTSIDE-REQUIREMENTS-AUDIT-01 | TiTiler/uvicorn مثبَّتان في Dockerfile بنطاقات، فلا تكفي ملفّات requirements للفحص؛ `nats:2-alpine` مرجعٌ متغيّر. | supply-chain | [`services/raster-tiler-service/Dockerfile:23`](../../services/raster-tiler-service/Dockerfile) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **source repaired** (#997) — قفلُ ٧٧ تبعيّة TiTiler وdigests للصور و`pip-audit` حاجب لكلّ ملفّات الصور؛ **لا فحصَ طبقات OS**. |
| SUP-16-ADR-IDENTIFIERS-COLLIDE-AND-STATUS-STALE-01 | معرِّفا ADR-0002 وADR-0003 مكرَّران لوثيقتين مختلفتين؛ ADR-0033 `Proposed` بينما field-management يفرض assertion موقّعة. | architecture · docs | [`docs/adr/`](../../docs/adr/) · [`field-management-service/main.py:115`](../../services/field-management-service/main.py) · [`DEEP_REVIEW_SUPPLEMENT_20260913.md`](../../docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md) | **fixed documentation** (#997) — ADR-0036/0037 للمتعارضين وتصحيحُ حالة ADR-0033؛ لا تغييرَ في المسار. |
| CP997-01-NOTIFICATION-PREFS-READ-LEGACY-INT-USER-ID-01 | عاملُ التسليم كان يحوّل الموضوع إلى `int` ويقرأ عمود `user_id` القديم بينما واجهة التفضيلات تكتب `(tenant_id, user_ref)` نصّاً — كلُّ تفضيلٍ محفوظٍ عبر الـAPI يُفوَّت، وموضوعٌ UUID يسقط قبل الاستعلام. | notification | [`agents/notification/agent.py:237`](../../agents/notification/agent.py) · [`routers/notifications.py:107`](../../services/sahool-platform/api/routers/notifications.py) · [`migrations/v38_notif_channels.sql:41`](../../migrations/v38_notif_channels.sql) | **fixed** (#997، مراجعة Copilot) — القراءة بـ`user_ref` نصّاً؛ الاختبار يُثبِّت الربط. |
| CP997-02-HELM-NETWORKPOLICY-STATIC-PORTS-DENY-RASTER-8001-01 | سياسةُ الشبكة تسمح بقائمة منافذ يدويّة (8000 · 8093 · 8130 · 8140 · 8097) بينما raster انتقل إلى 8001 — الـService يشير إلى منفذٍ تمنعه السياسة. | helm · deploy | [`helm/sahool/templates/networkpolicy.yaml:9`](../../helm/sahool/templates/networkpolicy.yaml) · [`helm/sahool/values.yaml:68`](../../helm/sahool/values.yaml) | **fixed** (#997، مراجعة Copilot) — المنافذُ تُشتقّ من `workloads[*].port` المُفعَّلة؛ **لا `helm` محلّيّاً فالقالبُ غيرُ مُصيَّرٍ هنا**، والـjob `helm lint/render` في CI هو الشاهد. |
| CP997-03-PITR-GUARD-HARDCODED-PG16-REJECTS-DEPLOYED-PG15-01 | `restore_postgres.sh --pitr` كان يرفض كلَّ قاعدةٍ فيزيائيّة لا تحمل `PG_VERSION=16` بينما قاعدةُ Compose المنشورة `postgis/postgis:15-3.4` — سيرُ النسخ/الاستعادة المُعلَن لا يستعيد المكدّسَ الجاري. | ops · backup | [`scripts/restore_postgres.sh:57`](../../scripts/restore_postgres.sh) · [`docker-compose.v9.yml:258`](../../docker-compose.v9.yml) · [`docs/runbooks/POSTGRES_RECOVERY.md`](../../docs/runbooks/POSTGRES_RECOVERY.md) | **fixed** (#997، مراجعة Copilot) — `SUPPORTED_PG_MAJORS` (15 16) ومطابقةُ الأكبر مع `RECOVERY_PG_MAJOR`/`postgres` المحلّيّ؛ اختباران يشهدان القبولَ والرفض. **لا تمرينَ استعادةٍ حيّ.** |
| CP997-04-EDGE-LEGACY-REPLAY-FILLS-DEVICE-FROM-ENV-01 | إعادةُ تشغيل ملفّات الطابور القديمة كانت تملأ `device_id` الغائب من بيئة العمليّة الحاليّة — بعد استبدال جهازٍ تُنسَب قراءةٌ قديمة إلى الجهاز الخطأ. | edge | [`services/edge-inference/sync_service.py:124`](../../services/edge-inference/sync_service.py) | **fixed** (#997، مراجعة Copilot) — الهويّةُ من المحفوظ فقط؛ ملفٌّ بلا هويّةٍ مخزّنة يُحتفَظ به ولا يُرسَل. |
| CP997-05-EDGE-IDEMPOTENCY-GLOBAL-UNIQUE-VS-TENANT-LOOKUP-01 | الفهرسُ الفريد عالميّ على `idempotency_key` بينما بحثُ الصفّ القائم مقيّدٌ بالمستأجِر — مفتاحُ المستأجِر A يجعل إدراج B يصطدم ثمّ لا يجد صفّاً فيُرفَع 409 على حدثٍ صالح. | edge · db | [`migrations/v9_edge_idempotency.sql:10`](../../migrations/v9_edge_idempotency.sql) · [`routers/edge.py:43`](../../services/sahool-platform/api/routers/edge.py) | **open — محجوبة خلف GATE-01** (#997، مراجعة Copilot). العلاجُ فهرسٌ جزئيّ على `(tenant_id, idempotency_key)` بهجرةٍ جديدة، و`migrations/MANIFEST.txt` **مسارٌ مجمَّد** لا يُفتح إلّا بتفويض مالك (`gate01-physical-effect-freeze` حمّر `v231` على `b5a3ce32` فأُزيلت). إلى حينه الهدفُ يطابق الفهرسَ القائم (هدفٌ بلا فهرس يُسقِط كلَّ إدراج) والحدُّ مُعلَن في docstring الراوتر؛ المفاتيحُ `uuid4().hex` فالتصادمُ عبر المستأجِرين عمليّاً معدوم. **قرارُ المالك (2026-09-13):** قَبِل الحدَّ الموثَّق وحلّ المحادثة ودمج #997 (`62fc7b01`)؛ الهجرةُ تنتظر تفويضَ GATE-01 متى شاء. |
| CP997-06-PROMETHEUS-SOIL-TARGET-NAME-UNRESOLVABLE-01 | هدفُ scrape `sahool-soil:8000` لا يطابق اسمَ خدمة Compose `sahool-soil-service` ولا alias له — DNS يفشل وscrape التربة يبقى down. | observability | [`prometheus/prometheus.yml:41`](../../prometheus/prometheus.yml) · [`docker-compose.v9.yml:2283`](../../docker-compose.v9.yml) | **fixed** (#997، مراجعة Copilot) — الهدفُ باسم الخدمة المُعلَن؛ **scrape حيّ غير مقيس**. |
| CP997-07-ARCHITECTURE-GRAPH-OMITS-FRONTEND-COMPOSE-01 | رسمُ البنية يكتشف ملفّات Compose في الجذر فقط، فـ`frontend/docker-compose.web.yml` (على سطح `compose_surface.py`) لا يُسهِم بخدماته ولا حوافّه. | architecture · ci | [`scripts/ci/architecture_graph.py:131`](../../scripts/ci/architecture_graph.py) · [`scripts/ci/compose_surface.py:46`](../../scripts/ci/compose_surface.py) | **fixed** (#997، مراجعة Copilot) — يستهلك `compose_files()`؛ الرسمُ المولَّد بقي مستقرّاً (122 عقدة · 315 حافّة) لأنّ ملفَّ الواجهة بلا `depends_on`. |
| CP997-08-EDGE-IDEMPOTENCY-KEY-WIDTH-API-128-VS-DB-32-01 | الـAPI يقبل مفتاحاً حتّى 128 حرفاً بينما العمود `VARCHAR(32)` — طلبٌ صالح بطول 33–128 يسقط عند حدّ القاعدة (500) لا عند التحقّق (422). | edge · db | [`services/sahool-platform/api/edge_models.py:21`](../../services/sahool-platform/api/edge_models.py) · [`migrations/v9_edge_idempotency.sql:7`](../../migrations/v9_edge_idempotency.sql) | **fixed** (#997، مراجعة Copilot) — العقدُ يُضبَط على المخزَّن: `max_length=32` فيُرفَض الأطولُ بـ422 قبل القاعدة. الخيارُ الآخر (توسيعُ العمود إلى 128) هجرةٌ على MANIFEST المجمَّد خلف GATE-01 فلم يُسلَك؛ مفاتيحُ الجهاز `uuid4().hex` = 32 بالضبط. |
| CP997-09-PLATFORM-PUSH-STILL-ON-FCM-LEGACY-ENDPOINT-01 | `shared/fcm.py` (HTTP v1 بحسابٍ خدميّ وإيصالٍ مُسمّى) وُجِد بينما مسارُ تنبيهات المنصّة `real_channel_sender` يرسل إلى `fcm/send` القديم بـ`FCM_SERVER_KEY` — كلُّ push تُنشئه المنصّة يتجاوز تحقّقَ الاعتماد والإيصال. | notification · platform | [`services/sahool-platform/api/alert_senders.py:132`](../../services/sahool-platform/api/alert_senders.py) · [`shared/fcm.py:115`](../../shared/fcm.py) | **fixed** (#997، مراجعة Copilot) — `_send_push` عبر `shared.fcm.send_push`؛ `FCM_SERVER_KEY` زال من المصدر ومن allowlist الانجراف؛ ثلاثُ حالاتٍ بـHTTP وهميّ. **لا تسليمَ إلى جهازٍ حقيقيّ.** |
| REPORT2-C01-ONE-FAILING-CHANNEL-BLOCKS-THE-HEALTHY-ONES-01 | التقرير الجنائيّ الموحَّد ٢ (C01، P1): فشلُ البريد كان يرفع `DeliveryUnavailable` من `_deliver_channel` فيغادر `dispatch` قبل Telegram وPush، ويُعاد الحدثُ كلُّه حتّى dead-letter دون أن تُجرَّب القنواتُ السليمة. | notification | [`agents/notification/agent.py:375`](../../agents/notification/agent.py) · [`test_notification_delivery_reliability.py`](../../tests_v9/test_notification_delivery_reliability.py) | **fixed** (2026-09-13): القنواتُ تُخطَّط ثمّ تُجرَّب كلُّها، وتُجمَع الفاشلةُ ويُرفَع بعد آخرها فتُعاد الرسالة؛ الإيصالاتُ `sent` تُتخطّى في المحاولة التالية. اختبارُ قبول: بريدٌ يفشل مرّتين ثمّ يتعافى، Telegram يُرسَل مرّةً واحدة عبر المحاولات الثلاث، وكلُّ الإيصالات `sent` في النهاية. **لا NATS حيّ** — المقيس بمحاكاة القاعدة والمزوّدين. |
| REPORT2-C05-GITLEAKS-PATH-ALLOWLIST-WAS-DIRECTORY-WIDE-01 | التقرير الجنائيّ الموحَّد ٢ (C05): مدخلُ `docs/audits/patches/.*\.patch$` في `.gitleaks.toml` يُعفي أيَّ patch يُلقى في المجلّد لاحقاً — مقيسٌ بـgitleaks 8.24.3: توكنٌ اصطناعيّ في patch جديد **تُخطّي الملفُّ كلُّه** («Skipping file due to global allowlist»). | security · ci | [`.gitleaks.toml:22`](../../.gitleaks.toml) · [`tests_v9/test_gitleaks_allowlist_scope.py`](../../tests_v9/test_gitleaks_allowlist_scope.py) | **fixed** (2026-09-13): المدخلُ ضُيِّق إلى `SUP-08-actuator-v2-PENDING-GATE-01.patch` وحده (التوكنُ الاصطناعيّ يُبلَّغ الآن، وSUP-08 ما زال يُعفى)، واختبارٌ يفرض أنّ كلَّ مدخلِ `paths` يصيب ملفّاً متعقَّباً واحداً بالضبط. |
| REPORT2-C02-WS-CONNECTION-KEY-SUBJECT-IS-GLOBALLY-UNIQUE-01 | التقرير الجنائيّ الموحَّد ٢ (C02): `ConnectionManager` يفهرس الجلسات بـ`user_id` وحده ويرفض تعارضَ المستأجِر — فلو تكرّر `sub` بين مستأجِرين لرُفِض اتّصالٌ مشروع. | notification · identity | [`agents/notification/agent.py:552`](../../agents/notification/agent.py) · [`migrations/v9_foundation.sql:38`](../../migrations/v9_foundation.sql) · [`services/auth/main.py:607`](../../services/auth/main.py) | **not a defect — مقيس (2026-09-13):** `users.id` مفتاحٌ أساسيّ `SERIAL` عالميّ يحمل `tenant_id` واحداً، و`sub = str(user_id)`؛ فالموضوعُ أحاديُّ المستأجِر بعقد الهويّة والرفضُ يخصّ توكناً مزوَّراً لا مستخدماً متعدّدَ المستأجِرين. يُعاد الفتحُ إن صار الحسابُ متعدّدَ المستأجِرين. |
| REPORT2-C03-DEDUP-FOLLOWS-STREAM-MESSAGE-NOT-EVENT-ID-01 | التقرير الجنائيّ الموحَّد ٢ (C03): `_delivery_key` يقدّم `_delivery_id` (stream:sequence) على `event_id` — إعادةُ تسليم الرسالة نفسها لا تتكرّر، لكنّ إعادةَ نشر الحدث المنطقيّ نفسه في رسالةٍ جديدة تُنتِج مفتاحاً آخر. | notification | [`agents/notification/agent.py:247`](../../agents/notification/agent.py) | **open — قرارُ نطاق:** أيُّ idempotency يُراد: redelivery فقط (الحالُ الآن، مقيسٌ باختبار) أم إعادةُ نشر الحدث أيضاً؟ الثاني يحتاج هويّةَ حدثٍ موثوقةً من المنتِج وتضمينَ المستأجِر/المستخدم/القناة، واختبارَين منفصلَين. |
| REPORT2-C04-QUEUED-PUSH-RECEIPTS-HAVE-NO-CONSUMER-01 | التقرير الجنائيّ الموحَّد ٢ (C04): `record_only` يحفظ إيصالَ push بحالة `queued` ويُقرّ الرسالةَ؛ لا قارئَ لهذه الصفوف في الشجرة (مقيس: المُنتِج وحده يقرأ `notification_delivery`)، فتفعيلُ FCM لاحقاً لا يُصرِّف القديم. | notification · mobile | [`agents/notification/agent.py:315`](../../agents/notification/agent.py) | **open — قرارُ سياسة:** مستهلكُ الصفوف المعلَّقة (إعادةُ إرسالٍ عند التفعيل؟ عمرٌ أقصى؟ إلغاء؟) قرارُ منتَج؛ بعده اختبار flag off → queued → flag on → تسليمٌ واحد أو انتهاءٌ موثَّق. |
| REPORT3-RASTER-SERVICE-TOKEN-GUARD-UNBOUND-01 | التدقيق الموحَّد ٣ (P0): `require_service_token(x_agent_token, agent_token)` بوسيطين إلزاميّين بينما 14 نداءً في خمسة راوترات (storage 6 · processing 3 · jobs 2 · timeseries_routes 2 · imagery_search 1) تمرّر الترويسة وحدَها — كلُّ طلب على تلك النقاط يسقط بـTypeError (500) قبل أيّ تحقّق، منذ `3b77847b`، وبلا اختبار يستورد الراوترات (التقرير عدّ 11 في أربعة). | raster | `services/raster-service/raster_security_context.py:246` · `services/raster-service/routers/storage.py:28` | **fixed** (2026-09-13): `agent_token` اختياريّ يُقرأ من `raster_settings.AGENT_TOKEN` عند النداء؛ `tests_v9/test_raster_service_token_binding.py` يربط كلَّ نداء بالتوقيع الحقيقيّ عبر AST ويقيس 401/503 للنداء بوسيط واحد. |
| REPORT3-AI-ADVICE-EVENT-OK-IS-CONSTANT-01 | التدقيق الموحَّد ٣ (P0): `/internal/events/ai-advice` يمرّر `critical=False` إلى `_emit_domain_event` (best-effort موثَّق) ثمّ يُعيد `ok: True` ثابتة — فابتلاعُ فشل الإصدار يبدو إدامةً. | platform/ai_agronomist | `services/sahool-platform/api/routers/internal_service.py:186` · `services/sahool-platform/api/main.py:685` | **fixed** (2026-09-13): `_emit_domain_event` يُعيد bool (True كُتب/False ابتُلع غير حرج)؛ الردّ يحمل `ok`/`persisted`/`reason` الحقيقيّة، و`_record_ai_advice_event` في المستشار يعرض `not_persisted`. اختبار: `tests_v9/test_ai_advice_persistence_and_grounding_honesty.py`. |
| REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01 | التدقيق الموحَّد ٣ (P0/RAG-10): `mode = "generated_grounded"` يُوسَم لمجرّد أنّ النموذج أجاب ولو كانت مقتطفات RAG صفراً (سياقُ KG وحدَه)، و`guardrail_result` عبارةٌ ثابتة «لا قرار» حتّى مع نصّ مولَّد يُعاد للمستخدم. | ai_agronomist | `services/ai_agronomist/ai_evidence_runtime.py:853` · `services/ai_agronomist/ai_evidence_runtime.py:950` | **fixed** (2026-09-13): نصٌّ مولَّد بلا مقتطف RAG يُحجَب (`generation_status=suppressed_ungrounded`) ويبقى جوابُ الأدلّة؛ `_guardrail_result` يُعلِن `generated_text_checked: False` حين يُعاد نصٌّ مولَّد. **حدّ:** لا حاجزَ يُنفَّذ بعدُ على النصّ المولَّد — الوسمُ صادق لا أكثر؛ تنفيذُ حاجزٍ فعليّ قرارُ نطاق. |
| REPORT3-LOCAL-AI-RAG-SINGLE-INIT-ATTEMPT-01 | التدقيق الموحَّد ٣ (P0): `_init_models_background` محاولةٌ واحدة بلا إعادة؛ تأخّرُ Qdrant/Ollama عند الإقلاع يترك الخدمة 503 إلى الأبد و`/readyz` يقول «قيد التحميل». | local-ai-rag | `services/local-ai-rag/main.py:430` | **fixed** (2026-09-13): `services/local-ai-rag/init_retry.py` (محاولات محدودة `RAG_INIT_MAX_ATTEMPTS`، تراجع أسّيّ بسقف) وحالةٌ مشتركة يقرؤها `/readyz` فيميّز `init_failed` عن `initialising` مع المحاولات وآخر خطأ. اختبار: `tests_v9/test_local_ai_rag_init_retry.py`. |
| REPORT3-U01-ROW-COUNT-PROMOTES-TO-FIELD-VERIFIED-01 | التدقيق الموحَّد ٣ (U01، P1): `aggregate_evidence` يرفع `evidence_level` إلى `field_verified` عند 30 صفّاً دون استقلال الحقول/المواسم ولا مراجعة مختصّ — 30 صفّاً للحقل والموسم نفسيهما بنسبة نجاح صفر حصلت على شارة «مُتحقَّق ميدانيّاً». | learning | `services/sahool-platform/api/evidence_registry.py:16` · `frontend/src/sections/LearningDashboardPage.tsx:33` | **fixed** (2026-09-13): العتبة ⇒ `field_sample_complete` (عيّنة مكتملة بانتظار المراجعة)؛ `field_verified` يشترط `reviewed=True`؛ `independence` (حقول/مواسم/مزارع/مجهولة الوحدة) و`review_status` تُعرَض؛ بوّابةُ التكيّف تبقى مغلقة على غير المراجَع؛ `learning_feedback` يقترح `expert_review`؛ شارةُ الواجهة الجديدة. **حدّ:** لا مسارَ مراجعةٍ في API بعدُ — `reviewed` يمرّره المُنادي، وطلبُ `/adapt` ما زال يقبل `evidence_level` من العميل (قائم قبل هذه الجولة). |
| REPORT3-U02-IMMATURE-ACTUAL-COUNTED-AS-SUCCESS-01 | التدقيق الموحَّد ٣ (U02، P1): `_derive_rec_success` يشترط القبول والغلّتين ولا يفحص `matured_within_lag`، و`process_season_closed_event` يعدّ الأزواج بلا شرط النضج — قيمةٌ فعليّة مبكّرة دخلت النجاح والترشيح؛ الاختبارُ القائم غطّى `actual=None` وحدَه. | learning | `services/sahool-platform/core/outcome_reconciler.py:55` · `services/sahool-platform/api/learning_feedback.py:157` | **fixed** (2026-09-13): `recommendation_outcome_eligibility(row, require_acceptance)` سياسةٌ واحدة (missing_prediction/missing_actual/non_finite_value/immature/not_accepted) يستعملها الموحِّد والترشيح؛ التقييم يحمل `excluded_count`/`excluded_reasons`؛ دقّةُ التوقّع لا تشترط قبولَ التوصية. |
| REPORT3-U03-ONE-LINKED-CASE-COUNTS-AS-TWO-SAMPLES-01 | التدقيق الموحَّد ٣ (U03، P2): صفّان مربوطان بالقرار نفسه (`outcome_record` + `recommendation_outcomes`) يعطيان `linked_group_count=1` و`sample_count=2` — عدُّ الصفوف يُقرأ استقلالَ شواهد. | learning | `services/sahool-platform/api/learning_summary.py:219` | **fixed** (2026-09-13): `independent_case_count` (مجموعة مربوطة = حالة واحدة + كلّ صفّ غير مربوط) و`rows_by_source` و`sample_count_basis: rows` تُعرَض معاً؛ النموذجان يبقيان. |
| REPORT3-U04-MISSING-WATER-VOLUME-BECOMES-ZERO-01 | التدقيق الموحَّد ٣ (U04، P1): `summarize_operational_records` يجمع الحجمَ الغائب صفراً ثمّ `ai_feature_row` يُصدِر `water_m3_per_ha=0` عن ريٍّ سُجِّلت ساعاتُه بلا حجم — يفسد تعلّم كفاءة الريّ. | farm_ledger/learning | `services/sahool-platform/core/farm_operations_ledger.py:235` · `services/sahool-platform/core/farm_operations_ledger.py:284` | **fixed** (2026-09-13): الحجمُ None حين لا قياس؛ `water_records_total/measured/unmeasured` و`water_measurement_complete` في الملخّص وصفّ التعلّم؛ النِّسَبُ المشتقّة None لا 0 (بما فيها `farm_closed_loop`). **تكملة (2026-09-14، Copilot على #1001):** مسارا SQL في الراوتر (ملخّص HTTP وملخّص الموسم) كانا ما زالا `COALESCE(SUM, 0)` ثمّ `or 0.0` — الآن `SUM` بلا COALESCE + عدّادا القياس، والقراءةُ عبر `water_summary_from_row`/`water_summary_payload` في وحدة النطاق؛ الردّ يحمل `water_volume_m3: null` و`water_measurement`، والنوعُ في الواجهة `number \| null`. |
| REPORT3-U05-FIELD-FORM-NOT-PROJECTED-TO-PRACTICE-01 | التدقيق الموحَّد ٣ (U05): `project_submission` يستبعد `kind=field_form` (مقصود لحماية نموذج المشاهدات)؛ النماذجُ الديناميكيّة محفوظة بلا ترجمةٍ إلى ممارسة/نتيجة قابلة للتحليل. | scout-ingest/farm_ledger | `shared/contracts/ingest/projection.py:84` · `services/scout-ingest-service/field_forms_api.py:465` | **open — قرارُ مالك:** يتطلّب تعريفَي نموذجٍ (ممارسة/متابعة) بإصدارين وربطاً صريحاً عبر الكاتب الحاليّ — قرارُ منتج ومخطّط، لا يُخترع هنا. |
| REPORT3-U06-FARM-LEDGER-OPERATION-NOT-IDEMPOTENT-01 | التدقيق الموحَّد ٣ (U06، P2): `POST /api/v1/farm-ledger/operations` يولّد `operation_id` جديداً في كلّ طلب بلا `Idempotency-Key` — إعادةُ طابور الهاتف بعد انقطاع الردّ تُنشئ عمليّتين للمصدر نفسه. | farm_ledger | `services/sahool-platform/api/routers/farm_operations_ledger.py:212` · `services/sahool-platform/api/main.py:755` | **fixed** (2026-09-13): العقدُ القائم `_idem_key`/`_idempotent` داخل معاملة `tenant_connection` مع `CommandStore`؛ الإعادةُ تُعيد النتيجةَ المخزَّنة بالمعرّف الأصليّ؛ المفتاحُ نفسه بحمولةٍ مختلفة ⇒ 409 (`request_digest` في حمولة الأمر — توافقٌ خلفيّ للمسارات بلا بصمة). اختبار العقد في `tests_v9/test_idempotency_mobile.py`؛ **حدّ:** لا قطعَ اتّصالٍ على قاعدة حيّة هنا. **تكملة (2026-09-14، Copilot على #1001):** فحوصُ النطاق (حقل/موسم/وحدة/مزرعة) صارت داخل العمل نفسه فيُصنَّف المفتاحُ المُعاد بحمولةٍ مختلفة 409 قبل أيّ 404/422. |
| REPORT3-U02-SEASON-CLOSE-ORDER-BY-UNDEFINED-COLUMN-01 | مراجعةُ Copilot على #1001 (مكتومة): `process_season_closed_event` يرتّب `recommendation_outcomes` بـ`created_at,id` بينما الجدولُ (v49) يملك `outcome_id`/`issued_at`/`outcome_recorded_at` فقط — كلُّ إغلاق موسم كان يفشل بعمود غير معرَّف قبل أيّ إدامة، ولم يكشفه اختبارٌ لأنّ الاتّصال الوهميّ لا يفسّر SQL. وفحصُ الإعادة كان قبل القفل الاستشاريّ فيُصدِر تسليمان متزامنان الحدثَ مرّتين. | learning | `services/sahool-platform/api/learning_feedback.py:151` (قبل الإصلاح) · `migrations/v49_zone_key_recommendation_outcomes.sql:17` | **fixed** (2026-09-14): `ORDER BY COALESCE(outcome_recorded_at, issued_at),outcome_id` والقفلُ قبل فحص الإعادة؛ اختبارٌ يقرأ أعمدة v49 من DDL ويطابق ORDER BY عليها ويؤكّد ترتيب القفل (`tests_v9/test_irrigation_and_decision_architecture.py`). **حدّ:** الاتّصالُ الوهميّ لا يفسّر SQL — الاختبار يطابق الأعمدة نصّيّاً لا بتنفيذ. |
| REPORT3-U07-REVIEW-DECISION-NOT-PERSISTED-01 | مراجعةُ Copilot على #1001 (مكتومة): `resolve_learning_source` يُصدِر `review_status`/`agronomically_approved`/`review` (U07) لكنّ كاتبَ `online_learning_updates` في `phase_runtime_store` يُديم `traceability_status` وحدَه — قرارُ المراجعة يضيع عند الكتابة فلا تميّز السجلّاتُ المُدامة المُتتبَّعَ من المُعتمَد. | learning | `services/sahool-platform/api/phase_runtime_store.py:462` · `migrations/v151_learning_source_lineage.sql:16` | **open — قرارُ مالك (GATE-01):** الجدولُ لا يملك عموداً للمراجعة وإضافتُه هجرة؛ الحدُّ مُعلَن في توثيق `resolve_learning_source`، والصفوفُ بلا `review_status` تُعَدّ `unreviewed` في الملخّص (لا اعتمادَ ضمنيّاً). |
| REPORT3-LEARNING-OPTIONAL-READS-POISON-OUTER-TRANSACTION-01 | مراجعةُ Copilot على #1001 (مكتومة): القراءاتُ «best-effort» في `routers/learning_summary.py` (recommendation_outcomes/dispatch_decisions) والخمسُ التكميليّة في حالة الحقل-الموسم (`routers/seasons.py`) تجري داخل معاملة `tenant_connection` الخارجيّة؛ جدولٌ غائب في نشرٍ جزئيّ يُعلِّم المعاملةَ فاشلة فيفشل الالتزامُ ويعود 503 رغم الالتقاط المحلّيّ — الوعدُ بـ«الغياب لا يُعطّل» كان كاذباً. | learning/seasons | `services/sahool-platform/api/routers/learning_summary.py:78` · `services/sahool-platform/api/routers/seasons.py:553` | **fixed** (2026-09-14): كلُّ قراءة اختياريّة داخل `async with conn.transaction()` (نقطة حفظ تُرتَدّ وحدَها) والالتقاطُ خارجها؛ اختبار `test_optional_reads_run_inside_savepoints` يفرض النمط على الملفّين. **حدّ:** بلا Postgres حيّ هنا — الاختبار بنيويّ على المصدر. |
| REPORT3-DECISION-EXPLAIN-RAW-JSONB-METRICS-01 | مراجعةُ Copilot على #1001: `decision_explain._evidence_summary` يمرّر `metrics` كما يأتي من asyncpg (JSONB نصّاً خاماً بلا codec) إلى `evidence_from_persisted_outcomes` فيرفع `.get` على str — تفعيلُ `FEATURE_DECISION_STUDIO` مع أيّ صفّ نتيجة ⇒ 500 (مسارُ المعايرة يفكّ العمود نفسه). | decision | `services/sahool-platform/api/routers/decision_explain.py:58` | **fixed** (2026-09-14): يُفكّ النصّ قبل البناء؛ اختبار `test_decision_explain_evidence_decodes_raw_jsonb_metrics`. |
| REPORT3-U01-YIELD-ROWS-HAVE-NO-REGION-01 | مراجعةُ Copilot على #1001 (مكتومة): v49 بلا عمود `region`، فكلُّ نتيجة توصية تبلغ `normalize_recommendation_outcome` بـ`region=None` وتُجمَّع تحت `_unspecified` في لوحة التعلّم — صفوفُ تعلّم الغلّة تختفي من بطاقات المناطق الحقيقيّة. | learning | `services/sahool-platform/api/routers/learning_summary.py:79` · `migrations/v49_zone_key_recommendation_outcomes.sql:17` | **fixed** (2026-09-14): الاستعلامُ يشتقّ المنطقة من `fields.region` (`LEFT JOIN fields`) ويمرّرها في الصفّ. **حدّ:** `outcome_record.region` يُكتَب من القرار لا من الحقل — مصدران قد يختلفان؛ توحيدُهما قرارُ نمذجة. |
| REPORT3-LEARNING-DASHBOARD-READS-ROOT-AGGREGATES-01 | مراجعةُ Copilot على #1001: `LearningDashboardPage` تقرأ `outcome_count`/`success_rate`/`regions_verified` من جذر ردّ `/api/v1/learning/summary` بينما `summarize_learning` يضع الإجماليّ تحت `overall` ولا يُصدِر `regions_verified` أصلاً — البطاقاتُ الإجماليّة تعرض «—»/0 على بيانات موجودة، والنوعُ `LearningSummary` في `services/api.ts` كان يصف عقداً لا يوجد. | frontend/learning | `frontend/src/sections/LearningDashboardPage.tsx:195` · `frontend/src/services/api.ts:1064` · `services/sahool-platform/api/learning_summary.py:156` | **fixed** (2026-09-14): القراءة من `summary.overall`، وعدُّ «مُتحقَّق ميدانيّاً» من بطاقات المناطق، والنوعُ يطابق العقد؛ اختبارٌ سكونيّ يمنع العودة إلى الجذر. |
| REPORT3-IDEMPOTENCY-KEY-NOT-IN-CORS-ALLOW-HEADERS-01 | مراجعةُ Copilot على #1001: عقدُ idempotency (`_idem_key` على create_activity/update_field/create_field/farm-ledger) يشترط ترويسة `Idempotency-Key`، و`allow_headers` في وسيط CORS للمنصّة لا يحملها — preflight المتصفّح يفشل قبل بلوغ أيّ معالِج idempotent (سابقٌ على هذا الـPR لكلّ النقاط الأخرى). | platform/cors | `services/sahool-platform/api/main.py:815` · `shared/security/cors_policy.py:30` | **fixed** (2026-09-14): `PLATFORM_ALLOW_HEADERS` في `shared.security.cors_policy` يشمل الترويسة و`main.py` يستهلكه (بلا نموّ في سطوره — حارس التفكيك)؛ اختبار `test_idempotency_key_header_is_allowed_by_cors`. |
| REPORT3-U01-ADAPT-TRUSTS-CLIENT-EVIDENCE-LEVEL-01 | مراجعةُ Copilot على #1001: `POST /api/v1/calibration/{region}/adapt` يكتب `EvidenceRecord.evidence_level` من حمولة الطلب فوق الدليل المُجمَّع، فيرسل العميلُ `field_verified` بلا مراجعة ويبلغ `auto_apply_eligible` — البوّابةُ الجديدة (المراجعة قبل الاعتماد) كانت تُلتَفّ من الطلب نفسه. | calibration | `services/sahool-platform/api/routers/calibration.py:237` · `services/sahool-platform/api/adaptive_calibration.py:49` | **fixed** (2026-09-14): مستوى `field_verified` من الحمولة يُخفَّض إلى `field_sample_complete` مع تحذير و`review_gate` صريح؛ البوّابةُ تفتح فقط لدليلٍ من السجلّ بمراجعة مُثبَتة (اختباران). **حدّ:** لا مسارَ مراجعةٍ موثوق في API بعد — `adapt-from-evidence` وتطبيقُه محجوبان صراحةً (`REPORT3-U07-REVIEW-DECISION-NOT-PERSISTED-01`). |
| REPORT3-U01-COUNT-ONLY-CONSUMERS-CALL-30-ROWS-VERIFIED-01 | مراجعةُ Copilot على #1001 (مكتومة): `evidence_map._classify` و`routers/decision_confidence._evidence_value` يصنّفان أيَّ حقل بـ≥ 30 قياساً «مؤكَّداً ميدانيّاً» (أخضر / درجة 1.0) من العدّ وحدَه — استعلاماتُهما لا تحمل حالةَ مراجعة، فتناقض سجلَّ الدليل الذي صار يشترط المراجعة. | evidence_map/decision | `services/sahool-platform/api/evidence_map.py:55` · `services/sahool-platform/api/routers/decision_confidence.py:49` | **fixed** (2026-09-14): الفئةُ `field_sample_complete` (كهرمانيّ، «بانتظار المراجعة») في الخريطة والأسطورة والنوع في الواجهة، ودرجةُ الدليل 0.8 «بانتظار مراجعة مختصّ»؛ `field_verified` في الأسطورة محجوزٌ لدليل مراجَع لا يُنتِجه العدّ. |
| REPORT3-U01-COUNT-ONLY-CONSUMERS-INCLUDE-UNEVALUATED-ROWS-01 | مراجعةُ Copilot على #1001: استعلاما خريطة الدليل وثقة القرار يعدّان صفوف `outcome_record` بـ`COUNT(*)` بينما `evidence_registry` يُسقِط الصفوف التي `metrics.n_evaluated == 0` — 30 صفّاً فارغاً/معلّقاً كانت تُعرَض «عيّنة مكتملة» في الخريطة وتمنح درجة 0.8 في الثقة. | evidence_map/decision | `services/sahool-platform/api/routers/evidence_map.py:59` · `services/sahool-platform/api/routers/decision_confidence.py:161` | **fixed** (2026-09-14): شرطُ SQL موحَّد `EVALUATED_OUTCOME_PREDICATE` (في `api/evidence_map.py`) على العدّ والنجاحات وآخر نتيجة وعلى عدّ الثقة؛ اختبار `test_count_only_consumers_count_evaluated_rows_only` يفرضه على الملفّين. **حدّ:** بلا Postgres حيّ هنا — الاختبار بنيويّ على المصدر. |
| REPORT3-U07-TRACEABLE-IS-NOT-AGRONOMICALLY-APPROVED-01 | التدقيق الموحَّد ٣ (U07، P2): `resolve_learning_source` يُعيد `traceable/applies=True` لنوعٍ معروف ومعرّف غير فارغ ولو كان صناعيّاً؛ و`verify_against_data` يقبل Boolean فيرفع الحالة إلى «مؤكّدة» بلا أثرٍ لِما أكّدها. | learning/knowledge | `services/sahool-platform/core/learning_source_lineage.py:56` · `services/sahool-platform/knowledge/farmer_knowledge.py:171` | **fixed** (2026-09-13): `review_status`/`agronomically_approved`/`review` (مراجِع + حكم + أدلّة) مستقلّة عن `traceable` في النَّسَب والملخّص؛ `verify_against_data(evidence=, verified_by=)` يحفظ الدليل، والتأييدُ بلا مرجع يبقى `PENDING`. **حدّ:** التحقّقُ من وجود المصدر فعلاً (لا شكلِه) يحتاج قراءةَ قاعدة — يُربَط بكاشف الأيتام القائم لاحقاً. |
| REPORT3-U08-FOLLOWUP-TASK-NOT-LINKED-TO-OUTCOME-01 | التدقيق الموحَّد ٣ (U08): `field_tasks` تحمل موعداً وحالة لكن لا رابطَ بين موعد المتابعة ونتيجة الحالة؛ `actual=None` لا يميّز «لم يحن التقييم» من «حان ولم يردّ العميل»؛ وموافقةُ التعلّم المشترك غير مُنفَذة. | tasks/learning | `services/sahool-platform/api/routers/tasks.py:87` · `services/sahool-platform/storage/lite_store.py:204` | **open — قرارُ مالك:** يحتاج حقلاً/قيداً في جداول قائمة (هجرة خلف GATE-01) وسياسةَ بيانات للمشاركة — يُقرَّر مع الشريحة P2 لا هنا. |
| REPORT3-RAG-01-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): لا تمييزَ بين تضمين الوثيقة والاستعلام: `OllamaEmbeddingProvider.embed` يرسل النصّ خاماً، وseed/ingest/retrieve تستعمل الدالّة نفسها؛ Nomic يطلب `search_document:`/`search_query:`. | rag | `services/sahool-platform/core/rag/production_qdrant.py:690` · `services/qdrant-seed/seed.py:231` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-02-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): إعادةُ البذر الافتراضيّة تحافظ على الحجر (`__seed_quarantine__`)؛ إصلاحُ البادئات لا يغيّر أهليّة الرؤية. | rag | `services/qdrant-seed/seed.py:42` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-03-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): مرشّحات crop/region/field_id تستبعد بذوراً عالميّة بلا هذه الحقول (`topic` فقط) — مقيس بالمسبار. | rag | `services/sahool-platform/core/rag/production_qdrant.py` (`matches_scope_filters`) · `services/qdrant-seed/seed.py:330` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-04-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): إعادةُ البذر خارج الخدمة لا تجدّد BM25: `_ensure_sparse_index` يعيد التقرير المحفوظ عند `_sparse_ready` — مقيس بالمسبار. | rag | `services/rag-retrieval/main.py:37` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-05-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): توافقُ الأبعاد لا يثبت توافق فضاء التضمين (لا digest نموذج/بادئات/corpus revision). | rag | `services/qdrant-seed/seed.py` · `services/rag-retrieval/main.py` (readiness) | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-06-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): `local-ai-rag` مسارٌ مستقلّ (`OllamaEmbeddings` + `QdrantVectorStore`) لا يغطّيه إصلاحُ مزوّد واحد. | rag | `services/local-ai-rag/main.py:150` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-07-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): البذرُ ليس إعادة فهرسة شاملة: يُضمّن `KNOWLEDGE_BASE` فقط بمعرّفات `seed:<id>` بلا SEED_REVISION/tenant. | rag | `services/qdrant-seed/seed.py` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-08-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): درجاتُ dense لا تمثّل ترتيب الخدمة (RRF + جوار + تداخل لفظيّ + توسيع KG). | rag | `services/rag-retrieval/main.py` (`HybridQdrantRetriever`) | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-09-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): مسارُ search يستعمل header/body بلا JWT أو توكن خدمة مستقلّ. | rag | `services/rag-retrieval/main.py:206` | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-11-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): readiness يفحص البنية لا وجودَ محتوى مفيد لكلّ مستأجِر. | rag | `services/rag-retrieval/main.py` (readiness) | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
| REPORT3-RAG-12-01 | تدقيق RAG (2026-09-13، ثُبِّت على `9c4e372a` والملفّات لم تتغيّر حتّى `d786d18b`؛ المسبار الستّيّ أُعيد إنتاجه هنا): نسبةُ العطب إلى البادئات وحدها غير مثبتة؛ عائلة v1/v1.5 موصوفة English والمحتوى عربيّ يمنيّ. | rag | بطاقة نموذج Nomic · التقرير | **open — قرارُ مالك:** لا إصلاحَ بادئاتٍ ولا إعادةَ بذرٍ بلا مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد عربيّ (dense/sparse/fusion/expansion) — خطّة التقرير 1→9؛ RAG-10 عولج جزئيّاً في REPORT3-CHAT-GENERATED-GROUNDED-WITHOUT-RAG-HITS-01. |
...
- **الحالة:** fixed — دُمِج #1011 في `main` (`9117c39e`) وقسمُ «External source references» في الوثيقة الأصل يحمل الـSHA الأربعة كاملةً مع مساراتها (`docs/architecture/DECISION_FABRIC_SOURCE_COMPARISON_20260916.md:143-160`)، وFinding 6 يحمل سببَه المقيس (`:94`، `runtime.go:506 → :530`) مع تصحيحَي Go/Zenoh. يبقى القياسُ نفسه في وثيقة التحقّق؛ الوثيقةُ الأصل تُحيل إليها لا تُكرّرها.
- **المصدر (عند الاكتشاف):** [`docs/audits/DECISION_FABRIC_EXTERNAL_SOURCE_VERIFICATION_20260916.md`](../../docs/audits/DECISION_FABRIC_EXTERNAL_SOURCE_VERIFICATION_20260916.md) يُثبِّت الـSHA الأربعة (`dc40c029` · `84e00a16` · `397e57ca` · `dbf8988b`) ويقيس كلَّ دعوى بـ`path:line`: اثنتان مؤكَّدتان بالكامل (LinkMind · OM1)، وواحدة مؤكَّدة ضمن نطاقها المُعلَن (OpenAgentFlow)، وواحدة (WGAI) مؤكَّدة بحدود كلمة «monitoring». لا دعوى مكذوبة. وأُضيف ما أغفلته الوثيقة: السببُ المقيس لـFinding 6 (LLM في OM1 هو مُختارُ الفعل — `internal/runtime/runtime.go:506 → :530`).
- **تاريخيّ (كما كانت الفجوة قبل الإغلاق — يُقرأ للتأريخ لا للحالة):** كانت `open` على وثيقة PR #1011 @ `a19543ef`: قسمُ «External source references used for the comparison» يُسمّي `landingbj/LinkMind` · `OpenAgentFlow/OpenAgentFlow` · `OpenMind/OM1` · `dromara/wgai` بالاسم بلا SHA ولا تاريخ، وآخرُ التزامٍ في LinkMind يحمل تاريخَ يوم القياس نفسه — فأحكامٌ معماريّة أُسنِدت إلى هدفٍ يتحرّك يوميّاً بلا مرساة (صنف «رقمٌ يصف كوناً لم يعد قائماً» منذ اليوم الأوّل). وشرطُ الإغلاق المُعلَن حينها — نقلُ كتلة التثبيت وجملة Finding 6 من §٧ في وثيقة التحقّق إلى الوثيقة الأصل، لا وجودُ وثيقة التحقّق وحدها — **تحقّق** في #1011 (`3458b3fa` → دُمِج `9117c39e`).
- **حدُّ صدقٍ باقٍ:** المراسي مثبَّتة لكنّ سطورَها (`:143-160` · `:94`) تنزاح بأيّ تحريرٍ لاحق للوثيقة الأصل؛ الـSHA لا تنزاح. (صياغةُ هذا القسم أُعيدت بعد مراجعة Copilot على #1011 — كانت نقاطُ المصدر والشرط بصيغة الحالة المفتوحة تحت سطرِ `fixed`.)
