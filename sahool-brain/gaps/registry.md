...
| RAG-BM25-CROSS-TENANT-CORPUS-STATS-01 | `score` يقرأ `n_docs` و`doc_freq` و`avg_len` العالميّة قبل أيّ عزل، و`search` يرشّح بعده. | rag · retrieval · tenancy | [`production_qdrant.py`](../../services/sahool-platform/core/rag/production_qdrant.py) (`BM25Index.score`) | **open** — مُعادُ إنتاجه: ٠٫٤١١ ← ٠٫٠١٢ بمستنداتِ مستأجِرٍ آخر تحمل المصطلح · ← ٧٫٢٨٩ بمستنداتٍ طويلة **لا تحمله** (عبر `avg_len`). **ولا إفشاءَ محتوًى** — تأثيرٌ في الترتيب وحده، ومُربِكٌ مباشر لأيّ قياس تكافؤ. والنِّسَبُ تعتمد على العيّنة فالمُثبَت **الخاصّيّة** لا رقمٌ بعينه. أعلى أولويّةً من الجيران لأنّه يغيّر الترتيب لكلّ مستأجِر ولو كانت كلّ payload قانونيّة. |
| RAG-NEIGHBOR-FILTER-SCOPE-BYPASS-01 | `_expand_neighbors` يدخل بالمستند والترتيب بلا إعادة تطبيق `crop`/`field_id`/`region`/`source_type`. | rag · retrieval | [`production_qdrant.py`](../../services/sahool-platform/core/rag/production_qdrant.py) (`_expand_neighbors`) | **open** — مُعادُ إنتاجه: استعلامُ `field_id=F1` أعاد جاراً من `F2`. **وعزلُ المستأجِر محفوظ** (المفتاح يحمل `tenant_id`) — فهو عطلُ دقّةِ نطاقٍ لا خرقُ عزل، وتصنيفُه خرقاً كان سيرفعه فوق أولويّته. |
| RAG-STORAGE-ID-AS-LOGICAL-IDENTITY-01 | التوثيق يقول إنّ الهويّة المنطقيّة دائماً `metadata.chunk_id`، والكود يُرقّي UUID التخزين عند غيابه. | rag · identity | [`production_qdrant.py`](../../services/sahool-platform/core/rag/production_qdrant.py) (`from_payload`) | **open** — مُعادُ إنتاجه: `chunk_id = 'storage-uuid-42'`. ارتدادُ هجرةٍ عمليّ لا هويّةٌ قانونيّة: إعادةُ كتابةٍ لاحقة تُنتِج هويّتين للنقطة نفسها ما لم تُصاحِب الهجرةَ خريطةٌ صريحة. وهو صنفُ «وثيقةٌ تصف ما لا يفعله الكود» الذي أمسكَته المراجعة على #837. |
...
- **المقيس — الكودُ لا يزال يمسّها، وأكثرَ من قبل:**
  ```
  الجدول                              مراجعُه في العامل قبل ⇒ بعد
  iot_command_dispatch                        2 ⇒ 3
  marketplace_plugin_execution_runs           2 ⇒ 3
  marketplace_plugin_runtime_events           3 ⇒ 3
  model_promotion_history_runtime             1 ⇒ 4
  model_rollback_history_runtime               3 ⇒ 3
  runtime_event_outbox                        3 ⇒ 3
  ```
  ومع ذلك فقدت الستّةُ كلُّها `phase_runtime_workers.py` قارئاً وكاتباً في الجرد المولَّد.
- **والآليّةُ مقيسةٌ لا مُستنتَجة:** كان اسمُ الجدول **حرفيّاً في SQL** (`FROM iot_command_dispatch` · `UPDATE iot_command_dispatch`)، فصار **وسيطاً مُسمّى** (`reclaim_expired(conn, table="iot_command_dispatch")`) وتُبنى الجملةُ داخل المُطالِب الموحَّد. والماسحُ يكتشف بالحرفيّات، فلم يعد يرى شيئاً.
- **والأثرُ على بوّابةٍ حاجبة، لا على تقرير:** `db_writer_ownership_baseline.json` راتشِتٌ **ينزل ولا يصعد**، وقد نزل: **٩٣ ⇒ ٨٩** مخالفة، والمنسوبُ إلى `phase_runtime_workers.py` **٥ ⇒ ١**. أي أنّ الأساسَ سجّل «تحسّناً» بأربع مخالفاتٍ **هو عمًى خالص**، ولا سبيل لاستعادته لاحقاً لأنّ الراتشِت لا يصعد.
- **وأخطرُ ما فُقِد `iot_command_dispatch` بعينه** — الجدولُ الذي تدور عليه بطاقةُ M-03، والذي يخرج منه `sahool.actuator.dispatch.requested`: طلبُ إرسالٍ فيزيائيّ. فحارسُ ملكيّة الكتابة صار أعمى عن أخطر ما كان يراه.
- **وهذا صنفُ «الغيابُ يُقرَأ براءة» بعينه**، وقد وقع في شريحتي أنا: الإصلاحُ صحيح، والجردُ صار يكذب عليه.
- **لماذا `open`:** العلاجُ ليس تراجعاً عن التجريد (هو صحيح)، بل جعلُ الماسح يرى الجدولَ حين يُمرَّر وسيطاً إلى مُطالِبٍ معروف — أو تسجيلُ مواضع الوصول صراحةً. وكلاهما تغييرٌ في مُنتِج الجرد وشريحةٌ مستقلّة، ويلزمها **استعادةُ الأساس إلى ٩٣** بعد استعادة الرؤية.
- **حدُّ صدق:** المقيسُ عددُ المراجع النصّيّة في الملفّ وفرقُ الجرد المولَّد بين `90df6145` و`99000487`. لم يُشغَّل الماسحُ يدويّاً على صيغةٍ مُصطنَعة لعزل السبب، والآليّةُ مُستنتَجةٌ من شكل الكود قبل وبعد — وهي قرينةٌ قويّةٌ لا برهانٌ معزول.
