...
|---|---|---|
| SUP-01..07، SUP-09..10 | fixed in source + regression tests | JWT/tenant/NATS receipts، ERP transaction/cursor، bot persisted work-order، Edge durable envelope، Compose env، backup cleanup |
| SUP-11..15 | source repaired; live verification remains open | PITR preparation، Helm/PVCs، OTLP/exporters، native mobile/FCM v1، image requirements audit؛ RPO/RTO وhandset وcluster لم تُقَس |
| SUP-16 | fixed documentation | ADR-0033 status and unique ADR-0036/0037 identifiers |
| SUP-08 | **open — GATE-01 CLOSED** | Exact review-only patch and source hashes under `docs/audits/patches/`; no frozen source changed; independent adjudication and device/receipt work required |
