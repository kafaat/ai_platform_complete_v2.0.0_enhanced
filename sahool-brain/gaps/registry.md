...
**والحزمة العاشرة أنزلت نصف العلاج صحيحاً:** `requirements-preflight.txt` يُعلِن
`pytest>=8.0` و`jsonschema>=4.20`. وهو العلاج المطابق للتصنيف المُصحَّح — تبعيّة ناقصة
تُعالَج بإعلان تبعيّة، لا بكتابة مُتحقِّق ولا بتليين اختبار. يبقى النصف الآخر: **وصل
