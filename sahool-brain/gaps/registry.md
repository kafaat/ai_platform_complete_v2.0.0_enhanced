...
| NATS-BROKER-HAS-NO-AUTHENTICATION-SO-ACTUATOR-COMMANDS-ARE-UNGUARDED-01 | وسيطُ NATS في مكدّس v9 بلا مصادقةٍ البتّة، فأيُّ حاويةٍ على الشبكة الداخليّة تنشر إلى `actuator.command` بلا اعتماد — وحدُّ الأثر الفيزيائيّ يقوم على تقسيم الشبكة وحدَه. | security · runtime · physical-effect | [`nats/nats.conf`](../../nats/nats.conf) · [`docker-compose.v9.yml`](../../docker-compose.v9.yml) (`sahool-nats:320-342`) | **open** — **P0-adjacent (أثرٌ فيزيائيّ).** قِيس **حيّاً** في حزمة المالك `evidence/live-20260901T125135Z/B1_nats_bypass.txt`: `nats_publish=succeeded_unauthenticated` — نشرٌ ناجحٌ إلى `actuator.command` من حاويةٍ بلا وكيلٍ ولا اعتماد. **وأُكِّد ساكناً في الشجرة:** `nats/nats.conf` يحمل `http: 8222` وكتلةَ `jetstream` **فقط** — لا `authorization` ولا `accounts` ولا `users` ولا رمزٌ ولا nkey؛ و`command` في compose يُمرّر هذا الملفَّ وحدَه. **والتعليقُ في رأس الملفّ هو الشاهدُ على كيف بقي صامتاً:** «منفذ العميل 4222 افتراضيّ في nats-server ولا يحتاج تصريحاً هنا» — صادقٌ عن **المنفذ**، وصامتٌ عن **المصادقة**؛ والملفُّ غرضُه المُعلَن حدودُ التخزين، فلم تدخل المصادقةُ نطاقَه قطّ ولا يُوفّرها شيءٌ آخر. **والتخفيفُ يُقال بلا تهوين:** الشبكةُ `sahool-internal` غيرُ منشورةٍ على المضيف، فيلزم الوجودُ عليها. لكنّ **كلَّ** خدمةٍ في المكدّس عليها — بما فيها الأقلُّ امتيازاً — فالحدُّ ليس امتيازاً بل موضعاً. **والعلاجُ ليس شيفرةً بل تزويداً:** كتلةُ `authorization` بمستخدمين مُقيَّدي المواضيع (`publish`/`subscribe` allow-lists) بحيث لا ينشر إلى `actuator.command` إلّا المُصرَّح، ثمّ حقنُ الاعتمادات في `NATS_URL` لكلّ خدمة (~٧ مواضع). **قرارُ تزويدٍ يملكه المالك، فلم يُنفَّذ هنا ولا يُدَّعى.** |
