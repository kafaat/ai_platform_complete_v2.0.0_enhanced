...
- **الحالة:** fixed — شرطُ الإغلاق تحقّق على فرع #1011 (`audit/decision-fabric-source-comparison-20260916`) بعد دمج #1012 في `main` (`0f240f53`): قسمُ «External source references» في الوثيقة الأصل صار يحمل الـSHA الأربعة كاملةً مع مساراتها (`DECISION_FABRIC_SOURCE_COMPARISON_20260916.md:143-160`)، وFinding 6 يحمل سببَه المقيس (`:94`، `runtime.go:506 → :530`) مع تصحيحَي Go/Zenoh. يبقى القياسُ نفسه في وثيقة التحقّق؛ الوثيقةُ الأصل تُحيل إليها لا تُكرّرها. **حدُّ صدق:** الإغلاقُ مقيسٌ على الفرع قبل دمج #1011؛ إن أُعيدت صياغةُ القسم قبل الدمج بلا SHA عادت الفجوة.
- **الحالة السابقة (للتأريخ):** open — الفجوة في وثيقة PR #1011 (`docs/architecture/DECISION_FABRIC_SOURCE_COMPARISON_20260916.md` @ `a19543ef`) التي لم تُدمَج بعد؛ القياسُ البديل مسجَّل في الشجرة.
