...
| ABSENT-READING-COERCED-TO-ZERO-READS-AS-A-MEASUREMENT-01 | قراءاتٌ غائبة من المزوّد تُصفَّر بـ`or 0` — فالمطرُ المفقود يُقرأ «جافّ» والريحُ المفقودة «ساكنة». وهي أخطرُ صور الكذب الصامت: صفرٌ مُختلَقٌ يدخل قرارَ رشٍّ بوصفه قياساً. | weather · data-quality | [`open_meteo.py`](../../services/weather-service/open_meteo.py) (`:218` · `:231` · `:326` · `:427` · `:431-436` مقابل `:441`) | **open** — مقيسٌ بالسطر: `wind_kmh = float(current.get("wind_speed_10m") or 0.0)` · `"precipitation_mm": current.get("precipitation") or 0` · و`_at(..., idx, 0)` للحرارة والرطوبة والمطر والغيوم في المسار الساعيّ. **والشاهدُ الأقوى داخل الملفّ نفسه:** السطر `:441` يُبقي الهبّةَ `None` صراحةً إن غابت (`... if gust is not None else None`) — فالملفُّ **يعرف** كيف يكون صادقاً في موضعٍ ولا يفعل في أخواته. فليست جهلاً بالنمط بل تفاوتاً فيه. **وH1 (#931) أغلق نصفَ الحرارة بقرارٍ مُعلَن**، وبقيت البقيّة. **ولا حارسَ للصنف:** `test_missing_temperature_is_not_zero.py` يحرس الحرارةَ وحدَها، وتوسيعُه إلى بقيّة القراءات هو العلاجُ المُقترَح. |
| A-SILENT-CLAMP-HIDES-AN-OUT-OF-RANGE-INPUT-01 | `rh = max(0.0, min(100.0, rh_pct))` يقصّ الرطوبةَ النسبيّة بلا أثر: مُدخَلٌ خارج المدى (خطأُ وحدةٍ أو حسّاسٌ معطوب) يُقصّ إلى حدٍّ صالحٍ ويدخل حسابَ ET0 كأنّه قياس. | weather · data-quality | [`vapor_pressure.py`](../../services/weather-service/vapor_pressure.py) (`:42`) | **open** — خطورةٌ منخفضةٌ لكنّ الصنفَ هو الصنف: القصُّ يُنتِج رقماً **معقولاً** من مُدخَلٍ غيرِ معقول، فيختفي الدليلُ على أنّ شيئاً كان خطأً. و`120%` تصير `100%` فتُحسَب ET0 على رطوبةٍ لم تُقَس. **ولا حدَّ مُعلَناً في العقد** يقول إنّ المدخلَ يُقصّ — فالمستهلكُ لا يعلم. **والعلاجُ المُقترَح:** إمّا رفضُ الخارج عن المدى (فاشلاً مغلقاً كأخواته)، وإمّا إعلانُ القصّ في مخرَجٍ يقرؤه المستهلك — لا القصُّ الصامت. |
| READINESS-PROBES-THE-UPSTREAM-ON-EVERY-CALL-01 | `readyz` يضرب المزوّدَ الخارجيَّ حيّاً في كلّ نداء — فجاهزيّةُ الخدمة تصير دالّةً في صحّة طرفٍ ثالث، والمُنسِّقُ الذي يستطلعها كثيراً يُنتِج حِملاً على المزوّد ويعرّض الخدمةَ لإعادة تشغيلٍ بسببٍ خارجها. | weather · ops · resilience | [`weather_runtime.py`](../../services/weather-service/weather_runtime.py) (`readyz` `:75-87`) | **open** — مقيسٌ بالمصدر: `upstream = await _facade_attr("readiness_probe")()` ثمّ `status = "ready" if upstream.get("ok") else "degraded"`. **وحدُّ صدقٍ يُصغِّرها:** الحالةُ `degraded` لا `not-ready`، فالخدمةُ لا تُعلن سقوطَها لعطلٍ خارجيّ — وهو تصميمٌ سليم. **وما يبقى عطلاً** أنّ كلَّ استطلاعِ جاهزيّةٍ نداءٌ شبكيٌّ خارجيّ: ثلاثةُ أسئلةٍ مختلفة (أأقلعت؟ · أتتحرّك الحلقة؟ · أطازجٌ البيان؟) يُجاب عنها بنداءٍ واحدٍ للطرف الثالث. وهو أخو `WEATHER-IDLE-WORKER-READS-AS-DEAD-01` المُسجَّل. **والعلاجُ المُقترَح:** يقرأ `readyz` آخرَ نتيجةِ استطلاعٍ مخبّأةً بعمرٍ مُعلَن بدل الضرب الحيّ. |
...
| IRRIGATION-READS-MISSING-RAIN-AS-NO-RAIN-01 | `rain_recent_mm` المفقودُ يُقسَر إلى صفر، و«لا مطر» **يرفع** كمّيّةَ الريّ الموصى بها — فالغيابُ ينحاز إلى الإسراف لا إلى الحفظ. | weather · platform · agronomy | [`field_workspace_weather.py`](../../services/sahool-platform/api/routers/field_workspace_weather.py) (مسارُ `irrigation-advice`) · [`weather_advice.py`](../../services/sahool-platform/api/weather_advice.py) (`irrigation_advice:70`) | **open** — **سلوكٌ محفوظٌ عمداً عبر نقل الواجهة في P6، لا اختيارٌ جديد:** `irrigation_advice` يُعلن `rain_recent_mm: float` فلا يقبل `None`، والقسرُ كان قائماً في الموصّل قبل النقل. **ولمَ لم يُصلَح في الشريحة نفسِها:** البدائلُ الثلاثةُ كلُّها قرارُ منتَج لا أثرٌ جانبيّ — ٥٠٣ يُحوِّل ردّاً قائماً إلى فشل · أو توسيعُ عقد `irrigation_advice` ليقبل الغياب ويُخرِج «كمّيّةً بلا خصمِ مطر» مُعلَنةً · أو الإبقاء. **وخدمةُ الطقس تُصرِّح بالناقص أصلاً** (`missing_fields` · `quality_status`)، فالمعلومةُ تصل المسارَ ويُلقيها هو — والفجوةُ في الاستهلاك لا في المصدر. مُوثَّقٌ في موضعه بالكود بهذا المعرّف. |
...
| TEMPLATE-INJECTION-IN-A-JOB-THAT-CAN-SIGN-01 | `${{ … }}` داخل `run:` يُصيَّر **نصَّ السكربت** قبل أن يبدأ bash، فمُدخَلُ إطلاقٍ حرُّ النصّ يصير شيفرة — وفي وظيفةٍ تحمل `attestations: write` يُصدِر الحقنُ **شهادةَ منشأٍ موقّعة**. | security · ci · supply-chain | [`runtime-verification-promotion.yml`](../../.github/workflows/runtime-verification-promotion.yml) · [`runtime-image-provenance.yml`](../../.github/workflows/runtime-image-provenance.yml) · [`test_privileged_workflow_injection_guard.py`](../../tests_v9/test_privileged_workflow_injection_guard.py) | **fixed** — **مقيسٌ بجارِدٍ على الشجرة لا بمسحٍ نمطيّ: ستّةُ مواضع** (٤ في مسار الترقية · ٢ في مسار الصور)، كلُّها `run` داخل وظيفةٍ ذاتِ صلاحية كتابة. **وأخطرُها في `approval-receipt`** — الوظيفةِ المحميّة بـ`environment: runtime-verification-approval` والحاملةِ `attestations:write` و`id-token:write`: هي التي تُصدِر شهادةَ الموافقة التي يقوم عليها رفعُ `runtime_verified`. **والبيئةُ المحميّة تحرس مَن يوافق لا ما يُنفَّذ بعد الموافقة** — الحمولةُ تعمل **بعد** إجازة المُراجِع وبصلاحياتها، والمُراجِعُ رأى `target_sha` وسيطاً لا شيفرة. فالحقنُ هنا **نقضُ فصلِ السلطة نفسِه** لا مجرّد تنفيذِ أمرٍ في عدّاء. **وفحصُ الشكل كان داخل ما يحرسه:** `[[ … =~ ^[0-9a-f]{40}$ ]]` مكتوبٌ في السكربت الذي حُقِن أصلاً، فلا يفحص شيئاً؛ وفي `runtime-image-provenance.yml` كان يقع **بعد** الاستعمال أيضاً. والعلاجُ: تمريرٌ عبر `env` (بيانات لا شيفرة) + فحصُ الشكل **قبل** الاستعمال + نقلُ `packages`/`id-token`/`attestations` من مستوى الملفّ إلى الوظيفة الموقِّعة وحدَها (كان `publish-manifest` يرثها بلا مستهلك). **والنطاقُ مُشتقٌّ لا قائمةٌ مكتوبة:** «كلُّ وظيفةٍ تملك صلاحيةَ كتابة»، وقراءةُ الوراثة من مستوى الملفّ جزءٌ من الاشتقاق. مُكذَّبٌ بطفرتين مُسجَّلتين، كلٌّ قتلت اختبارَها المُسمّى وحدَه. **وحدُّ صدقٍ لازم:** يُقاس **الشكل** — قالبٌ في متن وظيفةٍ ذاتِ صلاحية. لا يُقاس أنّ القيمةَ المُمرَّرة عبر `env` مُتحقَّقٌ منها، ولا يبلغ `uses:` ولا `workflow_call`. **تصحيحٌ لازم بعد وصول أدلّة المختبر الخام (`zizmor.json`):** قلتُ إنّ أرقامَ الماسحات «منقولةٌ لا مقيسة» لتعذُّر تشغيلها هنا، **ثمّ وصل الدليلُ فقلبَ نصفَ الحكم**. zizmor **رأى هذا بالضبط**: ثمانيةُ `template-injection` بدرجة **High/High** على `runtime-verification-promotion.yml` — **أكبرُ تجمّعٍ في المستودع كلِّه** — وثلاثةُ `excessive-permissions` High/High على `runtime-image-provenance.yml` تسمّي الأسطر ١١–١٣ حرفيّاً. **فالعطلُ لم يكن في الكشف بل في الترتيب:** رفع التقريرُ «حقنَين مؤكَّدين» من poutine على مُدخَلَين **مقيَّدَي النوع** فوق ثمانيةِ High/High على المسار الذي يوقّع شهادةَ الموافقة. واشتقاقي **مطابقةٌ لا اكتشاف**، والفضلُ للأداة في الرصد ولي في الترتيب. **ووسّع الدليلُ الخارجيُّ نطاقي أيضاً:** حدُّ «وظيفةٍ ذاتِ صلاحيةِ كتابة» ترك ثلاثةَ مواضع في `verify-candidate` — **بلا صلاحيةِ كتابة، لكنّه يُنتِج المصنوعةَ التي يوقّعها المُمتاز**. فصار النطاقُ حدَّ الثقة في الملفّ، و**٦ ⇒ ١٣ موضعاً** أُصلِحت جميعاً (٧ في مسار الترقية · ٣ في `path3` · ٢ في الصور · ١ في `ci.yml`). **وطولٌ ليس شكلاً:** `test "${#TARGET_SHA}" = 40` كان يقبل أربعين محرفاً أيّاً كانت، و`path3_run_id` يُحقَن بلا فحصٍ أصلاً. |
| A-RUNBOOK-THAT-DESCRIBES-UNBUILT-CAPABILITY-01 | رَكْضةُ تشغيلٍ تصف بصيغة الحاضر قدراتٍ **غيرَ مبنيّة**، فيتبعها مشغّلٌ ويرى أخضرَ ويسجّل أنّ ما لم يقع قد وقع. | governance · supply-chain · ci | [`runtime-image-provenance.yml`](../../.github/workflows/runtime-image-provenance.yml) · [`path3-runtime-verification.yml`](../../.github/workflows/path3-runtime-verification.yml) · [`path3_image_evidence_guard.py`](../../scripts/ci/path3_image_evidence_guard.py) · [`test_image_supply_chain_contract.py`](../../tests_v9/test_image_supply_chain_contract.py) | **fixed** — **مقيسٌ لا مُستنتَج:** قالت الرَّكْضة «The workflow **now**: يبني بـ`provenance=mode=max` وSBOM · يُثبّت Trivy ببصمة · يفشل على HIGH/CRITICAL · يُصدِر مُسنَدين ويتحقّق منهما · ويربط أربعَ بصماتِ أدلّة»؛ والمقيسُ على `main` يومَها: **`provenance: false`** · **صفرُ ذكرٍ لـTrivy** · لا SBOM · لا تحقّق · لا بصمات. **والخطرُ ليس هفوةً توثيقيّة:** الطمأنينةُ الكاذبةُ تُصنَع داخل سلسلة التوريد التي وُجِدت الوثيقةُ لحمايتها — `ATTESTED-IS-NOT-CERTIFIED-01` مقلوباً. **نُفِّذت §٥ و§٦ بأمرٍ صريحٍ من المالك:** `mode=max` + `sbom` · Trivy 0.74.0 مثبَّتٌ ببصمةٍ **قِستُها بنفسي** (`2ae6fe3e…`) · تحميلُ قاعدةٍ **يفشل** عند تعذّرها (فقاعدةٌ غائبةٌ ليست صفرَ ثغرات) · فحصٌ حاجبٌ **بالبصمة لا بالوسم** · جردُ CycloneDX · مُسنَدا منشأٍ وجرد · **تحقّقٌ من النوعين صراحةً** · وأربعُ بصماتٍ مربوطةٌ **داخل** البيان يرفض المُجمِّعُ نقصَها. وPATH-3 يرفض البيانَ الأعمى بحارسٍ مستقلّ ويُعيد التحقّقَ من النوعين — **فمُنتِجٌ يشهد لنفسه ليس شاهداً**. **ومُكذَّبٌ بعشر طفراتٍ مُسجَّلة**، كلٌّ قتلت اختبارَها المُسمّى وحدَه. **وطفرةٌ نجت فكشفت ثقباً في اختباري**: جدولي حمل `evidence: {}` وحدَها، و`{}` **قاموسٌ** فلا يبلغه فرعُ «ليست خريطة» — فصفٌّ بلا الكتلة رأساً كان يمرّ. أُغلِق. **وحدُّ صدقٍ لازم:** المفروضُ **العقدُ في الـYAML** — أنّ الخطوة معلنةٌ بمعاملاتها. **ولا يُقاس أنّ التشغيل الحقيقيّ نجح**: ذلك يحتاج عدّاءً موثوقاً وسجلَّ حاويات، وهما خارج ما يفرضه ملفٌّ في الشجرة. الادّعاء: «الخطوةُ موجودةٌ ومُعامَلةٌ كما وُصِفت» لا «الصورةُ فُحِصت». |
