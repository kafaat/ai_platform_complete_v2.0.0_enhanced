...
| PG-APP-ROLE-TRANSITIVE-PRIVILEGE-CLOSURE-01 | حارس أدلّة PG الحيّ كان يُثبِت **خصائص الصفّ المباشرة** على `sahool_app` وحدها؛ و`pg_auth_members` يمنح صلاحياتٍ موروثة أو `SET ROLE` عبر سلاسل عضويّة لا تظهر في تلك الخصائص — فحدُّ الصلاحية المقيس كان أضيق ممّا يُنفَّذ فعلاً | ci/live-pg · security | [`scripts/ci/live_pg_role_closure_guard.py`](../../scripts/ci/live_pg_role_closure_guard.py) · [`tests_v9/test_live_pg_role_closure_guard.py`](../../tests_v9/test_live_pg_role_closure_guard.py) · `.github/workflows/ci.yml` (وظيفة `live-pg-fake-connection-proofs`، خطوة *Prove the evidence role has no direct or transitive role memberships*) | **fixed (TOOL_PROVEN / LIVE_RUN_PENDING)** — استعلامٌ تكراريّ على `pg_auth_members` يشترط أن يكون الإغلاق العبوريّ **فارغاً** لدور الأدلّة المخصَّص، ويحفظ `ADMIN`/`INHERIT`/`SET` لكلّ منحة. أشدّ عمداً من نموذج تفويضٍ إنتاجيّ: عضويّةٌ خاملة اليوم تصير نافذةً بتغيير خيارٍ لاحقاً بلا مسّ خصائص الدور. فشلٌ مغلق على غياب الدور/`psql`/جسمٍ مشوَّه، والمصنوعة المرفوعة تحمل **أسباباً ثابتة** لا تشخيصات libpq (مضيف/منفذ/كلمة مرور). ٤/٤ طفرات مُثبَتة بالتكذيب · ٩ اختبارات وحدة. **حدّ الصدق:** لم يُشغَّل بعدُ على قاعدةٍ حيّة في CI — الخضرة تنتظر أوّل تشغيل للوظيفة. |
