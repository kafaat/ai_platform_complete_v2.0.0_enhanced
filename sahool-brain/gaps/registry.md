...
| PIP-AUDIT-GATES-19-OF-36-REQUIREMENTS-01 | **١٧ ملفّ متطلّبات خارج `pip-audit` الحاجب**، وتشغيلُه عليها يقيس ٤ حمراء: decision-service (أُصلِح) · `bots/telegram` (aiohttp 3.13.5 · ٢٨ ثغرة، انظر الصفّ التالي) · `requirements-dev.txt` (nltk 3.10.3 عبر `safety 3.8.1` — PYSEC-2026-3740 بلا إصدارٍ مُصلِح) · `requirements-irr-f01-test.txt` (pytest 8.4.2 — أُصلِح). المسحُ الإرشاديّ في CI يراها ويبتلعها بـ`\|\| true`. | dependencies · ci | [`ci.yml` (pip-audit gating)](../../.github/workflows/ci.yml) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٧ | **open** — توسيعُ القائمة الحاجبة بالـ١٣ النظيفة قرارُ مالك (يمسّ `ci.yml`؛ يُحمِّر عند أوّل ثغرةٍ جديدة فيها وهذا مقصودُه). |
| TELEGRAM-BOT-AIOHTTP-VULNERABLE-UNDER-AIOGRAM-PIN-01 | `aiogram==3.28.2` يجرّ aiohttp 3.13.5 بـ٢٨ ثغرة (PYSEC-2026-237/2112/2113/3545/3546/3547…) و**يرفض** aiohttp ≥3.14.x (`pip` ResolutionImpossible). aiogram 3.31.0 يقبلها — لكنّ البوت بلا اختبارٍ يستورد aiogram (`tests_v9/test_telegram*.py` لا تلمسه) فترقيتُه بلا شاهد. | bots · dependencies | [`bots/telegram/requirements.txt`](../../bots/telegram/requirements.txt) | **open** — يلزم شاهدٌ لسطح aiogram المُستخدَم ثمّ الترقية إلى 3.31.0. |
| UNWIRED-IRRIGATION-REAL-WORKFLOW-FUNCTIONS-01 | `workflow_definitions.py` تحمل **١٣ دالّةَ ريّ «حقيقيّ»** (`_irrigation_real_validate/schedule/execute/execute_compensate/verify/approval_gate/suspend_until_approved` …) لا يستدعيها أحد — سيرُ عمل الريّ الحقيقيّ مكتوبٌ **وغيرُ موصول**. | irrigation · platform | [`workflow_definitions.py:182-477`](../../services/sahool-platform/api/workflow_definitions.py) · `execution-audit/generated/EXECUTION_DEPENDENCY_AUDIT_REPORT.md` | **open** — وصلٌ أو حذفٌ لكلّ دالّة قرارُ مالك (يمسّ مسارَ أثرٍ فيزيائيّ). |
...
| AGENT-MEMORY-PARALLEL-BRAIN-STALE-01 | `agent-memory/` (`FACTS.md` · `CORRECTIONS.md` · `JOURNAL.jsonl` ١٢٤ سطراً) ذاكرةٌ موازية لـ`sahool-brain/`، آخر تحقّقٍ مُعلَن 2026-06-18، بلا حارسٍ ولا رابطٍ من `index.md` — ذاكرتان تفترقان بصمت. | brain · governance | [`agent-memory/FACTS.md`](../../agent-memory/FACTS.md) · [`DEEP_STRUCTURAL_REVIEW_20260913.md`](../../docs/audits/DEEP_STRUCTURAL_REVIEW_20260913.md) §٨ | **open** — دمجٌ في `sahool-brain/` أو أرشفةٌ قرارُ مالك. |
