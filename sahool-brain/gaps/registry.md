...
## IMAGERY-LATEST-UNBOUND-TO-A-SCENE-DAY-01 — FIXED_IN_CODE (P1) 2026-08-16

- **العلّة:** «latest» في الخريطة الحيّة لم تكن تعني الأحدث. `raster_cdse_tile_runtime.py:101` يبني نافذة `LATEST_WINDOW_DAYS` (٣٦٥ افتراضاً) ويُسلّمها إلى `process_index`، وهو يُرسِل `mosaickingOrder=leastCC` (`cdse_client.py:550`) — فالمعروض **أقلّ المشاهد غيوماً في سنة**، ويُخزَّن ساعةً تحت مفتاح كاش مبنيّ على `today` (تاريخ الطلب، `:153`) بلا شاهد على تاريخ البكسلات.
...
- **المصدر:** قياس مباشر على `main` عند `21179b7f`.

## RASTER-SERVICE-TESTS-UNWIRED-TO-CI-01 — OPEN (P2) 2026-08-16
