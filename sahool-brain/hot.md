...
> **آخر تحديث:** 2026-07-04 · `main` = `develop` = `claude/code-review-34hO3` = `259acbe`. [`log.md`](log.md) مدخل (ن-19، ن-18، ن-17). دفعتا وكلاء (٦ شقوق) + جسر الحوكمة (objective⇒dispatch dry-run) + سجلّ تغطية typed (18 طبقة) — العقد **88 endpoint** (كان 24 صباح اليوم)، vitest **731**. +التقويم اليمنيّ (منزلة/حميريّ/أمثال، display_only) و«ماذا أزرع؟» (دورة+ملاءمة شهر) — العقد 47 endpoint.
