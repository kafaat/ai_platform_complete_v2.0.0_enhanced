...
>
> **دورة 2026-06-27 (ج):** إصلاح خلل كموني عالي الخطورة (#504 `c3e40a8`) — كتابات phase9-12 كانت
> تضبط GUC المستأجِر عبر `set_config(...,is_local=true)` ثمّ تنفّذ INSERT/UPDATE خارج معاملة؛
> تحت asyncpg autocommit يُعاد ضبط GUC قبل الكتابة ⇒ FORCE-RLS يرفض تحت `sahool_app` (NOBYPASSRLS).
> مُقنَّع في CI لأنّ Integration Tests تتّصل بدور superuser. الإصلاح: مُجمِّع `_tenant_conn` (acquire
> داخل `conn.transaction()`) + لفّ كتل workers بـtransaction (SKIP LOCKED فعّال). حارس ساكن جديد.
>
