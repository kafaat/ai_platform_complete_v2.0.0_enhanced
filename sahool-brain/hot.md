...
> **آخر تحديث:** 2026-07-04 · `main` = `develop` = `claude/code-review-34hO3` = رأس ن-21. [`log.md`](log.md) مدخل (ن-21، ن-20، ن-19). ٣ دفعات وكلاء + جسر الحوكمة + كونسول الموافقات (+نقطة GET /approvals/pending الخلفيّة) — السجلّ **صفر partial** (covered:16)، العقد **102 endpoint** (كان 24 صباح اليوم)، vitest **758**. +التقويم اليمنيّ (منزلة/حميريّ/أمثال، display_only) و«ماذا أزرع؟» (دورة+ملاءمة شهر) — العقد 47 endpoint.
