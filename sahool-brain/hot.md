...
> **آخر تحديث:** 2026-07-02 · رأس `main` = الفرع المخصّص `claude/code-review-34hO3` = `d1ae020` · [`log.md`](log.md) مدخل (ن).
> 🔒 **دفعة أمان SEC-1/2/3 (بعد مراجعة أرشيف المستخدم — كلّ الادّعاءات مُتحقَّقة بالكود):** compose (fixed.yml dev-env يقتل footgun prod+bypass · light.yml خدمات→127.0.0.1 + حارسان) · Dockerfile non-root (6 + حارس) · **هويّة البوّابة الموثوقة** (X-Tenant-Id مصدر الحقيقة · body-override→403 · KG writes service-token · **تصحيح:** الموافقات تطلب tenant موثّق لا service-token كي لا تُكسَر عبر البوّابة). متابعة SEC-3.1: user/role للموافقات (يحتاج nginx يحقن X-User-Id).
