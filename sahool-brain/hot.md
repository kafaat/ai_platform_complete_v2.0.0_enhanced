...
> **آخر تحديث:** 2026-06-29 · رأس الفرع المخصّص `claude/code-review-34hO3`: [`ddd2434`](../) (CI قيد التشغيل) · [`log.md`](log.md) مدخل (ف).
> 🧩 **توحيد + تفكيك + تحصين RS256:** `main` superset؛ نماذج Pydantic→`api_models.py` وسياق زراعيّ→
> `api/field_context.py` (main.py 3282→2523). **تحصين JWT RS256:** المنصّة + ٨ خدمات صارت ترفض HS256 في
> الإنتاج (fail-closed، مهرب `SAHOOL_ALLOW_HS256_IN_PROD=1`)؛ vegetation أُضيف لها مسار RS256 الغائب.
> 0 PR مفتوح · 0 تعارض · `pytest -m unit` 1971 ✓.
