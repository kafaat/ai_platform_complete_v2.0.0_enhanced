...
- **قرار فرعيّ (least-grant↔scan):** claim/complete = SECURITY DEFINER يملكهما resolver (دورٌ يتجاوز عزلَ الصفوف) ⇒ sahool_ingest يبقى بلا UPDATE. + trigger BEFORE UPDATE يصون immutability الخامّ (يُسمح projection_* فقط).
...
- **القرار ⑩ («لكل القرار»، فوق `02727d2a`):** تاريخُ الفرع تحت PR #985 **لا يُعاد كتابتُه** لإرضاء `brain_append_only_guard`؛ التعارضُ بين حارس التكرار وحارس الإلحاق يُسجَّل فجوةً ويُحلّ في شريحةٍ على سطح الحجب لا في هذا الـPR.
- **السبب:** الحارسُ يقيس أزواجَ (التزام، والد) فلا يُرضيه إلّا تعديلُ التاريخ، والتاريخُ هنا دليلُ مراجعةٍ يستشهد به المالك وCopilot بمراجع SHA وblob؛ ومحوُه يمحو الشهادة. والوظيفةُ غيرُ مطلوبة في الـRuleset فلا تحجب الدمج. تخفيفُ الحارس أو حشوُ السجلّ مرفوضان بنصّ المالك.
- **القرار ⑪:** مرشّحُ المالك `request_dates.py` يُدمَج في الـPR نفسه لا في شريحةٍ لاحقة.
- **السبب:** دورةُ التوليد والختم تُدفَع مرّةً واحدة لإصلاح Copilot على كلّ حال، فضمُّه لا يكلّف دورةً إضافيّة؛ ويرفع هامشَ `main.py` تحت السقف من ٣ إلى ٢٩ سطراً. شرطُه أن يمرّ Ruff الذي لم يُقَس عند مؤلّفه — قِيس وأُصلِح.
- **القرار ⑫:** عيّنةُ `now` تُعلِن وقتاً واحداً (`resolved == time`) وتُسمّي صفَّ الساعة للحقول الساعيّة فقط بدل أن تُخفي أحدَ المصدرين.
- **السبب:** مراجعةُ Copilot أصابت: واجهةٌ أُضيفت لتقول الحقيقةَ عن زمن العيّنة كانت تقول حقيقتين. والخلطُ مشروع (ET0/VPD لا يأتيان من `current`) فيُعلَن باسمه لا يُلغى.
- **المصدر:** `services/sahool-platform/api/connectors/openmeteo.py` · `api/request_dates.py` · `tests_v9/test_weather_model_identity.py` · `sahool-brain/gaps/registry.md` (الصفّ الجديد) · فرع `claude/claude-md-docs-p6qqir`.
- **تصحيحُ القرار ⑪ (بيد المالك، `bcdd969a`):** مرشّحُ `request_dates.py` **رُدّ** — راتشِت ميزانية وحدات المنصّة (680) يرفض وحدةً جديدة، والمالكُ رفض رفعَ الأساس أو تسويغَ الملفّ. المحلّلان عادا إلى `main.py` بجسمهما، والاختباراتُ تقرأ التعريفَ من AST.
- **السبب:** سقفُ الأسطر وميزانيةُ الوحدات راتشِتان مستقلّان؛ إغلاقُ أحدهما بفتح الآخر ليس تفكيكاً. والتفكيكُ الحقيقيّ لـ`main.py` يذهب إلى خدمةٍ مالكة لا إلى وحدةٍ جديدة في المنصّة — قرارُ نطاقٍ لا يُتَّخذ في PR إصلاح.
- **القرار ⑬:** فجوةُ تغطية preflight (`PREFLIGHT-BLIND-TO-PLATFORM-SERVICE-TESTS-01`) تُسجَّل ولا تُنفَّذ في هذا الـPR — سطحُ preflight له عقدُه (`preflight_required.json`) واختبارُه، ويستحقّ شريحته كما استحقّها حارسُ التكرار.

## 2026-09-07 — استكمال شريحة APPEND-ONLY على فرع PR #987

- **القرار ①:** الاستثناءُ الضيّق لـ`brain_append_only_guard.py` يبقى **قبل** قاعدة `JOURNAL_SHRANK` نفسها، وتُوسَّع مواصفةُ طفراته بدل تبديل تصميمه أو إدخال محلّلٍ ثانٍ.
- **السبب:** تعليق المالك على PR #987 سمّى عطلاً مقيساً في المرساة المسجَّلة `[1]` وطلب الحفاظ على الطفرات الأربع القائمة ومعانيها. إبقاءُ الاستثناء خارج قاعدة الحجب يُعيد المرساةَ الحرفيّة ويجعل العلاجَ المقاس هو نفسه المرفوع، بينما استعمالُ حارس التكرار نفسه (`GLOBAL_ROW_UNIQUENESS_TARGETS` · `ROW_FULL_ID_RE` · `_strip_fenced_blocks` · `global_duplicate_row_identities`) يمنع انجراف قائمة نطاق أو نحوِ هويةٍ ثانٍ. والطفراتُ السبعُ الجديدة تقيس ما لم يكن مسجَّلاً من قبل: النطاق · بايتات الصفّ الكاملة · البادئة المحتفَظة · الكتل المسوّرة · حدّ السطر · تفرُّد الابن · ومسار القبول.
- **القرار ②:** الصفّان بالهويّة نفسها وببايتاتٍ مختلفة لا يُحسَمان آليّاً؛ الحلُّ المقبول يبقى سجلاً تدقيقيّاً نامياً يحفظ الأصلين.
- **السبب:** اختيارُ نسخةٍ واحدة عند اختلاف الدليل/الحالة يمحو شهادةَ جلسةٍ أُخرى، وهو بالضبط ما منعه تعليق المالك صراحةً. الشاهد `test_lossless_row_deduplication_conflicts_require_preserved_audit_evidence` يثبت أنّ النموَّ مع سجلّ تدقيقٍ صريح يُرضي الحارسين معاً بلا حشوٍ وبلا انتقاءٍ صامت.
- **المصدر:** `scripts/ci/brain_append_only_guard.py` · `tests_v9/test_brain_append_only_guard.py` · `docs/architecture/guard_mutation_registry.json` · تعليق PR #987 (`5563285194`) · فرع `fix/brain-row-deduplication-20260907` فوق `b54bf578`.

## 2026-09-06 — preflight يسأل جناحَ المنصّة، ولا يكذب حين لا يستطيع

- **القرار ①:** راتشِتُ ميزانية وحدات المنصّة يعمل في الطبقة **السريعة** لا الافتراضيّة.
- **السبب:** يعدّ ملفّاتٍ على القرص بلا استيراد (٠٫٦ ث) وحتميٌّ في كلّ بيئة؛ وهو الحارسُ الذي أسقط #985 بعد أخضرٍ محلّيّ — صنفُ ٢د/٢و بعينه: حارسٌ رخيص محبوسٌ في جناحٍ لا يُسأل.
- **القرار ②:** الجناحُ كاملاً يعمل في الطبقة الافتراضيّة **بشرط** تطابق إصدار FastAPI مع المثبَّت في `api/requirements.txt`، وإلّا يُتخطّى بصوتٍ عالٍ.
- **السبب:** مقيس: FastAPI 0.141 يُدرِج الراوترات كسولةً فتحمرّ ٣٢ حالةً لسببٍ بيئيّ بينما CI على 0.136.3 أخضر. حمرةٌ بيئيّة تُقرأ شيفريّةً تُعطّل الأداةَ في أوّل يوم (نفس درس حارس التكرار)؛ والتخطّي المُعلَن يقول «لم يُقَس» لا «مرّ». الشرطُ كتلةٌ مشحونة تُختبَر بالاستخراج لا بنسخة.
- **حدُّ صدق:** في هذه الحاوية الجناحُ «لم يُقَس» — الشهادةُ عليه من CI وحدَها.
- **المصدر:** `scripts/ci/preflight.sh` (٥ج · ٨ج) · `docs/architecture/preflight_required.json` · `tests_v9/test_local_preflight_contract.py` · فرع `claude/claude-md-docs-p6qqir` فوق `d95b8eaf`.

## 2026-09-06 — نقصٌ واحد ليس فقداً: حارسا الدماغ صارا قابلين للإرضاء معاً

- **القرار ①:** يُؤخَذ العلاجُ الثاني من الصفّ (`يقبل حارسُ الإلحاق نقصاً يُثبَت أنّه إزالةُ صفٍّ بهويّةٍ مكرَّرة`) لا الأوّل (وسمُ الصفّ منسوخاً).
- **السبب:** الأوّل يُبقي في الجدول صفّاً ثانياً يُعلِن الهويّةَ نفسَها، فلا يُرضي حارسَ التفرُّد إلّا بتشويه خليّة الهويّة — وتشويهُها يصير بابَ تحايلٍ على الحارس نفسِه. والثاني يُبرهَن آليّاً لكلّ زوج.
- **القرار ②:** الاستثناءُ ليس تخفيفاً بل **تصحيحُ صياغة**: القاعدةُ كانت تُقاس بالبايتات وما تحميه هو المحتوى. يمرّ النقصُ فقط إذا كان **كلُّ سطرٍ اختفى** صفّاً بهويّةٍ مُعلَنةٍ مرّتين فأكثر عند الوالد، على `gaps/registry.md` وحدَه، ويُطبَع `DUPLICATE_ROW_RECONCILED` فلا يمرّ صامتاً.
- **السبب:** التفريغُ إلى صفر — الواقعةُ الأصليّة `cb6598fe` — يُزيل أسطراً ليست صفوفاً مكرَّرة، فيبقى محجوباً حتّى في سجلٍّ يحوي مكرَّراً. مُكذَّبٌ بحالةٍ مخصَّصة لا بالثقة.
- **القرار ③:** معرفةُ «ما هو الصفُّ المكرَّر» تُستورَد من `brain_duplicate_gap_identity_guard.duplicate_row_lines`، ولا يُعاد اشتقاقُها في حارس الإلحاق.
- **السبب:** نفسُ مبدأ استيراد `APPEND_ONLY` من `resolve_merge_conflicts`: قراءتان للشيء نفسِه تنحرفان، وقد قِيس هنا مرّتين (استبعادُ كتل الكود · مرساةُ الهويّة المنقوطة) أنّ التفاصيل ليست بديهيّة.
- **حدُّ صدق:** الاستثناءُ يحدّ **أيَّ أسطرٍ تختفي**، لا ما يقوله الصفُّ الباقي؛ أمانةُ نصّ الضمّ مسألةُ مراجعةٍ لا قياس، ومُعلَنةٌ كذلك في وثيقة الحارس.
- **المصدر:** `scripts/ci/brain_append_only_guard.py` · `scripts/ci/brain_duplicate_gap_identity_guard.py` (`duplicate_row_lines`) · `tests_v9/test_brain_append_only_guard.py` (٦ حالات) · `docs/architecture/guard_mutation_registry.json` (طفرتان جديدتان + إعادةُ توجيه طفرة التقلّص) · فرع `claude/claude-md-docs-p6qqir` فوق `d95b8eaf`.

## 2026-09-07 — تنفيذان للفجوة نفسها، والأضيقُ يفوز

- **القرار:** يُسقَط تنفيذُ #988 بالكامل ويُعتمَد ما هبط في #987، لا لأنّه سبق بل لأنّه **أضيق**.
- **السبب:** قاعدتي تُجيز إسقاطَ صفٍّ مختلف النصّ ما دام يحمل هويّةً مكرَّرة؛ وقاعدةُ #987 لا تقبل إلّا المطابقَ بايتاً وتوجب حفظَ الأصلين عند الاختلاف. **الاستثناءُ يُقاس بما يمنعه لا بما يُجيزه**، والفقدُ الذي تُجيزه قاعدتي هو بعينه ما وُجِد الحارسُ لمنعه.
- **القرار ②:** لا يُعاد أيُّ جزءٍ من تنفيذي «لأنّه إضافيّ» — `row_identities` سقطت لأنّ لا مُستدعيَ لها بعد الإسقاط.
- **السبب:** شيفرةٌ ميتة في حارسٍ حاجب تُقرأ عقداً قائماً وهي لا تُشغَّل، وهو صنفُ العطل الذي يطارده هذا المستودع.
- **المصدر:** `main@08fa080e` (#987) · فرع `claude/claude-md-docs-p6qqir` (#988) · `docs/runbooks/CI_GATES_AND_PRE_PUSH_PROTOCOL.md` §٣.٢٥ز · `sahool-brain/gaps/registry.md` (الصفّ المضموم).

## 2026-09-07 — فهرسٌ على أعمدةٍ لا يُعرِّفها أحد: حذفٌ لا ترقيع

- **القرار ①:** تُحذَف كتلةُ `idx_market_price_history_crop_date` من `v9_new_tables.sql` بدل تصحيح أعمدتها أو حراسة العمود.
- **السبب:** لا مخطّطَ في الشجرة يحمل `crop_type`/`recorded_at` على هذا الجدول، والفهرسُ الصحيح يملكه `v229:87`. تصحيحُ الأعمدة يصنع فهرساً مكرَّراً، وحراسةُ العمود تُبقي شيفرةً لا تعمل أبداً.
- **القرار ②:** الحارسُ الجديد ساكنٌ في `tests/` ويحكم فقط على الجداول التي يعرف تعريفَها.
- **السبب:** حكمٌ على جدولٍ يُنشأ خارج البيان (خدمةٌ أخرى) تخمينٌ يرفع إنذاراتٍ كاذبة فيُعطَّل في أوّل يوم — الصنفُ الأكثر تكراراً هنا. المقيس: صفرُ كاذب على ٤٤٦ محكوماً.
- **القرار ③:** العلاجُ البنيويّ (إعادةُ تطبيق البيان مرّةً ثانية في CI) يُسجَّل فجوةً مفتوحة ولا يُنفَّذ الآن.
- **السبب:** يمسّ `ci.yml`؛ والحارسُ الساكن يغطّي الصنفَ المقيس لا الصنفَ كلَّه، وهذا مُعلَن في وثيقته.
...
### 2026-09-12 — إعادةُ إرساء ميزانيّة الطفرات على قياسٍ نظيف (`4a6673cc` · #993)

**القرار:** `tests_v9/test_mutation_sweep_headroom.py` — الزوجُ `(45.30, 654)` ⇒ `(45.73, 688)`، والعلامةُ ٧١١ ⇒ **٧٤٥**، وحدُّ الانجراف المشتقّ ٦٨٢ ⇒ **٧١٦**. الصيغةُ والكلفةُ (٠٫٧٧٥ د/طفرة) والسقفُ (٩٠) لم تُمَسّ.
**السبب:** جولةٌ حقيقيّةٌ واحدة — run 34704333703 · job 103581543997 على `4a6673cc` — **٤٥٫٧٣ دقيقة عند ٦٨٨**، زرع ٣٠٫٠٥ / جناح ١٣٫٥٣ (المكنسةُ ما تزال المهيمنة)، وهي **أبطأُ** من المرساة القائمة فتحلّ محلَّها بقاعدة الملفّ نفسِه «أبطأُ قياسٍ حقيقيّ».
**ما رُفِض وسببُه:** جولةُ `f645b137` (job 103568880518) **أُلغيت عند ٩٥٫٠٠ دقيقة** — الجناحُ ≈٦٣ دقيقة بسبب مُعرِّفِ حالةٍ بحجم ميغابايتين تحت `pytest -v` (أُصلِح في `4a6673cc`، فعاد ١٣٫٥٣). زمنٌ ملوَّثٌ بعطلٍ أُزيل لا يُرسى عليه: الملفُّ يقيس بيات الاقتران لا بيات الزمن.
**الأثر:** #993 يخضرّ على الحارس بلا تحريكِ رقمٍ بيد. **والحدّ:** حزمةُ الماء عند ٧٥٧ تبقى فوق ٧١٦ و٧٤٥؛ قرارُ `Mutation Budget Audit` بتوقيعٍ مطلوبٍ في الـRuleset يبقى قائماً — وجولةُ الإلغاء دليلُه: سطرُ سجلٍّ واحد، لا يراه حارسُ عدد الطفرات، استهلك هامشَ التسعين دقيقة كلَّه.
**مُصادَقٌ عليه:** المالك، 2026-09-12 («طبّق تحديث المرساة وادفع»).


### 2026-09-12 — قياسُ الدمج لا يُبدِّل المرساة (`c36f4514`)

**القرار:** تبقى مرساةُ `tests_v9/test_mutation_sweep_headroom.py` على `(45.73, 688)` رغم توفّر قياسٍ أحدث على `main`.
**السبب:** run 34715465343 · job 103611786905 على `c36f4514`: **٤٥٫١٥ د عند ٦٨٨** (زرع ٢٩٫١٢ / جناح ١٣٫٥٠) — **أسرعُ** من المرساة، وقاعدةُ الملفّ «أبطأُ قياسٍ حقيقيّ» تحفظ الأبطأ. ويُسجَّل رغم ذلك لأنّ الفارقَ ٠٫٥٨ دقيقة فقط: المرساةُ **ضيّقةٌ** لا محافظة، وأيُّ جولةٍ أبطأَ منها تُبدِّلها بالقاعدة نفسِها — وجولةُ الفرع (٣٤٫٧٧ د، زرع ٢٢٫٦٠) تُبيّن أنّ تبايُنَ العدّاء وحدَه ≈١٠ دقائق.
**وقرارٌ إجرائيّ:** الدمجُ ضغطٌ، فمرجعُ الحزم القادمة `c36f4514` لا `90d66ace` (ليس سلفاً لـ`main`؛ الشجرتان متطابقتان `c1d93a07`) — نفسُ ما سُجِّل عن `e035dac3` بعد #992.
**مُصادَقٌ عليه:** المالك، 2026-09-12 («قوم بإكمال المتبقي بما تراه مناسبا»).


### 2026-09-13 — patches للمراجعة المقابلة، PR #995

- الإذن: طلب المستخدم تنفيذ إصلاحات المراجعة كلها بصورة متتابعة وفتح PR؛ أعيد استخدام PR #995 الموجود ولم تُنشأ نسخة مكررة أو يُدمج main.
- `44dd9d9` / `04c7f86` / `fe7a92e`: tenant-verified notifications with retry receipts، atomic ERP sync، durable Edge envelope؛ السبب منع النجاح الكاذب وضياع الحدث/المؤشر.
- `268f33c` / `c9dda96`: أمر عمل مخطط محفوظ مع idempotency وFCM HTTP v1 بإيصال مسمى؛ لا خلط بين القبول والتنفيذ الفيزيائي/تسليم الجهاز.
- `bd32411` / `9b6d832` / `0c86f9f`: تصحيح النسخ/PITR، منافذ ومجلدات Helm وتوصيل observability، ومشاريع الموبايل الأصلية وقفل حزم الصورة والتطبيق.
- مرجع الحقيقة للمراجعة الحالية: `docs/audits/PATCH_CLOSURE_20260913.md`. الذاكرة القديمة agent-memory أرشيف، وADR التاريخيان 0002/0003 أُعطيا المعرفين 0036/0037 لحل التعارض؛ لا إعادة كتابة للتاريخ الأقدم هنا.
- حدود السلطة: لم تُفتح GATE-01 ولم يُنشأ adjudication. المقترح `SUP-08-actuator-v2-PENDING-GATE-01.patch` وmanifest بصماته للمراجعة فقط. لا استنتاج لشهادة إنتاج من اختبارات محلية أو Docker/Helm syntax.


مصالحة الفرع قبل الدفع: ضُم `aea41861` و`e528a538` من صاحب الفرع، مع الحفاظ على إصلاحات Copilot وسجلها. أعيد تطبيق commits هذه الجلسة فوق `e528a538`، ولذلك تقابل المراجع السابقة (قبل النشر) المعرفات الحالية التالية:

| قبل rebase | بعد rebase |
|---|---|
| `44dd9d9` | `1091c55` |
| `04c7f86` | `4e49c3d` |
| `fe7a92e` | `0309158` |
...
### 2026-09-13 — حفظ نتائج التحقق قبل فحص متن PR النهائي (#995، `07c14cca`)

اجتازت شيفرة `07c14cca` أجنحة الوحدة والمستودع والمنصة، ونجح زرع العيوب والتوليد. أُلحق سجل النتائج وأعيد توليد تابعياته دون تعديل تشغيلي. لا يُستخدم استثناء dirty-tree لتمرير بوابة Capability-Impact؛ تعاد البوابة بعد التزام المصنوعات. هذه معالجة للترتيب الذي تفرضه البوابة، وليست تخفيضاً لشروطها.

### 2026-09-13 — أربعةُ أحكامٍ من مراجعتي البنيويّة سقطت بالقياس، ومَن أسقطها (المراجعة المقابلة · #997)

**القرار:** تُعتمَد تصحيحاتُ المراجعة المقابلة (`docs/audits/DEEP_REVIEW_SUPPLEMENT_20260913.md`) على مراجعتي (`DEEP_STRUCTURAL_REVIEW_20260913.md`)، وتُدمَج إصلاحاتُ #997 بعد سدّ أربعة نواقص فيه.
**ما سقط من أحكامي، بالقياس لا بالمجاملة:** ① «NATS بلا مصادقة» — `nats/nats.conf:43` يفرض `user/password` وcompose يستوفيهما بـ`:?`، وأوامرُ الصمّام عبر MQTT؛ نقلتُ اسمَ صفٍّ قديم بدل مضمونه. ② «دوالُّ `irrigation.real` غير موصولة» — مسجَّلةٌ بـ`@register_handler` وتُحلّ من السجلّ؛ المُدقّقُ الساكن لا يرى المُزخرِفات (الخطأُ نفسُه الذي صحّحته في `_tenant_context_mw` ولم أعمّمه). ③ «أطوارُ `shared/phase9..12` بمستورِدٍ واحد هو الاختبار» — `api/phase9..12_*.py` تستوردها؛ خمّنتُ هويّةَ المستورِد. ④ «OTel Collector في v9» — ملفُّ إعدادٍ بلا خدمة. ومعها أخطاءُ عدّ (Helm 6 workloads/8 قوالب لا 4/12 · الملكيّة 73 لا 75 · Dart 48/12 لا 46/9).
**السبب:** أعدتُ تشغيل شواهد المراجعة المقابلة الخمسةَ عشر بنفسي على `9613db9a` — 15/15 بالنتيجة نفسها — وقرأتُ الأسطرَ المُستشهَد بها. مراجعتي قاست البنية (حجم · تركّز · حوكمة)؛ المقابلةُ قاست **الحدودَ بين المنتِج والمستهلك** (توكن RS256 يُرفض في الإشعارات · حدثٌ بلا مستأجر يُبثّ للجميع · مؤشّرُ ERP يتقدّم على الانقطاع) — وهناك كانت الأعطالُ الفعليّة. القاعدةُ المستخلَصة: **رسمُ استدعاءاتٍ ساكن لا يحكم على دالّةٍ مزخرَفة، والاسمُ المُتقادم لصفّ فجوةٍ لا يُنقَل بلا قراءة مضمونه.**
**ما سُدّ في #997 قبل الدمج:** إيجابُ gitleaks الكاذب على `docs/audits/patches/*.patch` (تسميتا فصلِ نطاق في firmware) بـallowlist مسارٍ مُعلَّل · ثلاثةُ تعليقات تُحيل إلى `_apply_tenant_guc` المحذوفة · إحالةُ «#995» الخاطئة في راتشِت JWT · وصفوفُ السجلّ المجمَّعة `SUP-01..07` صارت ستّةَ عشرَ صفّاً بمعرِّفاتٍ قياسيّة ومصادرَ وحالات.
**دَينٌ مُعلَن لا مُخفى:** #997 يضيف طفرتين مزروعتين فقط لـ2,651 سطراً؛ إصلاحاتُ الإشعارات وERP وEdge والبوت بلا زرعٍ مُكذِّب — تبقى شواهدها اختباراتِ انحدار لا مكنسة. و`SUP-08` (ربطُ توقيع الأمر بالجهاز) مفتوحةٌ خلف GATE-01 بلا مساس.
**مُصادَقٌ عليه:** المالك، 2026-09-13 («قوم بعمل ما يلزم»).
### 2026-09-13 — إلغاءُ «Require review from Code Owners» في الـRuleset بأمر المالك (#995 · `5d9bed92`)

**القرار:** أُطفئ بندُ `require_code_owner_review` في الـRuleset `main-protection` (ID 20645828) — بيد المالك من الواجهة، لا بشيفرة. البنودُ الأخرى كلُّها **لم تُمَسّ** ومقيسةٌ عبر `rules/branches/main` بعد الحفظ: `required_review_thread_resolution=true` · `dismiss_stale_reviews_on_push=true` · `required_approving_review_count=0` · ١٥ سياقاً مطلوباً · `enforcement=active`.
**السبب:** #995 (تسجيلُ ثلاثة راتشِتات في `capability-governance.yml`) عُلِّقت على اعتماد `@haithmgarallah-ye` لأنّ `/.github/workflows/` مملوكٌ في `CODEOWNERS` والمؤلّفُ `@kafaat` لا يعتمد طلبَه — وهو الثمنُ المُعلَن في قرار 2026-09-05(ب)①. المالكُ حكم بأنّ الثمنَ أعلى من نفعه («أحتاج إلى إلغاء هذا الشرط»)، فالحكمُ تغيّر بقرار من يملكه، والقرارُ السابق يبقى في السجلّ بسببه.
**ما يبقى محروساً:** `branch_protection_contract_guard` يفرض `require_code_owner_review` **مشروطاً** بمسّ `docs/architecture/gates/adjudications/` — فأيُّ PR تُصدِر تفويضَ GATE-01 ستحمرّ حتّى يُعاد تفعيلُ البند لها؛ الحراسةُ على الفعل الخطر باقية، وما زال هو ما فُقِد: عينٌ ثانية على تعديل الـworkflows (نزعُ حارس). `CODEOWNERS` لم يُعدَّل — صار إرشاديّاً للمسارات الثلاثة الأخرى.
**الترتيبُ المقيس:** إطفاءٌ أوّليّ للـRuleset كلِّه (Enforcement=Disabled) أُوقِف قبل الحفظ لأنّه كان سيُحمِّر الحارسَ على كلّ PR؛ ثمّ أُلغي البندُ وحدَه وحُفِظ (07:44Z)، ثمّ دُمِج #995 ضغطاً.
**مُصادَقٌ عليه:** المالك، 2026-09-13.
### 2026-09-13 — تسعُ محادثات Copilot على #997 تُصلَح في الفرع لا تُحَلّ بالنقر (قبل الدمج)

**القرار:** الدمجُ كان محجوباً بـ`required_review_thread_resolution` بعد أن خضرّت الفحوص (70/72). لم تُحَلّ المحادثاتُ لتمرير الدمج؛ قِيست كلُّ واحدةٍ على المصدر فثبتت التسعُ حقيقيّةً، وأُصلِحت في الفرع نفسِه بإضافة `v231_edge_idempotency_tenant_scope.sql` وتعديل تسعة مصادر واختباراتها (الصفوف `CP997-01..09`). بعد الدفع تُحَلّ المحادثاتُ بوصفها مُعالَجة، ويُنتظَر CI على الرأس الجديد، ثمّ يُدمَج ضغطاً.
**السبب:** حلُّ محادثةٍ بلا تعديل يُبلِّغ الـRuleset «عُولِجت» وهي لم تُعالَج — وهو الصنفُ نفسُه الذي يمنعه `no_report_only_change_guard` على الوثائق. والمالكُ أذن بـ«قوم بعمل ما يلزم» بعد تحقّقٍ قَبِل نتائجَ المراجعة المقابلة كلَّها؛ نتائجُ Copilot من الجنس نفسِه (عيوبٌ في الإصلاحات ذاتِها).
**ما لم يُفعَل ولماذا:** لم يُحذَف الاختبارُ الذي كان يُثبِّت الربطَ بـ`int` بل عُدِّل إلى الهويّة النصّيّة؛ ولم يُوسَّع حصرُ البطاقة M-03 للشكل الثنائيّ `(tenant_id, idempotency_key)` لأنّه شائعٌ في مُلّاكٍ آخرين — يُثبَّت موضعُ الحافّة وحدَه. `FCM_SERVER_KEY` زال من allowlist انجراف البيئة لأنّ لا مصدرَ يقرؤه بعد الآن.
**مُصادَقٌ عليه:** المالك، 2026-09-13 («قوم بعمل ما يلزم»).
### 2026-09-13 — `v231` تُسحَب من #997: الهجرةُ تمسّ `MANIFEST.txt` المجمَّد خلف GATE-01

**القرار:** سُحِبت `v231_edge_idempotency_tenant_scope.sql` وتسجيلُها (MANIFEST · `run_migrations.sql`) وما تبعها (بطاقة M-03 واختبارُ استشهاداتها · دليل v208 · اختبار chaos). ضُبِط `EdgeSyncRequest.idempotency_key` على `max_length=32` (عرضُ العمود) بدل توسيع العمود، وبقي هدفُ `ON CONFLICT` على الفهرس القائم مع حدٍّ مُعلَن في docstring الراوتر. محادثةُ Copilot عن التفرّد عبر المستأجِرين أُعيد فتحُها بالسبب لا حُلّت.
**السبب:** `gate01-physical-effect-freeze` حمّر على `b5a3ce32`: «مسارٌ مجمَّد مُعدَّل ولا تفويض يُغطّيه: migrations/MANIFEST.txt»، وكلُّ التفويضات القائمة `CONSUMED`. القيدُ القائم من المالك: «GATE-01 حاجب حوكمي، لا يُفتح بتحرير نصوص أو إعادة تشغيل CI. لا تحاول تجاوزه». فالعلاجُ الكامل لـ`CP997-05` قرارُ مالك (تفويضٌ واحد لهجرةٍ واحدة)، وما يمكن بلا هجرة نُفِّذ.
**مُصادَقٌ عليه:** المالك، 2026-09-13 (قيدُ GATE-01 القائم؛ «قوم بعمل ما يلزم»).
### 2026-09-13 — #997 يُدمَج مع حدٍّ موثَّق (`CP997-05`) بدل فتح GATE-01 الآن (`62fc7b01`)

**القرار:** المالك حلّ محادثة Copilot عن التفرّد عبر المستأجِرين ودمج #997 ضغطاً؛ أي اختار قبولَ الحدّ الموثَّق في docstring الراوتر وسجلّ الفجوات على إصدار تفويض GATE-01 لهجرة `v231` في هذه الجولة.
**السبب:** الهجرةُ تمسّ `migrations/MANIFEST.txt` المجمَّد، وفتحُه قرارُ حوكمةٍ مستقلّ لا يُربَط بجولة إصلاحات؛ والمفاتيحُ `uuid4().hex` فالتصادمُ عبر المستأجِرين عمليّاً معدوم إلى حينه. الشجرةُ المدموجة مطابقةٌ لرأس `3f7054d3` الذي خضّر 71/73 (2 متخطّاة).
**مُصادَقٌ عليه:** المالك، 2026-09-13 (بفعل الحلّ والدمج).
### 2026-09-13 — C01 وC05 من التقرير الجنائيّ الموحَّد ٢ تُنفَّذان بأمر المالك؛ PG01/PG02 لا تُعالَجان لأنّ `main` تجاوزتهما

**القرار:** نُفِّذ C01 (فشلُ قناةٍ لا يحجب القنواتِ السليمة) وC05 (استثناءُ Gitleaks لملفٍّ واحد) في PR مستقلّ بعد #998. لم تُنفَّذ PG01/PG02/PG04 لأنّها تصف حالةَ `refs/pull/997/merge`: تشغيلُ `main` على `62fc7b01` أنتج الدليلَ مربوطاً `exact_commit` وتحقّق منه `gh attestation verify` بمستوى L5 داخل CI، وفرضُ خلاصة التنفيذ موضعه `certify-run` بقرارٍ مُدوَّن في السياسة منذ 2026-08-13. ولم تُنفَّذ C03/C04 لأنّهما قرارا نطاقٍ وسياسة لا إصلاحان ميكانيكيّان.
**السبب:** الإصلاحُ يُقاس على الشجرة الحيّة لا على لقطة التقرير؛ وقياسُ C05 بالأداة نفسها أثبت أنّ المدخلَ الجامع كان يُعفي الملفَّ كلَّه لا يُخفّف قاعدةً.
**مُصادَقٌ عليه:** المالك، 2026-09-13 («نفّذ C01 وC05 في PR بعد دمج #998»).
### 2026-09-13 — صفوفٌ بائتة في السجلّ تُصوَّب بالقياس لا تُحذَف، ومدى الحارس صار إلزاميّاً

**القرار:** حين يثبت أنّ صفّاً `open` أُصلِح فعلاً في التزامٍ سابق، تُقدَّم حالتُه المقيسة (مع SHA الإصلاح واختباره) ويُبقى حكمُه الأصليّ بعدها — لا يُعاد كتابةُ الصفّ ولا يُحذَف. و`brain_append_only_guard` يرفض العملَ بلا `--base` بدل أن يخضرّ على HEAD وحده، وpreflight يشغّله بالمدى نفسه الذي تشغّله به CI؛ وpreflight يرفض تشغيلةً متزامنة في شجرة العمل نفسها.
**السبب:** ثلاثةُ صفوفٍ (#948 · #916/#889 · #972) بقيت `open` أشهراً بعد إصلاحها فحُسِبت دَيناً وهي ليست كذلك؛ وحادثتان موثَّقتان لخضرةٍ محلّيّة كاذبة (مدىً غائب · تشغيلتان متزامنتان) كلّفتا جولاتِ CI.
**مُصادَقٌ عليه:** المالك، 2026-09-13 («قوم بالبحث عن فجوات اخرى يمكن إغلاقها»).


### 2026-09-13 — إتمام التحضير المحلي للأعمال الستة فوق المالكين القائمين

**القرار:** تنفيذ محول FieldView كأداة مشغل للمعاينة، وإعداد عينات الممارسة داخل `shared.feature_store`، وتقوية هوية/حداثة قراءة المتحكم؛ توثيق القبول التجاري والميداني دون تفعيل تحكم أو تدريب أو معيار شامل.
**السبب:** طلب المالك «اكمل 6 الأعمال المؤجلة» مع تفضيله تحسين الموجود دون تعقيد. العقود والمخازن الحالية تكفي للتحضير؛ العينات والحقوق والنتائج الحقيقية ليست متاحة في هذه الشريحة.
**الأساس والمصدر:** `d786d18b5ae7a79d8ec4eae488695f115052cafd`؛ `docs/runbooks/CUSTOMER_PRACTICE_DEFERRED_WORK_AR.md:1`. قرار تنفيذ محلي، وليس تفويضاً ميدانياً أو اعتماد نقل سلطة.
### 2026-09-13 — شرائح التدقيق الموحَّد ٣ تُنفَّذ كشيفرة في PR واحد؛ U05/U08 وRAG-01…12 تبقى بقرار

**القرار:** نُفِّذ كلُّ ما يُغلَق بشيفرة واختبار بلا هجرة ولا قرار منتج (P0 الأربعة، U01–U04، U06، U07) فوق `d786d18b`؛ ولم تُنفَّذ U05/U08 (تحتاج حقولاً/قيوداً في جداول قائمة خلف GATE-01 وتعريفَي نموذج) ولا RAG-01…12 (تحتاج مجموعة تجريبيّة ذات إصدار وقياس قبل/بعد على محتوى عربيّ قبل أيّ بادئة أو بذر).
**السبب:** التقرير نفسه يشترط القبول المتتابع (1→9) ويحذّر من إعادة بذر المجموعة الحيّة أو رفع الحجر لنتائج خضراء؛ والهجراتُ محجوبة بـGATE-01 بقرار المالك. الشرائحُ المنفَّذة كلُّها تعدّل المالكَ الحاليّ لكلّ نطاق بلا خدمة أو مخزن جديد.
**مُصادَقٌ عليه:** المالك، 2026-09-13 («قوم باصلاح و إغلاق كل الشرائح على commit ب و فتح pr»).

### 2026-09-14 — إكمال مراجعة #1002 بعد دمج main

**القرار:** تصحيح التحقق وبصمة المصدر فوق `cf244c8ba915483bc2d167c069535e7efade90a7` مع حفظ الدمج وسجلّيه.
**السبب:** المصدر يشمل التكرارات والترتيب، وخطأ شكل JSON يجب أن يُردّ كخطأ تحقق. `measured_on` يبقى إسناداً للقياس الفعلي؛ عقد `measurement_basis_digest` القائم وإعادة التوليد مرجع التحقق.
**الدليل:** تصحيحا مراجعة #1002 فوق دمج main في `cf244c8ba915483bc2d167c069535e7efade90a7`: يرفض `scripts/ops/fieldview_import_preview.py` المستند غير الكائن قبل `.get`؛ تجزّئ `source_sha256` في `shared/feature_store/practice_review.py` قائمة `records` الأصلية بترتيبها وتكراراتها. تبقى الأقسام ومعرّفات مجموعات البيانات بعد إزالة التكرار. التحقق المحلي: 54 حالة ناجحة في `tests_v9/test_fieldview_import.py` و`tests_v9/test_practice_dataset_review.py`؛ تشمل رفض ستة أنواع JSON غير كائن، وتغيّر بصمة المصدر عند التكرار أو إعادة الترتيب، وثبات المخرجات المنقّاة وعدم تعديل المدخل. الدمج السابق حلّ تعارضات main بالفعل؛ لا حذف لأيّ من سجلَّي الفرعين. قياس CI مستقل عن الاختبار المحلي.

### 2026-09-14 — شريحة عرض التعلم تستعمل عدادات الملخص القائمة

**القرار:** إظهار الحالات بحسب الروابط والصفوف بحسب المصدر في لوحة التعلم، وتسمية العتبة جمعاً تقديرياً مع عرض المراجعة والمعايرة مستقلتين.
**السبب:** المصدر على `aa14b5da0c05ae273665973a773d1f0e3844ae7f` وفّر العدادات منذ #1001 لكن الواجهة أخفتها، وبقيت نصوص تعد بالاعتماد عند بلوغ العدد. العقد الحالي يكفي لإصلاح العرض؛ لا حاجة إلى خدمة أو مخزن جديد.
**الدليل والتفويض:** `docs/testing/LEARNING_DASHBOARD_EVIDENCE_20260914.md:1`؛ طلب المالك «قوم بتنفيذ وفتح pr». هذا تنفيذ لشريحة العرض، وليس اعتماداً زراعياً أو إجازة نقل سلطة أو تفعيل هجرة.

### 2026-09-14 — تحذير المراجعة يتبع الحالة المعلنة في #1004

**القرار:** الخادم مصدر نص التحذيرات؛ يبقى تحذير الجمع والمعايرة عاماً ويصدر طلب مراجعة مختص للحالة غير المراجَعة فقط. تعرض الهوية المحدودة عدد المعرّفات الفعلي.
**السبب والأساس:** مراجعة Copilot على #1004 عند `de9593807972582832b3372a02960c6cdd44879b` أثبتت تناقض حالة المراجعة وتكرار النص وإيهام معرفة الحقل والموسم؛ طلب المالك إصلاح الملاحظات وطلب مراجعة جديدة.
**الدليل:** `docs/testing/LEARNING_DASHBOARD_EVIDENCE_20260914.md`، قسم استكمال مراجعة Copilot؛ اختبارات خادم ومكوّن فشلت قبل الإصلاح ثم نجحت. قرار الاعتماد والأهلية والمخطط باقية كما هي.
### 2026-09-14 — دمجُ #1001 و#1002 متتابعاً وإحالةُ ما يجدّ بعدهما إلى PRs لاحقة

**القرار:** دُمِج #1001 (`6abf88c4`) بعد الجولة العاشرة من مراجعة Copilot وإصلاح بندها الأخير (عدُّ الصفوف المُقيَّمة فقط في خريطة الدليل وثقة القرار)، ثمّ #1002 (`db8a570d`) بعد دمج `main` في فرعه وإعادة توليد المصنوعات؛ ما يجدّ من ملاحظات بعد ذلك يذهب إلى PRs مضبوطة النطاق لا إلى إعادة فتح المدموج. #974 لا يُدمَج حتّى يحسم المالك تعارضَ وثيقة GATE-01 فيه.
**السبب:** الجولاتُ الأخيرة صارت ملاحظاتِ اتّساق عقود لا أعطالاً تشغيليّة، وCopilot يراجع ~80 من 121 ملفّاً بمستوى «Lite» في كلّ مرّة فلا تبلغ الحلقة صفراً بطبيعتها؛ وإرشادُ المراجعة: PRs مضبوطة النطاق. تعارضاتُ #1002 كلُّها مولَّدة (صنفُ «انحراف المصنوعات» في دليل البوّابات) فحلُّها ميكانيكيّ ومقيس.
**مُصادَقٌ عليه:** المالك، 2026-09-14 («هل ترى اغلاق ودمج الفرع…» ثمّ «قوم بالإصلاح» ثمّ «قوم بالدمج بشكل متتابع»).

### 2026-09-14 — تحديث أساس #1004 قبل الدمج المأذون

**القرار:** إدخال `main @ 39ad7b3bab62dc952b0d9313fbaa5e23282ca486` (#1005) في #1004، مع حفظ السجلات وإعادة توليد التعارضات السبعة؛ يذكر جدول الدليل نتيجتي 12 و18 مع سياقهما.
**السبب والدليل:** الفحوص على `94e5d2c696ff1e07174090555bb2c8dcadd875b2` انتهت إلى 66 نجاحاً واثنين متخطيين، وCopilot أوصى بالموافقة. تحرك main قبل الدمج، فلا تُنقل قابلية الدمج أو نتيجة CI تلقائياً إلى الرأس الجديد. الكود والاختبارات لم تتغير؛ الفرق الوارد من #1005 توثيق وسجلات ومصنوعات.
**التفويض:** طلب المالك «تحقق وادمج». الدمج النهائي مشروط بنجاح فحوص الرأس الناتج وشروط GitHub.

### 2026-09-14 — إكمال اتساق مستهلكي الدليل قبل دمج #1004

**القرار:** توحيد نص الجمع في صفحة النسب، وتحذير نقص/محدودية الحقل والموسم كلّاً على حدة، ووصف النسبة المجمّعة كمؤشر للصفوف المحسومة ومصادرها.
**السبب والأساس:** مراجعة Copilot على `4d9c3d885c8a63f63b19a6c0f01d2dd025b4c45c` كشفت هذه المواضع بعد حل تعارضات `main @ 39ad7b3b`. تقع ضمن U01/U03؛ لا يلزم توسيع عقد الحساب لإصلاح الدلالة المرئية.
**الدليل والتفويض:** `docs/testing/LEARNING_DASHBOARD_EVIDENCE_20260914.md`، قسم مراجعة المستهلك المشترك؛ 23 اختبار Python و18 اختبار واجهة، وTypeScript والبناء ناجحة. يستمر طلب المالك «تحقق وادمج» بعد فحوص الرأس الجديد وشروط GitHub.

### 2026-09-14 — إغلاق تحذير الهوية المختلطة قبل دمج #1004

**القرار:** فصل تحذير وجود صفوف بلا هوية عن تحذير محدودية الحقول أو المواسم؛ يغطي اختباران التنوع واجتماع التحذيرين. يبقى فصل حساب `success_rate` بين `outcome_record` و`recommendation_outcomes` خارج النطاق صراحة، مع وصف المؤشر المجمّع ومصدرَيه دون ادعاء فعالية ممارسة.
**السبب والدليل:** ملاحظة Copilot على `7640607648b289cca75726dff14da8f30e58224c` أعيد إنتاجها بحالتين فاشلتين؛ بعد الإصلاح نجحت اختبارات سجل الدليل الـ25. `docs/testing/LEARNING_DASHBOARD_EVIDENCE_20260914.md` يثبت السيناريوهين ومسار عرض التحذيرات. لا تغيير للاعتماد أو العتبة أو المخطط.
**التفويض:** المالك، «نعم، الفجوة حقيقية. أصلحها ثم ادمج»؛ مراجعة واحدة بعد إكمال التعديلات والدفع، ثم الدمج بعد فحوص الرأس النهائي وشروط GitHub.

### 2026-09-14 — تجميع تحسيني عرض الهوية في متابعة مستقلة

**القرار:** متابعة مستقلة بعد #1004 تجمع إزالة تحذير الهوية المحلي المكرر وإضافة المقام للدفعة المجهولة بالكامل؛ يبدأ فرعها من main المدمج، وتُختبر الملاحظتان معاً قبل الدفع.
**السبب والتفويض:** طلب المالك تقليل جولات المراجعة والاختبار المتكررة وتجميع الإصلاحات في فرع منفصل، ثم «استمر» بعد توضيح هذا المسار. ملاحظة إخفاء الدفعة المختلطة أُصلحت في #1004؛ الملاحظتان الجديدتان من review `5201957328` على `f85f40be` تتعلقان بالعرض، وتُحفظ فحوص الرأس الجاري مستقلة.
**الدليل:** 25 اختبار سجل دليل و20 اختبار واجهة وTypeScript ناجحة بعد إثبات الفشل قبل الإصلاح؛ `docs/testing/LEARNING_DASHBOARD_EVIDENCE_20260914.md`، قسم المتابعة المستقلة. لا ادعاء بموافقة Copilot أو قبول حي من هذه الاختبارات المحلية.

**تثبيت الأساس:** دُمج #1004 عند `e4c2479e73b3bc07fe22f06d760777823736e40e` بعد نجاح جولة `34886493240` على `f85f40be` (66 نجاحاً واثنان متخطيان). يبدأ فرع المتابعة من التزام الدمج نفسه، مع تطابق شيفرته مع النسخة المختبرة.

## 2026-09-15 — حدود إصلاح الجاهزية

المرجع الأساسي `5f592818d0fa57b21a392662397317c519625516`. القرار: إصلاح الحدود القائمة دون ترقية البذور أو فتح GATE-01؛ الحالة المركبة والسياسة تأتيان من المنصة، ومصادقة المستخدم في RAG تستخدم متحقق JWT المشترك ومفتاح التحقق، مع بقاء فحص إلغاء الرمز في البوابة، ودوام الصور يستخدم جدول الأتمتة ومجدولها القائمين. السبب: منع مصادر ثقة أو مسارات معالجة مكررة. [الدليل وحدود الإغلاق](../../docs/testing/readiness_software_repairs_20260915.md).

## 2026-09-16 — إصلاحاتُ ما بعد التحقّق تُقدَّم على تمكين B6

المرجع `fa92a1f7`. القرار: تنفيذ الأعطال المقيسة في تحقّق المتبقّي، وتأجيلُ تمكين التوصيات المولّدة (B6) إلى شريحة مستقلّة. السبب: B6 ليس إصلاحاً بل ميزة — الاحتواءُ قائمٌ ومحروس (نصُّ النموذج لا يُقرأ أصلاً، وحارسٌ مصدريّ يمنع إعادة إسناده)، والمطلوبُ إخراجُ مقترحٍ مُصنَّف يمرّ على جسر `guardrails-engine /v1/validate` القائم بعقد أدلّة فاشل-مغلق كما يفعل `supervisor-agent` اليوم؛ وهو تغييرُ عقدٍ يستحقّ مراجعةً مستقلّة لا أن يُحشَر مع إصلاحات. والعطلُ الصامت في مجدوِل الصور يسبقه أولويّةً لأنّه يُبطِل ضمانَ الاستعادة الذي أُضيف M4 لأجله.

## 2026-09-16 — تثبيتُ المراجع الخارجيّة في شريحة مستقلّة لا في #1011

المرجع `main @ 4bbb301f`؛ موضوعُ القياس `a19543ef` (رأس #1011، مفتوح وغير مدموج). القرار: وثيقةُ تحقّق مستقلّة في `docs/audits/` تُثبِّت الـSHA الأربعة وتقيس كلَّ دعوى، مع فجوةٍ مفتوحة تُغلَق بنقل كتلة التثبيت إلى الوثيقة الأصل — لا تعديلٌ لفرع #1011 من هنا. السبب: (١) أمرُ المالك «افتح شريحة مستقلّة بتثبيت الـSHA الأربعة وسبب Finding 6»؛ (٢) الوثيقةُ الأصل ليست على `main` بعد، فالشريحةُ المستقلّة لا تستطيع تحريرها بلا اعتمادٍ على #1011، وسجلُّ القياس يصلح مرجعاً في الحالين؛ (٣) الفرعُ المخصَّص `claude/project-exploration-dtjw3p` أُعيد إرساؤه على `origin/main` بعد قياس أنّ التزاماته التسعة السابقة مدموجةٌ محتوًى (#975: ثمانية ملفّات مطابقة بايتاً على `main` وأربعة تجاوزها `main`) — لا محتوًى غيرَ مدموج أُسقِط. الدليل: [`docs/audits/DECISION_FABRIC_EXTERNAL_SOURCE_VERIFICATION_20260916.md`](../../docs/audits/DECISION_FABRIC_EXTERNAL_SOURCE_VERIFICATION_20260916.md).

## 2026-09-16 — علاجُ `no_report_only_change_guard` داخل #1012 لا في شريحة منفصلة

المرجع `6de08a8d` (رأس #1012 الأحمر). القرار: إصلاحُ الحارس في الـPR نفسها التي قاسته، مع ستّ حالات وطفرتين، بدل شريحةٍ منفصلة تسبقها. السبب: (١) الحارسُ نفسه يوثّق سوابقه الثلاث بالصيغة «مقيسٌ على #N» داخل الشريحة التي أوقفها — هذا نمطُ الملفّ لا استثناء؛ (٢) شريحةٌ منفصلة تُضيف جولةَ مراجعةٍ ودمجٍ لعلاجٍ لا معنى له إلّا بهذه الحالة المقيسة؛ (٣) العلاجُ لا يُخفّف الحارس: «exclusively generated» و«report بخطّ اليد» و«سجلّ الاعتماد» تبقى محجوبةً بحالاتٍ مُثبَتة، والاستثناءُ يُطابق وعدَ الرأس القائم لا وعداً جديداً. الثمنُ المُعلَن: #1012 لم تعد وثائقيّة صرفة — تمسّ `scripts/ci/` فتصير جوهريّةً بالتصنيف، والخضرةُ الآن مزدوجةُ السبب؛ ولهذا أُثبِت بالاختبار أنّ **قائمةَ الملفّات الأصليّة** (بلا الحارس) هي ما يمرّ بالاستثناء الجديد.

## 2026-09-16 — #1012 تبقى شريحةَ الحارس (قرار C) و`config/` تصير جوهريّة بإذنٍ صريح

المرجع `fa4dff2c` (رأس #1012 قبل هذا الالتزام). القرار (المالك، حرفيّاً «أختار C» ثمّ «أعطي الإذن الصريح بإضافة config/ … بالصيغة المقيسة الضيّقة»): (١) لا تُغلَق #1012 ولا تُنقَل إلى #1011 الآن؛ تُغلِق **كلا** صنفَي الحارس المقيسَين في يومٍ واحد (`DOCS-ONLY-SLICE-UNLANDABLE-UNDER-MANDATORY-REGENERATION-01` على #1012 و`RUNTIME-CONFIG-TREE-NOT-SUBSTANTIVE-01` على #1011)؛ (٢) `config/` بادئةً كاملة بعد جردٍ لا بالحدس — شرطُ المالك «إذا كان داخله reports أو generated نضيّق»: الجردُ وجد ملفّين تقريريّين بالاسم يُستبعَدان أصلاً بالبند القائم، فلا حاجةَ للتضييق؛ (٣) لا دمجَ لرأسٍ وسيط (`baa50678` دليلٌ لا مرشّح) — الحكمُ على الرأس النهائيّ بعد CI كامل؛ (٤) نقلُ §٧ إلى #1011 مأذونٌ **بعد** دمج #1012 وتحديثِ فرعها من `main` لا قبله؛ (٥) معيارُ الثبات المعتمد: convergence/validity عند SHA الحاليّ لا byte-identity. السبب: قياسُ #1011 أثبت أنّ الحارس يحجب علاجَ حارسٍ آخر على شيفرةٍ حقيقيّة، فصار العلاجُ واجباً في الحالين وموضعُه الطبيعيّ شريحةٌ تحمل قياسَيه. الدليل: `tests_v9/test_no_report_only_change_guard.py` (25) · `guard_mutation_registry.json` (5 طفرات للحارس).
## 2026-09-16 — فرزُ الجولة الحيّة قبل إصلاحها

المرجع `4bbb301f`. القرار: تُفرَز إخفاقاتُ أيّ جولةٍ حيّة إلى «عطلُ الأداة» و«عطلُ المنصّة» **قبل** أيّ تعديل، ويُحسَب الإخفاقُ الكاذب قياساً مفقوداً لا نجاحاً. وما ثبت أنّه في أداةٍ خارجيّة لا يُعالَج هنا بترقيع: العطلان في `v25_sahool-internal` يُسلَّمان لمالكها مع دليلٍ من شجرتنا. ولا تُعدَّل عتبةٌ ولا يُصدَر اعتمادٌ لإتمام مِسبار، لأنّ إنشاء الاعتماد ترتيبٌ للمسرح لا قياس. وما ثبت أنّه تعارضُ إعدادٍ وشيفرة أُصلِح وحُرِس بعقدٍ مُشتَقّ لا بجدولٍ يدويّ.

## 2026-09-16 — حزمةٌ خارجيّة تُدمَج ولا تُطبَّق حرفيّاً

المرجع `29e2adb4`. القرار: حزمةٌ مربوطة بأساسٍ تحرّك تحته **تُرَاجَع وتُدمَج**، ولا تُطبَّق حرفيّاً ولو مرّ فحصُ بصماتها. البصمةُ تكشف أنّ الملفّ تغيّر ولا تكشف أنّ التغييرَين يتنافسان على المعنى نفسه؛ وهنا كان الطرفان يستبدلان الكتلةَ ذاتها بنيّتين مختلفتين، فالأخذُ الحرفيّ كان سيُسقِط عملَ أحدهما بلا أن يحمرّ شيء. وقرارٌ ثانٍ: اختباراتُ أيّ حزمةٍ مقبولة تُنقَل إلى المستودع لتحجب الدمج — اختبارٌ يبقى خارج الشجرة لا يحرس شيئاً. والتفعيلُ يبقى مطفأً حتّى قبولِ المالك بعد تحقّقٍ حيّ.

## 2026-09-16 — #1014: ملاحظتا #1013 تُنفَّذان · وشاهدٌ حيٌّ لجدول أتمتة الصور

المرجع PR #1014 (`9c7592ac`، ثمّ الرأسُ اللاحق بعد دمج `main @ 0f240f53`). القرار: (١) تعميمُ رسالتَي فشل `_require_service_token` (503 و401) في `services/guardrails-engine/main.py` بدل تسميتهما `/v1/validate` — التبعيّةُ تحرس مسارَين والرسالةُ الخاطئة في نداء الطوارئ أسوأ من المقتضبة؛ (٢) عقدُ `/v1/evaluate` عبر HTTP على التطبيق الحقيقيّ (`tests_v9/test_guardrails_evaluate_http_contract.py`) لا عبر مِرقاة تُحاكي محرّك الطبقات، لأنّ انحدارَ التوصيل كان يمرّ وكلُّ اختبارات B6 خضراء — صنفُ «نصُّ المصدر ليس سلوكَ التشغيل» الذي قُبِل مكتوماً على #1010؛ (٣) شاهدٌ حيٌّ لسياسة `imagery_automation_fields` (`tests_v9/test_imagery_automation_rls_live_pg.py`) يقيس السياسةَ المُهاجَرة نفسَها بخمس درجات، مُثبَتٌ محلّيّاً على PostgreSQL 16 بثلاث طفراتٍ حمراء، ومُوصَلٌ بوظيفة التكامل بعَلَمٍ يمنع أن يصير غيابُ الـDSN تخطّياً أخضر؛ (٤) لا إضافةَ إلى `_INLINE_PURGE_ALLOWLIST` لأنّ الاختبار لا يحتاج إسقاطاً أوسع من `load_service_main`، والاستثناءُ يُنقِص الحراسة بلا سبب. الفجواتُ الثلاث (`GUARDRAILS-TOKEN-MESSAGE-NAMES-THE-WRONG-ROUTE-01` · `GUARDRAILS-EVALUATE-HAS-NO-HTTP-CONTRACT-TEST-01` · `IMAGERY-AUTOMATION-TABLE-HAS-NO-LIVE-PG-WITNESS-01`) في السجلّ بمصادرها. **وملاحظتا Copilot على #1014 نفسِها:** فرعُ 503 لم يكن تبلغه أيُّ حالة (إعادةُ اسم المسار إلى رسالته كانت تمرّ خضراء) ⇒ حالةٌ تُفرِّغ توكنَ الوحدة وتؤكّد 503 برسالةٍ محايدة على المسارَين؛ وهذا القيدُ نفسُه هو ما كان ناقصاً — سطرُ سجلّ ولقطةٌ حارّة بلا قرارٍ مُسبَّب بـPR/SHA.
## 2026-09-16 — #1011 تُحضَّر محلّيّاً ولا تُدفَع قبل #1012

المرجع `main @ 16327fd9` (يحمل #1010 و#1013). القرار: تحضيرُ فرع #1011 على فرعٍ محلّيّ (`prep/1011`) — دمجُ `main`، نقلُ كتلتَي §٧، تصحيحُ العدد، إعادةُ التوليد — **بلا دفع** حتّى يُدمَج #1012 ويُعاد الدمجُ من `main`. السبب: (١) إذنُ المالك بالدفع إلى `audit/decision-fabric-source-comparison-20260916` مشروطٌ صراحةً بدمج #1012 أوّلاً؛ (٢) قياسٌ: على `main` الحاليّ تُحجَب #1011 بعد إعادة التوليد لأنّ `config/` ليست جوهريّة، والعلاجُ في #1012 — فالدفعُ قبله يُحمِّر بوّابةً عمداً؛ (٣) إغلاقُ `EXTERNAL-REFERENCE-WITHOUT-SHA-01` في السجلّ يُؤجَّل إلى ما بعد إعادة الدمج لأنّ قسمَها يصل `main` مع #1012، وإنشاءُ قسمٍ ثانٍ هنا يُحمِّر حارسَ تكرار الهويّات عند الدمج.

## 2026-09-16 — #1011 تُدفَع بعد دمج #1012 · الفجوةُ تُغلَق في قسمها لا في قسمٍ ثانٍ

المرجع `main @ 0f240f53` (#1012 مدموج). القرار: إعادةُ دمج `main` في التحضير المحلّيّ ثمّ الدفعُ إلى `audit/decision-fabric-source-comparison-20260916` — بإذن المالك المشروط الذي تحقّق شرطُه. السبب: (١) الشرطُ الذي أجّل الدفعَ («بعد استقرار/دمج #1012») صار مقيساً؛ (٢) `config/` صارت جوهريّةً في `main` فلا تُحجَب الشريحةُ بـ`no-report-only-change` بعد إعادة التوليد؛ (٣) قسمُ `EXTERNAL-REFERENCE-WITHOUT-SHA-01` وصل `main` مع #1012 فيُعدَّل في موضعه بدل استحداث قسمٍ ثانٍ يُحمِّر حارسَ تكرار الهويّات. لا يُمسّ في هذا الدفع شيءٌ من ملاحظات Copilot السادسة على #1012 (أربعةُ ملحوظات مكبوتة: صياغةُ §٧ · تعليقُ `_SELF` يقول «الاثنين» وفيه ثلاثة · ثلاثةُ مصنوعاتٍ ناقصة من قائمة الحارس · نطاقُ §٨) — تبقى شريحةً لاحقةً ضيّقة كي لا تتّسع #1011 خارج موضوعها.

## 2026-09-17 — مُحمِّلُ الخدمة المشترك يفشل على الانحدار الداخليّ ويتخطّى الخارجيّ فقط

المرجع `main @ 18328ad2`، متابعةُ #1014. القرار: تضييقُ التخطّي في `load_service_main` إلى الحزم الخارجيّة الغائبة فقط، بتصنيفٍ بالمسار لا بقائمة أسماء، والمجهولُ داخليّ. السبب: المُحمِّل مساعدٌ مشترك، فتخطٍّ شامل فيه يُخضِّر انكسارَ توصيلٍ في أيّ خدمة تعتمد عليه؛ وقائمةُ أسماءٍ خارجيّة كانت ستبيت مع كلّ تبعيّة جديدة، بينما وجودُ ملفٍّ تحت الجذر حقيقةٌ تُقاس وقتَ التشغيل. الاتّجاهُ الخاطئ المقبول: حزمةٌ خارجيّة باسمِ مجلّدٍ في المستودع تفشل بدل أن تُتخطّى — آمن. شريحةٌ مستقلّة عن #1015 عمداً.
## 2026-09-17 — AI-RUNTIME-WIRING-01: التسجيلُ أوّلاً، والنقلُ إلى `shared/` قبل التوصيل

المرجع `main @ 18328ad2`. القرار: (١) لا التزامَ توصيلٍ قبل تسجيل الفجوة بقياسها؛ (٢) موضعُ الاستدعاء الإنتاجيّ `orchestrate_recommendation` (المنصّة) لا نقاطُ ai_agronomist، لأنّه الوحيدُ الذي يُنتج توصيةً ويُثبِتها مع outbox في معاملةٍ واحدة وهو مدخلٌ مسجَّل، بينما ai_agronomist evidence-only بعقده وحارسُ `decision_contracts` يحظر عليه مفتاح التوصية؛ (٣) بما أنّ صورةَ المنصّة لا تنسخ `services/ai_agronomist`، تُنقَل عائلةُ الخطّ إلى `shared/ai/recommendation_runtime/` مع قشورِ إعادة تصدير — لا استيرادَ عابرٌ للحاويات يعمل في الاختبار وينكسر في التشغيل؛ (٤) الرايةُ تُقرأ من البيئة لا من `config/` غير المنسوخ؛ (٥) لا مسارَ جديداً (الميزانية 629) ولا تفعيلَ قبل شهادةٍ حيّة. البدائلُ المرفوضة وأسبابُها في قسم الفجوة.
## 2026-09-17 — مهلةُ الزرعة شريحةٌ مستقلّة عن cutover المكنسة

المرجع `main @ 18328ad2`. القرار: فصلُ مهلة الزرعة (`PLANT_TIMEOUT_SECONDS`) عن الالتزام الموحَّد لـMUT-SHARD-JOB-01. السبب: المهلةُ تحسينٌ لا يعتمد على الـRuleset ولا يمسّ موضعَ المكنسة، فتأخيرُها خلف فعلٍ خارج المستودع يُبقي زرعةً معلَّقة قادرةً على حرق 90 دقيقة بلا اسم؛ والدمجُ في التزامٍ واحد كان يُكبِّر نصفَ قطر الأثر لتغييرٍ يُفترَض أن يكون ذرّيّاً (العقد + ci.yml + الاختبار الثنائيّ + مرساة السجلّ). الدلالاتُ القائمة لا تتغيّر؛ يُضاف حكمٌ رابع فقط.
## 2026-09-16 — بعد دمج #1011 (`9117c39e`): ملاحظاتُ المراجعة المكبوتة تُجمَع في شريحةٍ ضيّقة لا تُلحَق برأسٍ أخضر

المرجع `main @ 9117c39e`. القرار: (١) ملاحظةُ Copilot المكبوتة على #1011 (تناقضُ قسم `EXTERNAL-REFERENCE-WITHOUT-SHA-01` بعد `fixed`) لا تُدفَع إلى #1011 وهو `clean` — فأيّ دفعٍ يُعيد دورةَ CI كاملةً (~٤٠ دقيقة) على رأسٍ أخضر لسطرِ توثيق؛ تُجمَع مع ملاحظات Copilot ٦ الأربع على #1012 في شريحةٍ ضيّقة على `claude/project-exploration-dtjw3p` المُعادِ من `main`. المالكُ دمج #1011 قبل الاختيار فصار القرارُ واقعاً. (٢) القائمةُ الصريحة لمصنوعات إعادة التوليد تُوسَّع بأربعة أهدافِ كتابةٍ اشتقاقيّةٍ صرفة بالمعيار نفسه المُعلَن فوقها (لا مفتاحَ إعفاءٍ/إثباتٍ/تحكيم)، لا باشتقاقها من `generated_write_targets.json` — المرفوضُ عمداً لأنّه يعدّ سجلَّ الاعتماد هدفَ كتابة. (٣) #1014 ليست من فتحي: عُولجت خيوطُها وتعارضاتُها بأمر المالك، وتبقى لدمجه هو.

## 2026-09-17 — بصمةُ نماذج الحافّة: تُربَط لا تُضاف، وشريحةُ التوصيل تُؤجَّل بقيدِ مورد

المرجع `main @ 2b88ce33`. **قراران:**

**(١) تأجيلُ شريحة التوصيل (P0) بقيدٍ مقيس لا بتفضيل.** تنفيذُها يفتح معاملةَ مستأجرٍ قبل المحرّك ويُبقيها طوال حسابه، وقد سُجِّل في #1020 أنّ ثمنَ ذلك **يُقاس في شريحة التنفيذ لا يُفترَض**. ولا قاعدةَ بيانات في بيئة هذه الجلسة (`pg_isready` لا استجابة)، فالقياسُ والشهادةُ الحيّة متعذّران. **السبب:** بناءُ تغييرٍ على المسار الإنتاجيّ مع تركِ القيد الوحيد الذي يبرّره غيرَ مقيس هو عينُ ما أُغلق اليوم ثلاثَ مرّات — دعوًى تُكتب قبل أن تُقاس. الشريحةُ تُستأنَف عند توفّر قاعدةٍ حيّة.

**(٢) بصمةُ نماذج الحافّة تُسجَّل بوصفها عقداً غيرَ مُنفَذ، لا بصمةً غائبة.** البصمةُ **موجودةٌ ومُنفَّذة** وقتَ التشغيل: `model_artifact_gate` يُجزّئ الملفّ ويقارنه قبل كلّ استدلال بأسبابٍ مسمّاة. فالمُسجَّل هو ما قِيس فعلاً: `sha256_env` حقلٌ مُعلَنٌ بصفر مراجع في `scripts/` و`tests_v9/` · ولا فحصَ عكسيّ من `_MODEL_ENV` إلى البيان · والفحصُ القائم بالاحتواء النصّيّ. **السبب:** تسميتُها «إضافةَ بصمة» كانت ستُنتج ازدواجاً مع حارسٍ يعمل، وتوحي بغيابِ حمايةٍ قائمة — وهو ادّعاءٌ كاذب في الاتّجاه المعاكس. **والدليلُ على أنّ الفجوة حقيقيّة لا لفظيّة:** طفرتان (إعادةُ تسمية `sha256_env` · ونموذجٌ في `_MODEL_ENV` بلا بيان) تمرّان **خضراوَين عند الحارس القائم** وتُحمِّران عند الجديد — قِيس، لا استُنتج.

**الحدّ:** `sam2-inference` بلا تحقّقِ بصمةٍ إطلاقاً، مُسجَّلٌ وخارجَ النطاق بقصدٍ معلن كي لا يتّسع نصفُ قطر الأثر، وشاهدٌ يُحمِّر إن اكتسب تحقّقاً فيُحدَّث القسم.

## 2026-09-17 — الخطوة ② تبدأ بإبطال بندِها المسجَّل، ولا تُنفِّذه

المرجع `main @ f063e1bd`، الشريحة #1020. **القرار:** لا يُبنى توصيلُ الخطّ على البند المسجَّل («في `orchestrate_recommendation`، والحالةُ من `_compose_canonical`»)؛ يُسجَّل بدلَه قياسُ ثلاثة عوائق وخياراتٌ ثلاثة، والمرجَّحُ **التوصيلُ في طبقة المسار غيرِ المتزامنة**.

**السبب المقيس:** (أ) `RecommendationRuntimePipeline.execute` معرَّفٌ `async`، و`orchestrate_recommendation` و`handle_recommendation_request` متزامنتان وتُستدعيان من حلقةٍ عاملة — فالتوصيلُ من داخلهما يحتاج إعادةَ هيكلةِ سلسلةٍ أو `asyncio.run` داخل حلقةٍ قائمة، وذاك يرمي وقتَ التشغيل. (ب) `guarded_runtime_context` يرفع `MissingCanonicalFieldState` **قبل** بناء أيّ مدخل، والمسارُ يقرأ السياقَ القانونيّ **بعد** عودة المحرّك، و`_compose_canonical` في راوتر آخر وتطلب `conn` لا يملكه أيٌّ منهما. (ج) هذا الترتيبُ موثَّقُ القصد: قراءةٌ على معاملة الكتابة نفسِها، «لقطةٌ واحدة».

**وتصحيحٌ لاستدلالي، بمراجعة #1020:** وصفتُ الخيارَ المرجَّح بأنّه «وحدَه يحلّ العائقَين ولا يمسّ ترتيبَ اللقطة» — وهو خطأ. هو أيضاً يقتضي تقديمَ فتحِ الاتّصال والقراءة قبل نداء المحرّك، أي تغييرَ لحظةِ القراءة بالنسبة إلى القرار. ما يبقى له: خاصّيّةُ «اللقطة الواحدة» **تُحفَظ** إن بقيت المعاملةُ مفتوحةً حتّى الكتابة، بل تصير الدعوى أصدق لأنّ القرارَ يستهلك ما قُرِئ فعلاً؛ **وثمنُه معلَن**: معاملةُ مستأجرٍ ممتدّةٌ طوال حساب المحرّك، أثرُها على القفل والاتّصالات يُقاس في شريحة التنفيذ لا يُفترَض.

**الحدّ:** لا شيفرةَ تشغيلٍ تتغيّر في هذه الشريحة، والفجوةُ تبقى `open`. وشاهدُ `tests_v9/test_recommendation_wiring_preconditions.py` يحرس الشروطَ باتّجاهٍ معكوس: زوالُ عائقٍ يُحمِّره ويطالب بتحديث القسم.

## 2026-09-17 — الرايات تُقرأ من البيئة في `shared/`، و`config/` إعادةُ تصدير لا تعريف

المرجع `main @ bb505a4d`، شريحةُ نقل AI-RUNTIME-WIRING-01 ①. القرار: (١) موضعُ التعريف الواحد `shared/ai/recommendation_runtime/flags.py` لأنّه ما تنسخه الصورتان، والمصدرُ البيئة (`SAHOOL_<FLAG>`) لأنّ الحاوية لا تحمل ملفّ إعداد؛ (٢) لا `try/except` حول استيراد الرايات في المستهلكين — البديلُ الصامت هو ما جعل الفجوة غيرَ مرئيّة سنةً؛ (٣) `config/guardrail_feature_flags.py` يبقى للقرّاء والاختبارات إعادةَ تصديرٍ بلا تعريف، ويحرسه شاهدٌ يرفض أيَّ إسناد فيه؛ (٤) الأسماءُ داخل الحزمة الجديدة هي الأسماءُ القديمة كي يبقى الفرقُ نقلاً صرفاً يُراجَع كنقل. القياسُ الجانبيّ: قيمةٌ غيرُ مفهومة في البيئة تُعدّ الافتراضَ لا تفعيلاً.

## 2026-09-17 — عدُّ أسطح النشر يُشتَقّ من السطح المشترك، لا من `glob` خاصّ بكلّ شاهد

المرجع `main @ 2b88ce33`، فرعُ `EDGE-MODEL-DIGEST-CONTRACT-UNENFORCED-01` (#1022)، بعد مراجعةٍ آليّة. القرار: (١) كلُّ شاهدٍ يسأل «أيُّ ملفّات compose؟» يستدعي `compose_surface.compose_files()` لأنّه ما تبنيه البوّاباتُ نفسُها من `git ls-files`؛ عدّادٌ خاصٌّ يرى مجموعةً أخرى ويُدخِل ملفّاً غيرَ متعقَّب في القياس، فيُصبح الحارسُ والبوّابةُ يجيبان إجابتين. (٢) مطابقةُ متغيّرِ البيئة تقبل صيغتَي compose معاً، لأنّ قارئاً يعرف صيغةً واحدة يُنتج **حمرةً كاذبة** عند تمثيلٍ مختلف لنفس الحقيقة — الوجهُ المقابل لِما تحرسه هذه الشريحة. (٣) شاهدُ استهلاك اسمِ البصمة يربط **الحلقتين** لا وجودَ نداء: وجودُ `os.getenv` في ملفٍّ لا يقول إنّ الاسمَ المُعلَن هو المقروء، والفرقُ بينهما هو بعينه عطلُ «الاسمُ يُفعِّل» الذي تُغلقه الشريحة. (٤) الاستيرادُ من `scripts/ci` داخل `tests_v9` يكون بـ`spec_from_file_location` لا بـ`sys.path.insert`: الثاني يبقى نافذاً بقيّةَ الجلسة فيُغيّر فضاءَ استيراد اختباراتٍ لا شأنَ لها بالسطح — تلويثُ حالةٍ عامّة من أجل استيرادٍ محلّيّ، وهو ما أُصلح صباحاً في `service_module.py`. القياسُ الجانبيّ: صنفُ «قارئٌ أضيقُ من دعواه» ظهر خمسَ مرّاتٍ في يومٍ واحد، أربعٌ منها في شيفرتي.

## 2026-09-18 — شاهدُ الانفصال يُقيَّد بحالة الفجوة في الاتّجاهين، ولا يُجمِّد العطل

المرجع `main @ 78e8bf5d`، شريحةُ `SENSOR-TELEMETRY-INGEST-REACHES-NO-AGRONOMIC-CONSUMER-01`. القرار: (١) لا تُوصَّل نقطةُ ابتلاع الأجهزة بالمستهلك الزراعيّ في هذه الشريحة — التوصيلُ قرارُ مسارِ بياناتٍ (أتكتب نقطةُ المنصّة في `soil_observations` بعقد خدمة التربة، أم تُوصَف صراحةً ابتلاعَ صحّةِ أجهزةٍ لا قياساً زراعيّاً؟) وله عوائقُ `AI-RUNTIME-WIRING-01` الثلاثةُ نفسُها؛ واختيارُ أحدهما بلا قياسٍ يُثبّت مساراً خاطئاً. (٢) **الشاهدُ يقيس اتّفاقَ الشجرة مع السجلّ لا الانفصالَ نفسَه**: شاهدٌ يقول «الجدولان منفصلان» وحدَه يُحوّل العطلَ إلى عقد، فيصير من يُصلحه غداً مُحمِّراً لاختبارٍ يمنعه. فالقيدُ في الاتّجاهين — تداخلٌ يوجب قلبَ الحالة، وانفصالٌ يوجب بقاءَها `open` — وهو نمطُ `test_recommendation_runtime_reachability.py` نفسُه. (٣) قاعدةُ النثر **ضيّقةٌ بقصد**: يُمنَع اقترانُ اسم القارئ الزراعيّ باسم جدول الأجهزة في كتلةٍ واحدة، ولا يُمنَع ذكرُ الجدول — فله ذكرٌ صادق (الكاتبُ وصداه، وبطّاريّةُ التوأم، وأنّ الكاميرا لا تبتلع فيه قياساً)، ومنعُ الذكر مطلقاً يُحمِّر على دعاوى صحيحة. (٤) صياغةُ التصحيح لا تنفي بالاسم: كتبتُ أوّلاً «`soil_observations` لا `device_telemetry`» فأحمرّت قاعدتي على نفسها بحقّ — والوصفُ الصحيح يسمّي المصدرَ الفعليّ، والتاريخُ موضعُه قسمُ الفجوة. القياسُ الجانبيّ: قراءةُ بندِ الطابور حرفيّاً قادت إلى الطريق الخطأ مرّتين في يومين، فالقياسُ قبل البناء ليس احتياطاً بل هو الشريحة.

## 2026-09-18 — الدعوى المطلَقة تُستبدَل بقياسٍ للجسر، لا بعبارةٍ ألين

المرجع `main @ 78e8bf5d`، #1024، بعد مراجعةِ المالك بالأدلّة. القرار: (١) حين يُكذَّب نطاقُ دعوًى («لا قارئَ في الشجرة كلِّها»)، لا تُحَلّ بإضافة «تقريباً» أو «فيما قِيس» — تُستبدَل بقياسِ الشيء الذي أُنكِر: الجسرُ يُسمّى ويُقاس في ثلاثة أبعاد (يقرأ الجدولَ · يكتب المخزنَ القانونيّ · أداةُ مشغّلٍ بوسيطٍ إلزاميّ لا يستدعيها عاملٌ ولا مجدوِل) وزيادةً وزنُ ما يُدرجه (`suspect`). فالعبارةُ اللينة تُخفي الجهلَ، والقياسُ يجعل تغيُّرَ الحقيقة يُحمِّر. (٢) نطاقُ البحث يُذكَر في الدعوى أو تُقيَّد الدعوى به: `grep` على `services/` لا يُنتج حكماً على «الشجرة كلِّها»، وهذا الفرقُ أسقط دعوًى في هذه الشريحة بعينها. (٣) التحقّقُ المحلّيّ لا يعتمد أنبوباً بلا `pipefail`: `pytest | tail` يُرجِع 0 على إخفاق، فرمزُ الخروج يُطبَع صراحةً في كلّ تشغيلٍ حاسم. (٤) شاهدُ وجودِ اسمٍ في SQL يقرأ **جُمَلَه** (`FROM` · `INSERT INTO`) لا نصَّه: الاسمُ يتكرّر في أسماء نقاط الحفظ والبيانات الوصفيّة ومفاتيح إزالة التكرار، فالاحتواءُ النصّيّ ينجو من نقل الجدول. القياسُ الجانبيّ: ثلاثٌ من أربع ملاحظاتٍ أصابت شيفرتي أو منهجي، وواحدةٌ منها لم يكشفها إلّا التكذيب بعد أن قرأتُ الشاهدَ ورضيتُ عنه.

## 2026-09-18 — يُكذَّب الإصلاحُ المقترَح قبل بنائه، لا بعد شحنه

المرجع `main @ 7e5b588c`، شريحةُ `RECONCILIATION-CURSOR-SKIPS-ROWS-THAT-BECOME-ELIGIBLE-01`. القرار: (١) الإصلاحُ البديهيّ — نقلُ رفع المؤشّر أسفلَ فحص المحوّل — **يُختبَر كطفرةٍ قبل أن يُقترَح**، وقد سقط: الصفُّ المدعومُ اللاحق يرفع المؤشّرَ فوق السابق على أيّ حال. فالشاهدُ يُثبّت **عدمَ كفايته** كي لا تُشحَن شريحةٌ تنقل سطراً وتُظهر أخضرَ ولا تُصلح. (٢) توصيفُ العلّة يُرفَع من «ترتيب سطر» إلى **نموذج الاستئناف**: علامةُ ماءٍ عليا واحدةٌ فوق تيّارٍ مُرشَّح تفقد بالبناء كلَّ صفٍّ تُخطّي لسببٍ قابلٍ للزوال؛ والإغلاقُ يحتاج مصيراً لكلّ صفّ أو طابورَ معلَّقاتٍ يُعاد فحصُه، بقبولٍ على قاعدةٍ حيّة. (٣) بديلُ الاتّصال في الشواهد يقرأ محمولاتِ الاستعلام **من نصّ المصدر** ويرفض ما لا يعرفه، والمصدرُ مثبَّتٌ ببصمةٍ تفشل مغلقةً — فلا يصف الشاهدُ شيفرةً غيرَ التي يدّعي وصفَها. (٤) كلُّ شاهدٍ يصف عطلاً قائماً يُقيَّد بحالة فجوته في الاتّجاهين، ويُرافَق بضابطٍ يُثبت أنّ المسار يعمل في الحالة السليمة — وإلّا كان «الفقدُ» أثراً للأداة لا للشيفرة. القياسُ الجانبيّ: المنهجُ كلُّه مُقتبَسٌ من حزمة أدلّة المالك، ويُقرّ بفضله.

## 2026-09-18 — الـRuleset قبل الدمج في cutover المكنسة، والمسوّدةُ هي القفل

المرجع `main @ c7d912c8`. القرار: (١) الترتيبُ **الـRuleset أوّلاً ثمّ الدمج** قيدُ سلامةٍ لا تفضيل: الدمجُ أوّلاً ينزع المكنسةَ من *Unit Tests* بينما حزمُها ليست حاجبةً بعد، فتصير ٧٠٠+ طفرة بلا بوّابة — «تحمرّ ولا تحجب»، الصنفُ الذي يطارده هذا المستودع. والاتّجاهُ المعاكس آمن: الإنفاذُ أشدُّ من الوثيقة، وثمنُه حمرةٌ إرشاديّةٌ واحدة. (٢) الطلبُ يبقى **مسوّدة** حتّى تُنفَّذ النقرة — قفلٌ ميكانيكيٌّ بدل الاعتماد على تذكُّر الترتيب. (٣) وظيفةُ `branch-protection-contract` تُستعمَل **شاهدَ اكتمالٍ للنقرة**: تقرأ الإنفاذَ الحيّ وتخضرّ حين يطابق العقد، فيُقاس فعلُ المالك بدل أن يُصدَّق. (٤) دعوًى تُكرَّر في الفحوص المجدوَلة تُتحقَّق قبل تكرارها: قلتُ «`prep/ci-sweep-dedupe` @ `9575be74`» مراراً وهو **محذوفٌ من البعيد**. القياسُ الجانبيّ: التكرارُ ليس مصدراً؛ الفرعُ زال ولم ينتبه أحد لأنّ لا أحدَ قاسه.
## 2026-09-18 — كلُّ شاهدٍ يُقاس بجدول صدقٍ على عوالمه الممكنة، لا على طرفَيه فقط

المرجع `main @ c7d912c8`، تتمّةُ #1025 بعد مراجعة المالك. القرار: (١) شاهدٌ يربط حالةَ فجوةٍ **واحدة** بعدّة سيناريوهات يجمعها **قبل** الحكم ويحكم مرّةً على `any(loss)` — والربطُ المنفرد يرفض الإصلاحَ الجزئيَّ برفضِ الحالتين معاً، فيحجب طريقَ الإصلاح الصحيح. (٢) حكمُ الربط نفسُه **يُختبَر** بجدول صدقٍ على مخرجاتٍ مضبوطة (حالةٌ × مجموعةُ الفواقد)، لا يُستنتَج من قراءة الشيفرة — فقد قرأتُ ربطي ورضيتُ عنه وهو غيرُ قابلٍ للإرضاء. (٣) يُثبَت أنّ جردَ الحالات اكتمل قبل الحكم، وإلّا أرضى الشاهدَ قطعٌ مبكّر. (٤) الصنفُ يُسمّى ويُسجَّل: **حارسٌ لا يقبل حالةً صادقة في عالمٍ ممكن** — مختلفٌ عن «أضيقَ/أوسعَ من دعواه»، لأنّه لا يُنتج حمرةً ولا خضرةً كاذبة بل استحالةً. القياسُ الجانبيّ: فحصُ الطرفين (يُحمِرّ على العطل · يخضرّ على السلامة) لا يكشفه؛ يلزم فحصُ ما بينهما.


## 2026-09-19 — #1028: القياس المستقل قبل الإنفاذ

المرجع #1028 فوق `main@38366e46844aee98fb3a4a421554691d2d5681d8`. وظيفة `gap-registry-report` مستقلة بلا `needs` وغير مانعة، حتى لا تحجب أخطاء الحوكمة السابقة دليل السجل الحقيقي. يُثبّت checkout على رأس الطلب، ويُرفق SHA الفعلي وبصمتا السجل والأداة مع JSON، باسم artifact يحوي الرأس والتشغيل والمحاولة. لا تُرقّى النتائج إلى بوابة ولا يُعدّل السجل تاريخيًا في هذه الشريحة؛ السبب أن قياس البنية لا يثبت انتقالًا في حالة الفجوة. التنفيذ في `.github/workflows/capability-governance.yml` وشواهده في `tests/architecture/test_gap_registry_measure.py`.

يُصحح تصنيف الكتالوج من مصدر YAML نفسه: التصريح الحرفي `true` وحده يثبت عدم الحجب؛ التعبير الديناميكي لا يُفترض إعفاءً. تحتفظ `discover_invocation_sites` بكل المواقع، ويستبعد عرض الحجب المواقع غير المانعة فقط، حتى لا يختفي الدليل ولا ينمو الأساس بسبب تسمية خاطئة.


## 2026-09-19 — حالة صريحة وتاريخ محفوظ بعد #1028

المرجع `main@ed376b54be7a1b9cd113c39681ba6679cbedc1dd` و[تدقيق النتائج](../../docs/audits/gap_registry_measurement_20260919.md). القرار: الحالة من العمود المعلن والنص القانوني، لا من آخر خلية أو اقتباس تاريخي؛ والخطأ البنيوي يظل نتيجة معلنة. توحيد الإغلاق الموثق يحفظ نطاق الدليل والمتبقي، ولا يرفعه إلى verified.

الألقاب والسياسات والأحكام المصححة تُعلن بدور صريح وتبقى في JSON بأدلتها. تاريخ الهوية المتكررة لا يُختزل إلا بمدخل حالي واحد والباقي تاريخي، مع بقاء كل النصوص والاختلافات مرئية. السبب: حذف التاريخ أو التخمين من الموضع يخفي الدليل، وخلط السياسة بحالة فجوة يصنع عدادًا مضللًا. يبقى التشغيل report-only حتى يُراجع هذا العقد ودليله على المصدر الحقيقي.


## 2026-09-19 — الوسمُ يُدفَع ثمنُه، ولا يُرقّى العدّ

المرجع `main@c3ebd6a7f9bf33bb3bde138533f99f493acd8740` (#1031) وفجوة `ANNOTATED-SUPERSESSION-CLOSES-A-GAP-WITHOUT-A-WITNESS-01`. **القرار:** يُربَط وسمُ `current`/`historical` بحقيقةٍ عن الشجرة لا بإقرار كاتبه — خلافةٌ تُغلِق ما يبقى `historical` مفتوحاً تلزمها استشهادٌ بشاهدٍ موجود. **السبب:** وجودُ الملفّ حقيقةٌ لا نيّة؛ ومن يلفّق إغلاقاً يلزمه زرعُ شاهدٍ حقيقيّ فتتولّاه بوّاباتُ التسجيل والعلامات. والاستشهادُ بالمصدر المُصلَح لا يُحتسَب، وإلّا صار الثمنُ ذكرَ الملفّ المعطوب.

**وقرارٌ ثانٍ بالسلب:** لا يُرقّى عدادُ غيرِ المصنَّف في `gap_registry_measure` إلى بوّابة، ولا تُجعَل وظيفةُ `gap-registry-report` حاجبة. **السبب:** #1031 نصّ صراحةً أنّ صفرَ غيرِ المصنَّف لا يثبت اكتمالَ الفجوات، وترقيةُ عدٍّ إلى حاجبٍ تُحوّل التصنيفَ إلى ضغطٍ على الحالة. البندان المُدخَلان لا يعتمدان على ذلك العدّ بحال: أحدهما تناقضٌ صريح والآخر شاهدٌ في الشجرة.

**وقيدُ تصميمٍ منطوق:** الحارسُ لا يطالب مدخلاً `open` بشاهدِ إغلاق. السبب أنّ ذلك يجعل **إعادةَ فتحِ** فجوةٍ وُسِمت مرّةً مستحيلةً — صنفُ «حارسٍ لا يقبل حالةً صادقة» المُغلَق في #1026؛ ويحرسه `test_a_reopening_is_never_asked_for_a_closure_witness` وطفرةٌ مسجَّلة تقتله إن سقط شرطُ الاتّجاه.
## 2026-09-19 — #1030: الإصلاح من سلسلة التوليد القانونية
...
**وقرارٌ ثانٍ:** الحسمُ يُوسَم بـ`resolved_at` ولا يُحذَف. السبب: جدولٌ يُحذَف منه عند النجاح يجعل الفراغَ يحمل معنيين متناقضين — «لم يُؤجَّل شيءٌ قطّ» و«أُسقط كلُّ شيءٍ بصمت» — وهو اللبسُ الذي وُجِد الجدولُ لإزالته.

**وثالثٌ بالسلب:** لم يُمَسّ `reconcile_soil_readings` ولم يُدَّعَ له سجلُّ تأجيل. السبب: مسحُه بلا محمولِ أهليّة ولا تخطٍّ على مستوى الصفّ، فلا صفَّ يعبره المؤشّرُ بلا معالجة — الصنفُ لا ينطبق، وتوسيعُ العلاج إليه كان سيُضيف جدولاً لا يكتبه شيء.

**ورابعٌ بالسلب:** لم يُلمَس مشروعُ Railway الحيّ `sahool-staging`، **ولا تتوقّف الشريحةُ على Railway بحال**. السبب: موضعُ v231 قبل `v206` في `MANIFEST.txt` يفرضه عقدٌ **قائمٌ في المستودع** (تسعةُ اختباراتٍ ساكنة + `apply_in_compose.sh` وهو سكربتُ خدمةِ `sahool-migrate` في compose)، وRailway مستهلِكٌ له لا مصدرُه. وكان تأطيري الأوّل «ارتباطاً بـRailway» تضخيماً لعقدٍ عامّ إلى اشتراطِ منصّةٍ واحدة؛ صحّحه المالك: الارتباطُ **مقترَحٌ عند الحاجة أو للاختبار لا إلزاميّ**.

## 2026-09-19 · تعطيلُ البند المشروط لمراجعة مالكي الكود — أمرُ مالكٍ صريح

**القرار:** البندُ المشروطُ في [`scripts/ci/branch_protection_contract_guard.py`](../../scripts/ci/branch_protection_contract_guard.py)
(`code_owner_violations`، يُطلَق عند مسّ `docs/architecture/gates/adjudications/`) **لم يعد يحجب**.
يبقى يُقاس ويُطبَع سطراً إرشاديّاً وسطرَ `ADVISORY`.

**السبب:** البندُ يشترط `require_code_owner_review = true` في Ruleset الفرع — إعدادُ GitHub لا شيءٌ في
الشجرة. فكلُّ PR تحمل تفويضَ GATE-01 كانت غيرَ قابلةٍ للدمج إلى أن يُغيَّر إعدادٌ لا يملكه وكيل. عُرِضت
ثلاثةُ بدائل (تفعيلُ الإعداد · إخراجُ التفويض من الشريحة · تعطيلُ البند) واختار المالكُ الثالثَ صراحةً
بعد أن بدأ تنفيذُ الثاني.

**الثمنُ المُعلَن:** `GATE01-AUTHORIZATION-ORIGIN-UNENFORCED-01` تعود من `mitigated` إلى `open`.
تفويضُ GATE-01 صار **ذاتيَّ الإصدار بلا منشأٍ مفروض**. والحدُّ مكتوبٌ في متن #1033 وفي رأس الحارس.

**ما يُبقي هذا على حدّ الصدق:** التعطيلُ **مُعلَن** — القياسُ باقٍ ومُثبَتٌ بالتكذيب (الطفرتان ١٧ و١٩
تموتان على شاهدَين يقيسان المخرَج)، والطفرةُ ١٨ قُوِّي شاهدُها بعد أن كشف التعطيلُ أنّها ستنجو.
تعطيلٌ صامتٌ كان سيجعل المستودع يظنّ الشرطَ نافذاً وهو ليس كذلك — وذاك أسوأُ من غيابه.

**الرجوع:** إعادةُ `advisory` إلى `problems` في `main()` — سطرٌ واحد. وشرطُ الإغلاق الأصليُّ قائم:
`Rulesets → main → Require review from Code Owners`. وموافقةُ `haithmgarallah-ye` على `50e0a54a`
(`review_id=5256640491`, COLLABORATOR, مالكُ كودٍ في [`.github/CODEOWNERS:19`](../../.github/CODEOWNERS))
...
## 2026-09-19 · تنفيذُ مقترحَي P1 وP2، وتأجيلُ P3 بقرارِ نطاقٍ مُعلَن

**القرار:** نُفِّذ من مقترحات المراجعة العميقة ما **يُضيف** إنفاذاً ولا يُنقِصه: أجلٌ مفروضٌ
على كلّ تفويض `ISSUED`، وأجلٌ مفروضٌ على إقرار النظير. **ولم يُنفَّذ P3.**

**سببُ تأجيل P3:** هو المقترحُ الوحيدُ الذي **يُنقِص** الحمايةَ — ينقل ثلاثةَ سجلّاتٍ من
التحكيم المقيَّد إلى قاعدةٍ بنيويّة. وعائدُه أكبرُ عائدٍ مقيس (يُحرّر أربعاً من ستّ فجواتٍ
محجوزة)، لكنّه **يُغيّر ما تحرسه البوّابة** — والمقترحُ نفسُه سجّل ذلك «قرارَ مالك»، وحكمُ
المالك في 2026-08-09 كان عن **الأثر الفيزيائيّ**. فتنفيذُه على تفويضٍ عامّ («قم بعمل وفقاً
للمقترحات») توسيعٌ لا يحتمله نصُّه.

**وما لا يُدَّعى:** هذه الشريحةُ لا تسدّ ثقبَ النظير — تجعل قرارَ سدّه **موقوتاً**. والنظيرُ
الحيُّ ما يزال خارج `frozen_paths`، ومُسّ في ٤ من ٤.

**PR:** #1037 · **الأساس:** `ebcf4527`

## 2026-09-19 · تنفيذُ P1 وP2، وتأكيدُ تأجيل P3

**القرار:** نُفِّذ من مقترحات المراجعة العميقة ما **يُضيف** إنفاذاً: أجلٌ مفروضٌ على كلّ
تفويض `ISSUED` (P2)، وأجلٌ مفروضٌ على إقرار النظير (P1). **وأُعيد تأكيدُ تأجيل P3.**

**سببُ التأجيل، وهو نفسُه بعد أمرِ «واصل»:** P3 هو المقترحُ الوحيدُ الذي **يُنقِص** الحماية
— ينقل ثلاثةَ سجلّاتٍ من التحكيم المقيَّد إلى قاعدةٍ بنيويّة. و«واصل بـP1/P2» تسميةٌ
صريحةٌ لاثنين دون الثالث، فلا تُقرأ إذناً فيه.

**وما لا يُدَّعى:** هذه الشريحةُ لا تسدّ ثقبَ النظير — تجعل قرارَ سدّه **موقوتاً**
(‏2026-11-30). والنظيرُ الحيُّ ما يزال خارج `frozen_paths`، ومُسّ في ٤ من ٤.

**PR:** #1037 · **الأساس:** `ac26f1d2`

## 2026-09-20 — مُرساةٌ تُختار بالقياس، واتّجاهٌ مُسجَّلٌ يُكذَّب قبل تنفيذه

**القرار ①:** أُغلِقت `BRAIN-TRANSITION-GUARD-MATCHES-A-DISCUSSED-POLICY-STATE-01`
بمُرساةٍ **غيرِ** المُسجَّلة في الفجوة. **السبب مقيسٌ لا ذوق:** الاتّجاهُ المكتوب كان
«الإرساءُ على العنوان» بحجّة أنّ الادّعاء لا يقع في المتن، والقياسُ يُكذِّب الحجّة —
٢٩ عنواناً و٤ أسطرِ حالةٍ من ٤٢٨ سطراً، و`**SAM2** closed` ادّعاءٌ في نثرٍ جارٍ.
المُنفَّذ: **الخطُّ الطباعيّ** مُرساةً (` ` · «» · “”)، مع استثناءِ موضع الادّعاء
القانونيّ من الإعفاء، ومنعِ الاقتباس من أن يُعفي إسناداً بقيمةٍ موجبة.

**القرار ②:** رسالةُ `JOURNAL_SHRANK` صارت **مشتقّةً من موضع الانخفاض**. السبب: #1036
حُجِبت ورأسُها أكبرُ من الأساس في السجلّات الأربعة، والرسالةُ الثابتة تصف علاجاً لا
يعمل على حالتها. **القاعدةُ لم تتغيّر؛ المتغيّرُ ما تقوله عند الحجب.**

**وما لا يُدَّعى:** الفجوةُ الأولى تركت أثراً واحداً محجوباً بصدق (`**البوّابة CLOSED
عالميّاً**` نثراً عارياً)، مُسجَّلاً بحرفه في الشاهد لا مطموساً.

**الأساس:** `a0bba343`

## 2026-09-20 — شريحةٌ واحدة لا ثلاث، وأرضيّةُ صفرٍ حيث المقيسُ صفر

**القرار ③:** أُضيف `noncanonical_heading_level` إلى القياس وحُجِب عليه **بأرضيّة
صفر**، لا براتشِت. السبب مقيس: اليتامى ٢١١ لأنّ أكثرَ العناوين سردٌ تاريخيّ، أمّا
العناوينُ العميقةُ ذاتُ شكل المعرّف فواحدةٌ قبل التصحيح وصفرٌ بعده.

**وقرارُ شكلٍ تابع:** الشكلُ المقبول **أضيقُ** من `ID` — الموصولُ بشَرطة وحدَه —
لأنّ الكلمة الكبيرة المفردة تلتقط عنوانَي قسمٍ ليسا مدخلَي فجوة.

**وقرارُ إجراء:** طلبَ المالكُ فرعاً/PR جديداً من `main@a0bba343`، وهذا الفرعُ منه
بالضبط. **وضُمَّت الشريحةُ إلى الشريحتين السابقتين في PR واحدة** بدل فتح ثانية:
الثلاثُ تمسّ `sahool-brain/gaps/registry.md` وحرّاسَه، وPRان متوازيتان عليه تُنتجان
تعارضاً في ملفٍّ إلحاقيٍّ طويل — وهو بعينه ما أنتج انخفاضَ #1036 المحفور في التاريخ.

**الأساس:** `a0bba343`

## 2026-09-20 — تثبيتُ `ruff` يُشتقّ من `ci.yml` ولا يُكتَب في الأداة

**القرار الأوّل — الاشتقاق لا النسخ.** كان بالإمكان كتابةُ `0.15.8` في
`preflight.sh` سطراً واحداً. رُفِض: عقدُ `tests_v9/test_local_preflight_contract.py`
يفتتح برفضِ «رقمٍ يُكتَب في السكربت» لأنّه يبيت ويُقرَأ قياساً؛ ونسخةٌ ثانيةٌ من
الإصدار كانت ستنحرف عن `ci.yml` يومَ تُرقّى، فيُعيد العطلَ بوجهٍ ثانٍ. **السبب:**
البوّابةُ الحاجبةُ هي مصدرُ الحقيقة، والأداةُ المحلّيّة تتبعها.

**القرار الثاني — تخطٍّ مُعلَن لا حجب.** عند اختلاف الإصدار لا تفشل البوّابتان بل
تُتخطّيان بصوتٍ عالٍ وتُعَدّان اثنتين. **السبب:** سابقةٌ مكتوبةٌ في الملفّ نفسِه
(شرطُ FastAPI في ٨ج) بحجّةٍ مقيسة — «حمرةٌ بيئيّة تُقرأ شيفريّةً أسوأ من تخطٍّ
مُعلَن»؛ والحجبُ كان سيجعلها «بوّابةً لا تُغلَق بعملٍ صحيح» لمن بيئتُه طليقة.

**القرار الثالث — ملفّا المتطلّبات يبقيان `ruff>=0.4.0`.** لم يُثبَّتا رغم أنّهما
مصدرُ الانجراف. **السبب:** تثبيتُهما يصنع **نسختين إضافيّتين** تنحرفان عن `ci.yml`
— وهو نفسُ العطل المُعالَج. الأداةُ تكشف الانجراف وتسمّي علاجَه بسطرٍ يُنسَخ؛
وتوحيدُ مصدر التثبيت (مثلاً `-c` على `ci.yml`) **قرارُ مالك** لم يُطلَب.

**القرار الرابع — تسجيلُ طفرتين لا الاكتفاء بزرعٍ يدويّ.** زُرِعت ستٌّ يدويّاً
وكلُّها مقتولة، لكنّ اثنتين سُجِّلتا في `guard_mutation_registry.json` تحت
`scripts/ci/preflight.sh`. **السبب:** ما لا يُسجَّل لا تزرعه المكنسة، فيبقى مُثبَتاً
في جلسةٍ واحدة فقط. وشرطُ التسجيل مُستوفًى: الاختباران بلا `skipif` وبلا اعتمادٍ
على تاريخ git، فيعملان في استنساخ CI بعمق ١.

**الأساس:** `98a61c5f`

## 2026-09-20 (تتمّة) — توحيدُ مصدر التثبيت: قرارُ المالك نُفِّذ

**القرارُ الثالث في مدخلة اليوم نُقِض بأمرِ المالك.** كان: «ملفّا المتطلّبات يبقيان
`ruff>=0.4.0` عمداً؛ توحيدُ المصدر قرارُ مالك لم يُطلَب». صدر الطلب، فنُفِّذ.

**وكيف نُفِّذ يخالف أقصرَ طريق:** لم تُثبَّت الملفّاتُ الأربعة بالرقم نفسِه — ذاك كان
يصنع أربع نسخٍ تنحرف، أي العطلَ المُعالَج بوجهٍ أوسع، وهو بالضبط سببُ رفضي الأوّل.
بل **رقمٌ واحد** في `constraints-ci-tools.txt` و`-c` في المستهلكين.

**والمستهلكُ الخامس أُدرِج عمداً:** كتلةُ §٢ في الرنبوك تُنسَخ وتُلصَق فهي مستهلكٌ
بحكم الواقع، وكانت تحمل الرقم. والشاهدُ يفحص **أمراً يُنسَخ** لا كلَّ ذكرٍ للرقم —
وإلّا منع §٣.٣ من روايةِ العطل الذي وقع.

**وبندُ الحياة (رمز الخروج `6`):** يُتحقَّق أنّ `ci.yml` ما تزال تُثبّت عبر ملفّ القيود.
**السبب:** ملفُّ قيودٍ هجرته البوّابةُ إعلانٌ لا يحكم شيئاً، والمقارنةُ عندئذٍ تقيس
ملفّاً لا تقرؤه CI — وهو الصنفُ الذي يطارده هذا المستودع.

**وقرارُ صدق:** أُعيد قياسُ دلالة `-c` بعد أن تبيّن أنّ قياسي الأوّل لم يفصل بين
احتمالَين (تصادف مجلّدُ العمل ودليلُ الملفّ). النتيجةُ لم تتغيّر، لكنّ القياسَ الأوّل
لم يكن يُثبِتها.

**الأساس:** `78b82464`
## 2026-09-20 — الرقمُ المكتوبُ بيدٍ يُربَط بالقياس أو يُعلِن شجرتَه

**القرار:** كلُّ ادّعاءٍ عدديٍّ مسمّى في `gaps/registry.md` يُقارَن بمخرج
`gap_registry_measure.py`، **إلّا** ما يذكر بصمةَ الشجرة التي قِيس عندها.

**السبب مقيسٌ لا ذوق:** حجبُ كلِّ الأعداد كان يُحمِّر على تسعة ادّعاءاتٍ تاريخيّةٍ
صحيحة، وحمايةٌ تُوقِف العملَ تُطفَأ فتصير صفراً. وإعفاءُ كلِّها كان يُبقي العطلَ الذي
وقع. فالمُرساةُ **إعلانُ الزمن**: رقمٌ عن الحاضر يصدق الآن، ورقمٌ عن الماضي يقول أيَّ
شجرة.

**وحدُّ صدقٍ في القرار:** هذا يربط سجلَّ الفجوات وحدَه. متنُ الـPR — وهو ما حمل
`273`/`51` — **خارج الشجرة، ولا يبلغه حارس**. لا أدّعي أنّ الصنفَ أُغلِق كلُّه.
## 2026-09-20 — «لم يُقَس» حالةٌ في البنية لا نيّةٌ في تعليق

**القرار:** كلُّ سطحٍ غيرِ مقيسٍ في جرد التشخيص يحمل `measurement_state: not_measured`
صراحةً بدل `[]` عارية. **السبب:** التعليقُ فوق الدالّة كان يقول الصوابَ ولا يقرؤه أحد؛
والمصنوعةُ تُقرأ آليّاً. `measured` + صفرُ صفوفٍ **دعوى**، و`not_measured` + صفرُ صفوفٍ
**امتناعٌ عن الدعوى**.

**وقرارٌ ثانٍ:** لا تُرقّى حافّةٌ معلَنةٌ إلى `resolved` بلا موضعِ استدعاءٍ مصدريٍّ أو
أثرٍ تشغيليّ — ولو بقي العدّادُ ٠ مقابل ٧٢٢. **حسمٌ مُختلَقٌ ليخضرّ تقريرٌ أسوأُ من
عدّادٍ صادقٍ عند الصفر**، وهذا شرطُ المالك نفسُه في خريطة البند ١.
...
## 2026-09-20 — البند ٤: رُفِض «تصحيحُ العقد» رغم أنّه العلاجُ المكتوب

**القرار:** لم أُصحّح `db_ownership.yml` لأيٍّ من الـ٧٤، مع أنّ نصَّ الأساس يُجيزه
صراحةً («أو بتصحيح العقد بأساسٍ مُعلَن»)، ومع أنّ القياسَ أعطاني مبرّراً قويّاً
(٥٥ مالكاً لا يذكر جدولَه في شجرته إطلاقاً).

**السبب مقيس:** `owner` في العقد و`target_owner` في خريطة الاستخراج **نيّةٌ واحدة**،
و٧٤/٧٤ من المخالفات مالكُها هدفٌ حيّ. فالتصحيحُ كان سيمحو الهدفَ ويُنزِل العدّادَ
إلى الصفر بينما البنيةُ تنحدر — أي **هدمُ المقياس باسم إصلاحه**.

**وقرارٌ ثانٍ — أرضيّةُ صفرٍ لا راتشِت.** الشجرةُ تحمل صفرَ كاتبٍ إضافيّ اليوم، فلا
حاجةَ إلى أساسٍ بدَين. وراتشِتٌ يبدأ بدَينٍ يُدرَّب قارئُه على تجاوزه.

**وقرارٌ ثالث — لا تجميدَ للعقد.** حصرُ النطاق بالأهداف **الحيّة** يُبقي بابَ التقاعد
المشروع مفتوحاً: تُحدَّث الخريطةُ في التغيير نفسِه فيصير التصحيحُ مشروعاً. بلا ذلك
كان الحارسُ يمنع عملاً صحيحاً، وهو الصنفُ الذي يُطفَأ.

**وما تُرِك للمالك:** إنقاصُ الـ٧٤ قرارُ برنامجٍ لا شريحة — يحتاج نقلَ الكتابة أي
الاستخراجَ نفسَه. لم أبدأه ولم أدّعِ تقدّماً فيه.

**الأساس:** `69e9d447` · **مدموجٌ:** #1047 ⇒ `b3109c7b` (squash على `main`، أمرُ المالك
2026-09-20، بعد `29e528c2`).

**وقياسُ الدمج مُثبَّتٌ لا موصوف:** ثُبِّت `expectedHeadSha=13da351c` في طلب الدمج، فلو
تحرّك الرأسُ بين القياس والأمر لرُفِض الطلبُ بدل أن يمرّ على شيفرةٍ لم تُقَس. والحالةُ
عند الدمج: ٦٢ فحصاً ناجحاً · ١ متخطّى (`runtime-stack-e2e-chaos`) · صفرُ أحمر ·
`mergeable_state: clean`.

**وعطلُ قياسٍ في عملي يُسجَّل لأنّه صنفٌ يتكرّر:** شغّلتُ الطبقةَ الكاملة من
`preflight.sh` **بلا تفعيل الـvenv**، فوقعت على مفسّر النظام المعطوب
(`pyo3_runtime.PanicException` من `cryptography`) وأعطت **٨ إخفاقات** — سبعةٌ منها
انهيارُ استيراد، ومنها أربعُ خطواتِ طفراتٍ قالت «حمرّ **بغير** الاختبار المُتوقَّع»
فبدت كأنّ الحرّاس فقدوا تكذيبَهم. والثامن (`schema_validation_guard --check`) مرّ
نظيفاً تحت الـvenv (`20 مخطَّطاً · rc=0`) والشجرةُ `dirty=0` — فسقط تفسيرُ «انحرافٌ
...
## 2026-09-20 — البنود ٥/٦/٨: قياسُ الحدّ بدل افتراضه، وتسجيلُ فقدٍ صامتٍ في طبقة النقل

**القرار الأوّل — الحدُّ البيئيّ يُقاس على الآلة قبل إعلانه.** السابقةُ في الجلسة نفسِها:
«الشهادةُ الحيّة مستحيلة» أُعلنت بعد فحص ثلاثة منافذ ولم يُفحَص المُضيف، وكان PostgreSQL
مثبَّتاً. فهنا قِيس كلُّ مكوّن: GPU غائب (`nvidia-smi` · `/dev/nvidia*`) · docker متوقّف ·
`torch` غائب ⇒ البند ٥ محجوبٌ عتاديّاً **بقياس** ويُدرَج BLOCKED بمدخله المفقود، لا
«مستحيل». و`nats-server` حُمِّل من الإصدارات فعمل ⇒ البند ٦ **قابلٌ للقياس** هنا.

**القرار الثاني — القبولُ يُقاس بإصدار الصورة لا بالأحدث.** `nats` في بايثون كان
حزمةَ فضاءِ أسماءٍ فارغة (`__file__=None`)؛ ثُبِّت `nats-py` 2.16.0 ثمّ أُعيد إلى
**2.10.0** كما يثبّته `api/requirements.txt`. شاهدٌ يربط إصدارَ المصنوعة بالتثبيت.

**القرار الثالث — خطوةُ قياسٍ خامسة خارج الرنبوك، لأنّ شكلَ النداء أثار شبهة.**
`_start_outbox_worker` يمرّر `nc.publish` الأساسيَّ لا `js.publish`. القياس: حدثٌ بلا
دفقٍ يغطّيه وُسِم `sent` بلا `last_error` **ولم يُحفَظ**. سُجِّل
`OUTBOX-RELAY-MARKS-SENT-WITHOUT-JETSTREAM-ACK-01` (open) بشرطِ إغلاقٍ قابلٍ للتكذيب،
**ولم يُصلَح هنا**: الإصلاحُ يمسّ عقدَ التسليم (PubAck أم توثيقٌ صريح + فحصُ تغطية) وهو
قرارُ تصميمٍ لا سطر. وقلبُ `FEATURE_NATS_PUBLISHERS` في الإنتاج قبل حسمه خطأٌ مقيس.

**القرار الرابع — سجلُّ الإثبات إلحاقيٌّ ومقيَّدٌ بـSHA، ونافذةُ الـ١٤ يوماً لا تُقاس
قبل مرورها.** `docs/evidence/operational_evidence_log.md` يحمل ما قِيس وما لم يُقَس
جنباً إلى جنب، و`not_yet_measurable` صريحةٌ حتّى 2026-10-04. الزمنُ لا يُعوَّض بقياسٍ
أكثف، وادّعاءُ استقرارٍ في يومٍ واحد هو الصنفُ الذي بُني السجلُّ لمنعه.

**ما تُرِك للمالك:** حكمُ Riva (مطلوبٌ/اختياريّ) · عدّاءٌ بـGPU لشطر البند ٥ ·
اختيارُ علاج الفجوة الجديدة (PubAck أم توثيق) · وإجراءُ انتقال مستهلكي الإشعارات على
Railway المرفوضُ من المراجعة الآليّة.
...
**إلحاقٌ (2026-09-20):** الشريحةُ أعلاه صارت #1051، دُمجت squash في `5ee9523d` على `main@cfb47067` بعد ٦٧ فحصاً أخضر/متخطّى.

## 2026-09-21 — مراجعةُ Railway الخارجيّة: يُصلَح في الشجرة ما يقع فيها، ويُسمّى للمالك ما يقع في النشر

**القرار الأوّل — الرفضُ لا الاشتقاق.** كان يمكن اشتقاقُ ختم الهويّة من `RAILWAY_GIT_COMMIT_SHA` فيُلغى المتغيّرُ اليدويّ. رُفض لأنّ الرنبوك قاس فراغَه في بناءٍ مُشغَّلٍ بتغيير إعداد، والاشتقاقُ من قيمةٍ تفرغ يُنتج ختماً فارغاً **بصمت** — أسوأ من ختمٍ قديمٍ يُرفَض بصوت. فالمتغيّرُ اليدويّ يبقى المصدرَ، والتشغيلُ من GitHub يُكذِّبه إن خالفه.

**القرار الثاني — `open` لا `fixed` لفجوة الختم.** الرفضُ في المصدر لا يُقاس إلّا ببناءٍ على Railway لم يقع؛ والإغلاقُ مشروطٌ بمطابقة `/runtime-identity` بعد البناء. والأثرُ التشغيليّ (فشلُ أوّل بناءٍ للخدمتين حتّى يُحدَّث المتغيّر) مُعلَنٌ سلفاً في الفجوة والرنبوك والـPR — لا يُكتشَف عرضاً.

**القرار الثالث — لا لمسَ لإعدادات Railway بلا كلمة المالك.** المراجعُ لم يغيّرها وأنا كذلك: `set-variables`/`redeploy` على بيئة `production` فعلٌ خارجيٌّ يُؤكَّد أوّلاً، وقيمُ المتغيّرات محجوبةٌ عن هذه الجلسة أصلاً. ما أستطيعه بكلمةٍ منه: ضبطُ `SAHOOL_GIT_SHA` في `auth-main` و`guardrails-engine` إلى الالتزام المبنيّ ثمّ بناءٌ من GitHub وقياسُ الهويّة.

**القرار الرابع — المراجعةُ الخارجيّة تُحفَظ وتُربَط ببصمتها، والانحرافُ عن الأصل يُعلَن لا يُخفى.** نُسِخت بايتاً بايت أوّلاً؛ ثمّ ضاعت مع الحاوية فأُعيدت كتابتُها من القراءة الكاملة: بصمةُ المخزَّن `90c6d20c…` وبصمةُ الأصل `b0918464…`، كلتاهما في E-03، والشاهدُ يربط المخزَّن. وتصحيحُ E-02 كُتب مدخلةً جديدة لا تعديلاً — السجلُّ إلحاقيّ.

**القرار الخامس — الدفعُ بعد البوّابات الثابتة لا بعد الأجنحة.** إيداعٌ محلّيٌّ انتظر جناحَين (١٥ دقيقة) فضاع بإعادة تهيئة الحاوية. الأجنحةُ تُقاس على العدّاء أصلاً؛ الاحتفاظُ بالعمل غيرِ المدفوع لأجلها خسارةٌ مقيسة لا حذر.

**ما تُرِك للمالك:** RW-01 (نشرُ `supervisor-agent` ورفاقه أو توثيقُ استبعادها) · RW-02 (ترقيةُ المنصّة من `fa92a1f7` وأعلامُها) · إذنُ تحديث المتغيّرين وإعادةِ البناء · الأختُ في `SettingsPage.tsx` · إعادةُ رفع الحزمة لاستعادة المطابقة البايتيّة.

**الأساس:** `f35c46b` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

**إلحاقٌ (2026-09-21):** الشريحةُ أعلاه صارت #1053، دُمجت squash في `c5643866` على `main@f35c46b` بعد ٧٤ فحصاً بلا أحمر. **وقرارٌ لاحق — `fixed` بطبقة البناء لا بطبقة التشغيل، مُعلَناً:** الرفضُ قِيس في بناءٍ إنتاجيٍّ فاشلٍ حقيقيّ (09:27 UTC) والختمُ الصحيح قِيس في بناءٍ ناجح (09:54)؛ القراءةُ الحيّة لـ`/runtime-identity` تتبع الملفَّ المخبوز حتماً لكنّها لم تُقَس من هنا، فكُتبت حدّاً لا استنتاجاً. ولم أُغيّر متغيّراً: وقع التحديثُ من طرفٍ آخر.

## 2026-09-21 — البند ٤، الدفعةُ الأولى: الفرزُ قياسٌ والصنفُ مشتقّ، والحلُّ قرارٌ مسمّى

**القرار الأوّل — الصنفُ يُشتقّ بقاعدةٍ لا يُختار.** كان يمكن كتابةُ ٧٢ حكماً بيدٍ؛ رُفض لأنّ الحكمَ اليدويّ لا يُكذَّب ولا يتبع الشجرة. الأبعادُ تُقاس بالمحرّك الحاجب نفسِه والصنفُ دالّةٌ عليها في الشاهد (`derive_category`)، فالمصنوعةُ تحمرّ حين تنحرف عن الشجرة أو عن القاعدة. الرأيُ (`remedy` · `decision_owner`) حقلٌ مفصول ولا يُقاس.

**القرار الثاني — تصحيحُ الهويّة في الماسح لا في العقد.** إعادةُ تسمية `erp-bridge` في العقد إلى `odoo-bridge` كانت ستمرّ الحارسَ لكنّها تنقض compose (`sahool-erp-bridge`) وخريطةَ الاستخراج (١٧ إحالة) وحارسَ المحاذاة. الهويّةُ الحقيقيّة هي هويّةُ compose، فرُبط المجلّدُ بها في مكانٍ واحد بشاهدٍ يرفض ربطاً بلا دليلٍ من compose — لا إعفاء.

**القرار الثالث — لا توسيعَ لـ`writers` ولا حلَّ في هذه الدفعة.** ٦٢ من ٧٢ مالكُها لا يكتب الجدولَ قطّ؛ توسيعُ `writers` كان سيُخفِض العدّاد بلا نقلِ سطر وهو ما يرفضه `ownership_extraction_alignment_guard` باسمه. والاستخراجُ إلى مالكٍ غيرِ منشور على Railway (agriai · weather · erp-bridge · actuator · rag · soil) يكسر الإنتاج؛ فالقرارُ للمالك: أيُّ استخراجٍ مقصود، وأيُّ تأجيلٍ يُعلَن.

**الأساس:** `27dc7a9a` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

**إلحاقٌ (2026-09-21):** الشريحةُ أعلاه صارت #1054، دُمجت squash في `b5e63fd9` بعد ٦٣ فحصاً بلا أحمر.

## 2026-09-21 — تصحيحُ الفرز: العددُ بُعدٌ لا صنف، والاشتقاقُ في مكانٍ واحد

**القرار الأوّل — الوصفُ يُشتقّ من الصنف والعدد معاً.** خطأُ «الكاتبُ الوحيد» لم يكن في الصفّ بل في اشتقاق الوصف من الصنف وحدَه؛ بدل توسيع الصنف (فينكسر التوافق) أُضيف بُعدٌ مستقلٌّ للعدد ودالّةُ وصفٍ تقرؤه. الصنفُ يبقى ما كان، والصفُّ لا يُحذَف ولا يُخفَّض الأساس.

**القرار الثاني — منطقُ الاشتقاق يعيش في الحارس لا في سكربتٍ خارج الشجرة.** المولّدُ الأوّل كان في مساحة العمل فلا يُقاس ولا يتبع؛ صار `--generate` يولّد الفرزَ مع الأساس و`--check` يقيس بياتَه صفّاً صفّاً، والشواهدُ تختبر الدوالَّ بحالاتٍ مضادّة لا بإعادة إنتاج النتيجة. لا حارسَ جديد — امتدادٌ لحارسٍ قائم بمصنوعته.

**القرار الثالث — قرارُ المالك في مسار التنفيذ يُقيَّد كما جاء:** لا نقلَ ملكيّةٍ جماعيّاً (الثلاثةُ `auth` مرشّحةٌ لمراجعةٍ محدَّدة؛ `users` مسارَا كتابة؛ `audit_log` قرارٌ مستقلّ) · الاستخراجُ مرحليٌّ بإثبات المالك حيّاً وقتَ الانتقال لا بلقطة · غيرُ المنشور يُؤجَّل بإعلان · «الخمسُ + الخمس» تُفصَل بنطاقاتها · تشغيلُ سكربتٍ بدور مالك الجدول ليس حلّاً · `fixed` لختم البناء يبقى مفصولاً عن التحقّق الحيّ (النشرُ المحدَّد لا رأسُ main).

**الأساس:** `b5e63fd9` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

**قرارٌ لاحق (2026-09-21) — الاشتقاقُ مع الرفض، لا الرفضُ بدلَه — نقضٌ لقراري الأوّل في شريحة #1053.** الحجّةُ الأولى («قيمةٌ تفرغ ⇒ ختمٌ فارغٌ بصمت») سقطت بالقياس: الفراغُ يُمسَك بفحص الأربعين خانةً القائم، فليس صامتاً. والرفضُ وحدَه قِيس ضررُه: بناءان فاشلان على أوّل دمجٍ بعده. فصار: غيرُ مثبَّت ⇒ اشتقاق · مثبَّتٌ مخالف ⇒ رفض · فراغٌ مزدوج ⇒ فشلٌ صريح. وما يبقى للمالك: حذفُ المتغيّر المثبَّت من خدمات النشر التلقائيّ (لم ألمسه).

**قرارٌ لاحق (2026-09-21، بعد دمج #1055 `d99c71c`) — مولِّدُ الفرز يتبع مخطّطَ الماسح v2 المدموج ولا يفرض مخطّطاً ثالثاً.** `deployment_evidence` كائنٌ مؤرَّخ داخل الصفّ (لا حقلين مسطّحين)، و`decision_owner` مفرداتُه اثنتان كما يفرضها شاهدُ #1055 — تعدّدُ الكُتّاب غيرِ المالك يبقى قرارَ **المالك** والمراجعةُ الهندسيّةُ السابقةُ له في نصّ العلاج؛ قيمةٌ ثالثة كانت ستُشعّب الحقلَ بلا سقف. **و`excluded_write_sites` من قاعدة الماسح نفسِها لا من قائمةٍ مكتوبة:** كلُّ موضعِ كتابةٍ في ملفٍّ خلفيّتُه SQLite وحدَها (16 على `c012f1f`) مع `contract_state` يقول أكان الإسقاطُ سيغيّر الراتشِت أم لا — #1055 سرد ثلاثةً (ما كان في الأساس أو حوله) وهي قاعدةٌ تاريخيّةٌ لا تُشتقّ من الشجرة. **وضجيجُ `_WRITE` (`DO UPDATE SET` ⇒ `set::…`، 9 مفاتيح على `main`) لا يُصلَح في هذه الشريحة:** إصلاحُه يغيّر `write_sites()` وسطحَ الجرد معاً، فيُقاس في شريحة قياسٍ مستقلّة (فجوةٌ مفتوحة). **و`measured_on` يُعاد ختمُه بإعادة التوليد على الشجرة المصحَّحة** لا بتحريره: الأساسُ 71 = 71 والحقلُ يسمّي التزاماً يعمل ماسحُه.

**قرارٌ لاحق (2026-09-21، بعد قياس `fdb7fee6`) — معرّفُ البناء يُشتقّ من الالتزام المبنيّ حين يغيب معرّفُ النشر، لا يُترَك فشلاً.** الحجّةُ: المقيسُ أنّ Railway لا يمرّر `RAILWAY_DEPLOYMENT_ID` إلى البناء (فارغٌ في السطر المُوسَّع بينما `RAILWAY_GIT_COMMIT_SHA` مملوء)، فبقاءُ الفشل هناك يعني أنّ الطريقَ الوحيدَ لبناءٍ ناجح هو تثبيتٌ يدويّ لـ`SAHOOL_BUILD_ID` — وهو الصنفُ نفسُه الذي أُغلق لـ`SAHOOL_GIT_SHA`. `railway-<sha[:12]>` يسمّي البناءَ بمصدره صدقاً، والفراغُ الكامل (لا تثبيت ولا نشر ولا التزام) يبقى فشلاً صريحاً.

## 2026-09-21 — نحوُ SQL يُعرِّف الكتابة، لا قائمةُ لفظاتٍ ممنوعة

**القرار الأوّل — الشدُّ بشرطِ استمرارٍ نحويّ لا بقائمةِ استثناءات.** العلاجُ الأسهلُ كان حجبَ اللفظات المرصودة (`set` · `skip` · `the` …) — وهو يبيت يومَ تظهر لفظةٌ سادسة، ويُخفي أنّ السببَ واحد: الماسحُ كان يقرأ الكلمةَ ولا يقرأ العبارة. فصار لكلّ عبارةٍ شرطٌ يفرضه معيارُ SQL أصلاً (`UPDATE … SET` · `INSERT INTO` قائمةَ أعمدةٍ أو مصدرَ صفوف · `DELETE FROM` فاصلاً نحويّاً أو نهاية)، فسقط الأربعةُ والعشرون بقاعدةٍ واحدة لا بأربعةٍ وعشرين استثناءً.

**القرار الثاني — الشدُّ لا يُقبَل بلا شاهدٍ إيجابيّ.** كلُّ تضييقٍ يخاطر بإسقاطِ كتابةٍ حقيقيّة، وذلك عطلٌ أسوأُ من الضجيج: الحارسُ يقيس ملكيّةَ الكتابة، وموضعٌ ساقطٌ يعني انحرافاً يمرّ أخضر. فعشرون صياغةً مشروعة (كنية · `ONLY` · `DEFAULT VALUES` · `INSERT … SELECT` · CTE) شاهدٌ حاجب، وطفرةٌ مُسجَّلة تُضيّق النمطَ دون النحو يجب أن تُسقِطها.

**القرار الثالث — الأساسُ لا يُعاد تصفيرُه ولا يُخفَّض بهذه الشريحة.** `survey()` ٧١ قبلَ الشدّ وبعدَه؛ الشدُّ يصحّح **ما يُقال** عن الشجرة لا يقلّل الدَّين. ولو تحرّك الراتشِتُ هنا لصار تصحيحُ قياسٍ يُقرأ إنجازَ استخراج.

**القرار الرابع — ثلاثةُ ملفّاتٍ فرقُها `measured_on` وحدَه تُعاد إلى بايتات `main`.** طزاجةُ مراسيها شأنُ شرائحها، وإدخالُها هنا يُوسّع الفرقَ بلا قياسٍ يسنده. وكلفةُ ذلك مقيسة (مجموعُ تحقّقِ حزمةٍ بُنيت قبل الإرجاع) لا مُهمَلة.

**الأساس:** `0606bb67` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

**قرارٌ لاحق (2026-09-21، بعد دمج #1057) — الإفراغُ بدل الحذف، مُعلَناً.** `set-variables` لا يحذف اسماً ووكيلُ Railway على حدّ الاستخدام، فأُفرغ المتغيّران بقيمةٍ فارغة: مكافئٌ للحذف في برنامج الختم (`'' or trig` · `os.environ.get(...) or trig`) ومقيسٌ ببناءَين ناجحَين؛ الحذفُ الاسميّ يبقى للمالك من اللوحة ولا يغيّر السلوك. **ولا يُدَّعى** أنّ النشرَ التلقائيّ صار بلا يدٍ إلّا بعد أوّل دمجٍ لاحق يُقاس.

## 2026-09-21 — الدفترُ الإلحاقيّ يُحرَس بعضويّةٍ في قائمةٍ واحدة، لا بحارسٍ ثانٍ

**القرار الأوّل — لا حارسَ جديد.** العطلُ (`AN-APPEND-ONLY-JOURNAL-OUTSIDE-THE-APPEND-ONLY-LIST-01`) لم يكن نقصَ آليّة بل نقصَ عضويّة: `brain_append_only_guard` قائمٌ وصحيحٌ ويستورد قائمتَه من `resolve_merge_conflicts.APPEND_ONLY`. فالعلاجُ مدخلةٌ واحدة في تلك القائمة — تُفعّل الحارسَ **و**مُحلِّلَ التعارضات معاً. حارسٌ ثانٍ كان سيُنتِج قائمةً ثانية تنحرف، وهو الانحرافُ الذي تقيسه هذه الشجرة مراراً.

**القرار الثاني — العضويّةُ تُسنَد بقياسٍ تاريخيّ لا باحتياط.** قبل الإضافة قِيس كاملُ تاريخ الملفّ: ١٩ زوجَ (التزام، أب) وانكماشٌ واحد هو الحادثةُ نفسُها. إضافةٌ تحجب زوجاً واحداً حقيقيّاً وصفراً من العمل الصحيح ليست تشديداً بل سدُّ ثقب. ولو كانت ستحجب عملاً مشروعاً لكان النقاشُ في القاعدة لا في العضويّة.

**القرار الثالث — الشاهدُ التاريخيّ يُحصَر ولا يُضعَّف الحارس.** إضافةُ ملفٍّ أُنشئ لاحقاً جعلت شاهدَي 2026-08 يحمرّان بـ`JOURNAL_ABSENT_AT_HEAD` — مفارقةٌ زمنيّة لا انحدار. الخياران كانا: تليينُ «الغيابُ عند الرأس يحجب» (وهو بعينه ما يمنع الصفرَ الصامت)، أو حصرُ الشاهدَين في الدفتر الذي هما عنه. اختير الثاني: الشاهدُ يبقى على سؤاله، والقاعدةُ تبقى على حدّها.

**القرار الرابع — ما لم يُعمَّم يُعلَن.** لا شيء يربط آليّاً «ملفٌّ يعلن أنّه إلحاقيّ» بـ«ملفٌّ في القائمة»؛ الشاهدُ الجديد يربطهما لهذا الملفّ وحدَه. التعميمُ (مسحُ الشجرة عن كلّ مَن يعلن ذلك) فجوةٌ مفتوحةٌ مُعلَنة، لا دَينٌ صامت.

**الأساس:** `69806c01` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

**قرارٌ لاحق (2026-09-21) — معرّفُ مدخلةِ الإثبات صنفٌ خامس يُتحقَّق منه، لا استثناءٌ ولا تسجيلٌ كاذب.** الخياراتُ الثلاثة كانت: تسجيلُ `E-2026-09-21-06/07` في سجلّ الفجوات (كذبٌ — إثباتٌ لا عطل)، أو حذفُ المعرّفين من رسالة الالتزام (كتمانٌ يرفضه نصُّ الحارس صراحةً في صنفَي التفويض والحاجب)، أو صنفٌ خامس يُقاس وجودُه في `docs/evidence/operational_evidence_log.md`. اختير الثالث لأنّه **أقوى من الاستثناء**: الاستشارةُ الأمنيّة تُستثنى اضطراراً لأنّ مصدرها خارج الشجرة، وما كان ملفّاً هنا يُقرأ — فيبقى المعرّفُ الملفَّق ساقطاً. ولا يُوسَّع النمطُ إلى بادئة: معرّفُ فجوةٍ يبدأ بالحرف نفسِه ولا يحمل شكلَ التاريخ يبقى فجوةً يُطالَب بتسجيلها. والأمثلةُ المُلفَّقة موضعُها الشواهدُ لا السجلّات: اقتباسُها هنا يُقرأ ادّعاءَ فجوةٍ غيرِ مسجَّلة — قِيس على هذه الشريحة.
## 2026-09-21 — D1 راستر: الكتابةُ عند مالكها، والنيّةُ عند المُرسِل في معاملته، والتسليمُ بعد الالتزام

**القرار:** حلُّ صفَّي `raster_registry::sahool-platform` و`raster_cache_invalidations::sahool-platform` بواجهة كتابةٍ مملوكة في raster-service (`routers/registry_writes.py`)، لا بإضافة `sahool-platform` إلى `writers` (البابُ الذي أغلقته `A-RATCHET-CLEARED-BY-DELETING-WHAT-IT-MEASURES-01`). **ونُقِض قرارٌ أوّل في الشريحة نفسِها:** الصياغةُ الأولى (`9f601a9`) نادت raster-service بـHTTP من داخل `_insert_field_within_tx` — والعقدُ هناك ينصّ «لا I/O خارج conn»، والحقلُ غيرُ مرئيّ لاتّصالٍ آخر قبل COMMIT (حكمُ PostgreSQL)، وتراجعُ المعاملة بعد قبول الأمر يترك إبطالاً زائداً. مراجعةُ المالك سمّت الثلاثة، فصار التصميم: **نيّةٌ دائمة** في `processing_jobs` (مملوكٌ للمنصّة بالعقد؛ إجاراتُه JSON على سابقة `PlatformBootstrapStore` — فلا جدولَ جديد ولا هجرة) تُكتب على اتّصال المعاملة داخل savepoint، **ومُوصِّلٌ بعد الالتزام** (في `api/spatial_sync.py` نفسِها — ميزانيّةُ وحدات المنصّة مجمَّدة) يطالب بإجارة ويُسلّم ولا يُكمِل إلّا بإقرار المالك. الأساسُ `main@69806c0` (بعد #1061 و#1062).

**قراراتٌ فرعيّة مقيسة:** ① المُوصِّل مهمّةُ مُجدوِلٍ داخل العمليّة (`register_default_tasks` تسجّله) لا خدمةَ compose ولا سطراً في `main.py` — الملفُّ على سقفه 2553 سطراً؛ والمهمّةُ تحت `cluster_singleton` فتُنفَّذ بنسخةٍ واحدة لكلّ تكّة. ② مسبحُ الوظائف (دورٌ يتجاوز عزلَ الصفوف) شرطُ التسليم عبر المستأجرين؛ بدونه لا مطالبة — مُعلَنٌ في الوحدة. ③ 404 من المالك تُعاد لا تُنهى: كاشُ مالك الحقل السالب في raster-service (15ث) قد يسبق رؤيةَ حقلٍ التُزم للتوّ؛ 400/401/403/422 دائمة. ④ طرفُ المالك صارم الآن — الحقلُ مرئيٌّ ومملوك وإلّا 404/403 — لأنّ الأوامر تصل بعد COMMIT، فسقط تنازلُ «يحجب عند الإثبات فقط» الذي فرضه HTTP-داخل-المعاملة (والثغرةُ التي سمّاها المراجع: إبطالٌ لمعرّفٍ وهميّ تحت المستأجِر نفسِه). ⑤ مفتاحُ الطلب حتميٌّ من `(reason, field_id, geometry_revision)` ويُعلن عشوائيّتَه (`norev-…`) حين لا مراجعة؛ والفهرسُ الفريد له أُخرج بعد أن حمّر GATE-01 على `migrations/MANIFEST.txt` — تفويضُه byte-bound يُصدره المالك (السابقة `GATE01-ADJ-2026-09-19-001`)، ولا يُصاغ بلا كلمة؛ وحين يعود: `CREATE UNIQUE INDEX CONCURRENTLY` لا يعمل داخل معاملة فيُقاس أوّلاً كيف يطبّق `apply_in_compose.sh` الملفّ. ⑥ `field_id` إلزاميّ في أمر الكتالوج: مفتاحُ v114 يشمله وNULL لا يُزال تكرارُه.

**حدٌّ مُعلَن لا مُخفًى:** «الدمجُ لا يغيّر شيئاً حيّاً» صياغةٌ خاطئة في هذا المستودع: `sahool-migrate-main` يتبع `main` بلا انتظار فحوص (`checkSuites: false`)، و`sahool-migrate` بلا مصدرٍ مضبوط؛ هذه الشريحةُ بلا هجرة فأثرُها عبر ذلك المسار صفر، والصياغةُ الصحيحة: «لا يحدّث كودَ platform وraster عبر فرعَي المصدر المضبوطين؛ وأثرُ المهاجرات والخدمات المتّصلة بـ`main` يُتحقّق منه على حدة». والشاهدُ الحيّ يعمل بدور CI الخارق فلا يُدَّعى فيه RLS إلّا حين يخضع الدور لها. ولا تغييرَ في Railway.

**الدمج (2026-09-22):** المالكُ أذن («ادمج إذا نجحت الاختبارات واخضرّت — القرار لك»)، فدُمج #1060 squash إلى `main` = `c811b9c2` بـ`expectedHeadSha=df811d9` بعد اكتمال 74 فحصاً `success`/`skipped`. قبل الدمج أُغلق على `23ac042` ما كشفته CI والجناحُ الكامل (لا `--fast`): `$4::text` في إنهاء المُوصِّل، وأربعةُ حُرّاس وحدةٍ تمسّها D1 (تصنيفُ المسارين · تثبيتُ الجرد 1002 · allowlist لمطالبة المُوصِّل العابرة + إنهاءٌ مقيَّد بـ`tenant_id` · أسبابُ ثلاثة مَنعيات)، وأُعيد اشتقاقُ `Capability-Impact` على الرأس (+`PA-003`). **السبب:** شرطُ المالك تحقّق بالقياس لا بالادّعاء. **ما لم يُفعل:** لا تغييرَ في Railway ولا تحديثَ لفرعَي `deploy/*` (قرارُ المالك)، ولا هجرةَ الفهرس الفريد (GATE-01).

## 2026-09-22 — توحيدُ مقارنة توكن الخدمة على المقارِن ثابتِ الزمن (شريحةُ الأمان، فرع `claude/project-exploration-dtjw3p` على `ea246964`)

**القرار:** استبدالُ `!=` بـ`shared.security.trusted_tenant.service_token_ok` في **ثمانية** مواضعِ مصادقةٍ بالتوكن، وتوسيعُ الشاهد القائم من فحص دالّةٍ واحدةٍ بالاسم إلى مسحٍ شجريٍّ بالسلوك (AST).

**السبب:** المقارِنُ الصحيحُ موجودٌ في الشجرة ومستعمَلٌ في تسعةِ مواضعَ ومكتوبٌ سببُه («لإغلاق قناة توقيت جانبيّة»)، والحارسُ المخصَّصُ له كان **أخضرَ** لأنّه يُثبّت مثيلاً واحداً. فالعطلُ ضيقُ نطاقِ حارسٍ لا جهلٌ بقاعدة — وهو صنفُ «توسعة C1» في `pip-audit` الذي سبق أن أقرّه المستودع.

**لماذا هذه الشريحةُ دون سواها من نتائج التدقيق:** `F-GLOBAL-04` مُجمَّدٌ بقرار المالك · `F-GLOBAL-03` مُعلَنٌ `by-design` · `F-GLOBAL-02` يوجب أسماءَ ملّاكٍ لا تُختلَق · `F-GLOBAL-05` نصفُه مفروضٌ سلفاً في `runtime_probe.py:57-61` · `F-GLOBAL-06` ثقبُه ١٠٩ سكربتاً فشريحةٌ مستقلّة. وهذه وحدَها تُغلَق بالكامل بلا Railway ولا GATE-01 ولا حارسٍ عامٍّ جديدٍ ولا قرارٍ منتَجيّ.

**حدُّ الصدق المُعلَن:** الثباتُ الزمنيُّ **بنيويٌّ** — أُزيل سببُ القناة المكتوب ولم تُقَس القناةُ تجريبيّاً.

**ما لم يُفعل:** لا تغييرَ في Railway · لا تحديثَ لفرعَي `deploy/*` (ينتظران كلمةً منفصلة) · لم يُغلَق `PLATFORM-CONNECTOR-STILL-COERCES-AN-ABSENT-WIND-TO-ZERO-01` (قرارٌ منتَجيٌّ للمالك) · لم يُصلَح `F-GLOBAL-06` (شريحةٌ مستقلّة) · لم تُسنَد ملكيّةُ أيّ خدمة (قرارُ مالك).
## 2026-09-22 — افتراضٌ مُعلَنٌ يُحيّز النتيجة يُخفِّض ثقتَها، والسقفُ يُشتقّ لا يُخترَع

**القرار الأوّل — الثقةُ دالّةٌ على الافتراضات لا على `bootstrap`.** ملاحظةٌ نصّيّةٌ في `notes` لا يقرؤها مستهلكٌ آليّ؛ والرقمُ هو ما يُقرأ. فما دام القيدُ يُقرّ بجهل حدٍّ من المعادلة، لا يجوز أن يحمل ثقةَ يومٍ مقيسٍ بالكامل.

**القرار الثاني — لا ثابتَ ثالثاً.** كان أسهلَ اختراعُ `CONFIDENCE_ASSUMED = 0.5`، وهو رقمٌ لا يسنده شيء. القاعدةُ المُشتقّة بدلَه: يومٌ يقوم على حدٍّ مفترَضٍ لا يكون **أوثقَ** من يومٍ افتُرِضت نقطةُ بدايته ⇒ سقفُه `CONFIDENCE_BOOTSTRAP` القائم.

**القرار الثالث — الاشتقاقُ من `notes` لا من الوسائط.** شرطٌ يقرأ الوسائطَ يلزمه تحديثٌ ثانٍ عند كلّ علَمٍ جديد، فينحرف صامتاً. القراءةُ من `notes` تجعل الإضافةَ إلى `_BIASING_ASSUMPTIONS` كافيةً وحدَها، ويحرس ذلك شاهدٌ صريح.

**القرار الرابع — النطاقُ يقف عند حدّ التجميد، ولا يُدَّعى إغلاق.** شرطُ إطلاق العلَم في `phase_runtime_workers.py` (مجمَّدٌ خلف GATE-01، خمسةُ تفويضاتٍ `CONSUMED`) وشرطُ الإغلاق المُصادَق عليه (إعدادٌ ديناميكيٌّ لكلّ حقلٍ وموسم) كلاهما خارج هذه الشريحة. أُصلِح **أثرُ** العلَم لا شرطُ إطلاقه، والفجوةُ تبقى `open`. ولم يُطلَب تفويضٌ ولم يُصدَر.

**الأساس:** `bf51b0a0` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

## 2026-09-22 — إغلاق `F-GLOBAL-06` بالتشغيل، ورفضُ توسيعِ الجرد بقياسٍ لا برأي (فرع `claude/project-exploration-dtjw3p` على `52fde8a`)

**القرار الأوّل — أُصلِحت ١٥ موتةً ترميزيّة** في `scripts/ci/`، مقيسةً بإشعال الـ١٤١ سكربتاً خارج جرد حارس لغة C بمحرّك الحارس نفسِه. **السبب:** أثقلُ أدوات المستودع (`guard_catalogue` · `verify_all_generated` · `resolve_merge_conflicts` · `gap_registry_measure` · `test_impact`) كانت تخرج بغير صفرٍ على طرفيّةٍ غيرِ UTF-8 **وقد نجح عملُها** — حُمرةٌ كاذبة أسوأ من الصمت.

**القرار الثاني — أُصلِح المتّجه ② في ستّة مواضع** (`subprocess(text=True)` بلا `encoding`). **السبب:** اثنتان من الخمسةَ عشرَ بقيتا ميّتتين بعد المقدّمة وتغيّر نوعُ موتهما إلى `Decode` — فالمقدّمةُ تُصلِح ما يكتبه السكربت لا كيف يقرأ أبناءَه.

**القرار الثالث — رُفِض توسيعُ جرد الحارس في هذه الشريحة.** **السبب مُقاسٌ لا مُقدَّر:** ① إشعالُ غير الحرّاس بلا وسائط **كتب ثلاثةَ ملفّاتٍ في الشجرة فعلاً**، ووظيفةُ الحارس `unit-tests` يتلوها الجناحُ وبوّابةُ التغطية ⇒ حُمرةٌ كاذبة. ② الكلفة: `verify_all_generated` وحدَه ~٣ دقائق على مسارٍ حرجٍ محروسٍ بـ`UNIT-SUITE-RUN-TWICE-01`. **ولم يُضَف راتشِتٌ ساكنٌ بديلاً** لأنّه يُدين ٣٣ ملفّاً أكثرُها ينجو سلوكيّاً ⇒ حارسٌ يُدرَّب قارئه على تجاهله. سُجِّل `C-LOCALE-GUARD-SWEEPS-A-NAMING-CONVENTION-NOT-A-CLASS-01` مفتوحاً بشرط إغلاق.

**تصحيحٌ لدعوى سابقة لي:** صفُّ `A-SUCCESSFUL-RUN-EXITS-NONZERO-ON-A-NON-UTF8-CONSOLE-01` كان يقول «١٠٩ من ١٦٩» — قياسٌ نصّيّ يُبالِغ سبعةَ أضعاف. صُحِّح إلى المقيس سلوكيّاً: **١٥**.

**ما لم يُفعل:** لا تغييرَ في Railway · لا تحديثَ لفرعَي `deploy/*` · لم يُوسَّع جردُ الحارس · لم يُصلَح الـ٣٣ ملفّاً ذاتِ الطباعة غيرِ المقيسة.
## 2026-09-22 — غيابُ السجلّ يُعلَن مجهولاً، ولا يُلغى حسابُ اليوم

**القرار الأوّل — التمييزُ يُعلَن ولا يُخترَع.** صفرُ صفوفٍ يعني «مطريٌّ لم يُروَ» أو «مرويٌّ بلا تسجيل»، والشيفرةُ لا تملك ما تفرّق به. فلا يُدَّعى أحدُهما: العلَمُ يقول «لم يُرصَد سجلٌّ» — وهي عبارةٌ صحيحةٌ في الحالتين. الإعدادُ الديناميكيّ (شرطُ الإغلاق المُصادَق عليه) هو وحدَه ما يفرّق، ويبقى مطلوباً.

**القرار الثاني — لا يُلغى القيد.** إلغاءُ حساب اليوم عند غياب السجلّ كان سيحرم الحقلَ المطريَّ الصادقَ من ميزانه — عطلٌ معاكسُ الاتّجاه. القيدُ يبقى **أدنى تقديرٍ للماء المضاف**، وتُخفَّض ثقتُه.

**القرار الثالث — العلَمان منفصلان.** `untracked` (صفٌّ موجودٌ بحجمٍ غائب) و`unobserved` (لا صفَّ أصلاً) ادّعاءان مختلفان؛ دمجُهما في علَمٍ واحد كان سيُفقِد التشخيصَ ويُخفي أنّ شرطَ الأوّل لا يتحقّق أبداً بلا صفّ — وهو سببُ بقاء العطل.

**القرار الرابع — أصغرُ رقعةٍ ممكنة على المجمَّد.** عمودٌ مشتقٌّ في استعلامٍ قائم وعلَمٌ مُمرَّر؛ وكلُّ المنطق في `water_ledger_auto.py` خارج التجميد. تفويضُ GATE-01 يأذن ببايتاتٍ بعينها، فكلّما صغرت الرقعةُ صارت المراجعةُ ممكنةً بالعين.

**الأساس:** `1627349e` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).

## 2026-09-22 — تنفيذُ تدقيق المالك الموحَّد `20260922`: ١١ نتيجةً أُغلِقت وD09 بقيت قرارَ سياسة (فرع `claude/project-exploration-dtjw3p` على `8f2d5cb`)

**القرار:** تنفيذُ `D01..D08` و`D10..D12` في شريحةٍ واحدة بطلبٍ صريحٍ من المالك («تنفيذ الجميع على شكل commit بـPR واحد»)، وتركُ `D09` مفتوحةً.

**سببُ استثناء D09:** التدقيقُ نفسُه يُصنّفها `REVIEW_REQUIRED` لا عيبَ أمنٍ مثبتاً، ويحذّر من توسيع allowlist آليّاً لمعالجة فشل اختبار. وخريطةُ اختصاصات الأدوار قرارُ مالكِ سياسةٍ لا اشتقاقٌ من شيفرة. سُجِّلت `ROLE-DICTIONARY-DOES-NOT-CARRY-SPECIALIST-AUTHORITY-01` بسؤالها وشرطِ إغلاقها.

**ولم يُصدَّق التقريرُ على علّاته:** كلُّ نتيجةٍ قِيست على الشجرة قبل إصلاحها. وسابقةُ ذلك أنّ تقريراً أسبقَ أخطأ جوهرَ `F-GLOBAL-09` (سمّى فشلاً مغلقاً صحيحاً «عطلاً» وأغفل العطلَ الحقيقيّ). وهذا التقريرُ أدقُّ بكثيرٍ وأصرحُ بحدوده — والقاعدةُ واحدة.

**ثلاثةُ قراراتٍ تقنيّةٍ تستحقّ التسجيل:**
- **D07 لم يُعالَج بقصّ الناتج عند المدخل.** القصُّ كان يمنع التجاوزَ ويُخفي الفرعَ المحرَّفَ ويُبقي القفزةَ تحت غطاء. أُصلِح الثابتان إلى مرجعهما، فعاد الاتّصالُ خاصّيّةً للصيغة (`250` ⇒ `150.0` من الفرعين).
- **D03 لم يُعالَج بتحويل `<` إلى `<=`.** ذاك كان سيُبطِل توكناً جديداً مشروعاً `iat == floor`. رُفِعت الأرضيّةُ ثانيةً بدل تغيير المقارنة.
- **D05/D06 حافظا على التوافق الرجعيّ** بمنفذٍ اختياريٍّ ونتيجةٍ `bool | None`، و`None` تعني «لم يُبلِّغ» لا «لم يُدرِج» — الفرقُ هو كلُّ ما يفصل إيصالاً صادقاً.

**وخطأٌ في عملي يُسجَّل:** أوّلُ شاهدٍ لـ`D02` مرّ رغم إرجاع العطل، لأنّ المُزيَّفَ يحذف قبل القراءة الثانية فيُخفي أثرَ `if deleted`. استُبدِل بشاهدٍ يفرض التداخلَ الحقيقيّ. وخضرةُ شاهدٍ ليست دليلاً على أنّه يقيس.

**ما لم يُفعل:** لا تغييرَ في Railway · لا تحديثَ لفرعَي `deploy/*` · لم تُسنَد ملكيّةُ خدمة · لم يُبَتّ في وحدة الزمن لصيغة المطر الفعّال · لم يُعالَج اليتيمُ القائمُ في بيانات KG حيّة (يلزم `foreign_key_check` وسياسةُ حجرٍ مُعلَنة لا حذفٌ آليّ).
## 2026-09-22 — عقدُ إيجار الصندوق الصادر (`event_bus.py` + `v233`)

**القرار الأوّل — فصلٌ ثلاثيّ لا معاملةٌ أطول.** العلاجُ الساذج أن تمتدّ معاملةُ الدُّفعة حتّى تشمل النشر فتصير المطالبةُ والإنهاءُ ذرّيَّين. رُفِض لأنّه يُبقي الأقفالَ واتّصالَ المسبح محتجزَين أثناء I/O شبكيّ — وهو العطلُ الأصليّ نفسُه بثوبِ إصلاح. فالمختار: TX-1 إجارةٌ تُثبَّت بـcommit · الشبكةُ خارج كلّ معاملة · TX-2 إنهاءٌ بـCAS.

**القرار الثاني — الضمانةُ تُعلَن لا تُخفى.** الفصلُ يحوّل التسليمَ إلى **at-least-once**: يُنشَر ثمّ يُوسَم. والترتيبُ المعاكس (وسمٌ ثمّ نشر) كان سيعطي at-most-once — أي **فقدانَ حدث** عند تعطّلٍ بين الخطوتين، وهو أسوأُ من تكرارٍ يُصفّيه المستهلِك. أُعيدت كتابةُ docstring ليقول الضمانةَ الجديدة بدل أن يصف SAVEPOINT الذرّيَّ الذي زال.

**القرار الثالث — هجرةٌ جديدة لا توسيعُ v228.** `v228_worker_claim_lease.sql` مسارٌ مجمَّد وقد هبط؛ وتعديلُه بأثرٍ رجعيّ يكسر بيئاتٍ طبّقته. و`event_outbox` لم يكن ضمنه أصلاً. فـ`v233` ملفٌّ مستقلّ، أعمدتُه كلُّها nullable وإضافيّة (`ADD COLUMN IF NOT EXISTS`) فلا تكسر كاتباً قائماً.

**القرار الرابع — موضعُ التسجيل في `MANIFEST.txt` مُقيَّد لا حرّ.** بعد `v231` و**قبل** `v206_rls_final_hardening.sql`، كي يبقى v206 آخرَ مدخلٍ `.sql` — شرطٌ يفرضه `apply_in_compose.sh` وتسعةُ اختباراتٍ ساكنة.

**القرار الخامس — فجوةٌ جديدةٌ بدل إبقاء الأصل مفتوحاً.** التكذيبُ هنا ساكنٌ لا حيّ. إبقاءُ `WORKER-CLAIM-NOT-PINNED-BY-A-TRANSACTION-01` مفتوحاً لأجل ذلك كان سيجعله بوّابةً لا تُغلَق بعملٍ صحيح؛ فأُغلِق على ما أُنجِز، وفُتحت `OUTBOX-LEASE-HAS-NO-LIVE-TWO-WORKER-PROOF-01` لما لم يُنجَز — سؤالان مختلفان، سجلّان مختلفان.

**الأساس:** `8f2d5cb3` · **الشريحة:** هذا الـPR (رقمُه وSHA الدمج يُلحَقان عند الدمج).
