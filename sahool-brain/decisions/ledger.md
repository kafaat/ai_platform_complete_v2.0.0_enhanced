...
## 13) إصلاح العرض + التدقيق + التفكيك + قفل التبعيّات (جلسة 2026-06-28، رأس `main`: `305eeeb`)

ثلاثة محاور فوق #504: إصلاح عرض تراكب المؤشّر، تحقّق نقديّ من تدقيق خارجيّ، ثمّ تفكيك تدريجيّ واسع
سلوك-محفوظ + قفل تبعيّات قابل لإعادة الإنتاج. المبدأ الحاكم: إصلاح أخطاء حقيقيّة، حفظ السلوك، عدم
إضعاف الحُرّاس، عدم تلفيق أدلّة حيّة، ودمج متسلسل مرتَّب للدفعات المتوازية.

| المكوّن | القرار + السبب |
|---|---|
| **إصلاح تراكب المؤشّر بالسبب الجذريّ** | #506 (`4e87e0e`) — التشخيص بـ3 وكلاء استكشاف متوازية كشف أنّ حاوية `sahool-platform` تفتقر `SAHOOL_AGENT_TOKEN`، فيفشل تمرير الصور المُصادَق للراستر (تراكب فارغ صامت لا خطأ). القرار: إضافة المتغيّر لكتلة بيئة المنصّة (`${SAHOOL_AGENT_TOKEN:?…}` fail-fast) + راوتر تمرير `field_imagery_backfill` يحقن `X-Agent-Token` + هندسة الحقل. السبب: علاج جذريّ (تكوين ناقص) لا ترقيع واجهة؛ **التأكيد الحيّ تحت `sahool_app` بيد Ops** (حدّ الصدق). |
| **تدقيق خارجيّ: تحقّق قبل تطبيق** | #507 (`7b17133`) — «تأكّد من الادعاءات»: فُحِص كلّ بند P0/P1 من تقرير تدقيق خارجيّ مقابل الكود. طُبِّق المؤكَّد فقط (محاذاة انجراف `MANIFEST`/`.env-test` + بداية `scripts/deps/lock.sh`)؛ رُفِض غير المؤكَّد. السبب: تقرير خارجيّ ليس أمراً تنفيذيّاً — يُتحقَّق منه نقديّاً (لا تطبيق أعمى، لا تلفيق إصلاحات لمشاكل غير قائمة). |
| **شبكة أمان CI قبل التفكيك العميق** | #509 (`85d9143`) — اختبارات `services/raster-service` (٥٩+) لم تكن تُشغَّل في أيّ وظيفة CI ⇒ أيّ انحدار يمرّ صامتاً. القرار: تأسيس وظيفة `raster-tests` **قبل** أيّ تفكيك عميق لـmain.py الراستر. السبب: لا تفكيك آمن بلا شبكة اختبار تشغّل المسارات الحقيقيّة (rasterio/tiles/clip). |
| **التفكيك بإعادة تصدير (سلوك محفوظ)** | #508-#524 — كلّ تفكيك (نماذج/مساعِدات/عملاء/مجالات/خطّافات) يُبقي العقد العامّ ثابتاً عبر barrel/`export *`؛ النقل **حرفيّ** لا إعادة كتابة. السبب: تقليل دَيْن البنية وخطر المراجعة بصفر تغيير سلوكيّ — قابل للتحقّق بـtypecheck/الاختبارات. |
| **تحديث الحُرّاس النصّيّة لا إضعافها** | عند تفكيك ملفّ يقرؤه حارس ساكن (يؤكّد وجود نصّ حرفيّ)، يُحدَّث الحارس ليقرأ المصدرين (القديم + الوحدة الجديدة) كي «يحرس العقد لا الموضع». السبب: الحفاظ على قوّة الحارس مع السماح بإعادة التنظيم — لا allowlist أعمى ولا حذف تأكيد. |
| **تقليم الاستيرادات غير المستعملة** | بعد نقل دوال من `api.ts`/`useApi.ts`، تُقلَّم رموز الاستيراد التي صارت غير مستعملة (درس TS2459 «معرَّف محلّيّاً غير مُصدَّر» + TS7006 anyتسلسليّ). السبب: تفادي فشل Frontend Typecheck؛ كلّ رمز تحتاجه الوحدة الجديدة يُصدَّر من مصدره. |
| **تجنّب دورة الاستيراد بإبقاء المقطعيّ مركزيّاً** | #524 — مُعالِجات الأخطاء العامّة (`asApiError`/`apiErrorMessage`/`ApiError`…) **بقيت في api.ts** لأنّها مقطعيّة تستهلكها وحدات أخرى؛ نقلها كان سيُحدث دورة `accountApi→api→accountApi`. السبب: التفكيك يخدم البنية لا يكسرها. |
| **قفل تبعيّات مستقلّ عن بيئة المُولِّد** | #520 (`61dba5c`) — أقفال `*.lock` مُلتزَمة للمسار الحرج (حلّ عابر + تجزئات) + بوّابة CI «Dependency Lock Drift». `lock.sh` يحلّ بـ`--universal --python-version` (مستقلّ عن مُفسِّر/منصّة المُطوِّر، مطابق لـCI بايتاً) + `--custom-compile-command` **يُصلح علّة كامنة** كانت تجعل `--check` يُبلِغ انجرافاً زائفاً دائماً (ترويسة القفل تسجّل مسار `-o`). السبب: معالجة انجراف سلسلة الإمداد بأداة قابلة لإعادة الإنتاج ومُدقَّقة. |
| **استهلاك Docker للأقفال + محاذاة py3.11** | #521 (`9f8a1e1`) — صور auth/platform/guardrails تُثبّت من `.lock` بـ`--require-hashes`. **محاذاة جوهريّة:** الصور `python:3.11-slim` لكنّ أقفال #520 وُلِّدت بـ3.12 ⇒ `PY_TARGET` صار `3.11` (الأدنى/الأكثر توافقاً، `--universal` يبقيه صالحاً لـ3.12). auth/Dockerfile ينسخ `*.py` (لتضمين وحدات #523). التحقّق: تثبيت `--require-hashes` فعليّ في venv 3.11 نجح. **مؤجَّل بصدق:** بناء صور Docker كاملة (طبقات OS) لم يُنفَّذ في البيئة؛ و`pip-audit` على `*.lock` فرصة قيمة لاحقة. |
| **عزل auth Dockerfile لإزالة تعارض الوكلاء المتوازيين** | الدفعة الأخيرة 4 وكلاء متوازيين في عُزلة worktree؛ كان وكيل قفل-Docker ووكيل تفكيك-auth سيتعارضان على `auth/Dockerfile`. القرار: وكيل القفل يجعل النسخ `*.py` شاملاً، فوكيل التفكيك لا يلمس الـDockerfile إطلاقاً (ملكيّة ملفّ منفصلة). السبب: تمكين التوازي بصفر تعارض، مع صحّة الصورة بعد دمج الاثنين (دُمج القفل أوّلاً). |
| **دمج متسلسل مرتَّب + إعادة توليف للحزمة المتقلّبة** | حزمة الإصدار (`SAHOOL_RELEASE_MANIFEST` generated_at/file_count + CHECKSUMS) تتعارض حتماً بين PRين متوازيين. القرار: دمج واحد تلو الآخر، وإعادة توليف كلّ PR متبقٍّ على main الجديد بإعادة توليد الحزمة (`build_release_bundle.py`) لحلّ التعارض. السبب: لا مفرّ من تسلسل ملفّ الحزمة؛ التوليد الحتميّ يجعل الحلّ آليّاً وموثوقاً. |
