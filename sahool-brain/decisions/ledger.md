...
## 2026-08-30 (د) — تُنفَّذ الوثيقةُ أو تُعاد صياغتُها، ولا تُترَك تصف ما ليس قائماً

- **القرار ①: تُنفَّذ §٥ و§٦ بدل إعادة صياغتهما.** عرضتُ خيارين وأمر المالكُ بالتنفيذ. **والسبب:** المواصفةُ كانت مكتوبةً كاملة، وتركُ الوثيقة تصف ما ليس قائماً هو العطلُ نفسُه الذي أغلقناه في `from_payload` — **متنٌ يَعِد بما لا يقع**.
- **القرار ②: بصمةُ Trivy تُقاس ولا تُنقَل.** أنزلتُ الأصلَ وطابقتُ ملفَّ البصمات الرسميَّ وسجّلتُ `2ae6fe3e…`. **والسبب:** رقمٌ منقولٌ في ملفٍّ حاجب يجعل البوّابةَ تثق بما لم يره أحدٌ هنا.
- **القرار ③: قاعدةُ ثغراتٍ غائبةٌ تفشل ولا تُقرأ خُلوّاً.** خطوةُ تحميلٍ مستقلّةٌ تفشل عند تعذّرها. **والسبب:** بلا ذلك يُخرِج Trivy تقريراً فارغاً وينجح — **تحويلُ غيابِ البيانات إلى نجاح**، وهو ما رفضه تقريرُ المختبر في OSV ثمّ لم يُعمِّمه.
- **القرار ④: يُفحَص ويُوقَّع بالبصمة لا بالوسم.** **والسبب:** الوسمُ قد يُعاد توجيهه بين الفحص والترقية، والبصمةُ لا. ولذلك `push: true` ولا `load: true` — **مُصدِّرُ docker المحلّيّ يُسقِط المُسنَدات**، فتُبنى صورةٌ تبدو موثَّقةً ولا تحمل توثيقاً في السجلّ.
- **القرار ⑤: يُطلَب نوعا المُسنَد صراحةً في الطرفين.** **والسبب:** تحقّقٌ بلا تسميةِ النوع يقبل أيَّ مُسنَدٍ موجود، فصورةٌ تحمل منشأً بلا جردٍ تمرّ وتبدو موثَّقة.
- **القرار ⑥: يُعاد التحقّقُ في PATH-3 رغم وقوعه في البناء.** **والسبب: مُنتِجٌ يشهد لنفسه ليس شاهداً** — والتكرارُ هنا فصلُ سلطةٍ لا هدرٌ.
- **القرار ⑦: البصماتُ تُربَط داخل البيان لا تُرفَق بجانبه.** **والسبب:** مصنوعةٌ مرفوعةٌ بجانبه تُستبدَل أو تُسقَط بلا أن يتغيّر البيان؛ والبصمةُ داخله تجعل كلَّ استبدالٍ يُحمِّر. ويُرفَض الناقصُ **عند التجميع** لأنّ البيانَ المُجمَّع هو العقد.
- **القرار ⑧: يُقال حدُّ ما فُرِض.** المقيسُ **العقدُ في الـYAML** لا نجاحُ التشغيل. **والسبب:** ادّعاءُ «الصورةُ فُحِصت» من ملفٍّ في الشجرة هو بعينه الخطأُ الذي أنشأ هذه الفجوة.
- **حدُّ صدق:** الزرعُ الآليّ يموت على `pyo3`؛ التكذيبُ يدويٌّ وقتلت كلُّ طفرةٍ اختبارَها المُسمّى **وحدَه**. **وأخطأتُ في تكذيبي مرّتين** — مَرسًى يطابق تعليقاتي لا الشيفرة، وقارئٌ نصّيٌّ يبحث عن `@sha256:` في مرجعٍ قالبيّ — **ومِكنستي اليدويّة كانت تفتقد شرطَ وحدانيّة المَرسى الذي يفرضه `guard_mutation`**.
- **المصدر:** فرع `claude/image-supply-chain` فوق `2878399a` · [`guard_mutation_registry.json`](../../docs/architecture/guard_mutation_registry.json).

## 2026-08-30 (ج) — يُصحَّح الحكمُ حين يصل الدليل، ويُصحَّح الحكمُ على النفس أوّلاً

- **القرار ①: يُسحَب ادّعائي «ما لم يره التقرير» قبل أيّ تصحيحٍ لغيري.** zizmor رصد المواضعَ نفسَها بثمانيةِ High/High. **والسبب:** تركُ الصياغة يوحي بأنّ الاشتقاق سبق الأداة، وهو ادّعاءُ سبقٍ لا يقوم — **والحكمُ يتبع الدليل ولو كلّف تنازلاً عن فضلٍ ادّعيتُه لنفسي**.
- **القرار ②: يُسمّى العطلُ بما هو — ترتيبٌ لا كشف.** **والسبب:** «لم يره» يوجّه العلاجَ إلى أداةٍ إضافيّة، و«رآه ولم يُرتَّب» يوجّهه إلى التحكيم. والثاني هو الواقع: وسمُ «confirmed» من ماسحٍ غلبَ **درجةَ High/High** من ماسحٍ آخر، على مُدخَلَين مقيَّدَي النوع أصلاً.
- **القرار ③: يُوسَّع النطاقُ بدليلٍ لا بحَذَر.** `verify-candidate` بلا صلاحيةِ كتابة، ويُنتِج المصنوعةَ التي يوقّعها المُمتاز. **والسبب: حدُّ الثقة الملفُّ لا الوظيفة** — المصنوعاتُ والمخرجاتُ تجري بين وظائف الملفّ الواحد، فالحقنُ في غير المُمتاز يُلوّث ما يوقّعه المُمتاز. ٦ ⇒ ١٣ موضعاً.
- **القرار ④: يُصلَح ما لا يُسيطَر عليه بدل أن يُستثنى.** `${{ matrix.shard }}` في `ci.yml` من مصفوفة الملفّ لا من مُدخَل. **والسبب:** قائمةُ إعفاءٍ من صنفٍ أمنيّ **تبيت وتُقرَأ ترخيصاً** — وهو `AN-EXEMPTION-LIST-WITH-NO-DESCENDING-CEILING-01` في طبقةٍ أمنيّة. والقاعدةُ بلا ثقوبٍ أرخصُ صيانةً من الثقب الموصوف.
- **القرار ⑤: طولٌ ليس شكلاً.** `test "${#TARGET_SHA}" = 40` يقبل أربعين محرفاً أيّاً كانت، و`path3_run_id` كان بلا فحص. **والسبب:** فحصٌ يبدو تحقّقاً ولا يقيّد المجموعة هو أسوأُ من غيابه — يُطمئن القارئ فيكفّ عن النظر.
- **القرار ⑥: يُقبَل وسمُ المختبر على نفسه ولا يُرفَع.** `LAB_HARNESS_VALIDATED_NOT_PRODUCTION`، وطفراتُه الثماني على ملفِّ عقودٍ خاصٍّ به. **والسبب:** الحكمُ الذي أعلنه صاحبُه بصدقٍ لا يُرقّى من طرفٍ ثانٍ؛ و`repository_mutated_by_this_lab: false` قِيس ولم يُدَّعَ.
- **حدُّ صدق:** أدواتُ المختبر ما تزال غيرَ متاحةٍ هنا — **قرأتُ مخرجاتِها لا أعدتُ تشغيلها**، فالمُثبَتُ أنّ ملفّاتها تقول هذا، لا أنّي أعدتُ إنتاجَ الرقم. وأدلّةُ العينة (`.venv`/الصور) خارج ما أحكم عليه.
- **المصدر:** فرع `claude/image-provenance-injection` فوق `f8af8632` · أرشيفا المالك · `evidence/zizmor.json` · `evidence/results-summary.json`.

## 2026-08-30 (ب) — يُحقَّق التقريرُ بنداً بنداً، ولا يُنقَل رقمٌ لم يُقَس

- **القرار ①: أرقامُ ماسحٍ لم يُشغَّل تُوسَم «منقولة».** ١٩٨ zizmor و٨ actionlint و٥ poutine و٩٥٥ مكوّناً — **ولا واحدةً من الأدوات متاحةٌ في هذه الحاوية**. **والسبب:** تمريرُها كأنّها مقيسة يبني قراراتٍ على عددٍ لم يره أحدٌ هنا؛ والفرقُ بين «قِيس» و«نُقِل» هو الفرقُ نفسُه بين «لم يجد» و«لم ينظر».
- **القرار ②: يُسحَب وسمُ «confirmed» حين يكذّبه إعلانُ المُدخَل.** `mode` (`type: choice`) و`skip_security` (`type: boolean`) — GitHub يرفض قيمةً خارج `options` عند الإطلاق. **والسبب:** الماسحُ رأى الشكل (`${{ }}` في `run`) ولم يقرأ القيد. **والنمطُ المطابقُ ليس استغلالاً قائماً** — وتصنيفُه كذلك يُنفِق انتباهاً على غير موضعه، وهو ما وقع: الملفّان المُسمّيان لا يحملان الموضعَ الخطر.
- **القرار ③: لا يُنفَّذ ما هو منفَّذ.** توصيةُ «أضِف اختباراً يمنع إزالة حارس `certify-run`» — الاختباراتُ قائمةٌ بثلاثة أرجل. **والسبب:** الجردُ قبل التنفيذ؛ وإضافةُ سلطةٍ ثانية للحقيقة الواحدة هي العطلُ الذي يطارده هذا المستودع.
- **القرار ④ — وهو الأهمّ: يُشتقّ النطاقُ من الخطر لا من اسم الملفّ.** «كلُّ وظيفةٍ تملك صلاحيةَ كتابة» ⇒ ستّةُ مواضع، **صفرٌ منها فيما سمّاه التقرير**. **والسبب:** الحقنُ في وظيفةٍ بلا امتياز يُتلِف جولة؛ الحقنُ في وظيفةٍ تحمل `attestations:write` **يُصدِر شهادةَ منشأٍ موقّعة**. الخطرُ في الصلاحية لا في المسار، فالاشتقاقُ منها.
- **القرار ⑤: يُصلَح مسارُ الترقية أوّلاً لأنّه موضوعُ البوّابة نفسِه.** `approval-receipt` محميّةٌ بـ`environment` وتحمل `attestations:write`. **والسبب: البيئةُ المحميّة تحرس مَن يوافق لا ما يُنفَّذ بعد الموافقة** — الحمولةُ تعمل بعد إجازة المُراجِع وبصلاحياتها، والمُراجِعُ رأى المُدخَل وسيطاً لا شيفرة. **فالحقنُ هنا نقضُ فصلِ السلطة**، وهو بالضبط ما جاء التقريرُ ليؤسّسه.
- **القرار ⑥: يُنقَل فحصُ الشكل قبل الاستعمال، لا يُكتفى بنقل القيمة إلى `env`.** **والسبب:** `[[ … =~ ^[0-9a-f]{40}$ ]]` كان مكتوباً في السكربت الذي حُقِن أصلاً — **حارسٌ داخل ما يحرسه ليس حارساً**، وتركُه بعد الإصلاح يُبقي القارئ يصدّق أنّ الشكل مفحوص.
- **القرار ⑦: يُنقَل الامتيازُ إلى الوظيفة التي تستعمله.** `publish-manifest` يجمّع JSON وكان يرث `packages`/`id-token`/`attestations`. **والسبب:** صلاحيةٌ لا مستهلك لها ليست احتياطاً بل سطحُ هجومٍ بلا مقابل — والحجّةُ نفسُها التي نُزِعت بها `id-token` من `certify-run.yml` سابقاً في هذا المستودع.
- **القرار ⑧: يُسجَّل ما لم يُنفَّذ بأسبابه.** SBOM/فحصُ الصورة/`attestation verify`/Broker مستقلّ/OPA — **لم تُنفَّذ**. **والسبب:** سلسلةُ توريدٍ قرارُ مالكٍ يوسّع سطحَ التشغيل والثقة، وليست تنظيفَ عطلٍ مقيس. **وتوصيةُ «فحصُ digest قبل الترقية» تصف مرحلةً غيرَ موجودة**: الملفُّ يدفع وسماً ثابتاً بالـSHA ولا يُرقّي شيئاً — والتصحيحُ يُقال ولو خالف تقريراً.
- **حدُّ صدق:** المقيسُ **شكلٌ** — قالبٌ في متن وظيفةٍ ذاتِ صلاحية. لا يُقاس أنّ القيمة المُمرَّرة عبر `env` مُتحقَّقٌ منها، ولا يبلغ الجارِدُ `uses:` ولا `workflow_call`. **والزرعُ الآليّ مات على `pyo3`** كسائر مُسجَّلات هذه الحاوية، فالتكذيبُ يدويٌّ: طُبِّقت كلُّ طفرةٍ مُسجَّلةٍ حرفيّاً فقتلت اختبارَها المُسمّى **وحدَه**، وCI هي الشاهد.
- **المصدر:** فرع `claude/image-provenance-injection` فوق main `5112a613` · [`guard_mutation_registry.json`](../../docs/architecture/guard_mutation_registry.json) (`behavioural` · `runtime-verification-promotion.yml[0]` · `runtime-image-provenance.yml[0]`).
