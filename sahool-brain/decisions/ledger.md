...
## 12) إصلاح RLS-معاملة في phase_runtime (جلسة 2026-06-27 ج، رأس `main`: `c3e40a8`)

| المكوّن | القرار + السبب |
|---|---|
| **مُجمِّع `_tenant_conn` للكتابات المُستأجَرة** | #504 (`c3e40a8`) — كتابات phase9-12 كانت `acquire()` ثمّ `set_config(...,is_local=true)` ثمّ `INSERT/UPDATE` **بلا معاملة محيطة**؛ asyncpg autocommit يعيد ضبط GUC قبل الكتابة ⇒ FORCE-RLS يرفض تحت `sahool_app` (NOBYPASSRLS). القرار: مُجمِّع `_tenant_conn` يحجز **داخل** `conn.transaction()` ثمّ يضبط السياق، يحاكي القانونيّ `api.main.tenant_connection`؛ ١١ كتلة كتابة مُستأجَرة استُبدِلت. السبب: إصلاح خطأ حقيقيّ بنمط المستودع القانونيّ + ذرّيّة الكتابات. |
| **استثناء الجداول العامّة** | كتابتا `model_registry_version`/`marketplace_apps` تُركتا `acquire()` عارياً (غير مُستأجَرتين، لا سياق RLS). السبب: عدم فرض معاملة-مستأجِر حيث لا مستأجِر. |
| **لفّ workers بمعاملة** | `run_outbox/plugin/model_registry/actuator` لُفّت بـ`conn.transaction()` ليصبح `FOR UPDATE SKIP LOCKED` فعّالاً (بلا معاملة = قفل بلا معنى ⇒ خطر معالجة مزدوجة). السبب: صحّة القفل التشاؤميّ. |
| **حارس ساكن بدل تكامليّ** | `test_phase_runtime_tenant_tx.py` ساكن (`unit`) لأنّ المسار الحيّ مُقنَّع بدور superuser في CI (`POSTGRES_USER=sahool_test`)؛ اختبار تكامليّ لن يكشف الانحدار. السبب: الحارس يمنع عودة النمط حيث لا يمكن لـCI كشفه — توثيق القناع نفسه. |
