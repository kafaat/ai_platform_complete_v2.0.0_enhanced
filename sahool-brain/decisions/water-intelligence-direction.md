...
- **Kc ديناميكيّ من NDVI** — ✅ **منفَّذ (#461):** `core/engines/fao56.py` يضيف طريقة FAO-56 §9.4
  ...
  **رصداً** بدل العمر (حقل متأخّر/مُجهَد ⇒ NDVI أدنى ⇒ Kcb أصدق). **حفظ السلوك:** غياب NDVI ⇒
  ...
  وML تقديريّة تحتاج معايرة محلّيّة (مُعلَنة في الافتراضات). **الربط الاستهلاكيّ ✅ منفَّذ (#462):**
  نقطة `POST /api/v1/fields/{id}/etc-dual` تجلب **NDVI الحقيقيّ المخزَّن للحقل**
  (`imagery_automation_fields.last_ndvi_mean` عبر `gather_field_freshness`) وتمرّره فعليّاً إلى
  `compute_etc_dual` — مع حقن المحصول (بطاقة عبر `crop_kc_profile` المُستخرَجة DRY) والعمر (تاريخ
  الزراعة) والملوحة (أحدث فحص). تدرّج صادق: NDVI مفقود ⇒ Kcb عمريّ؛ مصدر كلّ قيمة مُعلَن.
  **الاكتفاء الذاتيّ + الواجهة ✅ (#463):** الطقس صار اختياريّاً يُجلب حيّاً من **Open-Meteo**
  بإحداثيّات الحقل (`fetch_daily_forecast`+`fetch_current`، خارج اتّصال القاعدة؛ `weather_source`
  مُعلَن؛ تعذّر ⇒ 503 صادق) + **لوحة واجهة** `EtcDualPage.tsx` («ETc المزدوج» تحت الريّ). **متبقٍّ
  موثَّق:** جلب NDVI من COG الطازج (بدل المخزَّن) + LAI/غطاء التاج. (تطوير للفجوة 3/4 أعلاه.)
- **عمق جذور ديناميكيّ (Zr)** — ✅ **منفَّذ (#463):** `core/engines/fao56.py` يضيف (نقيّاً، إضافيّاً)
  `root_depth_m` (نموّ خطّيّ FAO-56 §8: `Zr=Zr_min+(Zr_max−Zr_min)·min(1,DAP/days_to_max)`) +
  `root_depth_for_crop` (يشتقّ `days_to_max` من `stage_days`) + `taw_from_root_depth`
  (`TAW=1000·(θFC−θWP)·Zr`، Eq. 82) + جدول θFC/θWP لكلّ قوام (Table 19). **صدق:** Zr_min/max وθ
  تقديريّة تحتاج معايرة محلّيّة (مُعلَنة). ١٨ اختباراً. **متبقٍّ:** ربط Zr بـTAW في حساب الاستنزاف.
