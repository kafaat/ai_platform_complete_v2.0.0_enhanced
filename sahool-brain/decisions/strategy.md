...
- **D — FieldState Water Canonicalization (جديدة — بتوجيه المستخدم، 2026-06-23):** دمج
  ...
  **D3 ✅ منفَّذ (#467، آمن لا يغيّر القرار):** قارئ كنسيّ `api/canonical_water.py` + كتلة `water` موحّدة
  ...
  تناقضات). **D2 (تحكيم الإجهاد المائيّ — يغيّر `execution_mode`) مؤجَّل بخطّة قرار رسميّة + إقرار** (عتبة
  الإجهاد قرار زراعيّ). ثمرة Bundle A لكنّها أكبر من «إصلاح تكرار»: تجعل الحالة المصدرَ الوحيد لقيم المياه.
