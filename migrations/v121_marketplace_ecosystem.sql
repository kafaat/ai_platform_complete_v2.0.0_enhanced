...
-- جداول مستأجِر: ENABLE + FORCE + سياسة عزل صريحة (USING + WITH CHECK) تطابق نمط v9.
...
ALTER TABLE plugin_permission_audit FORCE ROW LEVEL SECURITY;
...
DECLARE t TEXT;
BEGIN
    FOREACH t IN ARRAY ARRAY['marketplace_installations','plugin_permission_audit',
        'webhook_subscriptions','webhook_deliveries','usage_metering_records'] LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON %I', t || '_tenant_isolation', t);
        EXECUTE format(
            'CREATE POLICY %I ON %I USING (tenant_id::TEXT = NULLIF(current_setting(''app.current_tenant'', true), '''')) '
...
-- جداول مرجعيّة عامّة (بلا tenant_id): كتالوج تطبيقات/موصِّلات يقرأه كلّ المستأجِرين.
-- يلزم FORCE (وإلّا يتجاوزها المالك) + سياسة وصول مفتوحة؛ الكتابة محكومة في طبقة API/الإدارة.
ALTER TABLE marketplace_apps ENABLE ROW LEVEL SECURITY;
ALTER TABLE marketplace_apps FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS marketplace_apps_global_access ON marketplace_apps;
CREATE POLICY marketplace_apps_global_access ON marketplace_apps USING (true) WITH CHECK (true);

ALTER TABLE connector_registry ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_registry FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS connector_registry_global_access ON connector_registry;
CREATE POLICY connector_registry_global_access ON connector_registry USING (true) WITH CHECK (true);
