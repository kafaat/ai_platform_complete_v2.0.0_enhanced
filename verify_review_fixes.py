...
def bands(ndvi):  # bands تعطي NDVI مطلوباً تقريباً (B08 vs B04)
    return {"B02":0.05,"B03":0.1,"B04":0.1,"B05":0.2,
            "B08":0.1*(1+ndvi)/(1-ndvi) if ndvi<1 else 5.0,"B11":0.15,"B12":0.1}
...
check("_daily_at يُرجع الافتراضي لفهرس خارج الطول (لا IndexError)",
...
def cov(obs, exp): return ce.CoverageConfidence(pixels_observed=obs, pixels_expected=exp).score
s49, s50, s51 = cov(49,100), cov(50,100), cov(51,100)
check("ثقة التغطية متّصلة عند 0.5 (قفزة < 0.02)", abs(s50 - s49) < 0.02, f"s49={s49:.3f} s50={s50:.3f}")
...
check("التسمية المعتمدة fertilizer.applied في CURRENT_VERSIONS",
...
async def _journal_test():
    ...
    await j.record_start(invocation_id="inv1", tool_id="actuator.valve",
                         input_data={"a":1}, actor_capabilities=["x"],
                         tenant_id="t1", contract_version="1.0")
...
res_irr = asyncio.run(wf.simulate_wofost("f","قمح صلب","loam",15.3,44.2,_dt.date(2025,1,1),1.0,True))
res_rain = asyncio.run(wf.simulate_wofost("f","قمح صلب","loam",15.3,44.2,_dt.date(2025,1,1),1.0,False))
