...
from shared.gis.cloud_native_runtime import RasterRegistryRecord, stac_item_from_record
...
def stac_landing_page(*, api_base: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "links": [
            {"rel": "self", "type": "application/json", "href": href("/api/v1/gis/cloud-native/stac")},
            {"rel": "root", "type": "application/json", "href": href("/api/v1/gis/cloud-native/stac")},
            {"rel": "service-desc", "type": "application/vnd.oai.openapi+json;version=3.0", "href": href("/openapi.json")},
            {"rel": "conformance", "type": "application/json", "href": href("/api/v1/gis/cloud-native/stac/conformance")},
            {"rel": "data", "type": "application/json", "href": href("/api/v1/gis/cloud-native/stac/collections")},
            {"rel": "search", "type": "application/geo+json", "href": href("/api/v1/gis/cloud-native/stac/search"), "method": "GET"},
            {"rel": "search", "type": "application/geo+json", "href": href("/api/v1/gis/cloud-native/stac/search"), "method": "POST"},
            {"rel": "queryables", "type": "application/schema+json", "href": href("/api/v1/gis/cloud-native/stac/queryables")},
        ],
    }
...
def stac_queryables() -> dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "index_type": {"type": "string", "enum": ["ndvi", "ndmi", "ndre", "savi", "truecolor", "evi", "all"]},
...
def stac_collections_response(records: Iterable[RasterRegistryRecord], *, api_base: str = "") -> dict[str, Any]:
    ...
    for idx, items in sorted(by_index.items()):
        ...
        bbox = [[min(b[0] for b in bboxes), min(b[1] for b in bboxes), max(b[2] for b in bboxes), max(b[3] for b in bboxes)]] if bboxes else [[-180, -90, 180, 90]]
    ...
    return {"collections": collections, "links": [{"rel": "root", "href": f"{api_base}/api/v1/gis/cloud-native/stac"}]}


def filter_records_for_stac(records: Iterable[RasterRegistryRecord], *, field_id: str | None = None, index_type: str | None = None, min_quality: int | None = None, max_cloud: float | None = None, bbox: list[float] | None = None) -> list[RasterRegistryRecord]:
    out = []
    for r in records:
...
def rank_scenes(records: Iterable[RasterRegistryRecord], *, prefer_recent: bool = True, max_cloud_pct: float = 35) -> list[dict[str, Any]]:
    ...
    for rec in records:
        ...
        age_penalty = max(0, min(15, (now_ord - rec_date.toordinal()) / 30)) if prefer_recent and rec_date else 0
    ...
    for i, (score, rec, quality) in enumerate(scored, start=1):
        ranked.append(asdict(RankedScene(
...
def build_scene_processing_plan(records: Iterable[RasterRegistryRecord], *, field_id: str | None = None, index_type: str | None = None) -> dict[str, Any]:
    ...
    return {
        "pipeline": ["discover", "cloud_shadow_score", "rank", "select", "mosaic", "cog_validate", "tilejson", "cache_warm"],
...
def ogc_collections() -> dict[str, Any]:
    return {
        "collections": [
            {"id": "fields", "title": "Fields", "itemType": "feature", "crs": ["http://www.opengis.net/def/crs/OGC/1.3/CRS84"], "links": [{"rel": "items", "href": "/api/v1/gis/cloud-native/ogc/collections/fields/items"}]},
            {"id": "rasters", "title": "Raster products", "itemType": "coverage", "crs": ["http://www.opengis.net/def/crs/OGC/1.3/CRS84"], "links": [{"rel": "tiles", "href": "/api/v1/gis/cloud-native/ogc/collections/rasters/tiles"}]},
        ]
    }


def ogc_feature_collection(rows: Iterable[dict[str, Any]], *, number_matched: int | None = None) -> dict[str, Any]:
    features = []
    for row in rows:
    ...
    return {"type": "FeatureCollection", "features": features, "numberReturned": len(features), "numberMatched": number_matched if number_matched is not None else len(features)}


def tile_cache_plan(records: Iterable[RasterRegistryRecord], *, minzoom: int = 8, maxzoom: int = 14) -> dict[str, Any]:
    entries = []
    for rec in records:
        cache_key = _stable_hash({"id": rec.id, "cog": rec.cog_url, "z": [minzoom, maxzoom]})
        entries.append({"raster_id": rec.id, "index_type": rec.index_type, "cache_key": cache_key, "minzoom": minzoom, "maxzoom": maxzoom, "ttl_seconds": 86400 if (rec.quality_score or 0) >= 70 else 21600})
    return {"strategy": "cdn+nginx+redis", "entries": entries, "purge_on": ["raster_registry_update", "geometry_revision_rollback"]}
...
def ai_boundary_extraction_plan(*, field_id: str, input_type: str, bbox: list[float] | None = None, model: str = "sam2-geosam") -> dict[str, Any]:
    return {
        "field_id": field_id,
        "model": model,
        "input_type": input_type,
        "bbox": bbox,
        "steps": ["load_imagery", "prompt_or_auto_segment", "polygonize", "simplify", "topology_validate", "human_review", "commit_revision"],
        "status": "planned",
...
def management_zone_summary(values: list[float], *, n_zones: int = 3) -> dict[str, Any]:
    ...
    labels = ["low", "medium", "high"] if n_zones == 3 else [f"zone_{i+1}" for i in range(n_zones)]
    counts = [0] * n_zones
    for value in clean:
        z = sum(1 for cut in cuts if value >= cut)
        counts[min(z, n_zones - 1)] += 1
    return {"n_zones": n_zones, "zones": [{"zone": labels[i], "count": counts[i], "pct": round(100 * counts[i] / len(clean), 1)} for i in range(n_zones)]}


def apply_undo_redo(session: dict[str, Any], *, action: str, event: dict[str, Any] | None = None) -> dict[str, Any]:
    undo = list(session.get("undo_stack") or [])
    redo = list(session.get("redo_stack") or [])
    if action == "push":
        if event is None:
            raise ValueError("event is required for push")
        undo.append({**event, "pushed_at": datetime.now(timezone.utc).isoformat()})
        redo.clear()
    elif action == "undo":
        if undo:
    ...
    return {**session, "undo_stack": undo, "redo_stack": redo, "can_undo": bool(undo), "can_redo": bool(redo)}
