...
import asyncio, hashlib as _hashlib, json as _json, logging, os, re, time
...
async def init_db(dsn: Optional[str] = None, min_size: int = 2, max_size: int = 10) -> asyncpg.Pool:
    global _db_pool
...
async def get_pool() -> asyncpg.Pool:
    if _db_pool is None: return await init_db()
    return _db_pool
...
async def close_pool():
    global _db_pool
    if _db_pool: await _db_pool.close(); _db_pool = None
...
async def close_redis():
    global _redis_client
    if _redis_client: await _redis_client.close(); _redis_client = None

# ── NATS ───────────────────────────────────────────────────────
_nats_conn = None

async def init_nats(url: Optional[str] = None):
    global _nats_conn
    try:
        import nats
...
def get_nats(): return _nats_conn

async def close_nats():
    global _nats_conn
    if _nats_conn and not _nats_conn.is_closed:
        await _nats_conn.close(); _nats_conn = None
...
async def tenant_context(conn: asyncpg.Connection, tenant_id: str):
    ...
    try: yield conn
    finally: await conn.execute("SELECT set_config('app.current_tenant', '', true)")
...
async def require_tenant(request: Request) -> str:
    ...
    if not tid: raise HTTPException(400, "tenant_id missing")
    return _validate_tenant_id(tid)

async def query_with_tenant(conn, tenant_id: str, query: str, *args):
    async with tenant_context(conn, tenant_id): return await conn.fetch(query, *args)

async def execute_with_tenant(conn, tenant_id: str, query: str, *args):
    async with tenant_context(conn, tenant_id): return await conn.execute(query, *args)
...
def validate_role(role: str) -> str:
    if role not in VALID_ROLES:
        logging.warning(f"Invalid role: {role!r} -> farmer"); return "farmer"
    return role
...
def setup_prometheus(service_name: str):
    ...
    _REQUEST_COUNT = Counter("sahool_http_requests_total","HTTP requests",
        ["service","method","path","status"])
    _REQUEST_LATENCY = Histogram("sahool_http_request_duration_seconds","Latency",
        ["service","path"])
...
def metrics(service_name: str):
    async def _mw(request: Request, call_next) -> Response:
        ...
        if _REQUEST_COUNT:
            _REQUEST_COUNT.labels(service=service_name, method=request.method,
                path=request.url.path, status=response.status_code).inc()
        if _REQUEST_LATENCY:
...
def add_probes(app: FastAPI, pool_getter=None, service_name: str = "service"):
    @app.get("/healthz")
    @app.get("/health")
    async def _health(): return {"status":"alive","service":service_name}

    @app.get("/readyz")
    async def _ready():
        if pool_getter:
            try:
                p = await pool_getter() if asyncio.iscoroutinefunction(pool_getter) else pool_getter()
    ...
    @app.get("/metrics")
    async def _metrics(): return Response(generate_latest(), media_type=CONTENT_TYPE_LATEST)

# ── Lifespan ───────────────────────────────────────────────────
def service_lifespan(app_or_db=None, *args, with_db: bool=True, with_redis: bool=True, with_nats: bool=False, **kwargs):
    @asynccontextmanager
    async def lifespan(_app_inner=None):
        if with_db:    await init_db()
        if with_redis: await init_redis()
        if with_nats:  await init_nats()
        yield
        if with_db:    await close_pool()
        if with_redis: await close_redis()
        if with_nats:  await close_nats()
    return lifespan

# ── NATS Events ────────────────────────────────────────────────
async def publish_event(subject: str, data: dict, service: str="unknown") -> bool:
    nc = get_nats()
    if not nc or nc.is_closed: return False
    try:
        js = nc.jetstream()
        await js.publish(subject, _json.dumps(data, ensure_ascii=False).encode())
        return True
    except Exception as e:
        logging.warning(f"[{service}] NATS {subject}: {e}"); return False

async def dispatch(event_type: str, data: dict, tenant_id: str="", service: str="unknown") -> bool:
    subject = f"sahool.tenant.{tenant_id or 'default'}.{event_type}"
    return await publish_event(subject, {"event_type":event_type,"tenant_id":tenant_id,
        "data":data,"timestamp":datetime.now(timezone.utc).isoformat()}, service=service)

async def wrapped_send(method: str, url: str, payload: dict, headers: Optional[dict]=None) -> dict:
    import httpx
...
def cached(ttl_seconds: int=300, prefix: str="cache"):
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            rc = get_redis()
            if not rc: return await func(*args, **kwargs)
            ...
            try:
                v = await rc.get(key)
                if v: return _json.loads(v)
            except Exception: pass
            result = await func(*args, **kwargs)
            try: await rc.setex(key, ttl_seconds, _json.dumps(result, ensure_ascii=False))
            except Exception: pass
            return result
        return wrapper
    return decorator
...
async def get_data(key: str, default=None):
    rc = get_redis()
    if not rc: return default
    try:
        v = await rc.get(key); return _json.loads(v) if v else default
...
def check_rate_limit(key: str, max_requests: int=5, window_seconds: int=60) -> bool:
    now = datetime.now(timezone.utc); window = now - timedelta(seconds=window_seconds)
    _rate_store[key] = [t for t in _rate_store[key] if t > window]
    if len(_rate_store[key]) >= max_requests: return False
    _rate_store[key].append(now); return True

# ── App factory ────────────────────────────────────────────────
def create_app(service_name: str, version: str="9.0.0",
               with_db: bool=True, with_redis: bool=True, with_nats: bool=False) -> FastAPI:
    cors_origins = os.getenv("CORS_ORIGINS","http://localhost:3000").split(",")
    app = FastAPI(title=f"SAHOOL {service_name}", version=version,
    ...
    **kwargs,
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logging.warning(f"retry_request: attempt {attempt+1}/{max_attempts} failed: {e} — retrying in {delay:.1f}s")
