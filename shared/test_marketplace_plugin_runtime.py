...
def manifest(permissions=None):
    return {
        ...
        "version": "1.0.0",
        "author": "sahool",
        "permissions": permissions or ["field.read", "tiles.read", "recommendations.write", "webhooks.write"],
...
def test_plugin_execution_plan_allows_declared_low_risk_action():
    ...
    result = plan_plugin_execution(app, installation, "recommendation.propose", {"field_id": "f1"}, {"api_calls_day": 0}, "idem-1")
...
def test_plugin_execution_denies_missing_permission_and_quota_exceeded():
    ...
    blocked = plan_plugin_execution(app2, installation2, "raster.tile.read", {}, {"tiles_day": 50_000}, "idem-3")
...
def test_sensitive_action_requires_review_not_direct_execution():
    ...
    app["status"] = "approved"
    installation = install_marketplace_app(app, "11111111-1111-1111-1111-111111111111", "admin")["installation"]
...
def test_plugin_output_validation_blocks_direct_side_effects():
    app, installation = approved_app_and_installation()
    plan = plan_plugin_execution(app, installation, "recommendation.propose", {}, {}, "idem-5")["plan"]
    safe = validate_plugin_output(plan, {"effects": [{"kind": "recommendation_proposal"}]})
    assert safe["valid"] is True
    unsafe = validate_plugin_output(plan, {"effects": [{"kind": "direct_db_write"}, {"kind": "actuator_command"}]})
...
def test_plugin_event_envelope_sanitizes_secrets_and_rejects_unknown_events():
    ...
    env = build_plugin_event_envelope(plan, "plugin.recommendation.proposed", {"ok": 1, "token": "secret"})
...
def test_plugin_runtime_report_summarizes_actions():
    app, installation = approved_app_and_installation()
    report = build_plugin_runtime_report(app, installation, ["field.context.read", "recommendation.propose"])
