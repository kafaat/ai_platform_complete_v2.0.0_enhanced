...
def _runtime():
    return {
        ...
        "canonical_state": {
            "state_id": "cfs_1",
            "field_id": "field-1",
            "state": {"confidence": 0.82, "operational_truths": {"ndvi": 0.42}},
        },
...
def test_verification_effect_and_feature_store_batch():
    rt = _runtime()
    plan = plan_closed_loop_execution(
        rt["recommendation_lifecycle"], mode="full_autonomy", policy={"full_autonomy_enabled": True}
    )
    cmd_id = plan["commands"][0]["command_id"]
    verification = verify_execution_effect(
        plan, telemetry={"acknowledged_command_ids": [cmd_id], "applied": {"water_mm": 10}}
    )
...
def test_model_registry_and_experiment_assignment_are_deterministic():
    ...
    a = assign_experiment_variant(
        entity_id="field-1", experiment_key="water_model", variants=["champion", "challenger"]
    )
    b = assign_experiment_variant(
        entity_id="field-1", experiment_key="water_model", variants=["champion", "challenger"]
    )
...
def test_run_phase9_autonomy_cycle_produces_learning_candidate():
    cycle = run_phase9_autonomy_cycle(
        canonical_runtime=_runtime(), mode="full_autonomy", policy={"full_autonomy_enabled": True}
    )
