...
def _domain() -> dict:
    ...
    with contextlib.redirect_stdout(_sink):
        ...
        try:
            ...
            p, f = ch.run_all()
            out["chaos"] = {"status": "pass" if f == 0 else "fail", "passed": p, "total": p + f}
        except Exception as ex:
...
def _invariant_flags() -> dict:
    ...
    return {
        "tenant_rls_fail_closed": has("migrations/v9_rls_tenant_isolation.sql", "FORCE", "NULLIF"),
        "temporal_invariant": has(
            "services/sahool-platform/api/field_lifecycle.py", "enforcement_mode", "occurred_at"
        ),
        "sync_idempotency": has(
            "services/edge-inference/sync_service.py", "idempotency_key", "occurred_at"
        ),
        "edge_dedup": has("services/sahool-platform/api/main.py", "ON CONFLICT (idempotency_key)"),
        "approval_locked": has("services/guardrails-engine/human_in_loop.py", "FOR UPDATE"),
        "raster_provenance": has("services/raster-service/raster_provenance.py", "provenance_hash"),
