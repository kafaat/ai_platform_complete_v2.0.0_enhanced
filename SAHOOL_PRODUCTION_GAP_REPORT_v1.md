...
| H3 | أحداث `DISPATCH_*` لموزِّع القرار (تدقيق) | ✅ مُصلَح | #357 |
| H4-PM | توحيد Penman-Monteith في `core/engines/et0.py` | ✅ مُصلَح | #356 |
| UX | الخطّ الزمنيّ الموحّد للحقل (MVP، قراءة فقط) | ✅ مُصلَح | #358 |
| UX | عرض الثقة/المصدريّة في شاشة التوصيات (Provenance UX) | ✅ مُصلَح | #359 |
| SEC | تصليب `sharing.py` (إزالة fallback المستأجِر `default`) | ✅ مُصلَح | #360 |
| PRE-PROD | Advisor Context Binding (إرفاق الحالة القانونيّة) | ✅ مُصلَح | #361 |
| PRE-PROD | Guardrails Contract (رفض السياق الناقص، fail-closed) | ✅ مُصلَح | #362 |
| PRE-PROD | RBAC viewer tightening (حجب الماليّ/التحليليّ) | ✅ مُصلَح | #363 |
| R6 | النصف البوّابيّ: `nginx auth_request` يحقن `X-Tenant-Id` موثوقاً | ⏳ يحتاج تحقّقاً حيّاً | — |
| H1 | تفويض fail-closed عالميّ (96/319 نقطة مكشوفة — غالبها GET مرجعيّ) | ⏳ يحتاج قراراً | — |
| H2 | أنبوب الطقس الشبكيّ خامد (سقالة بلا منتِج — انظر الملحق) | ⏳ قرار: ميزة لا إصلاح | — |
| H3 | `WORK_ORDER_*` | ⏳ لا نقطة كتابة (لا أحداث مُخترَعة) | — |
| H5 | دمج الملوحة في الريّ المبسّط (يغيّر نصيحة الحقول المالحة) | ⏳ يحتاج إقراراً زراعيّاً | — |
| OFFLINE | مزامنة الحقول offline الكاملة (انظر الملحق) | ⏳ ميزة كبرى: قرار سياسة تعارض + Flutter | — |
| C4/M1 | الموبايل: عميل WebSocket + بنية push + شاشة طقس/timeline | ⏳ يتطلّب بيئة Flutter | — |
...
- **مزامنة offline تدقيقيّة لا تطبيقيّة (اكتشاف حرج):** `/api/v1/sync` +
  `core/offline_first.py` يملكان طابوراً/idempotency/supersession، لكنّ
  `persist_synced_operation` تكتب **كلّ** عمليّة في جدول تدقيق واحد
  (`offline_synced_operations`) **بلا تفريع حسب النوع ولا كتابة في جداول الـdomain**.
  أي حتى الأنواع الخمسة «المدعومة» تُسجَّل ولا تُطبَّق. وFlutter **بلا تخزين محلّيّ
  للبيانات أصلاً** (`offline_field_map` = بلاطات خريطة لا مزامنة بيانات). فإكمالها =
  **ميزة كبرى** (طبقة dispatch جديدة + سياسة تعارض + عمل Flutter كامل)، لا توسيع enum.
  أصغر شريحة آمنة لاحقة: `field.update` خادميّ عبر dispatcher يعيد استخدام التزامن
  التفاؤليّ (`row_version`/409) القائم.
