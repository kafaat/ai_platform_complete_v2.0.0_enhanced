...
// إعادة تصدير عملاء HTTP للمستهلِكين القائمين (import { kongApi, ... } from '.../services/api').
export {
  authApi,
  indicatorsApi,
  kongApi,
...
// ══════════════════════════════════════════════════════════════════
// AUTH / ACCOUNT — مُستخرَج إلى accountApi.ts ومُعاد تصديره (export * أدناه):
//   LoginPayload · AuthResponse · login · register · logout · إعادة تعيين كلمة
//   المرور · MFA · changePassword · تحقّق البريد/الهاتف · الدعوات · تكوين المستأجِر.
// مُعالِجات الأخطاء العامّة (asApiError/apiErrorMessage/isMfaRequiredError/ApiError)
// تبقى هنا لأنّها مقطعيّة تستهلكها دوالّ أخرى في هذا الملفّ.
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// تكوين المستأجِر (fetchTenantConfig + TenantConfig/TenantBranding) مُستخرَج إلى
// accountApi.ts ومُعاد تصديره عبر `export * from './accountApi'` أدناه.
...
// دوالّ مجال الحقول مُستخرَجة إلى fieldsApi.ts ومُعاد تصديرها (export * أدناه):
//   FieldDetail · FieldUpdatePatch · fetchFieldDetail · updateField · الوصفات
//   اليدويّة · importField · mergeFields/splitField · FieldWorkspace وما يتبعها.

// دوالّ مجال الصور مُستخرَجة إلى imageryApi.ts ومُعاد تصديرها:
export * from './imageryApi';
...
// دوالّ مجالات vegetation/weather/soil مُستخرَجة (تُعاد تصديرها):
export * from './vegetationApi';
export * from './weatherApi';
export * from './soilApi';
// دوالّ مجالَي الحقول والمصادقة/الحساب مُستخرَجة (تُعاد تصديرها):
...
// بيانات mock مُستخرَجة إلى apiMocks.ts (تُستهلَك عبر tryReal في MOCK_MODE):
// دوالّ الغطاء النباتيّ/الطقس/التربة نُقِلت إلى وحدات مجالها وتستورد mock-ها مباشرةً،
// فبقي MOCK_DASHBOARD وحده مُستهلَكاً هنا؛ MOCK_FIELDS يُعاد تصديره فقط.
import { MOCK_DASHBOARD } from './apiMocks';
export { MOCK_FIELDS } from './apiMocks';

// مجال أخذ العيّنات المخبريّة ومناطق الإنتاجيّة (Lab Sampling + productivity zones)
// نُقِل إلى labSamplingApi.ts ويُعاد تصديره أدناه عبر `export *`.
export * from './labSamplingApi';
