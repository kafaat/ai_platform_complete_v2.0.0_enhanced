...
// ── تجزئة الباروميل (Barrel split): مفاتيح الاستعلام + هوكات الطقس/التربة ──
// QK مُستخرَجة إلى وحدة ورقيّة (leaf) تشاركها الوحدات المجزّأة بلا اعتماد دائريّ،
// وهوكات الطقس/التربة نُقلت حرفيّاً إلى وحدات دومين يُعاد تصديرها هنا (التوافق الخلفيّ
// محفوظ: كلّ `import { … } from '.../hooks/useApi'` يبقى يعمل).
import { QK } from './useApiKeys';
export { QK } from './useApiKeys';
export * from './useWeatherApi';
export * from './useSoilApi';
// هوكات المعايرة (Calibration Workbench + حالة المعايرة الإقليميّة) نُقلت حرفيّاً إلى
// وحدة دومين يُعاد تصديرها هنا — التوافق الخلفيّ محفوظ (المستهلِكون + تجسّس الاختبار
// `vi.spyOn(useApiModule, 'useCalibration…')` يبقى يعمل عبر فضاء أسماء useApi).
export * from './useCalibrationApi';
// useDashboardData يستهلك useWeatherForecast محليّاً ⇒ استيراد صريح (القيمة) من
// وحدة الطقس (التي لا تستورد من useApi.ts ⇒ لا اعتماد دائريّ).
import { useWeatherForecast } from './useWeatherApi';
