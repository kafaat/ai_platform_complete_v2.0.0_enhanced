...
export function weatherTileSvg(
  ...
  const c0 = colorAt(value, cfg, -0.30 + seedA * 0.12);
  ...
  const particleOpacity = layer === 'wind' ? 0.92 : 0.70;
  ...
  const densityProfile = windDensity === 'low'
    ...
    : windDensity === 'medium'
      ? { rows: layer === 'wind' ? 14 : 11, cols: layer === 'wind' ? 7 : 5 }
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${WEATHER_TILE_SIZE}" height="${WEATHER_TILE_SIZE}" viewBox="0 0 256 256" preserveAspectRatio="none" role="img" aria-label="${cfg.labelAr}: ${shown} ${unit}">
    <defs>
      <radialGradient id="${heatId}" cx="${30 + seedA * 48}%" cy="${26 + seedB * 56}%" r="76%">
        <stop offset="0%" stop-color="${c3}" stop-opacity="0.70"/>
        <stop offset="42%" stop-color="${c2}" stop-opacity="0.58"/>
        <stop offset="78%" stop-color="${c1}" stop-opacity="0.50"/>
        <stop offset="100%" stop-color="${c0}" stop-opacity="0.42"/>
      </radialGradient>
      <linearGradient id="${gradId}" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="${c0}" stop-opacity="0.48"/>
        <stop offset="35%" stop-color="${c1}" stop-opacity="0.56"/>
