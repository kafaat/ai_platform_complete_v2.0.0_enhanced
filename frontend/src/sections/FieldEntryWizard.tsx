  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Noto Kufi Arabic', system-ui, sans-serif", background: "linear-gradient(160deg, #14201a 0%, #1c2b22 100%)", minHeight: "100vh", color: "#e8eee9", padding: 20 }}>
      ...
      <div style={{ maxWidth: 580, margin: "0 auto", background: "#1a2b21", borderRadius: 16, padding: 22, border: "1px solid #2d4a37", boxShadow: "0 8px 32px rgba(0,0,0,.3)" }}>
        ...
        {step === 4 && (<div>
          <H icon="🧪" t="التربة والمياه" s="اختر طريقة المتابعة — لكل خيار حالة مختلفة" />
          <div style={{ marginBottom: 14 }}>
            <button onClick={() => set("soilChoice", "provided")} style={cBtn(d.soilChoice === "provided")}><span>✏️ سأُدخل التحاليل الآن</span><span style={{ fontSize: 11, opacity: .7 }}>جاهز 🟢</span></button>
            <button onClick={() => set("soilChoice", "request_lab")} style={cBtn(d.soilChoice === "request_lab")}><span>🧫 اطلب تحليلاً مخبرياً</span><span style={{ fontSize: 11, opacity: .7 }}>بانتظار المعمل 🔵</span></button>
            <button onClick={() => set("soilChoice", "skip")} style={cBtn(d.soilChoice === "skip")}><span>⏭️ تخطّي (توصيات عامة)</span><span style={{ fontSize: 11, opacity: .7 }}>محدود 🟡</span></button>
          </div>
          {d.soilChoice === "request_lab" && <div style={{ fontSize: 12, color: "#cdddd2", background: "#0d1611", borderRadius: 8, padding: "10px 14px", marginBottom: 14, borderRight: "3px solid #4a90d4", lineHeight: 1.6 }}>🧫 سيُرسل طلب تحليل للمعمل. توصيات الري والطقس متاحة الآن، وتُفعّل الدقيقة عند وصول النتائج.</div>}
          {d.soilChoice === "skip" && <div style={{ fontSize: 12, color: "#cdddd2", background: "#0d1611", borderRadius: 8, padding: "10px 14px", marginBottom: 14, borderRight: "3px solid #d4a017", lineHeight: 1.6 }}>⏭️ توصيات عامة فقط (ري، طقس، NDVI). توصيات المبيدات تبقى محجوبة دائماً (سلامة المستهلك).</div>}
          {d.soilChoice === "provided" && (<div>
        ...
        {step === 6 && (<div>
          ...
          {(() => {
            const filled = [d.lat && d.lon, d.crop, d.plantingDate, d.irrigation, d.salinity, d.ph, d.waterSalinity, d.om];
            const weights = [15, 15, 10, 10, 15, 15, 15, 5];
            const score = filled.reduce((s, f, i) => s + (f ? weights[i] : 0), 0);
            const precise = d.salinity && d.ph && d.waterSalinity;
            const msg = precise ? "🟢 بياناتك كاملة — توصياتك دقيقة وقيّمة"
              : score >= 60 ? "🟡 توصيات تقريبية جيدة — أكمل التحاليل الحاكمة لدقّة كاملة"
              : score >= 30 ? "🟠 توصيات عامة الآن — كلما أكملت بياناتك، زادت قيمة توصياتك"
              : "⚪ ابدأ بإدخال بياناتك — كل حقل يفتح توصيات أكثر قيمة";
            return (
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#9cb8a3", marginBottom: 6 }}>
                  <span>اكتمال البيانات</span><span style={{ fontWeight: 700, color: precise ? "#5cbf6e" : "#d4a017" }}>{score}%</span>
                </div>
                <div style={{ height: 8, background: "#0d1611", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${score}%`, background: precise ? "#5cbf6e" : "#d4a017", borderRadius: 4, transition: "width .4s" }} />
                </div>
                <div style={{ fontSize: 12, color: "#cdddd2", marginTop: 8, lineHeight: 1.6 }}>{msg}</div>
              </div>
            );
          })()}
