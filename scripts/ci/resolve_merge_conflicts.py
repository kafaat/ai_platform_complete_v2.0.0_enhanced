...
#
# **و`operational_evidence_log.md` دخلها بعطلٍ وقع، لا باحتياط** — `AN-APPEND-ONLY-
# JOURNAL-OUTSIDE-THE-APPEND-ONLY-LIST-01` (قِيس 2026-09-21): رأسُ #1061 المدموج
# (`b067f92c`) حذف منه المدخلتين `E-2026-09-21-06` و`E-2026-09-21-07` (٣٦٣٧ بايت)،
# وكلُّ بوّابةٍ خضراء، فلزمت شريحةُ استرجاعٍ (#1062). والسببُ أنّ فرعَ #1061 أُسِّس
# على `0606bb67` — **قبل** #1059 الذي كتب المدخلتين — فإعادةُ تأسيسٍ تأخذ جانبَ الفرع
# في هذا الملفّ تُعيد ماضياً وتمحو ما لحقه، **بلا تعارضٍ يُظهِرها**.
#
# وهو الصنفُ نفسُه الذي وُجِد لأجله `brain_append_only_guard` (تفريغُ `log.md` في
# `cb6598fe`) — والحارسُ يستورد هذه القائمة بعينها، فغيابُ الملفّ عنها كان غيابَ
# الحراسة عنه. والملفُّ يصف نفسه في سطره الثالث: «دفترٌ **إلحاقيٌّ** مقيَّدٌ بـSHA».
#
# **والإضافةُ مقيسةٌ لا مفترَضة:** على كامل تاريخ الملفّ **١٩ زوجَ (التزام، أب)
# وانكماشٌ واحد** — هو `b067f92c ← d2742aba` بعينه. أي أنّ الحارسَ كان سيحجب الحادثةَ
# ولا يحجب شيئاً من العمل الصحيح؛ ليس «بوّابةً لا تُغلَق بعملٍ صحيح».
APPEND_ONLY = (
    "sahool-brain/log.md",
