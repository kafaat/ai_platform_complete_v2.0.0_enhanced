...
فعلاً** في `gaps/registry.md` — قسماً `## ` أو صفَّ جدول يبدأ عموده الأوّل بالمعرّف. الأسطر القائمة قبل الحارس مُجمَّدة في أساس
...
def check() -> int:
    ...
    if found:
        ...
        for v in found:
            print(f"  ✗ {v}")
        print(
            "\nكلّ تأجيل في hot.md يحتاج معرّف فجوة مُعلَناً في "
            "sahool-brain/gaps/registry.md **بأحد الشكلين** — قسم '## المعرّف' أو صفّ "
            "جدول يبدأ عموده الأوّل بالمعرّف — قاعدة CLAUDE.md «لا فجوة بلا مصدر + حالة»."
        )
        return 1
