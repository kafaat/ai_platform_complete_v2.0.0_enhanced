...
(جرد ⇒ كتالوج ⇒ حزمة)، ومداها الحقيقيّ **٧٤ خطوة موزّعة على ١٩ workflow** (مقيس
على `2bf8814f`؛ كان ٤٧/١٨ عند الكتابة). الفارق ليس
...
_GENERATE_FLAG.update(
    {
        ...
        "tenant_guc_scope_guard.py": "--generate",
        # أساسُ ملكيّة الكتابة (`OWNERSHIP-CONTRACT-DECLARED-BUT-NEVER-MEASURED-01`):
        # راتشِتٌ ينزل ولا يصعد، فإعادةُ التوليد تُستعمَل **بعد** نقلِ كتابةٍ إلى
        # مالكها أو تصحيحِ العقد بأساسٍ مُعلَن — لا لابتلاع مخالفةٍ جديدة (تلك
        # يحجبها `--check` قبل أن تبلغ هنا).
        "db_writer_ownership_guard.py": "--generate",
    }
)
...
# بيانُ أهداف الكتابة مشتقٌّ من **شجرة صياغة** المولّدات، فيتغيّر كلّما أُضيف كاتبٌ
# أو نُقِل هدف. واشتقاقُه ساكنٌ رخيص (~ثانيتان) ولا يُشغّل المكنسة، فلا استدعاء
# دائريّ — وهذا ما تغيّر عن أوّل تصميمٍ له: قياسٌ ديناميكيّ كان يُشغّلها فيكسر
# نقطة ثباتها، ولذلك كان يُسجَّل استثناءً بدل أن يُوصَل.
_GENERATE_FLAG.update({"generated_write_targets.py": "--generate"})
...
def main() -> int:
    ...
    if blind:
        ...
        if unknown:
            # **حاجبٌ الآن، لا إبلاغ.** شرط إغلاق
            # `GENERATED-WRITE-ONLY-GENERATORS-UNCLASSIFIED-01` مكتوبٌ في سجلّ
            # الفجوات: «إمّا فرض تصنيف صريح على هذه المجموعة كما يُفرَض على معلني
            # `--check`، وإمّا إثبات أنّ المجموعة لا يمكن أن تنمو» — والثاني لا يسنده
            # شيء. وكانت الكتلة تطبع ثمّ تُرجِع صفراً، أي **عبارةٌ صادقة بلا فرض**:
            # تنتظر قارئاً ينتبه إلى سطرٍ بين مئات، وهو بعينه صنف «الوجود ليس قياساً».
            #
            # والتحويل قِيس آمناً قبل إجرائه: المجموعة **صفر** على `258a5835`، فلا
            # يُحمِرّ شيئاً اليوم ويحجب أوّل مولّد كتابة يدخل الشجرة بلا تصنيف غداً.
            # ولمَ الحجب صحيح: مولّدٌ بلا `--check` لا يستطيع الإبلاغ عن انحرافه
            # أصلاً، فمصنوعته تنحرف بصمت ولا بوّابة تراها.
            print(
            ...
            print(
                "\n  صنِّفه في `generated_chain_known_drift.json` بسببٍ مكتوب، "
                "أو أعطِه `--check` وأدخِله الاكتشاف. الطباعة وحدها لا تحرس."
            )
            return 1
