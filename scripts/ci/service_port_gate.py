...
def main() -> int:
    ...
    if "localhost:8126/healthz" not in text(erp.get("healthcheck", {})):
        failures.append(
            "docker-compose.unified.yml erp-bridge healthcheck must use localhost:8126/healthz"
        )
    ...
    out.write_text(
        json.dumps(
            {"gate": "service-port-gate", "passed": not failures, "failures": failures},
            indent=2,
            ensure_ascii=False,
        )
        + "\n"
