...
checks = {
    ...
    not in (root / "services/vegetation-analysis-service/vegetation_runtime.py").read_text(encoding="utf-8"),
    "vegetation current NDVI authoritative": "_current_ndvi_from_raster"
    in (root / "services/vegetation-analysis-service/routers/analysis.py").read_text(encoding="utf-8"),
    "vegetation readiness raster-aware": "authoritative-raster-only"
    in (root / "services/vegetation-analysis-service/routers/health.py").read_text(encoding="utf-8"),
    ...
    "agriai readiness pcse-aware": (
        ...
        and "verified_missing" in (root / "services/agriai-engine/main.py").read_text(encoding="utf-8")
    ),
