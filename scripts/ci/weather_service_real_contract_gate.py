...
# P2 decomposition: main.py registers routes call-style (app.get(path)(rt.handler)) and
# the runtime logic lives in weather_runtime.py — markers are checked against BOTH files.
# `app.get(` (no @) matches decorator and call-style registration alike.
...
def check_weather_service_runtime_source() -> None:
    main = read(WEATHER / "main.py") + read(WEATHER / "weather_runtime.py")
    ...
    if found:
        fail("weather-service main/runtime contains stub/not-implemented markers: " + repr(found))
...
def check_compose_and_local_scripts() -> None:
    ...
    if (
        "WEATHER_SERVICE_URL" not in compose_text
        or "sahool-weather-service:8000" not in compose_text
    ):
        fail(
            "compose files do not wire WEATHER_SERVICE_URL to sahool-weather-service internal port"
        )
    local_scripts = "\n".join(
        read(p)
        for p in [ROOT / "frontend" / "run_all.sh", ROOT / "frontend" / "run_all.ps1"]
        if p.exists()
    )
    forbidden = [
        "localhost:8092/health'",
        "localhost:8092/health ",
        "weather-service (WOFOST)",
        "8092:/health:",
    ]
...
def check_runtime_contract_with_testclient() -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        "sahool_weather_service_main", WEATHER / "main.py"
    )
    if not spec or not spec.loader:
        fail("unable to import weather-service main.py")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    # weather_runtime._facade_attr resolves monkeypatchable hooks via sys.modules["main"]
    # (matching the container's `uvicorn main:app`); register the module under that name
    # too so the fake fetch_*/readiness_probe below are actually picked up.
    ...
    async def fake_forecast(lat: float, lon: float, *, days: int = 7, model: str = "best_match"):
        return {
            "location": {"lat": lat, "lon": lon},
            "source": "open-meteo",
            "daily": [{"date": "2026-07-09", "et0_mm": 5.1}],
        }
