...
def payload_pathspec() -> list[str]:
    ...
    **بلا مستدعٍ إنتاجيّ منذ حذف `generated_at`** (‏`RELEASE-MANIFEST-SELF-REGENERATION-01`):
    كان مستهلكها الوحيد هو اشتقاق الختم. تبقى لأنّ حذفها وحذف اختبارها خارج نطاق
    تلك الشريحة، **وتُعلَن هنا** بدل أن تبقى شيفرةً ميّتة يحرسها اختبار أخضر — وهو
    الوهم نفسه الذي تعالجه حرّاس هذا المستودع.
...
def main() -> int:
    ...
    manifest = {
        ...
        # `generated_at` حُذِف — `RELEASE-MANIFEST-SELF-REGENERATION-01`.
        #
        # **ولم يكن ساعة حائط عند حذفه.** كان يُشتقّ بـ`generated_at_utc(payload=…)`
        # من آخر التزام مسّ الحمولة، وهو حتميّ تماماً عند رأسٍ ثابت (مُقاس على
        # `fc79fbbe`: بناءان ⇒ نفس القيمة بالثانية).
        #
        # العيب أضيق وبنيويّ: **البيان يُولَّد قبل الالتزام الذي يحمله.** فبعد
        # التزامه يصير ذلك الالتزامُ «آخر تعديل للحمولة»، وإعادةُ التوليد تكتب زمنه
        # الجديد ⇒ المصنوعة **لا تستطيع مطابقة إعادة توليدها على التزامها نفسه** إلّا
        # بدورة ذاتيّة لا تنتهي (قِيس سابقاً: ثلاث دورات، وفرقُ ثانية في كلّ مرّة).
        #
        # والحذف — لا الاستثناء — هو العلاج، لسببين مقيسين: الحقل **بلا مستهلِك واحد**
        # في المستودع كلّه، و**لا يمثّل وقت البناء الحقيقيّ أصلاً** بل زمن التزامٍ سابق.
        # وقتُ البناء الفعليّ، عند الحاجة، يذهب إلى `attested_at` في إثباتٍ **خارج**
        # المصنوعة المُلتزَمة. أمّا إعفاء البيان من فحص الانحراف فيُبقي العيب ويُطفئ
        # الكاشف.
        #
        # `deterministic_time.py` يبقى: يستهلكه `runtime_environment_preflight.py`
        # أيضاً، ولا يُحذف عقدٌ لأنّ أحد مستهلكيه استغنى عنه.
        #
        # `source_root: root.name` حُذِف سابقاً: **صفر قارئ** في المستودع كلّه (المُنتِج
        # وحده)، وكان يطبع **اسم مجلّد السحب** فيختم الآلة في مصنوعة إصدار. قِيس
        # فعليّاً: التوليد داخل شجرة عمل اسمها `wt768` كتب `"source_root": "wt768"`.
        # نفس صنف `RUNTIME-ENV-PREFLIGHT-STAMPS-THE-MACHINE-01`. حقلٌ بلا قارئ يسرّب
        # البيئة ليس provenance بل ضجيج يتعارض.
