...
class Client:
    ...
    def call(
        ...
        req = request.Request(
            self.url + path, data=body, method=method, headers={**self.headers, **(headers or {})}
        )
        try:
...
def backup(client: Client, destination: Path) -> dict:
    ...
    for name in names:
        ...
        _, content = client.call(
            "GET", f"/collections/{safe_name}/snapshots/{parse.quote(snapshot_name, safe='')}"
        )
    ...
    manifest = {
        ...
        "qdrant_url_redacted": parse.urlsplit(client.url).hostname,
        "snapshots": rows,
    }
...
def restore_drill(client: Client, directory: Path, evidence_path: Path) -> dict:
    ...
    for row in manifest["snapshots"]:
        ...
        try:
            client.call(
                "POST",
                f"/collections/{target_path}/snapshots/upload?priority=snapshot",
                body,
    ...
    evidence = {
        ...
        "drill_mode": "temporary_collection_non_destructive",
        "collections": evidence_rows,
    }
...
def main() -> int:
    ...
    try:
        if args.command == "verify":
            ...
            result = (
                backup(client, args.directory)
                if args.command == "backup"
                else restore_drill(client, args.directory, args.evidence)
            )
