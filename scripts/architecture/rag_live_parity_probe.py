...
def run_probe(*, tenant_id: str, queries: list[str], final_k: int, qdrant_url: str,
              collection: str, ollama_url: str, model: str, retrieval_url: str,
              subject_sha: str, contract_sha256: str, qdrant_api_key: str | None = None) -> dict:
    if not queries:
    ...
    for query in queries:
        emb = _post(f"{ollama_url.rstrip('/')}/api/embeddings", {"model": model, "prompt": query}).get("embedding")
        ...
        direct = _post(
            ...
            {"vector": emb, "limit": final_k, "with_payload": True,
        ...
        d={_fp(str((r.get("payload") or {}).get("page_content") or "")) for r in direct if (r.get("payload") or {}).get("page_content")}
        c={_fp(str(r.get("text") or "")) for r in canonical if r.get("text")}
        union=d|c
        overlap=(len(d&c)/len(union)) if union else 1.0
        rows.append({"query_sha256": _fp(query), "direct_hits": len(d), "canonical_hits": len(c), "jaccard": round(overlap,6)})
    return {
...
def main()->int:
    ...
    contract_path=Path(__file__).resolve().parents[2]/'docs/architecture/rag_embedding_contract.json'
    contract_sha=hashlib.sha256(contract_path.read_bytes()).hexdigest()
    receipt=run_probe(
        tenant_id=a.tenant_id, queries=a.query, final_k=a.final_k,
        ...
        subject_sha=a.subject_sha.lower(), contract_sha256=contract_sha,
        qdrant_api_key=os.getenv('QDRANT_API_KEY') or None,
    )
    Path(a.out).write_text(json.dumps(receipt,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(f"rag_live_parity_receipt_written queries={receipt['query_count']} min={receipt['min_jaccard']} mean={receipt['mean_jaccard']}")
    return 0
