...
def findings(receipt: dict, subject_sha: str) -> list[str]:
    ...
    p = s.get("parity_acceptance") or {}
    out: list[str] = []
    if receipt.get("schema") != p.get("receipt_schema"):
        out.append("receipt schema mismatch")
    if receipt.get("subject_sha") != subject_sha:
        out.append("receipt subject SHA mismatch")
    if (
        receipt.get("embedding_contract_sha256")
        != hashlib.sha256(CONTRACT.read_bytes()).hexdigest()
    ):
        out.append("embedding contract digest mismatch")
    if (
        receipt.get("embedding_provider") != "ollama"
        or receipt.get("embedding_model") != "nomic-embed-text"
    ):
        out.append("embedding identity mismatch")
    if receipt.get("collection") != "sahool_agri_kb":
    ...
    if float(receipt.get("mean_jaccard", 0)) < float(p.get("min_mean_jaccard", 0.8)):
        out.append("mean jaccard below threshold")
    try:
        datetime.fromisoformat(str(receipt.get("observed_at")).replace("Z", "+00:00"))
    except Exception:
        out.append("invalid observed_at")
    if not isinstance(receipt.get("vector_size"), int) or receipt.get("vector_size", 0) <= 0:
        out.append("invalid live vector size")
    return out
...
def main() -> int:
    ...
    a = ap.parse_args()
    if len(a.subject_sha) != 40:
        print("rag_live_parity_receipt_fail invalid subject sha")
        return 1
    r = json.loads(Path(a.receipt).read_text(encoding="utf-8"))
    f = findings(r, a.subject_sha.lower())
    if f:
        for x in f:
