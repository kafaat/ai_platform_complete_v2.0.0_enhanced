...
#
# Exit 0 = all acceptance checks pass; non-zero = a gate failed (details on stderr).
set -euo pipefail
...
( cd "$ROOT/services/field-management-service" && python3 -m pytest tests/ -m integration -q ) \
  || fail "live PostgreSQL isolation test failed"
pass "cross-tenant read 404 + connection-reuse isolation (RLS)"
