...
def add(checks: list[Check], name: str, status: str, message: str, detail: dict[str, object] | None = None) -> None:
...
def check_required_files(root: Path, checks: list[Check]) -> None:
    ...
    add(checks, "required-files", "fail" if missing else "pass", "required bootstrap assets present" if not missing else "missing bootstrap assets", {"missing": missing})


def check_env(root: Path, checks: list[Check]) -> None:
    env = merged_env(root)
    missing = [k for k in REQUIRED_ENV if not env.get(k)]
    defaults = [k for k, v in env.items() if k in REQUIRED_ENV and any(p.search(v) for p in DEFAULT_SECRET_PATTERNS)]
...
def check_migrations(root: Path, checks: list[Check]) -> None:
    ...
    add(checks, "migrations", "fail" if missing else "pass", "runtime migrations registered" if not missing else "missing runtime migrations", {"missing": missing})
...
def check_compose_static(root: Path, checks: list[Check]) -> None:
    ...
    add(checks, "compose-static", status, "compose service/security contract satisfied" if status == "pass" else "compose static contract failed", {"missing_services": missing_services, "forbidden": forbidden})
...
def check_compose_config(root: Path, checks: list[Check]) -> None:
    if not command_available("docker"):
        ...
        return
    code, output = run_command(["docker", "compose", "-f", "docker-compose.v9.yml", "config", "--quiet"], root, timeout=45)
    add(checks, "docker-compose-config", "pass" if code == 0 else "fail", "docker compose config passed" if code == 0 else "docker compose config failed", {"exit_code": code, "output": output})


def check_ports(checks: list[Check], host: str = "127.0.0.1", ports: Iterable[int] = (80, 443, 8000, 8080, 8090, 8092, 5432, 6379, 4222, 9000)) -> None:
    occupied: list[int] = []
    for port in ports:
    ...
    add(checks, "local-port-scan", "warn" if occupied else "pass", "ports currently occupied; verify they belong to SAHOOL" if occupied else "default ports are available", {"occupied": occupied})
...
def check_http_runtime(checks: list[Check], base_url: str) -> None:
    ...
    reachable = [p for p, r in health.items() if isinstance(r["status"], int) and int(r["status"]) < 500]
    add(checks, "runtime-health-http", "pass" if reachable else "skip", "runtime health endpoints reachable" if reachable else "runtime not reachable or not started", {"reachable": reachable, "results": health})

    metrics = {}
    for path in METRICS_PATHS:
        status, body = http_get(base_url.rstrip("/") + path)
        metrics[path] = {"status": status, "sample": body[:160]}
    metrics_ok = [p for p, r in metrics.items() if isinstance(r["status"], int) and int(r["status"]) < 500]
    add(checks, "runtime-metrics-http", "pass" if metrics_ok else "skip", "metrics endpoints reachable" if metrics_ok else "metrics not reachable or runtime not started", {"reachable": metrics_ok, "results": metrics})


def check_static_gates(root: Path, checks: list[Check]) -> None:
    gates = [
        ("production-gate", ["bash", "scripts/production_validation_gate.sh"]),
        ("observability-gate", [sys.executable, "scripts/observability/validate_observability_assets.py", "--root", "."]),
        ("helm-gate", [sys.executable, "scripts/deploy/validate_helm_readiness.py", "--env", "production"]),
        ("release-gate", [sys.executable, "scripts/release/validate_release_package.py", "--root", "."]),
        ("ci-gate", [sys.executable, "scripts/ci/validate_ci_gates.py", "--root", "."]),
        ("migration-manifest-gate", [sys.executable, "scripts/migrations/validate_migration_manifest.py", "--root", "."]),
    ]
    for name, cmd in gates:
        ...
        add(checks, name, "pass" if code == 0 else "fail", f"{name} passed" if code == 0 else f"{name} failed", {"exit_code": code, "output": output})
...
def summarize(checks: list[Check]) -> dict[str, object]:
    ...
    readiness = "blocked" if counts.get("fail", 0) else ("attention" if counts.get("warn", 0) else "ready")
...
def render_text(summary: dict[str, object], checks: list[Check]) -> str:
    lines = ["SAHOOL Runtime Bootstrap Doctor", f"readiness: {summary['readiness']}", f"counts: {summary['counts']}", ""]
...
def main() -> int:
    ...
    output = json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n" if args.format == "json" else render_text(summary, checks)
    if args.output:
