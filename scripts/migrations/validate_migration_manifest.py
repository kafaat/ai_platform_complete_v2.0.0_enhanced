...
def main() -> int:
    ...
    sql_files = sorted(
        p.name for p in (root / "migrations").glob("*.sql") if not p.name.endswith(".down.sql")
    )
    ...
    duplicate_prefixes = {
        k: v for k, v in prefixes.items() if len(v) > 1 and k not in IGNORED_DUPLICATE_PREFIXES
    }
    ...
    legacy_sql_text = (
        legacy_sql.read_text(encoding="utf-8", errors="replace") if legacy_sql.exists() else ""
    )
    legacy_py_text = (
        legacy_py.read_text(encoding="utf-8", errors="replace") if legacy_py.exists() else ""
    )
    if "MANIFEST.txt" not in legacy_sql_text:
        errors.append(
            "scripts_v9/run_migrations.sql does not declare MANIFEST.txt as source of truth"
        )
    ...
    missing_from_legacy_sql = [
        entry for entry in entries if f"migrations/{entry}" not in legacy_sql_text
    ]
    if missing_from_legacy_sql:
        errors.append(
            "scripts_v9/run_migrations.sql is missing manifest entries: "
            + ", ".join(missing_from_legacy_sql[:20])
        )
