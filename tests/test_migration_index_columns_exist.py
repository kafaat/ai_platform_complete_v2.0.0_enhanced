...
البيان مرّتين على PostgreSQL حيّ، وهو ما تفعله الآن خطوةُ إعادة التطبيق الثانية في وظيفة
*Integration Tests* (`MIGRATION-REAPPLY-IDEMPOTENCY-UNMEASURED-01`، بإذن المالك). هذا
الحارسُ يقيس البديلَ الساكن الذي يُحمِّر في ثوانٍ، قبل أن يصل العطلُ إلى أيّ قاعدة، وعلى
صنفٍ واحد (أعمدةُ الفهارس)؛ والخطوةُ الحيّة تقيس كلَّ DDL يفترض شكلَ ما قبله — كلاهما يلزم.
"""
