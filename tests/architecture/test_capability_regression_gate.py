...
s = importlib.util.spec_from_file_location("g", ROOT / "scripts/ci/capability_regression_gate.py")
...
def test_blocks_maturity_and_evidence_regression():
    kinds = {x["kind"] for x in g.compare(doc(), doc(maturity=2, evidence_level=2))}
    assert {"maturity_regression", "evidence_level_regression"} <= kinds


def test_blocks_removal_and_dependency_edge_removal():
    assert g.compare(doc(), {"capabilities": []})[0]["kind"] == "removed"
    assert any(
        x["kind"] == "dependency_edges_removed" for x in g.compare(doc(), doc(dependencies=[]))
    )
