...
وموضعه في ``tests/ci/`` لا ``tests/architecture/`` **قرار لا صدفة**: #692 أضاف
``arch_test_ci_coverage_guard`` الذي يُلزِم كلّ ملفّ في ``tests/architecture/`` بأن
يُذكَر في workflow. لو وُضِع هنا لتناقض الثابتان مباشرةً — حارسٌ يطلب الإدراج واختبارٌ
يؤكّد عدمه. الحدّان صحيحان في نطاقيهما، والفصل يُبقيهما معاً بلا مهادنة أيّهما.
"""
