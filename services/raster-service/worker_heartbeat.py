"""Worker heartbeat — capability-aware liveness for raster-service workers (V21 §2.2 / CT-06).

خلفيّة (تدقيق الحاويات V21 §2.2): healthcheck عاملَي الراستر (إبطال الكاش · فحص backfill)
كان ينجح بمجرّد وجود ``DATABASE_URL``/``JOBS_DATABASE_URL`` — لا يُثبِت أنّ العمليّة حيّة
ولا أنّ حلقة الاستطلاع تتحرّك. هنا يكتب العامل نبضةً (heartbeat) كلّ دورة إلى ملفّ محلّيّ
(last_poll_at + عدّادات + الحالة)، ويقرأ الـhealthcheck حداثتها وحالتها بدل مجرّد وجود
متغيّر بيئة. ملفّ لا جدول: الفحص خفيف ولا يتطلّب وصول قاعدة بيانات.

نسخة قائمة بذاتها (stdlib فقط) لحاوية raster-service المنفصلة — تعذّر استيراد وحدة المنصّة
عبر الخدمات، فالنسخة الصغيرة المعزولة مقبولة وتُبقي الفحص خفيفاً وآمناً في بيئة CI الدنيا.
تطابق ``services/sahool-platform/api/worker_heartbeat.py`` سلوكاً.

آليّة صادقة: عامل مُتعطّل/متوقّف ⇒ النبضة تتقادم ⇒ الفحص يفشل (لا يُخفى التوقّف). خطأ في
المُشغّل يُسجَّل في النبضة (last_error_at + failed_total + state) — فتُرى الأعطال في الحالة.
"""
