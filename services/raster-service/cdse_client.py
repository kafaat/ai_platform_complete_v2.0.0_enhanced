...
INDEX_EXPR: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    # مؤشرات زراعية متقدمة شائعة في منصات المراقبة العالمية (Advanced/on-demand).
    "reci": (("B05", "B08"), "(s.B08 / s.B05) - 1.0"),
    "gci": (("B03", "B08"), "(s.B08 / s.B03) - 1.0"),
    "arvi": (
        ("B02", "B04", "B08"),
        "(s.B08 - (2.0 * s.B04 - s.B02)) / (s.B08 + (2.0 * s.B04 - s.B02))",
    ),
    "sipi": (("B02", "B04", "B08"), "(s.B08 - s.B02) / (s.B08 - s.B04)"),
    "nbr": (("B08", "B12"), "(s.B08 - s.B12) / (s.B08 + s.B12)"),
    "ccci": (
        ("B04", "B05", "B08"),
        "((s.B08 - s.B05) / (s.B08 + s.B05)) / ((s.B08 - s.B04) / (s.B08 + s.B04))",
    ),
    "vari": (("B02", "B03", "B04"), "(s.B03 - s.B04) / (s.B03 + s.B04 - s.B02)"),
    "gli": (("B02", "B03", "B04"), "(2.0 * s.B03 - s.B04 - s.B02) / (2.0 * s.B03 + s.B04 + s.B02)"),
    "bsi": (
        ("B02", "B04", "B08", "B12"),
        "((s.B12 + s.B04) - (s.B08 + s.B02)) / ((s.B12 + s.B04) + (s.B08 + s.B02))",
    ),
}
