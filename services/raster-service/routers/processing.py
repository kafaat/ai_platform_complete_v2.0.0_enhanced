...
نُقلت المُعالِجات حرفيّاً مع تغيير ``@app`` إلى ``@router``. منطق المعالجة
الخلفيّة (``_run_processing`` / ``_run_batch_processing``) والنماذج والحالة تبقى
في ``main`` وتُشار إليها عبر ``main.X``.
"""
