...
class TestDataIntegrity:
    ...
    def test_no_empty_texts(self):
        for e in ALJAWF_KNOWLEDGE:
            assert len(e["text"].strip()) >= 20, f"entry {e['id']}: نصّ قصير جدّاً"
...
class TestSampleNamingCorrection:
    ...
    def test_sa_naming_in_sunaydar_entries(self):
        ...
        sunaydar_entries = [
            e for e in ALJAWF_KNOWLEDGE if "السنيدار" in e["text"] or "السنيدار" in e["source"]
        ]
    ...
    def test_sa_range_documented(self):
        ...
        sa_mentions = [
            e for e in ALJAWF_KNOWLEDGE if "SA1..SA13" in e["text"] or "SA1..SA13" in e["source"]
        ]
        assert len(sa_mentions) >= 3, f"يجب توثيق SA1..SA13 في ≥3 entries، وُجد {len(sa_mentions)}"

    def test_water_sample_distinguishes_sa_from_sodium_class(self):
        # entry 110 (مياه آبار) يجب أن يميّز بين أسماء العيّنات و
        # تصنيف Sodium Hazard
        water_entry = next((e for e in ALJAWF_KNOWLEDGE if e["id"] == 110), None)
        ...
        assert (
            "SA1..SA13" in text or "أسماء العيّنات" in text or "USDA" in text or "تصنيف" in text
...
class TestTopicCoverage:
    ...
    def test_has_decision_rules(self):
        rules = [e for e in ALJAWF_KNOWLEDGE if e["topic"] == "decision_rule"]
...
class TestValidationHelper:
    ...
    def test_validate_passes_for_clean_data(self):
        ...
        assert result["valid"], f"البيانات الأساسية يجب أن تكون صالحة، وُجد: {result['errors'][:3]}"
...
class TestNoCoreLeakage:
    ...
    def test_lives_outside_core(self):
        ...
        assert (
            "core" not in str(file_path).split("/sahool-platform/")[-1].split("/")[0]
            if "/sahool-platform/" in str(file_path)
            else True
        )

    def test_no_core_imports(self):
        # CRITICAL: aljawf_knowledge.py لا يستورد من core/
        content = (Path(__file__).parent / "aljawf_knowledge.py").read_text(encoding="utf-8")
        forbidden = [
            "from core",
            "import core",
            "from sahool_core",
        ...
        for bad in forbidden:
            assert bad not in content, f"aljawf_knowledge يستورد من النواة: '{bad}'"
