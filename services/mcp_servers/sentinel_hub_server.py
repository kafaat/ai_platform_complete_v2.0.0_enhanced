...
async def fetch_sentinel_hub_token() -> str:
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await retry_request(client.post(
        ...
        "tools": [
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "bands": {"type": "array", "items": {"enum": ["B02","B03","B04","B05","B06","B07","B08","B8A","B11","B12"]}},
...
async def call_tool(request: Request):
    ...
    if tool_input.request_id:
        IDEMPOTENCY_CACHE[tool_input.request_id] = {"result": result, "expires_at": datetime.now(timezone.utc) + timedelta(seconds=CACHE_TTL_SECONDS)}
    return result
...
async def _execute_tool(tool_input: ToolInput) -> Dict[str, Any]:
    ...
    if name == "fetch_sentinel2_l2a":
        ...
        payload = {
            "input": {"bounds": {"bbox": list(bbox), "properties": {"crs": "http://www.opengis.net/def/crs/EPSG/0/4326"}},
                      "data": [{"type": "sentinel-2-l2a", "dataFilter": {"timeRange": {"from": req.date_range.split("/")[0]+"T00:00:00Z", "to": req.date_range.split("/")[1]+"T23:59:59Z"}, "maxCloudCoverage": req.cloud_cover_max}}]},
            "output": {"responses": [{"identifier": "default", "format": {"type": "image/tiff"}}]},
            "evalscript": evalscript
        }
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await retry_request(client.post(f"{SH_BASE_URL}/api/v1/process", json=payload, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}))
            resp.raise_for_status()
        return {"content": [{"type": "text", "text": json.dumps({"status":"success","source":"Sentinel-2 L2A","bands":req.bands,"bbox":bbox,"cloud_cover_max":req.cloud_cover_max,"data_size_bytes":len(resp.content),"timestamp":datetime.now(timezone.utc).isoformat()}, ensure_ascii=False)}]}
    elif name == "fetch_sentinel1_grd":
        req = Sentinel1Request(**args)
        bbox = await get_field_bbox(req.field_id)
        token = await fetch_sentinel_hub_token()
        payload = {
            "input": {"bounds": {"bbox": list(bbox)}, "data": [{"type": "sentinel-1-grd", "dataFilter": {"timeRange": {"from": req.date_range.split("/")[0]+"T00:00:00Z", "to": req.date_range.split("/")[1]+"T23:59:59Z"}, "polarization": req.polarization}}]},
            "output": {"responses": [{"identifier": "default", "format": {"type": "image/tiff"}}]}
        }
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await retry_request(client.post(f"{SH_BASE_URL}/api/v1/process", json=payload, headers={"Authorization": f"Bearer {token}"}))
            resp.raise_for_status()
        return {"content": [{"type": "text", "text": json.dumps({"status":"success","source":"Sentinel-1 GRD","polarization":req.polarization,"bbox":bbox,"data_size_bytes":len(resp.content),"timestamp":datetime.now(timezone.utc).isoformat()}, ensure_ascii=False)}]}
    elif name == "compute_ndvi":
        req = NDVIRequest(**args)
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await retry_request(client.post("http://sahool-vegetation:8000/v1/analyze", json={"field_id": req.field_id, "date": req.date}, timeout=60.0))
            resp.raise_for_status()
            ndvi_data = resp.json()
