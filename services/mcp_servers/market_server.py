...
logging.basicConfig(level=logging.INFO, format='{"time":"%(asctime)s","svc":"market-mcp","msg":"%(message)s"}')
...
async def tool_search_products(args: dict) -> dict:
    ...
    if query:
        ...
        conditions.append(f"(product_name ILIKE ${len(params)} OR product_name_ar ILIKE ${len(params)} OR sku ILIKE ${len(params)})")
    if category:
    ...
    async with pool.acquire() as conn:
        await conn.execute("SELECT set_config('app.current_tenant', $1, true)", str(tenant_id) if tenant_id else "")  # CRIT-10/11 FIX
...
async def tool_get_supplier(args: dict) -> dict:
    ...
    async with pool.acquire() as conn:
        await conn.execute("SELECT set_config('app.current_tenant', $1, true)", str(tenant_id) if tenant_id else "")  # CRIT-10/11 FIX
        if sid:
            row = await conn.fetchrow("SELECT * FROM market_suppliers WHERE supplier_id=$1::uuid", sid)
        else:
            row = await conn.fetchrow("SELECT * FROM market_suppliers WHERE name ILIKE $1", f"%{sname}%")
        if not row:
            return {"supplier": None, "products": []}
        products = await conn.fetch("SELECT product_id, product_name, product_name_ar, category, unit_price_usd, stock_qty, currency FROM market_products WHERE supplier_id=$1::uuid AND active=true", str(row["supplier_id"]))
    return {"supplier": record_to_dict(row), "products": [record_to_dict(p) for p in products], "product_count": len(products)}
        ...
        async with conn.transaction():
            order_id = await conn.fetchval(
                """INSERT INTO market_procurement_orders
                    ...
                    RETURNING order_id""",
                tenant_id, field_id, "draft", total_estimated, delivery_date, notes, auto_approve
            )
            for it in items:
                await conn.execute(
                    ...
                    order_id, it.get("product_id") if it.get("product_id") else None, it["product_name"],
                    float(it["quantity"]), it.get("unit", "kg"), float(it.get("max_unit_price_usd", 0)), it.get("notes", "")
                )
            new_status = "draft"
            if total_estimated <= auto_approve:
                new_status = "approved"
                await conn.execute("UPDATE market_procurement_orders SET status=$1, approved_at=NOW() WHERE order_id=$2", new_status, order_id)
            else:
                await conn.execute("UPDATE market_procurement_orders SET status=$1 WHERE order_id=$2", "pending_approval", order_id)
    return {
        "order_id": str(order_id), "status": new_status, "total_estimated_usd": round(total_estimated, 2),
        "item_count": len(items), "auto_approved": total_estimated <= auto_approve,
        "message": "Order created." + (" Auto-approved (below threshold)." if total_estimated <= auto_approve else " Awaiting approval.")
    }
...
async def tool_get_procurement_status(args: dict) -> dict:
    ...
    async with pool.acquire() as conn:
        await conn.execute("SELECT set_config('app.current_tenant', $1, true)", str(tenant_id) if tenant_id else "")  # CRIT-10/11 FIX
        row = await conn.fetchrow("SELECT * FROM market_procurement_orders WHERE order_id=$1::uuid", order_id)
        if not row:
            raise HTTPException(404, "Order not found")
        items = await conn.fetch("SELECT * FROM market_procurement_items WHERE order_id=$1::uuid", order_id)
...
async def tool_create_sales_listing(args: dict) -> dict:
    ...
    async with pool.acquire() as conn:
        ...
        listing_id = await conn.fetchval(
            ...
            tenant_id, batch_id if batch_id else None, crop, qty, price,
            grade, certs, from_date, until_date, pickup, notes, "active"
        )
    return {"listing_id": str(listing_id), "status": "active", "": round(qty*price, 2), "qr_trace_url": f"/trace/{batch_id}" if batch_id else None}
...
async def tool_price_history(args: dict) -> dict:
    ...
    return {"category": category, "days": days, "aggregation": agg, "history": [record_to_dict(r) for r in rows]}
    ...
    async with pool.acquire() as conn:
        ...
        proc = await conn.fetchrow(
            """SELECT COUNT(*) as total_orders, SUM(total_estimated_usd) as total_value,
                ...
                WHERE tenant_id=$1::uuid AND created_at >= CURRENT_DATE - INTERVAL '30 days'""", tenant_id)
        ...
        alerts = await conn.fetch(
            """SELECT product_name, category, unit_price_usd,
             ...
             LIMIT 10""", tenant_id)
    return {
        "tenant_id": tenant_id, "period": "last_30_days",
...
async def tool_get_market_price(args: dict) -> dict:
    ...
    return {"price_yer_kg": round(price * 250, 2), "price_usd_kg": round(price, 4), "trend": "stable", "updated": datetime.now(timezone.utc).isoformat()}
...
async def tool_get_price_trend(args: dict) -> dict:
    ...
    return {"current_price": round(current, 4), "price_change_pct": round(change, 2), "forecast": "stable" if abs(change) < 5 else ("up" if change > 0 else "down"), "trend_data": [{"date": str(r["recorded_date"]), "price": float(r["price_usd"])} for r in rows[:7]]}
...
async def mcp_tools_list():
    return {"tools": [
        {"name": "market_search_products", "description": "Search agricultural products catalog"},
...
@app.get("/products")
async def api_search_products(q: Optional[str] = None, category: Optional[str] = None, limit: int = 20, user: dict = Depends(_get_current_user)):
...
@app.get("/sales")
async def api_search_sales(crop: Optional[str] = None, grade: Optional[str] = None, limit: int = 20, user: dict = Depends(_get_current_user)):
