#!/usr/bin/env python3
"""حارس تفكيك supervisor-agent — نظير حارس المنصّة/raster.

يفرض ضمانات تُثبّت تفكيك ``main.py`` إلى ``routers/`` ضدّ أيّ انحدار لاحق:
  ...
  5) المسارات الحرجة حاضرة + أرضيّة عدد المسارات (≥14) — التفكيك لا يُسقط مساراً.
...
# الجذر يوفّر حزمة shared (from shared.helpers import ...) اللازمة لاستيراد main.
...
except Exception:  # noqa: BLE001 — تبعيّات supervisor-agent الدنيا غير متوفّرة
    pytest.skip("supervisor-agent main import unavailable", allow_module_level=True)
...
# مسارات تمثيليّة حقيقيّة من الخدمة — فقدانها انحدار حرج. وعدد المسارات لا يهبط
...
def test_critical_routes_present():
    """المسارات التمثيليّة الحرجة كلّها حاضرة في app.routes (لا تختفي بعد التفكيك)."""
    paths = _app_paths()
