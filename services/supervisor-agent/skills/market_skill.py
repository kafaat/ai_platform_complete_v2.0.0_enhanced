    ...
    async def execute(
        ...
        if intent == "price_current":
            ...
            result = await self.mcp.call_tool(
                self.server, "get_market_price",
                {"crop": crop, "market": market}
            )
            content = result.get("content", [{}])[0].get("text", "{}")
            price_data = json.loads(content)
            return {
                "type": "price_current", "crop": crop, "market": market,
            ...
            crop = context.get("crop", "wheat") if context else "wheat"
            result = await self.mcp.call_tool(
                self.server, "get_price_trend",
                {"crop": crop}
            )
            content = result.get("content", [{}])[0].get("text", "{}")
            trend_data = json.loads(content)
            return {
                "type": "price_forecast", "crop": crop,
            ...
            result = await self.mcp.call_tool(
                self.server, "create_forward_contract",
                {
                    "farmer_id": user_id, "field_id": field_id, "crop": crop,
                    "estimated_yield_kg": yield_est, "harvest_date": harvest, "quality_grade": "A"
                }
            )
            content = result.get("content", [{}])[0].get("text", "{}")
