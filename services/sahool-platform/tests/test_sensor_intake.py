...
from core.sensor_intake import ingest_batch, ingest_reading


class TestValidation:
    def test_valid_reading_accepted(self):
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="soil_moisture", value=35.5)
        assert r.accepted
    ...
    def test_none_value_rejected_explicitly(self):
        ...
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="soil_moisture", value=None)
    ...
    def test_out_of_range_rejected(self):
        ...
        r = ingest_reading(
            tenant_id="t1", field_id="f1", sensor_type="soil_temperature", value=500.0
        )
        assert not r.accepted

    def test_negative_humidity_rejected(self):
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="air_humidity", value=-10)
        assert not r.accepted

    def test_unknown_sensor_type_rejected(self):
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="alien_sensor", value=5)
        assert not r.accepted

    def test_sensor_ceiling_is_medium_not_high(self):
        # CRITICAL: الحسّاس قرينة قويّة، لا دليل مخبري — سقف medium
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="soil_ec", value=2.5)
        assert r.observation["confidence"] == "medium"
        # ليس high (المختبر فقط high)

    def test_missing_timestamp_warns(self):
        # تحذير لا رفض — يُستخدم وقت الاستقبال
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="soil_moisture", value=40)
        assert r.accepted
...
class TestObservationOutput:
    def test_maps_to_correct_observable_id(self):
        r = ingest_reading(tenant_id="t1", field_id="f1", sensor_type="soil_ec", value=3.0)
        assert r.observation["observable_id"] == "soil_ec_ds_m"

    def test_geo_tag_preserved(self):
        r = ingest_reading(
            tenant_id="t1", field_id="f1", sensor_type="soil_moisture", value=40, lon=44.5, lat=16.2
        )
