...
def test_learning_update_cutover_has_exactly_one_authoritative_branch():
    p = ROOT / "services/sahool-platform/api/phase_runtime_store.py"
    t = p.read_text()
    s = t.index("async def persist_phase10_learning_outputs")
    e = t.index('        scenario = outputs.get("scenario_result")', s)
    b = t[s:e]
    cs = b.index("if mode.strict_decision_service_required:")
    elsepos = b.index("            else:", cs)
    cut = b[cs:elsepos]
    assert "conn.execute" not in cut
