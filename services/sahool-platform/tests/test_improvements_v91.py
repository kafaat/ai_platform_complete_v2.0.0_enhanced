...
class TestImprovementsV91:
    def test_source_check_rejects_invalid(self):
        ...
        with pytest.raises(Exception):
            lite_store.add_observation("t1","al_jawf","S3","2026-01-01",
                value=2.5, source="banana", db_path=db)
        os.unlink(db)
    ...
    def test_source_check_accepts_valid(self):
        ...
        for src in ("sensor","lab","manual","model","imported"):
            lite_store.add_observation("t1","al_jawf","S3","2026-01-01",
                value=2.5, source=src, db_path=db)
    ...
    def test_observable_validation_rejects_unknown(self):
        ...
        with pytest.raises(ValueError):
            lite_store.add_observation("t1","al_jawf","S99","2026-01-01",
                value=1, source="lab", db_path=db)
        os.unlink(db)

    def test_observable_validation_can_be_disabled(self):
        db = _db()
        lite_store.add_observation("t1","al_jawf","CUSTOM","2026-01-01",
            value=1, source="lab", validate=False, db_path=db)
    ...
    def test_get_observations_filters(self):
        db = _db()
        lite_store.add_observation("t1","al_jawf","S3","2026-01-01", value=2.5, source="lab", db_path=db)
        lite_store.add_observation("t1","al_jawf","S4","2026-01-02", value=7.8, source="lab", db_path=db)
