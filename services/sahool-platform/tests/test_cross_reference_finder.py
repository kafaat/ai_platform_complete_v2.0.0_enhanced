...
from core.cross_reference_finder import (
    SearchContext, find_similar_recommendations, find_similar_activities,
    find_similar_calibrations, cross_reference_summary, SimilarityMatch)
from core.learning.recommendation_log import RecommendationRecord
from core.activity_log import (
    plan_activity_from_recommendation, mark_completed, ActivityStatus)


def _make_rec(rec_id, tenant_id, crop, days_ago=30,
              indicators=None, outcome_yield=None):
    ...
    return RecommendationRecord(
        rec_id=rec_id, tenant_id=tenant_id, district_id="d1",
        zone_id="z1", crop=crop, issued_date=date,
        recommendation_ar="test", quality_grade="READY",
...
class TestTenantIsolation:
    ...
    def test_other_tenant_data_not_leaked(self):
        ...
        log = [
            _make_rec("r_mine", "tnt_001", "wheat",
                     indicators={"ndvi": 0.6}),
            _make_rec("r_other", "tnt_999", "wheat",
                     indicators={"ndvi": 0.6}),
        ]
        ctx = SearchContext(tenant_id="tnt_001", field_id="f1", crop="wheat",
                           current_indicators={"ndvi": 0.6})
    ...
    def test_calibration_tenant_isolation(self):
        cal_log = [
            {"tenant_id": "tnt_001", "crop_id": "wheat", "zone_factor": 0.9,
             "calibration_id": "c1", "date": "2025-06-01"},
            {"tenant_id": "tnt_999", "crop_id": "wheat", "zone_factor": 0.5,
             "calibration_id": "c_leak", "date": "2025-06-01"},
        ]
        ctx = SearchContext(tenant_id="tnt_001", field_id="f1", crop="wheat")
...
class TestSimilarityScoring:
    ...
    def test_different_crop_lower_score(self):
        ...
        if results:
            assert not any("wheat" in r and "نفس" in r
                          for r in results[0].why_similar_ar)

    def test_similar_indicators_add_to_score(self):
        log = [_make_rec("r1", "tnt_001", "wheat",
                        indicators={"ndvi": 0.58, "soil_moisture": 22})]
        ctx_close = SearchContext(tenant_id="tnt_001", field_id="f1", crop="wheat",
                                 current_indicators={"ndvi": 0.55, "soil_moisture": 21})
        ctx_far = SearchContext(tenant_id="tnt_001", field_id="f1", crop="wheat",
...
class TestSummary:
    def test_summary_counts_by_type(self):
        matches = [
            SimilarityMatch(source_type="recommendation", source_id="r1",
                          source_date="2026-01-01", tenant_id="t1", field_id="f1",
                          crop="wheat", similarity_score=0.8, why_similar_ar=["a"]),
            SimilarityMatch(source_type="activity", source_id="a1",
                          source_date="2026-01-01", tenant_id="t1", field_id="f1",
                          crop=None, similarity_score=0.5, why_similar_ar=["b"]),
        ]
    ...
    def test_summary_limits_to_top_5(self):
        ...
        matches = [
            SimilarityMatch(source_type="recommendation", source_id=f"r{i}",
                          source_date="2026-01-01", tenant_id="t1", field_id="f1",
                          crop="wheat", similarity_score=0.9-i*0.01,
                          why_similar_ar=["x"])
            for i in range(10)
        ]
        summary = cross_reference_summary(matches)
