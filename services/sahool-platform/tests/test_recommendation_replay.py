...
from core.recommendation_replay import (
    explain_recommendation, detect_drift, audit_chain)
...
def _make_rec_with_provenance(rec_id="r1", versions=None, snapshot=None):
    ...
    return RecommendationRecord(
        rec_id=rec_id, tenant_id="t1", district_id="d1",
        zone_id="z1", crop="wheat", issued_date="2026-05-28",
        recommendation_ar="اروِ 20مم", quality_grade="READY",
...
def _make_rec_without_provenance():
    ...
    return RecommendationRecord(
        rec_id="r_old", tenant_id="t1", district_id="d1",
        zone_id="z1", crop="wheat", issued_date="2025-03-01",
        recommendation_ar="توصية قديمة", quality_grade="LIMITED",
        predicted_yield_t_ha=None, confidence="low")
...
class TestExplainRecommendation:
    ...
    def test_explanation_includes_input_snapshot(self):
        rec = _make_rec_with_provenance(
            snapshot={"ndvi": 0.55, "soil_moisture": 22.0})
        result = explain_recommendation(rec)
...
class TestDriftDetection:
    def test_same_versions_no_drift(self):
        ...
        rec = _make_rec_with_provenance(
            versions={"fao56": "v2.1", "wofost": "7.2"})
    ...
    def test_multiple_drifts_all_reported(self):
        ...
        report = detect_drift(rec, {"fao56": "v2.3", "wofost": "7.3",
                                    "fuzzy": "v1.0"})
        assert report.drift_detected
...
class TestAuditChain:
    ...
    def test_audit_counts_drift(self):
        ...
        recs = [_make_rec_with_provenance("r1"),
                _make_rec_with_provenance("r2")]
