...
def _user(role=UserRole.AGRONOMIST, tenant_id="tnt_001"):
    return UserSchema(user_id="u1", tenant_id=tenant_id, role=role, name_ar="مهندس")
...
def _history(count=1, tenant_id="tnt_001"):
    ...
    return [
        RecommendationRecord(
            ...
            crop="wheat",
            issued_date=recent,
            recommendation_ar=f"توصية {i}",
            quality_grade="READY",
            predicted_yield_t_ha=3.5,
            confidence="medium",
            provenance={
                "input_snapshot": {"ndvi": 0.55},
                "engines_used": ["fao56"],
...
class TestEnrichment:
    ...
    def test_finds_similar_when_history_matches(self):
        enriched = enrich_with_context(
            ...
            field_id="fld_03",
            crop="wheat",
            recommendation_history=_history(3),
...
class TestAuthorizeAndDeliver:
    def test_authorized_user_delivered(self):
        ...
        enriched = {"rec_id": "r1", "base": {}, "cross_reference": {}, "provenance": {}}
        delivery = authorize_and_deliver(u, enriched, tenant_id="tnt_001", farm_id="frm_01")
        assert delivery.delivered

    def test_worker_blocked_from_request(self):
        # CRITICAL: WORKER لا يملك RECOMMENDATION_REQUEST
        u = _user(UserRole.WORKER)
        delivery = authorize_and_deliver(u, {"rec_id": "r1"}, tenant_id="tnt_001")
    ...
    def test_cross_tenant_blocked(self):
        ...
        delivery = authorize_and_deliver(u, {"rec_id": "r1"}, tenant_id="tnt_999")  # tenant مختلف
        assert not delivery.delivered
    ...
    def test_safety_critical_logged(self):
        ...
        u = _user(UserRole.AGRONOMIST)
        delivery = authorize_and_deliver(
            u, {"rec_id": "r1"}, tenant_id="tnt_001", is_pesticide=True
        )
        assert delivery.delivered
...
class TestFullPipeline:
    ...
    def test_full_pipeline_respects_inactive(self):
        ...
        u = UserSchema(
            user_id="u1", tenant_id="tnt_001", role=UserRole.OWNER, name_ar="x", is_active=False
        )
        delivery = full_delivery_pipeline(
            user=u,
            tenant_id="tnt_001",
