...
from core.evidence_class import (
    classify_evidence, enforce_indication_ceiling, EvidenceClass)
...
class TestCorroboration:
    ...
    def test_same_source_limited_elevation(self):
        ...
        from core.evidence_class import corroborate_indications
        r = corroborate_indications("ملوحة",
            [("SI", True), ("NDVI", True), ("BSI", True)])  # all optical
    ...
    def test_independent_sources_elevate_more(self):
        from core.evidence_class import corroborate_indications
        r = corroborate_indications("ملوحة",
            [("SI", True), ("RVI", True), ("district_context", True)])  # 3 sources
    ...
    def test_corroboration_never_lifts_blocked_even_nonstrict(self):
        ...
        from core.evidence_class import corroborate_indications
        r = corroborate_indications("إجهاد",
            [("NDVI", True), ("RVI", True), ("CWSI", True)], is_strict_governor=False)
        assert r.lifts_blocked is False
...
class TestConflictHandling:
    def test_minority_conflict_downgrades(self):
        ...
        from core.evidence_class import corroborate_indications
        all_agree = corroborate_indications("ملوحة",
            [("SI", True), ("RVI", True), ("district_context", True)])
        with_conflict = corroborate_indications("ملوحة",
            [("SI", True), ("RVI", True), ("NDVI", False)])
