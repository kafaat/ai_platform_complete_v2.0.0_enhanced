...
def test_legacy_dispatch_writer_is_retired_before_any_platform_db_access_after_cutover():
    t=P.read_text(); s=t.index('@router.post("/api/v1/decision/dispatch/execute")'); e=t.index('@router.post("/api/v1/decision/dispatch/consume")',s); b=t[s:e]
    ...
    assert 'execution-plan' in cut and 'dispatch-authorization' in cut and 'execution-request' in cut
