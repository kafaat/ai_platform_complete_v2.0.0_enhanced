...
from core.recommendation_engine import (
    generate_recommendation, RecommendationStatus, FarmerSignal)


class TestMaestro:
    def test_blocked_gives_no_recommendation(self):
        v = dict(quality_grade="BLOCKED", blocking_observables=["S3", "S4"],
                 missing_A=["S3", "S4"], missing_B_count=0)
    ...
    def test_farmer_view_hides_backend_detail(self):
        ...
        class Irr:
            et0_mm=10.0; etc_mm=8.0; m3_per_ha=350.0
        v = dict(quality_grade="MEDIUM", missing_A=[], missing_B_count=2,
    ...
    def test_confidence_lowered_without_calibration(self):
        v = dict(quality_grade="HIGH", missing_A=[], missing_B_count=0,
                 blocking_observables=[])
    ...
    def test_unsuitable_crop_triggers_danger(self):
        class Suit:
            suitability_class="N"
        v = dict(quality_grade="MEDIUM", missing_A=[], missing_B_count=1,
                 blocking_observables=[])
