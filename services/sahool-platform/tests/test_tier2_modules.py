...
from core.multi_season_analytics import (
    analyze_yield_trend, analyze_salinity_trend, detect_rotation_pattern,
    multi_season_summary, TrendDirection)
from core.transfer_learning import (
    DistrictProfile, suggest_transfer, transfer_summary, TransferConfidence)
from core.vrt_manual_maps import (
    build_zone_treatment, build_execution_map, map_summary_for_print,
    TreatmentType, TreatmentColor)
...
class TestRotationPattern:
    def test_monoculture_detected(self):
        ...
        r = detect_rotation_pattern("fld_03", [
...
class TestTransferLearning:
    def _source(self, **kw):
        defaults = dict(
            district_id="src", tenant_id="tnt_001",
            governorate_id="al_bayda", crop_seasons_count=4,
            avg_yield_t_ha=3.5, typical_salinity_ds_m=1.3,
            dominant_soil_type="clay_loam",
            common_crops=["wheat"], avg_zone_factor=0.92,
        )
        defaults.update(kw)
    ...
    def _target(self, **kw):
        defaults = dict(
            district_id="tgt", tenant_id="tnt_001",
    ...
    def test_similar_districts_high_confidence(self):
        ...
        assert sugg.confidence in (TransferConfidence.HIGH,
    ...
    def test_low_similarity_blocked(self):
        ...
        src = self._source(governorate_id="other_gov",
                          dominant_soil_type="sandy",
                          typical_salinity_ds_m=5.0)
        sugg = suggest_transfer(self._target(), [src], crop="wheat")
        # قد يكون LOW أو NONE حسب السياق
        assert sugg.confidence in (TransferConfidence.NONE,
...
class TestVRTManualMaps:
    def test_simple_zone_treatment(self):
        z = build_zone_treatment(
            "z_1", TreatmentType.NITROGEN,
            rate_per_ha=80, rate_unit="kg/ha", area_ha=2.5,
    ...
    def test_phi_blocked_strips_rate(self):
        ...
        z = build_zone_treatment(
            "z_p", TreatmentType.PESTICIDE,
            rate_per_ha=1.5, rate_unit="L/ha", area_ha=2.0,
            phi_status="blocked", days_to_safe=7)
    ...
    def test_nitrogen_no_phi_check(self):
        ...
        z = build_zone_treatment(
            "z_n", TreatmentType.NITROGEN,
            rate_per_ha=60, rate_unit="kg/ha", area_ha=2.0)
        assert z.rate_per_ha == 60

    def test_color_coding_progression(self):
        # rate low/mid/high → green/yellow/red
        low = build_zone_treatment("z", TreatmentType.NITROGEN,
                                   rate_per_ha=45, rate_unit="kg/ha",
                                   area_ha=1.0,
                                   rate_range_for_color=(40, 100))
        high = build_zone_treatment("z", TreatmentType.NITROGEN,
                                    rate_per_ha=95, rate_unit="kg/ha",
                                    area_ha=1.0,
                                    rate_range_for_color=(40, 100))
    ...
    def test_missing_data_returns_gray(self):
        ...
        z = build_zone_treatment(
            "z", TreatmentType.NITROGEN,
            rate_per_ha=50, rate_unit="kg/ha", area_ha=1.0)
        assert z.color == TreatmentColor.GRAY

    def test_full_execution_map(self):
        zones = [
            build_zone_treatment(f"z_{i}", TreatmentType.NITROGEN,
                               rate_per_ha=50+i*10, rate_unit="kg/ha",
                               area_ha=2.0,
                               rate_range_for_color=(40, 100))
            for i in range(3)
        ]
        emap = build_execution_map(
            "fld_03", "حقل القمح", TreatmentType.NITROGEN, zones)
    ...
    def test_pesticide_map_includes_safety_warnings(self):
        ...
        zones = [build_zone_treatment(
            "z", TreatmentType.PESTICIDE,
            rate_per_ha=1.0, rate_unit="L/ha", area_ha=1.0,
            phi_status="safe")]
        emap = build_execution_map(
            "fld", "x", TreatmentType.PESTICIDE, zones)
        # تحذيرات يجب أن تكون موجودة
        assert any("الأطفال" in w or "الرياح" in w
                  for w in emap.safety_warnings_ar)
