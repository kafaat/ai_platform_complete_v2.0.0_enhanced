...
from core.cross_reference_finder import (
    SearchContext, find_similar_recommendations, SimilarityMatch)
from core.recommendation_bridge import (
    safe_delivery, validate_pipeline, enforce_pipeline,
    ContextPipelineError, EnrichedRecommendation,
...
def _rec(rec_id, tenant_id="t1", district="al_bayda", days_ago=20,
         crop="wheat"):
    date = (datetime.now() - timedelta(days=days_ago)).date().isoformat()
    return RecommendationRecord(
        rec_id=rec_id, tenant_id=tenant_id, district_id=district,
        zone_id="z1", crop=crop, issued_date=date,
        recommendation_ar="x", quality_grade="READY",
...
class TestSilentDistrictBugFix:
    ...
    def test_same_district_increases_score_explicitly(self):
        ...
        ctx_with = SearchContext(tenant_id="t1", field_id="f1", crop="wheat",
                                district_id="al_bayda")
    ...
    def test_different_district_no_bonus(self):
        ...
        ctx = SearchContext(tenant_id="t1", field_id="f1", crop="wheat",
                           district_id="dhamar")   # مديرية مختلفة
        log = [_rec("r1", district="al_bayda")]
...
class TestOutcomeQualityBridge:
    ...
    def test_match_carries_outcome_quality_field(self):
        ...
        m = SimilarityMatch(
            source_type="recommendation", source_id="r1",
            source_date="2026-01-01", tenant_id="t1", field_id="f1",
            crop="wheat", similarity_score=0.8, why_similar_ar=["x"])
...
class TestContractPipelineEnforcement:
    ...
    def _user(self, role=UserRole.AGRONOMIST):
        return UserSchema(user_id="u1", tenant_id="tnt_001",
                         role=role, name_ar="x")
    ...
    def test_complete_pipeline_passes(self):
        delivery = safe_delivery(
            user=self._user(), tenant_id="tnt_001", field_id="fld_03",
            farm_id="frm_01", crop="wheat",
    ...
    def test_missing_cross_reference_caught(self):
        ...
        incomplete = EnrichedRecommendation(
            rec_id="bad", base_recommendation={},
            cross_reference={},   # count مفقود
            provenance={"model_versions": {"fao56": "v1"},
                       "input_snapshot": {}},
            auth_decision={"tenant_id": "t1", "resource_tenant_id": "t1",
                          "permission": "x", "reason_ar": "x"},
            delivered=True, reason_ar="x")
    ...
    def test_enforce_raises_on_incomplete(self):
        ...
        incomplete = EnrichedRecommendation(
            ...
            provenance={}, auth_decision={"tenant_id": "t1"},
            delivered=True, reason_ar="x")
        try:
            enforce_pipeline(incomplete)
            assert False, "كان يجب رفع ContextPipelineError"
...
class TestSafeDelivery:
    ...
    def _user(self):
        return UserSchema(user_id="u1", tenant_id="tnt_001",
                         role=UserRole.AGRONOMIST, name_ar="x")
    ...
    def test_safe_delivery_propagates_district(self):
        ...
        delivery = safe_delivery(
            user=self._user(), tenant_id="tnt_001", field_id="fld_03",
            farm_id="frm_01", crop="wheat", district_id="al_bayda",
    ...
    def test_safe_delivery_blocks_unauthorized(self):
        ...
        worker = UserSchema(user_id="w1", tenant_id="tnt_001",
                           role=UserRole.WORKER, name_ar="x")
        delivery = safe_delivery(
            user=worker, tenant_id="tnt_001", field_id="f", farm_id="fa",
            crop="wheat",
