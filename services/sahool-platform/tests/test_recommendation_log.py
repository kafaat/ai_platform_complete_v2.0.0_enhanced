...
from core.learning.recommendation_log import (
    RecommendationRecord, log_recommendation, load_log,
    record_outcome, compute_mape)


def _rec(rec_id="r1", pred=5.0, actual=None):
    return RecommendationRecord(
        rec_id=rec_id, tenant_id="t1", district_id="d1", zone_id="z1",
        crop="wheat", issued_date="2026-01-01",
        recommendation_ar="توصية", quality_grade="READY",
        predicted_yield_t_ha=pred, confidence="medium", actual_yield_t_ha=actual)
...
class TestConcurrencySafety:
    def test_concurrent_outcomes_no_loss(self):
        ...
        from pathlib import Path
        from core.learning.recommendation_log import (
            RecommendationRecord, log_recommendation, record_outcome, load_log)
        p = Path(tempfile.mktemp(suffix=".csv"))
        for i in range(5):
            log_recommendation(p, RecommendationRecord(
                rec_id=f"r{i}", tenant_id="t", district_id="d", zone_id="z",
                crop="wheat", issued_date="2026-01-01", recommendation_ar="x",
                quality_grade="READY", predicted_yield_t_ha=5.0, confidence="medium"))
        threads = [threading.Thread(target=lambda i=i: record_outcome(p, f"r{i}", 4.0+i*0.1, "2026-06-01")) for i in range(5)]
