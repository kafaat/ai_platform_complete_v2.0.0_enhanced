...
from core.canonical_schemas import FieldQuality, IrrigationMethod, ObservationSource, UserRole
...
class TestFactoryAutogenerates:
    ...
    def test_make_user_has_uuid(self):
        u = make_user(tenant_id="tnt_test", role=UserRole.AGRONOMIST, name_ar="مهندس")
    ...
    def test_make_field_has_uuid(self):
        f = make_field(tenant_id="tnt_001", farm_id="frm_01", name_ar="حقل القمح")
        assert f.id_uuid is not None
        uuid.UUID(f.id_uuid)
    ...
    def test_make_recommendation_has_uuid(self):
        r = make_recommendation(tenant_id="t1", recommendation_ar="اروِ")
...
class TestBackwardCompat:
    ...
    def test_explicit_readable_preserved(self):
        ...
        f = make_field(tenant_id="t1", farm_id="frm_01", name_ar="حقل", field_id_readable="fld_03")
...
class TestUniqueness:
    ...
    def test_multiple_fields_unique_uuids(self):
        ...
        fields = [make_field(tenant_id="t1", farm_id="frm_01", name_ar="حقل") for _ in range(100)]
        uuids = {f.id_uuid for f in fields}
        assert len(uuids) == 100

    def test_recommendation_uuids_unique(self):
        recs = [make_recommendation(tenant_id="t1", recommendation_ar="x") for _ in range(50)]
...
class TestDefaultsPropagation:
    ...
    def test_season_default_planned(self):
        s = make_crop_season(
            tenant_id="t1", field_id="f1", crop_id="wheat", season_name_ar="صيف", season_year=2026
        )
...
class TestMakeDefaultPair:
    def test_returns_uuid_and_readable(self):
        u, r = make_default_pair_for_entity(EntityKind.FIELD, context="test")
        uuid.UUID(u)
