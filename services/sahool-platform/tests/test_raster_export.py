...
from core.spatial.raster_export import grid_to_png, export_summary


class TestPngGeneration:
    def test_basic_grid_produces_valid_png(self):
        grid = [[0.5, 0.6], [0.7, 0.4]]
        r = grid_to_png(grid=grid, indicator="ndvi",
                        south=16.0, west=44.0, north=16.1, east=44.1)
        assert r.png_bytes
        # PNG signature
    ...
    def test_bounds_preserved(self):
        r = grid_to_png(grid=[[0.5]], indicator="ndvi",
                        south=16.0, west=44.0, north=16.1, east=44.1)
...
class TestErrorHandling:
    def test_empty_grid_raises(self):
        ...
        try:
            grid_to_png(grid=[], indicator="ndvi",
                        south=0, west=0, north=1, east=1)
    ...
    def test_unknown_indicator_raises(self):
        try:
            grid_to_png(grid=[[0.5]], indicator="alien_index",
                        south=0, west=0, north=1, east=1)
            assert False, "كان يجب رفع ValueError"
        except ValueError as e:
