...
from core.guardrails import check_guardrails, GuardrailSeverity
...
class TestHaltConditions:
    ...
    def test_severe_salinity_halts(self):
        r = check_guardrails(soil_ec_ds_m=10.0, crop_salinity_threshold_ds_m=6.0,
            has_governing_data=True, zone_factor_calibrated=True)
        assert not r.passed

    def test_deficit_salt_buildup_halts(self):
        # CRITICAL: عجز حادّ بماء مالح → توقّف (الفيزياء ترفض)
        r = check_guardrails(deficit_salinity_risk="high", has_governing_data=True,
            zone_factor_calibrated=True)
        assert not r.passed
        assert r.halted
...
class TestWarnConditions:
    def test_uncalibrated_warns_not_halts(self):
        ...
        r = check_guardrails(pesticide_phi_satisfied=True, has_governing_data=True,
            zone_factor_calibrated=False)
    ...
    def test_salinity_above_threshold_warns(self):
        ...
        r = check_guardrails(soil_ec_ds_m=7.0, crop_salinity_threshold_ds_m=6.0,
            has_governing_data=True, zone_factor_calibrated=True)
        assert r.passed
...
class TestPassThrough:
    def test_all_clear_passes(self):
        r = check_guardrails(pesticide_phi_satisfied=True, has_governing_data=True,
            soil_ec_ds_m=3.0, crop_salinity_threshold_ds_m=6.0,
            zone_factor_calibrated=True)
        assert r.passed
    ...
    def test_guardrail_governs_over_success(self):
        ...
        r = check_guardrails(pesticide_phi_satisfied=False,  # خط أحمر وحيد
            has_governing_data=True, soil_ec_ds_m=2.0,
            crop_salinity_threshold_ds_m=6.0, zone_factor_calibrated=True)
