...
_INDEX_POLICIES = {
    ...
    "BSI": IndexPolicy(
        "BSI",
        IndexCadence.ON_DEMAND,
        "تقدير نوع التربة (مرّة عند الإنشاء أو التربة العارية)",
        None,
        "نوع التربة ثابت جيولوجياً — يُحسب مرّة عند الإنشاء أو بعد الحصاد "
        "(تربة عارية)، ثم يُوقَف. حسابه دورياً هدر للتكلفة",
    ),
    "clay_iron": IndexPolicy(
        "clay_iron",
        IndexCadence.ON_DEMAND,
        "تمييز نسيج التربة (الطين/الحديد)",
        None,
...
def should_compute_now(
    index_id: str, days_since_last: int | None, purpose_active: bool = False
) -> dict:
    ...
    if policy.cadence == IndexCadence.CONTINUOUS:
        ...
        return {
            "compute": due,
            "reason_ar": (
                f"{index_id}: {'حان وقت التحديث' if due else 'حديث، لا داعي'} "
...
def cost_summary(active_indices: list[str]) -> dict:
    ...
    for idx in active_indices:
            ...
            continue
        {
            ...
            IndexCadence.EVENT: event,
        }[p.cadence].append(idx)
    return {
        "continuous": continuous,
        "on_demand": on_demand,
        "event": event,
        "note_ar": (
            f"دائمة (تكلفة متكرّرة): {len(continuous)} · "
