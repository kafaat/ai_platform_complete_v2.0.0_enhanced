...
class ColumnProfile:
    ...
    # الـmapping المقترَح
    suggested_mapping: Optional[CanonicalCategory] = None
...
def infer_column_type(values: list[Any]) -> FieldType:
    ...
    # محاولة date
    date_count = sum(
        1 for v in non_null if _try_parse_date(str(v)) is not None
    )
    ...
    # محاولة number
    number_count = sum(
        1 for v in non_null if _try_parse_number(str(v)) is not None
    )
    if number_count / len(non_null) > 0.8:
        # int أم float؟
        int_count = sum(
            1 for v in non_null
            if (n := _try_parse_number(str(v))) is not None
            and n == int(n)
        )
...
def suggest_mapping_for_column(
...
) -> tuple[Optional[CanonicalCategory], float]:
    ...
    best_cat: Optional[CanonicalCategory] = None
    best_conf = 0.0
    ...
    if best_cat:
        ...
        if expected_numeric and not is_numeric:
            best_conf *= 0.5
        elif best_cat in (CanonicalCategory.SOWING_DATE,
                          CanonicalCategory.HARVEST_DATE,
...
def check_plausibility(
    ...
    category: Optional[CanonicalCategory],
) -> list[str]:
    ...
    if profile.min_val is not None and profile.min_val < expected_min:
        notes.append(
            f"قيم أدنى من المتوقَّع: min={profile.min_val}, "
            f"متوقَّع >={expected_min}"
        )
    if profile.max_val is not None and profile.max_val > expected_max:
        notes.append(
            f"قيم أعلى من المتوقَّع: max={profile.max_val}, "
            f"متوقَّع <={expected_max}"
        )
    return notes
...
def build_report(
    ...
    # ٥. الـcoverage
    mapped = [p for p in profiles if p.suggested_mapping
              and p.suggested_mapping != CanonicalCategory.UNKNOWN]
...
def format_report_ar(report: OnboardingReport) -> str:
    ...
    for p in report.columns:
        mapped = (
            p.suggested_mapping.value if p.suggested_mapping else "غير محدَّد"
        )
        lines.append(
