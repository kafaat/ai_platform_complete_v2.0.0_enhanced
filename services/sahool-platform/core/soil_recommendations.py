...
def fertilizer_hint_from_ph(soil_ph: float | None) -> FertilizerHint:
    ...
    return FertilizerHint(
        soil_ph=soil_ph, ph_class=cls, hints_ar=hints, requires_lab=False,
...
_IRRIGATION_BY_TEXTURE = {
    "رملي": IrrigationHint("رملي",
        "ري متكرّر بكمّيات صغيرة",
        "الرمل يحتفظ بماء قليل (TAW منخفض) ويصرف سريعاً — دفعات صغيرة متقاربة تقلّل الفقد بالتسرّب"),
    "طمي-رملي": IrrigationHint("طمي-رملي",
        "ري معتدل التكرار",
        "سعة احتفاظ متوسطة-منخفضة — توازن بين التكرار والكمّية"),
    "طميي": IrrigationHint("طميي",
        "ري متوازن (الأمثل)",
        "الطمي أمثل سعة احتفاظ — جدولة مريحة، أقل إجهاد مائي"),
    "طيني": IrrigationHint("طيني",
...
class CropBias:
    ...
    warning_ar: str = ("⚠️ النسيج عامل واحد فقط. القرار النهائي يحتاج "
                       "الملوحة وpH والماء والمناخ عبر نظام الملاءمة.")


_CROP_BIAS_BY_TEXTURE = {
    "رملي": CropBias("رملي",
        favored_ar=["محاصيل جذرية (بطاطس، جزر)", "أشجار متحمّلة للجفاف (نخيل، لوز)",
                    "بقوليات خفيفة"],
        cautioned_ar=["القمح (يحتاج ريّاً مكثّفاً في الرمل)", "الأرز (غير مناسب)"]),
    "طميي": CropBias("طميي",
        favored_ar=["معظم المحاصيل (الأمثل)", "خضروات", "حبوب"],
        cautioned_ar=[]),
    "طيني": CropBias("طيني",
        favored_ar=["الحبوب (قمح، شعير)", "الأعلاف", "محاصيل تتحمّل الرطوبة"],
        cautioned_ar=["المحاصيل الجذرية (صعوبة النمو في الطين الثقيل)",
                      "محاصيل حسّاسة للتشبّع"]),
}
...
def soil_to_recommendations(texture: str | None, soil_ph: float | None) -> dict:
    ...
    out = {"irrigation": None, "fertilizer": None, "crop_bias": None,
