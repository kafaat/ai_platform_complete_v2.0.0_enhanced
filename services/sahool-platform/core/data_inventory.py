...
class DataSource:
    ...
    notes: Optional[str]
    trigger: Optional[str]   # لـplanned status
...
def _naive_yaml_parse(path: str) -> dict:
    ...
    raise RuntimeError(
        "PyYAML غير مثبَّت. ثبّته: pip install pyyaml"
    )


_INVENTORY_CACHE: Optional[dict[str, Theme]] = None
...
def sources_by_criticality(criticality: str) -> list[DataSource]:
    ...
    themes = load_inventory()
    return [
        s for theme in themes.values()
        for s in theme.sources
        if s.criticality == criticality
    ]
...
def planned_sources_with_triggers() -> list[DataSource]:
    ...
    return [
        s for theme in themes.values()
        for s in theme.sources
        if s.status == 'planned' and s.trigger
    ]


def find_source(source_id: str) -> Optional[DataSource]:
    """يبحث عن مصدر بـID."""
    themes = load_inventory()
...
def gaps_report() -> dict[str, list[str]]:
    ...
    for theme_id, theme in themes.items():
        critical_active = [
            s for s in theme.sources
            if s.criticality == 'A' and s.status == 'active'
        ]
        critical_planned = [
            s for s in theme.sources
            if s.criticality == 'A' and s.status == 'planned'
        ]
