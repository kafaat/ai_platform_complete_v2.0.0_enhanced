...
class UserRole(str, Enum):
    """أدوار النظام. الأدوار الخمسة الأولى **داخل المستأجِر** (ترتيب الصلاحيات تنازلي).

    PLATFORM_ADMIN دورٌ **عابر للنطاق التشغيليّ** (إدارة المنصّة) منفصلٌ عمداً عن
    OWNER: حوكمة الصلاحيات تقتضي `platform_admin ≠ tenant_owner` — مدير المنصّة
    يدير المنصّة (مستخدمون/تدقيق/DLQ) ولا يملك بيانات المستأجِر افتراضيّاً. الوصول
    العابر للمستأجرين يكون عبر break-glass صريح (سبب+مدّة+MFA+تدقيق) لا تلقائيّاً.
    """

    OWNER = "owner"  # كل صلاحيات المستأجِر + إدارة الفريق (داخل tenant واحد)
    ...
    PLATFORM_ADMIN = "platform_admin"  # إدارة المنصّة — لا بيانات مستأجِر افتراضيّاً
