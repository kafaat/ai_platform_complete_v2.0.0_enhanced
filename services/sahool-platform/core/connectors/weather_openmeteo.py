...
class OpenMeteoConnector(BaseConnector):
    ...
    def parse_response(self, resp: dict, lat: float, elevation_m: float,
                       day_index: int = 0) -> WeatherInputs:
        ...
        return WeatherInputs(
            ...
            latitude=lat, elevation_m=elevation_m,
        )

    def fetch(self, lat: float = 0, lon: float = 0, elevation_m: float = 0,
        ...
        try:
            ...
            return ConnectorResult(
                source=self.source_name, status=FetchStatus.OK,
                data=wx.__dict__,
