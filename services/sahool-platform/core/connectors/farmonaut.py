...
def validate_field_polygon(points: dict) -> tuple[bool, str]:
    ...
    for key, pt in points.items():
        ...
        if not (
            YEMEN_BOUNDS["lat"][0] <= lat <= YEMEN_BOUNDS["lat"][1]
...
def estimate_monthly_credits(
    hectares: float, fields_count: int, weather_calls_per_day: int = 4
) -> CreditEstimate:
    ...
    return CreditEstimate(
        satellite_units=round(satellite),
        weather_units=round(weather, 1),
        api_units=round(api, 1),
...
class FarmonautConnector(BaseConnector):
    ...
    def build_submit_request(self, name: str, crop_code: str, points: dict, uid: str) -> dict:
    ...
    def fetch(
        ...
        field_id: str = "",
        sense_date: str = "",
        image_type: ImageType = ImageType.NDVI,
        is_cloudy: bool = False,
        ...
        **kwargs,
        ...
        return ConnectorResult(
            source=f"{self.source_name}:{actual.value}",
            status=status,
            data=_live_response,
            error_margin=0.05 if actual != ImageType.RVI else 0.10,
            note_ar=(f"مؤشر {actual.value}" + (" (رادار — سحب)" if is_cloudy else " (بصري)")),
        )
