...
def harmonize_unit(value: float, from_unit: str) -> UnitResult:
    ...
    if u in _AMBIGUOUS_UNITS:
        return UnitResult(
            None, u, False, f"وحدة غامضة ({u}) — تحتاج سياقاً (مثلاً المساحة) للتحويل. مرفوضة."
        )
    ...
    for (src, dst), factor in _UNIT_CONVERSIONS.items():
        if src == u and dst in _CANONICAL_UNITS:
            return UnitResult(round(value * factor, 4), dst, True, f"حُوّلت {u}→{dst} (×{factor})")
    return UnitResult(None, u, False, f"وحدة غير قابلة للتتبّع إلى SI/FAO ({u}) — مرفوضة")
...
def spatial_substitution_validity(
    measurement_type: str, distance_m: float, base_confidence: str = "medium"
) -> SpatialValidity:
    ...
    if L is None:
        return SpatialValidity(
            False, "none", 0.0, f"نوع قياس غير معروف ({measurement_type}) — لا إحلال مكاني"
        )
