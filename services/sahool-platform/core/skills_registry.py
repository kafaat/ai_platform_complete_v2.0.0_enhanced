...
def available_for_field(quality_grade: str, available_inputs: set) -> list[SkillSignature]:
...
def registry_health() -> dict:
    ...
    return {
        ...
        "issues": issues,
        "summary_ar": (
            f"إجمالي {len(_REGISTRY)} skill، "
            f"{len([s for s in _REGISTRY.values() if s.safety_critical])} "
            f"حرجة للسلامة" + (f"، {len(issues)} ناقصة التوثيق" if issues else "، التوثيق كامل")
        ),
    }
