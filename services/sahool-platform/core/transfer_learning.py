...
def _compute_district_similarity(
    ...
    if (source.governorate_id and target.governorate_id
    ...
    if (source.dominant_soil_type and target.dominant_soil_type
        ...
        decays.append(f"تربة مختلفة (مصدر: {source.dominant_soil_type}، "
                     f"هدف: {target.dominant_soil_type})")
    ...
    if (source.typical_salinity_ds_m and target.typical_salinity_ds_m):
        ...
        denom = max(source.typical_salinity_ds_m,
                   target.typical_salinity_ds_m, 1e-6)
...
def suggest_transfer(
    ...
    *,
    ...
    same_tenant_sources = [
        ...
        if s.tenant_id == target_district.tenant_id
        and s.district_id != target_district.district_id
    ]
    ...
    if not same_tenant_sources:
        return TransferSuggestion(
            ...
            notes_ar=("لا مديريات مصدر متاحة في نفس tenant — "
                     "لا نقل ممكن. ابدأ بمعايرة محلّية من الموسم الأوّل."),
        )

    # يجب أن يحوي المحصول
    relevant_sources = [
        s for s in same_tenant_sources
        if crop in s.common_crops
    ]
    if not relevant_sources:
        return TransferSuggestion(
            ...
            suggested_zone_factor=None,
            confidence=TransferConfidence.NONE,
            notes_ar=(f"لا مديرية مصدر زرعت '{crop}' من قبل — "
                     "لا نقل ممكن لهذا المحصول."),
        )
    ...
    if best_score < min_similarity:
        return TransferSuggestion(
            ...
            notes_ar=(f"أفضل مرشّح ({best_source.district_id}) أقلّ من العتبة "
    ...
    if best_source.avg_zone_factor is not None:
        ...
        reduction = {TransferConfidence.HIGH: 1.0,
                    TransferConfidence.MEDIUM: 0.95,
                    TransferConfidence.LOW: 0.85}[confidence]
        ...
        suggested_zf = round(
            best_source.avg_zone_factor * reduction +
            1.0 * (1 - reduction), 3)
    ...
    return TransferSuggestion(
        ...
        reasons_ar=best_reasons,
        decay_factors_ar=best_decays,
        notes_ar=(f"اقتراح أوّلي من '{best_source.district_id}' بثقة "
