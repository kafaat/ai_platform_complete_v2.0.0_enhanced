...
def design_rcbd(
    *,
    ...
    return TrialDesign(
        treatments=trts,
        n_blocks=n_blocks,
        n_replications=n_blocks,
        total_plots=total,
        valid=valid,
        warnings_ar=warnings,
        layout_note_ar=(
            f"RCBD: {len(trts)} معاملة × {n_blocks} كتلة = {total} قطعة. "
    ...
    *,
    ...
    if not others:
        return TrialAnalysis(
            ...
            confidence="none",
            promotion_signal=False,
            note_ar="لا معاملات لمقارنتها بالشاهد",
        )
    ...
    if not meets_mde:
        note = (
            f"الفرق ({effect_pct:+.1f}%) دون الحد الأدنى للمغزى ({mde_pct}%) — "
            "عديم القيمة العملية مهما كان 'دالّاً' إحصائياً"
        )
    elif promotion:
        note = (
            f"معاملة «{best}» تتفوّق على الشاهد بـ{effect_pct:+.1f}% "
            "(ذو مغزى عملي) — إشارة ترقية للممارسة، تحتاج تكرار مواسم للتثبيت"
        )
    else:
        note = (
            f"معاملة «{best}» تتفوّق بـ{effect_pct:+.1f}% لكن الثقة محدودة — "
