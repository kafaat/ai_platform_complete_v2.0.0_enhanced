...
def compute_district_baseline(
    district_id: str, observable_id: str,
    ...
    if n == 0:
        return DistrictBaseline(district_id, observable_id, 0, None, None, None,
            "none", False, "لا مزارع محلّلة في هذه المديرية بعد")
    ...
    if n < MIN_FARMS_FOR_BASELINE:
        ...
        note = (f"{n} مزارع محلّلة فقط (<{MIN_FARMS_FOR_BASELINE}) — "
                f"سياق أولي غير موثوق للتعميم")
    else:
        # التجانس يحدّد الثقة: تشتّت عالٍ = مديرية غير متجانسة
        conf = "medium" if (spread is None or spread < med * 0.3) else "low"
        usable = True
        note = (f"خط أساسي من {n} مزارع محلّلة. "
                f"{'متجانس' if conf=='medium' else 'متفاوت — استخدم بحذر'}")
    return DistrictBaseline(district_id, observable_id, n, med, avg, spread,
        conf, usable, note)
...
def context_for_low_data_farmer(
    baseline: DistrictBaseline, observable_name_ar: str,
) -> FarmerContext:
    ...
    if not baseline.is_usable:
        return FarmerContext(
            ...
            context_ar=(f"مديريتك تحتاج {MIN_FARMS_FOR_BASELINE} مزارع محلّلة "
    ...
    return FarmerContext(
        ...
        context_ar=(f"متوسط {observable_name_ar} لدى {baseline.n_farms} مزارع "
                    f"محلّلة في مديريتك ≈ {baseline.median_value}. "
                    f"هذا سياق المنطقة، وليس قيمة حقلك."),
        is_field_specific=False,  # حاسم: ليس قيمة حقله
        motivation_ar=("حلّل تربتك لتعرف قيمتك الفعلية وتحصل على توصيات دقيقة — "
                       "قد يختلف حقلك عن المتوسط"),
