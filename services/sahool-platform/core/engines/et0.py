...
def extraterrestrial_radiation_mj(lat_deg: float, doy: int) -> float:
    """الإشعاع خارج الغلاف الجوّي Ra (MJ/m²/يوم) — FAO-56 eq. 21-25.

    يُحسب من (خطّ العرض + يوم السنة) عبر الميل الشمسي وزاوية الغروب والمسافة
    الشمسيّة النسبيّة — **لا ثابت مُختلق**. هذا هو إصلاح خطأ الصحّة الموثّق (H4):
    كانت مواقع تستعمل Ra ثابتاً (15.0 في season_simulation، 14.0 في weather_analytics)
    فينحرف الناتج عن الواقع الجغرافيّ. تغيّر المخرجات المتوقّع (قبل/بعد) لخطّ عرض
    اليمن 15.5° ويوم 100: Ra بمكافئ التبخّر ≈ 15.43 mm/يوم محسوبةً مقابل 15.0/14.0
    ثابتةً سابقاً. تحقّق مرجعيّ: مثال FAO-56 رقم 8 (lat −20°، doy 246) ⇒ Ra≈32.2 MJ.
    """
