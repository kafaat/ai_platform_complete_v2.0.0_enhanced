...
def compute_water_gap(
    *,
    ...
    if etc_mm <= 0:
        return WaterGap(etc_mm, rainfall_mm, 0.0, 0.0, growth_stage,
                       0.0, False, None, "none",
    ...
    if gap <= 0:
        note = (f"المطر ({rainfall_mm}مم) يلبّي الاحتياج ({etc_mm}مم) — "
                "لا حاجة لريّ تكميلي")
    elif gap_pct < 20:
        note = (f"فجوة طفيفة ({gap_pct:.0f}%) — يمكن تجاوزها بمخزون التربة "
                "دون ريّ تكميلي")
    ...
    return WaterGap(
        etc_mm=etc_mm, rainfall_mm=rainfall_mm, gap_mm=round(gap, 1),
        gap_pct=round(gap_pct, 1), stage=growth_stage,
        stage_sensitivity=sensitivity, needs_supplemental=needs,
        recommended_mm=recommended, confidence=confidence,
...
def seasonal_summary(monthly_gaps: list[WaterGap]) -> dict:
    ...
    return {
        ...
        "note_ar": (f"الموسم: فجوة إجمالية {total_gap:.0f}مم؛ "
