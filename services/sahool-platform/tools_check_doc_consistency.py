...
def check():
    ...
    disk_tests = sum(len(re.findall(r'def test_', open(f, encoding='utf-8').read()))
    ...
    leak_patterns = [
        ...
        r"\bAl-?[Jj]awf\b", r"\bA[lL]-?[Jj]awf\b", r"\bAljawf\b",
        r"\b[Ss]akha\b", r"\bSunaydar\b", r"\bSunaidar\b",
        r"\bAl-?Hazm\b", r"\bAlhazm\b",
        r"\bAl-?Bayda\b", r"\bAlbayda\b", r"\bal_bayda\b",
        r"\bal_jawf\b", r"\baljawf\b", r"\btihama\b", r"\bTihama\b",
        # Arabic forms (verbatim)
        r"الجوف", r"السنيدار", r"الحزم",
        r"البيضاء", r"تهامة",
        # specific field/farm identifiers
        r"\b142ha\b", r"\b6\.17ha?\b",
        r"\bfld_yem_alb_\d+\b",
    ]

    # الفحص (أ): الكود في core/ فقط — نواة محايدة 100%
    core_files = [p for p in glob.glob(str(ROOT / "core/**/*.py"), recursive=True)
    ...
    forbidden_dir_patterns = [
        r"al_?jawf", r"tihama", r"al_?bayda", r"al_?hazm",
        r"sakha", r"sunaydar", r"aljawf-\d+ha",
    ]
    structure_leaks = []
    for root_dir, dirs, _ in os.walk(ROOT):
        # تخطّى examples/ و node_modules
        if any(skip in root_dir for skip in
              ["examples", "node_modules", "__pycache__", ".git"]):
            continue
        for d in dirs:
