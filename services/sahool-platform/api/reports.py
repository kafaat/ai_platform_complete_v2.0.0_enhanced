...
class FieldReport:
    ...
    estimated_yield_total_kg: float | None = None
    actual_yield_kg_ha: float | None = None  # بعد الحصاد فقط
    actual_yield_total_kg: float | None = None

    # Soil
    soil_samples_count: int = 0
    last_soil_ph: float | None = None
    last_soil_ec: float | None = None
...
class OperationReport:
    ...
    def total_yield_kg(self) -> float:
        return sum(
            (f.actual_yield_total_kg or f.estimated_yield_total_kg or 0) for f in self.fields
        )
...
def field_to_pdf_bytes(field_report: FieldReport) -> bytes:
    ...
    try:
        ...
        raise RuntimeError("reportlab غير مُثبَّت. ثبّته بـ: pip install reportlab") from err
    ...
    data = [
        ...
        [
            f"{field_report.estimated_yield_kg_ha:.0f} كغ/هـ"
            if field_report.estimated_yield_kg_ha
            else "-",
            "الإنتاج المتوقّع",
        ],
    ]
    ...
    elements.append(
        Paragraph(
            f"تمّ التوليد: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}",
            rtl_style,
        )
    )
