...
# USDA-SCS: نقطةُ انتقال الفرعين. عندها يتساوى الفرعان (150.0) فيكون الانتقالُ متّصلاً.
# القيمةُ ليست اختياراً بل خاصّيّةُ الصيغة: أيُّ حدٍّ آخرَ يُنتِج قفزة.
_SCS_BREAKPOINT_MM = 250.0


def _effective_rain(rain_mm: float) -> float:
    """المطر الفعّال (USDA-SCS): جزء من المطر يصل الجذور — لمجموعٍ **شهريّ** بالمِلِّيمتر.

    **العطلُ الذي وُجِد هذا لأجله (D07):** كان الحدُّ ``75`` والثابتُ ``92.5``، وكلاهما
    محرَّفٌ عن الصيغة المرجعيّة. فأنتجت الدالّةُ **ماءً من العدم**: ``75`` مم مطرٍ تُرجِع
    ``100`` مم «فعّالة»، و``100`` تُرجِع ``102.5``. وعند الحدّ قفزةٌ مقدارُها **٣٤.٠١ مم**
    بين ``74.99`` (⇒ 65.99) و``75`` (⇒ 100).

    **والاتّجاه هو الأسوأ:** المطرُ الفعّال يُطرَح من الاحتياج، فمطرٌ مُبالَغٌ يُنقِص
    الاستنزافَ المحسوب ⇒ ريٌّ دون الحاجة. وخمسةُ مستهلكين يقرؤونها:
    ``root_zone_balance`` · ``irrigation_mpc`` · ``water_ledger_auto`` · ``weather_advice``
    وهذه الوحدةُ نفسُها.

    **والصيغةُ المرجعيّة متّصلةٌ بالبناء** (USDA-SCS، مجموعٌ شهريّ بالمِلِّيمتر):

        Pe = P·(125 − 0.2·P)/125    لـ P ≤ 250
        Pe = 125 + 0.1·P            لـ P > 250

    وعند ``P = 250`` يُعطي الفرعان ``150.0`` كلاهما — فالاتّصالُ خاصّيّةُ الصيغة لا
    مصادفةَ تقريب. ولذلك **لم يُقصَّ الناتجُ عند المدخل**: القصُّ كان سيُخفي الفرعَ
    المحرَّفَ ويُبقي القفزةَ قائمةً تحت غطاء.

    **حدُّ صدقٍ مُعلَن:** المرجعُ يصف مجموعاً **شهريّاً**، والمستهلكون يمرّرون مطرَ
    **يومٍ** (``day.rain_mm``). فالوحدةُ الزمنيّةُ غيرُ متّسقةٍ مع المرجع، وهذا قيدٌ
    قائمٌ **لم تُصلِحه هذه الشريحة** — أُصلِح الثابتان المحرَّفان ولم يُبَتّ في وحدة
    الزمن، لأنّ ذلك يوجب مرجعاً زراعيّاً ومعايرةً لا اشتقاقاً من الشيفرة.
    """
    if rain_mm <= 0:
        return 0.0
    if rain_mm <= _SCS_BREAKPOINT_MM:
        return rain_mm * (125 - 0.2 * rain_mm) / 125
    return 125 + 0.1 * rain_mm
