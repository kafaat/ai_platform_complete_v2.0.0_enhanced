"""نماذج الطلب/الاستجابة (Pydantic) لنواة سهول — منصّة sahool-platform.

استُخرِجت حرفيّاً من ``api/main.py`` لتقليص حجم الوحدة الأحاديّة وفصل عقود البيانات
(schemas) عن منطق FastAPI/DB. ``api/main.py`` يعيد استيرادها وتصديرها كما هي
(سلوك محفوظ بالكامل — لا تغيير في الحقول/الأنواع/التحقّق). الراوترات الـ127 تُبقي
``from api.main import <Model>`` عاملاً عبر إعادة التصدير.

لا تستورد هذه الوحدة من ``api.main`` (تفادي الاستيراد الدائريّ) — النماذج حقول صرفة
بلا مُتحقِّقات ولا استدعاء دوال main، وتعتمد فقط على pydantic وtyping. ترتيب التعريف
محفوظ كي تُحلّ الإشارات المتداخلة بين النماذج (نموذج يحوي نموذجاً آخر حقلاً).
"""
