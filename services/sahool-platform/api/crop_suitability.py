...
class SuitabilityScore:
    ...
    def to_dict(self) -> Dict:
        return {
            "crop": self.crop, "name_ar": self.name_ar,
            "score": round(self.score, 3), "rating_ar": self.rating_ar,
            "reasons_ar": self.reasons_ar,
        }


def _score_range(value: float, optimal: Tuple[float, float], tolerable: Tuple[float, float]) -> float:
...
def score_crop(cond: FieldConditions, tol: CropTolerance) -> SuitabilityScore:
    ...
    if cond.irrigated or cond.season_rain_mm is None:
        ...
        rain_s = 1.0 if cond.season_rain_mm >= tol.rain_min_mm else cond.season_rain_mm / tol.rain_min_mm
        if rain_s < 0.7:
            reasons.append(f"المطر ({cond.season_rain_mm} مم) دون حاجة {tol.name_ar} ({tol.rain_min_mm})")
    ...
    score = (ph_s * _WEIGHTS["ph"] + ec_s * _WEIGHTS["ec"]
             + rain_s * _WEIGHTS["rain"] + temp_s * _WEIGHTS["temp"])
    ...
    return SuitabilityScore(
        crop=tol.crop, name_ar=tol.name_ar, score=score,
        rating_ar=rating, reasons_ar=reasons,
    )


def rank_crops(cond: FieldConditions, crops: Optional[List[str]] = None) -> Dict:
