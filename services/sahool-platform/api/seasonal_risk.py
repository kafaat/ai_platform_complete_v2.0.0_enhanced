...
_ZONE_HAZARDS: Dict[str, List[Dict]] = {
    "tihama": [
        {"hazard_ar": "موجات حرّ شديدة", "season_ar": "الصيف (يونيو-سبتمبر)",
         "risk_to_ar": "الإزهار وعقد الثمار", "severity": "high"},
        {"hazard_ar": "رطوبة بحريّة عالية", "season_ar": "على مدار السنة",
         "risk_to_ar": "أمراض فطريّة", "severity": "medium"},
        {"hazard_ar": "سيول مفاجئة", "season_ar": "موسم الأمطار (يوليو-أغسطس)",
         "risk_to_ar": "انجراف/إغراق", "severity": "medium"},
    ],
    "western_highlands": [
        {"hazard_ar": "أمطار غزيرة وقت الحصاد", "season_ar": "الصيف الممطر",
         "risk_to_ar": "تعفّن الحبوب/تدنّي الجودة", "severity": "medium"},
        {"hazard_ar": "ضباب ورطوبة", "season_ar": "موسم الأمطار",
         "risk_to_ar": "أمراض فطريّة (خاصّةً البنّ/الخضار)", "severity": "medium"},
        {"hazard_ar": "صقيع خفيف", "season_ar": "ليالي الشتاء (القمم العالية)",
         "risk_to_ar": "البراعم الزهريّة", "severity": "low"},
    ],
    "central_highlands": [
        {"hazard_ar": "صقيع شتوي", "season_ar": "ليالي الشتاء (ديسمبر-فبراير)",
         "risk_to_ar": "المحاصيل الحسّاسة والبراعم", "severity": "high"},
        {"hazard_ar": "بَرَد", "season_ar": "العواصف الرعديّة الربيعيّة/الصيفيّة",
         "risk_to_ar": "تلف ميكانيكي للثمار والأوراق", "severity": "medium"},
        {"hazard_ar": "تذبذب المطر", "season_ar": "الصيف",
         "risk_to_ar": "إجهاد مائي للبعليّ", "severity": "medium"},
    ],
    "eastern_plateau": [
        {"hazard_ar": "حرارة شديدة + جفاف", "season_ar": "الصيف",
         "risk_to_ar": "الإجهاد الحراري والمائي", "severity": "high"},
        {"hazard_ar": "سيول الأودية المفاجئة", "season_ar": "الأمطار النادرة",
         "risk_to_ar": "جرف المحاصيل", "severity": "medium"},
    ],
    "inland_desert": [
        {"hazard_ar": "موجات حرّ مدمّرة (>42°م)", "season_ar": "الصيف",
         "risk_to_ar": "الإزهار وكلّ المراحل الحسّاسة", "severity": "high"},
        {"hazard_ar": "جفاف ممتدّ", "season_ar": "معظم العام",
         "risk_to_ar": "نقص الماء (اعتماد كلّي على الجوفي)", "severity": "high"},
        {"hazard_ar": "عواصف ترابيّة/رياح", "season_ar": "الربيع/الصيف",
         "risk_to_ar": "إجهاد ميكانيكي وتعرية", "severity": "medium"},
    ],
    "southern_coast": [
        {"hazard_ar": "حرارة ورطوبة عالية", "season_ar": "الصيف",
         "risk_to_ar": "الإجهاد والأمراض الفطريّة", "severity": "high"},
        {"hazard_ar": "رياح موسميّة (المهرة)", "season_ar": "الخريف (موسم ظفار)",
         "risk_to_ar": "إجهاد ميكانيكي", "severity": "medium"},
    ],
}
...
def stage_risk_check(zone: str, stage_ar: str) -> Dict:
    ...
    for h in cal["hazards"]:
        if any(k in h["hazard_ar"] or k in h["risk_to_ar"] for k in keywords) \
    ...
    return {
        ...
        "risk_level_ar": (
            "مرتفع — تجنّب هذه المرحلة في نافذة الخطر" if any(
                h["severity"] == "high" for h in relevant)
            else "متوسّط — راقب الطقس" if relevant
            else "منخفض — لا مخاطر بارزة معروفة لهذه المرحلة"
        ),
        "advice_ar": (
            "اضبط موعد الزراعة بحيث تقع هذه المرحلة خارج نافذة الخطر "
            "(راجع تقويم الزراعة)." if relevant else
            "لا تعديل خاصّ مطلوب لهذه المرحلة."
        ),
...
def chill_hours_estimate(zone: str) -> Dict:
    ...
    crops_chill = {
        "التفاح": 800, "الكمّثرى": 600, "اللوز": 300, "الخوخ": 400,
        "العنب": 100, "الرمّان": 100, "التين": 100,
    }
    ...
    return {
        ...
        "verdict_ar": (
            f"الإقليم يوفّر ~{chill} ساعة برودة. " + (
                ...
                if chill >= 600 else
                "كافٍ للأشجار متوسّطة الاحتياج (خوخ/لوز/رمّان/تين/عنب) لا التفاح التجاري."
                if chill >= 400 else
                "محدود — يناسب أشجاراً قليلة الاحتياج (رمّان/تين/عنب) + لوز بحذر."
                if chill >= 100 else
                "⚠ لا برودة كافية — الأشجار المتساقطة المحتاجة للبرودة (تفاح) "
