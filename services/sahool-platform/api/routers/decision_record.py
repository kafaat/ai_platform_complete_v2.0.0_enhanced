...
async def _mirror_to_decision_service(fn, payload: dict, *, tenant_id: str, label: str) -> None:
    """جسر انتقاليّ (INTERIM BRIDGE): مِرْآة **غير موثوقة** (best-effort) إلى decision-service.

    المنصّة هي مصدر السجلّ المؤقّت (Source of Record): كتابة قاعدة البيانات المحليّة تمّت
    ونجحت **قبل** استدعاء هذه الدالّة، وهي وحدها ما تُبنى عليه استجابة الطلب. هذه المِرْآة
    نسخة تشغيليّة إلى decision-service (ليست مصدر السجلّ بعدُ) — **يجب ألّا ترفع أبداً** إلى
    مسار الطلب: أيّ فشل يُسجَّل ويُرصَد فقط، ولا يفقد بيانات المنصّة ولا يُفشِل الطلب.
    """
    try:
        await fn(payload, tenant_id=tenant_id)
    except Exception as e:  # noqa: BLE001 — مِرْآة best-effort: الفشل يُسجَّل ولا يُفشِل الطلب
        logger.warning(
            "decision-service mirror (%s) فشلت — كتابة المنصّة موثوقة ومحفوظة: %s", label, e
        )
...
async def persist_decision_if_enabled(
    ...
    *,
    ...
    try:
        ...
        # الجسر الانتقاليّ: كتابة المنصّة أعلاه موثوقة ومحفوظة؛ نعكسها best-effort فقط.
        await _mirror_to_decision_service(
            ...
            label=f"auto-persist decision {decision_id}",
        )
        return True
...
async def record_decision(
    ...
    # الجسر الانتقاليّ: كتابة المنصّة أعلاه هي المصدر الموثوق وقد التُزِمت (committed).
