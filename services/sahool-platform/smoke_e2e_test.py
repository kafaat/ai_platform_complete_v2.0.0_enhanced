...
from api.main import app  # noqa: E402  (must follow env + sys.path setup above)
...
def ck(n, cond, d=""):
    (P if cond else F).append(n)
    print(f"  {'✓' if cond else '✗'} {n}" + (f" — {d}" if d and not cond else ""))


with TestClient(app, raise_server_exceptions=False) as c:
    print("\n── SMOKE ──")
    r = c.get("/healthz")
    ck("GET /healthz = 200", r.status_code == 200, f"{r.status_code}")
    ck("healthz body is dict", isinstance(r.json(), dict))
    r = c.get("/readyz")
    ck("GET /readyz = 200 (DB pool ready)", r.status_code == 200, f"{r.status_code}")
    r = c.get("/openapi.json")
    ck(
        "GET /openapi.json = 200 [OpenAPI fix: model_rebuild]",
        r.status_code == 200,
        f"{r.status_code}",
    )
    if r.status_code == 200:
        ck(
            "OpenAPI exposes routes (>50)",
            len(r.json().get("paths", {})) > 50,
            str(len(r.json().get("paths", {}))),
        )
    ...
    ck(
        "POST /confidence/ndvi (naive date) = 200 [H8 fix: no 500]",
        r.status_code == 200,
        f"{r.status_code}: {r.text[:160]}",
    )
    if r.status_code == 200:
        ck(
            "confidence returns a score",
            "confidence" in r.text or "score" in r.text or "level" in r.text,
        )
    ...
    ck(
        "DB/compute endpoint reachable (not 503 pool-missing)",
