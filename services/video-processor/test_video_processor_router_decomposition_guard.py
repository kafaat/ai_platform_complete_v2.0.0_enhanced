#!/usr/bin/env python3
"""حارس تفكيك video-processor — نظير حارس المنصّة/raster.

يفرض ضمانات تجعل الاستخراج إلى routers/ آمناً من الانحدار:
  ...
  5) المسارات الحرجة الممثِّلة حاضرة + أرضيّة عدد المسارات ≥ 12 (التفكيك لا يُسقط مساراً).
...
except Exception:  # noqa: BLE001 — تبعيّات video-processor الدنيا غير متوفّرة
    pytest.skip("video-processor main import unavailable", allow_module_level=True)
...
# تثبّت مكاسب التفكيك ضدّ أيّ انحدار لاحق: المسارات الممثِّلة لا تختفي، وعدد
# المسارات لا يهبط تحت 12، وكلّ وحدة في routers/ مُضمَّنة (لا راوتر يتيم).
...
def test_critical_routes_present():
    """المسارات الممثِّلة كلّها حاضرة في app.routes (لا تختفي بعد التفكيك)."""
    paths = _app_paths()
