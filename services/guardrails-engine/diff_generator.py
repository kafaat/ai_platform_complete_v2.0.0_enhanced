...
class ActionDiffGenerator:
    ...
    async def generate(self, original: Dict, checks: List[Dict], 
                       farm_context: Dict) -> Dict[str, Any]:
        """
    ...
    def _suggest_organic_alternative(self, banned_chemical: str, crop: str, 
                                     pest_target: str) -> List[str]:
        ...
        return alternatives.get(banned_chemical, ["Consult local agricultural extension for organic alternatives"])

    def _calculate_impact(self, original: Dict, safe: Dict, changes: List[Dict], 
                          farm_context: Dict) -> Dict:
        ...
        cost_change_pct = ((safe_cost - original_cost) / original_cost * 100) if original_cost > 0 else 0

        # Yield impact estimate (simplified)
        original_yield = original.get("projected_yield_kg_ha", farm_context.get("expected_yield_kg_ha", 2000))
        safe_yield = original_yield * 0.95  # Conservative: 5% yield reduction for safer methods

        yield_change_pct = ((safe_yield - original_yield) / original_yield * 100) if original_yield > 0 else 0
    ...
    def _generate_arabic_summary(self, changes: List[Dict], impact: Dict) -> str:
        ...
        for i, change in enumerate(changes, 1):
            severity_emoji = {"CRITICAL": "🔴", "HIGH": "🟠", "MEDIUM": "🟡", "LOW": "🟢"}.get(change["severity"], "⚪")
            ...
            if "organic_alternatives" in change:
                lines.append(f"   🌿 **بدائل عضوية:** {', '.join(change['organic_alternatives'][:2])}")
            lines.append("")

        lines.append("📊 **تأثير التعديلات:**")
        lines.append(f"• التكلفة: {'تنخفض' if impact['cost_change_pct'] <= 0 else 'ترتفع'} بـ {abs(impact['cost_change_pct'])}%")
        lines.append(f"• الإنتاجية: {'تنخفض' if impact['yield_change_pct'] <= 0 else 'ترتفع'} بـ {abs(impact['yield_change_pct'])}%")
        lines.append(f"• السلامة: تحسن من {impact['safety_score_original']}/10 إلى {impact['safety_score_safe']}/10")
    ...
    def _generate_english_summary(self, changes: List[Dict], impact: Dict) -> str:
        ...
        lines.append(f"• Cost: {'decreases' if impact['cost_change_pct'] <= 0 else 'increases'} by {abs(impact['cost_change_pct'])}%")
        lines.append(f"• Yield: {'decreases' if impact['yield_change_pct'] <= 0 else 'increases'} by {abs(impact['yield_change_pct'])}%")
        lines.append(f"• Safety: improves from {impact['safety_score_original']}/10 to {impact['safety_score_safe']}/10")
