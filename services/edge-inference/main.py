...
_pest_detector: Optional[EdgePestDetector] = None
_yield_estimator: Optional[EdgeYieldEstimator] = None
...
def get_pest_detector() -> EdgePestDetector:
    ...
    if _pest_detector is None:
        _pest_detector = EdgePestDetector(
            model_path=f"{MODEL_CACHE_DIR}/pest_detector_quantized.onnx",
            device=DEVICE
        )
...
def get_yield_estimator() -> EdgeYieldEstimator:
    ...
    if _yield_estimator is None:
        _yield_estimator = EdgeYieldEstimator(
            model_path=f"{MODEL_CACHE_DIR}/yield_estimator_quantized.onnx",
            device=DEVICE
        )
...
class PestDetectionRequest(BaseModel):
    ...
    crop: str = Field(default="wheat", pattern="^(wheat|barley|maize|sorghum|millet|rice|potato|tomato|coffee)$")
...
class YieldEstimationRequest(BaseModel):
    ...
    growth_stage: str = Field(default="vegetative", pattern="^(seedling|vegetative|flowering|grain_filling|maturity)$")
...
async def health_check() -> EdgeHealthCheck:
    ...
    return EdgeHealthCheck(
        device=DEVICE, offline_mode=OFFLINE_MODE,
        models_loaded={"pest_detector": _pest_detector is not None, "yield_estimator": _yield_estimator is not None},
...
async def detect_pest(
    ...
    request = PestDetectionRequest(field_id=field_id, crop=crop, confidence_threshold=confidence_threshold, return_image=return_image)
    ...
    start = time.time()
    try:
        detections = detector.predict(image_bytes, confidence_threshold=request.confidence_threshold)
    except Exception:
        # صورة تالفة/غير صالحة → 400 واضح بدل 500
        raise HTTPException(400, "تعذّر معالجة الصورة — تأكّد أنّها صورة صالحة")
    inference_time_ms = (time.time() - start) * 1000
    result = {
        "field_id": request.field_id, "crop": request.crop,
        "detections": detections, "inference_time_ms": round(inference_time_ms, 2),
        "device": DEVICE, "timestamp": datetime.now(timezone.utc).isoformat(),
...
async def estimate_yield(
    ...
    _require_service_token(x_agent_token)
    request = YieldEstimationRequest(field_id=field_id, crop=crop, image_count=image_count, growth_stage=growth_stage)
    estimator = get_yield_estimator()
    if len(files) < request.image_count:
        raise HTTPException(status_code=400, detail=f"Expected {request.image_count} images, got {len(files)}")
    all_features = []
    ...
    for file in files[:request.image_count]:
        ...
        try:
            ...
            raise HTTPException(400, "تعذّر معالجة إحدى الصور — تأكّد أنّها صور صالحة")
        all_features.append(features)
    start = time.time()
    yield_prediction = estimator.predict_yield(features=all_features, crop=request.crop, growth_stage=request.growth_stage)
    inference_time_ms = (time.time() - start) * 1000
    result = {
        "field_id": request.field_id, "crop": request.crop, "growth_stage": request.growth_stage,
        "sample_images": len(all_features), "estimated_yield_kg_ha": round(yield_prediction["yield_kg_ha"], 2),
        "confidence_interval": {"lower": round(yield_prediction["yield_kg_ha"] * 0.85, 2), "upper": round(yield_prediction["yield_kg_ha"] * 1.15, 2)},
        "biomass_proxy": round(yield_prediction.get("biomass_proxy", 0), 2),
        "plant_count_estimate": int(yield_prediction.get("plant_count", 0)),
        "inference_time_ms": round(inference_time_ms, 2), "device": DEVICE,
...
async def trigger_sync(x_agent_token: str = Header(None)):
    _require_service_token(x_agent_token)
    if OFFLINE_MODE:
        return {"status": "offline", "message": "Cannot sync in offline mode. Results queued locally."}
    sync = get_sync_service()
    synced_count = await sync.process_queue()
    return {"status": "success", "synced_items": synced_count, "remaining_queue": sync.queue_size(), "timestamp": datetime.now(timezone.utc).isoformat()}
