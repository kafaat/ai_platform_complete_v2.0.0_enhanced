...
from open_meteo import fetch_current, fetch_forecast, fetch_historical, fetch_tile_sample
...
async def current_weather(
    ...
    try:
        return await fetch_current(lat, lon, model=model)
...
async def forecast_weather(
    ...
    try:
        return await fetch_forecast(lat, lon, days=days, model=model)
    except Exception as exc:
...
async def historical_weather(
    ...
    try:
        return await fetch_historical(lat, lon, start_date=start_date, end_date=end_date)
    except Exception as exc:
...
async def _cached_sample(lat: float, lon: float, time: str, model: str, key_prefix: str = "sample"):
    ...
    if state != "fresh":
        try:
            sample = await fetch_tile_sample(lat, lon, time_key=time, model=model)
...
@app.get("/v1/weather/operation-tile-data/{z}/{x}/{y}")
...
@app.get("/v1/weather/tile-series/{z}/{x}/{y}")
...
@app.get("/v1/weather/tile-cache/stats")
