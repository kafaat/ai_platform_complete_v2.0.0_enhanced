...
DEFAULT_SERIES_HOURS = (0, 1, 3, 6, 12, 24, 48)
...
def parse_series_hours(hours: str) -> list[int]:
    """إزاحاتُ الساعات المطلوبة: **مدًى محدودٌ لا قائمةُ سماحٍ مغلقة**.

    **العطلُ الذي عالجه هذا التغيير:** كانت القائمةُ مغلقةً على
    ``{0,1,3,6,12,24,48}``، فكلُّ إزاحةٍ أبعد **تُرمى صامتةً**. والمنصّةُ تُعلن
    ``horizon_hours`` حتّى ١٦٨ وتُرسِل السلّم الممدَّد ``…,72,96,120,144,168``
    — فيصل منه سبعةٌ ويُرمى خمسة. النتيجةُ المقيسة: ``horizon_hours=168``
    يُرجِع **النوافذَ نفسَها** التي يُرجِعها ``48``، والفرقُ لا يظهر في أيّ
    رسالةٍ ولا رمزِ خروج.

    **ولمَ لم يُقصَّ بدل الرمي؟** القصُّ (``min(max, h)``) يُنتِج قيمةً لم
    يطلبها أحد ويُخفي أنّ الطلبَ كان خارج المدى — فيُبدِّل كذباً بكذب. أمّا
    الرميُ فيبقى صادقاً **بشرط أن يُعلَن**، ولذلك تُرجِع الدالّةُ ما عُوين فعلاً
    ويُعلنه مُستدعوها في أجسامهم.

    والحدودُ الثلاثة تُبقي الكلفةَ مقيَّدة: مدًى ``0..MAX_SERIES_HOUR`` · بلا
    تكرارٍ (مع حفظ الترتيب) · وسقفُ ``MAX_SERIES_FRAMES`` إطاراً.
    """
    values: list[int] = []
    for part in (hours or "").split(","):
        try:
            ...
            continue
        if 0 <= h <= MAX_SERIES_HOUR:
            values.append(h)
    if not values:
        return list(DEFAULT_SERIES_HOURS)
    # إزالةُ التكرار مع حفظ الترتيب، ثمّ سقفُ الإطارات.
    return list(dict.fromkeys(values))[:MAX_SERIES_FRAMES]
