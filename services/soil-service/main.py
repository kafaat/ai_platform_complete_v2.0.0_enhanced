...
class SoilReading(BaseModel):
    ...
    ec_level: Optional[float] = Field(None, ge=0, le=50)
...
async def ingest_reading(reading: SoilReading, x_agent_token: str = Header(None)):
    ...
    async with _pool.acquire() as conn:
        ...
        """,
        reading.field_id, reading.sensor_id,
        reading.temperature, reading.moisture_pct,
        reading.ph_level, reading.ec_level,
        reading.n_ppm, reading.p_ppm, reading.k_ppm,
