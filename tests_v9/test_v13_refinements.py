...
def test_confidence_aggregation():
    ...
    from api.confidence_aggregation import (
        aggregate, ConfidenceInput, AggregatedConfidence,
        irrigation_confidence, fertilizer_confidence, yield_prediction_confidence,
    )
...
def test_retry_hints():
    ...
    from api.failure_modes import (
        detect, RetryPolicy, FailureCategory, FAILURE_CATALOG,
    )
    ...
    failures_without_retry = [
        code for code, f in FAILURE_CATALOG.items()
        if f.retry is None
    ]
    ...
    if (f.failure_mode.retry and 
        f.failure_mode.retry.policy == RetryPolicy.EXP_BACKOFF and
        f.failure_mode.retry.max_attempts == 5):
    ...
    if (f.failure_mode.retry and 
        f.failure_mode.retry.policy == RetryPolicy.WAIT_NEXT_CYCLE):
        results.append(("✓", "cloud high → WAIT_NEXT_CYCLE"))
    
    # Invalid polygon → MANUAL_INTERVENTION
    f = detect("field.polygon_invalid")
    if (f.failure_mode.retry and
        f.failure_mode.retry.policy == RetryPolicy.MANUAL_INTERVENTION):
    ...
    if (f.failure_mode.retry and
        f.failure_mode.retry.initial_delay_sec == 10 and
        f.failure_mode.retry.max_attempts == 10):
