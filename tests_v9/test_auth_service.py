...
class TestJWTStructure:
    ...
    def test_token_has_required_claims(self):
        token = make_token()
        payload = jwt.decode(token, TEST_JWT_SECRET,
                             algorithms=["HS256"], audience="sahool")
        for claim in ["sub","email","role","tenant_id","jti","iss","aud","exp","iat","nbf"]:
            assert claim in payload, f"Missing claim: {claim}"

    @pytest.mark.unit
    def test_token_iss_is_sahool(self):
        payload = jwt.decode(make_token(), TEST_JWT_SECRET,
                             algorithms=["HS256"], audience="sahool")
    ...
    def test_expired_token_rejected(self, expired_token):
        ...
        with pytest.raises(JWTError):
            jwt.decode(expired_token, TEST_JWT_SECRET,
                       algorithms=["HS256"], audience="sahool")

    @pytest.mark.unit
    def test_admin_role_in_token(self, admin_token):
        payload = jwt.decode(admin_token, TEST_JWT_SECRET,
                             algorithms=["HS256"], audience="sahool")
    ...
    def test_wrong_audience_rejected(self):
        ...
        with pytest.raises(Exception):
            jwt.decode(token, TEST_JWT_SECRET,
                       algorithms=["HS256"], audience="wrong_audience")
    ...
    def test_tampered_token_rejected(self):
        ...
        with pytest.raises(Exception):
            jwt.decode(tampered, TEST_JWT_SECRET,
                       algorithms=["HS256"], audience="sahool")
...
class TestAuthEndpoints:
    ...
    async def test_login_wrong_password(self, http_client):
        from conftest import service_urls
        resp = await http_client.post(f"{service_urls['auth']}/auth/login",
