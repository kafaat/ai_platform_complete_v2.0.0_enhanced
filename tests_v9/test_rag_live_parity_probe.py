...
def mod():
    s = importlib.util.spec_from_file_location("ragprobe", P)
...
def test_live_probe_is_read_only_and_dimension_bound(monkeypatch):
    ...
    def post(url, payload, headers=None):
        ...
        if "/points/search" in url:
            return {"result": [{"payload": {"page_content": "same text"}}]}
...
def test_live_probe_rejects_vector_dimension_mismatch(monkeypatch):
    m = mod()
    monkeypatch.setattr(
        m, "_get", lambda *_a, **_k: {"result": {"config": {"params": {"vectors": {"size": 4}}}}}
    )
    monkeypatch.setattr(m, "_post", lambda *_a, **_k: {"embedding": [1, 2, 3]})
