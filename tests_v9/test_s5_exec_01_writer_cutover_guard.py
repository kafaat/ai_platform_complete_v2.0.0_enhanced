...
def _sandbox(tmp_path: Path, monkeypatch):
    freeze = json.loads((ROOT / "docs/architecture/s5_exec_01_edge_freeze.json").read_text())
    (tmp_path / "docs/architecture").mkdir(parents=True)
    (tmp_path / "docs/architecture/s5_exec_01_edge_freeze.json").write_text(json.dumps(freeze))
...
def test_new_frozen_writer_without_contract_fails_closed(tmp_path, monkeypatch):
    ...
    freeze["writer_cutover_set_runtime_only"][0]["writers"].append("services/sahool-platform/api/new_writer.py")
    (tmp_path / "docs/architecture/s5_exec_01_edge_freeze.json").write_text(json.dumps(freeze))
...
def test_removing_strict_mode_marker_is_blocked(tmp_path, monkeypatch):
    ...
    p = tmp_path / rel
    p.write_text(p.read_text().replace("if mode.strict_decision_service_required:", "if False:", 1))
...
def test_dispatch_writer_must_be_retired_in_strict_mode(tmp_path, monkeypatch):
    ...
    p = tmp_path / rel
    p.write_text(p.read_text().replace("legacy_dispatch_writer_retired_after_decision_sor_cutover", "legacy_dispatch_still_live", 1))
