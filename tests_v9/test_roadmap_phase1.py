...
def test_timeline_assembly():
    from api.field_timeline import assemble_timeline, TimelineCategory
    results = []
    events = [
        {"event_type": "field.created", "occurred_at": "2026-01-01T08:00:00+00:00", "payload": {"area_ha": 4.0}},
        {"event_type": "lifecycle.transitioned", "occurred_at": "2026-02-01T06:00:00+00:00", "payload": {"to_stage": "PLANTED"}},
        {"event_type": "operation.irrigation.completed", "occurred_at": "2026-02-05T18:00:00+00:00", "payload": {"water_m3": 320}},
        {"event_type": "scouting.pin.created", "occurred_at": "2026-03-10T10:00:00+00:00", "payload": {"issue": "tuta"}},
        {"event_type": "trueup.apply", "occurred_at": "2026-06-01T12:00:00+00:00", "payload": {"k_new": 1.05}},
    ]
...
def test_timeline_filter():
    ...
    events = [
        ...
        {"event_type": "operation.harvest.completed", "occurred_at": "2026-06-01T08:00:00+00:00", "payload": {}},
        {"event_type": "scouting.pin.created", "occurred_at": "2026-03-01T08:00:00+00:00", "payload": {}},
    ]
...
def test_pin_creation_valid():
    ...
    results = []
    p = make_pin("pin-1", "fld-1", 16.0, 45.0, "pest", "high", "new", "seasonal",
                 crop="tomato", issue_code="tomato.tuta", note_ar="إصابة")
    d = p.to_dict()
...
def test_pin_rejects_bad_issue_code():
    ...
    results = []
    r = validate_pin(16.0, 45.0, "pest", "high", "new", "seasonal",
                     crop="tomato", issue_code="wheat.rust")
    if not r.valid:
...
def test_pin_taxonomy():
    from api.scouting_pins import get_crop_issues, YEMEN_CROP_ISSUES, NUTRIENT_DEFICIENCY_GUIDE
    results = []
...
def test_manual_broadcast():
    from api.manual_converter import convert_zone, ApplicationMethod, EquipmentSpec
    results = []
    d = convert_zone("z1", 120, 0.5, ApplicationMethod.BROADCAST_TERRACE,
...
def test_walk_plan_order():
    ...
    plan = generate_walk_plan("fld-1", "wheat", zones, ApplicationMethod.BROADCAST_TERRACE,
                              EquipmentSpec(terrace_area_m2=200, cap_weight_kg=0.5))
    d = plan.to_dict()
...
def test_manual_converter():
    from api.manual_converter import convert_zone, ApplicationMethod, EquipmentSpec, kg_per_terrace
    results = []
    # تحويل أساسي
    if abs(kg_per_terrace(120, 200) - 2.4) < 1e-9:
    ...
    if abs(d.terraces_count - 25) < 1e-6 and abs(d.kg_per_terrace - 2.4) < 1e-6:
        results.append(("✓", f"نثر: 25 مصطبة، 2.4كغ/مصطبة"))
    # رفض معدّات ناقصة
    try:
...
def test_deterministic_randomization():
    from api.trial_randomization import (
        generate_block_assignments, verify_assignment, RandomizationConfig,
    )
    results = []
