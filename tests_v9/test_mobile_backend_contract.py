...
MOBILE_CALLS = [
    "/api/v1/auth/login", "/api/v1/auth/signup", "/api/v1/auth/me", "/api/v1/auth/logout",
    "/api/v1/me", "/api/v1/observations", "/api/v1/queue/status", "/api/v1/recommendations",
    "/api/v1/sync", "/api/v1/weather/current", "/api/v1/weather/forecast",
    "/api/v1/weather/historical", "/api/v1/fields",
    # raster-service (منفذ منفصل)
    "/upload/drone", "/upload/raster", "/process", "/jobs/{id}", "/jobs/{id}/result", "/info/{id}",
]
...
def _extract_backend_endpoints():
    ...
    paths = set()
    main_py = os.path.join(os.path.dirname(__file__),
                           '../services/sahool-platform/api/main.py')
    raster_py = os.path.join(os.path.dirname(__file__),
                             '../../sahool-raster-service/app/api/endpoints.py')
    for fp, decorators in [(main_py, ['@app']), (raster_py, ['@router', '@app'])]:
...
def test_auth_paths_have_api_v1_prefix():
    ...
    results = []
    authsvc = os.path.join(os.path.dirname(__file__),
                           '../../sahool_mobile/src/api/authService.ts')
