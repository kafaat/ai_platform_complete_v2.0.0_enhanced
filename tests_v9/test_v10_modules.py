...
def test_lifecycle_state_machine():
    ...
    from api.field_lifecycle import (
        LifecycleStage, is_valid_transition, VALID_TRANSITIONS,
    )
...
def test_prescriptions():
    ...
    from api.prescriptions import (
        ...
        PrescriptionType, prescription_to_csv, prescription_to_dict,
    )
    ...
    zones = [
        ZoneCharacteristics(
            zone_id="z1", zone_class=ZoneClass.HIGH,
            area_ha=1.5, ndvi_mean=0.72, soil_ph=7.4,
            soil_om=2.5, soil_n_ppm=18, soil_texture="loamy",
            soil_depth_cm=50,
        ),
        ZoneCharacteristics(
            zone_id="z2", zone_class=ZoneClass.MEDIUM,
            area_ha=1.8, ndvi_mean=0.52, soil_ph=7.8,
            soil_om=1.5, soil_n_ppm=10, soil_texture="loamy",
            soil_depth_cm=45,
        ),
        ZoneCharacteristics(
            zone_id="z3", zone_class=ZoneClass.PROBLEM,
            area_ha=0.9, ndvi_mean=0.35, soil_ph=8.2,
            soil_ec=5.2, soil_n_ppm=8, soil_texture="sandy",
            soil_depth_cm=30,
        ),
    ]
    ...
    seed_rx = gen.generate_seeding(
        field_id="fld-test", season_id="s",
        crop="wheat", zones=zones,
    )
...
def test_yield_heuristics():
    ...
    from api.yield_heuristics import (
        LifecycleFeatures, StressLevel,
        estimate_yield, detect_anomalies,
    ...
    if est_s.yield_score < 0.70 and est_s.stress_level in (StressLevel.HIGH, StressLevel.CRITICAL):
        results.append(("✓", f"stressed field: yield_score={est_s.yield_score}, stress={est_s.stress_level.value}"))
    else:
        results.append(("✗", f"stressed expected <0.70/high, got {est_s.yield_score}/{est_s.stress_level.value}"))
    ...
    events = [
        {"event_type": "lifecycle.transition", "timestamp": "2026-02-01T00:00:00+00:00",
        ...
        {"event_type": "lifecycle.transition", "timestamp": "2026-05-01T00:00:00+00:00",
    ...
    if f.irrigation_count == 2 and f.moisture_stress_events == 1 and f.days_in_growing > 85:
        ...
        results.append(("✗", f"feature extraction: irr={f.irrigation_count}, stress={f.moisture_stress_events}, days={f.days_in_growing}"))
...
def test_reports_csv():
    ...
    fields = [
        FieldReport(
            field_id="fld-1", field_name_ar="حقل الشمال",
            farm_id="frm-1", tenant_id="ten-1",
            area_ha=4.2, crop="wheat", season_label="شتاء 2026",
            planting_date="2026-02-15", harvest_date=None,
            lifecycle_stage="GROWING",
            irrigation_events=5, total_water_m3=420.0,
            fertilizer_events=2, total_nitrogen_kg=80.0,
            avg_ndvi=0.68,
            estimated_yield_kg_ha=2400, estimated_yield_total_kg=10080,
            soil_samples_count=2, last_soil_ph=7.4,
            anomalies=["تنبيه ري"],
        ),
        FieldReport(
            field_id="fld-2", field_name_ar="قطاع المحور",
            farm_id="frm-1", tenant_id="ten-1",
            area_ha=2.8, crop="corn", season_label="شتاء 2026",
            planting_date="2026-03-01", harvest_date=None,
            lifecycle_stage="PLANTED",
            irrigation_events=2, total_water_m3=180.0,
        ),
    ]
