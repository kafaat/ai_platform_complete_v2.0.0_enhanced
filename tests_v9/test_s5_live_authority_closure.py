...
def test_three_canonical_guards_are_required_before_adjudication(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mod, "_guard", lambda *a, **kw: {"passed": True, "returncode": 0, "output": "ok"})
...
def test_preflight_binds_collection_checkout_to_exact_subject(monkeypatch):
    required = (
        "DECISION_SOR_PLATFORM_URL", "DECISION_SOR_SERVICE_URL",
        "DECISION_SOR_ADMIN_DATABASE_URL", "DECISION_SOR_PLATFORM_ROLE",
        "DATABASE_URL", "SAHOOL_AGENT_TOKEN", "FIELD_SERVICE_URL",
        "TENANT_A", "TENANT_B", "FIELD_A", "KG_SERVICE_URL", "KG_TENANT_ID",
    )
