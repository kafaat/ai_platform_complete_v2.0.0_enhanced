...
def test_manual_operations_panel_is_mounted_in_field_workspace():
    panel = (ROOT / "frontend/src/sections/FieldWorkspaceIrrigationPanel.tsx").read_text(encoding="utf-8")
...
def test_ui_preserves_legal_lifecycle_distinctions():
    text = (ROOT / "frontend/src/sections/IrrigationManualOperationsPanel.tsx").read_text(encoding="utf-8")
    for label in (
        "اعتماد",
        "بدء الري",
        "إيقاف الري",
