...
استثناء واحد موثَّق (allowlist): ``zlmediakit/zlmediakit``. الصورة الرسميّة لا تنشر أيّ
وسم إصدار (semver/تاريخ) إطلاقاً — فقط وسوم فروع متحرّكة (master/master_py/feature-*)،
وتوصي وثائق المنبع صراحةً بـ``:master``؛ ومشروع GitHub لا يُصدر releases. التثبيت الحقيقيّ
هو مِعرِّف ``sha256`` يُحقن عبر ``ZLMEDIAKIT_IMAGE`` في خطّ الإصدار (release pipeline) —
غير قابل للحلّ في هذه البيئة (لا سحب من السجلّ). لا نبدّل إلى نسخة مجتمعيّة متقادمة (مثل
``panjjo/zlmediakit`` التي أحدث وسمها 1.2.x، ووسم ``v6.0.0`` المُشار إليه في
``docker-compose.unified.yml`` يُعيد 404) لأنّها تُخاطر ببوّابة الفيديو الحيّة وتخالف
تخطيط ``/opt/media/conf`` الذي يعتمده entrypoint الإعداد.
...
_ALLOWLIST: dict[str, str] = {
    "zlmediakit/zlmediakit": (
        "الصورة الرسميّة لا تنشر أيّ وسم إصدار — فقط وسوم فروع متحرّكة؛ التثبيت الحقيقيّ "
        "sha256 عبر ZLMEDIAKIT_IMAGE في خطّ الإصدار (غير قابل للحلّ في هذه البيئة)."
    ),
}
...
def _effective_default(image_value: str) -> str | None:
    ...
    بلا افتراض (لا مِعرِّف ثابت داخل الملفّ ⇒ تُعامَل كمخالفة).
    """
