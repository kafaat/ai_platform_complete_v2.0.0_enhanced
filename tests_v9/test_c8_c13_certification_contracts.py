...
def test_c8_live_parity_receipt_does_not_overclaim_cutover(tmp_path):
    import hashlib
    ...
    path = tmp_path / "rag-receipt.json"
    path.write_text(json.dumps(receipt), encoding="utf-8")
    rc, out = run("C8", "--receipt", str(path), "--subject-sha", subject)
    assert rc == 0
    assert out["status"] == "LIVE_PARITY_VERIFIED"
