...
def test_a_tokens_lifetime_equals_its_declared_window_exactly():
    ...
    **ولمَ الربطُ داخل الحرفيّ لا سطرُ إسنادٍ قبله:** ``main.py`` عند سقف راتشِت
    التفكيك **تماماً** (2553/2553، هامش صفر)، فأيّ سطرٍ مُضاف يحجب. والمخرجان
    الآخران مرفوضان: رفعُ السقف يُبطِل الراتشِت، وحذفُ تعليلٍ قائم ليتّسع المكان هو
    بعينه ما تُسمّيه طفرةُ ``visual_fixme_baseline_guard`` المُسجَّلة — «عدٌّ يُعاقِب
    التوثيق يُدرِّب كاتبه على حذفه». فالتعليل هنا حيث لا يُعاقِبه عدّاد، والشيفرة
    محايدة الأسطر. وترتيبُ التقييم في ``dict`` مضمونٌ يساراً-يميناً — **قِيس** لا
    افتُرِض: ``{"a": (n := 0), "b": n + 100}`` يُعطي ``b == 100``.
    """
