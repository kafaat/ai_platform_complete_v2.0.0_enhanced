...
PUBLIC_ALLOWLIST: set[str] = {
    ...
    # ── مراقبة تشغيليّة (صحّة الأتمتة — شبيهة بـ/metrics، لا بيانات مستأجِر) ──
    # TODO(security): مرشّحة للمصادقة لاحقاً — تكشف صحّة الأتمتة فقط (لا بيانات
    # مستأجِر)؛ أُبقيت عامّة الآن اتّساقاً مع نيّة المراقبة. انظر التقرير.
    "/api/v1/automation/scheduler-status",  # حالة المُجدوِل — مراقبة تشغيليّة.
    "/api/v1/automation/imagery/status",  # حالة سحب الصور — مراقبة تشغيليّة.
}
