...
def test_a_conflicted_manifest_is_named_not_left_to_guesswork():
    """`CONFLICTED-MANIFEST-DISABLES-THE-LAYER-IT-FEEDS-01` — السببُ يُسمَّى.

    بيانُ أهداف الكتابة قد يكون **هو نفسُه** بين المتعارضات. عندئذٍ يحمل مراسمَ
    تعارضٍ فلا يُحلَّل، فتُرجِع `_measured_write_targets()` مجموعةً فارغة، فيتدهور
    كلُّ ملفٍّ إلى «مصدر» ويقف المُصنِّفُ حيث كان يحلّ آليّاً.

    والتدهورُ **آمنٌ بالتصميم** («مصدر» تعني قف لا اكتب فوق عمل) — فالعطلُ عطلُ
    **إبانةٍ لا صحّة**: الأداةُ تفقد قدرتَها صامتةً بلا أن تقول لماذا، فيقرأ
    المُشغِّلُ «مصادر» ويظنّها مصادرَ حقّاً.

    مقيسٌ على دمجٍ حقيقيّ لا مِسبار: بالبيان المتعارض وقف المُصنِّفُ عند ثلاثة
    ملفّات، وبحلّ البيان **وحدَه** بلا أيّ تغييرٍ آخر انحصر الوقوفُ في وثيقة
    السياسة — فالفارقُ من البيان لا من المحتوى.
    """
    assert rmc._manifest_is_conflicted([rmc.MANIFEST_REL]), (
        "البيانُ بين المتعارضات ولم يُكتشَف — يعود التدهورُ مجهولَ السبب"
    )
    assert not rmc._manifest_is_conflicted(["docs/architecture/other.json"]), (
        "تعارضٌ لا يشمل البيان ولا ينبغي أن يُنسَب إليه"
    )
    # مرساةٌ على المسار المُطبَّع لا على `substring`: مسارٌ ينتهي بالاسم نفسِه في
    # مجلّدٍ آخر ليس بياننا، ومطابقتُه تُنتِج رسالةً تكذب على قارئها.
    assert not rmc._manifest_is_conflicted(
        ["vendor/docs/architecture/generated_write_targets.json"]
    ), "طابَق مساراً آخر يشترك في الذيل — المرساةُ نصّيّة لا مسارٌ مُطبَّع"
