...
def test_device_identity_is_persistent_and_not_deleted_on_logout():
    device = (MOBILE / "services" / "device_id_service.dart").read_text(encoding="utf-8")
    auth = (MOBILE / "services" / "auth_service.dart").read_text(encoding="utf-8")
...
def test_submission_sends_device_header_and_coordinator_uses_shared_dio():
    api = (MOBILE / "features/field_forms/data/field_forms_api.dart").read_text(encoding="utf-8")
    coord = (MOBILE / "features/field_forms/data/field_forms_coordinator.dart").read_text(
        encoding="utf-8"
    )
    service = (MOBILE / "services/api_service.dart").read_text(encoding="utf-8")
...
def test_authenticated_startup_and_field_open_are_wired():
    main = (MOBILE / "main.dart").read_text(encoding="utf-8")
    workspace = (MOBILE / "screens/field_workspace_screen.dart").read_text(encoding="utf-8")
