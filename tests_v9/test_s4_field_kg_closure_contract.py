...
def test_field_live_gate_proves_restricted_role_and_role_closure():
    s=(ROOT/'scripts/staging/field_management_live_gate.sh').read_text()
    for token in ('rolsuper','rolbypassrls','rolcreatedb','rolcreaterole','pg_auth_members','reachable_privileged_role_count','owner_or_superuser_proof_accepted'):
        assert token in s
...
def test_kg_freeze_is_deterministic_and_has_real_consumers():
    ...
    spec=importlib.util.spec_from_file_location('kgfreeze',p); assert spec and spec.loader
    m=importlib.util.module_from_spec(spec); sys.modules[spec.name]=m; spec.loader.exec_module(m)
    d=m.document(); assert d['legacy_platform_store_absent'] is True
...
def test_kg_runtime_collector_is_read_only_and_subject_bound():
    s=(ROOT/'scripts/staging/kg_runtime_parity_collector.py').read_text()
...
def test_s5_dead_farmonaut_provider_identity_is_retired():
    ...
    policy=json.loads((ROOT/'docs/architecture/platform_shrink_ratchet.json').read_text())
    assert 'services/sahool-platform/core/connectors/farmonaut.py' not in policy['baseline']['platform_provider_clients']
...
def test_s5_platform_cdse_duplicate_is_retired():
    ...
    policy=json.loads((ROOT/'docs/architecture/platform_shrink_ratchet.json').read_text())
    assert 'services/sahool-platform/core/connectors/copernicus.py' not in policy['baseline']['platform_provider_clients']
