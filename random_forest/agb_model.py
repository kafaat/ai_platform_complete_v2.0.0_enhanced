...
class SAHOOLAGBModel:
    ...
    EQ4_COEFFICIENTS = {
        "قمح صلب":   {"intercept": -12.0, "ndvi": 125.0, "evi": 45.0, "gndvi": 38.0, "vv": 2.1, "vh": 1.8, "hi": 0.40},
        "شعير":       {"intercept": -10.0, "ndvi": 118.0, "evi": 42.0, "gndvi": 35.0, "vv": 2.0, "vh": 1.7, "hi": 0.38},
        "ذرة صفراء": {"intercept": -20.0, "ndvi": 210.0, "evi": 75.0, "gndvi": 65.0, "vv": 3.5, "vh": 2.8, "hi": 0.45},
        "طماطم":     {"intercept": -15.0, "ndvi": 180.0, "evi": 60.0, "gndvi": 55.0, "vv": 2.8, "vh": 2.2, "hi": 0.70},
        "بطاطس":     {"intercept": -14.0, "ndvi": 160.0, "evi": 55.0, "gndvi": 45.0, "vv": 2.5, "vh": 2.0, "hi": 0.75},
        "خضروات":    {"intercept": -12.0, "ndvi": 145.0, "evi": 50.0, "gndvi": 42.0, "vv": 2.3, "vh": 1.9, "hi": 0.65},
    }
    ...
    def _equation4(self, f: AGBFeatures) -> float:
        ...
        sar_correction = 0.01 * (f.vv_backscatter + 20) * c["vv"] + \
                         0.008 * (f.vh_backscatter + 25) * c["vh"]
    ...
    def predict(self, f: AGBFeatures) -> dict:
        ...
        if self._trained and HAS_SKLEARN and self._rf_model:
            ...
            X = np.array([[
                ...
                f.chm_mean, f.point_density, f.canopy_cover
            ]])
    ...
    def train_synthetic(self, n_samples: int = 500):
        ...
        for _ in range(n_samples):
            ...
            feat = AGBFeatures(ndvi=ndvi, evi=evi, gndvi=gndvi, savi=savi,
                               vv_backscatter=vv, vh_backscatter=vh,
                               chm_mean=chm, point_density=dens,
                               canopy_cover=cover, crop=crop)
...
if __name__ == "__main__":
    ...
    test_cases = [
        ("field_01", "قمح صلب",  AGBFeatures(ndvi=0.72, evi=0.61, gndvi=0.68, savi=0.65, area_ha=23.5, crop="قمح صلب")),
        ("field_02", "شعير",      AGBFeatures(ndvi=0.58, evi=0.49, gndvi=0.55, savi=0.52, area_ha=32.0, crop="شعير")),
        ("field_03", "ذرة صفراء",AGBFeatures(ndvi=0.44, evi=0.37, gndvi=0.41, savi=0.40, area_ha=18.7, crop="ذرة صفراء")),
        ("field_04", "طماطم",    AGBFeatures(ndvi=0.66, evi=0.56, gndvi=0.63, savi=0.60, area_ha=41.3, crop="طماطم")),
        ("field_05", "قمح صلب",  AGBFeatures(ndvi=0.74, evi=0.63, gndvi=0.70, savi=0.67, area_ha=28.9, crop="قمح صلب")),
    ]
